...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\google\protobuf\text_format.py
# Compiled at: 2018-04-17 20:38:14
# Size of source mod 2**32: 23913 bytes
        ...
        if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_ENUM:
            ...
            if field.cpp_type == descriptor.FieldDescriptor.CPPTYPE_STRING:
                ...
                if type(value) is str:
                    ...
                    out.write(_CEscape(value.decode('utf-8', errors='replace'), as_utf8))
                out.write('"')
            else:
...
class _Tokenizer(object):
    ...
    def NextToken(self):
        ...
        self._SkipWhitespace()
        if not self._lines:
            if len(self._current_line) <= self._column:
                self.token = ''
                return
        else:
