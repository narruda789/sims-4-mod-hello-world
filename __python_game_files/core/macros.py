...
# Compiled at: 2014-12-19 18:21:36
    ...
    def get_init_mapping(self, node):
        ...
        for statement in self._init.body:
            if isinstance(statement, ast.Pass):
                ...
                if not isinstance(target, ast.Attribute) and isinstance(target.value, ast.Name) or target.value.id != 'self':
                    raise TypeError('Class macro __init__ body may only assign to self')
                dest = '{}.{}'.format(target.value.id, target.attr)
                assert isinstance(statement.value, ast.Name), 'Class macro __init__ body may only assign from arguments'
                            ...
                            if symbol == '.':
                                for macro_name, macro_value in import_macros.items():
                                    if macro_value == MACRO_SYMBOL and name != 'macro':
                                        ...
                                        if isinstance(macro_value, MacroBase):
                                            if name != macro_value.from_module:
                                                continue
...
class ModuleSuiteImporter:
    ...
    def _resolve_relative(self, name, package=None, level=0):
        if package and not hasattr(package, 'rindex'):
