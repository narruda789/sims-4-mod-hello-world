...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\sims4\color.py
# Compiled at: 2018-12-17 21:12:54
# Size of source mod 2**32: 4239 bytes
import enum, math, colorsys, sims4.math
__all__ = [
 'Color', 'from_rgba', 'to_rgba', 'interpolate', 'pseudo_random_color',
 'red_green_lerp']
...
class Color(ColorARGB32, enum.EnumBase):
...
def from_rgba_as_int(r, g, b, a=MAX_INT_COLOR_VALUE):
    value = _convert_from_rbga_to_color(a, 16777216, as_int=True) + _convert_from_rbga_to_color(r, 65536, as_int=True) + _convert_from_rbga_to_color(g, 256, as_int=True) + _convert_from_rbga_to_color(b, as_int=True)
    return ColorARGB32(value)


def to_rgba(color):
    return (
     _convert_from_color_to_rgba(color, 65536),
     _convert_from_color_to_rgba(color, 256),
     _convert_from_color_to_rgba(color),
     _convert_from_color_to_rgba(color, 16777216))


def to_rgba_as_int(color):
    return (
     _convert_from_color_to_rgba(color, 65536, as_int=True),
     _convert_from_color_to_rgba(color, 256, as_int=True),
     _convert_from_color_to_rgba(color, as_int=True),
     _convert_from_color_to_rgba(color, 16777216))
...
def red_green_lerp(n, a=1.0, s=0.9, v=0.9):
    return from_rgba(*(colorsys.hsv_to_rgb)(n * 0.4, s, v), **{'a': a})