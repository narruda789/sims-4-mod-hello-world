...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\sims4\callback_utils.py
# Compiled at: 2017-08-28 19:43:11
# Size of source mod 2**32: 8664 bytes
import collections, contextlib, enum, sims4.log, sims4.reload, sims4.repr_utils
logger = sims4.log.Logger('callback_utils')
...
def add_callbacks(events, *callbacks):
    if sims4.reload.currently_reloading:
        return
    for event in events:
        sims4.log.assert_raise('Callback_Utils', (isinstance(event, CallbackEvent)), '{} is an instance of {}',
          event, (type(event)), owner='bhill')
        for callback in callbacks:
            sims4.log.assert_raise('Callback_Utils', (callback not in _callbacks[event]), '{} is a duplicate callback being added for event {}',
              callback,
              event, owner='bhill')
...
class CallableList(list):
    ...
    def register(self, callback):
        if callback in self:
            logger.warn('Attempted to register an already registered callback {}.', callback)
            return
        self.append(callback)

    def unregister(self, callback):
        if callback not in self:
            logger.warn('Attempted to unregister non-registered callback {}.', callback)
            return
