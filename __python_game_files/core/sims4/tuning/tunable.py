...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\sims4\tuning\tunable.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 139588 bytes
from _sims4_collections import frozendict
from re import template
import collections, inspect, random, types
...
import enum, paths, sims4.color, sims4.log, sims4.math, sims4.resources, sims4.tuning.instance_manager, sims4.tuning.instances
...
class Tunable(TunableBase):
    __slots__ = ('_type', '_default', '_raw_default', '_source_location', '_source_query',
                 '_source_sub_query', '_convert_defined_values')
    ...
    def load_etree_node(self, node, source, expect_error):
        ...
        if node.text is None:
            if self.default is None:
                if self._type in (int, float, BoolWrapper):
...
class TunableTuple(TunableBase):
    ...
    def add_member_desc(export_dict, name, val):
        ...
        if val.TAGNAME in export_dict.keys():
            ...
            export_dict[val.TAGNAME] = [
             sub_dict]


def function_has_only_optional_arguments(fn):
    full_arg_spec = inspect.getfullargspec(fn)
    if len(full_arg_spec.args) == len(full_arg_spec.defaults or ()):
        if len(full_arg_spec.kwonlyargs) == len(full_arg_spec.kwonlydefaults or ()):
            return True
    return False
...
class TunableFactory(TunableTuple):
        ...
        def __eq__(self, other):
            if not isinstance(other, TunableFactory.TunableFactoryWrapper):
                ...
                if not self.factory == other.factory:
                    ...
                    return super().__eq__(other) or False
                return self._tuned_values == other._tuned_values or False
            return True
    ...
    def _create_dict(self, items, locked_args):
        ...
        elif isinstance(factory, types.MethodType) and factory.__self__ is self:
            factory_args = inspect.getfullargspec(factory).args
            if factory_args:
                if factory_args[0] == 'self':
...
class TunableReferenceFactory(TunableFactory):
    ...
    def _process_factory_tunables(factory_type, factory_tunables):
        ...
        for name, tunable in dict(factory_tunables).items():
            if tunable is None:
                continue
            if name in instance_tunables:
                tunable = OptionalTunable(disabled_name='use_default', disabled_value=(AutoFactoryInit.AUTO_INIT_IGNORE_VALUE),
...
class TunableVariant(TunableTuple):
    ...
    def export_desc(self):
        ...
        for name in self.locked_args.keys():
            sub_dict = {Attributes.Name: name, 
             Attributes.DisplayName: get_default_display_name(name), 
             Attributes.Class: self.VARIANTNULLCLASS}
            if self.VARIANTNULLTAG in export_dict.keys():
                export_dict[self.VARIANTNULLTAG].append(sub_dict)
            else:
                export_dict[self.VARIANTNULLTAG] = [
                 sub_dict]
    ...
    def invoke_callback(self, instance_class, tunable_name, source, value):
        if not self._has_callback:
            ...
            if self._variant_map is not None:
...
class OptionalTunable(TunableVariant):
    ...
    def __init__(self, tunable, enabled_by_default=False, disabled_value=None, disabled_name='disabled', enabled_name='enabled', **kwargs):
        ...
        (super().__init__)(locked_args={disabled_name: disabled_value}, 
    ...
    def load_etree_node(self, node, source, expect_error):
        ...
        for child_node in node:
            ...
            try:
                ...
                if False and not self.allow_none:
                    if value is None:
        ...
        if self.minlength is not None:
            if len(tunable_collection) < self.minlength:
...
class TunableSet(_TunableCollection):
    ...
    def _get_collection_types(self):
        return (
         set, set.add, frozenset, lambda n: n is None)
    ...
    def _process_dict_value(self, value, tunable_name, source):
        if value is not None:
            ...
            if False and self.allow_none:
                ...
                dict_items = {getattr(item, key_name):getattr(item, value_name) for item in value if getattr(item, value_name) is not None}
...
class TunableEnumEntry(_TunableHasPackSafeMixin, Tunable):
    ...
    def export_desc(self):
        ...
        if self.invalid_enums:
            ...
            if hasattr(self._type, '_elements'):
                ...
                if hasattr(self._type, '_dynamic_entry_owner'):
    ...
    def _convert_to_value(self, content):
        if content is None:
            return
        if hasattr(self._type, '_dynamic_entry_owner'):
            if self._type._dynamic_entry_owner is not None:
...
class TunableReference(_TunableHasPackSafeMixin, Tunable):
    __slots__ = ('pack_safe', '_manager', '_class_restrictions', '_reload_dependent',
                 'cache_key', 'needs_deferring', '_allow_none')
...
class HasTunableReference:
    ...
    def _get_tunable_reference(cls, reference_type, *args, class_restrictions=DEFAULT, **kwargs):
        ...
        return reference_type(args, manager=cls.tuning_manager, 
...
class HasTunableReferenceFactory(AutoFactoryInit):
    ...
    def TunableReferenceFactory(cls, **extra_kwargs):
        if '_AUTO_REFERENCE_FACTORY' not in vars(cls):
            cls._AUTO_REFERENCE_FACTORY = (TunableReferenceFactory.create_auto_factory)(cls, manager=cls.tuning_manager, 
             class_restrictions=(
 cls,), **extra_kwargs)
...
class TunableResourceKeyReferenceBase(_TunableHasPackSafeMixin, Tunable):
    ...
    def load_etree_node(self, node, source, expect_error):
        ...
        if error is not None:
            ...
            if self.pack_safe and not self._does_resource_key_exist(value):
                raise UnavailablePackSafeResourceError
            return value
...
class TunableResourceKey(_TunableHasPackSafeMixin, Tunable):
    ...
    def __init__(self, default=None, resource_types=(), allow_none=False, **kwargs):
        ...
        self._allow_none = allow_none
        self.cache_key = '{}_{}'.format('TunableResourceKey', self.pack_safe)
    ...
    def load_etree_node(self, node, source, expect_error):
        value = super().load_etree_node(node, source, expect_error)
        if self.pack_safe:
            if self.validate_pack_safe:
                if not sims4.resources.does_key_exist(value):
                    raise UnavailablePackSafeResourceError
        return value
...
class TunableThreshold(TunableTuple):
    ...
    def __init__(self, description='Value/comparison pair used to define a Threshold.', value=None, default=None, **kwargs):
        ...
        (super().__init__)(value=value, comparison=TunableOperator((sims4.math.Operator.GREATER_OR_EQUAL), description='The comparison to perform against the value.'), 
...
class TunableInterval(TunableSingletonFactory):
    ...
    def __init__(self, tunable_type, default_lower, default_upper, minimum=None, maximum=None, description='A tunable interval between lower_bound and upper_bound.', **kwargs):
        if not issubclass(tunable_type, TunableBase):
            (super().__init__)(lower_bound=TunableRange(tunable_type, default=default_lower, minimum=minimum, description='The lower bound of the interval.'), upper_bound=TunableRange(tunable_type, default=default_upper, maximum=maximum, description='The upper bound of the interval.'), 
             description=description, **kwargs)
        else:
            (super().__init__)(lower_bound=tunable_type(default=default_lower, minimum=minimum, description='The lower bound of the interval.'), upper_bound=tunable_type(default=default_upper, maximum=maximum, description='The upper bound of the interval.'), 
             description=description, **kwargs)
...
class TunableIntervalLiteral(TunableSingletonFactory):
    ...
    def __init__(self, tunable_type, default, minimum=None, maximum=None, description='A literal value that is to be used as the lower \n                 and upper bound of an interval. This allows both literal and \n                 range interval tunings in the same TunableVariant without \n                 having to manually set the lower and upper bounds to the same\n                 value.', **kwargs):
        if not issubclass(tunable_type, TunableBase):
            (super().__init__)(value=TunableRange(tunable_type, minimum=minimum,
  maximum=maximum,
  default=default,
  description='The upper and lower bounds.'), 
             description=description, **kwargs)
        else:
            (super().__init__)(value=tunable_type(minimum=minimum, maximum=maximum,
  default=default,
  description='The upper and lower bounds'), 
             description=description, **kwargs)
...
class TunableLiteralOrRandomValue(TunableVariant):
    ...
    def __init__(self, tunable_type=int, default=10, minimum=0, maximum=None, description='A literal value or a random number within the specified range.', **kwargs):
        super().__init__(literal=TunableIntervalLiteral(tunable_type=tunable_type,
          default=default,
          minimum=minimum,
          maximum=maximum),
          random_in_range=TunableInterval(tunable_type,
          default, default, minimum=minimum,
          maximum=maximum),
          default='literal',
...
class TunableColor(TunableVariant):
    ...
    class TunableColorRGBA(TunableSingletonFactory):
        ...
        def __init__(self, description='A color.', **kwargs):
            (super().__init__)(r=TunableRange(int, 255, 0, 255, description='red value (0-255)'), 
             g=TunableRange(int, 255, 0, 255, description='green value (0-255)'), 
             b=TunableRange(int, 255, 0, 255, description='blue value (0-255)'), 
             a=TunableRange(int, 255, 0, 255, description='alpha value (0-255) (0 is transparent, 255 is opaque)'), 
             description=description, **kwargs)
    ...
    class TunableColorHex(TunableSingletonFactory):
        ...
        def __init__(self, description='A color.', **kwargs):
            (super().__init__)(hex_code=Tunable(str, '#FFFFFFFF', description="An ARGB color in hex, same as one would use in HTML. A leading '0x' or '#' is allowed but not required. You can omit the alpha, in which case opaque is assumed."), 
             description=description, **kwargs)

    def __init__(self, description='A color.', **kwargs):
        (super().__init__)(rgb=TunableColor.TunableColorRGBA(), 
         hex=TunableColor.TunableColorHex(), 
         name=TunableEnumEntry(sims4.color.Color, sims4.color.Color.WHITE), 
         description=description, **kwargs)
...
class TunableLocator(TunableResourceKeyReferenceBase):
    __slots__ = ()
    ...
    @property
    def resource_type(self):
        return sims4.resources.Types.LOCATOR