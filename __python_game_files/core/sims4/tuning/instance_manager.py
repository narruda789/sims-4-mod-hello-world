...
# Size of source mod 2**32: 35378 bytes
...
import argparse, os.path, paths, sims4.core_services, sims4.log, sims4.reload, sims4.resources, sims4.tuning.merged_tuning_manager, sims4.tuning.serialization, time
...
class TuningInstanceManager(Service):
    ...
    def _execute_gen(self, log_fn=None):
        if log_fn is None:
            ...
            if paths.TREE_DUMP_TUNING_SNAPSHOT_BEFORE_ZONE:
                from server_commands.memory_commands import py_tree_dump
                py_tree_dump(_connection=(sims4.commands.NO_CONTEXT))
            ...
            try:
                sims4.resources.release_preload_buffers()
...
class InstanceManager:
    ...
    def get_ordered_types(self, only_subclasses_of=None):

        def key(cls):
            result = tuple((x.__name__.lower() for x in reversed(cls.__mro__[:-1])))
            return (len(result), result)

        if only_subclasses_of is None:
            result = list(self.types.values())
        else:
    ...
    def register_tuned_class(self, instance, resource_key):
        if resource_key in self._tuned_classes:
            logger.info('Attempting to re-register class instance {} (Key:{}) with {}.', (self._tuned_classes[resource_key]),
              resource_key, self, owner='manus')
            return
    ...
    def create_class_instances(self):
        ...
        logger.info('Creating {:4} tuning class instances managed by {}.', (len(res_id_list)),
          self, owner='manus')
    ...
    def _create_class_instance(self, resource_key, group_id):
        ...
        try:
            ...
            logger.exception('An error occurred while attempting to create tuning instance: {}. Resource Key: {}.', cls,
              resource_key, owner='manus')

    def load_data_into_class_instances(self):
        logger.info('Loading {:4} tuning class instances managed by {}.', (len(self._tuned_classes)),
          self, owner='manus')
    ...
    def invoke_registered_callbacks_gen(self, registered_callbacks_timing_file=None):
        logger.info('Invoking callbacks for {:4} tuning class instances managed by {}.', (len(self._tuned_classes)),
          self, owner='manus')
    ...
    def _invoke_tunable_callbacks(self, cls, return_call_back_timing=False):
        ...
        for helper in tuning_callbacks:
            ...
            try:
                ...
                logger.exception('Exception in a tunable callback for variable {} in instance class {}.', (helper.name),
                  cls, owner='manus')
    ...
    def _invoke_verify_tunable_callbacks(self, cls):
        ...
        for helper in tuning_callbacks:
            try:
                ...
                logger.exception('Exception in a verify tunable callback for variable {} in instance class {}.', (helper.name),
                  cls, owner='manus')
    ...
    def export_descriptions(self, export_path, filter_fn=None):
        ...
        for cls in sorted((self._class_templates), key=(lambda cls: cls.__name__)):
            ...
            if filter_fn is None or filter_fn(cls):
