...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\unittest\case.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 58613 bytes
import sys, functools, difflib, logging, pprint, re, warnings, collections, contextlib, traceback
from . import result
    ...
    def __exit__(self, exc_type, exc_value, tb):
        ...
        for m in self.warnings:
            ...
            if self.expected_regex is not None:
                if not self.expected_regex.search(str(w)):
                    continue
            self.warning = w
...
class TestCase(object):
    ...
    def assertSequenceEqual(self, seq1, seq2, msg=None, seq_type=None):
        if seq_type is not None:
            ...
            raise (isinstance(seq2, seq_type) or self.failureException)('Second sequence is not a %s: %s' % (
        ...
        if differing is None:
            ...
            for i in range(min(len1, len2)):
                ...
                if len1 == len2:
                    if seq_type is None and type(seq1) != type(seq2):
                        return
