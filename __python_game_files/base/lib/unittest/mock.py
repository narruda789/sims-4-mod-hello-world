...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\unittest\mock.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 83056 bytes
__all__ = ('Mock', 'MagicMock', 'patch', 'sentinel', 'DEFAULT', 'ANY', 'call', 'create_autospec',
           'FILTER_DIR', 'NonCallableMock', 'NonCallableMagicMock', 'mock_open',
           'PropertyMock', 'seal')
__version__ = '1.0'
import inspect, pprint, sys, builtins
from types import ModuleType
from functools import wraps, partial
_builtins = {name for name in dir(builtins) if not name.startswith('_') if not name.startswith('_')}
BaseExceptions = (
 BaseException,)
...
def _get_signature_object(func, as_instance, eat_self):
    if isinstance(func, type) and not as_instance:
        ...
        if not isinstance(func, FunctionTypes):
            try:
                func = func.__call__
            except AttributeError:
                return
        ...
        try:
            return (
             func, inspect.signature(sig_func))
        except ValueError:
            return
...
def _copy_func_details(func, funcopy):
    for attribute in ('__name__', '__doc__', '__text_signature__', '__module__', '__defaults__',
                      '__kwdefaults__'):
        try:
...
def _set_signature(mock, original, instance=False):
    if not _callable(original):
        ...
        name = name.isidentifier() or 'funcopy'
    context = {'_checksig_':checksig, 
     'mock':mock}
...
def _check_and_set_parent--- This code section failed: ---

 L. 321         0  LOAD_GLOBAL              _is_instance_mock
                2  LOAD_FAST                'value'
                4  CALL_FUNCTION_1       1  '1 positional argument'
                6  POP_JUMP_IF_TRUE     12  'to 12'

 L. 322         8  LOAD_CONST               False
               10  RETURN_VALUE     
             12_0  COME_FROM             6  '6'

 L. 323        12  LOAD_FAST                'value'
               14  LOAD_ATTR                _mock_name
               16  POP_JUMP_IF_TRUE     44  'to 44'
               18  LOAD_FAST                'value'
               20  LOAD_ATTR                _mock_new_name
               22  POP_JUMP_IF_TRUE     44  'to 44'

 L. 324        24  LOAD_FAST                'value'
               26  LOAD_ATTR                _mock_parent
               28  LOAD_CONST               None
               30  COMPARE_OP               is-not
               32  POP_JUMP_IF_TRUE     44  'to 44'

 L. 325        34  LOAD_FAST                'value'
               36  LOAD_ATTR                _mock_new_parent
               38  LOAD_CONST               None
 ...
 L. 326        44  LOAD_CONST               False
               46  RETURN_VALUE     
             48_0  COME_FROM            42  '42'

 L. 328        48  LOAD_FAST                'parent'
               50  STORE_FAST               '_parent'

 L. 329        52  SETUP_LOOP           84  'to 84'
               54  LOAD_FAST                '_parent'
               56  LOAD_CONST               None
               58  COMPARE_OP               is-not
               60  POP_JUMP_IF_FALSE    82  'to 82'

 L. 332        62  LOAD_FAST                '_parent'
               64  LOAD_FAST                'value'
               66  COMPARE_OP               is
               68  POP_JUMP_IF_FALSE    74  'to 74'

 L. 333        70  LOAD_CONST               False
               72  RETURN_VALUE     
             74_0  COME_FROM            68  '68'

 L. 334        74  LOAD_FAST                '_parent'
               76  LOAD_ATTR                _mock_new_parent
               78  STORE_FAST               '_parent'
               80  JUMP_BACK            54  'to 54'
             82_0  COME_FROM            60  '60'
               82  POP_BLOCK        
             84_0  COME_FROM_LOOP       52  '52'

 L. 336        84  LOAD_FAST                'new_name'
               86  POP_JUMP_IF_FALSE   100  'to 100'

 L. 337        88  LOAD_FAST                'parent'
               90  LOAD_FAST                'value'
               92  STORE_ATTR               _mock_new_parent

 L. 338        94  LOAD_FAST                'new_name'
               96  LOAD_FAST                'value'
               98  STORE_ATTR               _mock_new_name
            100_0  COME_FROM            86  '86'

 L. 339       100  LOAD_FAST                'name'
              102  POP_JUMP_IF_FALSE   116  'to 116'

 L. 340       104  LOAD_FAST                'parent'
              106  LOAD_FAST                'value'
              108  STORE_ATTR               _mock_parent

 L. 341       110  LOAD_FAST                'name'
              112  LOAD_FAST                'value'
              114  STORE_ATTR               _mock_name
            116_0  COME_FROM           102  '102'

 L. 342       116  LOAD_CONST               True
              118  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `RETURN_VALUE' instruction at offset 118
...
class NonCallableMock(Base):
    ...
    def __get_return_value(self):
        ...
        if ret is DEFAULT:
            ret = self._get_child_mock(_new_parent=self,
              _new_name='()')
    ...
    def __get_side_effect(self):
        ...
        if sf is not None:
            if not callable(sf):
                if not isinstance(sf, _MockIter):
    ...
    def __getattr__(self, name):
        if name in frozenset({'_mock_methods', '_mock_unsafe'}):
            raise AttributeError(name)
        else:
            if not self._mock_methods is not None or name not in self._mock_methods or name in _all_magics:
                raise AttributeError('Mock object has no attribute %r' % name)
            else:
                if _is_magic(name):
                    raise AttributeError(name)
                elif not self._mock_unsafe:
                    ...
                    if result is _deleted:
                        ...
                        if result is None:
                            ...
                            result = self._get_child_mock(parent=self,
                              name=name,
                              wraps=wraps,
                              _new_name=name,
                              _new_parent=self)
                            ...
                            if isinstance(result, _SpecState):
    ...
    def __dir__(self):
        ...
        from_dict = [e for e in from_dict if e.startswith('_') if _is_magic(e)]
        ...
        if name in _allowed_names:
            ...
            if name == '__class__':
                ...
                if self._mock_sealed:
                    mock_name = hasattr(self, name) or f"{self._extract_mock_name()}.{name}"
        ...
        if not any_order:
            if expected not in all_calls:
                raise AssertionError('Calls not found.\nExpected: %r\nActual: %r' % (
                 _CallList(calls), self.mock_calls)) from cause
            return
    ...
    def _mock_call(_mock_self, *args, **kwargs):
        ...
        if effect is not None:
            if _is_exception(effect):
                ...
                result = _callable(effect) or next(effect)
...
class _patch(object):
    ...
    def __enter__(self):
        ...
        if spec is False:
            spec = None
        elif spec_set is False:
            spec_set = None
        elif autospec is False:
            autospec = None
        else:
            if spec is not None:
                if autospec is not None:
        ...
        if new_callable is not None:
            ...
            if not spec is not None:
                ...
                if issubclass(Klass, NonCallableMock) and self.attribute:
    ...
    def __exit__(self, *exc_info):
        ...
        if self.is_local and self.temp_original is not DEFAULT:
            ...
            if not self.create:
                if not hasattr(self.target, self.attribute) or self.attribute in ('__doc__',
                                                                                  '__module__',
                                                                                  '__defaults__',
                                                                                  '__annotations__',
                                                                                  '__kwdefaults__'):
...
_unsupported_magics = {
 ...
 '__init__', '__new__', '__prepare____instancecheck__',
 '__subclasscheck__',
 '__del__'}
_calculate_return_value = {'__hash__':lambda self: object.__hash__(self), 
 '__str__':lambda self: object.__str__(self), 
 '__sizeof__':lambda self: object.__sizeof__(self)}
_return_values = {'__lt__':NotImplemented, 
...
_side_effect_methods = {'__eq__':_get_eq, 
 '__ne__':_get_ne, 
 '__iter__':_get_iter}
...
class _Call(tuple):
    ...
    def __repr__(self):
        name = self.from_kall or self.name or 'call'
        if name.startswith('()'):
            name = 'call%s' % name
        else:
            return name
            ...
            if is_type and instance:
                ...
                if not instance:
                    if 'return_value' not in kwargs:
                        mock.return_value = create_autospec(spec, spec_set, instance=True, _name='()',
                          _parent=mock)
...
def _must_skip(spec, entry, is_type):
    ...
    for klass in spec.__mro__:
        ...
        if isinstance(result, (staticmethod, classmethod)):
            return False
        if isinstance(getattr(result, '__get__', None), MethodWrapperTypes):
            return is_type
        return False
...
def mock_open(mock=None, read_data=''):
    ...
    _state = [
     _iterate_read_data(read_data), None]
