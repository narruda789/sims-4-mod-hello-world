...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\unittest\test\testmock\testpatch.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 57407 bytes
...
class PatchTest(unittest.TestCase):
    ...
    def test_new_callable_incompatible_with_new(self):
        self.assertRaises(ValueError,
          patch, foo_name, new=(object()), new_callable=MagicMock)
        self.assertRaises(ValueError,
          (patch.object), Foo, 'f', new=(object()), new_callable=MagicMock)

    def test_new_callable_incompatible_with_autospec(self):
        self.assertRaises(ValueError,
          patch, foo_name, new_callable=MagicMock, autospec=True)
        self.assertRaises(ValueError,
          (patch.object), Foo, 'f', new_callable=MagicMock, autospec=True)
    ...
    def test_patch_multiple_decorating_class(self):
        ...
        SomeTest = patch.multiple(foo_name,
          f=DEFAULT, g=3, foo=DEFAULT)(SomeTest)
        thing = SomeTest()
        thing.test_one()
        thing.test_two()
        self.assertEqual(Foo.f, original_f)
    ...
    def test_patch_multiple_new_callable(self):
        ...
        patcher = patch.multiple(Foo,
          f=DEFAULT, g=DEFAULT, new_callable=Thing)
        result = patcher.start()
        try:
    ...
    def test_create_and_specs(self):
        for kwarg in ('spec', 'spec_set', 'autospec'):
            p = patch('.doesnotexist' __name__, create=True, **{kwarg: True})
