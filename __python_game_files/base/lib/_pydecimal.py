...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\_pydecimal.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 234943 bytes
__all__ = [
 'Decimal', 'Context',
 'DecimalTuple',
...
class Decimal(object):
    ...
    def from_float(cls, f):
        if isinstance(f, int):
            ...
            if isinstance(f, float) and not _math.isinf(f):
                if _math.isnan(f):
                    return cls(repr(f))
...
Parse error at or near `RETURN_VALUE' instruction at offset 60
    ...
    def __str__(self, eng=False, context=None):
        ...
        if leftdigits == dotplace:
            ...
            if context is None:
                context = getcontext()
            exp = [
             'e', 'E'][context.capitals] + '%+d' % (leftdigits - dotplace)
    ...
    def _fix(self, context):
        if self._is_special:
            if self._isnan():
                ...
                Etop = context.Etop()
                exp_max = self or [
                 context.Emax, Etop][context.clamp]
    ...
    _pick_rounding_function = dict(ROUND_DOWN=_round_down,
      ROUND_UP=_round_up,
    ...
    def __pow__(self, other, modulo=None, context=None):
        if modulo is not None:
            ...
            if self._sign == 1:
                if other._isinteger():
                    result_sign = other._iseven() or 1
                else:
                    if self:
    ...
    def normalize(self, context=None):
        if context is None:
            ...
            if dup._isinfinity():
                return dup
            return dup or _dec_from_triple(dup._sign, '0', 0)
        exp_max = [
    ...
    def _rescale(self, exp, rounding):
        if self._is_special:
            ...
            return self or _dec_from_triple(self._sign, '0', exp)
        if self._exp >= exp:
    ...
    def _round(self, places, rounding):
        if places <= 0:
            ...
            return self._is_special or self or Decimal(self)
    ...
    def to_integral_exact(self, rounding=None, context=None):
        if self._is_special:
            ...
            if self._exp >= 0:
                return Decimal(self)
            return self or _dec_from_triple(self._sign, '0', 0)
        if context is None:
            context = getcontext()
        if rounding is None:
            rounding = context.rounding
    ...
    def is_normal(self, context=None):
        return self._is_special or self or False
        if context is None:
            context = getcontext()
    ...
    def ln(self, context=None):
        if context is None:
            ...
            if ans:
                return ans
            return self or _NegativeInfinity
        if self._isinfinity() == 1:
            return _Infinity
        if self == _One:
            return _Zero
    ...
    def logb(self, context=None):
        ...
        if context is None:
            ...
            if self._isinfinity():
                return _Infinity
            return self or context._raise_error(DivisionByZero, 'logb(0)', 1)
    ...
    def logical_and(self, other, context=None):
        if context is None:
            ...
            other = _convert_other(other, raiseit=True)
            return self._islogical() and other._islogical() or context._raise_error(InvalidOperation)
...
class Context(object):
    ...
    def _raise_error(self, condition, explanation=None, *args):
        ...
        if error in self._ignored_flags:
            ...
            self.flags[error] = 1
            return self.traps[error] or (condition().handle)(self, *args)
...
def _ilog(x, M, L=8):
    y = x - M
    R = 0
    while not (R <= L and abs(y) << L - R >= M):
        if not R > L or abs(y) >> R - L >= M:
...
def _dexp(c, e, p):
    ...
    return (
     _div_nearest(_iexp(rem, 10 ** p), 1000), quot - p + 3)
...
def _convert_for_comparison(self, other, equality_op=False):
    if isinstance(other, Decimal):
        ...
        if isinstance(other, _numbers.Rational):
            ...
            context._raise_error(FloatOperation, 'strict semantics for mixing floats and Decimals are enabled')
        return (
         self, Decimal.from_float(other))
    return (
     NotImplemented, NotImplemented)


DefaultContext = Context(prec=28,
