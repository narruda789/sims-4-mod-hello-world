...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\lzma.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 13330 bytes
__all__ = [
    ...
    def __init__(self, filename=None, mode='r', *, format=None, check=-1, preset=None, filters=None):
        ...
        if mode in ('r', 'rb'):
            ...
            if mode in ('w', 'wb', 'a', 'ab', 'x', 'xb'):
                ...
                self._compressor = LZMACompressor(format=format, check=check, preset=preset,
                  filters=filters)
                self._pos = 0
            else:
        ...
        if self._mode == _MODE_READ:
            raw = _compression.DecompressReader((self._fp), LZMADecompressor, trailing_error=LZMAError,
              format=format,
              filters=filters)
...
def open(filename, mode='rb', *, format=None, check=-1, preset=None, filters=None, encoding=None, errors=None, newline=None):
    ...
    binary_file = LZMAFile(filename, lz_mode, format=format, check=check, preset=preset,
      filters=filters)
    if 't' in mode:
