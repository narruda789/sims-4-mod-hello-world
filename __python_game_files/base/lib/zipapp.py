...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\zipapp.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 7741 bytes
import contextlib, os, pathlib, shutil, stat, sys, zipfile
__all__ = [
...
def main(args=None):
    ...
    if args.info:
        ...
        sys.exit(0)
    elif os.path.isfile(args.source) and not args.output is None:
        if os.path.exists(args.output):
            if os.path.samefile(args.source, args.output):
                raise SystemExit('In-place editing of archives is not supported')
        if args.main:
            raise SystemExit('Cannot change the main function when copying')
    create_archive((args.source), (args.output), interpreter=(args.python),
      main=(args.main),
