...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\ipaddress.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 77355 bytes
__version__ = '1.0'
import functools
IPV4LENGTH = 32
IPV6LENGTH = 128
...
def summarize_address_range(first, last):
    if isinstance(first, _BaseAddress):
        raise isinstance(last, _BaseAddress) or TypeError('first and last must be IP addresses, not networks')
    elif first.version != last.version:
...
def collapse_addresses(addresses):
    ...
    for ip in addresses:
        if isinstance(ip, _BaseAddress):
            ...
            if nets:
                if nets[(-1)]._version != ip._version:
                    raise TypeError('%s and %s are not of the same version' % (
                     ip, nets[(-1)]))
            nets.append(ip)
...
class _BaseNetwork(_IPAddressBase):
    ...
    def address_exclude(self, other):
        if not self._version == other._version:
            raise TypeError('%s and %s are not of the same version' % (
             self, other))
        elif not isinstance(other, _BaseNetwork):
            ...
            if other == self:
                ...
                while s1 != other:
                    if s2 != other:
                        ...
                        raise AssertionError('Error performing exclusion: s1: %s s2: %s other: %s' % (
                         s1, s2, other))
    ...
    def _get_networks_key(self):
        return (
         self._version, self.network_address, self.netmask)
    ...
    def subnets(self, prefixlen_diff=1, new_prefix=None):
        ...
        if new_prefixlen > self._max_prefixlen:
            raise ValueError('prefix length diff %d is invalid for netblock %s' % (
             new_prefixlen, self))
        ...
        if not ip_str:
            ...
            if len(parts) < _min_parts:
                ...
                if skip_index is not None:
                    ...
                    parts[0] or parts_hi -= 1
                    if parts_hi:
                        msg = "Leading ':' only permitted as part of '::' in %r"
                        raise AddressValueError(msg % ip_str)
                    parts[(-1)] or parts_lo -= 1
                    if parts_lo:
                ...
                elif len(parts) != cls._HEXTET_COUNT:
...
class IPv6Address(_BaseV6, _BaseAddress):
    ...
    def teredo(self):
        if self._ip >> 96 != 536936448:
            return
        return (
         IPv4Address(self._ip >> 64 & 4294967295),
         IPv4Address(~self._ip & 4294967295))
