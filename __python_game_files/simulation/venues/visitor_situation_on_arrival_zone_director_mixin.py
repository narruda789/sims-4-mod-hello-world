...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\venues\visitor_situation_on_arrival_zone_director_mixin.py
...
class VisitorSituationOnArrivalZoneDirectorMixin:
    ...
    def _verify_tuning_callback(cls):
        if cls.place_travel_companion_in_same_arrival_situation:
            if not cls.place_all_user_sims_in_same_arrival_situation:
        ...
        if cls.place_travel_companion_in_same_arrival_situation:
            if cls.travel_companion_arrival_situation is not None:
    ...
    def _sim_info_already_in_arrival_situation(self, sim_info, situation_manager):
        ...
        for seed in seeds:
            if sim_info in seed.invited_sim_infos_gen() and seed.situation_type is self.user_sim_arrival_situation:
                if not services.current_zone().time_has_passed_in_world_since_zone_save() or seed.allow_time_jump:
                    return True
    ...
    def create_arrival_situation_for_sim(self, sim_info, situation_type=DEFAULT, during_spin_up=False):
        ...
        if during_spin_up:
            if self._sim_info_already_in_arrival_situation(sim_info, situation_manager):
                return
    ...
    def create_arrival_situation(self, situation_type, guest_list, situation_manager):
        ...
        situation_manager.create_situation(situation_type, guest_list=guest_list,
          user_facing=False,
        ...
        if self.travel_companion_arrival_situation:
            for sim_info in self._traveled_sim_infos:
                if not sim_info.is_selectable:
                    self.create_arrival_situation_for_sim(sim_info, situation_type=(self.travel_companion_arrival_situation),
                      during_spin_up=True)
