...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\venues\cafe_venue\cafe_to_go_customer_situation.py
...
class _OrderAndWaitForCoffeeState(CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if self._order_coffee_timeout is not None:
            self._create_or_load_alarm(ORDER_COFFEE_TIMEOUT, (self._order_coffee_timeout), (lambda _: self.timer_expired()),
    ...
    def handle_event(self, sim_info, event, resolver):
        if event != TestEvent.InteractionStart:
            ...
            if not resolver(self._interaction_of_interest):
                return
            return self.owner.sim_of_interest(sim_info) or None
...
class _LeaveCafeState(CommonInteractionCompletedSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if self._timeout is not None:
            self._create_or_load_alarm(LEAVE_TIMEOUT, (self._timeout), (lambda _: self.timer_expired()),
              should_persist=True, reader=reader)
    ...
    def _additional_tests(self, sim_info, event, resolver):
        if not self.owner.sim_of_interest(sim_info):
            ...
            return resolver.interaction.is_finishing or False
        return True
...
class CafeToGoCustomerSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (SituationStateData(1, _PreOrderCoffeeState, factory=(cls.pre_order_coffee_state)),
         SituationStateData(2, _OrderAndWaitForCoffeeState, factory=(cls.order_coffee_state)),
         SituationStateData(3, _LeaveCafeState, factory=(cls.leave_cafe_state)))
    ...
    def sim_of_interest(self, sim_info):
        if self._to_go_sim is not None:
            if self._to_go_sim.sim_info is sim_info:
                return True
        return False


lock_instance_tunables(CafeToGoCustomerSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
