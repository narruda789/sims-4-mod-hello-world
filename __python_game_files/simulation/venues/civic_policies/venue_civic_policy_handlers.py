...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\venues\civic_policies\venue_civic_policy_handlers.py
...
def generate_venue_game_service_data(*args, zone_id: int=None, **kwargs):
    ...
    if active_zone is None or active_zone.is_zone_shutting_down:
        allow_uninstantiated_zones = False
    voting_open = street_service.voting_open
    for zone_id, instance in venue_game_service._zone_provider.items():
        zone = zone_manager.get(zone_id, allow_uninstantiated_zones=allow_uninstantiated_zones)
        if zone is None:
            continue
        ...
        entry = {'zone':zone_str, 
         ...
         'active':str(type(venue_service.active_venue)) if zone is active_zone else '', 
