...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\venues\npc_summoning.py
# Compiled at: 2019-11-06 18:04:00
# Size of source mod 2**32: 8165 bytes
...
class CreateAndAddToSituation(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value):
        if value.situation_job is not None:
            ...
            if value.situation_job not in jobs:
                logger.error('CreateAndAddToSituation {} references a job {} that is not tuned in the situation {}.', source,
                  (value.situation_job), (value.situation_to_create), owner='manus')
            ...
            logger.error('CreateAndAddToSituation {} references a situation {} \n                without referencing a job and the situation does not have a default job.\n                Either tune a default job on the situation or tune a job reference\n                here.',
              source, (value.situation_to_create), owner='sscholl')

    FACTORY_TUNABLES = {'description':'Create a new situation of this type and add the NPC to its tuned job.', 
     'situation_to_create':Situation.TunableReference(pack_safe=True), 
    ...
    def __call__(self, all_sim_infos, purpose=None, host_sim_info=None):
        ...
        def _create_situation(sim_infos):
            ...
            services.get_zone_situation_manager().create_situation((self.situation_to_create), guest_list=guest_list,
              user_facing=False)
...
class ResidentialLotArrivalBehavior(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'description':'\n            NPC behavior on a residential lot. The behavior is different depending \n            on the lot belonging to the player versus NPC. Greeted behavior can \n            modify behavior as well.\n            ', 
     'player_sim_lot':CreateAndAddToSituation.TunableFactory(), 
    ...
    def __call__(self, sim_infos, purpose=None, host_sim_info=None):
        ...
        for sim_info in sim_infos:
            if sim_info.is_npc:
                sim_info.lives_here or npc_infos.append(sim_info)
            else:
        ...
        if npc_infos:
            ...
            elif is_active_household_residence:
