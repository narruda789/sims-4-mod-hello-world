...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\familiars\familiar_interactions.py
# Compiled at: 2019-06-25 18:16:56
...
class FamiliarPickerInteraction(PickerSuperInteraction):
    INSTANCE_TUNABLES = {'picker_action':TunableVariant(description='\n            The action to take after selecting a familiar from the picker.\n            ',
       set_active_familiar=SetActiveFamiliarAction.TunableFactory(),
       unbind_familiar=UnbindFamiliarAction.TunableFactory(),
       default='set_active_familiar',
       tuning_group=GroupNames.PICKERTUNING), 
     'incompatible_region_disabled_tooltip':TunableLocalizedStringFactory(description='\n            Tooltip that displays if a pet familiar is in an incompatible region.\n            '), 
    ...
    def picker_rows_gen(cls, inst, target, context, **kwargs):
        ...
        if familiar_tracker is not None:
            ...
            for familiar_info in familiar_tracker:
                is_enable = True
                row_tooltip = None
                if familiar_info.pet_familiar_id is not None:
                    if rabbit_hole_service.is_in_rabbit_hole(familiar_info.pet_familiar_id):
                        ...
                        is_enable = sim_region is None or sim_region.is_region_compatible(current_region) or False
                        row_tooltip = inst_or_cls.incompatible_region_disabled_tooltip
                row = BasePickerRow(name=(familiar_info.name), icon_info=(familiar_info.icon_info),
...
class NameFamiliarInteraction(ImmediateSuperInteraction):
    ...
    def _run_interaction_gen(self, timeline):
        ...
        dialog.show_dialog(on_response=on_response, text_input_overrides=text_input_overrides,
          icon_override=familiar_icon,
          text_override=familiar_description)
        return True
        if False:
            yield None