...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\carry\carry_utils.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 22962 bytes
import functools
...
import animation.arb, element_utils, interactions.constraints, sims4.log
...
def interact_with_carried_object(sim, target, posture_state=DEFAULT, interaction=None, create_target_track=None, animation_context=None, must_run=False, sequence=()):
    if not must_run:
        if interaction is not None:
            if interaction.staging:
                if sim.posture_state.is_source_or_owning_interaction(interaction):
                    return sequence
    ...
    def maybe_do_begin(_):
        nonlocal is_carrying_other_object
        nonlocal posture_state
...
def holster_carried_object(sim, interaction, unholster_predicate, flush_before_sequence=False, sequence=None):
        ...
        if interaction.can_holster_incompatible_carries:
            for _, carry_posture in interaction.get_uncarriable_objects_gen(allow_holster=False, use_holster_compatibility=True):
                ...
                if not carry_posture.holstered:
                    sequence = holster_object(carry_posture, flush_before_sequence=flush_before_sequence,
                      sequence=sequence)
    ...
    for _, carry_posture, carry_object in get_carried_objects_gen(sim):
        if carry_posture.holstered and carry_object not in holstered_objects and unholster_predicate(carry_object):
            sequence = unholster_object(carry_posture, flush_before_sequence=flush_before_sequence,
              sequence=sequence)
...
def holster_object(carry_posture, flush_before_sequence=False, sequence=None):
    ...
    carry_nothing_posture = create_posture((CarryPostureStaticTuning.POSTURE_CARRY_NOTHING), (carry_posture.sim),
      None, track=(carry_posture.track))
    ...
    def holster(timeline):
        if carry_object.is_sim:
            ...
            carry_posture.asm.context.register_custom_event_handler((functools.partial(hide_held_props, carry_posture.sim)), None,
            ...
            result or logger.error('Failed to setup asm to holster {}. {} ', carry_object, result, owner='rmccord')
        ...
        if flush_before_sequence:
            holster_element = (
             holster_element, flush_all_animations)
        yield from element_utils.run_child(timeline, holster_element)
        if False:
            yield None

    return (
     lambda _: _set_holster(),
     build_critical_section(interact_with_carried_object((carry_posture.sim), carry_object, interaction=(carry_posture.source_interaction),
...
def unholster_object(carry_posture, flush_before_sequence=False, sequence=None):
    ...
    def unholster(timeline):
        if carry_object.is_sim:
            ...
            return carry_posture.holster_count or None
        ...
        if flush_before_sequence:
            unholster_element = (
             unholster_element, flush_all_animations)
        yield from element_utils.run_child(timeline, unholster_element)
        if False:
            yield None

    return build_critical_section(interact_with_carried_object((carry_posture.sim), carry_object, animation_context=(carry_posture.asm.context),
      interaction=(carry_posture.source_interaction),
      must_run=True,
      sequence=(build_critical_section(unholster, lambda _: _set_unholster()))), sequence, flush_all_animations)


def get_carried_objects_gen(sim):
    posture_left = sim.posture_state.left
    if posture_left is not None:
        if posture_left.target is not None:
            yield (
             Hand.LEFT, posture_left, posture_left.target)
    posture_right = sim.posture_state.right
    if posture_right is not None:
        if posture_right.target is not None:
            yield (
             Hand.RIGHT, posture_right, posture_right.target)
...
def create_carry_nothing_constraint(hand, debug_name='CarryNothing'):
    ...
    if hand == Hand.LEFT:
        entries = (
         PostureManifestEntry(None, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_NONE, MATCH_ANY, MATCH_ANY),)
    else:
        entries = (
         PostureManifestEntry(None, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_ANY, MATCH_NONE, MATCH_ANY),)
