...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\restaurants\host_situation.py
# Compiled at: 2018-08-27 21:55:44
# Size of source mod 2**32: 24123 bytes
...
import alarms, clock, services, sims4.resources
...
class HostSituationStateMixin:
    ...
    def get_valid_group_to_seat(self):
        ...
        if zone_director is not None:
            if not zone_director.can_find_seating_for_group((situation.get_num_sims_in_job(None)), consider_occupied=(situation.is_player_group())):
                return
        return situation
...
class _ArrivingState(CommonInteractionCompletedSituationState, HostSituationStateMixin):
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if self.owner is not None:
            if self.owner.get_staff_member() is not None:
            ...
            if resolver(self._interaction_of_interest):
                if self.owner.sim_of_interest(sim_info):
                    if not resolver.interaction.is_finishing_naturally:
                        ...
                        self._fail_count += 1
                        if self._fail_count < self.max_attempts_to_run_social:
                            if self.owner.push_right_this_way_social(self._group):
...
class _ShowToTableState(CommonSituationState, HostSituationStateMixin):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event != TestEvent.InteractionExitedPipeline:
            ...
            return self.owner is None or self.owner.sim_of_interest(sim_info) or None
            return resolver(self._interaction_of_interest) or None
...
class HostSituation(BusinessEmployeeSituationMixin, StaffedObjectSituationMixin, SituationComplexCommon):
    ...
    def _states(cls):
        return [SituationStateData(1, _ArrivingState, factory=(cls._arriving_situation_state)),
         SituationStateData(2, _HostStationState, factory=(cls._host_station_situation_state)),
         SituationStateData(3, _ShowToTableState, factory=(cls._show_to_table_situation_state)),
         SituationStateData(4, _RightThisWayState, factory=(cls._right_this_way_situation_state))]
    ...
    def push_right_this_way_social(self, group):
        ...
        if zone_director is None:
            ...
            return host.push_super_affordance(self.right_this_way_interaction, target, context) or False
        return True
    ...
    def push_host_show_table_interaction(self, group, zone_director, master_sim):
        ...
        if not host.push_super_affordance((self.host_show_to_table_interaction), table, context,
    ...
    def push_master_sim_interaction(self, group, table, master_sim, master_sim_context, host, master_sim_chair):
        if not master_sim.push_super_affordance((self.requesting_sim_show_to_table_interaction), table, master_sim_context,
