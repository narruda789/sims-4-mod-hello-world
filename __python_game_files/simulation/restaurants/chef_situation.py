...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\restaurants\chef_situation.py
# Compiled at: 2018-10-01 20:29:01
# Size of source mod 2**32: 27578 bytes
...
class ChefSituation(BusinessEmployeeSituationMixin, StaffedObjectSituationMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'begin_state':_ChefBeginState.TunableFactory(description='\n            The first state a chef starts in. This should route them to the\n            chef station and perform the generic swipe. When the interaction of\n            interest completes, the cooking objects will appear on the chef\n            station.\n            ',
     ...
     'buff_to_active_cooking_states_count_delta_map':TunableMapping(description='\n            A mapping of buffs to a delta that will adjust how many active\n            cooking states it takes for an order to be completed.\n            ',
       ...
       value_name='Active Cooking States Before Delivery Delta',
    ...
    def _states(cls):
        return [
         SituationStateData(0, _ChefBeginState, cls.begin_state),
         SituationStateData(1, _ChefPanState, cls.pan_state),
         SituationStateData(2, _ChefPotState, cls.pot_state),
         SituationStateData(3, _ChefCuttingBoardState, cls.cutting_board_state),
         SituationStateData(4, _ChefServeState, cls.serve_state)]
    ...
    def _get_role_state_affordance_override_kwargs(self):
        ...
        if isinstance(self._cur_state, _ChefCuttingBoardState):
    ...
    def _create_and_slot_cooking_object(cls, chef_station, cooking_object, slot, find_existing=False):
        if find_existing:
            for slot_info in chef_station.get_runtime_slots_gen():
                if type(slot) is str:
                    if not slot_info.slot_name_hash == SlotComponent.to_slot_hash(slot):
                        continue
            ...
            slot_result or logger.error('Failed to slot the cooking object ({}) on the chef station. Please ensure the slot is unobstructed and make sure the tuning in the restaurants.chef_tuning instance in Object Editor looks correct for the various slot names.')
    ...
    def _create_cooking_objects(self):
        ...
        if chef_station is None:
            ...
            return
        self._created_pot = self._create_and_slot_cooking_object(chef_station, (ChefTuning.CHEF_STATION_POT_OBJECT),
          (ChefTuning.CHEF_STATION_POT_SLOT),
          find_existing=True)
        self._created_pan = self._create_and_slot_cooking_object(chef_station, (ChefTuning.CHEF_STATION_PAN_OBJECT),
          (ChefTuning.CHEF_STATION_PAN_SLOT),
          find_existing=True)
        self._created_cutting_board = self._create_and_slot_cooking_object(chef_station, (ChefTuning.CHEF_STATION_CUTTING_BOARD_OBJECT),
          (ChefTuning.CHEF_STATION_CUTTING_BOARD_SLOT),
          find_existing=True)
    ...
    def add_direct_order(self, ordered_recipe, ordering_sim):
        ...
        ordering_sim.household.funds.try_remove(meal_cost, reason=(Consts_pb2.TELEMETRY_INTERACTION_COST),
          sim=ordering_sim)
