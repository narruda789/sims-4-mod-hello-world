...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\restaurants\restaurant_diner_situation.py
# Compiled at: 2021-05-17 18:19:04
# Size of source mod 2**32: 84036 bytes
...
import enum, filters.tunable, services, sims4.resources
...
class _DinerReturnToCheckInStateMixin:
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event == TestEvent.InteractionComplete:
            if resolver(self._abandon_table_interaction):
                if self.owner.in_dining_party(sim_info):
...
def check_for_cancel_order(situation_state, sim_info, event, resolver):
    if event == TestEvent.InteractionExitedPipeline:
        if resolver(situation_state._cancel_order_interaction):
            if resolver.interaction.user_canceled:
                if situation_state.owner is not None:
                    if situation_state.owner.sim_of_interest(sim_info):
...
class _CheckInWithHostState(CommonInteractionCompletedSituationState):
    ...
    def on_activate(self, reader=None):
        if self.owner is not None:
            if not self.owner.background_situation.host_working():
                self.owner.check_sim_in()
                return
        if self.owner is not None:
            if self.owner.background_party_checked_in():
...
class _WaitForTableState(CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if zone_director is None:
            ...
            return
        if self.owner is not None:
            if self.owner.background_situation is not None:
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.RestaurantTableClaimed:
            if self.owner is not None:
                if self.owner.sim_of_interest(sim_info):
...
class _PrePlaceOrderState(_DinerReturnToCheckInStateMixin, CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if zone_director is not None:
            if self.owner is not None:
                if self.owner.num_of_sims > 0:
                    sim = next(self.owner.all_sims_in_situation_gen())
                    group_order = zone_director.get_active_group_order_for_sim(sim.id)
                    if group_order is not None:
                        if group_order.order_status >= OrderStatus.ORDER_GIVEN_TO_CHEF:
                            sim_order = group_order.get_sim_order(sim.id)
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event == TestEvent.RestaurantFoodOrdered:
            if self.owner is not None:
                if self.owner.sim_of_interest(sim_info):
...
class _WaitForFoodFromChefState(_DinerReturnToCheckInStateMixin, CommonSituationState):
    ...
    def handle_event(self, sim_info, event, resolver):
        super().handle_event(sim_info, event, resolver)
        if event == TestEvent.RestaurantOrderDelivered:
            if self.owner.sim_of_interest(sim_info):
...
class _EatFoodState(_DinerReturnToCheckInStateMixin, CommonInteractionCompletedSituationState):
    ...
    def _additional_tests(self, sim_info, event, resolver):
        if not self.owner is None:
            return self.owner.sim_of_interest(sim_info) or False
        else:
            if resolver.interaction.target is None:
                return False
            resolver = SingleObjectResolver(resolver.interaction.target)
            return self._interaction_target_tests.run_tests(resolver) or False
        return True
...
class RestaurantDinerSubSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'_arrival_state':_ArrivalState.TunableFactory(description='\n            The tuning for arrival state which will complete when the tuned\n            interaction is run.\n            ',
       ...
       display_name='01_arrival_state'), 
     '_check_in_with_host_state':_CheckInWithHostState.TunableFactory(description='\n            The state tuning for when the Sims are checking in with the host\n            at the restaurant (if there is one) to get a table.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='02_check_in_with_host_state'), 
     '_wait_for_table_state':_WaitForTableState.TunableFactory(description='\n            The state tuning for when the Sims are waiting for a claimed table\n            to begin their dining experience.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='03_wait_for_table_state'), 
     '_pre_place_order_state':_PrePlaceOrderState.TunableFactory(description='\n            The state tuning for when the Sims are at their assigned table\n            waiting for the waiter to come take the groups order.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='04_pre_place_order_state'), 
     '_order_from_chef_state':_OrderFromChefState.TunableFactory(description='\n            The state tuning for when a Sim needs to order from the chef \n            directly instead of from a waiter/waitress.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='05_order_from_chef_state'), 
     '_post_place_order_state':_PostPlaceOrderState.TunableFactory(description='\n            The state tuning for when the Sims have already ordered their meal\n            and are waiting for the food to be delivered to the table by the\n            wait staff.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='06_post_place_order_state'), 
     '_wait_for_food_from_chef_state':_WaitForFoodFromChefState.TunableFactory(description='\n            The state tuning for when the Sims are waiting for their orders to\n            be delivered by the chef.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='07_wait_for_food_from_chef_state'), 
     '_wait_for_food_from_waitstaff_state':_WaitForFoodFromWaitStaffState.TunableFactory(description='\n            The state tuning for when the Sims are waiting for their orders to\n            be delivered by the waitstaff.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='08_wait_for_food_from_waitstaff_state'), 
     '_eat_food_state':_EatFoodState.TunableFactory(description='\n            The state tuning for when the Sims have received their food and are\n            now chowing down on their meals.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='09_eat_food_state'), 
     '_prepare_to_leave_state':_PrepareToLeaveState.TunableFactory(description='\n            The state tuning for when the Sims are finished eating but not quite\n            ready to leave yet.\n            ',
       tuning_group=GroupNames.STATE,
       display_name='10_prepare_to_leave_state'), 
    ...
    def _states(cls):
        return [
         SituationStateData((DinerSubSituationState.ARRIVAL), _ArrivalState, factory=(cls._arrival_state)),
         SituationStateData((DinerSubSituationState.CHECK_IN_WITH_HOST), _CheckInWithHostState, factory=(cls._check_in_with_host_state)),
         SituationStateData((DinerSubSituationState.WAIT_FOR_TABLE), _WaitForTableState, factory=(cls._wait_for_table_state)),
         SituationStateData((DinerSubSituationState.PRE_PLACE_ORDER), _PrePlaceOrderState, factory=(cls._pre_place_order_state)),
         SituationStateData((DinerSubSituationState.ORDER_FROM_CHEF), _OrderFromChefState, factory=(cls._order_from_chef_state)),
         SituationStateData((DinerSubSituationState.POST_PLACE_ORDER), _PostPlaceOrderState, factory=(cls._post_place_order_state)),
         SituationStateData((DinerSubSituationState.WAIT_FOR_FOOD_FROM_CHEF), _WaitForFoodFromChefState, factory=(cls._wait_for_food_from_chef_state)),
         SituationStateData((DinerSubSituationState.WAIT_FOR_FOOD_FROM_WAITSTAFF), _WaitForFoodFromWaitStaffState, factory=(cls._wait_for_food_from_waitstaff_state)),
         SituationStateData((DinerSubSituationState.EAT_FOOD_STATE), _EatFoodState, factory=(cls._eat_food_state)),
         SituationStateData((DinerSubSituationState.PREPARE_TO_LEAVE), _PrepareToLeaveState, factory=(cls._prepare_to_leave_state)),
         SituationStateData((DinerSubSituationState.LEAVE), _LeaveState, factory=(cls._leave_state)),
         SituationStateData((DinerSubSituationState.ORDER_PREROLL), _OrderPrerollState, factory=(cls._order_preroll_state)),
         SituationStateData((DinerSubSituationState.WAIT_FOR_FOOD_PREROLL), _WaitForFoodPrerollState, factory=(cls._wait_for_food_preroll_state)),
         SituationStateData((DinerSubSituationState.EAT_PREROLL), _EatPrerollState, factory=(cls._eat_preroll_state))]
    ...
    def _on_set_sim_job(self, sim, job_type):
        ...
        if type(self._cur_state) == _ArrivalState:
            if not sim.is_npc:
...
class RestaurantDinerBackGroundSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return [
         SituationStateData(1, _DinerArrivalState, factory=(cls._diner_arrival_state)),
         SituationStateData(2, _DinerOrderState, factory=(cls._diner_order_state)),
         SituationStateData(3, _DinerEatState, factory=(cls._diner_eat_state)),
         SituationStateData(4, _DinerLeaveState, factory=(cls._diner_leave_state)),
         SituationStateData(5, _DinerAdvanceToSeatedOrEating, factory=_DinerAdvanceToSeatedOrEating)]
    ...
    def _on_set_sim_job(self, sim, job_type):
        ...
        situation_id = situation_manager.create_situation(individual_situation, guest_list=guest_list,
          user_facing=False,
          background_situation=self)
        ...
        if sim.id in self._guest_current_state_save_data:
            ...
            if self.cur_state_is_correct_state_type(_DinerOrderState):
                ...
                if self.cur_state_is_correct_state_type(_DinerAdvanceToSeatedOrEating):
    ...
    def advance_for_preroll(self):
        ...
        if rand == 0:
            ...
            if not self.order_for_table(order_status=(OrderStatus.ORDER_GIVEN_TO_CHEF)):
                self.advance_preroll_to_preorder()
            else:
    ...
    def order_for_table(self, order_status=OrderStatus.MENU_READY, active_group_order=None, fire_event=False):
        ...
        for sim in self.all_sims_in_situation_gen():
            if active_group_order is not None:
                ...
                if sim_order is not None:
                    if sim_order.recommendation_state == OrderRecommendationState.RECOMMENDATION_PROPOSAL:
        ...
        if not recipes:
            ...
            if active_group_order.is_ready_to_be_taken():
                ...
                return True
        elif self.waitstaff_working():
            zone_director.order_for_table([(sim.id, recipe.guid64) for sim, recipe in recipes], order_status=order_status,
              group_order=active_group_order)
        else:
    ...
    def cleanup_sub_situations(self, sim=None):
        for situation in self.sub_situations:
            if situation:
                if sim is not None:
                    if sim not in situation.all_sims_in_situation_gen():
                        if situation.get_num_sims_in_job() > 0:
                            continue
            if situation is not None:
                situation._self_destruct()
    ...
    def _destroy(self):
        ...
        business_manager = services.business_service().get_business_manager_for_zone()
        if business_manager is not None:
            if self._on_store_closed in business_manager.on_store_closed:
