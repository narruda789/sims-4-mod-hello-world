...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\weather\weather_event.py
# Compiled at: 2021-03-01 23:26:06
# Size of source mod 2**32: 19024 bytes
...
from weather.weather_enums import PrecipitationType, Temperature, CloudType, WeatherElementTuple, WeatherEffectType, WeatherOption
...
class WeatherEvent(HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(Types.WEATHER_EVENT)):
  ...
  default_upper=default_upper), **kwargs)
    ...
    INSTANCE_TUNABLES = {'precipitation':OptionalTunable(description='\n            The amount/type of precipitation for this weather event.\n            ',
     ...
     'cloud_states':TunableMapping(description='\n            The types of clouds for this weather event.\n            ',
       key_type=TunableEnumEntry(description='\n                The type of clouds.\n                ',
       tunable_type=CloudType,
       default=(CloudType.PARTLY_CLOUDY),
       invalid_enums=(
      CloudType.STRANGE, CloudType.VERY_STRANGE)),
       value_type=TunableWeatherElementTuple(default_lower=100,
       default_upper=100),
       minlength=1), 
     'wind':OptionalTunable(description='\n            The amount of wind for this weather event.\n            ',
       tunable=TunableWeatherElementTuple()), 
     'temperature':TunableEnumEntry(description='\n            The temperature.\n            ',
       tunable_type=Temperature,
       default=Temperature.WARM), 
     'thunder':OptionalTunable(description='\n            The amount of thunder for this weather event.\n            ',
       tunable=TunableWeatherElementTuple()), 
     'lightning':OptionalTunable(description='\n            The amount of lightning for this weather event.\n            ',
       tunable=TunableWeatherElementTuple()), 
    ...
    def get_transition_data(cls, previous_event, old_transition_data, duration):
        ...
        if key in old_transition_data:
            transition_element = old_transition_data[key]
            if transition_element.end_value != 0.0 or now <= transition_element.end_time:
                transition_data[key] = old_transition_data[key]
            ...
            for cloudtype in CloudType:
                ...
                if cloudtype not in transition_data.keys() and old_data is not None:
