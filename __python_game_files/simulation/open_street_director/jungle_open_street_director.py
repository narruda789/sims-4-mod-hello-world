...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\open_street_director\jungle_open_street_director.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 32679 bytes
...
class JungleOpenStreetDirector(OpenStreetDirectorBase):
    DEFAULT_LOCK = LockAllWithSimIdExceptionData(lock_priority=(LockPriority.PLAYER_LOCK), lock_sides=(LockSide.LOCK_FRONT),
      should_persist=True,
      except_actor=False,
    ...
    INSTANCE_TUNABLES = {'path_obstacle_data':TunableMapping(description='\n            Tuned data for the path obstacles in the open street. \n            \n            This includes which conditional layer the path obstacle is attached\n            to and what state that layer is in when the obstacle is locked.\n            ',
     ...
     'path_clearing_commodity':TunableReference(description='\n            The commodity that has to reach 100 in order for a path to be\n            considered clear.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.STATISTIC)), 
     'treasure_chest_tag':TunableTag(description='\n            The tag used to identify a treasure chest.\n            '), 
                ...
                for obstacle in obstacles:
                    if tag == self.TEMPLE_PATH_OBSTACLE:
                        if not obstacle.state_value_active(self.TEMPLE_PATH_OBSTACLE_UNLOCK_STATE):
                            ...
                            progress = 0
                    elif status == JungleOpenStreetDirector.PATH_LOCKED:
                        self._lock_path_obstacle(obstacle, tag)
                    else:
                        if status == JungleOpenStreetDirector.PATH_UNAVAILABLE:
                            self._permanently_lock_path_obstacle(obstacle, tag)
                        else:
                            if status == JungleOpenStreetDirector.PATH_UNLOCKED:
                    ...
                    self._set_path_obstacle_stat(obstacle, self.path_clearing_commodity, progress)
    ...
    def _temple_needs_reset(self):
        if self._temple_state == self.TEMPLE_STATE_NEEDS_RESET:
            return True
        if self._temple_state == self.TEMPLE_STATE_COMPLETE:
    ...
    def _set_path_obstacle_stat(self, path_obstacle, stat, value):
        stat_tracker = path_obstacle.statistic_tracker
        stat_tracker.set_value(stat, value)

    def _get_path_obstacle_stat(self, path_obstacle, stat):
        stat_tracker = path_obstacle.statistic_tracker
        stat_value = stat_tracker.get_value(stat)
        return stat_value
    ...
    def _lock_path_obstacle(self, path_obstacle, obstacle_tag, reset_commodity=False):
        ...
        if reset_commodity:
            self._set_path_obstacle_stat(path_obstacle, self.path_clearing_commodity, JungleOpenStreetDirector.MIN_CLEAR_COMMODITY)
    ...
    def _save_custom_open_street_director(self, street_director_proto, writer):
        ...
        for path_obstacle, tag in self._path_obstacles.items():
            ...
            progress = self._get_path_obstacle_stat(path_obstacle, self.path_clearing_commodity)
            clear_progress.append(progress if progress is not None else 0)
    ...
    def _get_new_temple_id(self, new_id=None):
        ...
        if new_id is not None:
            if new_id in temples:
                if new_id != self._current_temple_id:
                    return new_id
