...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\autonomy\content_sets.py
# Compiled at: 2020-05-08 13:27:50
# Size of source mod 2**32: 29695 bytes
...
import gsi_handlers.content_set_handlers, postures, sims4.log, sims4.reload
...
def aop_valid_for_scoring(aop, affordance, target, context, include_failed_aops_with_tooltip, considered=None, aop_pre_tests=None):
    if aop_pre_tests:
        test_result = aop_pre_tests(aop)
        if test_result:
        ...
        if considered is not None:
            considered[aop] = {'affordance':str(affordance), 
             'target':str(target), 
             'test':str(test_result)}
        if test_result:
            return test_result
...
def get_valid_aops_gen(target, affordance, si_affordance, si, context, include_failed_aops_with_tooltip, push_super_on_prepare=False, considered=None, aop_kwargs=frozendict(), aop_pre_tests=None):
    potential_interactions = (affordance.potential_interactions)(
 target, si_affordance, si, push_super_on_prepare=push_super_on_prepare, **aop_kwargs)
...
def any_content_set_available(sim, super_affordance, super_interaction, context, potential_targets=(), include_failed_aops_with_tooltip=False):
    ...
    with sims4.callback_utils.invoke_enter_exit_callbacks(sims4.callback_utils.CallbackEvent.CONTENT_SET_GENERATE_ENTER, sims4.callback_utils.CallbackEvent.CONTENT_SET_GENERATE_EXIT):
        ...
        for affordance in si_or_sa.all_affordances_gen(context=context):
            if affordance.is_super:
                logger.error('Content set contains a super affordance: {} has {}', si_or_sa,
                  affordance, owner='msantander')
            if for_pie_menu:
                if not affordance.allow_user_directed:
                    continue
...
def generate_content_set(sim, super_affordance, super_interaction, context, potential_targets=(), scoring_gsi_handler=None, include_failed_aops_with_tooltip=False, push_super_on_prepare=False, mixer_interaction_group=DEFAULT, check_posture_compatibility=False, aop_kwargs=frozendict(), aop_pre_tests=None):
    ...
    if not si_or_sa.is_social or potential_targets:
        ...
        with sims4.callback_utils.invoke_enter_exit_callbacks(sims4.callback_utils.CallbackEvent.CONTENT_SET_GENERATE_ENTER, sims4.callback_utils.CallbackEvent.CONTENT_SET_GENERATE_EXIT):
            ...
            for affordance in si_or_sa.all_affordances_gen(context=context, phase_index=phase_index):
                    ...
                    if for_pie_menu:
                        if not affordance.allow_user_directed:
                            if gsi_considered is not None:
                                gsi_considered[affordance] = {'affordance':str(affordance), 
                ...
                if targets is None:
                    continue
                if (include_failed_aops_with_tooltip or len(targets)) >= MINIMUM_TARGETS_TO_TEST_AFFORDANCE_EARLY:
                    ...
                    if affordance.test_globals.run_tests(resolver, skip_safe_tests=False, search_for_tooltip=False):
                        ...
                        if context.source == InteractionContext.SOURCE_AUTONOMY:
                            if not affordance.test_autonomous.run_tests(resolver, skip_safe_tests=False, search_for_tooltip=False):
                                continue
                        for target in targets:
                            valid_aops_gen = get_valid_aops_gen(target,
                              affordance, super_affordance, super_interaction, context,
                              include_failed_aops_with_tooltip, push_super_on_prepare=push_super_on_prepare,
                              considered=gsi_considered,
...
def get_buff_aops(sim, buff, super_interaction, context, potential_targets=(), gsi_considered=None, aop_pre_tests=None):
    ...
    for buff_affordance in buff.interactions.interaction_items:
        ...
        if targets is None:
            continue
        for target in targets:
            for aop, test_result in get_valid_aops_gen(target, buff_affordance,
              (super_interaction.super_affordance),
              super_interaction,
              context,
              False,
              considered=gsi_considered,
              aop_pre_tests=aop_pre_tests):
                ...
                if buff_affordance not in valid:
                    valid[buff_affordance] = [
                     (
                      si_weight, aop, test_result)]
                else:
...
def _test_affordance_and_get_targets(affordance, super_interaction, potential_targets, sim, considered=None):
    if _mixer_name_list:
        if not any((name.match(affordance.__name__) for name in _mixer_name_list)):
            return
    ...
    if not sim_specific_lockout:
        if sim.is_sub_action_locked_out(affordance):
            if considered is not None:
                considered[affordance] = {'affordance':str(affordance), 
                 'target':'', 
                 'test':'Locked out'}
            return
    ...
    if considered is not None:
        ...
        considered[affordance] = {'affordance':str(affordance),  'target':', '.join(target_strs), 
         'test':'Locked out'}
    return
    ...
    for affordance, score in scored.items():
        ...
        for weight, aop, test_result in valid[affordance]:
            ...
            if gsi_handlers.content_set_handlers.archiver.enabled:
                ...
                gsi_results[aop] = {'result_affordance':str(affordance), 
                 'result_target':str(aop.target), 
                 'result_loc_key':aop.affordance.display_name().hash, 
                 'result_target_loc_key':aop.affordance.display_name_target().hash}
            yield (
             weight, aop, test_result)
