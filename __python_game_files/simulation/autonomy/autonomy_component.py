...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\autonomy\autonomy_component.py
# Compiled at: 2020-06-22 17:32:47
# Size of source mod 2**32: 71876 bytes
import random
...
import alarms, autonomy.autonomy_modifier, caches, clock, date_and_time, elements, gsi_handlers, services, sims4.log
...
class AutonomyComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=AUTONOMY_COMPONENT):
  ...
  default_hour=9), **kwargs)
    ...
    def _attempt_full_autonomy_gen(self, timeline):
        if self._full_autonomy_request is not None:
            if self._full_autonomy_request.valid:
            ...
            if self.to_skip_autonomy():
                ...
                return False
            return self._test_full_autonomy() or False
        try:
            ...
            if not self._autonomy_enabled:
                ...
                if selected_interaction is not None:
                    if selected_interaction.transition is not None:
                        if self.owner.routing_master is not None:
                            if self.owner.routing_master.is_sim:
                                if self.owner.routing_master.transition_controller is not None:
    ...
    def run_test_autonomy_ping(self, affordance_list=None, commodity_list=None):
        autonomy_request = AutonomyRequest((self.owner), autonomy_mode=FullAutonomy, commodity_list=commodity_list,
    ...
    def _create_autonomy_request(self):
        autonomy_request = AutonomyRequest((self.owner), autonomy_mode=FullAutonomy, skipped_static_commodities=(self.standard_static_commodity_skip_set),
          limited_autonomy_allowed=False)
    ...
    def _push_interaction(self, selected_interaction):
        ...
        if selected_interaction.target:
            self.owner.add_lockout(selected_interaction.target, AutonomyModesTuning.LOCKOUT_TIME)
        return False
    ...
    @componentmethod
    def reset_multitasking_roll--- This code section failed: ---

 L. 682         0  LOAD_FAST                'interaction'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_TRUE     44  'to 44'
                8  LOAD_FAST                'interaction'
               10  LOAD_ATTR                source
               12  LOAD_GLOBAL              InteractionSource
               14  LOAD_ATTR                PIE_MENU
               16  COMPARE_OP               is
               18  POP_JUMP_IF_TRUE     44  'to 44'

 L. 683        20  LOAD_FAST                'interaction'
               22  LOAD_ATTR                source
               24  LOAD_GLOBAL              InteractionSource
               26  LOAD_ATTR                AUTONOMY
               28  COMPARE_OP               is
               30  POP_JUMP_IF_TRUE     44  'to 44'

 L. 684        32  LOAD_FAST                'interaction'
               34  LOAD_ATTR                source
               36  LOAD_GLOBAL              InteractionSource
               38  LOAD_ATTR                SCRIPT
               40  COMPARE_OP               is
 ...
 L. 685        44  LOAD_GLOBAL              UNSET
               46  LOAD_FAST                'self'
               48  STORE_ATTR               _multitasking_roll
             50_0  COME_FROM            42  '42'

Parse error at or near `STORE_ATTR' instruction at offset 48
    ...
    @componentmethod
    def skip_autonomy--- This code section failed: ---

 L. 800         0  LOAD_FAST                'si'
                2  LOAD_ATTR                source
                4  LOAD_GLOBAL              InteractionSource
                6  LOAD_ATTR                BODY_CANCEL_AOP
                8  COMPARE_OP               ==
               10  POP_JUMP_IF_TRUE     48  'to 48'

 L. 801        12  LOAD_FAST                'si'
               14  LOAD_ATTR                source
               16  LOAD_GLOBAL              InteractionSource
               18  LOAD_ATTR                CARRY_CANCEL_AOP
               20  COMPARE_OP               ==
               22  POP_JUMP_IF_TRUE     48  'to 48'

 L. 802        24  LOAD_FAST                'si'
               26  LOAD_ATTR                source
               28  LOAD_GLOBAL              InteractionSource
               30  LOAD_ATTR                SOCIAL_ADJUSTMENT
               32  COMPARE_OP               ==
               34  POP_JUMP_IF_TRUE     48  'to 48'

 L. 803        36  LOAD_FAST                'si'
               38  LOAD_ATTR                source
               40  LOAD_GLOBAL              InteractionSource
               42  LOAD_ATTR                VEHCILE_CANCEL_AOP
               44  COMPARE_OP               ==
 ...
 L. 804        48  LOAD_CONST               None
               50  RETURN_VALUE     
             52_0  COME_FROM            46  '46'

 L. 805        52  LOAD_FAST                'to_skip'
               54  POP_JUMP_IF_FALSE    86  'to 86'

 L. 806        56  LOAD_GLOBAL              logger
               58  LOAD_METHOD              debug
               60  LOAD_STR                 'Skipping autonomy for {} due to {}'
               62  LOAD_FAST                'self'
               64  LOAD_ATTR                owner
               66  LOAD_FAST                'si'
 ...
 L. 807        72  LOAD_FAST                'self'
               74  LOAD_ATTR                _autonomy_skip_sis
               76  LOAD_METHOD              add
               78  LOAD_FAST                'si'
               80  CALL_METHOD_1         1  '1 positional argument'
               82  POP_TOP          
               84  JUMP_FORWARD        128  'to 128'
             86_0  COME_FROM            54  '54'

 L. 809        86  LOAD_FAST                'si'
               88  LOAD_FAST                'self'
               90  LOAD_ATTR                _autonomy_skip_sis
               92  COMPARE_OP               in
               94  POP_JUMP_IF_FALSE   108  'to 108'

 L. 810        96  LOAD_FAST                'self'
               98  LOAD_ATTR                _autonomy_skip_sis
              100  LOAD_METHOD              remove
              102  LOAD_FAST                'si'
              104  CALL_METHOD_1         1  '1 positional argument'
              106  POP_TOP          
            108_0  COME_FROM            94  '94'

 L. 811       108  LOAD_GLOBAL              logger
              110  LOAD_METHOD              debug
              112  LOAD_STR                 'Unskipping autonomy for {} due to {}; {} is left.'
              114  LOAD_FAST                'self'
              116  LOAD_ATTR                owner
              118  LOAD_FAST                'si'
              120  LOAD_FAST                'self'
              122  LOAD_ATTR                _autonomy_skip_sis
              124  CALL_METHOD_4         4  '4 positional arguments'
              126  POP_TOP          
            128_0  COME_FROM            84  '84'

Parse error at or near `POP_TOP' instruction at offset 126
    ...
    def get_time_until_next_update(self, mode=FullAutonomy):
        ...
        if self.is_player_active():
            ...
            if self._last_autonomy_result_was_none:
                time_to_run_autonomy = self._get_last_no_result_time() + mode.get_no_result_delay_time()
            else:
                if self.owner.has_any_pending_or_running_interactions():
    ...
    def run_preroll_autonomy(self, ignored_objects):
        ...
        if self._queued_autonomy_request is not None:
            ...
            autonomy_request = PrerollAutonomyRequest((self.owner), autonomy_mode=PrerollAutonomy, commodity_list=commodity_list,
              static_commodity_list=static_commodity_list,
              distance_estimation_behavior=(AutonomyDistanceEstimationBehavior.IGNORE_DISTANCE),
        ...
        if self._push_interaction(selected_interaction):
            return (
             selected_interaction.affordance, selected_interaction.target)
        return (None, None)
            ...
            if autonomy.autonomy_util.info_start_time is not None:
                ...
                if caching_info is not None:
                    ...
                    while len(self._cached_mixer_interactions) < mixers_to_cache:
                        interaction = services.autonomy_service().find_best_action(autonomy_request, consider_all_options=True,
                        ...
                        if interaction.lock_out_time is not None:
                            interaction.lock_out_time.target_based_lock_out or autonomy_request.skipped_affordance_list.append(interaction.affordance)
    ...
    def get_time_until_next_wakeup(self, offset_time: TimeSpan=None):
        ...
        if sim_careers:
            ...
            for career in sim_careers.values():
                ...
                if earliest_time is None or wakeup_time < earliest_time:
    ...
    def _get_default_sleep_schedule_work_time(self, offset_time):
        ...
        if offset_time is not None:
            now += offset_time
        sleep_schedule = self._get_sleep_schedule()
        work_time = date_and_time.create_date_and_time(days=(int(now.absolute_days())), hours=(sleep_schedule.default_work_time.hour()),
    ...
    def debug_get_autonomy_timers_gen(self):
        ...
        if self._full_autonomy_alarm_handle is not None:
            yield (
             'Full Autonomy', '{}'.format(self._full_autonomy_alarm_handle.get_remaining_time()))
        else:
            yield ('Full Autonomy', 'None')
        if self._last_user_directed_action_time is not None:
            yield (
             'Last User-Directed Action', '{} ({} ago)'.format(self._last_user_directed_action_time, now - self._last_user_directed_action_time))
        else:
            if self._last_autonomous_action_time:
                yield (
                 'Last Autonomous Action', '{} ({} ago)'.format(self._last_autonomous_action_time, now - self._last_autonomous_action_time))
            if len(self._autonomy_skip_sis) > 0:
