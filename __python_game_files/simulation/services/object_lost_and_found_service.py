...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\services\object_lost_and_found_service.py
# Compiled at: 2021-06-28 14:47:26
...
from sims4.tuning.tunable import AutoFactoryInit, TunableSet, HasTunableSingletonFactory
from sims4.utils import classproperty
from ui.ui_dialog_notification import UiDialogNotification
import build_buy, persistence_error_types, services
...
class ObjectLostAndFoundService(Service):
    ...
    def _fixup_cloned_animals(self, old_id, new_obj):
        ...
        if animal_service is not None:
            if new_obj.animalobject_component is not None:
    ...
    def _return_lost_object(self, locator):
        ...
        if sim_info is not None and sim_info.is_instanced():
            if sim_info.zone_id != locator.zone_id:
                sim = locator.return_to_individual_sim or self._any_household_member_on_lot(locator.zone_id, sim_info.household.id, sim_info.id) or sim_info.get_sim_instance(allow_hidden_flags=ALL_HIDDEN_REASONS)
                if sim is None:
                    return (None, None)
            ...
            obj = self._any_household_member_on_lot(locator.zone_id, locator.household_id, locator.sim_id) or create_object((locator.object_data.guid), loc_type=(ItemLocation.HOUSEHOLD_INVENTORY))
            if obj is not None:
                obj.attributes = locator.object_data.SerializeToString()
                obj.scale = locator.object_data.scale
                self._fixup_cloned_animals(old_id=object_id, new_obj=obj)
    ...
    def add_game_object(self, zone_id, object_id, sim_id, household_id, time_before_lost, return_to_individual_sim):
        self.remove_clone_id(object_id)
        self.remove_object(object_id)
        return self._raw_add_object_data(zone_id, (ObjectLostAndFoundService.STREET_UNKNOWN),
          object_id,
          sim_id, household_id, time_before_lost,
          return_to_individual_sim=return_to_individual_sim) is not None
