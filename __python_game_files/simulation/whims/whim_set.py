...
from sims4.tuning.tunable import TunableEnumEntry, TunableVariant, OptionalTunable, TunableRange
...
class WhimSetBaseMixin:
    ...
    @classproperty
    def found_whim_types(cls):
        whim_types = set()
        for entry in cls.whims:
            if entry is not None and entry.whim is not None:
                whim_types.add(entry.whim.type)
