...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\group.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 103613 bytes
...
from sims4.utils import setdefault_callable, constproperty
from socials.social_focus_manager import SocialFocusManager
from socials.social_scoring import SocialGroupCostFunction
import alarms, build_buy, clock, date_and_time, distributor.ops, gsi_handlers, interactions.constraints, objects.components.line_of_sight_component, placement, routing, services, sims4.log, sims4.math, sims4.tuning.instances, socials.geometry, terrain
...
def get_fallback_social_constraint_position(sim, target, si, priority=None):
    if priority is None:
        ...
        if sim is None or target is None:
            return (None, None)
        sim_constraint = sim.si_state.get_total_constraint(priority=priority, to_exclude=si,
          existing_si=si)
        for constraint in sim_constraint:
            if constraint.geometry is None:
                return (
                 target.position, target.routing_surface)

        target_constraint = target.si_state.get_total_constraint(priority=priority, to_exclude=si,
          existing_si=si)
        ...
        return intersection.valid or (None, None)
    return (
     intersection.average_position, intersection.routing_surface)


SocialGroupLocation = collections.namedtuple('SocialGroupLocation', ('position', 'forward',
                                                                     'routing_surface'))
...
class SocialGroup(objects.components.ComponentContainer, HasStatisticComponent, metaclass=sims4.tuning.instances.TunedInstanceMetaclass, manager=services.get_instance_manager(resources.Types.SOCIAL_GROUP)):
    ...
    WORST_PLACED_SIM_RANDOMNESS = Tunable(description='\n        A floating point value that determines the randomness of the\n        selection of the worst placed Sim.  The worst placed Sim is given a\n        chance to change position by social adjustment. \n        \n        Larger values are more random.  A value of 0 means that there is no\n        randomness, while a value of 1000 means the selection is almost\n        completely random.\n        ',
      tunable_type=float,
      default=1.0)
    CENTER_PLACEMENT_JIG_OBJECT_DEF = TunableReference(description='\n        Temporary object (jig) to be placed when computing position of a sim joining a social group.',
    ...
    def _get_social_anchor_object(cls, si, target_sim):
        ...
        if (si.sim.parent is target_sim or target_sim) is not None:
            if target_sim.parent is si.sim:
    ...
    def get_objects_for_reset(self):
        ...
        if self._anchor_object is not None:
            if self._anchor_object not in self:
    ...
    def zone_id(self):
        if self.anchor is not None:
            if self.anchor.is_sim:
                return self.anchor.zone_id
        if self._group_leader is not None:
            if self._group_leader.is_sim:
    ...
    def _on_induction_status_updated(self, sim):
        ...
        if registered_si is None:
            ...
            basic_content = sim not in self._induction_map or self._induction_map[sim] or registered_si.basic_content
    ...
    def _add(self, sim, attaching_si):
        ...
        try:
            ...
            if social_group_leader_override is not None:
                if attaching_si.get_participant(social_group_leader_override) is not self._group_leader:
        ...
        if self._anchor_object is not None:
            ...
            if constraint is not None:
                                    ...
                                    if constraint is None:
                                        if (joining_sim.intended_position - focus_object.intended_position).magnitude() < 4.0 * self.radius_scale:
                                            ...
                                            if self._check_focus_position(target_position, focus_object.intended_routing_surface):
                                                found = True
                target_position = found or focus_object.intended_position + focus_object.intended_forward * self.radius_scale
                if self._check_focus_position(target_position, focus_object.intended_routing_surface):
                    found = True
                ...
                if target_routing_surface.type == SurfaceType.SURFACETYPE_OBJECT:
                    ...
                    if surface_provider_object is not None and surface_provider_object.disallow_social_group_placement:
                        used_object = None
                        ...
                        reposition_constraint = Circle((focus_object.position), (self.INCOMPATIBLE_OBJECT_REPOSITION_RADIUS),
                          routing_surface=world_routing_surface,
                          los_reference_point=(surface_provider_object.position))
                        ...
                        if handles:
                            ...
                            if goals:
                                ...
                                relocated_position, _ = placement.find_good_location(fgl_context)
    ...
    def make_constraint_default(cls, sim, target, position, routing_surface, participant_type=ParticipantType.Actor, picked_object=None, **kwargs):
        ...
        circle = Circle(position, (cls.max_radius), routing_surface, ideal_radius=(cls.radius_scale),
          debug_name='SocialConstraint')
    ...
    def _make_constraint(self, position, priority=None, from_init=True):
        if self._focus is None:
            ...
            if either_incompatible:
                fallback_position, fallback_routing_surface = get_fallback_social_constraint_position((self._initiating_sim), (self._target_sim),
                  registered_si,
                  priority=priority)
    ...
    def _get_constraint(self, sim):
        ...
        if geometric_constraint is None:
            ...
            return Anywhere()
        scoring_constraint = self.facing_restriction.create_constraint(sim,
          self, scoring_functions=(SocialGroupCostFunction(self, sim),))
    ...
    def _create_adjustment_alarm(self):
        ...
        elif self._recent_geometry_change():
    ...
    def _could_sim_route(self, sim):
        ...
        if not sim.posture.mobile:
            if not allow_posture_changes:
                return False
            elif sim.posture.owning_interactions:
                interactions_to_test = sim.posture.owning_interactions
            else:
                interactions_to_test = [
                 sim.posture.source_interaction]
            for interaction in interactions_to_test:
                if interaction is not None:
                    return interaction.satisfied or False

        else:
            constraint_intersection = sim.si_state.get_total_constraint(priority=(interactions.priority.Priority.Low),
              include_inertial_sis=True,
              force_inertial_sis=True,
              allow_posture_providers=(not allow_posture_changes))
            test_posture_intersection = constraint_intersection.intersect(STAND_OR_SIT_CONSTRAINT)
            return test_posture_intersection.valid or False
    ...
    def _can_sim_be_adjusted(self, sim, initial=False):
        if sim in self._non_adjustable_sims:
            return False
            if sim in self.geometry.members:
                if len(self.geometry.members) == 1:
                    return False
            ...
            if not initial:
                if not sim.queue.transition_in_progress:
                    if sim.is_moving:
                        return False
            ...
            if sim.in_non_adjustable_posture():
                return False
            return self._could_sim_route(sim) or False
        return True
    ...
    def execute_adjustment_interaction(self, sim_to_adjust, force_allow_posture_changes=False):
        ...
        if not allow_posture_changes:
            if not sim_to_adjust.posture.mobile:
        ...
        result = sim_to_adjust.execute_adjustment_interaction((self.SOCIAL_ADJUSTMENT_AFFORDANCE), constraint_to_satisfy,
          adjustment_prioriry,
          name_override='Satisfy[social_adjust]',
          allow_posture_changes=allow_posture_changes,
        ...
        if sim is not None:
            if sim.queue.has_adjustment_interaction():
                ...
                if not self.geometry or len(self.geometry) < 2:
                    return False
                if sim is None:
                    return self.adjust_sim_positions_dynamically or False
            if sim is not None:
                if sim not in self:
    ...
    def with_listener_focus(self, owner_sim, listener, *args):
        active_list = [
         (
          listener, self.FOCUS_LISTENER_SCORE)]
        focus_group = self._focus_group
    ...
    def _add_boredom(self, interaction):
        if not (interaction.is_super or interaction).visible or interaction.source != InteractionSource.PIE_MENU:
            return
        sim = interaction.sim
        ...
        for target_sim in interaction.get_participants(ParticipantType.TargetSim | ParticipantType.Listeners):
            ...
            for index, (affordance, count) in enumerate(self._boredom_registry[sim][target_sim]):
                if interaction.affordance is affordance:
                    self._boredom_registry[sim][target_sim][index] = (
                     affordance, count + 1)
                    break
            else:
    ...
    def get_potential_mixer_targets(self, sim):
        ...
        if self.group_leader_sim is None or sim is self.group_leader_sim:
            for sim_in_group in self:
                ...
                if si is not None:
                    si.is_finishing or potential_targets.add(sim_in_group)
    ...
    def get_social_animation_element(self, social_animation_ref):
        ...
        return unless(lambda : self._has_played_social_animation, build_element((set_has_played_social_animation,
         social_animation_ref)))