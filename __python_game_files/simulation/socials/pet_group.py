...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\pet_group.py
# Compiled at: 2017-09-19 17:50:32
...
from tag import Tag
import alarms, clock, services, sims4
...
class PetSocialGroup(SocialGroup):
    ...
    def _can_sim_be_adjusted(self, sim, initial=False):
        ...
        for group_sim in sim.get_main_group():
            if any((self._is_main_pet_social(interaction) and interaction.target is sim for interaction in group_sim.si_state)) or any((self._is_main_pet_social(interaction) and interaction.target is sim for interaction in group_sim.queue)):
                return False
    ...
    def _get_constraint(self, sim):
        ...
        for other_sim in self:
            if other_sim is sim:
                continue
            scoring_constraint = self.facing_restriction.create_constraint(sim, other_sim, scoring_functions=(
             self.scoring_function(sim, other_sim),))
    ...
    def is_locked_out(self, affordance):
        if self._is_main_pet_social(affordance):
            if self._lock_side_group_until is not None:
