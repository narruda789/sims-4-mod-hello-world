...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\throwing_group.py
# Compiled at: 2021-04-28 18:31:12
# Size of source mod 2**32: 7952 bytes
...
import alarms, clock, objects, sims4
...
class ThrowingSocialGroup(SocialGroup):
    INSTANCE_TUNABLES = {'adjustment_interval':TunableInterval(description='\n            How long Sims wait before adjusting positions.\n            ',
       ...
       default_lower=2,
       default_upper=5), 
     'scoring_function':ThrowingGroupCostFunction.TunableFactory(description='\n            The scoring function for this group.\n            '), 
    ...
    def get_objects_for_reset(self):
        reset_objects = super().get_objects_for_reset()
        if self._anchor_object in reset_objects:
            if isinstance(self._anchor_object, objects.terrain.TerrainPoint):
                reset_objects.remove(self._anchor_object)
    ...
    def _get_constraint(self, sim):
        ...
        for other_sim in self:
            ...
            scoring_constraint = self.facing_restriction.create_constraint(sim, facing_anchor, scoring_functions=(
             self.scoring_function(sim, other_sim, force_readjustment),))
