...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\social_tests.py
# Compiled at: 2020-11-12 10:43:21
# Size of source mod 2**32: 6982 bytes
import itertools
...
from sims4.tuning.tunable import HasTunableSingletonFactory, AutoFactoryInit, TunableEnumEntry, TunableSet, TunableMapping, Tunable
import caches, relationships.relationship_bit
...
class SocialContextTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def get_overall_short_term_context_bit(*sims):
        ...
        if prevailing_stc_tracks:
            ...
            for _, group in itertools.groupby(sorted(prevailing_stc_tracks, key=(lambda stc_track: stc_track.stat_type.type_id())),
              key=(lambda stc_track: stc_track.stat_type.type_id())):
                group = list(group)
                stc_magnitude = sum((stc_track.get_value() for stc_track in group)) / len(group)
                if prevailing_stc_track is None or abs(stc_magnitude) > abs(prevailing_stc_magnitude):
    ...
    def get_expected_args(self):
        return {'subject':self.participant, 
    ...
    def __call__(self, subject=(), target=()):
        ...
        if self.required_set:
            if social_context not in self.required_set:
        ...
        if self.context_score_mappings:
            score = self.context_score_mappings.get(social_context, 0)
            return TestResultNumeric(True, current_value=score, goal_value=0)
