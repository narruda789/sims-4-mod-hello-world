...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\jig_group.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 20357 bytes
from _collections import defaultdict
import interactions.constraints, routing, services, sims4.log, sims4.resources, socials.group
...
class JigGroup(SideGroup):
    INSTANCE_TUNABLES = {'jig':TunableJigVariant(), 
     'participant_slot_map':TunableMapping(description='\n            The slot index mapping on the jig keyed by participant type.\n            ',
    ...
    def _can_picked_object_be_jig(cls, picked_object):
        ...
        if picked_object.is_sim:
            return False
        if picked_object.carryable_component is not None:
            return False
        return True
    ...
    def _get_jig_transforms_gen(cls, initiating_sim, target_sim, picked_object=None, participant_slot_overrides=None):
        ...
        if cls._can_picked_object_be_jig(picked_object):
            ...
            if target_sim is not None and target_sim.routing_surface.type == SurfaceType.SURFACETYPE_POOL:
                ...
                extended_species = ocean is None or target_sim.in_pool or services.active_lot().is_position_on_lot(target_sim.position) or target_sim.extended_species
                age = target_sim.age
        ...
        elif max_pond_water_depth is not None:
        ...
        fgl_kwargs = {'stay_outside':cls.stay_outside,  'stay_in_enclosed_room':cls.stay_in_enclosed_room, 
        ...
        yield from cls.jig.get_transforms_gen(initiating_sim, target_sim, actor_slot_index=actor_slot_index,
    ...
    def make_constraint_default(cls, actor, target_sim, position, routing_surface, participant_type=ParticipantType.Actor, picked_object=None, participant_slot_overrides=None):
        if participant_type not in (ParticipantType.Actor, ParticipantType.TargetSim):
            ...
            return all_transforms or Nowhere('Unable to get constraints from jig.')
    ...
    def _set_sim_intended_location(self, sim, *, intended_location):
        for transform, locked_params in self._sim_transform_map.get(sim, ()):
            if transform_almost_equal_2d(transform, intended_location.transform):
                self._sim_routing_slot_map[sim] = (
                 transform, locked_params)
    ...
    def _consider_adjusting_sim(self, sim=None, initial=False):
        if not initial:
            for sim in self:
                ...
                if sis is not None:
                    if any((not si.staging for si in sis)):
                        return
