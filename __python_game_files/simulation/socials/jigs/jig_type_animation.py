...
# Compiled at: 2019-01-30 13:11:20
# Size of source mod 2**32: 9087 bytes
...
class SocialJigAnimation(AutoFactoryInit, HasTunableSingletonFactory):
    ...
    def _get_available_transforms_gen(self, actor, target):
        ...
        locked_params = {(
 'species', actor_name): SpeciesExtended.get_animation_species_param(actor.extended_species), 
         (
 'age', actor_name): actor_age.animation_age_param, 
         (
 'species', target_name): SpeciesExtended.get_animation_species_param(target.extended_species), 
         (
 'age', target_name): target_age.animation_age_param}
    ...
    def get_transforms_gen(self, actor, target, fallback_routing_surface=None, fgl_kwargs=None, **kwargs):
        ...
        for transform, jig_params in self._get_available_transforms_gen(actor, target):
            ...
            translation_a, orientation_a, translation_b, orientation_b, routing_surface = generate_jig_polygon(actor.location, transform.translation, actor_angle, target.location, Vector2.ZERO(), 0,
 ...
 reserved_space_b.left, reserved_space_b.right, reserved_space_b.front, reserved_space_b.back, fallback_routing_surface=fallback_routing_surface, 
             reverse_nonreletive_sim_orientation=self.reverse_actor_sim_orientation, **fgl_kwargs)
            if translation_a is None:
                continue
            yield (
             Transform(translation_a, orientation_a), Transform(translation_b, orientation_b), routing_surface, jig_params)
