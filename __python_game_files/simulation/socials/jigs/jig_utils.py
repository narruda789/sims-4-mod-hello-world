...
# Compiled at: 2020-11-30 19:39:13
# Size of source mod 2**32: 14065 bytes
import collections
from sims.sim_info_types import Species, Age, SpeciesExtended
import enum, placement, routing, sims4.log
...
SIMS_3_DISTANCE_MATRIX = {Species.HUMAN: {Species.HUMAN: 0.7, 
 ...
 Species.HUMAN, Age.CHILD): 0.7, 
                 Species.DOG: 1, 
                 (
 Species.DOG, Age.CHILD): 0.7, 
                 Species.CAT: 1, 
                 (
 Species.CAT, Age.CHILD): 0.7}, 
 ...
 Species.HUMAN, Age.CHILD): 0.7, 
                                Species.DOG: 1, 
                                (
 Species.DOG, Age.CHILD): 0.6, 
                                Species.CAT: 0.7, 
                                (
 Species.CAT, Age.CHILD): 0.6}, 
 
 Species.DOG: {Species.HUMAN: 1, 
               (
 Species.HUMAN, Age.CHILD): 1, 
               Species.DOG: 1, 
               (
 Species.DOG, Age.CHILD): 0.9, 
               Species.CAT: 1, 
               (
 Species.CAT, Age.CHILD): 0.9}, 
 
 (
 Species.DOG, Age.CHILD): {Species.HUMAN: 0.7, 
                              (
 Species.HUMAN, Age.CHILD): 0.7, 
                              Species.DOG: 1, 
                              (
 ...
 Species.CAT: {Species.HUMAN: 1, 
               (
 Species.HUMAN, Age.CHILD): 0.7, 
               Species.DOG: 1, 
               (
 Species.DOG, Age.CHILD): 0.5, 
               Species.CAT: 0.6, 
               (
 Species.CAT, Age.CHILD): 0.5}, 
 
 (
 Species.CAT, Age.CHILD): {Species.HUMAN: 0.7, 
                              (
 Species.HUMAN, Age.CHILD): 0.6, 
                              Species.DOG: 0.9, 
                              (
 Species.DOG, Age.CHILD): 0.3, 
                              Species.CAT: 0.5, 
                              (
 Species.CAT, Age.CHILD): 0.3}}

def get_sims3_social_distance(sim_a_species, sim_a_age, sim_b_species, sim_b_age):
    sim_a_key = (
     sim_a_species, sim_a_age)
...
DEFAULT_RESERVE_SPACE = {Species.HUMAN: ReserveSpace(0.5, 0.5, 0.5, 0.5), 
 (
 Species.HUMAN, Age.CHILD): ReserveSpace(0.5, 0.5, 0.5, 0.5), 
 Species.DOG: ReserveSpace(0.75, 1.0, 0.3, 0.3), 
 (
 Species.DOG, Age.CHILD): ReserveSpace(0.4, 0.5, 0.3, 0.3), 
 Species.CAT: ReserveSpace(0.4, 0.5, 0.3, 0.3), 
 (
 Species.CAT, Age.CHILD): ReserveSpace(0.2, 0.3, 0.2, 0.2), 
 SpeciesExtended.SMALLDOG: ReserveSpace(0.4, 0.5, 0.3, 0.3), 
 (
 SpeciesExtended.SMALLDOG, Age.CHILD): ReserveSpace(0.4, 0.5, 0.3, 0.3), 
 SpeciesExtended.FOX: ReserveSpace(0.4, 0.5, 0.3, 0.3), 
 (
 SpeciesExtended.FOX, Age.CHILD): ReserveSpace(0.4, 0.5, 0.3, 0.3)}
...
def _generate_single_poly_rectangle_points(sim_a_translation, sim_z_vector, sim_x_vector, a_left, a_right, a_front, a_back):
    all_points = [
     sim_a_translation + (sim_x_vector * -a_left + sim_z_vector * a_back),
     sim_a_translation + (sim_x_vector * a_right + sim_z_vector * a_back),
     sim_a_translation + (sim_x_vector * -a_left + sim_z_vector * -a_front),
     sim_a_translation + (sim_x_vector * a_right + sim_z_vector * -a_front)]
    polygon = sims4.geometry.Polygon(all_points)
    return polygon.get_convex_hull()
