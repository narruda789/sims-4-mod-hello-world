...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\clustering.py
# Compiled at: 2018-10-25 18:08:50
# Size of source mod 2**32: 33761 bytes
from contextlib import contextmanager
import itertools, math, weakref, time
...
import interactions.constraints, services, sims4.geometry, sims4.log, sims4.math
...
class ObjectClusterRequest(HasTunableFactory, AutoFactoryInit):
        ...
        for cluster in self.get_clusters_gen():
            for sub_constraint in constraint:
                ...
                if radius is not None:
                    if (constraint_position - cluster.position).magnitude_2d() > radius:
                        continue
    ...
    def _is_reachable_no_cache(self, a, b):
        ...
        if (a.position - b.position).magnitude_squared() <= self.density_epsilon:
            if self.facing_angle is not None:
                return self._is_facing(a, b) and self._is_facing(b, a) or False
    ...
    def _get_cluster_polygon(self, position, objects):
        ...
        if not len(polygon) == 2:
            if polygon.too_thin or polygon.too_small:
    ...
    def _generate_cluster(self, position, objects):
        ...
        los = LineOfSight(radius, (self.line_of_sight_constraint.map_divisions),
          ...
          debug_str_data=(
         '_generate_cluster LOS Constraint, Cluster Objects: {}', objects))
        ...
        convex_hull_constraint = Constraint(debug_name='ClusterConvexHull', routing_surface=routing_surface,
            ...
            if obj not in closed:
                ...
                if len(pending_neighbors) >= self.minimum_size - 1:
                    ...
                    while pending_neighbors:
                        ...
                        any((neighbor in c for c in itertools.chain(self._clusters, clusters))) or cluster.add(neighbor)
                ...
                for cluster in clusters:
                    ...
                    for obj in list(cluster):
                        if self.facing_angle is not None:
                            ...
                            is_facing = True
                        if not is_facing:
                            vector3_almost_equal_2d(centroid, (obj.position), epsilon=0.01) or cluster.remove(obj)
