...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\careers\career_base.py
# Compiled at: 2022-07-21 21:49:29
# Size of source mod 2**32: 247074 bytes
from _collections import defaultdict
from weakref import WeakKeyDictionary
import itertools, math, operator, random, weakref
...
from careers.career_enums import CareerOutfitGenerationType, CareerShiftType, ReceiveDailyHomeworkHelp, CareerCategory, CareerPanelType, GigResult, CareerEventDeclineOptions
...
import alarms, clock, date_and_time, distributor.system, enum, game_services, interactions.context, services, sims, sims4.log, sims4.math, telemetry_helper
...
class CareerBase(CoworkerMixin, CareerKnowledgeMixin, sims.sim_spawner_service.ISimSpawnerServiceCustomer):
    ...
    @property
    def is_late(self):
        if not self.is_work_time or self.currently_at_work or self.on_assignment:
            return False
    ...
    def get_is_quittable_shift(self, schedule_shift_type=CareerShiftType.ALL_DAY, career_category=None):
        if not career_category == CareerCategory.TeenSideHustle:
            if self.career_category == CareerCategory.TeenPartTime:
                return True
        if schedule_shift_type == self._current_shift_type or schedule_shift_type == CareerShiftType.ALL_DAY or self._current_shift_type == CareerShiftType.ALL_DAY:
            return True
        return False

    def is_time_during_shift(self, time=DEFAULT):
        if self._work_scheduler is None:
            if self._work_scheduler_override is None:
                return True
    ...
    def is_career_available(cls, inst, sim_info=DEFAULT, from_join=False):
        if inst is not None:
            if not inst._career_location.is_valid_career_location():
                return False
        inst_or_cls = inst if inst is not None else cls
        sim_info = inst_or_cls.sim_info if sim_info is DEFAULT else sim_info
        if not from_join:
            if inst_or_cls.career_category != CareerCategory.TeenSideHustle:
                if not inst_or_cls.allow_multiple_careers:
                    if any((career.guid64 != inst_or_cls.guid64 and career.career_category != CareerCategory.AdultPartTime and career.career_category != CareerCategory.Invalid and career.career_category != CareerCategory.TeenSideHustle and career.career_category == inst_or_cls.career_category for career in sim_info.career_tracker)):
                        return False
    ...
    def create_work_state_msg(self):
        ...
        if self.is_work_time:
            if self.taking_day_off:
                self._populate_work_state_time_off_msg(msg, (self.taking_day_off_reason), enable_go_to_work=(not self._sim_info.is_in_travel_group()))
            else:
                if self.currently_at_work or self.on_assignment:
            ...
            if self.career_panel_type == CareerPanelType.UNIVERSITY_COURSE:
                degree_tracker = self._sim_info.is_in_travel_group() or self._sim_info.degree_tracker
                if degree_tracker is not None:
                    if degree_tracker.allowed_to_enroll:
                        if not degree_tracker.course_infos:
                            msg.enable_go_to_work = True
            else:
                if self.career_panel_type == CareerPanelType.FREELANCE_CAREER:
                    msg.work_state = DistributorOps_pb2.SetAtWorkInfo.WORKDAY_OVER
                else:
                    if self.requested_day_off:
                        if self.requested_day_off_reason != career_ops.CareerTimeOffReason.WORK_FROM_HOME:
                        ...
                        if self.on_assignment:
                            if self._assignment_late:
                                ...
                                if self.has_completed_active_assignments:
                                    msg.work_state = DistributorOps_pb2.SetAtWorkInfo.WORKDAY_OVER
                                else:
                                    msg.work_state = self.has_attended_first_day or DistributorOps_pb2.SetAtWorkInfo.WORKDAY_OVER
                        else:
    ...
    def get_career_entry_data(self, career_history=None, career_track=None, user_level_override=None, career_level_override=None):
        ...
        if career_level_override is not None:
            if not isinstance(self, career_level_override.career):
                ...
                return (
                 career_level_override.level, career_level_override.user_level, career_level_override.track)
    ...
    def get_career_entry_level(cls, career_history=None, career_track=None, resolver=None):
        ...
        if career_history is None:
            ...
            if career_track is None:
            ...
            elif not histories or use_start_track:
                ...
                for history in histories:
                    if history.career_track is cls.start_track:
                        if len(histories) > 1:
                            continue
    ...
    def _loses_level_on_rejoin(cls, resolver=None):
        if resolver is not None:
            if FameTunables.CAREER_HOPPER_PERK is not None:
                ...
                if bucks_tracker is not None:
                    if bucks_tracker.is_perk_unlocked(FameTunables.CAREER_HOPPER_PERK):
                        return False
        return True
    ...
    def get_next_work_time(self, offset_time=None, check_if_can_go_now=False, consider_skipped_shifts=True, ignore_pto=False):
        ...
        if consider_skipped_shifts:
            if self.requested_day_off:
                valid_start_time = ignore_pto or start_time + TimeSpan.ONE
            else:
    ...
    def generate_outfit(self):
        if self.has_outfit():
            ...
            work_outfit.outfit_generator(self.sim_info, OutfitCategory.CAREER)
    ...
    def join_career(self, career_history=None, career_track=None, user_level_override=None, career_level_override=None, give_skipped_rewards=True, defer_rewards=False, schedule_shift_override=CareerShiftType.ALL_DAY, show_join_msg=True, disallowed_reward_types=(), force_rewards_to_sim_info_inventory=False, defer_first_assignment=False, schedule_init_only=False, allow_outfit_generation=True, show_icon_override_picker=True):
        new_level, new_user_level, current_track = self.get_career_entry_data(career_history=career_history,
        ...
        self._sim_info.career_tracker.update_history(self)
        self.career_start(schedule_init_only=schedule_init_only, allow_outfit_generation=allow_outfit_generation)
        ...
        if not show_icon_override_picker:
            if not UIUtils.get_hide_selected_notification_status():
                if show_join_msg and self.display_career_info:
                    if join_career_notification is not None:
    ...
    def career_start(self, is_load=False, is_level_change=False, schedule_init_only=False, allow_outfit_generation=True):
        ...
        self._work_scheduler = self.current_level_tuning.schedule(self,
          start_callback=(self._start_work_callback),
          schedule_immediate=schedule_immediate,
          early_warning_callback=(self.early_warning_callback),
          early_warning_time_span=early_warning_time_span,
          schedule_shift_type=(self._current_shift_type),
          init_only=schedule_init_only)
        ...
        if is_load:
            ...
            sim = self._get_sim()
            if allow_outfit_generation:
                if self.has_outfit():
                    sim_info = None if sim is None else sim.sim_info
                    work_outfit = self.current_level_tuning.get_work_outfit(sim_info)
                    work_outfit.generate_on_level_change or is_level_change or self.generate_outfit()
            if sim is not None:
                sim.update_sleep_schedule()
            services.get_event_manager().process_event((test_events.TestEvent.CareerEvent), sim_info=(self._sim_info),
              career=self)
    ...
    def career_stop(self, for_travel=False, is_level_change=False):
        if self._work_scheduler is not None:
            ...
            if not for_travel:
                self._work_scheduler = None
            elif self._current_gigs:
                for_travel or is_level_change or self.clear_all_gigs()
        else:
            if self._work_scheduler_override is not None:
            ...
            for_travel or self._remove_performance_statistics()
    ...
    def _load_days_worked_commodities(self, career_history, career_track):
        if self.guid64 in career_history:
            ...
            if history is not None:
                if career_track.guid64 in history:
    ...
    def _reset_performance_statistics(self):
        ...
        for metric in tuning.performance_metrics.statistic_metrics:
            if metric.reset_at_end_of_work:
                tracker = sim_info.get_tracker(metric.statistic)
                if tracker is None:
                    logger.error('Could not find statistic tracker for statistic {} when trying to reset performance statistic. Career: {} SimInfo:{}', (metric.statistic),
                      self, sim_info, owner='nsavalani')
                    return
    ...
    def _on_early_warning_alarm_response(self, dialog):
        ...
        if response == RESPONSE_ID_WORK_FROM_HOME:
            ...
            if response == RESPONSE_ID_TAKE_PTO:
                self.request_day_off(CareerTimeOffReason.PTO)
                self.add_pto(-1)
                self.resend_career_data()
            else:
                if response == RESPONSE_ID_CALL_IN_SICK:
    ...
    def _get_early_warning_alarm_responses(self):
        ...
        if message_tuning is None:
            return []
        responses = []
        responses.append(UiDialogResponse(dialog_response_id=RESPONSE_ID_GO_TO_WORK, text=(message_tuning.go_to_work_text),
          ui_request=(UiDialogResponse.UiDialogUiRequest.CAREER_GO_TO_WORK)))
        if self._offered_assignment_ids:
            responses.append(UiDialogResponse(dialog_response_id=RESPONSE_ID_WORK_FROM_HOME, text=(message_tuning.work_from_home_text),
              ui_request=(UiDialogResponse.UiDialogUiRequest.CAREER_WORK_FROM_HOME)))
        elif self.pto > 0:
            responses.append(UiDialogResponse(dialog_response_id=RESPONSE_ID_TAKE_PTO, text=(message_tuning.take_pto_text),
              ui_request=(UiDialogResponse.UiDialogUiRequest.CAREER_TAKE_PTO)))
        else:
            responses.append(UiDialogResponse(dialog_response_id=RESPONSE_ID_CALL_IN_SICK, text=(message_tuning.call_in_sick_text),
              ui_request=(UiDialogResponse.UiDialogUiRequest.CAREER_CALL_IN_SICK)))
        return responses
    ...
    def early_warning_callback(self):
        if not services.get_career_service().enabled:
            ...
            if self.should_skip_next_shift() or self._should_automatically_use_pto():
                return
            if self._requested_day_off_reason == CareerTimeOffReason.NO_TIME_OFF:
            ...
            if message_tuning is not None:
                self.send_career_message((message_tuning.dialog), additional_responses=(self._get_early_warning_alarm_responses()),
                  on_response=(self._on_early_warning_alarm_response))
    ...
    def _career_missing_work_response(self, dialog):
        if not dialog.accepted:
            if not dialog.response == ButtonType.DIALOG_RESPONSE_NO_RESPONSE:
                return
        sim = self._sim_info.get_sim_instance()
        if sim is None:
            return
    ...
    def _late_for_work_callback(self, _):
        ...
        if self._rabbit_hole_id:
            if services.get_rabbit_hole_service().is_in_rabbit_hole(self.sim_info.id, self._rabbit_hole_id):
                return
    ...
    def _start_work_callback(self, scheduler, alarm_data, extra_data):
        if self._at_work:
            ...
            if not services.get_career_service().enabled:
                ...
                if current_gig:
                    if current_gig.treat_work_time_as_due_date():
            ...
            if self._sim_info.is_in_travel_group():
                ...
                if not self.taking_day_off:
                    if not self._sim_info.is_in_travel_group():
                        sim = self._get_sim()
                        situation_jobs = None
                        if self._sim_info.is_npc:
                            if sim is not None:
                        ...
                        if sim is not None:
                            ...
                            for job in situation_jobs:
                                if job is not None:
                                    if job.confirm_leave_situation_for_work:
                                        responses = None
                                        if self.pto > 0:
                                            pto_response = UiDialogResponse(dialog_response_id=RESPONSE_ID_TAKE_PTO,
                                              text=(self.career_messages.situation_leave_confirmation.take_pto_button_text))
                                    ...
                                    def on_response(dialog):
                                        if dialog.accepted:
                                            ...
                                            if dialog.response == RESPONSE_ID_TAKE_PTO:
                                                self.request_day_off(CareerTimeOffReason.PTO)
                                                self.add_pto(-1)
                                                self.resend_career_data()

                                    self.send_career_message((self.career_messages.situation_leave_confirmation.dialog), on_response=on_response,
    ...
    def _try_offer_career_event(self):
        if services.get_persistence_service().is_save_locked():
            ...
            if self._sim_info.is_npc:
                return False
            if not self.allow_active_offlot:
            ...
            resolver = SingleSimResolver(self._sim_info)
            available_events = tuple((event for event in self.career_events if not self.is_career_event_on_cooldown(event) if event.tests.run_tests(resolver)))
    ...
    def on_career_event_declined(self, career_event):
        if self.requested_day_off or self.taking_day_off:
            return
        elif self.career_event_decline_option == CareerEventDeclineOptions.CAREER_RABBITHOLE:
            self.put_sim_in_career_rabbit_hole()
        else:
            if self.career_event_decline_option == CareerEventDeclineOptions.CANCEL_CURRENT_GIG:
                self.cancel_gig()
            else:
                if self.career_event_decline_option == CareerEventDeclineOptions.DO_NOTHING:
                    pass
    ...
    def _end_career_event(self, left_early=False):
        if self._career_event_manager is None:
            ...
            if current_gig is not None:
                if current_gig.extend_current_gig():
                    return
    ...
    def _end_gig_career_event(self, payout, gig):
        if gig is None:
            ...
            if result is not None:
                if result.evaluation == Evaluation.PROMOTED:
                    if not gig.uses_gig_notifications_for_promotions:
    ...
    def _build_regular_end_of_workday(self, payout, money_earned, pto_earned):
        ...
        op = EndOfWorkday(career_uid=(self.guid64), score_text=text,
          level_icon=(self._current_track.icon),
          money_earned=money_earned,
          paid_time_off_earned=pto_earned)
        if result is not None:
            if result.evaluation == Evaluation.PROMOTED:
                if result.dialog_factory is not None:
    ...
    def _end_regular_career_event(self, payout, left_early):
        ...
        if performance > 0:
            ...
            if not self.is_multi_sim_active:
        ...
        services.get_event_manager().process_event((test_events.TestEvent.WorkdayComplete), sim_info=(self._sim_info),
          career=self,
          time_worked=(span_worked.in_ticks()),
          money_made=money_earned)
    ...
    def _display_end_of_day_reports(self, left_early, payout):
        ...
        if not career_event is None:
            if career_event.end_of_day_reports is None or career_event.end_of_day_results_reported:
                return
        ...
        if not left_early:
            ...
            for sim_info in household:
                ...
                if current_career is not None:
                    if self.career_event_manager.is_same_current_event(current_career):
                ...
                if current_career_event is not None:
                    current_career_event.end_of_day_results_reported = current_career_event.end_of_day_results_reported or True
    ...
    def end_career_event_without_payout(self):
        ...
        if self.is_multi_sim_active:
            if not self._sim_info.is_npc:
                if self._sim_info.away_action_tracker is not None:
    ...
    def start_new_career_session(self, start_time, end_time):
        ...
        if not self.on_assignment:
            ...
            if gig is None or gig.odd_job_tuning is None:
                ...
                if tracker is None:
                    logger.error('Could not find statistic tracker for statistic {} on starting a new career session. Career: {} SimInfo:{}', (self.WORK_SESSION_PERFORMANCE_CHANGE),
                      self, (self._sim_info), owner='nsavalani')
            else:
                self._sim_info.add_statistic(self.WORK_SESSION_PERFORMANCE_CHANGE, self.WORK_SESSION_PERFORMANCE_CHANGE.initial_value)
            if self.is_school_career:
                if not self._sim_info.is_npc:
    ...
    def get_homework_help_percentage_helper(self, homework_help_enum):
        ...
        if homework_help_enum == ReceiveDailyHomeworkHelp.CHECKED_RECEIVE_HELP:
            return self.get_tuned_homework_progress()
        if homework_help_enum == ReceiveDailyHomeworkHelp.CHECKED_NO_HELP:
            return
    ...
    def get_homework_help_percentage(self):
        if self._sim_info.is_npc:
            ...
            return self.is_school_career or None
        ...
        if current_homework_help_status == ReceiveDailyHomeworkHelp.UNCHECKED:
            if not self.should_get_homework_help(current_homework_help_status):
                return
            return self.get_tuned_homework_progress()
        if current_homework_help_status == ReceiveDailyHomeworkHelp.CHECKED_RECEIVE_HELP:
            return self.get_tuned_homework_progress()
    ...
    def end_career_session(self):
        ...
        current_gig = self.get_current_gig()
        if not self.taking_day_off:
            if current_gig is None or current_gig.odd_job_tuning is None:
    ...
    def extend_career_session(self, stay_late_extension_override=None):
        if self._career_session_extended:
            ...
            return
        elif self._current_work_end is None:
    ...
    def leave_work(self, left_early=False):
        if self._career_event_manager is not None:
            ...
            if not self._sim_info.is_npc:
                if self._sim_info.away_action_tracker is not None:
                    self._sim_info.away_action_tracker.reset_to_default_away_action()
            left_early or self._try_invite_over()
    ...
    def _try_invite_over(self):
        if self.invite_over is None:
            ...
            return self.sim_info.is_npc or self.sim_info.is_at_home or None
            return self.sim_info.is_instanced(allow_hidden_flags=(HiddenReasonFlag.RABBIT_HOLE)) or None
        resolver = SingleSimResolver(self.sim_info)
        if random.random() > self.invite_over.chance.get_chance(resolver):
            return
        services.sim_filter_service().submit_filter((self.invite_over.sim_filter),
          (self._on_try_invite_over_sim_filter_response),
    ...
    def _send_workday_info_telemetry(self, work_type):
        with telemetry_helper.begin_hook(career_telemetry_writer, TELEMETRY_HOOK_CAREER_WORKDAY_START,
    ...
    def quit_career(self, post_quit_msg=True, update_ui=True):
    ...
    def can_change_level(self, demote=False):
        ...
        if new_level >= num_career_levels:
            if not self.current_track_tuning.branches:
                if self.current_track_tuning.overmax is None:
                    return False
        return True
    ...
    def _handle_promotion(self, previous_salary, previous_highest_level):
        ...
        if reward_text is None:
            return EvaluationResult((Evaluation.PROMOTED), (self.career_messages.promote_career_rewardless_notification), next_work_time, salary,
              salary_increase, None, display_career_info=(self.display_career_info))
        return EvaluationResult((Evaluation.PROMOTED), (self.career_messages.promote_career_notification), next_work_time, salary,
          salary_increase, None, reward_text, display_career_info=(self.display_career_info))
    ...
    def _demote(self, levels_to_demote=1):
        ...
        if self.can_be_fired:
            if current_performance <= current_level_tuning.fired_performance_level or self._level < levels_to_demote:
                return self._fire()
    ...
    def evaluate_career_performance(self, money_earned, pto_earned, gig=None):
        ...
        current_performance = self.work_performance
        resolver = SingleSimResolver(self._sim_info)
        if self.can_change_level(demote=True) or self.can_be_fired:
            if current_performance <= current_level_tuning.demotion_performance_level:
        ...
        if self.can_change_level(demote=False):
            ...
            aspiration_tracker = self._sim_info.aspiration_tracker
            if not promotion_aspiration is None:
                if aspiration_tracker is not None:
                    if aspiration_tracker.milestone_completed(promotion_aspiration):
    ...
    def handle_career_loot(self, hours_worked, left_early=False):
        if self.on_assignment:
            ...
            gig = self.get_current_gig()
            if gig:
                if not left_early or gig.odd_job_tuning is not None:
            ...
            if hours_worked <= 0:
                degree_tracker = self._sim_info.degree_tracker
                if degree_tracker is not None:
                    if degree_tracker.get_course_data(self.guid64) is not None:
                        self.process_homework()
            result = self.evaluate_career_performance(money_earned, pto_earned)
            if result is not None:
                result.display_dialog(self)
            self._send_telemetry(TELEMETRY_HOOK_CAREER_DAILY_END)
            span_worked = self.taking_day_off or create_time_span(hours=hours_worked)
            services.get_event_manager().process_event((test_events.TestEvent.WorkdayComplete), sim_info=(self._sim_info),
              career=self,
              time_worked=(span_worked.in_ticks()),
              money_made=money_earned)
    ...
    def send_career_message(self, dialog_factory, *additional_tokens, icon_override=None, on_response=None, display_career_info=False, additional_responses=None, **kwargs):
        ...
        if dialog is not None:
            ...
            (dialog.show_dialog)(additional_tokens=self.get_career_text_tokens() + additional_tokens, icon_override=icon_override, 
             secondary_icon_override=secondary_icon_override, 
             on_response=on_response, 
             career_args=career_args, **kwargs)
    ...
    def populate_set_career_op(self, career_op, gig=None):
        ...
        if self.career_panel_type == CareerPanelType.UNIVERSITY_COURSE and self.is_course_slot:
            ...
            if self.career_category == CareerCategory.UniversityCourse:
    ...
    def populate_set_career_op_with_data_for_played_sims(self, career_op, gig=None):
        ...
        if tooltip:
            ...
            if gig is None:
                gig = self.get_current_gig()
            elif gig is not None:
                gig.build_gig_msg((career_op.gig_info), (self._sim_info),
                  gig_time=(gig.get_gig_time()),
                  gig_customer=(gig.get_gig_customer()),
                  gig_extended_end_time=(gig.extended_end_time))
            elif gig is not None:
                ...
                elif gig is not None and self.current_gig_limit > 1:
    ...
    def sim_skewer_rabbit_hole_affordances_gen(self, context, **kwargs):
        for tone in self.get_available_tones_gen():
            yield AffordanceObjectPair(self.CAREER_TONE_INTERACTION,
 None,
 self.CAREER_TONE_INTERACTION,
 None, away_action=tone, 
             away_action_sim_info=self._sim_info, **kwargs)
        ...
        if self.career_event_manager is not None:
            if self.sim_info.is_npc:
                ...
                if services.current_zone_id() != home_zone_id:
                    ...
                    strat = SimSpawnPointStrategy(spawner_tags=(
    ...
    def get_assignments_to_offer(self, just_accepted=False, forced_assignment=None):
        ...
        if tutorial_service is not None:
            ...
            return weighted_assignments or []
        if not assignments:
        ...
        if num_assignments > len(weighted_assignments):
            logger.error('Bad tuning - number of active assignments is greater than the number of weighted assignments.', (self.active_assignment_amount),
              weighted_assignments, owner='shipark')
            return assignments
    ...
    def offer_assignments(self, forced_assignment=None):
        if self.on_assignment:
            ...
            loc_strings or logger.error("Assignment {} has no objectives to be offered so it can't be offered as a career assignment.", assignments)
        ...
        dialog.show_dialog(on_response=on_assignment_dialog_response, icon_override=IconInfoData(icon_resource=(self._current_track.icon)),
          additional_tokens=(
         assignment_text,))
    ...
    def load_from_persistable_sim_career_proto(self, proto, skip_load=False):
        ...
        if skip_load:
            ...
            if proto.HasField('called_in_sick'):
                ...
                if proto.HasField('requested_day_off_reason'):
    ...
    def cancel_gig(self, gig_id=None):
        if not self._current_gigs:
            ...
            if gig is None:
                ...
                return
            gig.notify_canceled()
            self.is_at_active_event or self.apply_gig_statistic_changes(gig)
            gig.collect_additional_rewards()
        ...
        if gig is None:
            ...
            if customer_id:
                ...
                if customer is None:
                    ...
                    return (None, None, None)
        return (
         follow_up_gig, schedule_time, customer)
    ...
    def _set_up_gig_timers(self, gig):
        if gig is None:
            gig = self.get_current_gig()
        elif not gig is None:
            if gig.gig_time is None:
                return
            if self.career_messages.career_early_warning_time is not None:
                early_warning_time_span = date_and_time.create_time_span(hours=(self.career_messages.career_early_warning_time))
        else:
            early_warning_time_span = None
        self._work_scheduler_override = gig.gig_time(schedule_immediate=True, start_callback=(self._start_work_callback),
          early_warning_callback=(self.early_warning_callback),
          early_warning_time_span=early_warning_time_span,
          required_start_time=(gig.get_gig_time()))
