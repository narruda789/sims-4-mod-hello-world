...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\careers\career_gig_ops.py
# Compiled at: 2020-04-10 15:49:48
...
class AddCareerGigOp(BaseLootOperation):
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        ...
        if self.gig_customer_filter is not None:
            results = services.sim_filter_service().submit_filter((self.gig_customer_filter),
              callback=None,
              allow_yielding=False,
              gsi_source_fn=(lambda : f"AddCareerGigOp{str(self)}"))
            if not results:
