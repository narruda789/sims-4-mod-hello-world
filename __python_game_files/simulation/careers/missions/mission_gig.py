...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\careers\missions\mission_gig.py
# Compiled at: 2021-05-21 05:58:11
# Size of source mod 2**32: 27952 bytes
...
class MissionGig(HomeAssignmentGig):
    ...
    def set_up_gig(self):
        ...
        for objective_data in self.mission_objective_data:
            ...
            if objective_data.is_invalid:
                continue
            if not self.allow_duplicate_objectives:
    ...
    def should_test_objective(self, objective):
        ...
        for objective_data in self.mission_objective_data:
            ...
            if mission_objective is not objective:
                ...
                continue
            if objective_data.requires_previous_objective_complete:
    ...
    def create_picker_row(cls, owner=None, gig_customer=None, customer_thumbnail_override=None, customer_background=None, enabled=True, customer_name=None, **kwargs):
        ...
        return OddJobPickerRow(customer_id=(gig_customer.id if gig_customer else 0),
          customer_description=customer_description,
          customer_thumbnail_override=customer_thumbnail_override,
          customer_background=customer_background,
          tip_title=(LocalizationHelperTuning.get_raw_text('')),
          tip_text=(LocalizationHelperTuning.get_raw_text('')),
          name=(display_name(owner)),
          icon=(cls.display_icon),
          row_description=(cls._get_reward_description()),
...
lock_instance_tunables(MissionGig, gig_prep_tasks=None,
  gig_time=None,
  gig_prep_time=None,
  gig_picker_localization_format=None,
  great_success_remaining_time=None,
  ...
  bonus_gig_aspiration_tuning=None,
  critical_failure_test=None)
    ...
    def create_picker_row(cls, owner=None, gig_customer=None, customer_thumbnail_override=None, customer_background=None, enabled=True, customer_name=None, **kwargs):
        ...
        if customer_name is None:
            if gig_customer:
                customer_name = LocalizationHelperTuning.get_sim_full_name(gig_customer)
        return OddJobPickerRow(customer_id=(gig_customer.id if gig_customer else 0),
          customer_description=picker_description,
          customer_thumbnail_override=customer_thumbnail_override,
          customer_background=customer_background,
          tip_title=(LocalizationHelperTuning.get_raw_text('')),
          tip_text=(LocalizationHelperTuning.get_raw_text('')),
          name=(display_name(owner)),
          icon=(cls.display_icon),
          row_description=(cls.get_pay_string(cls.gig_pay.lower_bound)),
          row_tooltip=row_tooltip,
          is_enable=enabled,
          customer_name=customer_name)
    ...
    def build_gig_msg(cls, inst, msg, sim, **kwargs):
        ...
        if inst is not None:
            ...
            for data in inst.mission_objective_data:
                if data.is_valid:
                    if not data.requires_previous_objective_complete:
