...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\careers\career_interactions.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 29259 bytes
import random
...
import alarms, clock, date_and_time, event_testing, interactions.base.super_interaction, interactions.rabbit_hole, services, sims4, terrain
...
class CareerSuperInteraction(interactions.base.super_interaction.SuperInteraction):
    ...
    def _test(cls, target, context, career_uid=None, skip_safe_tests=False, **kwargs):
        ...
        if career is None:
            ...
            return career.is_work_time or event_testing.results.TestResult(False, 'Not currently a work time for Sim({})', context.sim)
    ...
    def build_basic_elements(self, sequence=()):
        ...
        if career_level.fame_moment is not None:
            if self._should_run_fame_moment(career, career_level):
                self._start_fame_moment(career, career_level.fame_moment)
        if career.scholarship_info_loot is not None:
            if not career.seen_scholarship_info:
    ...
    def handle_event(self, sim_info, event, resolver):
        if self.has_been_canceled:
            ...
            self.unregister_for_scholarship_info_loot_callback()
            return
            if event == Career.SCHOLARSHIP_INFO_EVENT:
                if self._sim.sim_info is sim_info:
                    career = self.get_career()
                    if career is not None:
                        career.on_scholarship_info_shown()
        elif event == FameTunables.FAME_MOMENT_EVENT:
            if self._sim.sim_info is sim_info:
                career = self.get_career()
                if career is not None:
...
class CareerPickerSuperInteraction(PickerSingleChoiceSuperInteraction):
    ...
    INSTANCE_TUNABLES = {'continuation':OptionalTunable(description='\n            If enabled, you can tune a continuation to be pushed. PickedItemId\n            will be the id of the selected career.\n            ',
     ...
     'career_filter':TunableVariant(description='\n            Which career types to show.\n            ',
    ...
    def picker_rows_gen(cls, inst, target, context, **kwargs):
        ...
        for career in (inst_or_cls._valid_careers_gen)(target, context, **kwargs):
            ...
            row = ObjectPickerRow(name=(track.get_career_name(context.sim)), icon=(track.icon),
              row_description=(track.get_career_description(context.sim)),
              tag=career)
            yield row
        ...
        if context is None:
            ...
            if context.sim is not None:
                ...
                if career is not None:
                    ...
                    if career_interaction is not None:
...
class CareerTone(AwayAction):
    ...
    def run(self, callback):
        ...
        self._update_alarm_handle = alarms.add_alarm(self, time_span,
    ...
    def _update(self):
        ...
        if career is None:
            logger.error('CareerTone {} trying to update performance when Sim {} not at work', self,
              (self.sim_info), owner='tingyul')
            return
        gig = career.get_current_gig()
        if gig is not None:
            if gig.odd_job_tuning is not None:
                return
        ...
        for entry in self.periodic_sim_filter_loot:
            ...
            if random.random() > chance:
                continue
            services.sim_filter_service().submit_filter((entry.sim_filter),
