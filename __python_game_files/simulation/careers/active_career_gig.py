...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\careers\active_career_gig.py
# Compiled at: 2020-12-04 16:44:06
...
class ActiveGig(Gig):
    ...
    def _roll_cast(self):
        ...
        for potential_cast_member in self.gig_cast:
            if not potential_cast_member.filter_test.run_tests(resolver=resolver):
                continue
            generated_result = services.sim_filter_service().submit_matching_filter(sim_filter=(potential_cast_member.sim_filter), allow_yielding=False,
    ...
    def build_end_gig_dialog(self, payout):
        ...
        if medal_display_data is not None:
