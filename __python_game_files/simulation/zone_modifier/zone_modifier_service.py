...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\zone_modifier\zone_modifier_service.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 17599 bytes
import random
...
import alarms, services, sims4.telemetry, telemetry_helper
...
class ZoneModifierService(Service):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.SimActiveLotStatusChanged:
            ...
            zone_modifiers = self.get_zone_modifiers(services.current_zone_id())
            for zone_modifier in zone_modifiers:
                if not self._zone_mod_spin_up_state_complete:
                    if zone_modifier.ignore_route_events_during_zone_spin_up:
                        continue
    ...
    def check_for_and_apply_new_zone_modifiers(self, zone_id):
        ...
        if not removed_modifiers:
            if not added_modifiers:
                return
        if self._scheduler is not None:
    ...
    def _setup_zone_modifier_schedules(self):
        if self._scheduler is not None:
            self._scheduler.destroy()
        self._scheduler = ZoneModifierWeeklySchedule([], init_only=True,
          start_callback=(self._on_scheduled_alarm))
        zone_modifiers = self.get_zone_modifiers(services.current_zone_id())
        for zone_modifier in zone_modifiers:
