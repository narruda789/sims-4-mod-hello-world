...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\civic_policies\street_civic_policy_service.py
# Compiled at: 2020-06-23 23:07:00
# Size of source mod 2**32: 21400 bytes
...
import persistence_error_types, services, world, zone_types
...
class StreetService(Service, BaseCivicPolicyProviderServiceMixin):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.SimHomeZoneChanged:
            ...
            new_household_key = (
             sim_info.household_id, old_zone_id, new_zone_id)
        ...
        if not zone_ids:
            ...
            for street_data in neighborhood_proto.street_data:
                if street_data.world_id == zone_data.world_id:
                    return (
                     neighborhood_proto, street_data)

            return add or (None, None)
    ...
    def _get_open_notification(self):
        ...
        if provider is not None:
            if provider.new_enact_max_count() <= 0:
