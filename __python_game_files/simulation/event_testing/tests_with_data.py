...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\event_testing\tests_with_data.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 62187 bytes
...
from objects.object_tests import TagTestType, BasicStateCheckFactory
...
from tag import Tag
import build_buy, enum, event_testing.test_base, event_testing.test_events, services, sims4.resources, snippets
...
class ParticipantRanInteractionTest(event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'description':'Check to see if the Sim ran an affordance as a particular actor', 
     'participant':TunableEnumEntry(ParticipantType, ParticipantType.Actor, description='This is the role the sim in question should be to pass.'), 
     ...
     'target_filters':TunableTuple(description='\n            Restrictions on the target of this interaction.\n            ',
       ...
       object_states=OptionalTunable(description='\n                Limit test success only to target objects that are in certain states\n                ',
       tunable=(BasicStateCheckFactory()))), 
     ...
     'consider_user_cancelled_as_failure':Tunable(description='\n            If True, test will consider the interaction outcome to be Failure if\n            canceled by the user.\n            ',
       tunable_type=bool,
       default=True), 
     'consider_all_cancelled_as_failure':Tunable(description="\n            If True, test will consider the interaction outcome to be Failure if\n            canceled for any reason. If this box is checked and\n            consider_user_cancelled_as_failure is not checked, user cancel's\n            will still be treated as failures.\n            ",
       tunable_type=bool,
       default=False)}
    __slots__ = ('participant_type', '_affordances', '_affordance_lists', '_all_affordances',
                 'interaction_outcome', 'running_time', 'tags', 'object_tags', 'skill_tags',
                 'test_events', 'consider_user_cancelled_as_failure', 'consider_all_cancelled_as_failure',
                 'object_states')
    ...
    def __init__(self, participant, affordances, affordance_lists, interaction_outcome, running_time, skill_tags, target_filters, tags, test_event, consider_user_cancelled_as_failure, consider_all_cancelled_as_failure, **kwargs):
        ...
        self.object_states = target_filters.object_states
        if test_event == InteractionTestEvents.InteractionUpdate:
            self.test_events = (
             test_event, InteractionTestEvents.InteractionComplete)
        else:
            self.test_events = (
             test_event,)
    ...
    def get_expected_args(self):
        return {'sims':event_testing.test_constants.SIM_INSTANCE, 
    ...
    def __call__(self, sims=None, interaction=None):
        ...
        for sim_info in sims:
            ...
            if not tag_match:
                if not interaction.affordance in self._all_affordances:
                    return hasattr(interaction.affordance, 'proxied_affordance') and interaction.affordance.proxied_affordance in self._all_affordances or TestResult(False, 'Failed affordance check: {} not in {}', interaction.affordance, self._all_affordances)
            if self.skill_tags:
                interaction_skill = interaction.get_associated_skill()
                if not interaction_skill is None:
                    if len(self.skill_tags & set(interaction_skill.tags)) == 0:
            ...
            if self.object_tags is not None:
                if not self.target_matches_object_tags(interaction):
                    return TestResult(False, "Target of interaction didn't match object tag requirement.")
            if self.object_states is not None:
                if not self.target_matches_state_checks(interaction):
                    return TestResult(False, "Target of interaction didn't match state requirement.")
                if self.interaction_outcome is not None:
                    if self.consider_user_cancelled_as_failure:
                        if interaction.has_been_user_canceled:
                            if self.interaction_outcome != OutcomeResult.FAILURE:
                                return TestResult(False, 'Failed outcome check: interaction canceled by user treated as Failure')
                    if self.consider_all_cancelled_as_failure:
                        if interaction.has_been_canceled:
                            if self.interaction_outcome != OutcomeResult.FAILURE:
            ...
            elif self.consider_user_cancelled_as_failure:
                if interaction.has_been_user_canceled:
                    return TestResult(False, 'Failed outcome check: interaction canceled by user treated as Failure')
            if self.consider_all_cancelled_as_failure:
                if interaction.has_been_canceled:
    ...
    def target_matches_state_checks(self, interaction=None):
        if interaction is None or interaction.target is None or interaction.target.is_sim:
            return False
        return self.object_states(tested_object=(interaction.target))
...
class ParticipantStartedInteractionTest(event_testing.test_base.BaseTest):
    ...
    USES_EVENT_DATA = True
    FACTORY_TUNABLES = {'description':'Check to see if the Sim started an affordance as a particular actor', 
     'participant':TunableEnumEntry(ParticipantType, ParticipantType.Actor, description='This is the role the sim in question should be to pass.'), 
     'affordances':TunableList(TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION))), description="The Sim must have started either any affordance in this list or an interaction matching one of the tags in this tunable's Tags field."), 
     'tags':TunableSet(TunableEnumEntry(Tag, Tag.INVALID), description='The Sim must have run either an interaction matching one of these Tags or an affordance from the list of Affordances in this tunable.')}
...
class ParticipantRanAwayActionTest(event_testing.test_base.BaseTest):
    ...
    USES_EVENT_DATA = True
    FACTORY_TUNABLES = {'description':'Check to see if the Sim ran an away action', 
     'participant':TunableEnumEntry(ParticipantTypeActorTargetSim, ParticipantTypeActorTargetSim.Actor, description='This is the role the sim in question should be to pass.'), 
     'away_actions':TunableSet(description='\n            The Sim must have run an away action in this set\n            ',
       tunable=TunableReference((services.get_instance_manager(sims4.resources.Types.AWAY_ACTION)),
       pack_safe=True)), 
     'test_event':TunableEnumEntry(description='\n            The event that we want to trigger this instance of the tuned\n            test on.\n            AwayActionStart: Triggers when the away action starts.\n            AwayActionStop: Triggers when the away action stops.\n            ',
       tunable_type=AwayActionTestEvents,
       default=AwayActionTestEvents.AwayActionStop)}
    ...
    def get_expected_args(self):
        return {'sims':event_testing.test_constants.SIM_INSTANCE, 
         'away_action':event_testing.test_constants.FROM_EVENT_DATA}
...
class SkillTestFactory(TunableFactory):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(skill_to_test=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.STATISTIC)), description='The skill used to earn the Simoleons, if applicable.'), **kwargs)
...
class SimoleonsEarnedTest(event_testing.test_base.BaseTest):
    test_events = (
     event_testing.test_events.TestEvent.SimoleonsEarned,)
    USES_EVENT_DATA = True
    FACTORY_TUNABLES = {'description':'Require the participant(s) to (each) earn a specific amount of Simoleons for a skill or tag on an object sold.', 
     'event_type_to_test':TunableVariant(skill_to_test=SkillTestFactory(), tags_to_test=TagSetTestFactory(),
       description='Test a skill for an event or tags on an object.'), 
     'threshold':TunableThreshold(description='Amount in Simoleons required to pass'), 
     'household_fund_threshold':OptionalTunable(description='\n            Restricts test success based on household funds.\n            ',
    ...
    def get_expected_args(self):
        return {'sims':event_testing.test_constants.SIM_INSTANCE, 
         ...
         'skill_used':event_testing.test_constants.FROM_EVENT_DATA, 
        ...
        if self.household_fund_threshold is not None:
            for sim_info in sims:
                ...
                if household is None:
                    ...
                    return self.household_fund_threshold.threshold.compare(household_funds) or TestResult(False, 'Threshold test on household funds failed for sim {}', sim_info)
...
class FamilyAspirationTriggerTest(event_testing.test_base.BaseTest):
    test_events = (
     event_testing.test_events.TestEvent.FamilyTrigger,)
    USES_EVENT_DATA = True
    ...
    def get_expected_args(self):
        return {'sim_infos':ParticipantType.Actor, 
         'trigger':event_testing.test_constants.FROM_EVENT_DATA}
...
class WhimCompletedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    test_events = (
     event_testing.test_events.TestEvent.WhimCompleted,)
    USES_EVENT_DATA = True
    FACTORY_TUNABLES = {'whim_to_check': OptionalTunable(description='\n            Define them whim that is to be completed in order to pass the test.\n            ',
                        ...
                        enabled_name='Specific_Whim',
                        disabled_name='Any_Whim')}
    ...
    def __call__(self, whim_completed=None):
        ...
        if self.whim_to_check is not None:
            if self.whim_to_check.guid64 != whim_completed.guid64:
...
class OffspringCreatedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    test_events = (
     event_testing.test_events.TestEvent.OffspringCreated,)
    USES_EVENT_DATA = True
    ...
    def __call__(self, offspring_created=None):
        if offspring_created is None:
            ...
            return self.offspring_threshold.compare(offspring_created) or TestResult(False, 'OffspringCreatedTest: Not the desired amount of offspring created. {} {}', offspring_created, self.offspring_threshold)
        return TestResult.TRUE


class GenerationTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    test_events = (
     event_testing.test_events.TestEvent.GenerationCreated,)
    ...
    def __call__(self):
        if self.household_threshold is None:
            ...
            return self.generation_threshold.compare(generation_count) or TestResultNumeric(False, 'Generation Test: max generation is {}', generation_count, current_value=generation_count, goal_value=(self.goal_value()))
        else:
...
class CareerAttendenceTest(event_testing.test_base.BaseTest):
    test_events = (
     event_testing.test_events.TestEvent.WorkdayComplete,)
    USES_DATA_OBJECT = True
    USES_EVENT_DATA = True
    FACTORY_TUNABLES = {'description':'After a work day completes, did your sim work a desired of hours, earn a tuned amount (total over lifetime),                            at a specific or any career. Note: any career (leaving career untuned) means it checks against total of all of them.', 
     'career_to_test':TunableReference(manager=services.get_instance_manager(sims4.resources.Types.CAREER)), 
     'career_category':TunableEnumEntry(CareerCategory, CareerCategory.Invalid, description='Category the specified career is required to be in order to pass validation'), 
     'simoleons_earned':TunableThreshold(description='Amount in Simoleons required to pass'), 
     'hours_worked':TunableThreshold(description='Amount in hours required to pass')}
    ...
    def get_expected_args(self):
        return {'career':event_testing.test_constants.FROM_EVENT_DATA, 
         'data':event_testing.test_constants.FROM_DATA_OBJECT, 
    ...
    def __call__(self, career=None, data=None, objective_guid64=None):
        if career is None:
            ...
            total_money_made = 0
            total_time_worked = 0
            if self.career_to_test is not None and not isinstance(career, self.career_to_test):
            ...
            if not self.simoleons_earned.compare(total_money_made):
                return TestResultNumeric(False, 'CareerAttendenceTest: not the desired amount of Simoleons.', current_value=total_money_made,
                  goal_value=(self.simoleons_earned.value),
                  is_money=True)
            return self.hours_worked.compare(total_time_worked) or TestResultNumeric(False, 'CareerAttendenceTest: not the desired amount of time worked.', current_value=total_time_worked,
...
class TotalRelationshipBitTest(event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'description':'Gate availability by a relationship status.', 
     'use_current_relationships':Tunable(bool, False, description='Use the current number of relationships held at this bit rather than the total number ever had.'), 
     'relationship_bits':TunableSet(TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.RELATIONSHIP_BIT)), description='The relationship bit that will be checked.', class_restrictions='RelationshipBit')), 
    ...
    def __call__(self, data_object=None, objective_guid64=None):
        ...
        if relative_start_value is not None:
            ...
            return self.num_relations.compare(current_relationships) or TestResultNumeric(False, 'TotalRelationshipBitTest: Not enough relationships.',
...
class TotalTravelTest(event_testing.test_base.BaseTest):
    ...
    USES_DATA_OBJECT = True
    FACTORY_TUNABLES = {'description':'Gate availability by a relationship status.', 
     'number_of_unique_lots':Tunable(description='\n            The number of unique lots that this account has traveled to in order for this test to pass.',
       tunable_type=int,
       default=0)}
    ...
    def __call__(self, sims=None, data_object=None, objective_guid64=None):
        ...
        if zones_traveled >= self.number_of_unique_lots:
            return TestResult.TRUE
        return TestResultNumeric(False, 'TotalTravelTest: Not enough zones traveled to.',
          current_value=zones_traveled,
          goal_value=(self.number_of_unique_lots),
          is_money=False)
...
class TotalSimoleonsEarnedByTagTest(event_testing.test_base.BaseTest):
    ...
    USES_DATA_OBJECT = True
    FACTORY_TUNABLES = {'description':'Test for the total simoleons earned by selling objects tagged with tag_to_test.', 
     'tag_to_test':TunableEnumEntry(Tag, Tag.INVALID, description='The tags on the objects for selling.'), 
     'threshold':TunableThreshold(description='Amount in Simoleons required to pass')}
    ...
    def __call__(self, data_object=None, objective_guid64=None):
        ...
        if self.threshold.compare(total_simoleons_earned):
            return TestResult.TRUE
        return TestResultNumeric(False, 'TotalSimoleonsEarnedByTagTest: Not enough Simoleons earned on tag{}.',
          (self.tag_to_test), current_value=total_simoleons_earned,
          goal_value=(self.threshold.value),
          is_money=True)
...
class TotalTimeElapsedByTagTest(event_testing.test_base.BaseTest):
    test_events = (
     TestEvent.InteractionComplete, TestEvent.InteractionUpdate)
    USES_DATA_OBJECT = True
    FACTORY_TUNABLES = {'description':'Test for the total amount of time that interactions with tag_to_test has elapsed.', 
     'tag_to_test':TunableEnumEntry(Tag, Tag.INVALID, description='The tag on the interactions.'), 
     'length_of_time':TunableSimMinute(1, description='The total length of time that should be checked against.')}
    ...
    def get_custom_event_registration_keys(self):
        return [
         (
          TestEvent.InteractionComplete, self.tag_to_test),
         (
          TestEvent.InteractionUpdate, self.tag_to_test)]
    ...
    def __call__(self, data_object=None, objective_guid64=None):
        ...
        relative_start_value = data_object.get_starting_values(objective_guid64)
        if relative_start_value:
            time = 0
            total_time_elapsed -= interval_in_sim_minutes(relative_start_value[time])
        if total_time_elapsed >= self.length_of_time:
            return TestResult.TRUE
        return TestResultNumeric(False, 'TotalTimeElapsedByTagTest: Not enough time elapsed on tag{}.',
          (self.tag_to_test), current_value=(total_time_elapsed.in_hours()),
...
class InMultipleMoodsTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, data_object=None):
        ...
        for mood in self.required_moods:
            ...
            if last_time_in_mood is None or last_time_in_mood < required_time:
