...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\event_testing\event_data_object.py
# Compiled at: 2020-04-20 12:44:12
# Size of source mod 2**32: 39813 bytes
...
class ObjectiveData:
    ...
    def _get_objective_data(self, objective, should_add=True):
        ...
        if objective_uid not in self._stored_objective_count_data:
            if not should_add:
                return
            elif objective.data_type == ObjectiveDataStorageType.CountData:
                self._stored_objective_count_data[objective_uid] = ObjectiveData.CountData()
            else:
                if objective.data_type == ObjectiveDataStorageType.IdData:
                    self._stored_objective_count_data[objective_uid] = ObjectiveData.IdData()
                else:
                    logger.exception('Trying to add objective data for objective {} with invalid data type {}', objective,
                      (objective.data_type),
                      owner='jjacobson')
    ...
    def load(self, event_data_blob):
        ...
        for objective_data_proto in event_data_blob.objective_data:
            ...
            if objective is None:
                logger.debug('Objective of guid {} saved but not found in objective manager.  This is valid in the case of an uninstall.', (objective_data_proto.enum),
                  owner='jjacobson')
                continue
