...
# Size of source mod 2**32: 284119 bytes
...
class CollectionThresholdTest(event_testing.test_base.BaseTest):
 ...
 L. 147         0  LOAD_FAST                'test_targets'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_FALSE    18  'to 18'

 L. 148         8  LOAD_GLOBAL              TestResult
               10  LOAD_CONST               False
 ...
 L. 162       116  SETUP_LOOP          188  'to 188'
 ...
 L. 163       130  LOAD_FAST                'collection_id'
 ...
 L. 164       144  LOAD_FAST                'collection_tracker'
 ...
 L. 165       154  LOAD_FAST                'curr_value'
              156  LOAD_CONST               1
              158  INPLACE_ADD      
 ...
 L. 167       168  LOAD_FAST                'collection_tracker'
 ...
 L. 168       180  LOAD_FAST                'curr_value'
              182  LOAD_CONST               1
              184  INPLACE_ADD      
 ...
 L. 171       208  SETUP_LOOP          296  'to 296'
 ...
 L. 172       222  LOAD_FAST                'collection_id'
 ...
 L. 173       236  LOAD_FAST                'collection_tracker'
 ...
 L. 174       250  LOAD_FAST                'curr_value'
 ...
 L. 176       268  LOAD_FAST                'collection_tracker'
 ...
 L. 177       284  LOAD_FAST                'curr_value'
 ...
 L. 180       314  LOAD_GLOBAL              TestResult
              316  LOAD_ATTR                TRUE
              318  RETURN_VALUE     
            320_0  COME_FROM           310  '310'

 L. 182       320  LOAD_GLOBAL              Operator
 ...
 L. 183       336  LOAD_GLOBAL              TestResultNumeric
              338  LOAD_CONST               False
              340  LOAD_STR                 '{} failed collection check: {} {} {}'

 L. 184       342  LOAD_FAST                'self'
              344  LOAD_ATTR                who
              346  LOAD_ATTR                name

 L. 185       348  LOAD_FAST                'curr_value'

 L. 186       350  LOAD_FAST                'operator_symbol'

 L. 187       352  LOAD_FAST                'self'
              354  LOAD_ATTR                threshold
              356  LOAD_ATTR                value

 L. 188       358  LOAD_FAST                'curr_value'

 L. 189       360  LOAD_FAST                'self'
              362  LOAD_ATTR                threshold
              364  LOAD_ATTR                value

 L. 190       366  LOAD_CONST               False

 L. 191       368  LOAD_FAST                'self'
...
class CareerTimeUntilWorkTestFactory(SpecifiedCareerMixin, HasTunableSingletonFactory, AutoFactoryInit):
 ...
 L.4386        60  LOAD_FAST                'this_career'
               62  LOAD_CONST               None
               64  COMPARE_OP               is
               66  POP_JUMP_IF_FALSE    88  'to 88'

 L.4387        68  LOAD_GLOBAL              TestResult
               70  LOAD_CONST               False
 ...
 L.4398       176  LOAD_FAST                'time_span'
              178  LOAD_CONST               None
              180  COMPARE_OP               is
              182  POP_JUMP_IF_FALSE   212  'to 212'

 L.4399       184  LOAD_GLOBAL              TestResultNumeric
              186  LOAD_CONST               False

 L.4400       188  LOAD_STR                 '{0} does not currently have any hours scheduled in career {1}'

 L.4401       190  LOAD_FAST                'subject'

 L.4402       192  LOAD_FAST                'career'

 L.4403       194  LOAD_CONST               -1

 L.4404       196  LOAD_FAST                'self'
              198  LOAD_ATTR                hours_till_work
              200  LOAD_ATTR                lower_bound

 L.4405       202  LOAD_CONST               False

 L.4406       204  LOAD_FAST                'tooltip'
 ...
 L.4409       252  CONTINUE            100  'to 100'
            254_0  COME_FROM           250  '250'
            254_1  COME_FROM           244  '244'

 L.4410       254  LOAD_GLOBAL              TestResultNumeric
 ...
 L.4411       260  LOAD_FAST                'subject'
              262  LOAD_FAST                'career'
              264  LOAD_FAST                'self'
              266  LOAD_ATTR                hours_till_work

 L.4412       268  LOAD_FAST                'hours'

 L.4413       270  LOAD_FAST                'self'
              272  LOAD_ATTR                hours_till_work
              274  LOAD_ATTR                lower_bound

 L.4414       276  LOAD_CONST               False

 L.4415       278  LOAD_FAST                'tooltip'
 ...
 L.4417       292  LOAD_GLOBAL              TestResult
              294  LOAD_ATTR                TRUE
              296  RETURN_VALUE     
               -1  RETURN_LAST      
...
class CareerReferenceTestFactory(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value):
        if value.career is not None:
            if len(value.career_list) > 0:
                logger.error("Career Test in {} cannot have both 'career' and 'careers' set.", source, owner='mjuskelis')
    ...
    def __call__(self, subjects, tooltip=None, **kwargs):
        if len(self.career_list) > 0:
            if all((career is None for career in self.career_list)):
        ...
        for subject in subjects:
            ...
            if not subject.careers.values():
                return TestResult(False,
                  '{0} does not have any careers currently and a career is needed for this test: {1}/{2}:{3}',
                  subject,
                  (self.career),
                  (self.career_list),
                  (self.user_level),
                  tooltip=tooltip)
                for career in subject.careers.values():
                    if self._should_test_career(career) and career_test_variant(career):
...
class CareerPromotedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, career=None):
        if career is None:
            ...
            return self.careers.test_item(career) or TestResult(False, 'Career {}does not pass the White/Black list', career, tooltip=(self.tooltip))
        return TestResult.TRUE
