...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\distributor\ops.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 141184 bytes
import weakref
from protocolbuffers import Animation_pb2, DistributorOps_pb2, Routing_pb2, Sims_pb2, UI_pb2, Area_pb2, InteractionOps_pb2, Commodities_pb2, UI_pb2 as ui_ops, Clubs_pb2
from protocolbuffers.Consts_pb2 import MGR_UNMANAGED
import protocolbuffers.Audio_pb2, protocolbuffers.VFX_pb2
...
import distributor.fields, distributor.system, routing, services, sims4.color, sims4.hash_util, sims4.log
...
class Op:
    ...
    def add_additional_channel(self, manager_id, object_id, mask=None):
        if mask is None:
            mask = 4294967295
        channel = (
         manager_id, object_id, 0 if (self._force_execution_on_tag and manager_id != MGR_UNMANAGED) else mask)
...
class SetLocation(Op):
    ...
    def __repr__(self):
        return standard_repr(self, parent=(standard_brief_id_repr(self.op.parent_id)),
          slot_hash=(hex(self.op.slot_hash)),
          surface_object_id=(hex(self.op.surface_object_id)),
          joint_name_hash=(hex(self.op.joint_name_hash)),
          translation=(standard_float_tuple_repr(self.op.transform.translation.x, self.op.transform.translation.y, self.op.transform.translation.z)),
          orientation=(standard_float_tuple_repr(self.op.transform.orientation.x, self.op.transform.orientation.y, self.op.transform.orientation.z, self.op.transform.orientation.w)),
          level=(self.op.level))
...
def create_route_msg_src(route_id, actor, path, start_time, wait_time, track_override=None, mask_override=None):
    ...
    for n in path.nodes:
        ...
        if n.portal_object_id != 0:
            ...
            if last_routing_surface_id is None or last_routing_surface_id != n.routing_surface_id:
...
class OpenTutorial(Op):
    ...
    def __init__(self, tutorial_id=None):
        super().__init__()
...
class SetVisibility(Op):
    ...
    def write(self, msg):
        ...
        elif self.enable_drop_shadow is not None:
...
class SetBabySkinTone(Op):
    ...
    def __init__(self, skin_tone_and_cloth):
        super().__init__()
        self.baby_skin_tone = skin_tone_and_cloth[0]
        self.cloth_file_name = skin_tone_and_cloth[1]
...
class HouseholdCreate(GenericCreate):
    ...
    def __init__(self, obj, *args, is_active=False, **kwargs):
        ...
        additional_ops = (
         distributor.ops.SetMoney(obj.funds.money, False, 0),)
        if is_active:
...
class FollowRoute(Op):
    ...
    def write(self, msg):
        msg_src = distributor.ops.create_route_msg_src((self.path.nodes.id), (self.actor), (self.path),
          (self.start_time), 0, track_override=None, mask_override=(self.mask_override))
