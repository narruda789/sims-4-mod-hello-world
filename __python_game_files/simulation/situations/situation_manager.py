...
# Size of source mod 2**32: 92614 bytes
...
from situations.situation_types import SituationStage, GreetedStatus, SituationSerializationOption, SituationCategoryUid
from tag import Tag
...
class _SituationManagerSimData:
 ...
 L. 153         0  LOAD_FAST                'sim_job'
                2  LOAD_CONST               None
                4  COMPARE_OP               is-not
                6  POP_JUMP_IF_FALSE    52  'to 52'

 L. 154         8  SETUP_LOOP           74  'to 74'
 ...
 L. 157        52  LOAD_GLOBAL              SituationJob
...
class SituationManager(DistributableObjectManager):
    ...
    SITUATION_CATEGORY_PLAYER_PLANNED_DRAMA_NODE = sims4.tuning.tunable.TunableMapping(description='\n        A mapping of Situaiton Categories to Player Planned Drama Nodes used for scheduling.\n        If the Situation Category is found in this mapping, the corresponding Drama Node is used.\n        If the Situation Category is not found in this mapping, the tuned Default Player Planned Drama Node is used.\n        ',
      key_type=SituationCategoryUid,
      key_name='situation_category',
      ...
      class_restrictions=('PlayerPlannedDramaNode', )),
      value_name='drama_node')
    ...
    def enable_perf_cheat(self, enable=True):
        ...
        self.get(self._leave_situation_id).set_leave_enabled(enable)
    ...
    def create_situation(self, situation_type, guest_list=None, user_facing=True, duration_override=None, custom_init_writer=None, zone_id=0, scoring_enabled=True, spawn_sims_during_zone_spin_up=False, creation_source=None, travel_request_kwargs=frozendict(), linked_sim_id=GLOBAL_SITUATION_LINKED_SIM_ID, scheduled_time=None, existing_drama_node_uid=None, allow_uninstanced_main_traveler=False, **extra_kwargs):
        ...
        if guest_list is None:
            ...
            if host_sim_info is not None:
                if not existing_drama_node_uid:
                    ...
                    if zone_id:
                        if zone_id != current_zone_id and scheduled_time is None:
                            return (self._create_situation_and_travel)(situation_type,
 situation_id,
 guest_list,
 user_facing,
 duration_override,
 custom_init_writer,
 zone_id, scoring_enabled=scoring_enabled, 
                             creation_source=creation_source, 
                             linked_sim_id=linked_sim_id, 
                             travel_request_kwargs=travel_request_kwargs, 
                             allow_uninstanced_main_traveler=allow_uninstanced_main_traveler, 
 ...
 guest_list, user_facing=user_facing, 
        ...
        for situation in self.get_user_facing_situations_gen():
            if situation.linked_sim_id != GLOBAL_SITUATION_LINKED_SIM_ID:
                if guest_list is not None:
                    if guest_list.get_guest_info_for_sim_id(situation.linked_sim_id) is None:
                        continue
            if travel_request_situation is None:
                travel_request_situation = situation
                continue
            if situation.travel_request_behavior.restrict > travel_request_situation.travel_request_behavior.restrict:
                travel_request_situation = situation
    ...
    def _create_departing_seed_and_travel(self, situation_type, situation_id, guest_list=None, user_facing=True, duration_override=None, custom_init_writer=None, zone_id=0, scoring_enabled=True, creation_source=None, linked_sim_id=GLOBAL_SITUATION_LINKED_SIM_ID, allow_uninstanced_main_traveler=False, spawn_sims_during_zone_spin_up=False, **extra_kwargs):
        ...
        if not allow_uninstanced_main_traveler:
            if traveling_sim_info.client is None:
 ...
 zone_id, scoring_enabled=scoring_enabled, 
    ...
    def send_situation_start_ui_for_edit(self, actor, drama_node_uid):
        ...
        if situation_seed.special_object_definition_id is not None:
            ...
            elif situation_seed.guest_attire_color:
