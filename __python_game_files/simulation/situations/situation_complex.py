...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\situation_complex.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 48089 bytes
...
from tag import Tag
import alarms, autonomy.autonomy_modes, autonomy.autonomy_request, clock, event_testing, gsi_handlers, interactions.context, interactions.priority, services, sims4.log, situations
...
class SituationComplex(RegisterTestEventMixin, Situation):
    ...
    def test_interaction_complete_by_job_holder(self, sim_info, resolver, job_type, test):
        ...
        if sim is None:
            ...
            return self.sim_has_job(sim, job_type) or False
    ...
    def _choose_role_interaction(self, sim, push_priority=interactions.priority.Priority.High, run_priority=interactions.priority.Priority.High, allow_failed_path_plans=False):
        context = interactions.context.InteractionContext(sim,
          (interactions.context.InteractionSource.AUTONOMY), push_priority, run_priority=run_priority)
        ...
        autonomy_request = autonomy.autonomy_request.AutonomyRequest(sim,
          autonomy_mode=(autonomy.autonomy_modes.FullAutonomy), skipped_static_commodities=(sim.autonomy_component.standard_static_commodity_skip_set),
...
class SituationState:
    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        return (
         role_state_type, role_affordance_target)
    ...
    def _create_or_load_alarm_with_timespan(self, alarm_name, time_span, callback, repeating=False, use_sleep_time=False, should_persist=True, reader=None):
        ...
        alarm_handle = alarms.add_alarm(self, time_span, callback, repeating=repeating,
          use_sleep_time=use_sleep_time)
...
class CommonSituationState(ComplexSituationEffectTriggeringSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if self._time_out is not None:
            self._create_or_load_alarm((self._time_out_string), (self._time_out),
...
class CommonInteractionCompletedSituationState(CommonSituationState):
    ...
    def is_sim_running_interaction_of_interest(self, sim):
        if self._interaction_of_interest.tags:
            if sim.has_any_interaction_running_or_queued_of_tags(self._interaction_of_interest.tags):
                return True
        if self._interaction_of_interest.affordances:
            if sim.has_any_interaction_running_or_queued_of_types(tuple((affordance.get_interaction_type() for affordance in self._interaction_of_interest.affordances))):
                return True
        return False
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionComplete:
            for interaction_tuning in self._interactions_of_interest:
                if interaction_tuning in self._completed_interactions:
                    continue
                if resolver(interaction_tuning) and self._additional_tests(sim_info, event, resolver):
...
class TunableSituationJobAndRoles(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'jobs_and_roles': TunableMapping(description="\n            A mapping between a situation's jobs and default role states.\n            ",
                         ...
                         manager=(services.get_instance_manager(sims4.resources.Types.SITUATION_JOB))),
                         key_name='Situation Job',
                         value_type=TunableTuple(number_of_sims_to_find=TunableRange(description='\n                    The number of sims to find for this job.\n                    ',
                         tunable_type=int,
                         default=1,
                         minimum=1),
...
class SituationJobAndRoleState:
    FACTORY_TUNABLES = {'situation_job':TunableReference(manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB), description='A reference to a SituationJob that can be performed at this Situation.'), 
     'role_state':TunableReference(manager=services.get_instance_manager(sims4.resources.Types.ROLE_STATE), description='A role state the sim assigned to the job will perform')}
...
class SituationComplexCommon(SituationComplex):
    ...
    def _verify_tuning_callback(cls):
        if cls._resident_job is not None:
            if cls.resident_job() not in cls.get_tuned_jobs():
    ...
    def _change_state(self, new_state, reader=None):
        if False:
            if self.situation_serialization_option != situations.situation_types.SituationSerializationOption.DONT:
                if self._state_to_uid(new_state) == self.INVALID_STATE_UID:
