...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\situation.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 72518 bytes
import alarms, build_buy, clock, distributor, event_testing.resolver, event_testing.test_variants, random, services, sims.sim_info_tests, sims4.log, sims4.resources, situations.bouncer.bouncer_types, statistics.skill_tests, travel_group.travel_group_tests, ui.screen_slam
...
class TunableSituationInitiationTestVariant(TunableVariant):
  ...
  locked_args={'filter_target':None, 
 'relative_sim':ParticipantTypeActorTargetSim.Actor}), 
         test_all_sims_against_filter=sims.sim_info_tests.FilterTest.TunableFactory(description='\n            Test all sims to see if there are any sims that match the filter.\n            Passes if any sims match the filter.\n            ',
  locked_args={'filter_target':None, 
 'relative_sim':ParticipantTypeActorTargetSim.Actor}), 
         statistic=event_testing.statistic_tests.StatThresholdTest.TunableFactory(participant_type_override=(
 ParticipantTypeActorTargetSim,
 ParticipantTypeActorTargetSim.Actor)), 
         skill_tag=statistics.skill_tests.SkillTagThresholdTest.TunableFactory(participant_type_override=(
 ParticipantTypeActorTargetSim,
 ParticipantTypeActorTargetSim.Actor)), 
         sim_info=sims.sim_info_tests.SimInfoTest.TunableFactory(participant_type_override=(
 ParticipantTypeActorTargetSim,
 ParticipantTypeActorTargetSim.Actor),
  locked_args={'can_age_up': None}), 
         trait=sims.sim_info_tests.TraitTest.TunableFactory(participant_type_override=(
 ParticipantTypeActorTargetSim,
 ParticipantTypeActorTargetSim.Actor)), 
         unlock=event_testing.test_variants.TunableUnlockedTest(unlock_type_override={'allow_achievment': False}), 
         travel_group=travel_group.travel_group_tests.TravelGroupTest.TunableFactory(locked_args={'participant': ParticipantTypeActorTargetSim.Actor}), 
...
class TunableNPCHostedPlayerTestVariant(TunableVariant):
    ...
    def __init__(self, description='A single tunable test.', **kwargs):
        (super().__init__)(statistic=event_testing.statistic_tests.StatThresholdTest.TunableFactory(participant_type_override=(
 ParticipantTypeActorTargetSim,
 ParticipantTypeActorTargetSim.Actor),
 ...
 ParticipantTypeActorTargetSim.Actor),
  ...
  locked_args={'tooltip': None}), 
         ...
         region_test=event_testing.test_variants.RegionTest.TunableFactory(), 
         description=description, **kwargs)
...
class TunableMainGoalVisibilityTestVariant(TunableVariant):
    ...
    def __init__(self, description='A single tunable test.', **kwargs):
        (super().__init__)(situation_running_test=event_testing.test_variants.TunableSituationRunningTest(locked_args={'participant':None, 
 'check_for_initiating_sim':False}), 
         description=description, 
         default='situation_running_test', **kwargs)
...
NPCHostedSituationResult = namedtuple('NPCHostedSituationResult', ('player_sim_info',
                                                                   'host_sim_info',
                                                                   'failure_reason'))
...
class Situation(SituationGoalTuningMixin, SituationLevelDataTuningMixin, BaseSituation, HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SITUATION)):
    ...
    INSTANCE_TUNABLES = {'category':TunableEnumEntry(description='\n            The Category that the Situation belongs to.\n            ',
     ...
     '_icon':TunableResourceKey(description='\n            Icon to be displayed in the situation UI.\n            ',
       ...
       allow_none=True,
       tuning_group=GroupNames.UI), 
     'calendar_icon':TunableIconAllPacks(description='\n            Icon to be displayed in the calendar UI.\n            ',
       allow_none=True,
       tuning_group=GroupNames.UI), 
     ...
     'job_display_ordering':OptionalTunable(description='\n            An optional list of the jobs in the order you want them displayed\n            in the Plan an Event UI.\n            ',
       tunable=TunableList(tunable=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.SITUATION_JOB)))),
       tuning_group=GroupNames.UI), 
     'end_situation_dialog':OptionalTunable(description='\n            An optional UI Dialog to use when ending a user facing situation\n            ',
     ...
     'display_style':TunableEnumEntry(description='\n            The style overlay to apply to this situation.\n            ',
       tunable_type=SituationDisplayStyle,
       default=SituationDisplayStyle.DEFAULT,
       tuning_group=GroupNames.UI), 
    ...
    SITUATION_SCORING_REMOVE_INSTANCE_TUNABLES = ('main_goal', '_main_goal_visibility_test',
                                                  'minor_goal_chains', 'main_goal_audio_sting',
                                                  'goal_sub_text', 'goal_button_text',
                                                  'highlight_first_incomplete_minor_goal',
                                                  'suppress_scoring_progress_bar',
                                                  '_level_data', 'screen_slam_gold',
                                                  'screen_slam_silver', 'screen_slam_bronze',
                                                  'screen_slam_no_medal')
    SITUATION_START_FROM_UI_REMOVE_INSTANCE_TUNABLES = ('_cost', 'compatible_venues',
                                                        'venue_invitation_message',
                                                        'venue_situation_player_job',
                                                        'category', 'max_participants',
                                                        '_initiating_sim_tests',
                                                        '_icon', 'entitlement', 'job_display_ordering')
    SITUATION_USER_FACING_REMOVE_INSTANCE_TUNABLES = ('_display_name', 'end_situation_dialog',
                                                      'travel_request_behavior',
                                                      'recommended_job_object_notification',
                                                      'recommended_job_object_text',
                                                      'situation_description')
    NON_USER_FACING_REMOVE_INSTANCE_TUNABLES = ('_buff', 'targeted_situation', '_resident_job',
                                                '_relationship_between_job_members',
                                                'audio_sting_on_start', 'background_audio',
                                                'force_invite_only') + SITUATION_SCORING_REMOVE_INSTANCE_TUNABLES + SITUATION_START_FROM_UI_REMOVE_INSTANCE_TUNABLES + SITUATION_USER_FACING_REMOVE_INSTANCE_TUNABLES
    SITUATION_EVENT_REMOVE_INSTANCE_TUNABLES = ('_buff', '_cost', 'venue_invitation_message',
                                                'venue_situation_player_job', 'category',
                                                'main_goal', '_main_goal_visibility_test',
                                                'minor_goal_chains', 'goal_sub_text',
                                                'goal_button_text', 'highlight_first_incomplete_minor_goal',
                                                'suppress_scoring_progress_bar',
                                                'max_participants', '_initiating_sim_tests',
                                                '_icon', 'targeted_situation', '_resident_job',
                                                'situation_description', 'job_display_ordering',
                                                'entitlement', '_relationship_between_job_members',
                                                'main_goal_audio_sting', 'audio_sting_on_start',
                                                'background_audio', '_level_data',
                                                '_display_name', 'end_situation_dialog',
                                                'screen_slam_gold', 'screen_slam_silver',
                                                'screen_slam_bronze', 'screen_slam_no_medal',
                                                'force_invite_only', 'recommended_job_object_notification',
                                                'recommended_job_object_text', 'travel_request_behavior')
    ...
    def _verify_tuning_callback(cls):
        if cls._resident_job is not None:
            if cls._resident_job.filter is None:
                logger.error('Resident Job: {} has no filter,', (cls._resident_job), owner='manus')
        if cls.targeted_situation is not None:
            if cls.targeted_situation.target_job is None or cls.targeted_situation.actor_job is None:
        ...
        for job_relationships in cls.relationship_between_job_members:
            if job_relationships.job_x not in tuned_jobs:
                logger.error('job_x: {} has relationship tuning but is not functionally used in situation {}.', (job_relationships.job_x),
                  cls, owner='manus')
            if job_relationships.job_y not in tuned_jobs:
                logger.error('job_y: {} has relationship tuning but is not functionally used in situation {}.', (job_relationships.job_y),
                  cls, owner='manus')
            if len(job_relationships.relationship_bits_to_add) == 0:
                logger.error("relationship_bits_to_add cannot be empty for situation {}'s job pairs {} and {}.", cls,
                  (job_relationships.job_x), (job_relationships.job_y), owner='manus')
            else:
                for bit in job_relationships.relationship_bits_to_add:
                    if bit is None:
                        logger.error("relationship_bits_to_add cannot contain empty bit for situation {}'s job pairs {} and {}.", cls,
                          (job_relationships.job_x), (job_relationships.job_y), owner='manus')
                    ...
                    for guest_id in guest_ids:
                        ...
                        if guest_info is not None:
                            ...
                            if guest_zone_id is not None:
                                if guest_zone_id:
                                    if guest_zone_id not in possible_zones:
    ...
    def get_prepopulated_job_for_sims(cls, sim, target_sim_id=None):
        if target_sim_id:
            if cls.targeted_situation is not None:
                ...
                prepopulated = [
                 (
                  sim.id, cls.targeted_situation.actor_job.guid64),
                 (
                  target_sim_id, cls.targeted_situation.target_job.guid64)]
                return prepopulated
    ...
    def end_audio_sting(self):
        ...
        if level_data is not None:
            if level_data.audio_sting_on_end is not None:
    ...
    def _load_situation_states_and_phases(self):
        ...
        if not self._seed.goal_tracker_seedling:
            return
        elif self._seed.goal_tracker_seedling.goal_tracker_type == GoalTrackerType.STANDARD_GOAL_TRACKER:
            self._goal_tracker = situations.situation_goal_tracker.SituationGoalTracker(self)
        else:
            if self._seed.goal_tracker_seedling.goal_tracker_type == GoalTrackerType.DYNAMIC_GOAL_TRACKER:
                self._goal_tracker = situations.dynamic_situation_goal_tracker.DynamicSituationGoalTracker(self)
    ...
    def is_situation_available(cls, initiating_sim, target_sim_id=0):
        ...
        if is_targeted and target_sim_id:
            return cls.targeted_situation.target_job.can_sim_be_given_job(target_sim_id, initiating_sim.sim_info) or TestResult(False)
        else:
            if (target_sim_id == 0) != (is_targeted == False):
    ...
    def is_venue_location_valid(cls, zone_id):
        ...
        return services.current_zone().venue_service.get_zone_venue_type_valid_for_venue_types(zone_id, (cls.compatible_venues),
          compatible_region=compatible_region) is not None
