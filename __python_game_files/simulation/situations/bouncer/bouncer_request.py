...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\bouncer\bouncer_request.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 32733 bytes
...
import clock, gsi_handlers.situation_handlers, services, sims, sims4.log, situations.bouncer.bouncer, world
...
def exclusivity_compare(current_request, other_request):
    ...
    if option == BouncerExclusivityOption.EXPECTATION_PREFERENCE:
        if current_request._expectation_preference:
            if not other_request._expectation_preference:
                return determine_result(current_request._exclusivity)
        else:
            if not current_request._expectation_preference:
                if other_request._expectation_preference:
...
class BouncerRequest:
    ...
    def __init__(self, situation, callback_data, job_type, request_priority, user_facing, exclusivity, requested_sim_id=0, accept_alternate_sim=False, blacklist_sim_ids=None, spawning_option=RequestSpawningOption.DONT_CARE, requesting_sim_info=None, expectation_preference=False, common_blacklist_categories=0, for_persisted_sim=False, elevated_importance_override=False, accept_looking_for_more_work=False, specific_spawn_point=None, specific_location=None, reassign_within_situation=False):
        ...
        self._reassign_within_situation = reassign_within_situation
    ...
    def _can_spawn_now(self, during_zone_spin_up_service):
        if not self._allows_spawning:
            ...
            return during_zone_spin_up_service or True
    ...
    def spawner_tags(self, during_zone_spin_up=False):
        if not self._situation.use_spawner_tags_on_travel:
            if during_zone_spin_up:
                if self._situation.is_traveling_situation:
                    return (
                     world.spawn_point.SpawnPoint.ARRIVAL_SPAWN_POINT_TAG,)
    ...
    def should_preroll_during_zone_spin_up(self):
        return self._situation.should_preroll_during_zone_spin_up
    ...
    def clone_for_replace(self, only_if_explicit=False):
        if only_if_explicit:
            if not self._is_explicit:
                return
        request = BouncerRequest((self._situation), (self._callback_data),
          (self._job_type),
...
class BouncerRequestFactory(BouncerRequest):
    ...
    def __init__(self, situation, callback_data, job_type, request_priority, user_facing, exclusivity, requesting_sim_info=None):
        super().__init__(situation, callback_data, job_type=job_type, request_priority=request_priority,
    ...
    def _create_request(self, sim):
        request = BouncerRequest((self._situation), (self._callback_data),
          ...
          user_facing=(self._user_facing),
          exclusivity=(self._exclusivity),
          requested_sim_id=(sim.id),
          blacklist_sim_ids=(self._blacklist_sim_ids),
          spawning_option=(RequestSpawningOption.CANNOT_SPAWN),
          requesting_sim_info=(self._requesting_sim_info),
...
class BouncerFallbackRequestFactory(BouncerRequestFactory):
    ...
    def __init__(self, situation, callback_data, job_type, user_facing, exclusivity):
        super().__init__(situation, callback_data,
          job_type=job_type,
...
class BouncerHostRequestFactory(BouncerRequestFactory):
    ...
    def __init__(self, situation, callback_data, job_type, user_facing, exclusivity, requesting_sim_info):
        super().__init__(situation, callback_data, job_type=job_type, request_priority=(BouncerRequestPriority.EVENT_HOSTING),
          user_facing=user_facing,
          exclusivity=exclusivity,
          requesting_sim_info=requesting_sim_info)
