...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\patient_situation_base.py
# Compiled at: 2019-08-30 16:31:12
# Size of source mod 2**32: 14449 bytes
...
class PatientSituationBase(SituationComplexCommon):
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        if cls.arriving_state is not None:
            return [
             (
              cls.situation_job, cls.arriving_state.arriving_role_state)]
        return [
         (
          cls.situation_job, cls.waiting_role_state)]
    ...
    def start_situation(self):
        ...
        if reader is None:
            services.current_zone().is_zone_running or self._skip_ahead_for_preroll()
        else:
            if self.arriving_state:
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event is TestEvent.InteractionComplete:
            if patient is not None:
                if sim_info is patient.sim_info:
                    if resolver(self.owner.arriving_state.go_to_waiting_interactions):
                        self._change_state(WaitingState())
...
class WaitingState(SituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm(WAITING_TIMEOUT, (self.owner.waiting_timeout), (lambda _: self.timer_expired()),
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event is TestEvent.InteractionComplete:
            if patient is not None:
                if sim_info is patient.sim_info:
                    if resolver(self.owner.go_to_admitted_interactions):
    ...
    def timer_expired(self):
        ...
        patient = self.owner.get_patient()
        if patient is not None:
            if self.owner.force_sim_to_leave_lot_on_completion:
