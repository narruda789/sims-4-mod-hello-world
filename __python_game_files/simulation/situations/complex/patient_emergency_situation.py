...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\patient_emergency_situation.py
...
class PatientEmergencySituation(PatientSituationBase):
    ...
    def _states(cls):
        return (SituationStateData(1, ArrivingState),
         ...
         SituationStateData(3, _ProcedureState),
         SituationStateData(4, TreatedState))
    ...
    def should_state_type_load_after_time_jump(cls, state_type):
        if not super().should_state_type_load_after_time_jump(state_type):
            return False
        if state_type is _ProcedureState:
