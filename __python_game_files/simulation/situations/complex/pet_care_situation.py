...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\pet_care_situation.py
...
class PetCareSituation(SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(cls.WAIT_FOR_PET_CARE_WORKER_STATE_UID, _WaitForPetCareWorkerState),
         SituationStateData((cls.PET_ACCESSIBLE_ARRIVAL_STATE_UID), _PetAccessibleArrivalState, factory=(cls.pet_accessible_arrival_state)),
         SituationStateData((cls.PET_INACCESSIBLE_ARRIVAL_STATE_UID), _PetInaccessibleArrivalState, factory=(cls.pet_inaccessible_arrival_state)),
         SituationStateData((cls.PET_ROUTES_TO_CRATE_STATE_UID), _PetRoutesToCrateState, factory=(cls.pet_routes_to_crate_state)),
         SituationStateData((cls.LEAVE_STATE_UID), _LeaveState, factory=(cls.leave_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.pet_care_worker_job_and_role_state.job, cls.pet_care_worker_job_and_role_state.role_state),
         (
          cls.pet_job_and_role_state.job, cls.pet_job_and_role_state.role_state)]
    ...
    def _on_add_sim_to_situation(self, *args, **kwargs):
        ...
        pet = self.pet_sim()
        if pet_care_worker is not None:
            if pet is not None:
    ...
    def _on_remove_sim_from_situation(self, sim):
        ...
        if self._cur_state is not None:
            if self._state_to_uid(self._cur_state) == PetCareSituation.PET_ROUTES_TO_CRATE_STATE_UID:
...
lock_instance_tunables(PetCareSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
