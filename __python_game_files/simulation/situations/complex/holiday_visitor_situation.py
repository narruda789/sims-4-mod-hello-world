...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\holiday_visitor_situation.py
...
import interactions, services, sims4.log, sims4.tuning.instances, sims4.tuning.tunable, situations.bouncer, random
...
class _PushInteractionState(CommonInteractionCompletedSituationState):
    ...
    def _push_interaction_or_next_state(self):
        ...
        if self._iteration_count >= self._iterations or holiday_visitor_npc_sim is None or self.owner.selected_target is None:
            self._change_state(self.owner.holiday_visitor_npc_job.hang_out_state())
            return
        context = interactions.context.InteractionContext(holiday_visitor_npc_sim, (interactions.context.InteractionContext.SOURCE_SCRIPT),
...
class HolidayVisitorNPCSituation(WalkbyLimitingTagsMixin, SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _ArrivalState, factory=(cls.holiday_visitor_npc_job.arrival_state)),
         SituationStateData(2, _PushInteractionState, factory=(cls.holiday_visitor_npc_job.push_interaction_state)),
         SituationStateData(3, _HangOutState, factory=(cls.holiday_visitor_npc_job.hang_out_state)),
         SituationStateData(4, _LeaveState, factory=(cls.holiday_visitor_npc_job.leave_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (cls.holiday_visitor_npc_job.situation_job,
...
sims4.tuning.instances.lock_instance_tunables(HolidayVisitorNPCSituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.WORKER),
