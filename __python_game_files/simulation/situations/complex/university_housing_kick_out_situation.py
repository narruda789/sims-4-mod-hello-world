...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\university_housing_kick_out_situation.py
# Compiled at: 2020-07-30 16:16:42
...
class UniversityHousingKickOutSituation(SituationComplexCommon):
        ...
        if self._kick_out_reason in self.start_dialog_map:
            ...
            if sim_info is not None:
                if map_entry is not None:
                    ...
                    dialog.show_dialog()
                    end_situation = False
        if self._kick_out_reason == UniversityHousingKickOutReason.BABY:
            if not self._additional_sim_ids:
                logger.error('Attempting to kick sim out of university housing for {} without a valid parent', (self._kick_out_reason),
                  owner='bnguyen')
        if end_situation:
            self._self_destruct()
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event == TestEvent.HouseholdSplitPanelClosed:
            ...
            if event == TestEvent.SimEnrolledInUniversity:
    ...
    def _show_household_split_dialog(self, source_household_id):
        ...
        household_split(sourceHouseholdId=source_household_id, targetHouseholdId=target_household_id,
          cancelable=False,
          allow_sim_transfer=allow_sim_transfer,
          selected_sim_ids=selected_sim_ids,
          destination_zone_id=(self._university_housing_destination_zone_id),
          callback_command_name='university.end_kickout_situation')
