...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\yoga_class.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 25392 bytes
...
from sims4.tuning.tunable import TunableInterval, TunableVariant, TunableTuple, TunableSimMinute, TunableList, TunableSet, TunableEnumWithFilter, TunableMapping, TunableEnumEntry, OptionalTunable
...
import enum, services, sims4, situations, tag
...
class _ClassPoseState(CommonInteractionCompletedSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        for sim in self.owner.all_sims_in_job_gen(class_member_job):
            ...
            self._create_or_load_alarm(alarm_name, class_member_delay,
              (lambda _, sim=sim: self._member_delay_timer_expired(sim)),
              should_persist=True)

        self._create_or_load_alarm((self.MISS_INTERACTION_TIMEOUT), (self.miss_interaction_time_delay),
          (lambda _: self._on_interaction_of_interest_complete()),
          should_persist=True)
...
class ClassPoseVariant(TunableVariant):
    ...
    def __init__(self, description='The variant of different class poses.', **kwargs):
        (super().__init__)(description=description, pose_greeting=_ClassPoseGreeting.TunableFactory(), 
         pose_bridge=_ClassPoseBridge.TunableFactory(), 
         pose_dance=_ClassPoseBridge.TunableFactory(), 
         pose_downwarddog=_ClassPoseDownwardDog.TunableFactory(), 
         pose_halfmoon=_ClassPoseHalfMoon.TunableFactory(), 
         pose_halfmoon_mirror=_ClassPoseHalfMoon_Mirror.TunableFactory(), 
         pose_tree=_ClassPoseTree.TunableFactory(), 
         pose_tree_mirror=_ClassPoseTree_Mirror.TunableFactory(), 
         pose_triangle=_ClassPoseTriangle.TunableFactory(), 
         pose_triangle_mirror=_ClassPoseTriangle_Mirror.TunableFactory(), 
         pose_Corpse=_ClassPoseCorpse.TunableFactory(), 
         pose_boat=_ClassPoseBoat.TunableFactory(), 
         pose_warrior=_ClassPoseWarrior.TunableFactory(), 
         pose_handstand=_ClassPoseHandstand.TunableFactory(), 
         pose_sideplank=_ClassPoseSidePlank.TunableFactory(), 
         pose_sideplank_mirror=_ClassPoseSidePlank_Mirror.TunableFactory(), 
         default='pose_greeting', **kwargs)
...
class YogaClassSituation(InstructedClassSituationMixin, situations.situation_complex.SituationComplexCommon):
    INSTANCE_TUNABLES = {'class_pose_map':TunableMapping(description='\n            The static map to mapping yoga pose state to the certain pose enum.\n            Put it here instead of in the module tuning is for pack safe reason.\n            This should only be tuned on prototype, and not suggesting to change/override\n            in tuning instance unless you have very strong reason.\n            ',
       key_type=TunableEnumEntry(tunable_type=ClassPoseEnum, default=(ClassPoseEnum.GREETING)),
       key_name='pose_enum',
       value_type=ClassPoseVariant(),
       value_name='pose_content',
       tuning_group=YOGA_CLASS_GROUP), 
     'class_pose_states':TunableList(description='\n                The sequence of the yoga poses we want the class to run.\n                ',
       tunable=TunableEnumEntry(tunable_type=ClassPoseEnum, default=(ClassPoseEnum.GREETING)),
       tuning_group=GroupNames.STATE), 
     '_alternate_instructor_job':SituationJob.TunableReference(description='\n                The situation job given to instructors staffing a mat if the player uses it to host their own class.\n                ',
       tuning_group=GroupNames.SITUATION), 
     'instructor_situations':SituationSearchByDefinitionOrTagsVariant(description="\n            Situations that define NPC instructors.  NPC instructors will normally be on the guest list for scheduled\n            classes, but player-run classes that are run on instructor's mats will find the instructor tied to the mat\n            using this tuning in order to place them into the 'alternate instructor job'.\n            ",
       tuning_group=GroupNames.SITUATION), 
     'instructor_mat_tags':TunableSet(description="\n                The instructor's yoga mat tags.\n                ",
       tunable=TunableEnumWithFilter(tunable_type=(tag.Tag),
       filter_prefixes=('Func_YogaClass', ),
       default=(tag.Tag.INVALID)),
       tuning_group=YOGA_CLASS_GROUP)}
    REMOVE_INSTANCE_TUNABLES = set(Situation.SITUATION_SCORING_REMOVE_INSTANCE_TUNABLES + Situation.SITUATION_START_FROM_UI_REMOVE_INSTANCE_TUNABLES + Situation.SITUATION_USER_FACING_REMOVE_INSTANCE_TUNABLES + ('_instructor_job', )) - {
     '_display_name', 'venue_situation_player_job'}

    @classmethod
    def _states(cls):
        class_pose_map = cls.class_pose_map
        situation_states = [SituationStateData(1, _PreClassState, factory=(cls.pre_class_state.situation_state)),
         SituationStateData(2, _PostClassState, factory=(cls.post_class_state.situation_state)),
         SituationStateData(3, _ClassPoseBridge, factory=(class_pose_map[ClassPoseEnum.BRIDGE])),
         SituationStateData(4, _ClassPoseDance, factory=(class_pose_map[ClassPoseEnum.DANCE])),
         SituationStateData(5, _ClassPoseDownwardDog, factory=(class_pose_map[ClassPoseEnum.DOWNWARD_DOG])),
         SituationStateData(6, _ClassPoseGreeting, factory=(class_pose_map[ClassPoseEnum.GREETING])),
         SituationStateData(7, _ClassPoseHalfMoon, factory=(class_pose_map[ClassPoseEnum.HALFMOON])),
         SituationStateData(8, _ClassPoseTree, factory=(class_pose_map[ClassPoseEnum.TREE])),
         SituationStateData(9, _ClassPoseTriangle, factory=(class_pose_map[ClassPoseEnum.TRIANGLE])),
         SituationStateData(10, _ClassPoseCorpse, factory=(class_pose_map[ClassPoseEnum.CORPSE])),
         SituationStateData(11, _ClassPoseBoat, factory=(class_pose_map[ClassPoseEnum.BOAT])),
         SituationStateData(12, _ClassPoseWarrior, factory=(class_pose_map[ClassPoseEnum.WARRIOR])),
         SituationStateData(13, _ClassPoseHandstand, factory=(class_pose_map[ClassPoseEnum.HANDSTAND])),
         SituationStateData(14, _ClassPoseSidePlank, factory=(class_pose_map[ClassPoseEnum.SIDEPLANK]))]
...
class YogaClassScheduleMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.yoga_class is not None:
    ...
    def _start_yoga_class(self, scheduler, alarm_data, extra_data):
        ...
        for yoga_idle_situation in situation_manager.get_situations_by_type(self.yoga_class.yoga_instructor_idle_situation):
            ...
            if yoga_instructor_mat is None:
                ...
                continue
            if any((yoga_class_situation_type.tags & situation.tags for situation in situation_manager.get_situations_sim_is_in(yoga_instructor))):
                continue
            ...
            created_class_id = situation_manager.create_situation(yoga_class_situation_type, guest_list=guest_list,
              user_facing=False,
              default_target_id=(yoga_instructor_mat.id),
