...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\bowling_venue.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 28479 bytes
...
import alarms, date_and_time, filters, services, sims4
...
class BowlingVenueSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'group_filter':TunableReference(description='\n            The aggregate filter that we use to find the sims for this\n            situation.\n            ',
     ...
     'bowling_venue_job_and_role':TunableSituationJobAndRoleState(description='\n            The job and role state for the bowler on venue.\n            ',
       tuning_group=GroupNames.SITUATION), 
     'required_bowling_lanes':TunableThreshold(description='\n            The number of bowling lane objects that needed to trigger this situation.\n            ',
       value=Tunable(description='\n                The value of a threshold.\n                ',
       tunable_type=int,
       default=1),
       default=sims4.math.Threshold(value=1, comparison=(sims4.math.Operator.GREATER_OR_EQUAL.function)),
       tuning_group=GroupNames.SITUATION)}
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event == TestEvent.ObjectDestroyed:
            ...
            if self._bowling_lane_chosen:
                if destroyed_obj.id == self._bowling_lane.id:
                    self._bowling_lane = None
                    self._self_destruct()
...
lock_instance_tunables(BowlingVenueSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
...
class BowlingVenueMixin:
    ...
    def _initialize_alarms(self):
        ...
        self._bowling_situation_alarm_handle = alarms.add_alarm(self, date_and_time.create_time_span(minutes=5),
    ...
    def _start_bowling_situation(self, bowling_lane_obj):
        ...
        if not self.bowling_venue.bowling_situations_schedule.entries:
            ...
            return weighted_situations or None
        situation_to_create = weighted_random_item(weighted_situations)
        guest_list = situation_to_create.get_predefined_guest_list()
        if guest_list is None:
            guest_list = SituationGuestList(invite_only=True)
        situation_id = situation_manager.create_situation(situation_to_create, guest_list=guest_list,
    ...
    def _moonlight_change(self, alarm_handle=None):
        if not self.bowling_venue.moonlight_on_off_schedule:
            return
        elif self._get_desired_is_moonlight_on():
    ...
    def _get_desired_is_moonlight_on(self):
        if not self.bowling_venue.moonlight_on_off_schedule:
            ...
            return moonlight_schedule or None
    ...
    def _activate_moonlight_affordance(self, affordance):
        ...
        for bowling_lane in self._bowling_lanes:
            ...
            affordance.state_settings.execute_helper(affordance.target)
