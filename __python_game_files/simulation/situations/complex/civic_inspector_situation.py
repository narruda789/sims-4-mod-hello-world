...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\civic_inspector_situation.py
# Compiled at: 2020-04-28 16:13:52
# Size of source mod 2**32: 16788 bytes
...
class _BaseInspectionClass(CommonSituationState):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionComplete:
            if resolver(self.owner.overlook_issues_success_interaction):
...
class _FinalChecksState(CommonSituationState):
    FACTORY_TUNABLES = {'civic_policy_follow_rules': TunableList(description='\n                a civic policy would pass if either civic policy is not enabled\n                or (is enabled and objects are placed and not broken)\n                ',
                                    tunable=TunableTuple(street_civic_policy_test=(street_civic_policy_tests.StreetCivicPolicyTest.TunableFactory()),
                                    skip_test_condition=TunableTestSet(description="\n                        If these tests return true, we need to skip the civic policy test.\n                        Example use: In apartments, certain civic policies shouldn't be tested.\n                        "),
    ...
    def check_if_policies_followed(self):
        ...
        for policy in self.civic_policy_follow_rules:
            ...
            if len(policy.skip_test_condition) > 0:
                if not (policy.skip_test_condition.run_tests(inspector_resolver) or policy.street_civic_policy_test()):
                    continue
                total_policies += 1
                ...
                current_policy_followed or self.owner.policies_not_followed.append(policy.policy_display_name)
...
class CivicInspectorSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'inspector_job_and_role_state':TunableSituationJobAndRoleState(description='\n            The job and role state for the eco-inspector.\n            ',
     ...
     'inspector_entry':_InspectorEntryState.TunableFactory(description='\n           Inspector Entry State. Listens for portal allowance.\n            ',
       display_name='1. Inspector Entry State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'civic_inspect_outside':_InspectorOutsideState.TunableFactory(description='\n           Inspecting from outside, not allowed inside.\n            ',
       display_name='2. Civic Inspect Outside',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'civic_inspect_inside':_InspectorInsideState.TunableFactory(description='\n            Allowed inside, but may inspect outside if not objects.\n            ',
       display_name='3. Civic Inspect Inside',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'final_checks':_FinalChecksState.TunableFactory(description='\n            Checks if civic policy is being followed using the conditions\n            described in tuning. \n            ',
       display_name='4. Final Checks',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'leave':_LeaveState.TunableFactory(description='\n            Final checks before leaving.\n            ',
       display_name='5. Leaving state.',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'inspection_successful_notification':UiDialogNotification.TunableFactory(description='\n            A TNS that is displayed to inform of successful inspection.\n            '), 
    ...
    def _states(cls):
        return (
         SituationStateData(1, _WaitState),
         SituationStateData(2, _InspectorEntryState, factory=(cls.inspector_entry)),
         SituationStateData(3, _InspectorInsideState, factory=(cls.civic_inspect_inside)),
         SituationStateData(4, _InspectorOutsideState, factory=(cls.civic_inspect_outside)),
         SituationStateData(5, _FinalChecksState, factory=(cls.final_checks)),
         SituationStateData(6, _LeaveState, factory=(cls.leave)))
    ...
    def notify_result_and_push_bill_modifier(self):
        ...
        if self.policies_not_followed:
            additional_tokens = (
             (LocalizationHelperTuning.get_bulleted_list)(*(None, ), *self.policies_not_followed),)
        elif self.follow_status == PolicyFollowing.NOT_FOLLOWED:
            ...
            if self.follow_status == PolicyFollowing.PARTIAL_FOLLOWED:
