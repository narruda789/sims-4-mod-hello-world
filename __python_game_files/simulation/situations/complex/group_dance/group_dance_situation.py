...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\group_dance\group_dance_situation.py
...
import placement, services, sims4.log, situations.situation_types
...
class _PreSituationState(CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm((self.PRE_GROUP_DANCE_TIMEOUT), (self.owner.pre_situation_state.time_out),
          (lambda _: self.timer_expired()),
          should_persist=True)

    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionStart:
            if resolver._interaction.affordance is self.owner.constraint_affordance:
...
class GroupDanceSituation(SituationComplexCommon):
    ...
    INSTANCE_TUNABLES = {'pre_situation_state':TunableTuple(description='\n            Information related to the pre dance situation state.\n            ',
       ...
       default=15,
       minimum=1),
       tuning_group=DANCE_TUNING_GROUP), 
     'constraint_affordance':JigPartConstraintInteraction.TunableReference()}
    ...
    def _states(cls):
        return (
         SituationStateData(1, _PreSituationState, factory=(cls.pre_situation_state.situation_state)),)
    ...
    def _create_situation_geometry(self):
        ...
        dance_floor = self._get_dance_floor()
        start_transform = dance_floor.transform if dance_floor is not None else leader_sim.transform
        starting_location = placement.create_starting_location(transform=start_transform, routing_surface=routing_surface)
        fgl_context = placement.create_fgl_context_for_object(starting_location, (self._jig_object), search_flags=search_flags,
          ignored_object_ids=(self._get_ignored_object_ids()))
        translation, orientation = placement.find_good_location(fgl_context)
        if translation is not None:
            self._jig_object.move_to(routing_surface=routing_surface, translation=translation,
    ...
    def _route_sim(self, sim, jig_index):
        ...
        aop = AffordanceObjectPair((self.constraint_affordance), (self._jig_object),
          (self.constraint_affordance),
          None,
          jig_object=(self._jig_object),
          jig_part_index=jig_index)
    ...
    def sim_finish_routing(self, sim_info):
        if sim_info.id in self._routing_sims:
            ...
            next_state = self._routing_sims or self.get_next_dance_state()
            self._change_state(next_state())
        else:
