...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\group_interaction_situation.py
# Compiled at: 2018-07-22 17:25:27
# Size of source mod 2**32: 11588 bytes
...
import services, sims4.log, situations.situation_types
...
class _InteractionState(CommonInteractionCompletedSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        aop = AffordanceObjectPair((self.owner.affordance), (self.owner._target_object),
...
class _PreSituationState(CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm((self.PRE_GROUP_INTERACTION_TIMEOUT), (self.owner.pre_situation_state.time_out),
...
class GroupInteractionSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'pre_situation_state':TunableTuple(description='\n            Information related to the pre interaction situation state.\n            ',
     ...
     'constraint_affordance':SuperInteraction.TunableReference(description='\n            The interaction that puts the followers into the constraint.\n            '), 
     ...
     'leader_job':SituationJob.TunableReference(description='\n            The situation job for leader.\n            ',
       tuning_group=GROUP_INTERACTION_TUNING_GROUP), 
     'member_job':SituationJob.TunableReference(description='\n            The situation job for member.\n            ',
       tuning_group=GROUP_INTERACTION_TUNING_GROUP), 
     'interaction_state':_InteractionState.TunableFactory(description='\n            The state that sim is doing the interaction.\n            ',
       tuning_group=GROUP_INTERACTION_TUNING_GROUP), 
    ...
    def _states(cls):
        return (
         ...
         SituationStateData(2, _InteractionState, factory=(cls.interaction_state)))
    ...
    def on_sim_finish_routing(self, sim_info):
        if sim_info.id in self._routing_sims:
            ...
            next_state = self._routing_sims or self.get_next_interaction_state()
            self._change_state(next_state())
        else:
