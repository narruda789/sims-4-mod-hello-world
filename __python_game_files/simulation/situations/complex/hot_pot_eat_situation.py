...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\hot_pot_eat_situation.py
# Compiled at: 2020-10-14 19:13:32
...
class HotPotEatSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'guest_job':SituationJob.TunableReference(description='\n            The situation job for those gathering to eat\n            '), 
     ...
     'gather_at_hot_pot_state':_GatherAtHotPot.TunableFactory(description='\n            The state to bring all picked Sims to gather at the hot pot.\n            ',
       display_name='1. Gather at Hot Pot State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'start_hot_pot_state':_StartHotPot.TunableFactory(description='\n            The state to have an actor start the hot pot cooking.\n            ',
       display_name='2. Start Hot Pot State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'wait_for_meal_ready_state':_WaitForMealReady.TunableFactory(description='\n            The state that will have all picked Sims waiting around the hot pot, talking.\n            ',
       display_name='3. Wait for Meal Ready State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'eat_hot_pot_state':_EatHotPot.TunableFactory(description='\n            The state that will have all Sims consume from the hot pot.\n            ',
       display_name='4. Eat Hot Pot',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'hot_pot_empty_state':TunableReference(description="\n            The object state value that represents the hot pot becoming emptied of all food,\n            This occurs when a Sim uses 'Empty Hot Pot'\n            ",
       manager=services.get_instance_manager(sims4.resources.Types.OBJECT_STATE),
       class_restrictions=('ObjectStateValue', ),
       pack_safe=True)}
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _GatherAtHotPot, factory=(cls.gather_at_hot_pot_state)),
         SituationStateData(2, _StartHotPot, factory=(cls.start_hot_pot_state)),
         SituationStateData(3, _WaitForMealReady, factory=(cls.wait_for_meal_ready_state)),
         SituationStateData(4, _EatHotPot, factory=(cls.eat_hot_pot_state)))
...
lock_instance_tunables(HotPotEatSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
