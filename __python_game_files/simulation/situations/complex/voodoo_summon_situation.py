...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\voodoo_summon_situation.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 5240 bytes
...
import interactions, services, sims4.tuning.instances, sims4.tuning.tunable, situations.bouncer
...
class VoodooSummonSituation(situations.situation_complex.SituationComplexCommon):
    INSTANCE_TUNABLES = {'summoned_job':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                          A reference to the SituationJob used for the Sim summoned.\n                          '),
     ...
     'come_here_affordance':sims4.tuning.tunable.TunableReference(manager=services.get_instance_manager(sims4.resources.Types.INTERACTION),
...
sims4.tuning.instances.lock_instance_tunables(VoodooSummonSituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.PRE_VISIT),
...
class _ComeHereState(SituationState):
    ...
    def _push_interaction(self):
        ...
        if target_sim is None:
            return False
        context = interactions.context.InteractionContext((self.owner._summoned_sim), (interactions.context.InteractionContext.SOURCE_SCRIPT),
