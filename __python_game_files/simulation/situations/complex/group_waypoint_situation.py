...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\group_waypoint_situation.py
# Compiled at: 2021-05-04 14:45:50
# Size of source mod 2**32: 58998 bytes
...
class _GroupWaypointStartState(CommonInteractionStartingSituationState):
    ...
    def timer_expired(self):
        ...
        if self.owner.social_group:
            ...
            if self.owner.get_num_sims_in_job(self.owner.leader_job_and_role.job) > 0:
                if self.owner.get_num_sims_in_job(self.owner.member_job_and_role.job) >= _GroupWaypointStartState.min_required_member_sims:
                    if num_group_sims >= _GroupWaypointStartState.min_required_group_sims:
                        self.owner.begin_routing()
                        return
...
class _GroupWaypointRouteState(CommonSituationState):
    ...
    def on_route_end(self, *args, **kwargs):
        ...
        if self._shutdown_situation_on_route_end:
            ...
            if self.owner._next_interaction is not None:
...
class _GroupWaypointInteractState(CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if leader_interaction is not None:
        ...
        elif member_interaction is not None:
            self._test_event_register(TestEvent.InteractionExitedPipeline, member_interaction)
        elif reader is None:
            if leader_interaction is None:
                if member_interaction is None:
                    self._route_to_next_waypoint()
                    return
            ...
            if self.owner is not None and self._num_queued_interactions == 0:
                self._route_to_next_waypoint()
        else:
    ...
    def _perform_interaction_on_sim(self, sim, target, interaction_type):
        interaction_context = InteractionContext(sim, (InteractionContext.SOURCE_SCRIPT), (Priority.High), insert_strategy=(QueueInsertStrategy.FIRST),
          must_run_next=True)
    ...
    def _on_interaction_complete(self):
        ...
        if self._num_queued_interactions <= 0:
            if self._post_interaction_delay > 0:
                self._timeout_handle = self._create_or_load_alarm((self.POST_INTERACTION_DELAY_TIMEOUT), (self._post_interaction_delay),
                  (self._route_to_next_waypoint),
                  should_persist=True)
            else:
                self._route_to_next_waypoint()
...
class GroupWaypointSituation(SituationComplexCommon):
    ...
    class _WaypointByTagStrategy(HasTunableSingletonFactory, AutoFactoryInit, _GroupWaypointStrategy):
        ...
        def advance_counter(self, num_visited_waypoints, min_waypoints, _):
            ...
            return (
             num_visited_waypoints, False)

        def get_next_interaction_choices(self, leader_sim, resolver, current_count, min_waypoints):
            return (
             self.interaction_tuning, False, False)
    ...
    class _PredefinedWaypointStrategy(HasTunableSingletonFactory, AutoFactoryInit, _GroupWaypointStrategy):
        ...
        def advance_counter(self, num_visited_waypoints, min_waypoints, max_waypoints):
            if self.current_waypoint is not None:
                ...
                if num_visited_waypoints >= max_waypoints:
                    if max_waypoints is not GroupWaypointSituation.INFINITE_WAYPOINTS:
                        logger.debug('Fulfilled exit condition of GroupWaypointSituation. Visited {} waypoints. Ending situation.', num_visited_waypoints)
                        return (num_visited_waypoints, True)
                if self.current_waypoint in self.ending_waypoints:
                    if num_visited_waypoints >= min_waypoints:
                        logger.debug('Fulfilled exit condition of GroupWaypointSituation. Reached and end waypoint. Ending situation.')
                        return (num_visited_waypoints, True)
            self.current_waypoint_sequence = []
        ...
        def get_solo_waypoint_generator(self, interaction_context):
            return self.solo_waypoint_generator((self.current_waypoint_sequence), (self.waypoint_graph.constraint_radius),
    ...
    INSTANCE_TUNABLES = {'waypoint_strategy':TunableVariant(description='\n            The strategy that manages waypoint selection.\n            ',
     ...
     'starting_state':_GroupWaypointStartState.TunableFactory(description='\n            State that runs while the Sims are routing to the next waypoint.\n            ',
       display_name='1. Start State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'route_state':_GroupWaypointRouteState.TunableFactory(description='\n            State that runs while the Sims are routing to the next waypoint.\n            ',
       display_name='2. Route State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'interact_state':_GroupWaypointInteractState.TunableFactory(description='\n            State that runs while the Sims are interacting with a point-of-interest at a waypoint.\n            ',
       display_name='3. Interact State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'leader_job_and_role':TunableSituationJobAndRoleState(description='\n            The job and role state for the leader.\n            '), 
     ...
     'npc_go_home_interaction':OptionalTunable(description='\n            Optional interaction to push on NPCs to send them "home" when the situation ends.\n            ',
       ...
       manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION)),
       class_restrictions=('SuperInteraction', ))), 
     'solo_waypoint_interaction':TunableReference(description='\n            Interaction that routes a solo sim from point to point when they\n            are not in a group.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.INTERACTION),
       class_restrictions=('WaypointInteraction', ))}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if reader is None:
            ...
            if leader_interaction is not None or member_interaction is not None:
                ...
                self._next_interaction = (leader_interaction,
                 member_interaction,
                 interaction_delay,
                 target_def)
            services.current_zone().register_callback(ZoneState.HITTING_THEIR_MARKS, self._on_zone_state_hitting_their_marks)
    ...
    def choose_interactions_for_waypoint(self, choices):
        if not choices:
            ...
            return weighted_choices or None
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _GroupWaypointStartState, factory=(cls.starting_state)),
         SituationStateData(2, _GroupWaypointRouteState, factory=(cls.route_state)),
         SituationStateData(3, _GroupWaypointInteractState, factory=(cls.interact_state)),
         SituationStateData(4, _StartSoloSituationState))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.leader_job_and_role.job, cls.leader_job_and_role.role_state),
         (
          cls.member_job_and_role.job, cls.member_job_and_role.role_state)]
    ...
    def route_to_waypoint(self):
        if self._social_group is not None:
            ...
            enqueue_result = leader_sim.push_super_affordance((self.solo_waypoint_interaction), None,
              interaction_context, waypoint_generator=waypoint_generator)
            if not enqueue_result or enqueue_result.interaction.is_finishing:
    ...
    def on_sim_added_to_social_group(self):
        if type(self._cur_state) == _GroupWaypointStartState:
    ...
    def _on_set_sim_role_state(self, sim, job_type, role_state_type, role_affordance_target):
        super()._on_set_sim_role_state(sim, job_type, role_state_type, role_affordance_target)
        if self.num_invited_sims == 1:
            if job_type is not self.leader_job_and_role.job:
