...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\wedding_situation.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 25459 bytes
...
import alarms, clock, event_testing, interactions, services, sims4.tuning.tunable, situations.situation_complex
...
class WeddingSituation(situations.situation_complex.SituationComplexCommon):
    INSTANCE_TUNABLES = {'betrothed':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                The SituationJob for the betrothed.\n                '),
     ...
     'bartender':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                The SituationJob for the Bartenders.\n                '),
       ...
       tuning_group=GroupNames.ROLES), 
     'caterer':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                The SituationJob for the Caterers.\n                '),
     ...
     'guest':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                The SituationJob for the Guests.\n                '),
       ...
       manager=(services.get_instance_manager(Types.STATIC_COMMODITY))),
       guest_reception_pre_cake_role_state=RoleState.TunableReference(description="\n                Guest's role state during the reception, before the cake is\n                cut, if the ceremony was successful.\n                "),
       ...
       tuning_group=GroupNames.ROLES), 
     'ceremony':situations.situation_complex.TunableInteractionOfInterest(description='\n            Starting this interaction starts the ceremony. Completing it starts\n            the reception.\n            ',
       tuning_group=GroupNames.TRIGGERS), 
     'cold_feet':situations.situation_complex.TunableInteractionOfInterest(description='\n            Running this mixer interaction during the ceremony causes it to\n            FAIL.\n            ',
       tuning_group=GroupNames.TRIGGERS), 
     'kiss':situations.situation_complex.TunableInteractionOfInterest(description='\n            Running this mixer interaction during the ceremony causes it to\n            SUCCEED.\n            ',
       tuning_group=GroupNames.TRIGGERS), 
     'cut_cake':situations.situation_complex.TunableInteractionOfInterest(description='\n            Once this interaction is run guests can eat cake.\n            ',
       tuning_group=GroupNames.TRIGGERS), 
     'guests_become_impatient_timeout':TunableSimMinute(description='\n            If the ceremony is not started in this amount of time the guests\n            will grow impatient.\n            ',
       default=60,
       tuning_group=GroupNames.TRIGGERS), 
     'guests_start_reception_timeout':TunableSimMinute(description='\n            If the party is already in the ImpatientGatherState, after this\n            amount of time we switch the party to PostCakeReceptionState\n            so everyone can enjoy.',
       default=120,
       tuning_group=GroupNames.TRIGGERS), 
     'engaged_bit':relationship_bit.RelationshipBit.TunableReference(description='\n            The Rel Bit to look for when auto-populating betrothed Sims.\n            ',
       tuning_group=GroupNames.ROLES), 
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, GatherState),
         SituationStateData(2, ImpatientGatherState),
         SituationStateData(3, CeremonyState),
         SituationStateData(4, PreCakeReceptionState),
         SituationStateData(5, PostCakeReceptionState),
         SituationStateData(6, FailedReceptionState))
    ...
    def is_situation_available(cls, *args, **kwargs):
        if is_available_pack(Pack.GP11):
    ...
    def _on_sim_removed_from_situation_prematurely(self, sim, sim_job):
        ...
        if sim in self._betrothed_sims:
            if not isinstance(self._cur_state, FailedReceptionState):
...
class CeremonyState(SituationState):
    ...
    def _make_guests_watch(self, _):
        ...
        for sim_info in tuple(self._guest_sim_infos_to_force_watch):
            ...
            autonomy_request = AutonomyRequest(sim, autonomy_mode=FullAutonomy,
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == event_testing.test_events.TestEvent.InteractionComplete:
            if resolver(self.owner.kiss):
                ...
                if resolver(self.owner.cold_feet):
                    self._change_state(FailedReceptionState())
    ...
    def _on_finishing_callback(self, interaction):
        if self._interaction is not interaction:
            ...
            interaction.uncanceled or self._change_state(FailedReceptionState())
            return
...
class PreCakeReceptionState(SituationState):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionComplete:
            if resolver(self.owner.cut_cake):
