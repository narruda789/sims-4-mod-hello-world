...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\infected_situation.py
# Compiled at: 2019-01-17 16:19:17
# Size of source mod 2**32: 13099 bytes
...
class InfectedSituation(SubSituationOwnerMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'infected_state':_InfectedState.TunableFactory(display_name='Infected State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'possessed_start_time':TunableTimeOfDay(description='\n            Time of day Sims become possessed.\n            ',
       tuning_group=GroupNames.SITUATION), 
     'possessed_duration_hours':TestedSum.TunableFactory(description='\n            How long the possession lasts.\n            ',
       tuning_group=GroupNames.SITUATION), 
     'possessed_situation':Situation.TunableReference(description='\n            Possessed situation to place Sim in.\n            ',
       class_restrictions=('PossessedSituation', ),
       tuning_group=GroupNames.SITUATION), 
     'default_job_and_role':TunableSituationJobAndRoleState(description='\n            The job/role the infected Sim will be in.\n            ',
       tuning_group=GroupNames.SITUATION), 
     'possessed_buff_tag':TunableTag(description='\n            Tag for buffs that can add the Possessed Mood through the Infection\n            System. Possessed status is refreshed when these buffs are added\n            or removed.\n            ',
       filter_prefixes=('Buff', )), 
     'possessed_buff_no_animate_tag':TunableTag(description='\n            Possession buffs with this tag will not play the start possession\n            animation.\n            ',
       filter_prefixes=('Buff', )), 
     'possession_time_buff':TunableBuffReference(description='\n            The buff to add to the Sim when it is the possessed start time.\n            ')}
    ...
    def _states(cls):
        return (
         SituationStateData.from_auto_factory(0, cls.infected_state),)
    ...
    def start_situation(self):
        ...
        self._start_possession_alarm = alarms.add_alarm(self, time_span,
          (self._on_possession_start),
          repeating_time_span=time_span_day,
          repeating=True)
    ...
    def _start_possession_situation(self, animate_possession_override=None):
        ...
        self._create_sub_situation((self.possessed_situation), guest_list=guest_list,
          user_facing=False,
          animate_possession=animate_possession)
    ...
    def get_possession_source(self):
        ...
        for source in self._possession_sources:
            ...
            if buff_duration is None or buff_value > buff_duration:
                buff_duration = buff_value
                longest_source = buff

        return (
         longest_source, buff_duration)