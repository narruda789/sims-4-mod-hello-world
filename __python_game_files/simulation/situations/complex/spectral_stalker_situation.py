...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\spectral_stalker_situation.py
# Compiled at: 2019-05-22 04:17:29
# Size of source mod 2**32: 7491 bytes
...
class SpectralStalkerSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'enter_state':_SpectralStalkerEnterState.TunableFactory(description='\n            The first state the Spectral Stalker starts in. This should handle any\n            spawning behavior. When the Interaction Of Interest completes, the \n            Stalker will be pushed to chase the target Sim.\n            ',
       tuning_group=STALKER_GROUP), 
     'chase_state':_SpectralStalkerChaseState.TunableFactory(description='\n            The stalker chases the target Sim.\n            ',
       tuning_group=STALKER_GROUP), 
     'idle_state':_SpectralStalkerIdleState.TunableFactory(description="\n            The stalker Idles until it's time to chase again.\n            ",
       tuning_group=STALKER_GROUP), 
     'exit_state':_SpectralStalkerExitState.TunableFactory(description='\n            The Spectral Stalker exits the world.\n            ',
       tuning_group=STALKER_GROUP), 
     'target_job':SituationJob.TunableReference(description='\n            The Situation Job on the target of the Spectral Stalker.\n            '), 
    ...
    @classmethod
    def _states(cls):
        return [SituationStateData(0, _SpectralStalkerEnterState, cls.enter_state),
         SituationStateData(1, _SpectralStalkerChaseState, cls.chase_state),
         SituationStateData(2, _SpectralStalkerIdleState, cls.idle_state),
         SituationStateData(3, _SpectralStalkerExitState, cls.exit_state)]
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if sim_info is self._target_sim_info:
            if event == TestEvent.OnSimReset or event == TestEvent.SimDeathTypeSet:
                self._stalk_finish_ticks = None
