...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\possessed_situation.py
# Compiled at: 2019-02-01 19:09:39
...
class _PossessedState(CommonSituationState):
    ...
    def _refresh_possession(self, e):
        ...
        elif self.owner.sim_info.has_buff(buff_type):
...
class PossessedSituation(SubSituationMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'default_job_and_role':TunableSituationJobAndRoleState(description='\n            The job the possessed Sim will be in.\n            ',
     ...
     'animate_possession_state':_AnimatePossessionState.TunableFactory(display_name='0. Animate Possession State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'possessed_state':_PossessedState.TunableFactory(display_name='1. Possessed State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'animate_timeout_state':_AnimateTimeoutState.TunableFactory(display_name='2. Animate Timeout State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
    ...
    def _states(cls):
        return (
         SituationStateData.from_auto_factory(0, cls.animate_possession_state),
         SituationStateData.from_auto_factory(1, cls.possessed_state),
         SituationStateData.from_auto_factory(2, cls.animate_timeout_state),
         SituationStateData(3, WaitForSimAssignment))
