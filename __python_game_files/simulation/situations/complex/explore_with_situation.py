...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\explore_with_situation.py
    ...
    def _on_explore_complete(self, *args):
        ...
        for sim in situation_sims:
            for target_sim in situation_sims:
                if not sim is target_sim:
                    if target_sim in rel_pair_granted[sim] or sim in rel_pair_granted[target_sim]:
                        continue
...
class ExploreWithSituation(SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _PrepareToExploreState, factory=(cls.prepare_to_explore_state)),
         SituationStateData(2, _ExploreState, factory=(cls.explore_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.leader_job_and_role.job, cls.leader_job_and_role.role_state),
         (
          cls.guest_job_and_role.job, cls.guest_job_and_role.role_state)]
...
lock_instance_tunables(ExploreWithSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
