...
# Compiled at: 2016-11-07 17:36:24
# Size of source mod 2**32: 14694 bytes
...
class FireSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (SituationStateData(1, _PanicState),
         ...
         SituationStateData(5, _SaveToddlerState))
        ...
        if event is TestEvent.InteractionComplete:
            ...
            if resolver(self.owner.got_to_safety_interaction):
                ...
                if resolver(self.owner.panic_interaction):
                    if services.get_fire_service().has_toddler_to_save_for_sim(sim_info):
...
class _SaveToddlerState(SituationState):
    ...
    def _on_set_sim_role_state(self, sim, job_type, role_state_type, role_affordance_target):
        ...
        InteractionCancelCompatibility.cancel_interactions_for_reason(sim, (InteractionCancelReason.DEATH),
          (FinishingType.FIRE),
          'Interaction was canceled due to saving a toddler from a fire.',
          additional_cancel_sources=(
         InteractionSource.SCRIPT,))
        InteractionCancelCompatibility.cancel_interactions_for_reason(toddler, (InteractionCancelReason.DEATH),
          (FinishingType.FIRE),
          'Interaction was canceled due to a toddler being saved from a fire.',
          additional_cancel_sources=(
         InteractionSource.SCRIPT,))
