...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\staff_member_situation.py
# Compiled at: 2018-07-22 17:25:27
# Size of source mod 2**32: 13677 bytes
...
class _ActivelyWorkingState(CommonInteractionCompletedSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if not (staff_member and staff_member.queue.running and self._interaction_of_interest(staff_member.queue.running)):
            self._create_or_load_alarm(ACTIVELY_WORKING_TIMEOUT, (self.owner.actively_working_timeout), (lambda _: self.timer_expired()),
...
class _BoredState(CommonInteractionStartedSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if self.owner.bored_timeout:
            self._create_or_load_alarm(BORED_TIMEOUT, (self.owner.bored_timeout), (lambda _: self.timer_expired()),
              should_persist=True, reader=reader)
...
class StaffMemberSituation(StaffedObjectSituationMixin, SituationComplexCommon):
    ...
    def _states(cls):
        return [SituationStateData(1, _ArrivingState, factory=(cls._arriving_situation_state)),
         SituationStateData(2, _ActivelyWorkingState, factory=(cls._actively_working_situation_state)),
         SituationStateData(3, _BoredState, factory=(cls._bored_situation_state))]
    ...
    def _on_set_sim_job(self, sim, job_type):
        ...
        if arrival_interaction is not None:
            if staff_member is not None:
