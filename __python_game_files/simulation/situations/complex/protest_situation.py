...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\protest_situation.py
# Compiled at: 2019-10-22 15:05:03
...
import interactions, itertools, random, services, sims4
...
class _ProtestingState(CommonInteractionCompletedSituationState):
    ...
    def get_sign_definition(self):
        if self._sign_definition is None:
            ...
            self._sign_definition or logger.error('Could not find a sign for the protester to hold.  Please verify tuning on situation: {0}.', self.owner)
    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        ...
        if sign_definition is not None:
            (sim.push_super_affordance)((self._create_sign_interaction), 
             chosen_point, 
             context, **{CreateCarriedObjectSuperInteraction.INTERACTION_PARAM_KEY: sign_definition})
...
class ProtestSituation(SituationComplexCommon):
    ...
    def get_predefined_guest_list(cls):
        ...
        protester_results = services.sim_filter_service().submit_matching_filter(sim_filter=protester_filter, callback=None,
