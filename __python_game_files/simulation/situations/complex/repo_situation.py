...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\repo_situation.py
# Compiled at: 2020-04-30 15:22:36
# Size of source mod 2**32: 23551 bytes
import enum, operator, random
...
class _FindObjectState(CommonSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        while self.owner.objects_to_take:
            ...
            current_object = obj.self_or_part_in_use or obj
            break
...
class _IdleAtObjectState(CommonSituationState):
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event == TestEvent.InteractionComplete:
            if repo_person is not None and sim_info is repo_person.sim_info:
            ...
            if resolver(self.owner.ask_not_to_take_interaction):
                ...
                if random.random() < ask_not_to_take_chance:
                    ...
                    if self.owner.ask_not_to_take_failure_notification is not None:
...
class RepoSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (SituationStateData(1, _WaitForRepoPersonState),
         SituationStateData(2, _FindObjectState, factory=(cls.find_object_state)),
         SituationStateData(3, _NothingToTakeState, factory=(cls.nothing_to_take_state)),
         SituationStateData(4, _IdleAtObjectState, factory=(cls.idle_at_object_state)),
         SituationStateData(5, _RepossessObjectState, factory=(cls.repossess_object_state)),
         SituationStateData(6, _LeaveState, factory=(cls.leave_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.repo_person_job_and_role_state.job, cls.repo_person_job_and_role_state.role_state),
         (
          cls.debtor_sim_job_and_role_state.job, cls.debtor_sim_job_and_role_state.role_state)]
    ...
    def _on_add_sim_to_situation(self, sim, job_type, role_state_type_override=None):
        ...
        if self.debtor_sim() is not None:
            if self.repo_person() is not None:
...
lock_instance_tunables(RepoSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
