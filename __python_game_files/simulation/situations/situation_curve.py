...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\situation_curve.py
# Compiled at: 2019-09-11 14:44:55
# Size of source mod 2**32: 20522 bytes
import operator
...
import date_and_time, services, sims4.log
...
class _DesiredSituations(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def get_create_params(user_facing=False):
        ...
        return {'weighted_situations': TunableList(description='\n            A weighted list of situations to be used while fulfilling the\n            desired Sim count.\n            ',
                                  tunable=TunableTuple(situation=Situation.TunableReference(pack_safe=True),
                                  params=TunableTuple(description='\n                    Situation creation parameters.\n                    ', 
                                 locked_args=create_params_locked, **create_params),
...
class SituationCurve(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value, **kwargs):
        ...
        for item in value.entries:
            ...
            for day, enabled in days.items():
                if enabled:
                    if day in keys:
                        logger.error('WalkbyTuning {} has multiple population curves for the day {}.', source,
                          day, owner='manus')
                    else:
                        keys.add(day)
            ...
            if item.walkby_desire_by_time_of_day:
                for hour in item.walkby_desire_by_time_of_day.keys():
                    if hour < 0 or hour > 24:
                        logger.error('Situation Curve in {} has in invalid hour of the day {}. Range: [0, 24].', source,
                          hour, owner='manus')

            else:
                logger.error("Situation Curve in {}'s days {} has no walkby desire population curve.", source,
                  days, owner='manus')
    ...
    def _get_desired_situations(self):
        if not self.entries:
            ...
            return situation_schedule or None
...
class ShiftlessDesiredSituations(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    @TunableFactory.factory_option
    def get_create_params(**kwargs):
        return {'_desired_siutations':_DesiredSituations.TunableFactory(get_create_params=kwargs), 
         'desired_sim_count_multipliers':TunableMultiplier.TunableFactory(description='\n                Tunable tested multiplier to apply to the desired sim count.\n                ',
           locked_args={'base_value': 1})}
