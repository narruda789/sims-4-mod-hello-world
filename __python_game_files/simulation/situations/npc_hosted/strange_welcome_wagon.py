...
class StrangePreWelcomeWagon(SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, HasFrontDoorStrangeSituationStartingState, factory=(cls.has_front_door_situation_starting_state)),
         SituationStateData(2, HasNoFrontDoorStrangeSituationStartingState, factory=(cls.has_no_front_door_situation_starting_state)))
    ...
    def get_predefined_guest_list(cls):
        ...
        door_knocker_results = services.sim_filter_service().submit_matching_filter(sim_filter=(cls._door_knocker_situation_job.filter), callback=None,
        ...
        fruitcake_bearer_results = services.sim_filter_service().submit_matching_filter(sim_filter=(cls._fruitcake_bearer_situation_job.filter), callback=None,
          requesting_sim_info=active_sim_info,
          allow_yielding=False,
          blacklist_sim_ids=blacklist,
          gsi_source_fn=(cls.get_sim_filter_gsi_name))
        fruitcake_bearer = fruitcake_bearer_results[0]
        guest_list.add_guest_info(SituationGuestInfo((fruitcake_bearer.sim_info.sim_id), (cls._fruitcake_bearer_situation_job),
          (RequestSpawningOption.DONT_CARE),
          (BouncerRequestPriority.EVENT_VIP),
          expectation_preference=True))
        blacklist.add(fruitcake_bearer.sim_info.sim_id)
        guaranteed_infected_results = services.sim_filter_service().submit_matching_filter(sim_filter=(cls._other_infected_job.filter), callback=None,
          requesting_sim_info=active_sim_info,
          allow_yielding=False,
          blacklist_sim_ids=blacklist,
          gsi_source_fn=(cls.get_sim_filter_gsi_name))
        guaranteed_infected = guaranteed_infected_results[0]
        guest_list.add_guest_info(SituationGuestInfo((guaranteed_infected.sim_info.sim_id), (cls._other_infected_job),
          (RequestSpawningOption.DONT_CARE),
          (BouncerRequestPriority.EVENT_VIP),
          expectation_preference=True))
        other_infected = services.sim_filter_service().submit_filter(sim_filter=(cls._other_infected_job.filter), callback=None,
          requesting_sim_info=active_sim_info,
          allow_yielding=False,
          blacklist_sim_ids=blacklist,
          gsi_source_fn=(cls.get_sim_filter_gsi_name))
        if not other_infected:
            return guest_list
        elif len(other_infected) > cls._extra_infected - 1:
        ...
        for infected in infected_to_come:
            guest_list.add_guest_info(SituationGuestInfo((infected.sim_info.sim_id), (cls._other_infected_job),
              (RequestSpawningOption.DONT_CARE),
              (BouncerRequestPriority.EVENT_VIP),
              expectation_preference=True))
    ...
    def create_welcome_wagon(self):
        ...
        if not situation_manager.is_user_facing_situation_running():
            situation_manager.create_situation((self._welcome_wagon_situation), guest_list=(self._guest_list.clone()),
              user_facing=True,
...
sims4.tuning.instances.lock_instance_tunables(StrangePreWelcomeWagon, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.NORMAL),
...
class WaitToInfectSituationState(CommonInteractionCompletedSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm(WAIT_TO_BE_LET_IN_TIMEOUT, (self._timeout), (lambda _: self.timer_expired()),
...
class InfectSituationState(CommonInteractionCompletedSituationState):
    FACTORY_TUNABLES = {'interaction_to_push':TunableReference(description='\n            The interaction that will be pushed on all non-selectable sims\n            when this situation state begins if there is a front door.\n            ',
       ...
       display_name='Interaction To Push If Front Door Exists.'), 
     'post_welcome_wagon_situation':TunableReference(description='\n            The post welcome wagon agent situation that will attempt to clean\n            up after the infected.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION))}
    ...
    def on_deactivate(self):
        if self._infection_spread > 0:
            services.get_zone_situation_manager().create_situation((self._post_welcome_wagon_situation), user_facing=False,
              scoring_enabled=False)
        super().on_deactivate()
...
class StrangeWelcomeWagon(SituationComplexCommon):
    INSTANCE_TUNABLES = {'wait_to_infect_state':WaitToInfectSituationState.TunableFactory(description='\n            Second state of the situation.  In this state the sims should\n            be waiting to be let into the house.\n            ',
       tuning_group=GroupNames.STATE), 
     'infect_state':InfectSituationState.TunableFactory(description='\n            The third situation state.  In this state everyone parties!\n            ',
       tuning_group=GroupNames.STATE), 
     '_player_sim_job':TunableReference(description='\n            The job for all of the player sims.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB))}

    @classmethod
    def _states(cls):
        return (SituationStateData(1, WaitToInfectSituationState, factory=(cls.wait_to_infect_state)),
         SituationStateData(2, InfectSituationState, factory=(cls.infect_state)))
    ...
    def _on_remove_sim_from_situation(self, sim):
        ...
        if situation_sim is not None:
            if situation_sim.current_job_type is not self._player_sim_job:
                situation_manager.make_sim_leave_now_must_run(sim)
        super()._on_remove_sim_from_situation(sim)


sims4.tuning.instances.lock_instance_tunables(StrangeWelcomeWagon, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.NORMAL),
  creation_ui_option=(situations.situation_types.SituationCreationUIOption.NOT_AVAILABLE))
...
class StrangePostWelcomeWagon(SituationComplexCommon):
    INSTANCE_TUNABLES = {'delay_state':DelayState.TunableFactory(description='\n            A state in which the game delays creating the agents in order to\n            give the infected some time to leave.\n            ',
     ...
     'secret_agent_job':TunableReference(description='\n            The job for all of the agents.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB)), 
     'secret_agents_to_create':TunableRange(description='\n            The number of agents that will be created.\n            ',
       tunable_type=int,
       default=1,
       minimum=1)}
    REMOVE_INSTANCE_TUNABLES = Situation.NON_USER_FACING_REMOVE_INSTANCE_TUNABLES
    ...
    def _states(cls):
        return (SituationStateData.from_auto_factory(0, cls.delay_state),
         SituationStateData(1, CleanUpState, factory=(cls.clean_up_state)),
         SituationStateData(2, NothingToSeeHereState, factory=(cls.nothing_to_see_here_state)))
    ...
    def _create_agents(self):
        for _ in range(self.secret_agents_to_create):
            request = BouncerRequest(self, callback_data=(_RequestUserData()),
              ...
              accept_looking_for_more_work=(self.secret_agent_job.accept_looking_for_more_work))
            self.manager.bouncer.submit_request(request)


sims4.tuning.instances.lock_instance_tunables(StrangePostWelcomeWagon, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.NORMAL),
  creation_ui_option=(situations.situation_types.SituationCreationUIOption.NOT_AVAILABLE))