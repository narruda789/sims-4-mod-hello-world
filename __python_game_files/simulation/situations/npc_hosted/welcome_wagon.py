    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        ...
        if self.owner._fruitcake_id is not None:
            ...
            if bearer_recipe is None:
                logger.error('No recipes tuned for the fruitcake-bearer. Recipe                creation failed for {} situation',
                  self, owner='shipark')
                return (role_state_type, None)
            target = create_craftable(bearer_recipe,
              sim,
...
class PreWelcomeWagon(SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _HasFrontDoorSituationStartingState, factory=(cls.has_front_door_situation_starting_state)),
         SituationStateData(2, _HasNoFrontDoorSituationStartingState, factory=(cls.has_no_front_door_situation_starting_state)))
    ...
    def get_predefined_guest_list(cls):
        ...
        if not door_knocker_results:
            ...
            fruitcake_bearer_results = services.sim_filter_service().submit_filter((cls._fruitcake_bearer_situation_job.filter), callback=None,
              requesting_sim_info=active_sim_info,
              allow_yielding=False,
...
sims4.tuning.instances.lock_instance_tunables(PreWelcomeWagon, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.NORMAL),
...
class WelcomeWagon(SituationComplexCommon):
    INSTANCE_TUNABLES = {'wait_to_be_let_in_state':_WaitToBeLetInState.TunableFactory(description='\n                Second state of the situation.  In this state the sims should\n                be waiting to be let into the house.\n                ',
       tuning_group=GroupNames.STATE), 
     'party_situation_state':_PartySituationState.TunableFactory(description='\n                The third situation state.  In this state everyone parties!\n                ',
       tuning_group=GroupNames.STATE), 
     '_door_knocker_situation_job':TunableReference(description='\n                The job for the situation door knocker.  This sim will end up\n                being the host for the situation.\n                ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB)), 
     '_fruitcake_bearer_situation_job':TunableReference(description='\n                The job for the bearing of the vile nastiness known as...\n                \n                \n                ...fruitcake...\n                ',
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(3, _WaitToBeLetInState, factory=(cls.wait_to_be_let_in_state)),
         SituationStateData(4, _PartySituationState, factory=(cls.party_situation_state)))
...
sims4.tuning.instances.lock_instance_tunables(WelcomeWagon, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.NORMAL),
  creation_ui_option=(situations.situation_types.SituationCreationUIOption.NOT_AVAILABLE))