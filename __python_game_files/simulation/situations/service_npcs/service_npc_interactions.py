...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\service_npcs\service_npc_interactions.py
...
import event_testing, services, sims4.resources, ui
...
class PickServiceNpcSuperInteraction(PickerSuperInteraction):
    INSTANCE_TUNABLES = {'service_npcs':TunableList(description='\n            A list of the service npcs that will show up in the dialog picker\n            ',
      ...
      ServiceNpcHireable,),
       pack_safe=True),
       already_hired_tooltip=TunableLocalizedStringFactory(description='\n                    Tooltip that displays if the service has already been\n                    hired.\n                    '),
    ...
    def picker_rows_gen(cls, inst, target, context, **kwargs):
        ...
        for service_npc_data in service_npc_data_tuples:
            ...
            if not is_enabled:
                if tooltip is None:
                    continue
                row = ui.ui_dialog_picker.ObjectPickerRow(is_enable=is_enabled, name=display_name,
                ...
                if allows_recurring:
                    ...
                    row = ui.ui_dialog_picker.ObjectPickerRow(is_enable=is_enabled, name=(service_npc_type._recurring.recurring_name),
                      icon=(service_npc_type.icon),
                      row_description=display_description,
                      tag=tag)
                    yield row
        ...
        for entry in cls.non_service_npcs:
            ...
            if not result:
                if result.tooltip is None:
                    continue
                row = ui.ui_dialog_picker.ObjectPickerRow(is_enable=(result == TestResult.TRUE), name=(entry.name()),
                  icon=(entry.icon),
    ...
    def on_choice_selected(self, choice_tag, **kwargs):
        ...
        if tag is not None:
            if isinstance(tag, PickServiceNpcSuperInteraction._ServiceNpcRecurringPair):
                ...
                if tag.hire_interaction is not None:
