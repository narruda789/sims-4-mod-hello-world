...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\wildlife_encounter_director.py
# Compiled at: 2020-08-18 14:01:33
...
import services, sims4.log, random, alarms, date_and_time
...
class WildlifeEncounterDirectorMixin:
    ...
    WILDLIFE_ENCOUNTER_GROUPS = TunableMapping(description='\n        Mapping of Group enum to max instances of active broadcasters per group\n        ',
      key_type=TunableEnumEntry(tunable_type=WildlifeEncounterGroups,
      default=(WildlifeEncounterGroups.INVALID),
      invalid_enums=(
     WildlifeEncounterGroups.INVALID,)),
      value_type=TunableRange(tunable_type=int,
      minimum=0,
      default=1,
      display_name='max_active_instances'))
    INSTANCE_TUNABLES = {'wildlife_encounter_tuning': OptionalTunable(tunable=TunableList(description='\n                List of details for each encounter area wildlife broadcaster\n                ',
                                    ...
                                    pack_safe=True),
                                    chance_of_activation=TunablePercent(description='\n                        Percent chance that this broadcaster will be activated.\n                        ',
                                    default=0.0),
                                    group_name=TunableEnumEntry(tunable_type=WildlifeEncounterGroups,
                                    default=(WildlifeEncounterGroups.INVALID)))),
                                    enabled_name='encounter_area_list')}
    ...
    def on_startup(self):
        ...
        if self.wildlife_encounter_tuning:
            ...
            if current_day != self._last_encounter_roll_date:
                ...
                self._alarm_handler = alarms.add_alarm(self, time_span, (self.roll_active_encounters_callback), repeating=True,
            ...
            for encounter_data in wildlife_encounter_data_list:
                ...
                for area_object in area_objects:
                    if not area_object.has_component(types.STATE_COMPONENT):
                        continue
                    if self._wildlife_group_count[encounter_data.group_name] > self.WILDLIFE_ENCOUNTER_GROUPS[encounter_data.group_name]:
                        area_object.set_state(self.INACTIVE_STATE.state, self.INACTIVE_STATE)
                        continue
                    if random.random() < encounter_data.chance_of_activation:
...
class WildlifeEncounterTestByGroup(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    FACTORY_TUNABLES = {'wildlife_encounter_group':TunableEnumEntry(description='\n            The wildlife encounter group to test against.\n            ',
       tunable_type=WildlifeEncounterGroups,
       default=WildlifeEncounterGroups.INVALID,
       invalid_enums=(
      WildlifeEncounterGroups.INVALID,)), 
