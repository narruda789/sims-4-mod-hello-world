...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\walkby_magic_duel_situation.py
# Compiled at: 2019-12-05 16:20:41
# Size of source mod 2**32: 4865 bytes
import itertools, situations
...
class WalkbyMagicDuelSituation(FavoriteObjectSituationMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'go_to_dueling_ground_state':GoToDuelingGroundSituationState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':True},
       tuning_group=GroupNames.STATE), 
     'duel_state':DuelSituationState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':True},
       tuning_group=GroupNames.STATE)}

    @classmethod
    def _states(cls):
        return (SituationStateData.from_auto_factory(1, cls.go_to_dueling_ground_state),
         SituationStateData.from_auto_factory(2, cls.duel_state))
...
sims4.tuning.instances.lock_instance_tunables(WalkbyMagicDuelSituation, creation_ui_option=(situations.situation_types.SituationCreationUIOption.NOT_AVAILABLE),
  duration=0,
