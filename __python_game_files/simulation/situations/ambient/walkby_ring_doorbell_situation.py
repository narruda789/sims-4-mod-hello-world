...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\walkby_ring_doorbell_situation.py
# Compiled at: 2018-07-20 19:03:09
...
import alarms, clock, interactions, interactions.context, services, sims4.log, sims4.tuning.instances, sims4.tuning.tunable, situations.bouncer
...
class WalkbyRingDoorBellSituation(SituationSupportKeyMixin, WalkbyLimitingTagsMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'walker_job':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                          A reference to the SituationJob used for the Sim performing the walkby\n                          '),
     ...
     'wait_for_invitation_delay':sims4.tuning.tunable.TunableSimMinute(description='\n                                        The amount of time to wait for a Sim to greet the walker Sim.',
       default=60,
       tuning_group=GroupNames.SITUATION), 
    ...
    def _states(cls):
        return (SituationStateData(2, _RingDoorBellState),
         SituationStateData(3, _MailboxState),
         SituationStateData(4, _WaitForInvitationState),
         SituationStateData(5, _LeaveState))
    ...
    @classmethod
    def _can_start_walkby(cls, lot_id):
...
sims4.tuning.instances.lock_instance_tunables(WalkbyRingDoorBellSituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.WALKBY),
