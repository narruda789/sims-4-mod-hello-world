...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\soaker_family_situation.py
...
class SoakerFamilySituation(SituationComplexCommon, SubSituationOwnerMixin):
    INSTANCE_TUNABLES = {'group_filter':TunableAggregateFilter.TunableReference(description='\n            The aggregate filter that we use to find the sims for this\n            situation.\n            '), 
     ...
     'wait_for_sub_situation_state':WaitforSubSituationEnd.TunableFactory(description='\n            A state for getting the Sims to \n            ',
    ...
    def _states(cls):
        return (SituationStateData(1, GetSimsState),
         SituationStateData(2, WaitforSubSituationEnd, cls.wait_for_sub_situation_state))
    ...
    def on_all_sims_spawned(self):
        ...
        for sim_id in list(self._guest_list.get_invited_sim_ids()):
            ...
            sub_situation_id = self._create_sub_situation((self.soaker_situation), guest_list=guest_list,
              user_facing=False)
