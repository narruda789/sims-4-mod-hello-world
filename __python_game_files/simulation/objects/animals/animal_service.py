...
class AnimalService(Service):
    ...
    def on_cleanup_zone_objects(self, client):
        ...
        for animal_id, home_data in assignments_to_check.items():
            ...
            if home_data is None:
                if animal is None:
                    ...
                    if lot_owner_household is not None:
                        if is_residential:
                            continue
                        household_id = animal.get_household_owner_id()
                        if household_id is None or household_manager.get(household_id) is None:
                        ...
                        if has_owning_household:
                            if home_data.persist_assignment_in_household_inventory:
                                if build_buy.is_household_inventory_available(owner_household_id) and build_buy.object_exists_in_household_inventory(obj_id_to_check, owner_household_id):
                                    continue
        ...
        homes_to_delete = []
        for home_id, home_data in self._registered_homes.items():
            if home_data.zone_id != zone_id:
                if home_data.open_street_id != street_id:
                    continue
                if obj_manager.get(home_id) is not None:
                    continue
                home_data.current_occupancy or homes_to_delete.append(home_id)
