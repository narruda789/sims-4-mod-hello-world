...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\routing_component.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 59658 bytes
...
import gsi_handlers, objects, placement, services, sims4.log
...
class RoutingComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=types.ROUTING_COMPONENT):
    ...
    FACTORY_TUNABLES = {'plan_context_data':TunableTuple(description='\n            Data used to populate fields on the path plan context.\n            ',
      ...
      SpeciesExtended.INVALID,)),
       context_override=(PathPlanContextWrapper.TunableFactory())))), 
     'walkstyle_behavior':WalksStyleBehavior.TunableFactory(description='\n            Define the walkstyle behavior for owners of this component.\n            '), 
    ...
    def __init__(self, owner, **kwargs):
        ...
        self._walkstyle_requests = [
         WalkStyleRequest((self.owner), walkstyle=(walkstyle_behavior.default_walkstyle), priority=(-1))]
    ...
    def get_valid_walkstyle_and_request(self, cost_tuning):
        ...
        for walkstyle_request in self._walkstyle_requests:
            if walkstyle_request.priority != WalkStylePriority.COMBO:
                if carrying_anything:
                    if walkstyle_behavior.incompatible_with_carry(walkstyle_request):
                        continue
                walkstyle_cost = cost_tuning.get(walkstyle_request.walkstyle, None)
                if walkstyle_cost is None:
                    return (
                     walkstyle_request.walkstyle, walkstyle_request)
        ...
        return (
         walkstyle_behavior.default_walkstyle, None)
    ...
    def _get_walkstyle_key(self):
        if self.owner.is_sim:
            return (
             self.owner.age, self.owner.gender, self.owner.extended_species)
        return (
         Age.ADULT, Gender.MALE, SpeciesExtended.HUMAN)

    @componentmethod
    def request_walkstyle(self, walkstyle_request, uid):
        if walkstyle_request.priority != WalkStylePriority.COMBO:
            if not has_walkstyle_info(walkstyle_request.walkstyle, *self._get_walkstyle_key()):
                return
    ...
    def remove_callbacks(self):
        ...
        if self.owner._on_location_changed_callbacks is not None:
            if self._check_violations in self.owner._on_location_changed_callbacks:
                self.owner.unregister_on_location_changed(self._check_violations)
        if self.owner._on_location_changed_callbacks is not None:
            if self._update_quadtree_location in self.owner._on_location_changed_callbacks:
    ...
    def should_track_path_transition(self, transition_type):
        if self._path_node_transitions[transition_type] <= 0:
            return False
        if transition_type == PathNodeTransition.PATH_NODE_TRANSITION_FIRST_INDOOR or transition_type == PathNodeTransition.PATH_NODE_TRANSITION_LAST_INDOOR:
            if not self.pathplan_context.get_key_mask() & FOOTPRINT_KEY_ON_LOT:
                return False
    ...
    def write_slave_data_msg(self, route_msg, path=None):
        ...
        for slave_data in self.get_routing_slave_data():
            ...
            if slave_data.should_slave_for_path(path):
                if slave_data.slave in transitioning_sims or slave_data.has_affordance_pause_slave_routing():
                    continue
    ...
    def get_stand_slot_reservation_violators(self, excluded_sims=()):
        ...
        for sim_nearby in placement.get_nearby_sims_gen((polygon.centroid()), routing_surface, radius=(polygon.radius()),
          exclude=excluded_sims):
            if sims4.geometry.test_point_in_polygon(sim_nearby.position, polygon):
                sim_nearby.ignore_blocking_near_destination or violators.append(sim_nearby)
    ...
    def get_approximate_cancel_location(self):
        ...
        if follow_path is not None:
            ...
            if ret is not None:
                return ret
        return (
         self.owner.intended_transform, self.owner.intended_routing_surface)
    ...
    def set_routing_path(self, path):
        if path is None:
            ...
            if self.owner.is_sim:
                if self.owner.transition_controller is not None:
                    if self._initial_carry_targets != self.owner.posture_state.carry_targets:
        ...
        origin_q = (
         float(self.owner.orientation.x), float(self.owner.orientation.y), float(self.owner.orientation.z), float(self.owner.orientation.w))
    ...
    def remove_route_event_provider(self, request):
        if self._route_event_provider_requests is not None:
            if request in self._route_event_provider_requests:
    ...
    def schedule_and_process_route_events_for_new_path(self, path):
        if self._route_event_context is None:
            return
        if self.owner.is_sim:
            if not self.owner.posture.mobile:
                return
    ...
    def update_route_events_for_current_path(self, path, current_time, time_offset):
        ...
        if gsi_handlers.route_event_handlers.archiver.enabled:
            if gsi_handlers.route_event_handlers.update_log_enabled:
    ...
    def should_route_instantly(self):
        ...
        if zone.force_route_instantly:
            return True
        if self.owner.is_sim:
            return zone.are_sims_hitting_their_marks and self.owner._allow_route_instantly_when_hitting_marks or False
