...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\display_component.py
...
class DisplayComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=types.DISPLAY_COMPONENT):
    ...
    def handle_event(self, sim_info, event, resolver):
        if sim_info is not None:
            ...
            return self._is_on_display_parent or None
    ...
    def slotted_to_object(self, parent):
        if self._should_change_display_state(parent):
            if self.owner.has_state(self.DISPLAY_STATE.state):
                self.owner.set_state(self.DISPLAY_STATE.state, self.DISPLAY_STATE)
        self._handle_inventory_changed()

    def unslotted_from_object(self, parent):
        if self._should_change_display_state(parent):
            if self.owner.has_state(self.DEFAULT_STATE.state):
