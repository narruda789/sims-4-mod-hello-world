...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\carryable_component.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 46207 bytes
import collections, copy, functools, operator
...
import build_buy, objects.components, placement, services, sims4.log, sims4.resources
...
class PutDownLiability(Liability):
    ...
    def release(self):
        ...
        if target_object.parent is not self._parent:
            ...
            return
        if not carry_component.attempted_putdown and carry_component.attempted_alternative_putdown or target_object.transient:
            ...
            if not CarryingObject.snap_to_good_location_on_floor(target_object):
                ...
                sim.household.id is not target_object.get_household_owner_id() or sim.inventory_component.player_try_add_object(target_object) or target_object.release_sim(sim)
...
class CarryTargetInteraction(ProxyInteraction):
    ...
    def get_participants(cls, inst, participant_type, *args, **kwargs):
        ...
        if participant_type == ParticipantType.CarriedObject:
            if inst_or_cls._carry_target_ref is not None:
                return {
                 inst_or_cls._carry_target_ref()}
...
class CarryableComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=objects.components.types.CARRYABLE_COMPONENT):
    ...
    class _CarryableAllowedHands(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'biped_allowed_hands':TunableVariant(locked_args={'both':(
          ...
          'right_only':(
           Hand.RIGHT,)},
           default='both'), 
         'quadruped_allowed_hands':TunableVariant(locked_args={'both':(
           Hand.LEFT, Hand.RIGHT), 
          'mouth_only':(
           Hand.RIGHT,), 
          'back_only':(
    ...
    FACTORY_TUNABLES = {'put_down_tuning':TunableVariant(description='\n            Define how Sims prefer to put this object down.\n            ',
     ...
     'state_based_put_down_tuning':TunableMapping(description='\n            A mapping from a state value to a putdownstrategy. If the owning\n            object is in any of the states tuned here, it will use that state\'s\n            associated putdownstrategy in place of the one putdownstrategy tuned\n            in the "put_down_tuning" field. If the object is in multiple states\n            listed in this mapping, the behavior is undefined.\n            ',
       ...
       value_type=TunableVariant(reference=TunablePutDownStrategySpeciesMapping(description='\n                    Tuning for how to score where a Sim might want to set an\n                    object down.\n                    ')),
       key_name='State',
       value_name='PutDownStrategy'), 
     'carry_affordances':OptionalTunable(tunable=TunableList(tunable=TunableReference(description='\n                    The versions of the HoldObject affordance that this object\n                    supports.\n                    ',
       manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION)))),
    ...
    def _get_adjusted_circle_constraint(self, sim, constraint):
        if not self.owner.is_sim:
            ...
            return isinstance(constraint, TunedCircle) or constraint
    ...
    def get_provided_aops_gen(self, target, context, **kwargs):
        for provided_affordance_data in self.provided_affordances:
            if not provided_affordance_data.affordance.is_affordance_available(context=context):
                ...
                if provided_affordance_data.affordance.is_social:
                    affordance = target.is_sim or provided_affordance_data.affordance
                ...
                if parent is not None:
                    if parent.is_sim:
                        ...
                        if carry_posture is not None and provided_affordance_data.is_linked:
    ...
    def get_initial_put_down_position(self, carrying_sim=None):
        ...
        if carrying_sim is None:
            return (
             self.owner.position, self.owner.routing_surface)
        ...
        return (
         position, carrying_sim.routing_surface)
    ...
    def get_put_down_aop(self, interaction, context, alternative_multiplier=1, own_inventory_multiplier=1, object_inventory_multiplier=DEFAULT, in_slot_multiplier=DEFAULT, on_floor_multiplier=1, visibility_override=None, display_name_override=None, additional_post_run_autonomy_commodities=None, add_putdown_liability=False, **kwargs):
        ...
        if not put_down_strategy.affordances:
        ...
        elif self._attempted_alternative_putdown:
            if self.owner.is_sim:
                self._attempted_alternative_putdown = True
                scored_aops = []
                for scored_aop in self._gen_affordance_score_and_aops(interaction, slot_types_and_costs=slot_types_and_costs,
                  ...
                  additional_post_run_autonomy_commodities=additional_post_run_autonomy_commodities,
            ...
            if add_putdown_liability:
                liabilities = (
                 (
                  PutDownLiability.LIABILITY_TOKEN, PutDownLiability(self.owner)),)
        else:
            liabilities = ()
        aop = AffordanceObjectPair(affordance, self.owner, affordance, None, slot_types_and_costs=slot_types_and_costs, 
         world_cost=world_cost, 
         sim_inventory_cost=sim_inventory_cost, 
         object_inventory_cost=object_inventory_cost, 
         terrain_transform=terrain_transform, 
         ...
         display_name_override=display_name_override, 
         additional_post_run_autonomy_commodities=additional_post_run_autonomy_commodities, 
         liabilities=liabilities, **kwargs)
    ...
    def _gen_affordance_score_and_aops(self, interaction, slot_types_and_costs, world_cost, sim_inventory_cost, object_inventory_cost, terrain_transform, terrain_routing_surface, objects_with_inventory, visibility_override, display_name_override, additional_post_run_autonomy_commodities, multiplier=1, add_putdown_liability=False):
        ...
        for affordance in put_down_strategy.affordances:
            if add_putdown_liability:
                ...
                liabilities = ()
            aop = AffordanceObjectPair(affordance, (self.owner),
              affordance,
              None,
    ...
    def _get_terrain_transform(self, interaction):
        if not self.owner.is_sim:
            if self.owner.footprint_component is None:
                return (None, None)
        else:
            sim = interaction.sim
        ...
        starting_location = placement.create_starting_location(position=starting_position, orientation=(sim.orientation),
        ...
        fgl_context = fgl_context_fn(starting_location, (self.owner), max_steps=MAX_PUTDOWN_STEPS,
