...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\get_put_component_mixin.py
...
import interactions.constraints, sims4.log
...
class GenericAnimation(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def get_access_constraint(self, put, inventory_owner):
        ...
        constraint_total = interactions.constraints.Constraint(debug_name='Required Surface For Generic Get Put',
          posture_state_spec=surface_posture_state_spec)
...
class CustomAnimation(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'get':TunableAnimationReference(callback=None), 
     'put':TunableAnimationReference(callback=None)}
...
class GetPutComponentMixin(HasTunableFactory, metaclass=ComponentMetaclass):
    ...
    def get_surface_target(self, sim):
        ...
        if body_target is not None:
            if body_target.inventory_component is not None:
                if body_target.inventory_component.inventory_type == inv_owner.inventory_component.inventory_type:
                    return body_target
        return inv_owner
    ...
    def _get_access_animation(self, is_put: bool):
        ...
        def append_animations(arb, sim, carry_target, carry_track, animation_context, surface_height):
            asm = sim.posture.get_registered_asm(animation_context, (animation_factory.asm_key),
              None,
              use_cache=False)
            asm.set_parameter('surfaceHeight', surface_height)
            result = sim.posture.setup_asm_interaction(asm, sim, None, (animation_factory.actor_name),
              None,
