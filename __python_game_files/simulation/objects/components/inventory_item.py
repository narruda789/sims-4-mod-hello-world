...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\inventory_item.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 67143 bytes
...
from event_testing.resolver import GlobalResolver, SingleActorAndObjectResolver, SingleSimResolver
...
from objects.components.state import ObjectStateValue, TunableStateValueReference
...
from sims4.tuning.tunable import TunableEnumEntry, TunableList, TunableReference, Tunable, AutoFactoryInit, HasTunableFactory, TunableTuple, OptionalTunable, TunableVariant, TunableSet, TunableSimMinute, TunableMapping, HasTunableSingletonFactory
import caches, itertools, services, sims4.log
...
class InventoryItemComponent(Component, HasTunableFactory, AutoFactoryInit, SuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin, component_name=types.INVENTORY_ITEM_COMPONENT, persistence_key=protocols.PersistenceMaster.PersistableData.InventoryItemComponent):
    ...
    STACK_SCHEME_OPTIONS = TunableMapping(description='\n        This mapping allows special functionality for dynamic stack schemes.  This allows things like:\n        - Ability to specify a stack icon.\n        - Ability to specify the tooltip text that is shown in the stack hovertip.\n        ',
      key_type=TunableEnumEntry(tunable_type=StackScheme,
      default=(StackScheme.NONE),
      invalid_enums=(
     StackScheme.NONE, StackScheme.DEFINITION, StackScheme.VARIANT_GROUP)),
      ...
      title=TunableLocalizedString(description='Tooltip title'),
      tooltip_description=TunableLocalizedString(description='Tooltip description'))))

    class TunableStackSchemeOverride(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'state_value':ObjectStateValue.TunablePackSafeReference(), 
         'stack_scheme':TunableEnumEntry(tunable_type=StackScheme,
           default=StackScheme.NONE)}
    ...
    def on_added_to_inventory(self):
        inventory = self.get_inventory()
        if self.enter_inventory_state_value:
            if self.enter_inventory_state_valid_inventory_types:
                if not inventory is not None or inventory.inventory_type in self.enter_inventory_state_valid_inventory_types:
    ...
    def on_before_removed_from_inventory(self):
        ...
        if inventory is None:
            return
        if self.exit_inventory_state_value:
            if not self.exit_inventory_state_valid_inventory_types or inventory.inventory_type in self.exit_inventory_state_valid_inventory_types:
        ...
        if owner is not None:
            ...
            if inventory is not None:
                if inventory.inventory_type not in (InventoryType.MAILBOX, InventoryType.HIDDEN):
                    ...
                    if owner.is_sim and not self.target_super_affordances:
                        if self.super_affordances:
    ...
    def populate_stack_icon_info_data(self, icon_info_msg):
        ...
        if stack_options is None:
            logger.error('{} does not have stack options, but they were requested for {}.', (self.get_stack_scheme()),
              (self.owner), owner='jdimailig')
            return
    ...
    def component_super_affordances_gen(self, **kwargs):
        if self.owner.get_users():
            ...
            lot = self.inventory_only or None
            ...
            for valid_type in self.valid_inventory_types:
                if valid_type == InventoryType.SIM:
                    if self.skip_carry_pose:
                        ...
                        if obj_inventory_found or self.skip_carry_pose:
                            continue
                        if not lot:
    ...
    def place_in_world_affordances_gen(self):
        if not self.inventory_only:
            return self.can_place_in_world or None
        elif self.skip_carry_pose:
            if self.no_carry_add_to_world_affordances is None:
                yield from self.DEFAULT_NO_CARRY_ADD_TO_WORLD_AFFORDANCES
            else:
                yield from self.no_carry_add_to_world_affordances
    ...
    def set_inventory_type(self, inventory_type, owner, from_removal=False):
        if self._current_inventory_type != None:
            ...
            from_removal or self.owner.update_object_tooltip()
    ...
    def get_inventory_plex_id(self):
        ...
        if not inventory_type is None:
            return InventoryTypeTuning.is_shared_between_objects(inventory_type) or None
        else:
            plex_service = services.get_plex_service()
            return plex_service.is_active_zone_a_plex or None
    ...
    def save(self, persistence_master_message):
        ...
        elif self.save_for_stack_compaction:
    ...
    def refresh_decay_modifiers(self):
        ...
        for decay_modifier in effects.decay_modifiers:
            ...
            if test_result or self._is_decay_modifiers_applied:
    ...
    def _try_get_sim_owner(self):
        ...
        while not getattr(current, 'is_sim', False):
            if current_inventory is not None and current_inventory.owner is not None and current_inventory.owner != current:
