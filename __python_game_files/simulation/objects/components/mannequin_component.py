...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\mannequin_component.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 34430 bytes
...
import distributor.fields, distributor.ops, enum, services, sims4.resources
...
def get_mannequin_group_data(mannequin_group, mannequin_data):
    ...
    key = (
     mannequin_group, mannequin_data.age, mannequin_data.gender)
...
class MannequinComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=MANNEQUIN_COMPONENT, persistence_key=SimObjectAttributes_pb2.PersistenceMaster.PersistableData.MannequinComponent):
    ...
    class _MannequinTemplateResource(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'resource_key':TunableResourceKey(description='\n                The SimInfo file to use.\n                ',
         ...
         'outfit':OptionalTunable(description='\n                If enabled, the mannequin will default to the specified outfit.\n                ',
           ...
           outfit_index=TunableRange(description='\n                        The outfit index.\n                        ',
           tunable_type=int,
           minimum=0,
           maximum=4,
           default=0)))}
        ...
        def create_sim_info_data(self, sim_id):
            ...
            if self.outfit is not None:
                outfit_to_set = (
                 self.outfit.outfit_category, self.outfit.outfit_index)
    ...
    def _on_outfit_generated(self, outfit_category, outfit_index, from_reconcile_mannequin=False):
        if from_reconcile_mannequin:
            return
    ...
    def on_add(self, *_, **__):
        ...
        if not zone.is_zone_loading:
            if not self.owner.is_downloaded:
    ...
    def on_state_changed(self, state, old_value, new_value, from_init):
        ...
        if old_appearance_modifier is not None:
            ...
            if new_appearance_modifier is not None:
                self._sim_info_data.appearance_tracker.add_appearance_modifiers((new_appearance_modifier.appearance_modifiers), state,
                  (new_appearance_modifier.priority),
                  (new_appearance_modifier.apply_to_all_outfits),
                  (new_appearance_modifier.additional_flags),
                  source=self)
    ...
    def _replace_outfits(self, sim_info_data, from_reconcile_mannequin=False):
        ...
        for outfit_category, outfit_list in sim_info_data.get_all_outfits():
            ...
            for outfit_index, outfit_data in enumerate(outfit_list):
                ...
                self._sim_info_data.generate_merged_outfit(sim_info_data, destination_outfit,
                  default_outfit,
                  source_outfit,
                  preserve_outfit_flags=True,
                  from_reconcile_mannequin=from_reconcile_mannequin)
                ...
                if mannequin_group_sharing_mode == MannequinGroupSharingMode.ACCEPT_THEIRS:
                    ...
                    for mannequin in mannequin_group_data:
                        if mannequin is not self:
                            mannequin._replace_outfits((self._sim_info_data), from_reconcile_mannequin=True)
                            mannequin._resend_mannequin_data()

                    return
                if mannequin_group_sharing_mode == MannequinGroupSharingMode.ACCEPT_YOURS:
                    self._replace_outfits((mannequin_group_data.get_mannequin_data()), from_reconcile_mannequin=True)
                    return
                    ...
                    for outfit_index, outfit_data in enumerate(outfit_list):
                        ...
                        if outfits_in_category is not None:
                            if len(outfits_in_category) >= get_maximum_outfits_for_category(outfit_category):
                                ...
                                continue
                        mannequin_data.generate_merged_outfit((self._sim_info_data), (mannequin_data.add_outfit(outfit_category, outfit_data)),
                          (mannequin_data.get_current_outfit()),
                          (
                         outfit_category, outfit_index),
                          preserve_outfit_flags=True,
                          from_reconcile_mannequin=True)

            mannequin_group_data.reconcile_mannequin_data()
            return
        if self.owner.id:
            for outfit_category in REGULAR_OUTFIT_CATEGORIES:
                self._sim_info_data.generate_merged_outfits_for_category(mannequin_data, outfit_category, preserve_outfit_flags=True, from_reconcile_mannequin=True)