...
# Size of source mod 2**32: 21271 bytes
...
class VehicleComponent(FootprintToggleMixin, Component, HasTunableFactory, AutoFactoryInit, component_name=types.VEHICLE_COMPONENT):
    ...
    def should_deploy_for_path(self, path, routing_surface):
        if not self.deploy_tuning is None:
            if routing_surface.type not in self.allowed_surfaces or path.length() <= self.minimum_route_distance:
                return False
            total_dist = 0.0
            if any((node.portal_object_id for node in path.nodes)) or any((node.tracked_terrain_tags for node in path.nodes)):
                if len(path.nodes) > 1:
                    ...
                    for next_node in nodes[1:]:
                        ...
                        if portal_obj and not self.can_transition_through_portal(portal_obj, next_node.portal_id):
                            break
                        else:
                            if portal_obj_id:
                                break
                            else:
                                if not self.can_transition_over_node(next_node, prev_node):
                                    break
