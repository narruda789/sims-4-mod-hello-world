...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\lighting_component.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 21417 bytes
...
import distributor.ops, distributor.system, services, sims4.log
...
class LightingComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=LIGHTING_COMPONENT, persistence_key=protocols.PersistenceMaster.PersistableData.LightingComponent):
    ...
    def light_material_states(self):
        return (
         self._material_state_on, self._material_state_off)
    ...
    @distributor.fields.ComponentField(op=(distributor.ops.SetHauntedLight))
    ...
    def _update_visual_effect(self):
        if self.visual_effect is None:
            ...
            if self._light_dimmer == 0:
                ...
                return
            elif self._light_dimmer == self.LIGHT_AUTOMATION_DIMMER_VALUE:
                auto_on_effect = True
            else:
    ...
    def update_lighting_enabled_state(self):
        if not self.is_power_off() or get_object_has_tag(self.owner.definition.id, LightingComponent.NON_ELECTRIC_LIGHT_TAG):
            if not self.disabling_state_values or any((self.owner.state_value_active(state_value) for state_value in self.disabling_state_values)) or self._disable_because_sold:
                ...
                if self.is_lighting_enabled():
                    ...
                    if not self.is_lighting_enabled():
