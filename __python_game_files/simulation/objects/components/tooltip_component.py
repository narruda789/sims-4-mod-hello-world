...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\tooltip_component.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 44114 bytes
...
import caches, objects.animals.animal_tests, objects.components, services, sims4.resources
...
class CustomTooltipTuningProvidingMixin:
    FACTORY_TUNABLES = {'custom_tooltips': TunableList(description='\n            List of possible tooltips that will be displayed on an object when\n            moused over.\n            Each tooltip has its set of tests which will be evaluated whenever\n            the object its created or when its state changes.  The test that \n            passes its the tooltip that the object will display.\n            ',
                          ...
                          additional_tests={'in_inventory':objects.object_tests.InInventoryTest.TunableFactory(locked_args={'tooltip': None}), 
                         'animal_test':objects.animals.animal_tests.AnimalTest.TunableFactory(locked_args={'tooltip': None})}),
...
class TooltipComponent(Component, CustomTooltipTuningProvidingMixin, TooltipProvidingComponentMixin, HasTunableFactory, AutoFactoryInit, component_name=types.TOOLTIP_COMPONENT):
    ...
    def register_calbacks(self):
        if not self._game_option_changed_callback_registered:
        ...
        if not self._stat_or_buck_changed_callback_registered:
    ...
    def update_object_tooltip(self):
        if not self.hovertip_requested:
                ...
                if self.tooltip_tests.run_tests(resolver):
                    for name, value, tooltip_override_data in tooltip_component._ui_metadata_gen():
                        ...
                        if external_field_data_tuple:
                            if not external_field_data_tuple.always_add:
            ...
            self.owner.hover_tip = self._ui_metadata_handles or ui_protocols.UiObjectMetadata.HOVER_TIP_DISABLED
            self.owner.update_ui_metadata()
    ...
    @componentmethod_with_fallback(lambda : None)
    def get_state_strings(self, first_strings=None):
        obj = self.owner
        int_token = 0
        ...
        bullet_points = first_strings if first_strings is not None else []
    ...
    def _ui_metadata_gen(self):
        ...
        for tooltip_data in self._get_custom_tooltips():
            ...
            for tooltip_key, tooltip_text in tooltip_data.tooltip_fields.items():
                if tooltip_text.text_tokens is not None:
                    ...
                    if None in tokens:
                        ...
                        tokens = ()
                    if tooltip_key == TooltipFields.rel_override_id and not len(tokens) == 0:
                        if tooltip_text.text_tokens.tokens[0].obj_property is None:
                    ...
                    if current_region is not None:
                        ...
                        if currency_type != BucksType.INVALID:
                            value_string = BucksUtils.BUCK_TYPE_TO_DISPLAY_DATA[currency_type].value_string
                            if value_string is not None:
                                currency_string = value_string
                    yield (
                     TooltipFieldsComplete(tooltip_key).name, currency_string(*tokens), tooltip_text.override_component_information)
                else:
                    yield (
                     TooltipFieldsComplete(tooltip_key).name, (tooltip_text.text)(*tokens), tooltip_text.override_component_information)
            ...
            if tooltip_data.display_object_preference is not None:
                icon_infos = owner.is_sim or []
                ...
                if object_preference_tracker is not None:
                    ...
                    for part in owner.parts:
                        if not part.restrict_autonomy_preference:
                            continue
                        self._get_restriction_icon_info_msg(object_preference_tracker, object_id,
                          icon_infos,
                    ...
                    if icon_infos:
                        yield (
                         TooltipFieldsComplete.icon_infos.name, icon_infos, None)

        if owner.non_deletable_by_user:
            if owner.is_in_inventory():
                non_sellable_text_data = TooltipComponent.NON_SELLABLE_BY_PLAYER_TEXT
                yield (
                 TooltipFieldsComplete.stolen_from_text.name, non_sellable_text_data.text(),
    ...
    def update_tooltip_field(self, field_id, field_data, priority=0, should_update=False, always_add=False, immediate=False):
        ...
        if data_priority_tuple is None or priority >= data_priority_tuple.field_priority:
