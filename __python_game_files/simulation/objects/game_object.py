...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\game_object.py
# Compiled at: 2022-08-26 18:13:12
# Size of source mod 2**32: 70981 bytes
...
import sims4.log, sims4.protocol_buffer_utils
...
import alarms, autonomy, build_buy, clock, distributor.fields, objects, placement, reset, routing, services
...
class GameObject(ClientObjectMixin, ReservationMixin, ScriptObject, reset.ResettableObjectMixin):
    INSTANCE_TUNABLES = {'_transient_tuning':Tunable(description='\n            If transient the object will always be destroyed and never put down.\n            ',
     ...
     'additional_interaction_constraints':TunableList(description='\n            A list of constraints that must be fulfilled in order to run the \n            linked affordances. This should only be used when the same \n            affordance uses different constraints based on the object.\n            ',
       ...
       affordance_links=(TunableAffordanceFilterSnippet())),
       tuning_filter=FilterTag.EXPERT_MODE), 
        ...
        if cls._provided_mobile_posture is not None:
            for affordance in cls._provided_mobile_posture.affordances:
                if affordance.provided_posture_type is None:
                    ...
                    if not affordance.provided_posture_type.unconstrained:
    ...
    def has_tag(self, tag):
        ...
        if self._persisted_tags:
            if tag in self._persisted_tags:
                return True
    ...
    def get_icon_info_data(self):
        return IconInfoData(obj_instance=self, obj_def_id=(self.definition.id),
          obj_geo_hash=(self.geometry_state),
          obj_material_hash=(self.material_hash),
          obj_name=(LocalizationHelperTuning.get_object_name(self)))
    ...
    def get_edge_constraint(self, constraint_width=1.0, inward_dir=False, return_constraint_list=False, los_reference_point=DEFAULT, sim=None):
        ...
        for start, stop in edges:
            ...
            if inward_dir:
                polygon = sims4.geometry.Polygon([start,
                 start + constraint_width * inward,
                 stop + constraint_width * inward,
                 stop])
            else:
                polygon = sims4.geometry.Polygon([start, stop,
                 stop - constraint_width * inward,
                 start - constraint_width * inward])
    ...
    def _notify_buildbuy_of_location_change(self, old_location):
        if self.persistence_group == PersistenceGroups.OBJECT:
            if self._should_invalidate_location():
    ...
    def set_build_buy_lockout_state(self, lockout_state, lockout_timer=None):
        if self._build_buy_lockout_alarm_handler is not None:
            ...
            if self._build_buy_lockout:
                if lockout_state:
                    return
            elif lockout_state and lockout_timer is not None:
    ...
    def unregister_on_location_changed(self, callback):
        if self._on_location_changed_callbacks is None:
            ...
            self._on_location_changed_callbacks = self._on_location_changed_callbacks or None
    ...
    def unregister_on_hidden_or_shown(self, callback):
        if self._on_hidden_or_shown_callbacks is None:
            ...
            self._on_hidden_or_shown_callbacks = self._on_hidden_or_shown_callbacks or None
    ...
    def is_in_navmesh(self):
        if self._routing_context is not None:
            if self._routing_context.object_footprint_id is not None:
                return True
        return False
    ...
    def _update_persistence_group(self):
        ...
        if self.persistence_group == objects.persistence_groups.PersistenceGroups.OBJECT:
            services.current_zone().lot.is_position_on_lot(self.position, 0) or remove_object_from_buildbuy_system(self.id)
            ...
            if self.persistence_group == objects.persistence_groups.PersistenceGroups.IN_OPEN_STREET:
                if services.current_zone().lot.is_position_on_lot(self.position, 0):
                    self.persistence_group = objects.persistence_groups.PersistenceGroups.OBJECT
                    add_object_to_buildbuy_system(self.id)

    def _fixup_pool_surface(self):
        if self.item_location == ItemLocation.FROM_WORLD_FILE or self.item_location == ItemLocation.FROM_CONDITIONAL_LAYER:
            if self.routing_surface.type != SurfaceType.SURFACETYPE_POOL:
                if build_buy.PlacementFlags.REQUIRES_WATER_SURFACE & build_buy.get_object_placement_flags(self.definition.id):
                    if get_water_depth_at_location(self.location) > 0:
                        routing_surface = self.routing_surface
    ...
    def on_remove(self):
        ...
        super().on_remove()
        zone.is_zone_shutting_down or self._remove_from_world()
        ...
        if self.is_fire_related_object:
            ...
            elif posture_graph.is_object_mobile_posture_compatible(self):
    ...
    def _natural_ground_status_change(self, *_, **__):
        if self.routing_surface is not None:
            if self.routing_surface.type == SurfaceType.SURFACETYPE_POOL:
                return
        elif self.is_on_natural_ground():
    ...
    def try_mark_as_new_object(self):
        return self.should_mark_as_new and services.current_zone().is_in_build_buy or None
    ...
    def is_surface(self, include_parts=False, ignore_deco_slots=False):
        if self._is_surface is None:
            ...
            key = (
             include_parts, ignore_deco_slots)
        ...
        def is_valid_surface_slot(slot_type):
            if not (ignore_deco_slots and slot_type.is_deco_slot):
                if slot_type.implies_owner_object_is_surface:
                    return True
            return False

        for runtime_slot in self.get_runtime_slots_gen():
            if not include_parts:
                if runtime_slot.owner is not self:
                    continue
    ...
    def get_save_lot_coords_and_level(self):
        ...
        if parent is not None and parent.is_sim:
            ...
            starting_location = placement.create_starting_location(position=starting_position, orientation=(parent.orientation),
              routing_surface=(self.location.world_routing_surface))
    ...
    def is_set_as_head(self):
        ...
        if parent is None:
            ...
            return self.is_same_object_or_part(parent_head) or False
        return True
