...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\gardening\gardening_loot_ops.py
# Compiled at: 2019-06-04 20:25:11
# Size of source mod 2**32: 6009 bytes
import placement, services, sims4.log
...
class CreatePlantAtLocationLootOperation(BaseLootOperation):
    FACTORY_TUNABLES = {'fruits_to_sprout_from':TunableList(description='\n            A list of weighted fruit definitions we want to create a plant from.\n            ',
       ...
       fruit=TunableReference(manager=(services.get_instance_manager(Types.OBJECT)), pack_safe=True),
       weight=(TunableMultiplier.TunableFactory())),
       minlength=1), 
     'states_to_set_on_plant':TunableList(description='\n            The states to set on the created plant.\n            ',
       tunable=TunableReference(manager=(services.get_instance_manager(Types.OBJECT_STATE)), class_restrictions=('ObjectStateValue', ),
       pack_safe=True),
       unique_entries=True)}
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        if not self._fruits_to_sprout_from:
            ...
            if parent_slot is None:
                starting_location = placement.create_starting_location(position=(subject.position),
                  routing_surface=(subject.routing_surface))
                fgl_context = placement.create_fgl_context_for_object(starting_location, created_fruit,
                  ignored_object_ids=(
                 created_fruit.id,))
            ...
            created_plant or logger.error(f"Failed to germinate {created_fruit}")
            return
