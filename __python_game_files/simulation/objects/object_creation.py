...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\object_creation.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 58097 bytes
import build_buy, enum, fishing.fishing_data, functools, objects.components.types, random, services, sims, sims4.log, sims4.resources
...
class _RecipeByTag(_RecipeBase, CreationDataBase):
    ...
    def choose_recipe(self):
        ...
        if not filtered_defs:
            logger.warn('_RecipeByTag could not find a recipe with the tag {}.', (self.recipe_tag),
              owner='skorman')
            return
...
class _RandomWeightedTaggedRecipe(_RandomRecipeBase, CreationDataBase, HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'weighted_recipe_tags': TunableList(description='\n            A list of weighted list of recipe tags that can be available for \n            recipe creation.\n            ',
                               ...
                               recipe_tag=_RecipeByTag.TunableFactory(recipe_factory_tuning={'pack_safe': True}),
...
class _RandomWeightedRecipe(_RandomRecipeBase, CreationDataBase, HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'weighted_recipes': TunableList(description='\n            A list of weighted list of recipes that can be available for recipe creation.\n            ',
                           tunable=TunableTuple(description='\n                The weighted recipe.\n                ',
                           recipe=_RecipeDefinition.TunableFactory(recipe_factory_tuning={'pack_safe': True}),
                           weight=(TunableMultiplier.TunableFactory())),
                           minlength=1)}
...
class _CloneObject(CreationDataBase, HasTunableSingletonFactory, AutoFactoryInit):
    ...
    FACTORY_TUNABLES = {'source_object':TunableVariant(description='\n            Where the object to be cloned can be found.\n            ',
       is_participant=_ParticipantObject.TunableFactory(),
       slotted_to_participant=_SlottedObject.TunableFactory(),
       default='slotted_to_participant'), 
     'definition_override':OptionalTunable(description='\n            Override to specify a different definition than that of the object\n            being cloned.\n            ',
...
class TunableFishingDataTargetVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, participant=_FishingDataFromParticipant.TunableFactory(), 
         reference=_FishingDataFromReference.TunableFactory(), 
         default='participant', **kwargs)
...
class _CraftableServing(CreationDataBase, GrabServingMixin, HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _get_recipe(self, resolver):
        ...
        if not (target is None or target.has_component(objects.components.types.CRAFTING_COMPONENT)):
...
class TunableObjectCreationDataVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, definition=_ObjectDefinition.TunableFactory(), 
         definition_tested=_ObjectDefinitionTested.TunableFactory(), 
         recipe=_RecipeDefinition.TunableFactory(), 
         recipe_by_tag=_RecipeByTag.TunableFactory(), 
         random_recipe=_RandomWeightedRecipe.TunableFactory(), 
         random_recipe_by_tags=_RandomWeightedTaggedRecipe.TunableFactory(), 
         serving=_CraftableServing.TunableFactory(), 
         clone_object=_CloneObject.TunableFactory(), 
         create_photo_object=_CreatePhotoObject.TunableFactory(), 
         random_by_tags=_RandomFromTags.TunableFactory(), 
         from_stored_object_info=_CreateObjectFromStoredObjectInfo.TunableFactory(), 
         from_fishing_data=_CreateObjectFromFishingData.TunableFactory(), 
...
class ObjectCreationMixin:
    ...
    def initialize_helper(self, resolver, post_add=None, creation_params=None):
        ...
        self._object_helper = CreateObjectHelper(reserved_sim, (self._definition),
          interaction,
          object_to_clone=(self.creation_data.get_source_object(self.resolver)),
          init=initialize_created_obj,
          post_add=post_add)
    ...
    def _setup_created_object(self, created_object, creation_stage):
        ...
        for initial_state in self.initial_states:
            ...
            if initial_state.tests is None or initial_state.tests.run_tests(self.resolver):
    ...
    def _place_object_no_fallback(self, created_object):
        ...
        if self.location.location == self.CARRY:
            return True
        return False
    ...
    def _place_object(self, created_object):
        ...
        if self._place_object_no_fallback(created_object):
            ...
            return True
        participant = self.destroy_on_placement_failure or self._get_fallback_location_target(created_object)
        if participant.is_sim:
            ...
            if location_type == self.INVENTORY:
                if self.location.mark_object_as_stolen_from_career:
                    interaction = self.resolver.interaction
                ...
                if participant is not None and participant.inventory_component is not None and created_object.inventoryitem_component is not None:
        ...
        if participant.inventory_component.player_try_add_object(created_object, hidden=(location_type == self.INVENTORY and self.location.place_in_hidden_inventory)):
            if self.notification_inventory:
                obj_name = [
                 LocalizationHelperTuning.get_object_name(created_object)]
        ...
        if not (sim is None or sim.is_sim):
...
class ObjectCreationOp(ObjectCreationMixin, BaseLootOperation):
    FACTORY_TUNABLES = {'creation_data':TunableObjectCreationDataVariant(description='\n            Define the object to create\n            ',
       all_objects_with_tag=_CreateAllObjectsWithTags.TunableFactory()), 
     'quantity':TunableRange(description='\n            The number of objects that will be created.\n            ',
       tunable_type=int,
       default=1,
       minimum=1,
       maximum=10)}

    def __init__(self, *, creation_data, initial_states, destroy_on_placement_failure, owner_sim, location, reserve_object, temporary_tags, quantity, notification_inventory, fallback_location_target_override, require_claim, set_sim_as_owner, set_value_to_crafted_tooltip, spawn_vfx, **kwargs):
