...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\pools\ocean.py
# Compiled at: 2020-03-05 20:56:35
# Size of source mod 2**32: 25713 bytes
...
class Ocean(SwimmingMixin, GameObject):
                    ...
                    for portal_id in portal_ids:
                        ...
                        if portal_instance is not None:
                            ...
                            if portal_id == portal_instance.there:
                                ...
                                if location:
                                    if build_buy.is_location_natural_ground(location.position, location.routing_surface.secondary_id):
    ...
    def on_finalize_load(self):
        ...
        if self._lot_locator_transforms:
            self._create_all_transforms_and_portals_for_initial_transforms((self._lot_locator_transforms),
              lot_transforms=True,
    ...
    def on_buildbuy_exit(self):
        ...
        if currently_in_portal_cache:
            portals_created or services.object_manager().remove_portal_from_cache(self)
        else:
            if not currently_in_portal_cache:
                if portals_created:
                    services.object_manager().add_portal_to_cache(self)
    ...
    def get_edge_constraint(self, constraint_width=1.0, inward_dir=False, return_constraint_list=False, los_reference_point=DEFAULT, sim=None):
        ...
        constraint = OceanStartLocationConstraint.create_simple_constraint(interval,
          constraint_width, sim, routing_surface=routing_surface, los_reference_point=los_reference_point)
    ...
    def get_nearest_constraint_start_location(self, species, age, start_position, interval: WaterDepthIntervals):
        surface_type = None
        if not interval == WaterDepthIntervals.WALK:
            if interval == WaterDepthIntervals.WET or interval == WaterDepthIntervals.WADE:
