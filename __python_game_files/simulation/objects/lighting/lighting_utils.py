...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\lighting\lighting_utils.py
# Compiled at: 2020-10-01 22:59:44
# Size of source mod 2**32: 11926 bytes
...
class _LightTargetAll(HasTunableSingletonFactory):
    ...
    def get_light_target_gen(self, target):
        yield from all_lights_gen(target)
        if False:
            yield None
...
class TunableLightTargetVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, this_light=_LightTargetInteraction.TunableFactory(), 
         all_lights=_LightTargetAll.TunableFactory(), 
         room_lights=_LightTargetRoom.TunableFactory(), 
         specific_lights=_LightTargetFromTags.TunableFactory(), 
         default='this_light', **kwargs)
...
class TunableDimmerValueVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, use_previous_setting=_DimmerValueFromPreviousSetting.TunableFactory(), 
         automated_by_client=_DimmerValueFromClient.TunableFactory(), 
         specify_dimmer_value=_DimmerValueSpecified.TunableFactory(), 
         do_not_change_dimmer_value=_DoNotChangeDimmerValue.TunableFactory(), 
         default='automated_by_client', **kwargs)
...
class TunableLightColorVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, use_previous_setting=_LightColorFromPreviousSetting.TunableFactory(), 
         do_not_change_light_color=_LightColorNoChange.TunableFactory(), 
         specify_light_color=_LightColorSpecified.TunableFactory(), 
         default='do_not_change_light_color', **kwargs)
