...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\helpers\user_footprint_helper.py
...
import placement, routing, services, sims4.log
...
class UserFootprintHelper:
    ...
    def __init__(self, owner=None, width=None, cost=None, get_focus_fn=None, get_enabled_fn=None):
        ...
        if get_focus_fn is None:
            if hasattr(owner, 'register_on_location_changed'):
    ...
    def force_move_sims_in_polygon(polygon, routing_surface, exclude=()):
        nearby_sims = placement.get_nearby_sims_gen((polygon.centroid()), routing_surface, radius=(polygon.radius()),
          exclude=exclude)
        for near_sim in nearby_sims:
            if sims4.geometry.test_point_in_polygon(near_sim.position, polygon):
                ...
                if near_sim.si_state is None:
                    continue
                total_constraint = near_sim.si_state.get_total_constraint(include_inertial_sis=True,
    ...
    def _on_sim_posture_event(self, change, dest_state, track, old_posture, new_posture):
        ...
        elif change == PostureEvent.TRANSITION_COMPLETE:
            if self._footprints.get(sim) == FP_REMOVED_BY_POSTURE:
                self._add_user_footprint(sim)
...
def push_route_away(sim):
    ...
    sim.push_super_affordance((UserFootprintHelper.MOVE_SIM_AFFORDANCE), sim, context,
      name_override='MoveSimFromDiscouragementRegion')