...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\base_interactions.py
# Compiled at: 2021-03-08 15:51:41
# Size of source mod 2**32: 34429 bytes
import operator
...
import element_utils, interactions, services, sims4.log, sims4.math, sims4.resources
...
class JoinInteraction(ProxyInteraction):
    ...
    def get_name(cls, inst, target=DEFAULT, context=DEFAULT, **kwargs):
        ...
        if cls.joinable_info.join_available:
            if cls.joinable_info.join_available.loc_custom_join_name is not None:
...
class AskToJoinInteraction(ProxyInteraction, ImmediateSuperInteraction):
    ...
    def _get_name(cls, inst, target=DEFAULT, context=DEFAULT, **kwargs):
        ...
        if cls.joinable_info.invite_available:
            if cls.joinable_info.invite_available.loc_custom_invite_name is not None:
    ...
    def _push_join_interaction(self, join_sim):
        join_interaction = JoinInteraction.generate((self.proxied_affordance), join_interaction=(self.join_interaction),
          joinable_info=(self.joinable_info))
...
class AggregateSuperInteraction(SuperInteraction):
    ...
    def _get_tested_aops(cls, inst, target, context, **interaction_parameters):
        inst_or_cls = inst if inst is not None else cls
        if inst is not None:
            if inst._valid_aops is not None:
    ...
    def _constraint_gen(cls, inst, sim, target, participant_type=ParticipantType.Actor, **kwargs):
        ...
        if inst_or_cls.use_aggregated_affordance_constraints:
            ...
            affordances = []
            if inst is not None:
                valid_aops = inst._valid_aops
                if valid_aops is None:
                    valid_aops = (inst._get_tested_aops)((inst.target), (inst.context), **inst.interaction_parameters)
                    if valid_aops is None:
                        return Nowhere('Using aggregated affordance constraints, but no valid aops.')
                affordances = [aop.super_affordance for aop, _ in valid_aops]
            affordances = affordances if affordances else [affordance_tuple.affordance for affordance_tuple in inst_or_cls.aggregated_affordances]
            if not affordances:
                yield Nowhere
            for aggregated_affordance in affordances:
                intersection = ANYWHERE
                constraint_gen = aggregated_affordance._constraint_gen
                if aggregated_affordance.is_social:
    ...
    def _do_perform_gen(self, timeline):
        ...
        if not aops_valid:
            ...
            for aop in aops_valid:
                ...
                if not self.use_aggregated_affordance_constraints or interactions.si_state.SIState.test_compatibility(interaction, force_concrete=True):
                    compatible_interactions.append(interaction)

            return compatible_interactions or ExecuteResult.NONE
...
class AggregateMixerInteraction(MixerInteraction):
    INSTANCE_TUNABLES = {'aggregated_affordances': TunableList(description='\n                A list of affordances composing this aggregate. A random one\n                will be chosen from sub-action weights if multiple interactions\n                pass at the same priority.\n                ',
                                 ...
                                 tunable_type=int,
                                 default=0),
                                 affordance=MixerInteraction.TunableReference(description='\n                        The aggregated affordance.\n                        ',
                                 pack_safe=True)),
                                 tuning_group=(GroupNames.GENERAL))}
    _allow_user_directed = True
    ...
    def _do_perform_gen(self, timeline):
        ...
        if not aops_valid:
            ...
            return interactions_by_weight or ExecuteResult.NONE
...
class RenameImmediateInteraction(ImmediateSuperInteraction):
    ...
    INSTANCE_TUNABLES = {'display_name_rename':OptionalTunable(TunableLocalizedStringFactory(description="If set, this localized string will be used as the interaction's display name if the object has been previously renamed.")), 
     'rename_dialog':TunableVariant(description='\n            The rename dialog to show when running this interaction.\n            ',
    ...
    def _run_interaction_gen(self, timeline):
        ...
        elif target_name_component.allow_description:
