...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\object_tests.py
# Compiled at: 2022-08-26 18:13:12
# Size of source mod 2**32: 164947 bytes
import build_buy, collections, functools, itertools
...
import enum, event_testing.test_base, objects.components.inventory_enums, routing, services, sims, sims4, statistics.mood, tag
        ...
        if obj.is_sim:
            ...
            if self.test_type == TagTestType.CONTAINS_NO_TAGS_IN_SET:
                if not object_tags & self.tag_set:
                    return True
                return False
        logger.error('ObjectTagFactory received unrecognized TagTestType {}, defaulting to False', (self.test_type), owner='tingyul')
        return False
...
class CraftActualItemFactory(TunableFactory):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(items_to_check=TunableList(TunableReference((services.definition_manager()), description='Object that qualifies for this check.')), 
         description='This option tests crafted item against a list of possible items', **kwargs)
...
class CraftTaggedItemFactory(TunableFactory):
    ...
    def __init__(self, description=DEFAULT_DESCRIPTION, **kwargs):
        (super().__init__)(tag_set=TunableSet(TunableEnumEntry((tag.Tag), (tag.Tag.INVALID), pack_safe=True, description='What tag to test for'), description='The tag of objects we want to test ownership of'), 
...
class BasicStateCheckFactory(TunableFactory):
    ...
    def factory(tested_object, state_set, test_type, object_requirement, **kwargs):
        if tested_object.state_component is None:
            return False
            if object_requirement is not None:
                if object_requirement.type == BasicStateCheckFactory.TAG_TYPE:
                    if not object_requirement.tag(tested_object, None):
                        return False
        elif object_requirement.type == BasicStateCheckFactory.DEFINITION_TYPE:
            if tested_object.definition is not object_requirement.definition:
                return False
    ...
    def __init__(self, **kwargs):
        (super().__init__)(description="\n            This option tests crafted item's tags against a list of possible\n            tags.", 
         state_set=TunableSet(TunableReference(description='\n                What state to test for.',
  ...
  tunable_type=StateTestType,
  default=(StateTestType.CONTAINS_ANY_STATE_IN_SET)), 
         object_requirement=TunableVariant(tag=TunableTuple(tag=CraftTaggedItemFactory(description='\n                        The object must have this tag.\n                        '),
  locked_args={'type': BasicStateCheckFactory.TAG_TYPE}),
  definition=TunableTuple(definition=TunableReference(description='\n                        The object must have this definition.\n                        ',
  manager=(services.definition_manager())),
  locked_args={'type': BasicStateCheckFactory.DEFINITION_TYPE}),
  locked_args={'any_object': None},
  default='any_object'), **kwargs)
...
class TunableObjectStateValueThreshold(TunableThreshold):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(value=TunableReference(manager=services.get_instance_manager(sims4.resources.Types.OBJECT_STATE), 
...
class ObjectPurchasedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'test_type':TunableVariant(description='\n            The object we want to test for.\n            ',
       object=ObjectTypeFactory.TunableFactory(),
       tag_set=ObjectTagFactory.TunableFactory(),
       locked_args={'any_object': None},
       default='any_object'), 
    ...
    def __call__(self, obj=None):
        if obj is None:
            return TestResultNumeric(False, 'ObjectPurchasedTest: Object is None, normal during zone load.', current_value=0,
              goal_value=(self.value_threshold.value),
              is_money=True)
        else:
            if self.test_type is not None:
                if not self.test_type(obj):
                    return TestResultNumeric(False, 'ObjectPurchasedTest: Invalid object type: {}', obj, current_value=0, goal_value=(self.value_threshold.value),
                      is_money=True)
            obj_value = obj.depreciated_value if self.use_depreciated_value else obj.catalog_value
            return self.value_threshold.compare(obj_value) or TestResultNumeric(False, 'ObjectPurchasedTest: Incorrect or invalid value object purchased for test: {}, value: {}', obj,
              obj_value, current_value=obj_value, goal_value=(self.value_threshold.value), is_money=True)
        return TestResult.TRUE
...
class ObjectScoringPreferenceTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def get_expected_args(self):
        ...
        return {'affordance':ParticipantType.Affordance, 
         'targets':ParticipantType.Object, 
            ...
            for target in targets:
                if tracker.get_restricted_sims(target.id, self.any_sim) is None:
                    if self.require:
                        ...
                        return self.require or TestResult(False, 'Object has restriction but test does not want one', tooltip=(self.tooltip))
            ...
            for target in targets:
                if preference.use_only:
                    ...
                    for target_object in target_objects:
                        use_type = tracker.get_restriction((context.sim), target_object,
                          (preference.tag),
                          full_object=(target_object.is_sim or not target_object.is_part),
                          allow_test=False)
...
class ObjectEnvironmentScoreTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'description':"\n            Test the object's environment score for a particular mood againts a threshold.\n            ", 
     'sim_participant':OptionalTunable(description='\n            An Optional Sim to test Environment Score against. If disabled, the\n            Environment Score will not take into acount any Trait modifiers\n            relative to the Sim. If enabled, Trait modifiers will be taken into\n            account.\n            ',
       tunable=TunableEnumEntry(tunable_type=ParticipantTypeActorTargetSim, default=(ParticipantTypeActorTargetSim.TargetSim))), 
     'object_to_test':TunableEnumEntry(description='\n            The object particiant we want to check the environment score of.\n            ',
       tunable_type=ParticipantType,
       default=ParticipantType.Object), 
     'environment_score_type':TunableVariant(description='\n            The type of environment score to test against. This can be mood\n            based, positive scoring, or negative scoring.\n            ',
        ...
        for target in objects_to_test:
            ...
            if self.environment_score_type.scoring_type == EnvironmentScoreType.MOOD_SCORING:
                ...
                if not self.environment_score_type.threshold.compare(mood_score):
                    ...
                    return self.environment_score_type.threshold.compare(negative_score) or TestResult(False, 'Object does not meet negative environment score requirements.', tooltip=(self.tooltip))
...
class CraftedItemTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'skill_or_item':TunableVariant(description='\n            Whether to test for a specific item or use of a skill for the item.\n            ',
       crafted_with_skill=CraftedWithSkillFactory(),
       crafted_actual_item=CraftActualItemFactory(),
       crafted_tagged_item=CraftTaggedItemFactory(),
       default='crafted_with_skill'), 
     'quality_threshold':OptionalTunable(description='\n            If enabled, require the item to match a certain quality threshold.\n            ',
    ...
    def get_expected_args(self):
        return {'crafted_object':event_testing.test_constants.FROM_EVENT_DATA, 
         'skill':event_testing.test_constants.FROM_EVENT_DATA, 
         'quality':event_testing.test_constants.FROM_EVENT_DATA, 
         'masterwork':event_testing.test_constants.FROM_EVENT_DATA}
...
class CraftedWithIngredientsTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'ingredients_used_threshold':TunableThreshold(description='\n            Number of ingredients required for the item to have been crafted with\n            ',
       value=TunableRange(tunable_type=int,
       default=1,
       minimum=0),
       default=sims4.math.Threshold(value=1,
       comparison=(sims4.math.Operator.GREATER_OR_EQUAL.function))), 
     'participant':TunableEnumEntry(description='\n            The participant for which to check ingredient count on.\n            ',
       tunable_type=ParticipantTypeSingle,
       default=ParticipantTypeSingle.CraftingObject)}
    ...
    def __call__(self, crafted_object=None):
        if crafted_object is None:
            ...
            return self.ingredients_used_threshold.compare(num_ingredients_used) or TestResult(False, ('CraftedWithIngredientsTest: Item did not use required amount of ingredients.                                         ingredient count needed: {}, ingredients_used: {}, comparison: {}'.format(self.ingredients_used_threshold.value, num_ingredients_used, self.ingredients_used_threshold.comparison)),
...
class CountTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'participant':TunableObjectGeneratorVariant(description='\n            The participant that is being counted.\n            '), 
     'threshold':TunableVariant(description='\n            The value to compare the participant count to.\n            ',
       from_value=TunableThreshold(),
       from_count=_TunableParticipantCountThreshold.TunableFactory(),
       default='from_value')}
...
class ExistenceTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def _exists(self, obj):
        ...
        if self.participant == ParticipantType.StoredSimOrNameData:
            return True
        if obj.is_sim:
            if self.require_instantiatable:
                if not obj.can_instantiate_sim:
                    return False
            if self.require_instantiated:
    ...
    def __call__(self, test_targets=()):
        if self.exists:
            return test_targets and all((self._exists(obj) for obj in test_targets)) or TestResult(False, 'Participant {} does not exist', (self.participant), tooltip=(self.tooltip))
        else:
            if any((self._exists(obj) for obj in test_targets)):
...
class ScheduledSituationSpecialObjectTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'object_tags':OptionalTunable(description='\n            Tagging requirements on the object. \n            ',
       tunable=ObjectTagFactory.TunableFactory()), 
     'situation_type':TunablePackSafeReference(description='\n            The type of situations you want to look for. \n            ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION)), 
     'host_sim':TunableEnumEntry(description='\n            The sim to compare against the host sim of a situation. \n            ',
    ...
    def __call__(self, target_objects=None, host_sim=None):
        ...
        for object_target in target_objects:
            if self.object_tags:
                return self.object_tags(object_target) or TestResult(False, ('Object {} did not have the required tags.'.format(object_target.id)),
                  tooltip=(self.tooltip))
        ...
        for drama_node in drama_scheduler.all_nodes_gen():
            ...
            if situation_seed is None:
                continue
            if not situation_seed.special_object_definition_id is None:
                if situation_seed.host_sim_id != host_sim.sim_id or situation_seed.situation_type != self.situation_type:
                    continue
                for object_target in target_objects:
                    if object_target.definition.id == situation_seed.special_object_definition_id:
                        return TestResult.TRUE

        return TestResult(False, "No object was found that matched the drama node's situation's special object.",
          tooltip=(self.tooltip))
...
class InventoryTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'inventory_location':TunableVariant(description='\n            Who owns the inventory. Either look in the inventory of a \n            participant or specify an object inventory type directly.\n            \n            If participant returns multiple inventory owners, the test will \n            pass only if ALL of those owners pass the inventory content test.\n            ',
       ...
       tunable_type=ParticipantType,
       default=(ParticipantType.Actor)),
       locked_args={'location_type': PARTICIPANT_INVENTORY}),
       object_inventory_type=TunableTuple(inventory=TunableEnumEntry(description='\n                    Check the global Object inventory that has the specified type.\n                    EX: check in the global fridge inventory that exists for all\n                    fridges\n                    ',
       ...
       locked_args={'location_type': GLOBAL_OBJECT_INVENTORY}),
       hidden_inventory_objects=TunableTuple(inventory=TunableEnumEntry(description='\n                    Check in the hidden inventory for objects that can go into the\n                    specified inventory type. EX: check that there are mailbox\n                    objects in the hidden inventory\n                    ',
       tunable_type=(objects.components.inventory_enums.InventoryType),
       default=(objects.components.inventory_enums.InventoryType.UNDEFINED)),
       locked_args={'location_type': HIDDEN_INVENTORY}),
       default='participant_inventory'), 
     'contents_check':TunableVariant(description='\n            Checks to run on each object of the specified inventory\n            ',
       has_object_with_tag=CraftTaggedItemFactory(locked_args={'content_check_type': TAGGED_ITEM_TEST}),
       has_object_with_def=TunableTuple(definition=TunablePackSafeReference(description='\n                    The object definition to look for inside inventory.\n                    ',
       manager=(services.definition_manager())),
       locked_args={'content_check_type': ITEM_DEFINITION_TEST}),
       has_object_of_participant_type=TunableTuple(description='\n                Participant type we want to test if its in the selected\n                inventory.\n                ',
       participant=TunableEnumEntry(description='\n                    Which participant of the interaction do we want to validate\n                    on the inventory. \n                    ',
       tunable_type=ParticipantType,
       default=(ParticipantType.Object)),
       locked_args={'content_check_type': PARTICIPANT_TYPE_TEST}),
       has_object_with_states=BasicStateCheckFactory(locked_args={'content_check_type': ITEM_STATE_TEST}),
       has_object_in_situation=TunableTuple(description='\n                \n                ',
    ...
    def get_expected_args(self):
        ...
        if self.contents_check is not None:
            if self.contents_check.content_check_type == self.SITUATION_SPECIAL_OBJECT_TEST:
    ...
    def __call__(self, inventory_owners=None, content_check_participant=None, **kwargs):
        ...
        if not inventory_owners:
            ...
            return inventories or TestResult(False, 'Inventory {} does not exist or has no items', (self.inventory_location), tooltip=(self.tooltip))
        else:
        ...
        for household in households:
            ...
            if not self.required_count.compare(count):
                return TestResultNumeric(False, '{} Household Inventory does not have required number of objects in it',
                  household,
                  tooltip=(self.tooltip),
                  current_value=count,
                  goal_value=(self.required_count.value),
                  is_money=False)
        ...
        for inv in inventories:
            ...
            if not self.required_count.compare(count):
                return TestResultNumeric(False, 'Inventory {} does not have required number of objects in it',
                  inv,
                  tooltip=(self.tooltip),
                  current_value=count,
                  goal_value=(self.required_count.value),
        ...
        return TestResultNumeric(True, current_value=count,
          goal_value=(self.required_count.value),
          is_money=False)
        ...
        for obj in objs:
            if not obj.is_in_inventory():
                if not self.negate:
                    ...
                    if (self.inventory_types or self).negate:
                    ...
                    if self.test_hidden:
                        ...
                        return hidden or self.negate or TestResult(False, 'Failed InInventory test. Participant is unexpectedly not hidden in the inventory.', tooltip=(self.tooltip))
...
class ObjectRelationshipTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    ALL_OBJECTS = 1
    test_events = (
     TestEvent.ObjectRelationshipChanged,)
    FACTORY_TUNABLES = {'sims':TunableEnumEntry(description='\n            The Sim(s) to test.\n            ',
       tunable_type=ParticipantType,
       default=ParticipantType.Actor), 
     'targets':TunableVariant(all_objects=TunableTuple(locked_args={'subject_type': ALL_OBJECTS},
       object_filter=TunableVariant(description='\n                    A test to filter down to a specific type of object in order\n                    to check object relationships.\n                    ',
       definition_id=(ObjectTypeFactory.TunableFactory()),
       tags=(ObjectTagFactory.TunableFactory()),
       default='definition_id')),
       single_object=TunableTuple(locked_args={'subject_type': TARGET_OBJECTS},
     ...
     'relationship_status':TunableVariant(description="\n            Whether the object cannot have a relationship with the sim, or the\n            sim and object's relationship value is within a tuned range.\n            \n            For test on relationship range:\n            If a sim does not have a relationship with the object and\n            use_default_value_if_no_relationship is checked, the relationship \n            value used will be the initial value of the relationship statistic\n            used to track relationships on the object. If sim doesn't have a\n            relationship and use_default_value_if_no_relationship is NOT checked,\n            the test will fail (so you can use that to check if a relationship\n            exists).\n            ",
       ...
       tunable_type=bool,
       default=False),
       value_interval=TunableInterval(tunable_type=float, default_lower=(-100),
       default_upper=100)),
       locked_args={'no_relationship_exists': None},
       default='relationship_range'), 
     'can_add_relationship':OptionalTunable(Tunable(description='\n            If checked, this object must be able to add a new relationship.  If\n            unchecked, this object must not be able to add any more\n            relationships.\n            ',
       tunable_type=bool,
       default=True)), 
    ...
    def __call__(self, sims, targets=None):
            ...
            for target in targets:
                ...
                if self.relationship_status is None:
                    if has_relationship:
                        ...
                        if not self.relationship_status.use_default_value_if_no_relationship:
        ...
        if self.threshold.compare(passing_objects):
            return TestResult.TRUE
        return TestResultNumeric(False, 'Only {} objects pass the threshold for requirements for object relationships.',
          passing_objects,
          tooltip=(self.tooltip),
          current_value=passing_objects,
...
class CustomNameTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'participant':TunableEnumEntry(description='\n            The subject who is the object of this test.',
       tunable_type=ParticipantType,
       default=ParticipantType.Object), 
     'has_custom_name':OptionalTunable(Tunable(description='\n            If checked, the subject must have a custom name set. If unchecked, it cannot have a custom name set.',
       tunable_type=bool,
       default=True)), 
     'has_custom_description':OptionalTunable(Tunable(description='\n            If checked, the subject must have a custom description set. If unchecked, it cannot have a custom description set.',
       tunable_type=bool,
       default=True))}
    ...
    def __call__(self, targets=()):
        for target in targets:
            if target.is_sim:
                ...
                elif self.has_custom_name is not None and target.has_custom_name() != self.has_custom_name:
...
class InUseTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def get_expected_args(self):
        return {'actors':ParticipantType.Actor, 
         'targets':self.targets, 
         'picked_sim':ParticipantType.PickedSim}
    ...
    def __call__(self, actors=None, targets=None, picked_sim=None):
            ...
            if target.is_part:
                ...
                if self.number_using is not None:
                    if self.candidates == self.Candidates.PICKED_SIM:
                        ...
                        if self.candidates == self.Candidates.NON_ACTORS:
                    ...
                    if correct_count ^ self.negate:
                        return TestResult.TRUE
                    return TestResult(False, 'InUseTest: Number of users required to be {} {} and {}, but the actual number of users is {}.', ('without' if self.negate else 'within'),
                      (self.number_using.lower_bound), (self.number_using.upper_bound), num_using, tooltip=(self.tooltip))
                    if self.candidates == self.Candidates.PICKED_SIM:
                    ...
                    if self.candidates == self.Candidates.NOT_ACTORS_OR_ENSEMBLE:
        ...
        return TestResult(False, 'Failed in_use test because object {} in use', ('is' if self.negate else "isn't"),
          tooltip=(self.tooltip))
...
class HasFreePartTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def get_expected_args(self):
        return {'actor':ParticipantType.Actor, 
         'targets':self.targets}
            ...
            if parent_objects is None:
                if self.negate:
                    ...
                    if parent is not None:
                        if not self.require_parent_is_surface or parent.is_surface(include_parts=(self.check_part_owner)):
                ...
                if target.parent.is_sim:
                    test_parent = target.parent.sim_info
                else:
                    test_parent = target.parent
                if test_parent is not None:
                    if self.check_part_owner:
                        if test_parent.is_part:
                            test_parent = test_parent.part_owner
                if test_parent is not None:
                    if self.require_parent_is_surface:
                        if not test_parent.is_surface(include_parts=(self.check_part_owner)):
                            test_parent = None
                if self.negate:
                    if test_parent is not None:
...
class HasChildObjectOnPartTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def __call__(self, targets=()):
        if not targets:
            return TestResult(False, 'HasChildObjectOnPartTest: No test targets given.',
              tooltip=(self.tooltip))
        found_match = False
        for target in targets:
            if target.is_part:
            ...
            for part in target.parts:
                if part.subroot_index is None or self.subroot_index is None:
                    if part.subroot_index is not None or self.subroot_index is not None:
                        continue
        ...
        if not found_match:
            if self.negate:
                return TestResult(False, 'HasChildObjectOnPartTest: No part at subroot {} found without children.',
                  (self.subroot_index),
                  tooltip=(self.tooltip))
            return TestResult(False, 'HasChildObjectOnPartTest: No part at subroot {} found with children.',
              (self.subroot_index),
              tooltip=(self.tooltip))
        return TestResult.TRUE
...
class IsCarryingObjectTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'participant':TunableEnumEntry(description='\n            The subject of this test.\n            ',
       tunable_type=ParticipantType,
       default=ParticipantType.Actor), 
     'object_type':TunableVariant(object_reference=TunableReference(description='\n                A type of object required to be carried.\n                ',
       manager=(services.definition_manager())),
       object_tag=TunableEnumEntry(description='\n                A tag to test the target object for.\n                ',
       tunable_type=Tag,
       default=(Tag.INVALID)),
       default='object_reference'), 
    ...
    def __call__(self, test_targets=None):
        for target in test_targets:
            ...
            if sim is None:
                ...
                return is_carrying or self.negate or TestResult(False, 'IsCarryingObjectTest: {} is not carrying {}.', (sim.full_name), (self.object_type), tooltip=(self.tooltip))
...
class ConsumableTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, subject=None, target=None):
        ...
        if consumable_component is None:
            if self.is_consumable:
                return TestResult(False, 'Object {} is not a consumable but is expected to be.',
                  subject,
                  tooltip=(self.tooltip))
        else:
            if consumable_component is not None:
                if not self.is_consumable:
                    return TestResult(False, 'Object {} is a consumable but is expected not to be.',
                      subject,
                      tooltip=(self.tooltip))
            ...
            return threshold.compare(bites_left_in_subject) or TestResult(False, 'Object {} is expected to have {} {} bites left, but actually has {} bites left.',
              subject,
              (self.bites_left.operator),
              (self.bites_left.value),
              bites_left_in_subject,
              tooltip=(self.tooltip))
        return TestResult.TRUE
...
class CanSeeObjectTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def get_expected_args(self):
        return {'sim_info_list':ParticipantType.Actor, 
         'target_list':ParticipantType.Object}
...
class GameTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def get_expected_args(self):
        return {'participants':self.participant, 
         ...
         'objects':ParticipantType.Object, 
         'context':ParticipantType.InteractionContext}
    ...
    def __call__(self, participants, actor, target, objects, context):
        ...
        if game is None:
            ...
            for sim_info in sim_infos:
                ...
                for si in sim.si_state:
                    ...
                    if posture_target is not None and posture_target.game_component is not None:
        ...
        for participant in participants:
            if not participant.is_sim:
                ...
                if self.is_sim_turn is not None:
                    if game.is_sim_turn(sim) != self.is_sim_turn:
                ...
                if self.number_of_players is not None:
                    ...
                    if not self.number_of_players.lower_bound <= player_num <= self.number_of_players.upper_bound:
                        return TestResult(False, 'GameTest: Number of players required to be withing {} and {}, but the actual number of players is {}.', (self.number_of_players.lower_bound),
                          (self.number_of_players.upper_bound), player_num, tooltip=(self.tooltip))
                ...
                if self.high_score_comparison is not None:
                    if game.high_score is not None:
                        stat = sim.get_statistic(self.high_score_comparison.stat)
                        if stat is None:
                            return TestResult(False, 'GameTest: Failed to find statistic {} in Sim {}.', (self.high_score_comparison.stat),
                              sim, tooltip=(self.tooltip))
                        ...
                        if not threshold.compare(stat.get_value()):
                            ...
                            return TestResult(False, 'GameTest: Failed comparison check: Stat Value ({}) {} Game High Score ({}).', (stat.get_value()),
                              operator_symbol, (game.high_score), tooltip=(self.tooltip))
                ...
                if self.can_join is not None:
                    if game.is_joinable(sim) != self.can_join:
                        return TestResult(False, "GameTest: Sim's join status is not correct.", tooltip=(self.tooltip))
                if self.can_join_autonomously_after_user_directed is not None:
                    if game.is_joinable_autonomously_after_user_directed(context) != self.can_join_autonomously_after_user_directed:
                        return TestResult(False, "GameTest: Sim's join autonomously status is not correct.", tooltip=(self.tooltip))
                if self.require_challenge_availability:
                    if game.challenge_sims is not None:
                        if sim not in game.challenge_sims:
        ...
        for obj in objs:
            ...
            for target in test_targets:
                ...
                if self.test_household_owner:
                    ...
                    if self.consider_renter_as_household_owner and not owner_id:
                        if target_sim.is_renting_zone(current_zone_id):
                            owner_id = sim_id
                else:
...
ObjectCriteriaSingleObjectSpecificTest = collections.namedtuple('ObjectCriteriaSingleObjectSpecificTest', ('subject_type',
                                                                                                           'target'))
...
class ObjectCriteriaTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if test_events:
            if test_events[0] == ObjectCriteriaTestEvents.AllObjectEvents:
                self.test_events = (
                 TestEvent.OnExitBuildBuy,
                 TestEvent.ObjectStateChange,
                 TestEvent.ItemCrafted,
                 TestEvent.OnInventoryChanged)
    ...
    def object_meets_criteria(self, obj, active_household_id, active_sim_id, current_zone, positional_relationship_participants=None):
        if self.owned:
            if obj.get_household_owner_id() != active_household_id:
                return False
            else:
                if self.sim_ownership:
                    if obj.get_sim_owner_id() != active_sim_id:
                        return False
                if self.on_active_lot:
                    if not obj.is_on_active_lot():
                        return False
                if self.positional_relationship_test is not None:
                    return self.positional_relationship_test.evaluate(obj, participants=positional_relationship_participants) or False
            if self.completed:
                if obj.crafting_component is not None:
    ...
    def __call__(self, target_objects=None, positional_relationship_participants=None):
        ...
        if self.subject_specific_tests.subject_type == self.ALL_OBJECTS and not self.subject_specific_tests.quantity.compare(number_of_matches):
            return TestResultNumeric(False, 'There are {} matches when {} matches are needed for the object criteria tuning',
              number_of_matches,
              (self.subject_specific_tests.quantity.value), current_value=number_of_matches,
              goal_value=(self.subject_specific_tests.quantity.value),
              is_money=False,
              tooltip=(self.tooltip))
            if self.subject_specific_tests.total_value is not None:
                if not self.subject_specific_tests.total_value.compare(total_value):
                    return TestResultNumeric(False, 'The total value is {} when it needs to be {} for the object criteria tuning',
                      total_value,
                      (self.subject_specific_tests.total_value.value), current_value=total_value,
                      goal_value=(self.subject_specific_tests.total_value.value),
                      is_money=True,
                      tooltip=(self.tooltip))
                else:
                    if not target_objects or number_of_matches != len(target_objects):
...
class ObjectDefinitionCriteriaTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    ...
    def __call__(self, target_objects=None):
        if target_objects is None:
            return TestResult(False, 'There are no target objects.',
              tooltip=(self.tooltip))
        for obj in target_objects:
            if not self.identity_test(obj):
                return TestResult(False, "All of the specified targets don't meet the object definition criteria tuning.",
                  tooltip=(self.tooltip))
...
class SituationObjectComparisonTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self):
        ...
        if not threshold.compare(situation_count):
            ...
            return TestResult(False, 'Failed Situation Object Comparison Check: Situations ({}) {} Objects ({})',
              situation_count,
              operator_symbol,
              object_count,
              tooltip=(self.tooltip))
    ...
    def __call__(self, sims=tuple()):
        ...
        for sim_info in sims:
            ...
            if sim.routing_surface.type != routing.SurfaceType.SURFACETYPE_OBJECT:
                return TestResult(False, 'Sim {} is not routing on an object.',
                  sim,
                  tooltip=(self.tooltip))
            ...
            nearby_surfaces = quadtree.query(bounds=bounds, surface_id=surface_id,
              filter=(ObjectRoutableSurfaceTest.QUADTREE_FILTER))
            for surface_info in nearby_surfaces:
                ...
                return TestResult(False, 'Failed to find surface that matched object criteria for Sim {}',
                  sim,
                  tooltip=(self.tooltip))
            ...
            if stored_audio_component is None:
                ...
                if stored_audio_component.sound != self.sound:
                    ...
                    if not snippet is None:
                        if not snippet.fixed_length_audio != self.sound or snippet.looping_audio != self.sound:
...
class ActiveRoutingObjectsTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'tracking_category': TunableEnumEntry(description='\n            Category to test against. Test returns TRUE if the number of active\n            routing objects of this type is below the tuned maximum.\n            ',
                            tunable_type=ObjectRoutingBehaviorTrackingCategory,
                            default=(ObjectRoutingBehaviorTrackingCategory.NONE))}
...
class ObjectConnectivityTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, from_object, to_object):
        ...
        if isinstance(to_object, sims.sim_info.SimInfo):
            ...
            return routing.test_connectivity_pt_pt(from_object.routing_location, routing.Location(target_position, routing_surface=(to_object.location.routing_surface)), from_object.routing_context) or TestResult(False, 'No connectivity from {} to {}.', from_object, to_object)
        return TestResult.TRUE
...
class ObjectFashionOutfitPrevalentTrendTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'participant':TunableEnumEntry(description='\n            The specified participant to run the object fashion outfit prevalent trend tests on.\n            ',
       tunable_type=ParticipantTypeObject,
       default=ParticipantTypeObject.ObjectTrendiOutfitTrendTag), 
     'object_fashion_outfit_prevalent_tags':TunableList(description='\n            A list of the fashion outfit style tags to check for.\n            ',
    ...
    def __call__(self, target_objects=()):
        if not target_objects:
            return TestResult(False, 'Trying to test and object fashion outfit tag when the target to test is None',
              tooltip=(self.tooltip))
        for target_object in target_objects:
            if target_object in self.object_fashion_outfit_prevalent_tags:
                break
            else:
                return TestResult(False, 'Specified target object {} does not have a child with the tag {}',
                  target_object,
                  (self.object_fashion_outfit_prevalent_tags),
                  tooltip=(self.tooltip))
