...
# Size of source mod 2**32: 23723 bytes
    ...
    def __call__(self, test_targets=None, skill=None):
        ...
        for target in test_targets:
            if skill_tag is None:
                ...
                if num_passed >= quantity or self.invert:
        ...
        if self.invert:
            return TestResultNumeric(False, 'The number of applicable skills: {} was too high to pass: {} inverted.',
              num_passed,
              quantity,
              current_value=num_passed,
              goal_value=quantity,
...
class SkillHasUnlockedAll(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    def __call__(self, test_targets=()):
        ...
        for target in test_targets:
            ...
            for skill_cls in statistic_manager.all_skills_gen():
                if target_age not in skill_cls.ages or skill_cls.hidden:
                    continue
