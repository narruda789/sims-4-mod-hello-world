...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\statistics\mood.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 25539 bytes
...
class TunableModifiers(TunableTuple):
    ...
    def __init__(self, **kwargs):
        super().__init__(add_modifier=Tunable(description='\n                The modifier to add to a value\n                ',
          tunable_type=float,
          default=0),
          multiply_modifier=Tunable(description='\n                The modifier to multiply a value by\n                ',
          tunable_type=float,
          default=1))
...
class TunableEnvironmentScoreModifiers(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def combine_modifiers(self, object_mood_modifiers, object_negative_modifiers, object_positive_modifiers):
        ...
        return (
         new_mood_modifiers, new_negative_modifiers, new_positive_modifiers)

    def get_mood_modifiers(self, mood):
        mood_mods = self.mood_modifiers.get(mood)
        if mood_mods is not None:
            return (
             mood_mods.add_modifier, mood_mods.multiply_modifier)
        return self.DEFAULT_MODIFIERS

    def get_negative_modifiers(self):
        if self.negative_modifiers is not None:
            return (
             self.negative_modifiers.add_modifier, self.negative_modifiers.multiply_modifier)
        return self.DEFAULT_MODIFIERS

    def get_positive_modifiers(self):
        if self.positive_modifiers is not None:
            return (
             self.positive_modifiers.add_modifier, self.positive_modifiers.multiply_modifier)
        return self.DEFAULT_MODIFIERS
...
class TunableMoodDescriptionTraitOverride(TunableTuple):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(trait=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.TRAIT)),
  allow_none=True), 
         descriptions=TunableList(description='\n                Description for the UI tooltip, per intensity.\n                ',
  tunable=(TunableLocalizedString())), **kwargs)


class Mood(HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.MOOD)):
    ...
    def _verify_tuning_callback(cls):
        ...
        if cls.descriptions_trait_override.trait is not None:
            if len(cls.descriptions_trait_override.descriptions) != num_thresholds:
