...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\statistics\skill.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 58939 bytes
...
import caches, collections, distributor, enum, gsi_handlers.sim_handlers_log, operator, rewards.reward_tuning, services, sims4.log, sims4.resources, statistics.continuous_statistic_tuning, tag, telemetry_helper, ui.screen_slam
...
class Skill(HasTunableReference, ProgressiveStatisticCallbackMixin, statistics.continuous_statistic_tuning.TunedContinuousStatistic, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.STATISTIC)):
    ...
    INSTANCE_TUNABLES = {'stat_name':TunableLocalizedString(description='\n            The name of this skill.\n            ',
     ...
     'skill_description':TunableLocalizedStringFactory(description="\n            The skill's normal description.\n            ",
       export_modes=ExportModes.All,
       tuning_group=GroupNames.UI), 
     'locked_description':TunableLocalizedString(description="\n            The skill description when it's locked.\n            ",
       allow_none=True,
       export_modes=ExportModes.All,
       tuning_group=GroupNames.UI), 
     ...
     'level_data':TunableMapping(description='\n            Level-specific information, such as notifications to be displayed to\n            level up.\n            ',
      ...
      'secondary_icon':None}),
       ...
       tunable=rewards.reward_tuning.TunableSpecificReward(pack_safe=True)),
    ...
    REMOVE_INSTANCE_TUNABLES = ('min_value_tuning', 'max_value_tuning', 'decay_rate',
    ...
    def _tuning_loaded_callback(cls):
        ...
        cls._default_user_value = cls.convert_to_user_value(cls.get_value())
    ...
    def _verify_tuning_callback(cls):
        ...
        if len(success_multiplier_affordances) != len(set(success_multiplier_affordances)):
            logger.error("The same affordance has been tuned more than once under {}'s success multipliers, and they will overwrite each other. Please fix in tuning.", cls,
              owner='tastle')
        ...
        if len(monetary_payout_multiplier_affordances) != len(set(monetary_payout_multiplier_affordances)):
            logger.error("The same affordance has been tuned more than once under {}'s monetary payout multipliers, and they will overwrite each other. Please fix in tuning.", cls,
              owner='tastle')
    ...
    def can_add(cls, owner, force_add=False, **kwargs):
        if force_add:
            return True
        for skill in owner.all_skills():
            if cls in skill.skill_unlocks_on_max and skill.reached_max_level:
                return True

        if owner.age not in cls.ages:
            return False
        if owner.statistic_component.prevent_skills_from_being_added:
            return False
    ...
    def should_send_update(self, sim_info, stat_value):
        if sim_info.is_npc:
            if not self.update_client_for_npcs:
                return False
        if self.hidden:
            return False
    ...
    def get_user_value(cls, inst):
        if inst is not None:
            if inst._user_value is not None:
                return inst._user_value
            inst._user_value = super(__class__, inst).get_user_value()
            return inst._user_value
        return cls._default_user_value
    ...
    def set_value(self, value, *args, from_load=False, interaction=None, **kwargs):
        ...
        self._user_value = new_level
        if old_value == self.initial_value or old_value != new_value:
            event_manager.process_event((test_events.TestEvent.SkillValueChange), sim_info=sim_info,
              skill=self,
        ...
        if old_level < new_level or old_value == self.initial_value:
            ...
            event_manager.process_event((test_events.TestEvent.SkillLevelChange), sim_info=sim_info,
              skill=self,
              new_level=new_level,
              custom_keys=(
             self.stat_type,))
    ...
    def _callback_handler(stat_inst):
        stat_inst._clear_user_value_cache()
    ...
    def on_skill_level_up(self, old_level, new_level):
        ...
        services.get_event_manager().process_event((test_events.TestEvent.SkillValueChange), sim_info=sim_info,
          statistic=(self.stat_type),
          custom_keys=(
         self.stat_type,))
    ...
    def _show_level_notification(self, skill_level, ignore_npc_check=False):
        sim_info = self._tracker._owner
        if not ignore_npc_check:
            if sim_info.is_npc or skill_level == 1:
            ...
            if level_data is not None:
                ...
                notification.show_dialog(icon_override=IconInfoData(icon_resource=(self.icon)), secondary_icon_override=IconInfoData(obj_instance=sim_info),
                  additional_tokens=(
                 skill_level,),
    ...
    def send_commodity_update_message(cls, sim_info, old_value, new_value):
        ...
        return stat_instance is None or stat_instance.should_send_update(sim_info, new_value) or None
    ...
    def save_statistic(self, commodities, skills, ranked_stats, tracker):
        save_message = self.get_save_message(tracker)
        if save_message is not None:
            skills.append(save_message)
    ...
    def get_skill_provided_affordances(self):
        ...
        if level_data is None:
            return ((), ())
        return (
         level_data.super_affordances, level_data.target_super_affordances)
