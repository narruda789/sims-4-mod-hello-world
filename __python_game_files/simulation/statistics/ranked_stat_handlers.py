...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\statistics\ranked_stat_handlers.py
# Compiled at: 2018-05-18 17:56:35
    ...
    for sim_info in services.sim_info_manager().get_all():
        ...
        for commodity in commodity_tracker:
            if issubclass(type(commodity), RankedStatistic):
                ...
                entry = {'sim':str(sim_info), 
                 'ranked_stat':str(type(commodity)), 
                 'rank':str(commodity.rank_level), 
                 'points_to_next_rank':str(points_to_rank), 
                 'decaying':str(commodity.decay_enabled), 
                 'decay_rate':str(commodity.get_decay_rate())}
        ...
        if cur_sim_info.static_commodity_tracker is not None:
            for statistic in list(cur_sim_info.commodity_tracker):
                ...
                entry = {'simId':str(sim_id), 
                 'skill_guid':str(statistic.guid64), 
                 'skill_name':type(statistic).__name__, 
                 'skill_value':statistic.get_value(), 
                 'event_level':event_level, 
                 'rank_level':statistic.rank_level, 
                 'next_level':next_level - value, 
                 'next_rank':next_rank - value, 
                 'time_till_decay_starts':str(time_till_decay_starts)}
