...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\organizations\organization_situations.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 35504 bytes
...
class OrganizationEventBase(SituationComplexCommon):
    ...
    def get_members_sim_infos(cls, blacklist_sims=None):
        ...
        members_sim_ids = services.organization_service().generate_organization_members((cls.organization.guid64), amount=(cls.number_of_members.upper_bound),
...
lock_instance_tunables(OrganizationEventBase, exclusivity=(BouncerExclusivityCategory.FESTIVAL_GOER),
...
class JobAssignedSubSituationState(SubSituationState):
    ...
    def start_situation(self):
        ...
        self._sub_situation_id = services.get_zone_situation_manager().create_situation((self.sub_situation), guest_list=guest_list,
          user_facing=False)
        super().start_situation()
...
class FestivalOrganizationEventType(BaseGenericFestivalSituation):
    ...
    def get_members_sim_infos(cls, job, member_info, blacklist_sims=None):
        ...
        if blacklist_sims is not None:
            ...
            blacklist_sims = hh_sims
        members_sim_ids = services.organization_service().generate_organization_members((member_info.organization.guid64), amount=(member_info.number_of_members.upper_bound),
          blacklist_sims=blacklist_sims,
          additional_filter_terms=(member_info.additional_filters),
          minimum=(member_info.number_of_members.lower_bound))
        if not members_sim_ids:
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, GatherEventSituationState, factory=(cls.gather_state)),
    ...
    def get_predefined_guest_list(cls):
        ...
        grabbed_non_members = []
        for job, role_info in cls.non_member_job_and_role_states.jobs_and_roles.items():
            non_member_ids = filter_service.submit_matching_filter(sim_filter=(job.filter), number_of_sims_to_find=(role_info.number_of_sims_to_find),
              allow_yielding=False,
              blacklist_sim_ids={sim_info.sim_id for sim_info in services.active_household()},
              gsi_source_fn=(lambda : 'OrganizationSituations: Add non member sims to situation guest list based on {}'.format(str(job))))
...
class MajorOrganizationEvent(FestivalOrganizationEventType):
    INSTANCE_TUNABLES = {'activity_state':ActivityEventSituationState.TunableFactory(description='\n            The second state that this situation will be put into once the\n            gather state ends.\n            ',
       ...
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       display_name='2. Activity State'), 
     'end_state':EndEventSituationState.TunableFactory(description='\n            The third state that this situation will be put into once the activity\n            state ends.\n            ',
       locked_args={'allow_join_situation':False, 
      'time_out':None},
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       display_name='3. End State')}

    @classmethod
    def _states(cls):
        return (SituationStateData(1, GatherEventSituationState, factory=(cls.gather_state)),
         SituationStateData(2, ActivityEventSituationState, factory=(cls.activity_state)),
         SituationStateData(3, EndEventSituationState, factory=(cls.end_state)),
         SituationStateData(4, CooldownFestivalSituationState, factory=(cls.cooldown_state)))
...
class CreativityCelebrationEvent(FestivalOrganizationEventType):
    INSTANCE_TUNABLES = {'art_phase':ArtPhaseSituationState.TunableFactory(description='\n            The second state that this situation will be put into once the\n            gather state ends.\n            ',
       locked_args={'allow_join_situation': True},
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       display_name='2. Art Phase'), 
     'party_phase':PartyPhaseSituationState.TunableFactory(description='\n            The third state that this situation will be put into once the first\n            activity state ends.\n            ',
       locked_args={'allow_join_situation': False},
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       display_name='3. Party Phase'), 
     'end_state':EndEventSituationState.TunableFactory(description='\n            The fourth state this situation will be put into once the activity\n            states end.\n            ',
       locked_args={'allow_join_situation':False, 
      'time_out':None},
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       display_name='4. End State')}

    @classmethod
    def _states(cls):
        return (SituationStateData(1, GatherEventSituationState, factory=(cls.gather_state)),
         SituationStateData(2, ArtPhaseSituationState, factory=(cls.art_phase)),
         SituationStateData(3, PartyPhaseSituationState, factory=(cls.party_phase)),
         SituationStateData(4, EndEventSituationState, factory=(cls.end_state)),
         SituationStateData(5, CooldownFestivalSituationState, factory=(cls.cooldown_state)))
...
class MembershipSmartMajorOrganizationEvent(FestivalOrganizationEventType):
    ...
    def _states(cls):
        return (SituationStateData(1, GatherEventSituationState, factory=(cls.gather_state)),
         SituationStateData(2, ActivityLoopingSmartSituationState, factory=(cls.activity_state)),
         SituationStateData(3, EndEventSituationState, factory=(cls.end_state)),
         SituationStateData(4, CooldownFestivalSituationState, factory=(cls.cooldown_state)))