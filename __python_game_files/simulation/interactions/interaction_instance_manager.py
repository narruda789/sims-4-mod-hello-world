...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\interaction_instance_manager.py
# Compiled at: 2021-04-01 21:20:42
...
class InteractionInstanceManager(InstanceManager):
    ...
    def _use_animation_constraint_cache(self):
        ...
        for cls in self.types.values():
            if not cls._animation_constraint_dirty:
...
def get_animation_constraint_cache_debug_information():
    ...
    return [('BUILD_AC_CACHE', str(BUILD_AC_CACHE), 'Whether we are currently building AC Cache'),
     (
      'AC_CACHE SIZE', acc_size, 'dict size of _ac_cache')]