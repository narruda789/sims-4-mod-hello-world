...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\utils\destruction.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 13417 bytes
...
class ObjectDestructionElement(XevtTriggeredElement):
    FACTORY_TUNABLES = {'objects_to_destroy':TunableObjectGeneratorVariant(description='\n            The objects to destroy.\n            ',
     ...
     'tag_restriction':OptionalTunable(description='\n            If enabled, only objects that have the tag inside this tuned set\n            will get destroyed. Disable means all objects found as the\n            participant will get destroyed.\n            ',
      ...
      Tag.INVALID,),
       pack_safe=True)),
       disabled_name='no_restriction',
       enabled_name='add_restriction'), 
    ...
    def _get_objects_to_destroy_gen(cls, interaction, target=DEFAULT, context=DEFAULT, **interaction_parameters):
        ...
        for obj in objects:
            ...
            if cls.tag_restriction is not None:
                if not (obj.definition.has_build_buy_tag)(*cls.tag_restriction):
                    continue
    ...
    def on_affordance_loaded_callback(cls, affordance, object_destruction_element, object_tuning_id=DEFAULT):
        ...
        def get_simoleon_delta(interaction, target=DEFAULT, context=DEFAULT, **interaction_parameters):
            ...
            objs = tuple((ObjectDestructionElement._get_objects_to_destroy_gen)(object_destruction_element,
 interaction, target=target, context=context, **interaction_parameters))
    ...
    def _destroy_objects(self):
        ...
        for object_to_destroy in self._destroyed_objects:
            ...
            if not self.interaction.is_saved_participant(obj):
                if obj.parts:
                    if any((self.interaction.is_saved_participant(obj_part) for obj_part in obj.parts)):
