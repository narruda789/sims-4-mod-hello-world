...
# Size of source mod 2**32: 72629 bytes
...
class AddTraitLootOp(BaseLootOperation):
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        with telemetry_helper.begin_hook(loot_op_telemetry_writer, TELEMETRY_HOOK_TRAIT_ADDED, sim_info=subject) as (hook):
            hook.write_guid(TELEMETRY_TRAIT_ID, self._trait.guid64)
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        ...
        for trait_to_remove in traits_to_remove:
            if trait_to_remove is not None:
                with telemetry_helper.begin_hook(loot_op_telemetry_writer, TELEMETRY_HOOK_TRAIT_REMOVED,
                  sim_info=subject) as (hook):
                    hook.write_guid(TELEMETRY_TRAIT_ID, trait_to_remove.guid64)
