...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\utils\reserve.py
# Compiled at: 2020-09-16 14:00:06
...
class TunableReserveTypeVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        ...
        (super().__init__)(args, basic=ReservationHandlerBasic.TunableFactory(), 
         all=ReservationHandlerAllParts.TunableFactory(), 
         multi=ReservationHandlerMulti.TunableFactory(), 
         reserve_and_lock_all_parts=ReservationHandlerAllParts.TunableFactory(), 
         multi_reserve_all_unmovable_parts=ReservationHandlerUnmovableObjects.TunableFactory(), 
         interlocked=ReservationHandlerInterlocked.TunableFactory(), **kwargs)
...
class TunableReserveObject(TunableFactory, is_fragment=True):
    ...
    @staticmethod
    def _factory(sim, interaction, *, reserve_type_for_provided_target, subject_list, reserve_target=None, resolver=None, **kwargs):
        if reserve_target is not None:
            ...
            def _process_participants(reserve_type, subject):
                ...
                if resolver is None:
                    logger.error('Attempting to get reservation handle with no interaction or resolver')
                    return
                for obj in resolver.get_participants(subject):
                    if not obj is None:
                        if obj.is_sim:
                            continue
    ...
    def _on_tunable_loaded_callback(cls, fields, source, *, reserve_type_for_provided_target, subject_list, **kwargs):
        if reserve_type_for_provided_target is not None:
            if issubclass(ReservationHandlerMulti, reserve_type_for_provided_target.factory):
  ...
  default=DEFAULT_RESERVATION_PARTICIPANT_TYPES))), 
         verify_tunable_callback=TunableReserveObject.verify_tunable_callback, 
