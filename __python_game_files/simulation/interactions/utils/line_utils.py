...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\utils\line_utils.py
# Compiled at: 2020-09-25 14:11:51
...
class LineUtils:
    ...
    WAIT_IN_LINE_TOGETHER_SITUATION = TunableReference(description='\n        The situation that will be initiated when a sim joins a line if there\n        are any sims that should wait nearby.\n        ',
      ...
      class_restrictions=('WaitInLineTogether', ))
...
def start_wait_in_line_together_situation(initiating_sim, picked_sims, stored_interaction_data):
    ...
    situation_id = situation_manager.create_situation((LineUtils.WAIT_IN_LINE_TOGETHER_SITUATION), guest_list=guest_list,
      user_facing=False,
      creation_source=creation_source,
      target_id=(target.id if target is not None else None),
      line_key=(stored_interaction_data[2]))
    return situation_id
...
class WaitingLineInteractionChainLiability(Liability, HasTunableFactory):
    ...
    def release(self):
        ...
        if self._interaction.affordance in self._wait_for_continuations:
    ...
    def transfer(self, interaction):
        ...
        if interaction.target is not current_target:
            if current_target.has_component(WAITING_LINE_COMPONENT):
