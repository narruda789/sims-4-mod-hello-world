...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\object_retrieval_liability.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 11950 bytes
import build_buy, enum, services, sims4
...
class _SendToInventoryFallback(_ObjectRetrievalFallbackBase):
    ...
    def run_fallback(self, actor, obj):
        if actor.inventory_component.can_add(obj):
            if actor.inventory_component.player_try_add_object(obj):
                return
...
class ObjectRetrievalLiability(HasTunableFactory, AutoFactoryInit, PreparationLiability):
    ...
    FACTORY_TUNABLES = {'put_down_tuning':OptionalTunable(description='\n            If enabled, the liability will push this tuned affordance to place\n            the object before running the owning interaction. \n            ',
     ...
     'object_to_retrieve':TunableEnumEntry(description='\n            The target of the retrieval affordance.\n            ',
       tunable_type=ParticipantTypeSingle,
       default=ParticipantTypeSingle.Object), 
     'retrieval_affordance':TunableReference(description="\n            The affordance to push after the liability is released to pick up\n            the 'object to retrieve'.\n            ",
       manager=services.get_instance_manager(sims4.resources.Types.INTERACTION)), 
     'fallback_behavior':TunableVariant(description='\n            The fallback behavior to use on the retrieval object if the \n            retrieval affordance fails.\n            ',
       send_to_inventory=_SendToInventoryFallback.TunableFactory(),
       destroy_object=_DestroyObjectFallback.TunableFactory(),
       default='send_to_inventory')}
    ...
    def _push_affordance(self, affordance, target):
        ...
        context = InteractionContext((self._actor), (InteractionContext.SOURCE_SCRIPT),
          (Priority.High),
    ...
    def _clean_up_object(self):
        ...
        for child_obj in tuple(self._object.children):
            if child_obj.is_sim:
                ...
                reset_fn = lambda _: child_obj.reset((ResetReason.RESET_EXPECTED), source=self,
                  cause=('ObjectRetrievalLiability is cleaning up parent object {}.'.format(self._object)))
                element = build_element([reset_fn,
                 lambda _: self._clean_up_object()],
