...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\social\social_picker_super_interaction.py
# Compiled at: 2018-08-28 18:45:53
...
import autonomy, event_testing.results, interactions.base.super_interaction, services, sims4.log, sims4.resources, sims4.tuning
...
class SocialPickerSuperInteraction(SocialCompatibilityMixin, interactions.base.super_interaction.SuperInteraction):
    ...
    def _test(cls, target, context, **kwargs):
        ...
        if target_sim is None:
            ...
            return context.sim.queue.can_queue_visible_interaction() or event_testing.results.TestResult(False, 'Interaction queue is full.')
    ...
    def _run_interaction_gen(self, timeline):
        ...
        logger.assert_log((target_sim is not None), 'target_sim is invalid in SocialPickerSuperInteraction._run_interaction_gen()',
          owner='rez')
        self.force_inertial = True
        context = self.context.clone_for_sim((self.sim), bucket=(InteractionBucketType.BASED_ON_SOURCE))
        autonomy_request = AutonomyRequest((self.sim), autonomy_mode=(autonomy.autonomy_modes.SocialAutonomy),
          static_commodity_list=[
         self.SOCIAL_STATIC_COMMODITY],
          object_list=[
         target_sim],
          context=context,
...
sims4.tuning.instances.lock_instance_tunables(SocialPickerSuperInteraction,
