...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\base\create_object_interaction.py
# Compiled at: 2020-09-09 18:47:10
# Size of source mod 2**32: 11842 bytes
    ...
    def __init__(self, **kwargs):
        super().__init__(definition=TunableReference(description='\n                The definition of the object.\n                ', 
         manager=services.definition_manager(), **kwargs),
          verify_tunable_callback=(ObjectDefinition.verify_tunable_callback))
...
class CreateCarriedObjectMixin:
    ...
    INSTANCE_TUNABLES = {'definition':OptionalTunable(description="\n            The object to create; this can be set at runtime.\n            \n            If 'runtime parameter' is chosen, it will look at the parameter \n            passed in at runtime to determine which object to create.\n            The primary use of the 'runtime parameter' option is if\n            the interaction is pushed from code so consult a GPE before using it.\n            ",
     ...
     'continuation':SuperInteraction.TunableReference(description='\n            An interaction to push as a continuation to the carry.\n            ',
       allow_none=True), 
        ...
        def create_continuation_affordance_with_overrides():
            ...
            for continuation in reversed(self.continuation_with_affordance_overrides.continuation):
                ...
                if self.sim in local_actors:
                    ...
                    if aops_and_context and aops_and_context[0] is not None:
    ...
    def build_basic_content(self, sequence, **kwargs):
        ...
        self._object_create_helper = CreateObjectHelper((self.sim), (self._get_chosen_definition()), self, init=setup_object,
        ...
        def enter_carry(timeline):
            result = yield from element_utils.run_child(timeline, enter_carry_while_holding(self, (self.created_target), callback=claim_object,
              create_si_fn=(self._get_create_continuation_affordance()),
              track=(self.track),
              sequence=(build_critical_section(super_build_basic_content(sequence, **kwargs), flush_all_animations))))
            return result
            if False:
                yield None

        return (self._object_create_helper.create(set_carry_target, enter_carry),
