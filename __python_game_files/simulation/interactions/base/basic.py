...
# Size of source mod 2**32: 42699 bytes
from animation.animation_xevt_element import AnimationXevtElement, ObjectAnimationXevtElement
from animation.animation_utils import get_tested_animation_override
...
from interactions.utils.interaction_elements import FadeChildrenElement, SetVisibilityStateElement, UpdatePhysique, PutNearElement, AddToHouseholdElement, ReplaceObject, SaveParticipantElement, UpdateDisplayNumber, SetRoutingInfoAndState, ProceduralAnimationRotationElement
...
from interactions.utils.parent_object import ParentObjectElement, ParentObjectWithRoutingFormationSlave
...
from objects.components.canvas_component import UpdateFamilyPortrait, PaintingStateTransfer, UpdateObjectValue, CreatePhotoboothPortrait
...
from objects.components.state import TunableStateChange, TunableTransienceChange, SetStaggeredStateElement
...
from objects.gardening.gardening_component_crop_plant_element import HarvestCropsElement
from objects.household_inventory_management import SendToInventory
from objects.parenting_utils import SetAsHeadElement
from objects.set_location_element import SetLocationElement
...
from reservation.unreserve_object_element import UnreserveObjectElement
from retail.retail_elements import RetailCustomerAction
from routing.formation.formation_element import RoutingFormationElement, ReleaseRoutingFormationElement
...
from travel_group.travel_group_elements import TravelGroupAdd, TravelGroupRemove, TravelGroupExtend, TravelGroupEnd
from trends.trend_recording import RecordTrendsElement
from tutorials.tutorial import TutorialOpenElement
from world.dynamic_spawn_point import DynamicSpawnPointElement
import careers.career_tuning, interactions, services, sims4.log
...
class _BasicContent(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def __call__(self, interaction, sequence=(), **kwargs):
        if self.route_to_location is not None:
            ...
            if actor_or_object_target:
                if last_affordance_is_affordance:
                    if not interaction.is_super:
                        if self.sleeping == True and animation_ref is not None:
                            skip_animation = animation_sequence.repeat or interaction.sim.asm_auto_exit.asm is not None
            skip_animation = False
        if animation_ref is not None:
            sequence = skip_animation or animation_sequence
        else:
            if self.periodic_stat_change is not None:
                sequence = self.periodic_stat_change(interaction, sequence=sequence)
            elif self.progressive_stat_change is not None:
                ...
                if self.sleeping and animation_ref is not None:
                    sequence = animation_sequence.repeat or with_skippable_animation_time((interaction.sim,), sequence=sequence)
            return sequence
    ...
    def validate_tuning(self, interaction):
        if not self.start_autonomous_inertial:
            for conditional_action in self.conditional_actions:
                ...
                logger.error("Interaction {} has looping content that has no way to end. Autonomously, it doesn't start inertial, and it has no conditional action for exiting or going inertial.", interaction,
                  owner='tingyul')
        ...
        if not self.start_user_directed_inertial:
            for conditional_action in self.conditional_actions:
                ...
                logger.error("Interaction {} has looping content that has no way to end. User directed, it doesn't start inertial, and it has no conditional action for exiting or going inertial.", interaction,
                  owner='tingyul')
...
class _StagingContentBase(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    FACTORY_TUNABLES = {'content_set':lambda : ContentSetWithOverrides.TunableFactory(), 
...
class StagingContent(_StagingContentBase, _FlexibleLengthContent):
    EMPTY = _StagingContentBase(animation_ref=None,
      content_set=(ContentSet.EMPTY_LINKS),
      push_affordance_on_run=None,
      post_stage_autonomy_commodities=(),
      only_use_mixers_from_SI=False)
...
class FlexibleLengthContent(_FlexibleLengthContent):
    ...
    @TunableFactory.factory_option
    def animation_callback(callback=DEFAULT):
        return {'content': TunableVariant(staging_content=_StagingContentBase.TunableFactory(animation_callback=callback),
                      looping_content=_LoopingContentBase.TunableFactory(animation_callback=callback),
                      default='staging_content')}

    CONTENT_OVERRIDES = ('animation_ref', 'push_affordance_on_run', 'post_stage_autonomy_commodities',
                         'only_use_mixers_from_SI', 'staging', 'sleeping', 'content_set')
...
class TunableBasicContentSet(TunableVariant):
    ...
    def __init__(self, default=None, no_content=False, one_shot=False, looping_animation=False, flexible_length=False, animation_callback=DEFAULT, description=None, **kwargs):
        ...
        if looping_animation is True:
            options['looping_animation'] = LoopingContent.TunableFactory(animation_callback=(
             animation_callback,),
              locked_args={'start_autonomous_inertial':False, 
             'start_user_directed_inertial':False},
...
class BasicExtraVariantCore(TunableVariant):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(add_to_ensemble=AddToEnsemble.TunableFactory(), 
         ...
         animation=AnimationXevtElement.TunableFactory(), 
         ...
         create_photobooth_portrait=CreatePhotoboothPortrait.TunableFactory(), 
         ...
         harvest_crops=HarvestCropsElement.TunableFactory(), 
         ...
         object_animation=ObjectAnimationXevtElement.TunableFactory(), 
         ...
         parent_object_with_routing_slave=ParentObjectWithRoutingFormationSlave.TunableFactory(), 
         ...
         procedural_animation_rotation=ProceduralAnimationRotationElement.TunableFactory(), 
         ...
         release_routing_formation=ReleaseRoutingFormationElement.TunableFactory(), 
         ...
         set_routing_info_and_state=SetRoutingInfoAndState.TunableFactory(), 
         set_staggered_state=SetStaggeredStateElement.TunableFactory(), 
         ...
         special_case_moac_set_location=SetLocationElement.TunableFactory(), 
         ...
         tutorial=TutorialOpenElement.TunableFactory(), 
         university_enrollment_ui=UniversityEnrollmentElement.TunableFactory(), 
         unreserve_object_early=UnreserveObjectElement.TunableFactory(), 
         ...
         vfx=PlayVisualEffectElement.TunableFactory(), **kwargs)


class BasicExtraVariant(BasicExtraVariantCore):

    def __init__(self, **kwargs):
        (super().__init__)(footprint_toggle=TunableFootprintToggleElement(), 
         ...
         push_leave_lot_interaction=PushNpcLeaveLotNowInteraction.TunableFactory(), **kwargs)
