...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\routing_handlers.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 31473 bytes
import traceback
from gsi_handlers.gameplay_archiver import GameplayArchiver
from gsi_handlers.route_event_handlers import get_path_route_events_log
from objects import ALL_HIDDEN_REASONS
from sims4.gsi.schema import GsiGridSchema, GsiFieldVisualizers
import objects.system, routing, services
...
def surface_string(surface_id):
    ...
    if surface_id.type == routing.SurfaceType.SURFACETYPE_WORLD:
        ...
        if surface_id.type == routing.SurfaceType.SURFACETYPE_OBJECT:
            out_str = out_str + 'OBJECT:' + str(surface_id.primary_id)
        else:
            if surface_id.type == routing.SurfaceType.SURFACETYPE_POOL:
...
def archive_plan(planner, path, ticks, time):
    ...
    entry = {'planner_name':str(planner),  'planner_id':str(hex(planner.id)), 
     'result':result, 
     'x':round(path.route.origin.position.x, 4), 
     'y':round(path.route.origin.position.y, 4), 
     'z':round(path.route.origin.position.z, 4), 
     'qx':round(path.route.origin.orientation.x, 4), 
     'qy':round(path.route.origin.orientation.y, 4), 
     'qz':round(path.route.origin.orientation.z, 4), 
     'qw':round(path.route.origin.orientation.w, 4), 
     'level':surface_string(path.route.origin.routing_surface), 
     'ticks':ticks, 
     'time':round(time * 1000, 4), 
     'plan_time':round(plan_time, 4), 
     'dist':round(path.nodes.length, 4), 
     'num_goals':len(path.route.goals), 
     'num_starts':len(path.route.origins)}
    ...
    for goal, result in zip(path.route.goals, path.nodes.goal_results()):
        ...
        if cost >= 1000000.0:
            cost = 999999
        goals.append({'index':index,  'x':round(goal.location.position.x, 4), 
         'z':round(goal.location.position.z, 4), 
         'level':surface_string(goal.location.routing_surface), 
         'cost':round(goal.cost, 4), 
         'valid':goal.failure_reason.name, 
         'final_cost':cost, 
         'result':result_str, 
         'raw_result':result[1], 
         'group':goal.group})
    ...
    for start in path.route.origins:
        result = 'Not Chosen'
        starts.append({'x':round(start.location.position.x, 4),  'z':round(start.location.position.z, 4), 
         'level':surface_string(start.location.routing_surface), 
         'cost':round(start.cost, 4), 
         'result':result})
    ...
    while cur_path is not None:
        for node in cur_path.nodes:
            nodes.append({'x':node.position[0],  'z':node.position[2], 
             'level':surface_string(node.routing_surface_id), 
             'portal':str(node.portal_id) + '/' + str(node.portal_object_id), 
             'qx':node.orientation[0], 
             'qy':node.orientation[1], 
             'qz':node.orientation[2], 
             'qw':node.orientation[3]})
...
def archive_build(build_id):
    ...
    if build_record is not None:
        entry = {'build_id':build_record['id'], 
         'total_time_ms':build_record['total_time_ms']}
    details = []
    if build_record is not None:
...
def add_route_archive_fields(schema):
    ...
    with schema.add_has_many('Portals', GsiGridSchema) as (sub_schema):
...
def _archive_route_node_portals(entry, node, portal_object):
    if portal_object is None:
        return
    portal_id = node.portal_id
    portal = portal_object.get_portal_by_id(portal_id)
    if portal is None:
        return
    portal_entry, portal_exit = portal.get_portal_locations(portal_id)
    entry.append({'portal_obj_id':str(node.portal_object_id),  'portal_obj':str(portal_object), 
     'portal_id':node.portal_id, 
     'portal_type':str(portal_object.get_portal_type(portal_id)), 
     'portal_entry_surface':surface_string(portal_entry.routing_surface if portal_entry is not None else None), 
     'portal_entry_orientation':str(portal_entry.orientation if portal_entry is not None else None), 
     'portal_entry_position':str(portal_entry.position if portal_entry is not None else None), 
     'portal_exit_surface':surface_string(portal_exit.routing_surface if portal_exit is not None else None), 
     'portal_exit_orientation':str(portal_exit.orientation if portal_exit is not None else None), 
     'portal_exit_position':str(portal_exit.position if portal_exit is not None else None)})
...
def archive_node_data(entry, path):
    ...
    for index in range(len(path_nodes) - 1, 0, -1):
        ...
        _archive_route_node_portals(portals_entry, cur_node, portal_object)
        nodes_entry.append({'segment_duration':cur_node.time - prev_node.time,  'x':cur_node.position[0], 
         'y':cur_node.position[1], 
         'z':cur_node.position[2], 
         'routing_surface':surface_string(cur_node.routing_surface_id), 
         'portal_id':cur_node.portal_id, 
         'portal_object':str(portal_object), 
         'portal_object_id':str(cur_node.portal_object_id)})
...
def archive_object_route(object_info, path, stall_for_slaves_length):
    ...
    entry = {'object_name':str(object_info), 
     'path_id':id(path), 
     'duration':path.duration() if path is not None else None, 
     'length':path.length() if path is not None else None, 
     'stall_for_slaves_length':stall_for_slaves_length.in_real_world_seconds()}
...
def archive_sim_route(sim_info, interaction, path, stall_for_slaves_length):
    ...
    for event in route_events.values():
        route_events_entry.append({'time':event['time'],  'event_type':event['event_type'], 
         'tuning_instance':event['event_cls'], 
         'duration':event['duration'], 
         'executed':event['executed'] if 'executed' in event else 'Unknown'})
...
def archive_goals(handle, goal_list, discarded_goals, **kwargs):
    entry = {'count':len(goal_list), 
     'constraint':str(handle.constraint), 
     'geometry':str(handle.geometry), 
     'args':str(kwargs)}
    goals = []
    entry['Goals'] = goals
    for goal in goal_list:
        position = goal.location.world_transform.translation
        goals.append({'x':position.x,  'y':position.y, 
         'z':position.z, 
         'routing_surface':surface_string(goal.location.routing_surface), 
         'cost':goal.cost, 
         'height_clearance':goal.height_clearance, 
         'tag':goal.tag, 
         'requires_los_check':'X' if goal.requires_los_check else '', 
    ...
    for discarded_goal in discarded_goals:
        ...
        discarded_goals_entry.append({'x':x_str,  'y':y_str, 
         'z':z_str, 
         'routing_surface':routing_surface_str, 
         'cost':cost_str, 
         'height_clearance':discarded_goal.height_clearance, 
         'error':discarded_goal.failure.name, 
         'info':discarded_goal.info})
    ...
    callstack.reverse()
    entry['Callstack'] = callstack
    if not handle.sim is None:
        if not handle.sim.is_sim:
