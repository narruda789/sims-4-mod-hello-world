...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\zone_modifier_handlers.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 6968 bytes
...
def generate_zone_modifiers_view_data(zone_id: int=None, filter=None):
    ...
    if zone_modifier_service is None or venue_service is None:
    ...
    for zone_modifier in zone_modifiers:
        ...
        if filter_list is not None:
            if FILTER_SHOW_ACTIVE_ZONE_MODIFIERS in filter_list:
                if not is_active_zone_modifier:
                    continue
        zone_modifier_list.append({'id':zone_modifier.guid64, 
         'name':zone_modifier.__name__, 
         'assigned_to_zone':is_active_zone_modifier, 
         'enter_loots':len(zone_modifier.enter_lot_loot), 
         'exit_loots':len(zone_modifier.exit_lot_loot), 
         'scheduled_entries':len(zone_modifier.schedule(init_only=True).get_schedule_entries()), 
         'event_tests':len(registered_event_map[zone_modifier]), 
         'hidden':zone_modifier in current_venue_zones_modifiers, 
         'Event Tests':registered_event_map[zone_modifier], 
         'Schedule Event Time':event_times_map[zone_modifier], 
         'Architectural Objects Effects':objects_effects[zone_modifier]})
...
def _get_registered_events(zone_modifiers):
    ...
    for (event_enum, custom_key), handlers in event_mgr._test_event_callback_map.items():
        ...
        for handler in registered_handlers:
            events_handlers_map[handler].append({'event_name':str(event_enum), 
             'custom_key':str(custom_key)})
...
def _get_next_event_times(zone_modifiers):
    ...
    for zone_modifier in zone_modifiers:
        ...
        for start, end in weekly_schedule.get_schedule_entries():
            scheduled_entries.append({'start_time':'{0:D} {0:h}:{0:m}'.format(start), 
             'end_time':'{0:D} {0:h}:{0:m}'.format(end)})
        ...
        if zone_modifier.obj_tag_id_to_count:
            for tag_id, count in zone_modifier.obj_tag_id_to_count.items():
                ...
                for index, action in enumerate(zone_modifier.object_tag_to_actions[tag]):
                    entries.append({'tag':str(tag) if index == 0 else '', 
                     'count':count if index == 0 else '', 
                     'statistic':action.stat.__name__ if action.stat is not None else 'None', 
                     'multiplier':action.multiplier, 
                     'value':action.multiplier * count})
