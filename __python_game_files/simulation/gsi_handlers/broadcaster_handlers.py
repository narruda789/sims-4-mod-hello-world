...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\broadcaster_handlers.py
# Compiled at: 2017-04-20 20:35:17
...
def generate_broadcaster_data():
                ...
                if broadcaster_service_real_time is not None:
                    for broadcaster in itertools.chain(broadcaster_service.get_broadcasters_debug_gen(), broadcaster_service_real_time.get_broadcasters_debug_gen()):
                        ...
                        for linked_broadcaster in linked_broadcasters:
                            yield (
                             'Linked/{}'.format(broadcaster.broadcaster_id), linked_broadcaster)
    ...
    for status, broadcaster in _get_broadcasters_gen():
        ...
        entry = {'broadcaster_id':str(broadcaster.broadcaster_id), 
         'broadcaster_type':str(type(broadcaster).__name__), 
         'broadcasting_object':str(broadcaster.broadcasting_object), 
         'broadcasting_object_id':broadcaster.broadcasting_object.id if broadcaster.broadcasting_object is not None else 0, 
         'broadcaster_status':status, 
         'broadcaster_clock_type':str(broadcaster.clock_type), 
         'broadcaster_targets':', '.join(allowed_targets)}
        affected_object_info = []
        entry['affected_objects'] = affected_object_info
        for obj, data in broadcaster._affected_objects.items():
            affect_object_entry = {'object_name':str(obj),  'last_reaction_time':str(data[0]), 
             'in_area':str(data[1])}
