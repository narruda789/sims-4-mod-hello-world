...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\route_event_handlers.py
# Compiled at: 2018-05-24 16:50:05
...
def gsi_fill_route_event_data(route_event, path_log, additional_data=None):
    event_dict = {'time':route_event.time, 
    ...
    elif route_event.id in path_log.route_events:
...
def archive_route_events(path, sim, archive_type, clear=False):
    ...
    if archive_type == PATH_TYPE_UPDATE:
        if not path_log.route_events.values():
            return
    else:
        return sim.is_sim or None
    archive_data = {'duration':path.duration(),  'path_id':id(path), 
     'master_sim':str(path.sim)}
