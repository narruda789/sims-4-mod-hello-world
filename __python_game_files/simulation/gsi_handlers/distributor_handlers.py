...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\distributor_handlers.py
# Compiled at: 2018-04-23 14:39:06
# Size of source mod 2**32: 5696 bytes
import operator, protocolbuffers.Consts_pb2
...
def archive_operation(target_id, target_name, manager_id, message, payload_type, index, client):
    if message.type in EXCLUDE_OP_TYPES:
        ...
        entry = {'target_name':target_name,  'index':index, 
         'size':len(message.data), 
         'type':message_type, 
         'manager':manager_type, 
         'mask':mask_override, 
         'blockers':',\n'.join(blocker_entries), 
         'tags':',\n'.join(tag_entries), 
         'details':''}
        if LOG_OP_DETAILS:
            if not message.data:
