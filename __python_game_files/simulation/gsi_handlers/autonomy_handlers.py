...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\autonomy_handlers.py
...
def archive_autonomy_data(sim, result, mode_name, gsi_data):
    ...
    archive_data = {'sim_name':sim.full_name, 
    ...
    if gsi_data is not None:
        ...
        for interaction_result in gsi_data[GSIDataKeys.AFFORDANCE_KEY]:
            ...
            if interaction_result:
                ...
                entry = {'affordance':interaction_affordance.__name__,  'target':str(interaction.target), 
                 'autonomy_stage':AutonomyStageLabel.AFTER_SCORING, 
                 'affordance_score':float(interaction_result.score), 
                 'multitask_percentage':float(interaction_result.multitask_percentage), 
                 'scoring_priority':str(interaction_affordance.scoring_priority)[str(interaction_affordance.scoring_priority).index('.') + 1:] if hasattr(interaction_affordance, 'scoring_priority') else None, 
                 'affordance_relevant_desires':', '.join([desire.__name__ for desire in interaction_result.relevant_desires]), 
                 'affordance_status':'Scored' if interaction_affordance.use_best_scoring_aop else 'Scored, but highest scoring aop of same type will be used for probability', 
                 'affordance_details':interaction_result.score.details}
            else:
                entry = {'affordance':interaction.affordance.__name__, 
                 'target':str(interaction.target), 
                 'autonomy_stage':interaction_result.stage, 
                 'affordance_score':None, 
                 'multitask_percentage':None, 
                 'scoring_priority':None, 
                 'affordance_relevant_desires':', '.join([desire.__name__ for desire in interaction_result.relevant_desires]), 
                 'affordance_status':interaction_result.reason, 
                 'affordance_details':''}
        ...
        if gsi_data[GSIDataKeys.MIXER_PROVIDER_KEY]:
            for _, gsi_mixer_provider_data in gsi_data[GSIDataKeys.MIXER_PROVIDER_KEY].items():
                entry = {'buff_or_affordance':gsi_mixer_provider_data._name, 
                 'target':gsi_mixer_provider_data._target_name, 
                 'mixer_provider_score':gsi_mixer_provider_data._score, 
                 'mixers_results':str(gsi_mixer_provider_data.mixer_interaction_group_scoring_detail), 
                 'mixer_provider_details':gsi_mixer_provider_data._score_details}
...
def _create_si_state_entry(interaction, bucket_name):
    ...
    entry = {'interactionId':interaction.id,  'interactionName':interaction_name, 
     'target':str(interaction.target), 
...
def generate_autonomy_queue_view_data(sim_id: int=None):
    ...
    if autonomy_service._active_sim is not None:
        entry = {'position':'0', 
         'sim':str(autonomy_service._active_sim)}
