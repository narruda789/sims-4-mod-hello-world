...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\household_handlers.py
# Compiled at: 2021-06-09 12:33:33
# Size of source mod 2**32: 13141 bytes
...
def generate_household_data(*args, **kwargs):
    ...
    for household in household_manager._objects.values():
        ...
        entry = {'name':str(household.name), 
         'hidden':str(household.hidden), 
         'player':str(household.is_player_household), 
         'played':str(household.is_played_household), 
         'num_sims':str(len(household)), 
         'zone_name':zone_name, 
         'region':region_name, 
         'street':street_name, 
         'funds':str(household._funds.money), 
         'net_worth':str(household.household_net_worth()), 
         'region_id':str(region_id), 
         'home_zone_id':str(household.home_zone_id), 
         'household_id':str(household.id), 
         'premade_household_id':str(household.premade_household_id) if household.premade_household_id > 0 else 'None', 
         'move_in_time':str(household.home_zone_move_in_time)}
        sim_info_data = []
        entry['sim_infos'] = sim_info_data
        for sim_info in household.sim_info_gen():
            sim_info_entry = {'sim_id':str(sim_info.id), 
             'sim_name':sim_info.full_name, 
             'species':str(sim_info.species), 
             'gender':str(sim_info.gender), 
             'age':str(sim_info.age), 
             'is_ghost':str(sim_info.is_ghost), 
             'creation_source':format_enum_name(sim_info.creation_source), 
             'is_instanced':str(sim_info.is_instanced()), 
             'situations':sim_info.debug_get_current_situations_string()}
        ...
        if household._service_npc_record is not None:
            ...
            for service_type, rec in household._service_npc_record.items():
                ...
                e = {'guid':stype.__name__ if stype is not None else str(service_type),  'hired':rec.hired, 
                 'recurring':rec.recurring, 
                 'last_started':str(rec.time_last_started_service), 
                 'last_finished':str(rec.time_last_finished_service), 
                 'preferred_sims':', '.join((str(sim_mgr.get(i)) for i in rec._preferred_service_sim_ids)), 
                 'fired_sims':', '.join((str(sim_mgr.get(i)) for i in rec._fired_service_sim_ids)), 
                 'hiring_sim':str(sim_mgr.get(rec.hiring_sim_id)), 
                 'user_data':rec.user_specified_data_id, 
                 'user_data_selection_and_count':user_data_selection_str}
        ...
        if household.bills_manager is not None:
            ...
            if current_payment_owed is not None:
                e = {'source':'Lot', 
                 'amount':current_payment_owed - detail_amount}
                bills.append(e)
            ...
            for utility in Utilities:
                ...
                e = {'utility':utility.name, 
                 'consumed':details.net_consumption, 
                 'value':details.billable_amount, 
                 'due':due_str, 
                 'keep_excess':keep_excess_str, 
                 'active':active_str, 
                 'statistic':str(utility_info.statistic)}
                utilities.append(e)
