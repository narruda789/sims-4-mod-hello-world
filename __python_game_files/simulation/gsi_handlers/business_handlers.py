...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\business_handlers.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 9690 bytes
...
def generate_business_service_data(zone_id: int=None):
    ...
    if services.venue_service() is None:
    ...
    def _construct_business_manager_gsi_data(zone_id, business_manager, business_tracker=None):
        ...
        business_manager_entry = {'household_id':str(household.id) if household is not None else 'N/A',  'household_name':household.name if household is not None and household.name else '<Unnamed Household>', 
         'business_type':str(BusinessType(business_manager.business_type)), 
         'zone_id':str(hex(zone_id)), 
         'is_open':'x' if business_manager.is_open else '', 
         'time_since_open':str(business_manager.minutes_open), 
         'star_rating_value':business_manager._star_rating_value, 
         'star_rating':business_manager.get_star_rating(), 
         'funds':str(business_manager.funds.money), 
         'daily_revenue':business_manager._daily_revenue}
        other_data = []
        ...
        for sim_id, employee_data in employee_manager._employees.items():
            clock_in_time, payroll_data = employee_manager._employee_payroll.get(sim_id, (None,
                                                                                          None))
            sim_info = sim_info_manager.get(sim_id)
            entry = {'employee_sim_id':str(sim_id),  'employee_sim_name':str(sim_info), 
             'employee_type':str(BusinessEmployeeType(employee_data.employee_type)), 
             'daily_employee_wages':employee_manager._daily_employee_wages, 
             'clocked_in_time':str(clock_in_time), 
             'payroll_data':str(payroll_data)}
        ...
        for sim_id, business_customer_data in business_manager._customer_manager._customers.items():
            entry = {'sim_id':str(sim_id),  'sim_name':str(sim_info_manager.get(sim_id)), 
             'star_rating_value':business_customer_data.get_star_rating_stat_value(), 
             'star_rating':business_customer_data.get_star_rating(), 
             'buff_bucket_totals':str(business_customer_data.buff_bucket_totals)}
    ...
    if zone_business_manager is not None:
        if zone_business_manager.is_owned_by_npc:
...
def archive_business_event(event_from, sim, event_description, sim_id=None):
    entry = {'event_from':event_from, 
     'sim_id':str(sim.id) if sim is not None else str(sim_id), 
     'sim_name':sim.full_name if sim is not None else '', 
