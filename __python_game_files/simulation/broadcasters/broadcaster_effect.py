...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\broadcasters\broadcaster_effect.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 32759 bytes
...
import buffs.tunable, services, sims4.resources

class TunableBroadcasterEffectVariant(TunableVariant):

    def __init__(self, **kwargs):
        (super().__init__)(affordance=BroadcasterEffectAffordance.TunableFactory(), 
         buff=BroadcasterEffectBuff.TunableFactory(), 
         statistic_modifier=BroadcasterEffectStatisticModifier.TunableFactory(), 
         self_state_change=BroadcasterEffectSelfStateChange.TunableFactory(), 
         start_fire=BroadcasterEffectStartFire.TunableFactory(), 
         loot=BroadcasterEffectLoot.TunableFactory(), 
         state=BroadcasterEffectStateChange.TunableFactory(), 
         self_buff=BroadcasterEffectSelfBuff.TunableFactory(), 
         self_loot=BroadcasterEffectSelfLoot.TunableFactory(), **kwargs)
...
class _BroadcasterEffect(AutoFactoryInit, HasTunableSingletonFactory):
    ...
    def get_excluded_participant_type(self):
        pass
...
class _BroadcasterEffectTested(_BroadcasterEffect):
    ...
    def _should_apply_broadcaster_effect(self, broadcaster, affected_object):
        if self.excluded_participants is not None:
            participants = None
            if broadcaster.interaction is not None:
                participants = broadcaster.interaction.get_participants(self.excluded_participants)
            else:
                if broadcaster.excluded_participant_cache is not None:
                    participants = broadcaster.excluded_participant_cache.get(self.excluded_participants)
                elif participants is not None:
                    if affected_object in participants:
                        return False
                    if affected_object.is_sim:
                        if affected_object.sim_info is not None:
                            if affected_object.sim_info in participants:
                                return False
        else:
            resolver = broadcaster.get_resolver(affected_object)
            return self.tests.run_tests(resolver) or False
...
class BroadcasterEffectBuff(_BroadcasterEffectTested):
    ...
    def _apply_broadcaster_effect(self, broadcaster, affected_object):
        if not affected_object.is_sim:
            return
            key = (
             affected_object.id, broadcaster.broadcaster_id)
...
class BroadcasterEffectAffordance(_BroadcasterEffectTestedOneShot):
    ...
    def _get_target_and_context(self, resolver, affected_object):
        ...
        context = InteractionContext(affected_object, (InteractionContext.SOURCE_SCRIPT),
...
class BroadcasterEffectSelfBuff(_BroadcasterEffectTested):
    FACTORY_TUNABLES = {'broadcastee_count_interval':TunableInterval(description='\n            If the number of objects within this broadcaster is in this\n            interval, the buff will be applied. Includes lower and upper.\n            ',
       ...
       minimum=1,
       maximum=20), 
     'buff':buffs.tunable.TunableBuffReference(description='\n            The buff to apply\n            ')}
