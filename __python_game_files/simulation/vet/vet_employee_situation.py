...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\vet\vet_employee_situation.py
# Compiled at: 2017-10-09 15:55:41
# Size of source mod 2**32: 16006 bytes
import functools
...
import services, sims4, situations.bouncer
...
class VetManagedEmployeeSituationState(CommonSituationState):
    FACTORY_TUNABLES = {'transition_out_interaction':OptionalTunable(description='\n             When this interaction is run, this state can be transitioned out of;\n             we will try to advance to another state.  This can be used as a way \n             to switch states before the timeout occurs.\n             ',
     ...
     'state_specific_transitions':TunableMapping(description='\n            Mapping to allow direct transitions to other states using interactions.\n            ',
       key_type=TunableEnumEntry(VetEmployeeSituationStates, default=(VetEmployeeSituationStates.DEFAULT)),
    ...
    def handle_event(self, sim_info, event, resolver):
        if not self.owner.is_sim_info_in_situation(sim_info):
            ...
            return target_sim_info is None or self.owner.is_sim_info_in_situation(target_sim_info) or None
            ...
            if self._transition_out_interaction is not None:
                if resolver(self._transition_out_interaction):
...
class VetEmployeeSituation(BusinessEmployeeSituationMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'_default_state':VetManagedEmployeeSituationState.TunableFactory(description='\n                Default state for the vet employee, which can never be disabled.\n                ',
     ...
     '_managed_states':TunableMapping(description='\n            A mapping of state types to states.\n            ',
       key_type=TunableEnumEntry(VetEmployeeSituationStates, default=(VetEmployeeSituationStates.DEFAULT),
       invalid_enums=(
      VetEmployeeSituationStates.DEFAULT,)),
       value_type=TunableTuple(state=(VetManagedEmployeeSituationState.TunableFactory()),
       enable_disable=OptionalTunable(display_name='Enable/Disable Support',
       tunable=TunableTuple(enable_interaction=TunableInteractionOfInterest(description='\n                            Interaction of interest which will cause this state to be enabled.\n                            '),
    ...
    def _states(cls):
        state_data = []
        state_data.append(SituationStateData((VetEmployeeSituationStates.DEFAULT.value), VetManagedEmployeeSituationState,
          factory=(cls._default_state)))
        for state_type, state_tuning in cls._managed_states.items():
            state_data.append(SituationStateData((state_type.value), VetManagedEmployeeSituationState,
              factory=(functools.partial(state_tuning.state, state_type))))
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        target_sim_info = resolver.get_participant(ParticipantType.TargetSim)
        return target_sim_info is sim_info and self.is_sim_info_in_situation(sim_info) or None
    ...
    def _choose_next_state(self, invalid_states=None):
        ...
        if invalid_states is not None:
            ...
            return available_states or VetEmployeeSituationStates.DEFAULT
    ...
    def _gsi_additional_data_gen(self):
        yield (
         'Locked States', str(self._locked_states))

    def log_flow_entry(self, message):
        log_vet_flow_entry(repr(self.get_employee()), type(self).__name__, message)


lock_instance_tunables(VetEmployeeSituation, duration=0,
  creation_ui_option=(SituationCreationUIOption.NOT_AVAILABLE),
  exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.VENUE_EMPLOYEE))