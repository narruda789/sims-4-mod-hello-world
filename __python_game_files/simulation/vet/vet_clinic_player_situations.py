...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\vet\vet_clinic_player_situations.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 17685 bytes
...
import distributor, services, sims4
...
class VetClinicDiagnosisSituation(SituationComplexCommon):
    ...
    REMOVE_INSTANCE_TUNABLES = ('_buff', 'targeted_situation', '_resident_job', '_relationship_between_job_members',
                                'force_invite_only', 'screen_slam_gold', 'screen_slam_silver',
                                'screen_slam_bronze', 'screen_slam_no_medal', 'main_goal',
                                '_main_goal_visibility_test', 'minor_goal_chains',
                                'goal_sub_text', 'goal_button_text', '_hidden_scoring_override',
                                '_implies_greeted_status', '_level_data', 'weight_multipliers',
                                'recommended_job_object_notification', 'recommended_job_object_text') + Situation.SITUATION_START_FROM_UI_REMOVE_INSTANCE_TUNABLES

    @classmethod
    def _states(cls):
        return (
         SituationStateData(1, WaitForDiagnosisActorsState),
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [
         (
          cls.vet_job.job, cls.vet_job.role_state),
         (
    ...
    def on_added_to_distributor(self):
        ...
        if self._started_from_load:
            if None not in (self.get_pet(), self.get_vet()):
                self._change_state(DiagnosisSituationState())
...
lock_instance_tunables(VetClinicDiagnosisSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
  creation_ui_option=(SituationCreationUIOption.NOT_AVAILABLE),
  duration=0,
  situation_display_type_override=(SituationDisplayType.VET))