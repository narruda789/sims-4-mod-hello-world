...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\vet\vet_clinic_tests.py
# Compiled at: 2020-11-12 10:43:22
# Size of source mod 2**32: 6315 bytes
...
class AssignedToParticipantTest(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def test_target(self, target, resolver, tooltip=None):
        ...
        if not vet_zone_director:
            ...
            return vet_zone_director.is_assigned_to_vet(target.sim_id, participant.sim_id) or TestResult(False, ('{} is not assigned to {}'.format(target, participant)), tooltip=tooltip)
    ...
    def test_target(self, target, resolver, tooltip=None):
        ...
        if not vet_zone_director:
            ...
            if vet_zone_director.is_assigned_to_vet(target.sim_id, None):
                ...
                return TestResult.TRUE
            return self._invert or TestResult(False, ('{} not assigned to anyone'.format(target)), tooltip=tooltip)
        return TestResult.TRUE
...
class VetTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    FACTORY_TUNABLES = {'target':TunableEnumEntry(description='\n            When this test runs, it checks against this participant\n            for the test.\n            ',
     ...
     'test_to_perform':TunableVariant(description='\n            The test to perform.\n            ',
       ...
       locked_args={'patient_waiting_for_services':VetPatientWaitingForServicesTest(), 
      'patient_assigned_to_anyone':VetPatientAssignedToAnyoneTest(), 
      'patient_not_assigned_to_anyone':VetPatientAssignedToAnyoneTest(invert=True), 
      'vet_assigned_to_anyone':VetAttendingToAnyoneTest(), 
      'vet_not_assigned_to_anyone':VetAttendingToAnyoneTest(invert=True)},
       default='patient_not_assigned_to_anyone')}
