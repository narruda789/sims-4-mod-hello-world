...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\crafting\serving_interactions.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 33901 bytes
...
def find_serve_target(target, object_info, deliver_part=None):
    ...
    if not object_or_part.is_part:
        if not object_or_part.parts:
...
def push_object_pick_up_and_consume(parent_crafting_interaction, order_sim, object_to_serve, consume_affordance_override=None):
    ...
    if parent_crafting_interaction.sim is order_sim:
        return parent_crafting_interaction.process.orders or parent_crafting_interaction.should_push_consume(check_phase=False) or TestResult(False, 'Ordering Sim is not supposed to pick up the object if they are not thirsty.')
    context = InteractionContext(order_sim, (parent_crafting_interaction.source),
      (parent_crafting_interaction.priority),
      insert_strategy=(QueueInsertStrategy.NEXT),
      preferred_carrying_sim=(parent_crafting_interaction.sim))
...
class ServeObjectToSlotMixin(ServeObjectMixin):
    ...
    def _custom_content_sequence(self, sequence):
            ...
            for runtime_slot in object_or_part.get_runtime_slots_gen():
                ...
                if runtime_slot.empty:
                    ...
                    for child in child_objects:
                        if child.objectage_component:
                            current_age = child.in_use_by(self.sim) or child.get_current_age()
                            if current_age > old_age:
                                self.old_object = child
                                old_age = current_age
                                old_slot = runtime_slot

            if self.dest_slot is None:
                if self.old_object is not None:
                    logger.assert_raise((not self.old_object.in_use_by(self.sim)), 'Chose to destroy an old object ({}) still in use by actor ({}) in ({}) to place new object ({}) for interaction({}).  Please do more->GSI',
                      (self.old_object),
                      (self.sim),
                      (', '.join((str(reservation_handler) for reservation_handler in self.old_object.get_reservation_handlers()))),
        ...
        return (
         select_serve_slot, sequence)

    def _custom_claim_callback(self):
        super()._custom_claim_callback()
        if self.put_down:
            return
        elif self.dest_slot is not None:
...
class ServeObjectToCustomerMixin(ServeObjectMixin):
    ...
    def _build_sequence_with_callback(self, callback=None, sequence=()):
        ...
        return enter_carry_while_holding(self, (self.object_to_serve),
          create_si_fn=create_si,
          callback=callback,
          carry_sim=(self.order_sim),
          track=(self.object_info.carry_track),
          sequence=sequence)
...
class CraftingPhaseServeObjectSuperInteraction(CraftingPhaseSuperInteractionMixin, SuperInteraction):
    ...
    def build_basic_content(self, sequence, **kwargs):
        ...
        return (
         build_element(crafting_sequence),)
...
class CreateAndServeObjectSuperInteraction(CraftingPhaseCreateObjectSuperInteraction):
    INSTANCE_TUNABLES = {'fill_object_xevt_id':Tunable(int, 100, description='Xevt id for when to apply final states.'), 
     'fill_object_actor_name':Tunable(str, 'consumable', description='Name in Swing of the actor for the object being filled.')}
    ...
    def setup_asm_default(self, asm, *args, **kwargs):
        old_served_object = asm.get_actor_by_name(self.fill_object_actor_name)
        if old_served_object is not None:
            if old_served_object is not self.object_to_serve:
                asm.set_actor(self.fill_object_actor_name, None)
        else:
            return asm.set_actor(self.fill_object_actor_name, self.object_to_serve) or TestResult(False, 'CreateAndServeObjectSuperInteraction could not set actor {} on actor name {} for interaction {} and asm {}'.format(self.created_target, self.fill_object_actor_name, self, asm))
...
class ChooseDeliverySuperInteraction(CraftingPhaseSuperInteractionMixin, SuperInteraction):
    ...
    def _verify_tuning_callback(cls):
        if not cls.skip_serve_if_crafter_is_orderer:
            if cls.serve_to_slot_affordance is None:
                if cls.serve_to_sit_slot_affordance is None:
    ...
    def _pick_serve_affordance(self, order_sim, object_info, context):
        ...
        if self.serve_target_override is None:
            ...
            if target is None:
                ...
                target = self.target
            elif order_sim is not None:
                if order_sim.is_simulating:
                    order_surface_target = order_sim.posture_state.surface_target
                    if order_surface_target is not None:
                        if order_surface_target.is_part:
                            if deliver_to_sit_slot is not None:
                                serve_object = None
                                if target.is_part:
                                    serve_object = target.part_owner
                                if order_surface_target.part_owner is serve_object:
            ...
            return (
             deliver_to_slot, serve_target, None, carry_track)
    ...
    def _push_serve_affordance(self, order_sim, recipe):
        ...
        elif object_info.carry_track != carry_track:
        ...
        aop = AffordanceObjectPair(deliver_affordance, target, deliver_affordance,
          None, order_sim=order_sim,
    ...
    def _push_consume_affordance(self, order_sim, recipe):
        self._went_to_next_phase_or_finished_crafting = True
        return self.process.orders or self.should_push_consume(check_phase=False) or TestResult(False, 'Ordering Sim cannot push consume affordance but skip_serve_if_crafter_is_orderer is True.')
        serve_object = None
