...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\crafting\crafting_ingredients.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 21153 bytes
    ...
    def get_ingredient_quality_state(cls, quality_bonus):
        ...
        for quality_state_value, quality_details in IngredientTuning.INGREDIENT_QUALITY_MAPPING.items():
            if not bonus_selected is None:
                if not quality_details.quality_boost <= bonus_selected or bonus_selected >= quality_bonus:
...
class Ingredient:
    ...
    def get_cumulative_quality(self, ingredient_logger):
        ...
        ingredient_logger.append({'ingredient':str(self.ingredient_object) if self.obj_ref is not None else '',  'quality':str(quality_level), 
