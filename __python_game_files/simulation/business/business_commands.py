...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\business\business_commands.py
# Compiled at: 2018-03-05 20:00:39
# Size of source mod 2**32: 19567 bytes
...
import distributor, services, sims4.commands, sims4.log, telemetry_helper
...
def show_employee_management_dialog(opt_sim: OptionalSimInfoParam=None, _connection=None):
    ...
    for business_employee_type, business_employee_data in business_manager.tuning_data.employee_data_map.items():
        with ProtocolBufferRollback(msg.jobs) as (employee_job_msg):
            ...
            results = services.sim_filter_service().submit_matching_filter(number_of_sims_to_find=(business_employee_data.potential_employee_pool_size), sim_filter=(business_employee_data.potential_employee_pool_filter),
...
def sell_lot(_connection=None):
    ...
    dialog.show_dialog(on_response=sell_lot_response, additional_tokens=(
     sell_value,))
...
def get_hireable_employees(employee_type: BusinessEmployeeType, _connection=None):
    ...
    if business_manager is not None:
        ...
        if employee_data is not None:
            ...
            results = services.sim_filter_service().submit_matching_filter(number_of_sims_to_find=(employee_data.potential_employee_pool_size), sim_filter=(employee_data.potential_employee_pool_filter),
              requesting_sim_info=sim_info,
              allow_yielding=False,
              gsi_source_fn=get_sim_filter_gsi_name)
            for result in results:
