...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\business\business_zone_director_mixin.py
# Compiled at: 2017-10-05 21:58:36
# Size of source mod 2**32: 23834 bytes
...
class BusinessZoneDirectorMixin:
    INSTANCE_TUNABLES = {'customer_situation_interval':TunableSimMinute(description='\n            The amount of time, in Sim minutes, between attempts to create new\n            customer situations.\n            ',
     ...
     'employee_situation_data':TunableMapping(description='\n            A mapping of Business Employee Type to the data required by the zone\n            director for starting situations.\n            ',
      ...
      BusinessEmployeeType.INVALID,)),
       key_name='Business_Employee_Type',
       ...
       npc_employee_situation_count=TunableVariant(description='\n                    The number of situations to create at an NPC-owned business\n                    lot.\n                    ',
       tuned_count=TunableRange(tunable_type=int,
       minimum=0,
       default=3),
       default='tuned_count',
       locked_args={'defer_to_zone_director': DEFER_NPC_EMPLOYEE_COUNT_TO_ZONE_DIRECTOR})),
       value_name='Employee_Situation_Data',
       tuning_group=GroupNames.BUSINESS)}
    ...
    def _start_npc_employee_situations(self):
        ...
        for employee_type, employee_situation_data in self.employee_situation_data.items():
            ...
            for _ in range(num_to_create):
                situation_id = situation_manager.create_situation((self._get_npc_employee_situation_for_employee_type(employee_type)), guest_list=SituationGuestList(invite_only=True),
                  spawn_sims_during_zone_spin_up=True,
                  user_facing=False,
    ...
    def on_add_employee(self, sim_info, employee_data):
        if self._business_manager is None:
            ...
            return self._business_manager.is_open or None
        situation_manager = services.get_zone_situation_manager()
        if situation_manager is None:
            return
    ...
    def _start_employee_situation(self, employee_sim_id, employee_data, situation_manager):
        ...
        situation_id = situation_manager.create_situation((self._get_employee_situation_for_employee_type(employee_type)), guest_list=guest_list,
          spawn_sims_during_zone_spin_up=True,
          user_facing=False,
          creation_source=creation_source)
        self._employee_situation_ids[employee_type].add(situation_id)
    ...
    def _customer_situation_alarm_callback(self, *_, **__):
        return self._business_manager is None or self._business_manager.is_open or None
    ...
    def create_customer_alarm(self):
        self._customer_situation_alarm_handle = alarms.add_alarm(self, create_time_span(minutes=(self.customer_situation_interval)),
          (self._customer_situation_alarm_callback),
          repeating=True)