...
# Compiled at: 2019-12-02 17:31:27
# Size of source mod 2**32: 16539 bytes
import collections
...
EntryData = collections.namedtuple('EntryData', ('entry_text', 'entry_icon_info_data',
                                                 'entry_tooltip', 'entry_sublist',
...
SubListData = collections.namedtuple('SubListData', ('object_definition', 'item_count',
                                                     'num_objects_required', 'is_ingredient',
                                                     'new_item', 'object_display_name',
                                                     'item_icon_info_data', 'item_tooltip'))
...
class NotebookEntryBait(NotebookEntry):
    ...
    INSTANCE_TUNABLES = {'entry_text_rarity':TunableLocalizedStringFactory(description='\n            The text to display for rarity.\n            e.g.:\n            Rarity:\n{0.String}\n            '), 
     'entry_text_size_mapping':TunableMapping(description='\n            Mapping between fish size and the text to display for size.\n            ',
       ...
       tunable_type=str,
       default=None,
       source_query=(SourceQueries.SwingEnumNamePattern.format('fishType'))),
    ...
    def _get_entry_tooltip(self, entry_def):
        return EntryTooltip(HovertipStyle.HOVER_TIP_DEFAULT, {TooltipFields.recipe_name: LocalizationHelperTuning.get_object_name(entry_def), 
         TooltipFields.recipe_description: LocalizationHelperTuning.get_object_description(entry_def), 
         TooltipFields.rarity_text: self._get_entry_rarity_text(entry_def), 
         TooltipFields.subtext: self._get_entry_size_text(entry_def)})
    ...
    def get_definition_notebook_data(self, ingredient_cache=[]):
        ...
        for sub_entry_id, new_sub_entry in reversed(self.sub_entries):
            ...
            if not fish_definition is None:
