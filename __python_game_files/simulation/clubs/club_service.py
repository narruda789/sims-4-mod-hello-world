...
# Compiled at: 2020-07-17 15:07:03
# Size of source mod 2**32: 66019 bytes
...
import build_buy, enum, game_services, persistence_error_types, services, sims4, telemetry_helper
...
class ClubService(Service):
    CLUB_VALIDATION_EVENTS = (
     TestEvent.CareerEvent, TestEvent.AgedUp, TestEvent.SpouseEvent, TestEvent.SimoleonsEarned, TestEvent.OnExitBuildBuy)
    ...
    def __init__(self):
        ...
        self.broadcaster_extra = type('Club_Rule_Broadcaster', (BroadcasterRequest, object), {'broadcaster_types':lambda *_, **__: [
          club_tuning.ClubTunables.CLUB_RULE_BROADCASTER], 
    ...
    def start_gathering(self, club, start_source=ClubGatheringStartSource.DEFAULT, host_sim_id=0, invited_sims=(), zone_id=DEFAULT, ignore_zone_validity=False, **kwargs):
        if club not in self.clubs:
                ...
                for sim_info in invited_sims:
                    ...
                    if rabbit_hole_id is not None:
                        ...
                        def on_response(chose_leave_rabbit_hole=None):
                            ...
                            (self.start_gathering)(club, start_source=start_source, host_sim_id=host_sim_id, invited_sims=invited_sims, zone_id=zone_id, 
                             ignore_zone_validity=ignore_zone_validity, **kwargs)
            ...
            for sim_info in club.members:
                if sim_info not in invited_sims:
                    if not sim_filter_service.does_sim_match_filter((sim_info.sim_id), sim_filter=(club_gathering_default_job.filter),
                      gsi_source_fn=(lambda : self._get_sim_filter_gsi_name(True, sim_info=sim_info))):
                        continue
                guest_list.add_guest_info(SituationGuestInfo((sim_info.sim_id), club_gathering_default_job, (RequestSpawningOption.DONT_CARE),
                  (BouncerRequestPriority.BACKGROUND_HIGH), expectation_preference=True))
        ...
        (situation_manager.create_situation)(ClubTunables.DEFAULT_CLUB_GATHERING_SITUATION, guest_list=guest_list, 
         user_facing=False, custom_init_writer=init_writer, 
    ...
    def on_gathering_started(self, gathering):
        ...
        if current_gathering_for_club is not None:
            if current_gathering_for_club is not gathering:
    ...
    def on_sim_added_to_gathering(self, sim, gathering):
        ...
        if current_gathering_for_sim is not None:
            if current_gathering_for_sim is not gathering:
        ...
        for rule in self.club_rule_mapping[sim_info][interaction_type]:
            if rule.club is not club_gathering.associated_club:
                ...
                if not self._does_rule_pass_tests(sim_info, rule, aop_or_interaction):
                    continue
                rule.is_encouraged or discouraged_rules.append(rule)
                continue
            if not discouraged_rules:
                encouraged_rules.append(rule)

        if discouraged_rules:
            return (
             ClubRuleEncouragementStatus.DISCOURAGED, discouraged_rules)
        if encouraged_rules:
            return (
             ClubRuleEncouragementStatus.ENCOURAGED, encouraged_rules)
        return NO_EFFECT
    ...
    def get_encouragment_mapping_for_sim_info(self, sim_info, aop):
        ...
        if sim_info in self.sim_info_interacton_club_rewards:
            if interaction_type in self.sim_info_interacton_club_rewards[sim_info]:
    ...
    def get_front_page_bonus_for_mixer(self, sim_info, aop):
        ...
        if sim is not None:
            if sim in self.sims_to_gatherings_map:
                if self.interaction_is_encouraged_for_sim_info(sim_info, aop):
    ...
    def _does_rule_pass_tests(self, sim_info, rule, aop_or_interaction):
        if aop_or_interaction.target is not None:
            if aop_or_interaction.target.is_sim:
                target_sim_info = aop_or_interaction.target.sim_info
                if rule.with_whom is not None:
                    if target_sim_info is not sim_info:
                        if not rule.with_whom.test_sim_info(target_sim_info):
                            return False
        return True
    ...
    def create_rewards_buck_liability(self, sim_info, interaction):
        if any((isinstance(liability, ClubBucksLiability) for liability in interaction.liabilities)):
            return
        if self.interaction_is_encouraged_for_sim_info(sim_info, interaction.aop):
    ...
    def get_interaction_score_multiplier(self, interaction, sim=DEFAULT):
        ...
        if sim_info not in self.club_rule_mapping:
            ...
            return club_rules or base_multiplier
        ...
        if context.pick is not None:
            ...
            if target is None:
                ...
                if target.is_sim:
                    ...
                    return
            elif target is None or target is context.sim:
                return
            else:
    ...
    def generate_members_for_membership_criteria(self, number_of_members, admission_criteria):
        ...
        def _is_valid_member(sim_info):
            if sim_info.household is None:
                ...
                if not sim_info.is_human:
                    return False
                return all((criteria.test_sim_info(sim_info) for criteria in admission_criteria)) or False
            return True
        ...
        members = services.sim_filter_service().submit_matching_filter(sim_filter=member_filter, sim_constraints=allowed_sim_ids,
          number_of_sims_to_find=number_of_members,
    ...
    def _load_specific_criteria(self, criteria_data, for_rule=False):
        ...
        if not criteria.is_multi_select:
            ...
            if not criteria_info.resource_value:
                if not criteria_info.enum_value:
                    if not criteria_info.resource_id:
                        return
        try:
            ...
            if club_data is not None:
                ...
                if hangout_setting == ClubHangoutSetting.HANGOUT_VENUE:
                    ...
                    if hangout_venue is None:
                        ...
                        if hangout_setting == ClubHangoutSetting.HANGOUT_LOT:
                ...
                new_club = Club(club_id, name, icon, description, encouragement_commodity, discouragement_commodity, encouragement_buff, discouragement_buff, social_encouragement_buff,
                  leader=leader, leader_id=leader_id, member_ids=member_ids, recent_member_ids=recent_member_ids, membership_criteria=membership_criteria,
                  rules=club_rules,
                  hangout_setting=hangout_setting,
                  hangout_venue=hangout_venue,
                  hangout_zone_id=hangout_zone_id,
                  invite_only=invite_only,
                  uniform_male_child=uniform_male_child,
                  associated_color=associated_color,
                  uniform_female_child=uniform_female_child,
                  uniform_male_adult=uniform_male_adult,
                  uniform_female_adult=uniform_female_adult,
                  club_seed=club_seed,
                  bucks_tracker_data=bucks_tracker_data,
                  male_adult_mannequin=male_adult_mannequin,
                  male_child_mannequin=male_child_mannequin,
                  female_adult_mannequin=female_adult_mannequin,
                  female_child_mannequin=female_child_mannequin,
                  outfit_setting=outfit_setting,
    ...
    def add_club(self, club, from_load=False, suppress_telemetry=False):
        ...
        if not from_load:
            club.bucks_tracker.try_modify_bucks((ClubTunables.CLUB_BUCKS_TYPE), (ClubTunables.INITIAL_AMOUNT_OF_CLUB_BUCKS),
              reason=(None if from_load else 'Creating Club'),
    ...
    def on_finish_waiting_for_sim_spawner_service(self):
        ...
        if not ensemble_service.should_start_travel_ensemble(traveled_sims):
            return
        common_clubs = None
    ...
    def send_club_building_info(self):
        ...
        for zone_data in persistence_service.zone_proto_buffs_gen():
            ...
            if not (venue_tuning is None or venue_tuning.allowed_for_clubs):
                continue
            ...
            if not home_region.is_region_compatible(region_instance):
                if not current_region.is_region_compatible(region_instance):
                    continue
            if not venue_tuning.is_residential:
                if venue_tuning.is_university_housing:
