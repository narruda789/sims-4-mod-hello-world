...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\retail\retail_employee_situation.py
# Compiled at: 2018-02-20 17:05:22
# Size of source mod 2**32: 16333 bytes
import random
...
import services, sims4, situations
...
class RetailEmployeeSituation(BusinessEmployeeSituationMixin, SituationComplexCommon):
    ...
    class _EmployeeSituationState(HasTunableFactory, AutoFactoryInit, SituationState):
        ...
        def on_activate(self, reader=None):
            ...
            self._create_or_load_alarm((self.state_name), timeout,
        ...
        if event == TestEvent.InteractionComplete:
            ...
            if target_sim is self._employee_sim_info:
                if resolver(self.state_socialize.push_interaction):
                    ...
                    if resolver(self.state_restock.push_interaction):
                        self._change_state(self.state_restock())
                    else:
                        if resolver(self.state_clean.push_interaction):
                            self._change_state(self.state_clean())
                        else:
                            if resolver(self.state_slack_off.push_interaction):
                                self._change_state(self.state_slack_off())
                            else:
                                if resolver(self.state_ring_up_customers.push_interaction):
                                    self._change_state(self.state_ring_up_customers())
                                else:
                                    if resolver(self.go_home_interaction):
...
class _GoToStoreState(SituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm((self._GO_TO_STORE_TIMEOUT), (self.owner.role_state_go_to_store_timeout),
          (self._on_timeout),
          reader=reader)
        ...
        if employee_sim_info is not None:
            if not services.current_zone().is_zone_shutting_down:
                ...
                if sim_instance is not None:
                    if not sim_instance.is_being_destroyed:
                        if not self.owner._is_clocked_in:
    ...
    def handle_event(self, sim_info, event, resolver):
        if self.owner is None:
            return
        if event == TestEvent.InteractionComplete:
            if self.owner._employee_sim_info is sim_info:
