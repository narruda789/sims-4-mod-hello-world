...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\performance\object_leak_tracker.py
# Compiled at: 2018-10-09 20:50:39
# Size of source mod 2**32: 18887 bytes
from _collections import defaultdict
from collections import namedtuple
from xml.sax.saxutils import escape
import gc, time, weakref, caches, enum, interactions.constraints, services, sims4.log
...
class Node:
    __slots__ = ('_ref', '_status', '_pid', '_obj_type', '_manager_type', '_old_obj_id',
                 '_time_stamps')
    ...
    def _gc_callback(self, phase, info):
        ...
        for node in tuple(self._node_buckets[NodeStatus.PENDING]):
            ...
            if now.gc_pass < node_time.gc_pass + GC_PASS_THRESHOLD:
                continue
            if node_time.time is not None and now.time is not None:
                if (now.time - node_time.time).in_hours() < SIM_HOUR_THRESHOLD:
                    continue
