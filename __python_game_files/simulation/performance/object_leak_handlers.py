...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\performance\object_leak_handlers.py
# Compiled at: 2016-08-22 18:49:23
    ...
    for status in NodeStatus:
        for node in tracker.buckets[status]:
            ...
            for time_status, time_stamp in node.time_stamps.items():
                time_data.append({'status':time_status.name, 
                 'gc_pass':time_stamp.gc_pass, 
                 'time':str(time_stamp.time)})
