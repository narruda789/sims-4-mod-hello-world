...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\achievements\achievements.py
# Compiled at: 2020-07-08 19:08:20
# Size of source mod 2**32: 6402 bytes
...
import gsi_handlers.achievement_handlers, paths, services, sims4.log, telemetry_helper
...
class AchievementTracker(data_tracker.EventDataTracker):
    ...
    def _send_tracker_to_client(self, init=False):
        ...
        for achievement in self._completed_milestones:
            self.milestone_sent(achievement) or self._sent_milestones.add(achievement)
            msg.achievements_completed.append(achievement.guid64)
            if msg_empty:
                msg_empty = False

        for objective in self._completed_objectives:
            self.objective_sent(objective) or self._sent_objectives.add(objective)
    ...
    def complete_objective(self, objective_instance, milestone):
        result = super().complete_objective(objective_instance, milestone)
