...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sickness\sickness_handlers.py
# Compiled at: 2019-02-13 14:08:22
...
def generate_sick_sim_view():
    ...
    for sim_info in tuple(services.sim_info_manager().values()):
        ...
        sim_data.append({'sim_id':str(hex(sim_info.id)), 
         ...
         'last_recorded_progress':str(sim_info.sickness_tracker.last_progress), 
         'discovered':str(sim_info.sickness_tracker.has_discovered_sickness), 
         'Symptoms':[{'symptom':str(symptom),  'is_discovered':str(sim_info.was_symptom_discovered(symptom))} for symptom in sickness.symptoms], 
         'Examinations Performed':[{'interaction': str(interaction)} for interaction in exams_performed], 
         'Treatments Performed':[{'interaction': str(interaction)} for interaction in treatments_performed], 
    ...
    for sim_info in tuple(services.sim_info_manager().values()):
        ...
        if not sim_info.is_sick():
            ...
            for item in weighted_sicknesses:
                sickness_data.append({'sickness':str(item[1]), 
                 'weight':item[0], 
                 'chance':item[0] / total_weight})

            sim_data.append({'sim_id':str(hex(sim_info.id)), 
             'sim':sim_info.full_name, 
             'chance':sickness_service.get_sickness_chance(SingleSimResolver(sim_info)), 
             'Sickness Chances':sickness_data, 
             'Examinations Performed':[{'interaction': str(interaction)} for interaction in exams_performed]})
...
def archive_sim_sickness_event(sim_info, sickness, event_message):
    if sim_sickness_archiver.enabled:
        sim_sickness_archiver.archive(object_id=(sim_info.id),
          data={'game_time':str(services.game_clock_service().now()), 
         'sickness':str(sim_info.current_sickness.__name__), 
