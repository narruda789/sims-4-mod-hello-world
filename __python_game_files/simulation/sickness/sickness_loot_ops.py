...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sickness\sickness_loot_ops.py
# Compiled at: 2017-09-21 14:26:45
...
class _SicknessMatchingCritera(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value):
        if value.tags is None:
            if value.difficulty_range is None:
    ...
    FACTORY_TUNABLES = {'tags':OptionalTunable(description='\n            Optionally, only sicknesses that share any of the tags specified are considered. \n            ',
       tunable=TunableTags(filter_prefixes=('Sickness', ))), 
     'difficulty_range':OptionalTunable(description="\n            Optionally define the difficulty rating range that is required\n            for the Sim's sickness.\n            ",
    ...
    def give_sickness(self, subject):
        ...
        services.get_sickness_service().make_sick(subject, criteria_func=sickness_criteria,
          only_auto_distributable=False)


class GiveSicknessLootOp(BaseLootOperation):
    FACTORY_TUNABLES = {'sickness':TunableVariant(description='\n            Sickness to give to the subject.\n            ',
       locked_args={'random': None},
       specific_sickness=_GiveSpecificSickness.TunableFactory(),
       sickness_criteria=_SicknessMatchingCritera.TunableFactory(),
       default='random'), 
     'reset_diagnosis_progress':Tunable(description='\n            If checked then reset the diagnosis progress, make all\n            exams available, and clear the list of treatments performed.\n            ',
       tunable_type=bool,
       default=True)}
