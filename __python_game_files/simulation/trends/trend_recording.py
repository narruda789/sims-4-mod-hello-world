...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\trends\trend_recording.py
# Compiled at: 2019-05-06 18:40:38
...
class _TrendsCreationData(CreationDataBase):
    ...
    def get_definition(self, *_, **__):
        if not self._recorded_trend_tag:
            ...
            filtered_defs or logger.error('Could not find object definitions tagged as {} for recording trends.', self.filter_tags)
            return
...
class RecordTrendsElement(elements.ParentElement, ObjectCreationMixin, HasTunableFactory, AutoFactoryInit):
    ...
    def _record_static_trends(self):
        if self.celebrity_tests is not None:
            if self.celebrity_tests.run_tests(self.resolver):
                self.creation_data.record_trend_tag(TrendTuning.CELEBRITY_TREND)
                return
        if self._recorded_sim.age == Age.CHILD or self._recorded_sim.age == Age.TODDLER:
    ...
    def _run(self, timeline):
        sequence = [
         self._start_recording, self.sequence, self._stop_recording]
