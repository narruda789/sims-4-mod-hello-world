...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\filters\household_template.py
# Compiled at: 2020-08-27 14:36:01
# Size of source mod 2**32: 24340 bytes
import random
...
import id_generator, relationships.relationship_bit, services, sims4.log, sims4.resources, sims4.tuning.instances, tag
logger = sims4.log.Logger('HouseholdTemplate', default_owner='msantander')
HOUSEHOLD_FILTER_PREFIX = [
 'household_member']
...
class HouseholdTemplate(HasTunableReference, metaclass=sims4.tuning.instances.TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SIM_TEMPLATE)):
    ...
    def _verify_tuning_callback(cls):
        ...
        if cls._household_relationship:
            if not tag_to_household_member_index:
            ...
            if member_relationship_data.is_spouse:
                for member in (x_member, y_member):
                    ...
                    if sim_template.can_validate_age():
                        sim_template.matches_creation_data(age_min=(Age.YOUNGADULT)) or logger.error('Trying set spouse with sims of the inappropriate age. Check sim_template at index {} if set correctly.', member_index)
    ...
    def has_spouse(cls):
        for household_relationship in cls._household_relationship:
            if household_relationship.is_spouse or RelationshipGlobalTuning.MARRIAGE_RELATIONSHIP_BIT in set(household_relationship.relationship_bits):
                return True
    ...
    def create_household(cls, inst, zone_id, account=None, sim_creator=None, sim_name_type=SimNameType.DEFAULT, creation_source: str='household_template'):
        ...
        if sim_name_type == SimNameType.DEFAULT:
            for creator in sim_creators:
                if creator.sim_name_type != SimNameType.DEFAULT:
                    sim_name_type = creator.sim_name_type
                    break

        home_zone_id = zone_id or 0
        created_sim_infos, household = SimSpawner.create_sim_infos(sim_creators, zone_id=home_zone_id, account=account, starting_funds=(cls._household_funds),
        ...
        if matching_sim_creator_index is not None:
            return (
             household, created_sim_infos[matching_sim_creator_index])
        return household

    @flexmethod
    def set_household_relationships(cls, inst, created_sim_infos, household):
        inst_or_cls = inst if inst is not None else cls
        tag_to_sim_info = {household_member.household_member_tag:sim_info for household_member, sim_info in zip(inst_or_cls.get_household_members(), created_sim_infos) if household_member.household_member_tag != tag.Tag.INVALID if household_member.household_member_tag != tag.Tag.INVALID}
            ...
            if not source_sim_info is None:
                ...
                for bit_to_add in member_relationship_data.relationship_bits:
                    ...
                    if bit_track_node is not None:
                        ...
                        source_sim_info.relationship_tracker.add_relationship_bit((target_sim_info.id), bit_to_add,
                          force_add=True)