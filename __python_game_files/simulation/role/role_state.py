...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\role\role_state.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 26792 bytes
...
from tag import Tag
import autonomy, buffs.tunable, enum, role.role_state_base, services, sims4.log, sims4.resources, tag
...
class PushAffordanceFromRole(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'description':'\n            Push the specific affordance onto the sim.\n            ', 
     'affordance':TunableReference(manager=services.get_instance_manager(sims4.resources.Types.INTERACTION)), 
     'super_affordance_for_mixer':OptionalTunable(description='\n            If we want to push mixer directly in the affordance tuning for this\n            role state, we would need to provide a super affordance here to\n            handle the mixer.\n            ',
     ...
     'source':TunableEnumEntry(tunable_type=InteractionSource,
...
class DoParameterizedAutonomyPingFromRole(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def __call__(self, role_state, role_affordance_target, situation=None):
        ...
        autonomy_request = autonomy.autonomy_request.AutonomyRequest(sim,
          ...
          test_connectivity_to_target_object=(self.test_connectivity_to_target),
          reping_delay_on_fail=(self.full_ping_on_fail() if self.full_ping_on_fail is not None else None))
...
class RoleState(HasDependentTunableReference, role.role_state_base.RoleStateBase, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.ROLE_STATE)):
    INSTANCE_TUNABLES = {'_role_priority':TunableEnumEntry(RolePriority,
       RolePriority.NORMAL,
     ...
     '_buffs':TunableList(buffs.tunable.TunableBuffReference(pack_safe=True),
     ...
     '_off_lot_autonomy_buff':buffs.tunable.TunableBuffReference(description='A buff that\n            prevents autonomy from considering some objects based on the\n            location of the object (e.g. on lot, off lot, within a radius of the\n            sim).\n             \n            In the buff set: Game Effect Modifiers->Autonomy Modifier->Off Lot\n            Autonomy Rule.\n            ',
       allow_none=True), 
     'tags':TunableSet(TunableEnumEntry(Tag, Tag.INVALID),
    ...
    def _verify_tuning_callback(cls):
        for buff_ref in cls.buffs:
            if buff_ref is None:
                ...
                continue
            if buff_ref.buff_type._temporary_commodity_info is not None:
    ...
    def has_full_permissions(cls):
        ...
        if current_venue:
            if current_venue.allow_rolestate_routing_on_navmesh:
                return True
    ...
    def _get_target_for_push_affordance(self, situation_target, situation=None, role_affordance_target=None):
        if situation_target == SituationAffordanceTarget.NO_TARGET:
            ...
            if situation_target == SituationAffordanceTarget.TARGET_OBJECT:
                if situation is not None:
