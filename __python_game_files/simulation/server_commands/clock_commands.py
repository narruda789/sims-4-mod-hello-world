...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\server_commands\clock_commands.py
# Compiled at: 2021-06-04 20:03:06
# Size of source mod 2**32: 10982 bytes
from clock import ClockSpeedMode, GameSpeedChangeSource
from sims4.commands import CommandType
import clock, services, sims4.commands, sims4.log, telemetry_helper
...
def request_pause(pause_handle_name='From Command', _connection=None):
    ...
    game_clock_service.push_speed((ClockSpeedMode.PAUSED),
      source=(_get_speed_source(pause_handle_name)), reason=pause_handle_name,
      immediate=True)
...
def unrequest_pause(pause_handle_name='From Command', _connection=None):
    ...
    game_clock_service = services.game_clock_service()
    game_clock_service.pop_speed((ClockSpeedMode.PAUSED),
      source=(_get_speed_source(pause_handle_name)), reason=pause_handle_name,
      immediate=True)
    send_clock_telemetry_data(_connection)
    return False
...
def ignore_interaction_speed_change_requests(value: bool=True, _connection=None):
    clock.GameClock.ignore_game_speed_requests = value
