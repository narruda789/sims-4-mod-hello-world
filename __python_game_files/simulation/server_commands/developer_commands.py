...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\server_commands\developer_commands.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 36991 bytes
...
import areaops, date_and_time, gsi_handlers.dump, gsi_handlers.gameplay_archiver, paths, services, sims.sim_info_types, sims4.command_script, sims4.commands, sims4.gsi.archive, sims4.gsi.http_service, sims4.hash_util, sims4.log, sims4.resources, story_progression
...
def common_cheats(flags: str='', enable: bool=None, _connection=None):
    ...
    commands = [
     ('', True, 'rr.toggletime 12', None),
     ('b', True, 'bb.showwipobjects', None),
     ('r', True, 'routing.toggle_navmesh on', 'routing.toggle_navmesh off'),
     ('r', True, 'routing.toggle_visualization on', 'routing.toggle_visualization off'),
     ('', False, 'autonomy.ambient off', 'autonomy.ambient on'),
     ('', False, 'autonomy.household off', 'autonomy.household on'),
     ('', False, 'stats.fill_commodities_household', None),
     ('', False, 'stats.disable_all_commodities', 'stats.enable_all_commodities'),
     ('c', False, 'crafting.shorten_phases on', 'crafting.shorten_phases off'),
     ('', False, 'sims.reset_all', None),
     ('', False, 'death.toggle False', 'death.toggle True'),
     ('', False, 'fire.toggle_enabled off', 'fire.toggle_enabled on')]
    if flags == '*':
        flags = None
    if enable is None:
    ...
    for cmd_flags, client_cmd, enable_cmd, disable_cmd in commands:
        if flags is not None:
            if cmd_flags:
                if not any((f in flags for f in cmd_flags)):
                    continue
...
def world_perf_test(_connection=None):
    commands = [
     ...
     'stats.disable_all_commodities',
     'death.toggle False',
     'situations.enable_perf_cheats']
    output = sims4.commands.Output(_connection)
        ...
        if tgt_client is None:
            ...
            if server is not None:
                ...
                for _ in range(number_of_instances):
                    msg = GSI_Open(ip=(server_info[0]), port=(int(server_info[1])), zone_id=(int(tgt_client.zone_id)),
...
def gsi_dump(compress: bool=True, location=None, error_str='From Command: |gsi.dump', filename=None, _connection=None):
    ...
    if not os.path.isdir(location):
        ...
        return False
    full_path = gsi_handlers.dump.save_dump_to_location(location, console_output=output, compress_file=compress,
      error_str=error_str,
      filename=filename)
    try:
        if output is not None:
...
def debug_unhash(hash_value: int, unhash_db=None, _connection=None):
    FRIENDLY_MAP = {'all':None, 
     'resource':1,  'tuning':2,  'object':3,  'swarm':4}
    output = sims4.commands.Output(_connection)
