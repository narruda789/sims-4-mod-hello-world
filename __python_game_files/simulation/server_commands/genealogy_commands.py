...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\server_commands\genealogy_commands.py
# Compiled at: 2020-08-24 20:41:19
# Size of source mod 2**32: 17792 bytes
...
def genealogy_random_generate(sim_id: OptionalTargetParam=None, generations: int=4, set_to_min_lod=False, _connection=None):
    ...
    def add_parents(child, generation=0):
        if generation >= generations:
            return
        sim_creators = (
         SimCreator(gender=(Gender.MALE), age=(Age.ADULT), last_name=(child.last_name)), SimCreator(gender=(Gender.FEMALE), age=(Age.ADULT)))
        sim_info_list, _ = SimSpawner.create_sim_infos(sim_creators, account=(sim.account),
          zone_id=(sim.zone_id),
...
FamilyTreeNode = namedtuple('FamilyTreeNode', ('sim_id', 'proto_node', 'antecedent_depth',
                                               'descendant_depth', 'extended_info_depth',
                                               'include_spouse', 'step_depth'))
...
def genealogy_show_family_tree(sim_info_id: int, antecedent_depth: int=8, descendant_depth: int=2, _connection=None):
    ...
    sims_looked_at = set()
    sims_to_look_at.append(FamilyTreeNode(sim_id=sim_info_id, proto_node=(family_tree_msg.root),
      antecedent_depth=antecedent_depth,
      descendant_depth=descendant_depth,
      extended_info_depth=2,
      include_spouse=True,
      step_depth=2))
    with genealogy_caching():
        automation_output('FamilyTreeInfo; Status:Begin')
        while sims_to_look_at or additional_sims_to_look_at:
            if sims_to_look_at:
            ...
            if family_tree_node.include_spouse:
                ...
                if spouse_id:
                    additional_sims_to_look_at.append(FamilyTreeNode(sim_id=spouse_id, proto_node=(family_tree_node.proto_node.spouse),
                      antecedent_depth=0,
                      descendant_depth=(1 if family_tree_node.step_depth else 0),
                      extended_info_depth=0,
            ...
            if family_tree_node.antecedent_depth:
                for parent_id in sim_info.genealogy.get_parent_sim_ids_gen():
                    with ProtocolBufferRollback(family_tree_node.proto_node.parents) as (parent_family_tree_node):
                        sims_to_look_at.append(FamilyTreeNode(sim_id=parent_id, proto_node=parent_family_tree_node,
                          antecedent_depth=(family_tree_node.antecedent_depth - 1),
                          descendant_depth=(1 if family_tree_node.extended_info_depth else 0),
                          extended_info_depth=(family_tree_node.extended_info_depth - 1 if family_tree_node.extended_info_depth else 0),
                          include_spouse=True,
                          step_depth=(family_tree_node.step_depth - 1 if family_tree_node.step_depth else 0)))
            ...
            if family_tree_node.descendant_depth:
                for child_id in sim_info.genealogy.get_children_sim_ids_gen():
                    with ProtocolBufferRollback(family_tree_node.proto_node.children) as (child_family_tree_node):
                        sims_to_look_at.append(FamilyTreeNode(sim_id=child_id, proto_node=child_family_tree_node,
                          antecedent_depth=(1 if family_tree_node.extended_info_depth else 0),
                          descendant_depth=(descendant_depth - 1),
                          extended_info_depth=0,
                          include_spouse=True,
                          step_depth=0))

            if family_tree_node.extended_info_depth or family_tree_node.include_spouse:
                    ...
                    if family_tree_node.extended_info_depth:
                        if relationship.has_bit(sim_info.sim_id, FamilyRelationshipTuning.SIBLING_RELATIONSHIP_BIT):
                            with ProtocolBufferRollback(family_tree_node.proto_node.siblings) as (sibling_family_tree_node):
                                additional_sims_to_look_at.append(FamilyTreeNode(sim_id=(relationship.get_other_sim_id(sim_info_id)), proto_node=sibling_family_tree_node,
                                  antecedent_depth=1,
                                  descendant_depth=0,
                                  extended_info_depth=0,
                                  include_spouse=False,
                            ...
                            if sim_info_id == family_tree_node.sim_id and relationship.has_bit(sim_info.sim_id, FamilyRelationshipTuning.GRANDPARENT_RELATIONSHIP_BIT):
                                with ProtocolBufferRollback(family_tree_node.proto_node.grandparents) as (grandparent_family_tree_node):
                                    additional_sims_to_look_at.append(FamilyTreeNode(sim_id=(relationship.get_other_sim_id(sim_info_id)), proto_node=grandparent_family_tree_node,
                                      antecedent_depth=0,
                                      descendant_depth=1,
                                      extended_info_depth=0,
                                      include_spouse=True,
                                ...
                                if sim_info_id == family_tree_node.sim_id:
                                    if relationship.has_bit(sim_info.sim_id, FamilyRelationshipTuning.GRANDCHILD_RELATIONSHIP_BIT):
                                        with ProtocolBufferRollback(family_tree_node.proto_node.grandchildren) as (grandchild_family_tree_node):
                                            additional_sims_to_look_at.append(FamilyTreeNode(sim_id=(relationship.get_other_sim_id(sim_info_id)), proto_node=grandchild_family_tree_node,
                                              antecedent_depth=1,
                                              descendant_depth=0,
                                              extended_info_depth=0,
                                              include_spouse=False,
                        ...
                        if family_tree_node.include_spouse:
                            if relationship.has_bit(sim_info.sim_id, FamilyRelationshipTuning.DIVORCED_SPOUSE_RELATIONSHIP_BIT):
                                with ProtocolBufferRollback(family_tree_node.proto_node.divorced_spouses) as (divorced_spouse_family_tree_node):
                                    additional_sims_to_look_at.append(FamilyTreeNode(sim_id=(relationship.get_other_sim_id(sim_info_id)), proto_node=divorced_spouse_family_tree_node,
                                      antecedent_depth=0,
                                      descendant_depth=0,
                                      extended_info_depth=0,
                                      include_spouse=False,
                            ...
                            if relationship.has_bit(sim_info.sim_id, FamilyRelationshipTuning.DEAD_SPOUSE_RELATIONSHIP_BIT):
                                with ProtocolBufferRollback(family_tree_node.proto_node.dead_spouses) as (dead_spouse_family_tree_node):
                                    additional_sims_to_look_at.append(FamilyTreeNode(sim_id=(relationship.get_other_sim_id(sim_info_id)), proto_node=dead_spouse_family_tree_node,
                                      antecedent_depth=0,
                                      descendant_depth=0,
                                      extended_info_depth=0,
                                      include_spouse=False,
