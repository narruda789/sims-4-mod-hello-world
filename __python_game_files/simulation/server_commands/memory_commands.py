...
# Size of source mod 2**32: 33043 bytes
...
def garbage_collector_dump--- This code section failed: ---
 ...
 L. 233        42  LOAD_GLOBAL              populate_all_referants
               44  LOAD_FAST                'obj'
 ...
 L. 235        56  LOAD_CONST               0
               58  STORE_FAST               'index'

 L. 236        60  LOAD_STR                 'python_mem_dump.txt'
               62  STORE_FAST               'file_name'

 L. 237        64  SETUP_LOOP          100  'to 100'
               66  LOAD_GLOBAL              os
               68  LOAD_ATTR                path
 ...
 L. 238        78  LOAD_FAST                'index'
               80  LOAD_CONST               1
               82  INPLACE_ADD      
               84  STORE_FAST               'index'

 L. 239        86  LOAD_STR                 'python_mem_dump{}.txt'
 ...
 L. 245       150  LOAD_FAST                'cur_index'
              152  LOAD_CONST               1
              154  INPLACE_ADD      
              156  STORE_FAST               'cur_index'

 L. 246       158  SETUP_EXCEPT        172  'to 172'

 L. 247       160  LOAD_GLOBAL              str
              162  LOAD_FAST                'key'
 ...
 L. 251       188  SETUP_EXCEPT        254  'to 254'

 L. 252       190  LOAD_GLOBAL              hasattr
 ...
 L. 259       270  SETUP_EXCEPT        294  'to 294'

 L. 260       272  LOAD_GLOBAL              str
              274  LOAD_GLOBAL              sys
 ...
 L. 264       310  SETUP_EXCEPT        358  'to 358'

 L. 265       312  LOAD_GLOBAL              str
 ...
 L. 271       374  SETUP_EXCEPT        404  'to 404'

 L. 272       376  LOAD_FAST                'output_file'
...
@sims4.commands.Command('mem.tracemalloc_clear_saved_snapshot', command_type=(sims4.commands.CommandType.Automation))
def tracemalloc_clear_saved_snapshot(_connection=None):
    global g_saved_snapshot
    g_saved_snapshot = None
    output = sims4.commands.CheatOutput(_connection)
    output('Saved snapshot cleared')


@sims4.commands.Command('mem.tracemalloc_save_snapshot', command_type=(sims4.commands.CommandType.Automation))
def tracemalloc_save_snapshot(dump_comparison: bool=False, dump_comparison_tracebacks: bool=True, _connection=None):
    global g_saved_snapshot
    output = sims4.commands.CheatOutput(_connection)
    ...
    if dump_comparison and g_saved_snapshot is None:
        output('Warning: Could not generate a comparison as there was no previous snapshot')
    else:
        if dump_comparison:
            comparison_stat_diffs = new_snapshot.compare_tog_saved_snapshot'lineno'

            def write_comparison_statistics_to_file(file):
                file.write('Traceback, Size, Size Diff, Count, Count Diff\n')
                for stat_diff in comparison_stat_diffs:
                    file.write('%s, %i, %i, %i, %i\n' % (stat_diff.traceback, stat_diff.size, stat_diff.size_diff, stat_diff.count, stat_diff.count_diff))

            output('Writing comparison stats to file...')
            create_csv('tracemalloc_comparison_stats', callback=write_comparison_statistics_to_file, connection=_connection)
            if dump_comparison_tracebacks:
                output('Writing comparison tracebacks to file...')
                comparison_traceback_stat_diffs = new_snapshot.compare_tog_saved_snapshot'traceback'

                def write_comparison_traceback_to_file(file):
                    file.write('Traceback\n')
                    for stat_diff in comparison_traceback_stat_diffs:
                        file.write('Size: {} Size Diff: {} Count: {} Count Diff: {}\n'.format(stat_diff.size, stat_diff.size_diff, stat_diff.count, stat_diff.count_diff))
                        for line in stat_diff.traceback.format():
                            file.write('{}\n'.format(line))

                        file.write('\n')

                create_text_file('tracemalloc_comparison_tracebacks', callback=write_comparison_traceback_to_file, connection=_connection)
    g_saved_snapshot = new_snapshot
    output('Saved snapshot')


@sims4.commands.Command('mem.tracemalloc_dump_saved_snapshot', command_type=(sims4.commands.CommandType.Automation))
def tracemalloc_dump_saved_snapshot(dump_tracebacks: bool=True, _connection=None):
    output = sims4.commands.CheatOutput(_connection)
    if g_saved_snapshot is None:
        output('Snapshot is none. Please take a snapshot before dumping to file.')
        return
    output('Getting snapshot statistics...')
    stats = g_saved_snapshot.statistics('lineno')

    def write_statistics_to_file(file):
        file.write('Traceback, Size, Count, Average\n')
        for stat in stats:
            file.write('%s, %i, %i, %f\n' % (stat.traceback, stat.size, stat.count, stat.size / stat.count))

    create_csv('tracemalloc_stats', callback=write_statistics_to_file, connection=_connection)
    output('Top ten stats: ')
    for stat in stats[:10]:
        output(str(stat))

    if dump_tracebacks:
        output('Getting snapshot tracebacks...')
        traceback_stats = g_saved_snapshot.statistics('traceback')

        def write_traceback_to_file(file):
            file.write('Traceback\n')
            total_traceback_size = 0
            for stat in traceback_stats:
                file.write('Size: {} Count: {} Average: {}\n'.format(stat.size, stat.count, stat.size / stat.count))
                total_traceback_size = total_traceback_size + stat.size
                for line in stat.traceback.format():
                    file.write('{}\n'.format(line))

                file.write('\n')

            file.write('Total Size: {}'.format(total_traceback_size))
