...
def purchase_picker_response_by_ids(inventory_target: RequiredTargetParam, inventory_source: RequiredTargetParam, currency_type: int, dialog_id: int, delivery_method: int=PickerInteractionDeliveryMethod.INVENTORY, object_ids_or_definition_ids: bool=False, *ids_and_amounts_and_price: int, _connection=None):
    total_price = 0
    current_purchased = 0
    objects_to_buy = []
    definition_manager = services.definition_manager()
...
def sell_picker_response_by_ids(inventory_source: RequiredTargetParam, currency_type: int, dialog_id: int, *ids_and_amounts_and_price: int, _connection=None):
    total_price = 0
    inventory_manager = services.inventory_manager()
    client = services.client_manager().get(_connection)
    if client is None:
