...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\build_buy.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 50180 bytes
from contextlib import contextmanager
from indexed_manager import ObjectIDError
from objects import ALL_HIDDEN_REASONS
from objects.object_enums import ItemLocation
import id_generator, objects
...
import enum, pythonutils, sims4.reload, sims4.utils, routing, services, venues
...
def move_object_to_household_inventory(obj, failure_flags=0, object_location_type=ObjectOriginLocation.ON_LOT):
    ...
    elif failure_flags & HouseholdInventoryFlags.DESTROY_OBJECT:
...
def _get_object_data_from_household_inventory(object_id, household_id):
    if is_household_inventory_available(household_id):
...
def get_definition_id_in_household_inventory(object_id, household_id):
    object_data = _get_object_data_from_household_inventory(object_id, household_id)
    if object_data is not None:
        return (
         get_vetted_object_defn_guid(object_id, object_data.guid), object_data)
    return (None, None)
...
def get_object_in_household_inventory(object_id, household_id):
    ...
    def make_object(object_data):
        ...
        try:
            obj = objects.system.create_object((object_data.guid), cls_override=HouseholdInventoryObject,
              obj_id=object_id,
        ...
        for sim in household.instanced_sims_gen():
            ...
            if not career_tracker is None:
                ...
                for career in career_tracker:
                    ...
                    if customer_zone_id is None or customer_zone_id != zone_id:
                        continue
                    if not hasattr(gig, 'update_budget_spent'):
                        continue
...
def _sync_venue_service_to_zone_venue_type(zone_id, from_bb_venue_changed=False):
    ...
    if not active_venue_tuning_id is None:
        if not raw_active_venue_tuning_id is None:
...
def c_api_house_inv_obj_removed(zone_id, household_id, obj_id, obj_def_id):
    ...
    if animal_service is not None:
        if animal_service.is_registered_home(obj_id):
            ...
            if obj_id in animal_service.animal_assignment_map:
