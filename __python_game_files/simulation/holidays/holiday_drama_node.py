...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\holidays\holiday_drama_node.py
# Compiled at: 2022-03-10 20:35:09
# Size of source mod 2**32: 25248 bytes
...
import alarms, services, sims4.log, sims4.resources
...
HOLIDAY_HUNT_TOKEN = 'holiday_hunt'
...
class HolidayDramaNode(BaseDramaNode):
    ...
    def get_time_off_reason(self, sim_info, career_category, career_end_time):
        ...
        if career_category == CareerCategory.School:
            ...
            if career_category in WORK_CAREER_CATEGORIES:
                take_time_off = holiday_service.get_holiday_time_off_work(self.holiday_id)
            else:
                if career_category == CareerCategory.Volunteer or career_category == CareerCategory.UniversityCourse:
                    take_time_off = False
                else:
                    logger.error('Unexpected CareerCategory {} when determining if a holiday should give Sims time off.', career_category)
        if take_time_off:
            if self.is_running or self.get_calendar_start_time() < career_end_time:
    ...
    def search_obj(self, obj_id):
        self._searched_objs.add(obj_id)

    def check_if_obj_already_searched(self, obj_id):
        return obj_id in self._searched_objs
        ...
        if lot_decoration_service is not None:
            ...
            for sim_info in active_household.sim_infos:
                if not sim_info.is_human:
                    continue
                if self.tests_to_create_situation:
                    resolver = SingleSimResolver(sim_info)
                    if not self.tests_to_create_situation.run_tests(resolver):
                        continue
                sims_needing_situations.append(sim_info)
    ...
    def _create_holiday_situation(self, sim_info, holiday_goals):
        ...
        situation_id = services.get_zone_situation_manager().create_situation((HolidayTuning.HOLIDAY_SITUATION), guest_list=guest_list,
    ...
    def cleanup(self, from_service_stop=False):
        if self._holiday_alarm is not None:
            ...
            if self._state == HolidayState.PRE_DAY:
                ...
                household = services.household_manager().get(self._active_household_id)
                if household is not None:
                    if not household.holiday_tracker.is_holiday_cancelled(self.holiday_id):
        ...
        if specific_time is None:
            ...
            for drama_node in drama_scheduler_service.scheduled_nodes_gen():
                if drama_node.drama_node_type != DramaNodeType.HOLIDAY:
                    if drama_node.drama_node_type != DramaNodeType.PLAYER_PLANNED:
                        continue
    ...
    def _save_custom_data(self, writer):
        ...
        if self._searched_objs:
            writer.write_uint64s(HOLIDAY_HUNT_TOKEN, self._searched_objs)

    def _load_custom_data(self, reader):
        self._searched_objs = set(reader.read_uint64s(HOLIDAY_HUNT_TOKEN, ()))
        ...
        if self._holiday_start_time:
            if not self._holiday_end_time:
                if self._holiday_start_time + HolidayTuning.HOLIDAY_DURATION() < services.time_service().sim_now:
                    return False
        return True
