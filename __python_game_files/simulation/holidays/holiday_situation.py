...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\holidays\holiday_situation.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 17679 bytes
...
class HolidaySituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'holiday_situation_job_and_role_state':TunableSituationJobAndRoleState(description='\n            The Situation Job and Role State for the Sim celebrating the holiday.\n            ',
     ...
     '_progress_meter_settings':ValueBasedSituationMeterData.TunableFactory(description='\n            The meter used to track the holiday score. The min and max value\n            of this meter is locked to 0-100 as the score is reported to UI as\n            a percentage based on the number of traditions a Sim cares about.\n            ',
       tuning_group=GroupNames.SITUATION,
       locked_args={'_meter_id': 1})}
    REMOVE_INSTANCE_TUNABLES = ('main_goal', '_main_goal_visibility_test', 'minor_goal_chains',
                                'main_goal_audio_sting', 'goal_sub_text', 'goal_button_text',
                                'highlight_first_incomplete_minor_goal', 'screen_slam_gold',
                                'screen_slam_silver', 'screen_slam_bronze', 'screen_slam_no_medal',
                                'recommended_job_object_notification', 'recommended_job_object_text',
                                '_buff', 'targeted_situation', '_resident_job', '_relationship_between_job_members',
                                'force_invite_only') + Situation.SITUATION_START_FROM_UI_REMOVE_INSTANCE_TUNABLES
    ...
    def on_remove(self):
        ...
        if household is not None:
            if household.holiday_tracker is not None:
    ...
    def build_situation_start_message(self):
        ...
        build_icon_info_msg(IconInfoData(icon_resource=holiday_icon), display_name,
          (msg.icon_info),
          desc=(LocalizationHelperTuning.get_start_time_to_end_time(HolidayTuning.MAIN_HOLIDAY_START_TIME, end_time)))
...
lock_instance_tunables(HolidaySituation, exclusivity=(BouncerExclusivityCategory.NEUTRAL),
  creation_ui_option=(SituationCreationUIOption.NOT_AVAILABLE),
  duration=0,
  _implies_greeted_status=False,
  situation_display_type_override=(SituationDisplayType.SIM_SPECIFIC))