...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\drama_scheduler\festival_drama_node.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 31161 bytes
...
import alarms, elements, enum, interactions.context, services, sims4.resources
...
class FestivalDramaNode(BaseDramaNode):
    ...
    INSTANCE_TUNABLES = {'festival_open_street_director':TunableReference(description='\n            Reference to the open street director in question.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.OPEN_STREET_DIRECTOR)), 
     'street':TunableReference(description='\n            The street that this festival is allowed to run on.\n            ',
    ...
    def get_travel_lot_id(cls, reject_same_street=False):
        if reject_same_street:
            if cls.street is services.current_street():
                return
    ...
    def _create_open_street_director_request(self):
        ...
        request = OpenStreetDirectorRequest(festival_open_street_director, priority=(festival_open_street_director.priority),
          preroll_start_time=preroll_time,
          timeout=create_time_span(minutes=(self.delay_timeout)),
          timeout_callback=(self._request_timed_out_callback),
          premature_destruction_callback=(self._open_street_director_destroyed_early_callback))
        ...
        if cls.festival_dynamic_sign_info is None:
            ...
            if zone_protobuff is not None:
                ...
                if all((active_node.is_during_pre_festival() for active_node in running_nodes)):
                    ...
                    if any((active_node.is_on_festival_street() for active_node in running_nodes)):
                        festival_info.disabled_tooltip = ui_info.on_street_tooltip
                    else:
                        if active_sim_info.is_in_travel_group():
...
MajorOrganizationEventDisplayMixin = get_display_mixin(has_description=True, has_icon=True, has_tooltip=True, use_string_tokens=True,
...
class MajorOrganizationEventDramaNode(FestivalContestDramaNodeMixin, MajorOrganizationEventDisplayMixin, FestivalDramaNode):
    ...
    def _verify_tuning_callback(cls):
        if cls._display_data.instance_display_name is None:
            ...
            hasattr(cls.time_option, 'valid_time') or logger.error('Drama Node ({}) need a single time tuned in order to schedule,                          but does not. It will not schedule.', cls)
