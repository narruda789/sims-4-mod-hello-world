...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\world\travel_commands.py
# Compiled at: 2019-07-29 14:29:53
...
def get_sims_available_for_travel(opt_sim_id: OptionalTargetParam=None, opt_target_id: OptionalTargetParam=None, _connection=None):
    ...
    if actor_sim is not None:
        ...
        if not opt_sim_id:
            ...
            filtered_sims = services.sim_filter_service().submit_filter(sim_filter, None,
        ...
        if target_sim is not None:
            if target_sim is not actor_sim:
...
def get_default_selection_data(actor_sim, filter_list):
    ...
    for item in filter_list:
        ...
        if sim is not None:
            if gathering and club_service.sims_to_gatherings_map.get(sim) is gathering or ensembles:
                if any((sim in ensemble for ensemble in ensembles)):
                    default_selection_data.append((sim_info, True))
                    continue
