...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\world\spawn_actions.py
...
import objects, placement, services, sims4.log
...
class SpawnOnVehicleActionAffordance(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def list_pack_safe(list_pack_safe=False):
        ...
        if list_pack_safe:
            return {tuning_name: TunableReference(description=description, manager=(services.definition_manager()),
                            pack_safe=True)}
...
class TunableSpawnActionVariant(TunableVariant):
    ...
    def __init__(self, list_pack_safe=False, **kwargs):
        (super().__init__)(affordance=SpawnActionAffordance.TunableFactory(), 
         locked_args={'fade_in': SpawnActionFadeIn()}, 
         vehicle=SpawnOnVehicleActionAffordance.TunableFactory(list_pack_safe=list_pack_safe), 
         default='fade_in', **kwargs)