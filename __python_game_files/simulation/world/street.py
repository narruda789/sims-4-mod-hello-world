...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\world\street.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 12495 bytes
...
import sims4.log, sims4.resources, world
...
class Street(HasTunableReference, WeatherTuningMixin, SeasonalParametersMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.STREET)):
    ...
    def get_lot_to_travel_to(cls):
        if cls is services.current_street():
            return
        return world.get_lot_id_from_instance_id(cls.travel_lot)
...
def get_vacation_zone_id(zone_id):
    ...
    if street is None or street.vacation_lot is None:
        return zone_id
    lot_id = world.get_lot_id_from_instance_id(street.vacation_lot)
    if lot_id is None:
        return zone_id
    persistence_service = services.get_persistence_service()
    vacation_zone_id = persistence_service.resolve_lot_id_into_zone_id(lot_id,
