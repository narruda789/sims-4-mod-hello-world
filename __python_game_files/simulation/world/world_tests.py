...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\world\world_tests.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 44033 bytes
...
class LocationTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def location_tests(is_outside=True, is_inside_building=True, is_natural_ground=True, is_in_slot=True, is_venue_type=True, is_on_active_lot=True, in_common_area=True, is_fire_allowed=True, is_on_level=True, has_terrain_tag=True, valid_surface_types=True, is_in_pond=True):
        ...
        return TunableTuple(is_outside=OptionalTunable(description='\n                PLEASE BE AWARE. If checked, will verify if the subject of the\n                test does not have a roof over their head. If unchecked, will \n                test if the subject has a roof over their head. If the \n                intention is to determine if a subject is in/not in a building, \n                this test is deprecated in favor of is_inside_building. If you \n                would like to test if a subject is inside a building, but also \n                need to test if they have a roof over their head, please use \n                both options.\n                ',
          ...
          any_slot=(SlotTestType.TunableFactory()),
          surface_slot=(SurfaceSlotTest.TunableFactory()),
          specific_slot=(SpecificSlotTest.TunableFactory()),
          default='any_slot'))),
          is_venue_type=OptionalTunable(description='\n                If checked, will verify if the subject is at a venue of the\n                specified type.\n                ',
          disabled_name="Don't_Test",
    ...
    def __call__(self, test_target=None):
        for target in test_target:
            if isinstance(target, sims.sim_info.SimInfo):
                ...
                if target is None:
                ...
                elif self.location_tests.is_outside is not None:
                    ...
                    elif self.location_tests.is_inside_building is not None:
                    ...
                    if self.location_tests.is_natural_ground is not None:
                        if self.location_tests.is_natural_ground != target.is_on_natural_ground():
                ...
                if self.location_tests.is_venue_type.negate:
                    ...
                    if not (required_venue_tuning is None or isinstance(venue, self.location_tests.is_venue_type.venue_type)):
                        return TestResult(False, 'Object failed venue type test.', tooltip=(self.tooltip))
                if self.location_tests.is_on_active_lot is not None:
                    if not self.test_is_on_active_lot(self.location_tests.is_on_active_lot, target.position):
                ...
                if self.location_tests.has_terrain_tag is not None:
                    ...
                    is_terrain_tag_at_position = terrain.is_terrain_tag_at_position((position.x), (position.z), (self.location_tests.has_terrain_tag.terrain_tags),
                      level=(target.routing_surface.secondary_id),
                      test_floor_tiles=(self.location_tests.has_terrain_tag.test_floor_tiles))
                ...
                if self.location_tests.valid_surface_types is not None:
                    return target.routing_surface is None or self.location_tests.valid_surface_types.test_item(target.routing_surface.type) or TestResult(False, 'Object routing surface is incorrect.', tooltip=(self.tooltip))
...
class SlotTestType(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def run_slot_test(self, target):
        if target.parent is not None:
            return (
             self.require_slotted, 'Object failed slot test. Is in a slot.')
        return (
         not self.require_slotted, 'Object failed slot test. Is not in a slot.')
        ...
        if target.parent is not None:
            for slot in target.parent_slot.slot_types:
                if slot.implies_owner_object_is_surface:
                    return (
                     self.require_slotted, 'Object failed surface slot test. Slot is a surface.')

        return (
         not self.require_slotted, 'Object failed surface slot test. Surface slot not found.')
            ...
            for specified_slot in self.specific_slot_set.slot_types:
                for parent_slot in target.parent_slot.slot_types:
                    if parent_slot is specified_slot:
                        return (
                         self.require_slotted, 'Object failed specified slot test. Specified slot was found.')

        return (
         not self.require_slotted, 'Object failed specified slot test. Specified slot was not found.')
...
class DistanceTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def __call__(self, subjects=(), targets=()):
        ...
        for subject in subjects:
            ...
            for target in self.get_matching_objects_gen() if self.target.test_type == DistanceTest.TYPE_TAGS else targets:
                ...
                elif subject is None or target is None:
                ...
                if self.distance_threshold.compare(distance):
                    ...
                    if self.object_count_threshold is not None:
                        if self.object_count_threshold.compare(match_count):
                            return TestResult.TRUE
    ...
    def validate_tuning_for_objective(self, objective):
        ...
        if self.object_count_threshold is not None:
            if self.object_count_threshold.value == 0:
...
class VenueAvailabilityTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    FACTORY_TUNABLES = {'subject':TunableEnumEntry(description='\n            Who cares about the venues in the region. This paricipant will be\n            used to test for Region Compatibility if checked.\n            ',
     ...
     'ownership_test':OptionalTunable(description='\n            If enabled then when testing venue availability it will test the\n            ownership of the venue as tuned within.\n            \n            If disabled then no ownership test will be applied to the venue.\n            ',
       ...
       locked_args={'test_type': TEST_ANY_OWNERSHIP}),
       fail_if_owned_by_subject=TunableTuple(description='\n                    venues that are owned/lived in by the tuned subject will be \n                    excluded and considered unavailable.\n                    ',
       locked_args={'test_type': TEST_SUBJECT_OWNERSHIP}),
       default='fail_if_owned_by_anyone'),
    ...
    def __call__(self, subjects):
        ...
        if self.test_region_compatibility:
            if current_region is None:
                logger.error('VenueAvailabilityTest: Participant Type {} is not in a zone. Do you actually want to test region compatibility?', (self.subject),
                  owner='rmccord')
            ...
            if self.ownership_test:
                ...
                if travel_group is not None:
                    if travel_group.played:
                        if self.ownership_test.test_type is self.TEST_ANY_OWNERSHIP or set(subjects).intersection(set(travel_group.sim_info_gen())):
                            continue
                lot_data = None
    ...
    def __call__(self, test_targets):
        ...
        for participant in test_targets:
            ...
            if home_world_id != world_id:
                if self.negate:
                    return TestResult.TRUE
                return TestResult(False, 'Home world does not match the current world.',
    ...
    def __call__(self, test_targets):
        ...
        for participant in test_targets:
            ...
            if home_region is not current_region:
                if self.negate:
                    return TestResult.TRUE
                return TestResult(False, 'Home region does not match the current region.',
                  tooltip=(self.tooltip))
