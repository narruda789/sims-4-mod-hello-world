...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\world\mailbox_owner_helper.py
# Compiled at: 2020-04-29 19:42:12
...
import routing, services, sims4, tag
...
class ObjectConnectivityHandle(Handle):
    ...
    def __init__(self, obj, position):
        super().__init__(routing.Location(position, orientation=(obj.orientation),
          routing_surface=(obj.routing_surface)), obj.routing_surface)
        self.obj = obj
        self.assigned = False
...
class MailboxOwnerHelper:
    ...
    OWNER_STATES = TunableList(description='\n        A list of items and states used for setting ownership states\n        of plex mailboxes.\n        \n        This is contained within a list to protect against pack safe references.\n        ',
      ...
      tunable=TunableReference((services.definition_manager()),
      pack_safe=True)),
      default_state_value=TunableStateValueReference(description='\n                Default state of mailbox, used if not owned by active household.\n                ',
      pack_safe=True),
      active_state_value=TunableStateValueReference(description='\n                State to use when owned by active household.\n                ',
      pack_safe=True)))
    ...
    def assign_mailbox_owners(self):
        ...
        if not plex_service.is_active_zone_a_plex():
            ...
            return plex_door_infos or None
        ...
        for estimated_distance in routes:
            ...
            if household_id != 0:
                if household_id not in mapped_household_ids:
