...
class NeighborChatSituation(SituationComplexCommon):
    ...
    def _on_set_sim_job(self, sim, job_type):
        ...
        if job_type is self.neighbor_job_and_role_state.job:
            ...
            if job_type is self.knocker_job_and_role_state.job:
                self._knocker_sim = sim
            elif self._neighbor_sim is not None and self._knocker_sim is not None:
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.neighbor_job_and_role_state.job, cls.neighbor_job_and_role_state.role_state),
         (
          cls.knocker_job_and_role_state.job, cls.knocker_job_and_role_state.role_state)]
...
lock_instance_tunables(NeighborChatSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
