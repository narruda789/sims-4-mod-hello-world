...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\apartments\landlord_situation.py
# Compiled at: 2016-08-30 13:26:43
# Size of source mod 2**32: 4028 bytes
...
class LandlordSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'fix_problems_state':_FixProblemsState.TunableFactory(description='\n            Situation State for the Landlord to fix Apartment Problems.\n            ',
       ...
       display_name='01_fix_problems_situation_state'), 
     'landlord_situation_job_and_role_state':TunableSituationJobAndRoleState(description='\n            The Situation Job and Role State for the Landlord.\n            ')}
    ...
    def get_predefined_guest_list(cls):
        ...
        guest_list.add_guest_info(SituationGuestInfo((landlord_sim_info.sim_id), (cls.landlord_situation_job_and_role_state.job),
...
lock_instance_tunables(LandlordSituation, exclusivity=(BouncerExclusivityCategory.VISIT),
...
class LandlordWalkbyRingDoorBellSituation(WalkbyRingDoorBellSituation):
    ...
    def get_predefined_guest_list(cls):
        guest_list = SituationGuestList(invite_only=True)
        landlord_sim_info = services.get_landlord_service().get_landlord_sim_info()
        guest_list.add_guest_info(SituationGuestInfo((landlord_sim_info.sim_id), (cls.walker_job.situation_job),
          (RequestSpawningOption.DONT_CARE),
          (BouncerRequestPriority.EVENT_VIP),
          expectation_preference=True))
        return guest_list