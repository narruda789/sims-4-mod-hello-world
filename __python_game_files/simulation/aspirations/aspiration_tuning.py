...
# Size of source mod 2**32: 35313 bytes
...
class Aspiration(AspirationBasic):
    ...
    def is_valid_for_sim(cls, sim_info):
        if sim_info.is_young_adult_or_older:
            return not cls.is_child_aspiration and not cls.is_teen_aspiration
        if sim_info.is_teen:
            return not cls.is_child_aspiration
        if sim_info.is_child:
            return cls.is_child_aspiration
        return False
