...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\aspirations\timed_aspiration.py
...
TimedAspirationDefinitionDisplayMixin = get_display_mixin(has_description=True, has_icon=True, has_tooltip=True, use_string_tokens=True,
...
class TimedAspiration(TimedAspirationDefinitionDisplayMixin, AspirationBasic):
    INSTANCE_TUNABLES = {'duration':TunableVariant(weekly_schedule=WeeklySchedule.TunableFactory(description='\n                Determines days of the week when the aspiration will deactivate.\n                '),
     ...
     'on_failure_loot_actions':TunableList(description='\n           List of loots operations that will be awarded when \n           this aspiration fails.\n           ',
       tunable=LootActions.TunableReference()), 
     'on_cancel_loot_actions':TunableList(description='\n           List of loots operations that will be awarded when \n           this aspiration is cancelled.\n           ',
       tunable=LootActions.TunableReference()), 
     'warning_buff':OptionalTunable(description='\n            If enabled, the buff is given to the Sim as a warning the aspiration\n            duration is ending.\n            ',
...
class TimedAspirationData:
    ...
    def load(self, msg):
        ...
        self._end_alarm_handle = alarms.add_alarm(self, time_till_end,
          (self._aspiration_timed_out),
          cross_zone=True)
        time_till_warning = time_till_end - create_time_span(days=1)
        if time_till_warning > TimeSpan.ZERO:
            self._warning_alarm_handle = alarms.add_alarm(self, time_till_warning,
              (self._give_aspiration_warning),
              cross_zone=True)
        return True
