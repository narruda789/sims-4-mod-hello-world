...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\balloon\balloon_icon.py
...
class BalloonIcon(HasTunableFactory, AutoFactoryInit):
    ...
    def get_balloon_icons(self, resolver, balloon_type=DEFAULT, gsi_entries=None, gsi_category=None, gsi_interaction=None, gsi_balloon_target_override=None, gsi_test_result=None):
        ...
        if gsi_entries is not None:
            ...
            gsi_entries.append({'test_result':str(gsi_test_result), 
             'balloon_type':str(self.balloon_type), 
             'weight':self.weight, 
             'icon':str(self.icon(gsi_interaction, balloon_target_override=gsi_balloon_target_override)), 
             'balloon_category':self.gsi_category})
