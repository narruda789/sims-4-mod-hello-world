...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\server\live_drag_tuning.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 5916 bytes
...
import enum, event_testing, objects.object_tests
...
class TunableLiveDragTestVariant(TunableVariant):
    ...
    def __init__(self, description='A single tunable test for Live Dragged objects and their potential targets.', **kwargs):
        (super().__init__)(description=description, 
         state=event_testing.state_tests.TunableStateTest(locked_args={'tooltip': None}), 
         statistic=event_testing.statistic_tests.StatThresholdTest.TunableFactory(locked_args={'tooltip': None}), 
         object_has_no_children=objects.object_tests.ObjectHasNoChildrenTest.TunableFactory(locked_args={'tooltip': None}), **kwargs)
...
class LiveDragLootActions(LootActions):
    INSTANCE_TUNABLES = {'loot_actions': TunableList(TunableVariant(statistics=TunableStatisticChange(locked_args={'advertise':False, 
