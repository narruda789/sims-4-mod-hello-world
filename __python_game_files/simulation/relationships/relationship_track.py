...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\relationships\relationship_track.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 52102 bytes
...
import alarms, date_and_time, event_testing.tests, services, sims, sims4.log, sims4.resources, sims4.tuning
...
class BaseRelationshipTrack:
    ...
    def set_time_until_decay_begins(self, ticks_until_decay_begins):
        if self.delay_until_decay_is_applied is None:
            ...
            if self._cached_ticks_until_decay_begins != 0.0:
...
class RelationshipTrack(BaseRelationshipTrack, TunedContinuousStatistic, HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.STATISTIC)):
    ...
    INSTANCE_TUNABLES = {'relationship_obj_prefence_curve':TunableWeightedUtilityCurveAndWeight(description="\n            This curve lets you modify autonomous desire to interact with an \n            object if you have a relationship of this type with the object's\n            crafter.\n            ",
     ...
     'species_requirements':OptionalTunable(description='\n            If enabled then this relationship track will have species\n            requirements if it is attempting to be given to a pair of Sims.\n            ',
       ...
       invalid_enums=(
      Species.INVALID,))),
       species_list_two=TunableSet(description='\n                    A set of species that one of the Sims must have to be\n                    given this relationship track.\n                    ',
       tunable=TunableEnumEntry(description='\n                        A species that one of the Sims must have to be given\n                        this relationship track.\n                        ',
       tunable_type=Species,
       default=(Species.HUMAN),
       invalid_enums=(
    ...
    @classproperty
    def track_type(cls):
        return RelationshipTrackType.RELATIONSHIP
    ...
    def on_remove(self, on_destroy=False):
        ...
        self._cleanup_convergence_callback()
    ...
    def set_value(self, value, *args, headline_icon_modifier=None, **kwargs):
        ...
        if sim_info_a is not None:
            if sim_info_b is not None:
                ...
                event_manager.process_event((event_testing.test_events.TestEvent.PrerelationshipChanged), sim_info=sim_info_a,
                  sim_id=(sim_info_a.sim_id),
                  target_sim_id=(sim_info_b.sim_id))
                event_manager.process_event((event_testing.test_events.TestEvent.PrerelationshipChanged), sim_info=sim_info_b,
                  sim_id=(sim_info_b.sim_id),
                  target_sim_id=(sim_info_a.sim_id))
        ...
        if sim_info_a is not None:
            if sim_info_b is not None:
                event_manager.process_event((event_testing.test_events.TestEvent.RelationshipChanged), sim_info=sim_info_a,
                  sim_id=(sim_info_a.sim_id),
                  target_sim_id=(sim_info_b.sim_id))
                event_manager.process_event((event_testing.test_events.TestEvent.RelationshipChanged), sim_info=sim_info_b,
                  sim_id=(sim_info_b.sim_id),
                  target_sim_id=(sim_info_a.sim_id))
    ...
    @staticmethod
    def check_relationship_track_display_priorities--- This code section failed: ---

 L. 852         0  LOAD_CONST               None
                2  RETURN_VALUE     

 L. 857         4  FOR_ITER             64  'to 64'
                6  STORE_FAST               'statistic'

 L. 859         8  LOAD_GLOBAL              issubclass
               10  LOAD_FAST                'statistic'
               12  LOAD_GLOBAL              RelationshipTrack
               14  CALL_FUNCTION_2       2  '2 positional arguments'
               16  POP_JUMP_IF_TRUE     20  'to 20'

 L. 860        18  CONTINUE              4  'to 4'
             20_0  COME_FROM            16  '16'

 L. 863        20  LOAD_FAST                'statistic'
               22  LOAD_ATTR                track_type
               24  LOAD_GLOBAL              RelationshipTrackType
               26  LOAD_ATTR                SENTIMENT
               28  COMPARE_OP               ==
               30  POP_JUMP_IF_FALSE    34  'to 34'

 L. 864        32  CONTINUE              4  'to 4'
             34_0  COME_FROM            30  '30'

 L. 868        34  LOAD_FAST                'statistic'
               36  LOAD_ATTR                display_priority
               38  LOAD_CONST               0
               40  COMPARE_OP               ==
               42  POP_JUMP_IF_FALSE    46  'to 46'

 L. 869        44  CONTINUE              4  'to 4'
             46_0  COME_FROM            42  '42'

 L. 871        46  LOAD_FAST                'relationship_track_display_priority'
               48  LOAD_FAST                'statistic'
               50  LOAD_ATTR                display_priority
               52  BINARY_SUBSCR    
               54  LOAD_METHOD              append
               56  LOAD_FAST                'statistic'
               58  CALL_METHOD_1         1  '1 positional argument'
               60  POP_TOP          
               62  JUMP_BACK             4  'to 4'
               64  POP_BLOCK        

 L. 875        66  SETUP_LOOP          110  'to 110'
               68  LOAD_FAST                'relationship_track_display_priority'
               70  LOAD_METHOD              values
               72  CALL_METHOD_0         0  '0 positional arguments'
               74  GET_ITER         
               76  FOR_ITER            108  'to 108'
               78  STORE_FAST               'relationship_priority_level'

 L. 876        80  LOAD_GLOBAL              len
               82  LOAD_FAST                'relationship_priority_level'
               84  CALL_FUNCTION_1       1  '1 positional argument'
               86  LOAD_CONST               1
               88  COMPARE_OP               <=
               90  POP_JUMP_IF_FALSE    94  'to 94'

 L. 877        92  CONTINUE             76  'to 76'
             94_0  COME_FROM            90  '90'

 L. 881        94  LOAD_GLOBAL              logger
               96  LOAD_METHOD              error
               98  LOAD_STR                 'Multiple Relationship Tracks have the same display priority: {}'
              100  LOAD_FAST                'relationship_priority_level'
              102  CALL_METHOD_2         2  '2 positional arguments'
              104  POP_TOP          
              106  JUMP_BACK            76  'to 76'
              108  POP_BLOCK        
            110_0  COME_FROM_LOOP       66  '66'
...
Parse error at or near `FOR_ITER' instruction at offset 4
    ...
    def _decay_alarm_callback(self, handle):
        ...
        if sim_info_a is not None:
            if sim_info_b is not None:
                if sim_info_a.is_selectable or sim_info_b.is_selectable:
                    self.tracker.rel_data.relationship.send_relationship_info()
...
class ShortTermContextRelationshipTrack(RelationshipTrack):
    ...
    def on_add(self):
        ...
        if sim_a is not None:
            if sim_b is not None:
                if sim_a.is_in_group_with(sim_b):
    ...
    def remove_social_group_decay(self):
        ...
        elif self.decay_to_initial_modifier and self.tested_initial_modifier is not None:
            sim_info_a = self.tracker.rel_data.relationship.find_sim_info_a()
            sim_info_b = self.tracker.rel_data.relationship.find_sim_info_b()
            if sim_info_a is None or sim_info_b is None:
                return
...
class ObjectRelationshipTrack(BaseRelationshipTrack, TunedContinuousStatistic, HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.STATISTIC)):
    OBJECT_BASED_FRIENDSHIP_TRACKS = TunableMapping(description='\n        A mapping of sets of objects with a specific tag to a friendship track.\n        Any value added to the tuned track will apply to all objects within the set.\n        ',
      ...
      pack_safe=True),
      tuple_name='ObjectBasedFriendshipTrackTuple',
      export_modes=(ExportModes.All))
    ...
    def set_name_override(self, name_override_obj):
        self.tracker.rel_data.relationship.send_relationship_info(name_override_obj=name_override_obj)
