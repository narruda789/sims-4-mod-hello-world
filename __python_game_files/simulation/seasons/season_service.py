...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\seasons\season_service.py
# Compiled at: 2020-03-06 15:30:47
# Size of source mod 2**32: 45450 bytes
...
import build_buy, date_and_time, elements, persistence_error_types, services, sims4.log
logger = sims4.log.Logger('seasons', default_owner='jdimailig')

def get_street_or_region_with_seasonal_parameters():
    street_instance = services.current_street()
    if street_instance is not None:
        if street_instance.seasonal_parameters:
...
class SeasonService(Service):
    ...
    def get_seasons(self, num_seasons):
        ...
        if num_seasons < 1:
            return season_data
        season, content = self._season, self._season_content
    ...
    def _add_secondary_forward_wraparound_interp(self, seasonal_parameter, start_time, start_value, end_time, end_value, force_loop, sync_end_time=None):
        ...
        if target_value is not None:
            ...
            self._regional_seasonal_parameters_handles[seasonal_parameter] = self._season_timeline.schedule(build_element((
             lambda _: _regional_set_season_interp(),)), new_end_time)
        ...
        return (
         start_value, new_end_value, new_end_time)

    def set_season(self, season_type, source, interp_time=None):
        if self._season == season_type:
            return
        previous, next_season = self._season, season_type
        if interp_time is None:
            ...
            street_or_region = get_street_or_region_with_seasonal_parameters()
            if street_or_region is not None:
                for seasonal_parameter in street_or_region.seasonal_parameters.keys():
                    start_value, _, _, _ = self._get_regional_season_change_values(seasonal_parameter, start_time, street_or_region)
            ...
            def _send_update():
                ...
                for seasonal_parameter in SeasonParameters:
                    ...
                    new_end_time = end_time
                    end_value, _, _, _ = self._get_regional_season_change_values(seasonal_parameter, end_time, street_or_region)
                    if seasonal_parameter == SeasonParameters.FOLIAGE_REDUCTION:
                        ...
                        elif seasonal_parameter == SeasonParameters.FOLIAGE_COLORSHIFT and SeasonParameters.FOLIAGE_REDUCTION in param_to_start_value:
                            start_value, end_value, new_end_time = self._add_secondary_forward_wraparound_interp(seasonal_parameter, start_time,
                              start_value,
                              end_time,
                              end_value,
                              force_loop,
                ...
                if self._season_change_handler is not None:
                    if not self._season_change_handler.is_active:
                        self._season_change_handler.trigger_hard_stop()
                self._season_change_handler = self._season_timeline.schedule(build_element((
                 lambda _: self._handle_season_content_delayed(trigger_weather=True),)), end_time)

            self._season_change_handler = self._season_timeline.schedule(build_element((
             lambda _: _send_update(),)), start_time)
        self._schedule_season_aware_object_updates()
        services.get_event_manager().process_event((TestEvent.SeasonChangedNoSim), previous_season=previous,
          current_season=next_season)
        services.get_event_manager().process_events_for_household((TestEvent.SeasonChanged), (services.active_household()),
    ...
    def _schedule_season_change(self):
        if self._season_change_handler is not None:
            if not self._season_change_handler.is_active:
                self._season_change_handler.trigger_hard_stop()
        self._season_change_handler = self._season_timeline.schedule(build_element((
         lambda _: self.advance_season(SeasonSetSource.PROGRESSION),)), self._season_content.end_time)
    ...
    def _schedule_season_screen_slam(self):
        ...
        if scheduled_time is None:
            return
        self._season_screen_slam_handler = self._season_timeline.schedule(build_element((
         lambda _: self._send_screen_slam_message(),)), scheduled_time)
    ...
    def _schedule_mid_season_interpolation_update(self):
        if self._client_interpolation_handler is not None:
            self._client_interpolation_handler.trigger_hard_stop()
        self._client_interpolation_handler = self._season_timeline.schedule(build_element((
         lambda _: self._send_interpolation_update(mid_season_op=True),)), self._season_content.midpoint_time)
    ...
    def _get_regional_season_change_values(self, seasonal_parameter, time, street_or_region):
        seasons = self.get_four_seasons()
        lowest_time = None
        next_index = 0
        index = 0
        next_season_to_use = None
        changes = street_or_region.seasonal_parameters.get(seasonal_parameter)
        for frame in changes:
            ...
            if not lowest_time is None:
                if time_till_frame < lowest_time:
    ...
    def _process_regional_season_change_update(self, seasonal_parameter):
        ...
        if current_index is None:
            current_value, next_frame, end_time, next_index = self._get_regional_season_change_values(seasonal_parameter, start_time, street_or_region)
        else:
            changes = street_or_region.seasonal_parameters.get(seasonal_parameter)
        ...
        self._regional_seasonal_parameters_handles[seasonal_parameter] = self._season_timeline.schedule(build_element((
         lambda _: self._process_regional_season_change_update(seasonal_parameter),)), end_time)
