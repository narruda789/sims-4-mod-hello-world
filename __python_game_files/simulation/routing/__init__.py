...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\__init__.py
# Compiled at: 2021-02-11 18:58:23
# Size of source mod 2**32: 47542 bytes
from _math import Quaternion, Vector3, Transform
from enum_lib import Enum
import weakref, enum, sims4.reload
from objects.proxy import ProxyObject
import build_buy, placement, services
...
try:
    ...
    object_routing_surfaces = (
     SurfaceType.SURFACETYPE_OBJECT, SurfaceType.SURFACETYPE_POOL)
...
def get_routing_surface_asm_params(initial_routing_surface, target_routing_surface):
    return {('routingSurfaceTypeFrom', 'x'):get_animation_routing_surface_type_param(initial_routing_surface.type), 
     ('routingSurfaceTypeTo', 'x'):get_animation_routing_surface_type_param(target_routing_surface.type)}
...
class Goal(Destination):
    __slots__ = ('requires_los_check', 'path_id', 'connectivity_handle', 'path_cost',
                 'failure_reason', 'height_clearance')
...
class Path:
    ...
    def get_final_indoor_level_change_node(self):
        ...
        for node in self:
            ...
            location = (prev_level_index is not None and prev_level_index is not curr_level_index)(Location(*Vector3, **node.position), Quaternion(*node.orientation), node.routing_surface_id)
    ...
    def is_route_fail(self):
        if not self.nodes:
            ...
            return self.nodes.plan_success or True
        return False
    ...
    def remove_fake_portals(self):
        for node in self:
            if node.action == PathNodeAction.PATH_NODE_PORTAL_ANIMATE_ACTION:
                node.action = node.portal_id or node.portal_object_id or PathNodeAction.PATH_NODE_PORTAL_WALK_ACTION
