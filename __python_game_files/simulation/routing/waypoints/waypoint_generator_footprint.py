...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\waypoints\waypoint_generator_footprint.py
# Compiled at: 2019-04-12 17:15:56
# Size of source mod 2**32: 8260 bytes
import math
...
class _WaypointGeneratorFootprint(_WaypointGeneratorBase):
    ...
    def get_constraint_for_waypoint(self, x, z):
        ...
        return SmallAreaConstraint(geometry=geometry, debug_name='RoundedRectanglePoint',
    ...
    def get_start_constraint(self):
        ...
        if self._target.footprint_polygon.contains(sim.position):
            if sim.posture.posture_type in supported_postures:
                if sim.level == self._target.level:
                    return constraint
        ...
        return constraint.intersect(Constraint(geometry=geometry, debug_name='FootprintConstraint',
          routing_surface=(self._routing_surface)))
