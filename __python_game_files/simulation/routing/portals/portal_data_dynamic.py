...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\portals\portal_data_dynamic.py
# Compiled at: 2018-09-12 15:19:20
# Size of source mod 2**32: 10372 bytes
...
class _PortalTypeDataDynamic(_PortalTypeDataVariableJump):
    ...
    def _update_portal_location(self, locations, there_entry, there_exit, obj, other_obj, other_obj_angle_restriction):
        if there_entry.position == there_exit.position:
            ...
            if self.angle_restriction is not None:
                if not self._is_angle_valid(entry_orientation, obj, self.angle_restriction):
                    return
            if other_obj_angle_restriction is not None:
                return self._is_angle_valid(exit_orientation, other_obj, other_obj_angle_restriction) or None
    ...
    def get_portal_locations(self, obj):
        ...
        for portal_entry in self.object_portals:
            ...
            try:
                object_surfaces = services.sim_quadtree().query(bounds=bounds, surface_id=(there_entry.routing_surface), filter=(ItemType.ROUTABLE_OBJECT_SURFACE),
