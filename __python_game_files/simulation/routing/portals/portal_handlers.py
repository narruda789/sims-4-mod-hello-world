...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\portals\portal_handlers.py
...
with portal_schema.add_view_cheat('objects.focus_camera_on_object', label='Focus On Object') as (cheat):
...
with portal_schema.add_view_cheat('camera.focus_on_position', label='Focus On Portal Pair') as (cheat):
    cheat.add_token_param('object_id')
...
def generate_portal_data(zone_id: int=None):
    ...
    for portal in obj_manager.portal_cache_gen():
        ...
        for portal_instance in portal_component.get_portal_instances():
            ...
            instance_data.append({'object_id':str(portal.id),  'portal_tuning':str(portal_instance.portal_template.traversal_type), 
             'required_flags':PortalFlags(required_flags).name if required_flags else 'None', 
             'discouragement_flags':str(portal_instance.portal_template.discouragement_flags), 
             'cost_override':str(portal_instance._cost_override), 
             'cost_override_map':str(dict(portal_instance._cost_override_map.items()) if portal_instance._cost_override_map is not None else {}), 
             'there_id':portal_instance.there, 
             'there_entry_location':there_entry_location, 
             'there_exit_location':there_exit_location, 
             'there_cost':portal_instance.there_cost, 
             'back_id':portal_instance.back, 
             'back_entry_location':back_entry_location, 
             'back_exit_location':back_exit_location, 
             'back_cost':portal_instance.back_cost})
        ...
        portals.append({'object_id':str(portal.id),  'portal_provider':portal.__class__.__name__, 
         'definition':portal.definition.name, 
         'loc_x':round(portal_pos.x, 3), 
         'loc_y':round(portal_pos.y, 3), 
         'loc_z':round(portal_pos.z, 3), 
         'on_active_lot':portal.is_on_active_lot(), 
         'num_sims_cost_override':num_sims_cost_override, 
         'Data':portal_data_items, 
