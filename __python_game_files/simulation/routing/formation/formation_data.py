...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\formation\formation_data.py
...
import services, sims4.log, snippets
logger = sims4.log.Logger('RoutingFormations', default_owner='rmccord')

class RoutingFormation(HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SNIPPET)):
    INSTANCE_TUNABLES = {'formation_behavior':RoutingFormationBehavior.TunableFactory(), 
     'formation_routing_type':TunableVariant(description='\n            The purpose of the routing formation which governs how the slave\n            behaves on routes.\n            ',
    ...
    def required_interaction(self):
        if self.require_interaction_compatibility:
            if self.master is not None:
                if self.master.routing_component.is_moving:
                    return self._interaction
    ...
    def on_add(self):
        ...
        services.get_event_manager().process_event((TestEvent.RoutingFormationStarted), slave=(self.slave),
          master=(self.master))
...
class RoutingFormationList(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'formations': TunableList(description="\n            A list of routing formations. One will be chosen based on the number of members of the group.\n            It will use the closest match based on the number of offsets in the formation.\n            Note that you will generally always want to use the 'follow' formation type here.\n            ",
                     tunable=(RoutingFormation.TunableReference()),
                     minlength=1)}
    ...
    def choose_formation_based_on_group_size(self, desired_size):
        ...
        for formation in self.formations:
            ...
            if formation_size == desired_size:
                ...
                if not size_delta >= 0 or closest_size_delta is None or size_delta < closest_size_delta:
