...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\formation\formation_type_follow.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 29833 bytes
import enum, placement, services, sims4.log, sims4.math
...
class _RoutingFormationAttachmentNode:
    __slots__ = ('_parent_offset', '_offset', '_radius', '_angle_constraint', '_flags',
                 '_type', '_noise_behavior', '_spring_behavior')
...
class FormationTypeFollow(FormationTypeBase):
    ...
    def get_routing_slave_constraint(self):
        if not (self._slave_constraint is None or self._slave_constraint.valid):
            self._build_routing_slave_constraint()
    ...
    def find_good_location_for_slave(self, master_location):
        ...
        if routing_surface is None:
            ...
            elif self.slave.is_sim or isinstance(self.slave, StubActor):
            ...
            min_water_depth, max_water_depth = OceanTuning.make_depth_bounds_safe_for_surface_and_sim(routing_surface, (self.master),
            ...
            slave_routing_component = self.slave.routing_component
            if slave_routing_component is not None:
                slave_routing_context = slave_routing_component.routing_context
                if max_water_depth is None:
                    fgl_kwargs.update({'max_pond_water_depth': slave_routing_context.max_allowed_wading_depth})
        else:
            fgl_kwargs.update({'max_pond_water_depth': min(max_water_depth, slave_routing_context.max_allowed_wading_depth)})
        ...
        if fgl_tuning.slave_should_face_master:
            ...
            fgl_kwargs.update({'raytest_radius':self.RAYTRACE_RADIUS,  'raytest_start_offset':self.RAYTRACE_HEIGHT, 
             'raytest_end_offset':self.RAYTRACE_HEIGHT, 
             'ignored_object_ids':{
              self.master.id, self.slave.id}, 
             'raytest_start_point_override':master_location.transform.translation})
        ...
        starting_location = placement.create_starting_location(position=slave_position, orientation=orientation,
        ...
        if self.slave.is_sim:
            ...
            fgl_context = FindGoodLocationContext(starting_location, object_id=self.slave.id, 
             object_footprints=(footprint,) if footprint is not None else None, 
             search_flags=fgl_flags, 
             restrictions=restrictions, 
             connectivity_group_override_point=master_position, **fgl_kwargs)
    ...
    def update_slave_position(self, master_transform, master_orientation, routing_surface, distribute=True, path=None, canceled=False):
        ...
        if distribute:
            slave_transform = canceled or (self._final_transform if self._final_transform is not None else self.slave.transform)
        ...
        if path is not None:
            if path.length() < self.route_length_minimum:
                if slave_route_distance_sqrd < self.route_length_minimum * self.route_length_minimum:
                    return
        ...
        if distribute:
            slave_too_far_from_master or self._slave.move_to(routing_surface=routing_surface, transform=slave_transform)
        else:
