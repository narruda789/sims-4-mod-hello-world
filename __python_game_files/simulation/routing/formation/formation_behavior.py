...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\formation\formation_behavior.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 13145 bytes
...
class TunableBehaviorActionVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, run_interaction=_BehaviorActionRunInteraction.TunableFactory(), 
         default='run_interaction', **kwargs)
...
class _BehaviorTrigger(HasTunableFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'action':TunableBehaviorActionVariant(description='\n            The action we want to execute.\n            '), 
     'delay':OptionalTunable(description='\n            If enabled, force a delay before executing the action.\n            ',
       tunable=TunableSimMinute(default=4,
       minimum=0))}
...
class TunableBehaviorTriggerVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, route_stop=_BehaviorTriggerRouteEnd.TunableFactory(), 
         interaction_start=_BehaviorTriggerInteractionStart.TunableFactory(), 
         default='route_stop', **kwargs)
    ...
    def stop_actions(self, *_, from_release=False, path=None, **__):
        if path is not None:
            ...
            if slave_data is not None:
                if path.length() < slave_data.route_length_minimum:
                    return
