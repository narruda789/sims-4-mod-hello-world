...
# Compiled at: 2018-07-27 18:22:13
# Size of source mod 2**32: 12214 bytes
...
class LaundryService(RegisterTestEventMixin, Service):
    ...
    def _add_clothing_pile_to_hamper(self, sim):
        if not self._hampers:
            ...
            return LaundryTuning.PUT_CLOTHING_PILE_ON_HAMPER.tests.run_tests(sim_resolver) or False
    ...
    def on_spin_outfit_change(self, sim, outfit_category_and_index, interaction):
        if not self._affected_household is None:
            if not self.hero_object_exist:
                return
            ...
            if current_outfit not in no_pile_outfits:
                if target_outfit not in no_pile_outfits:
                    if sim.on_home_lot:
                        if current_outfit not in naked_outfits:
                            self._add_clothing_pile_to_hamper(sim) or self._generate_clothing_pile(interaction)
