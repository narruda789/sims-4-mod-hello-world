...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\master_controller.py
# Compiled at: 2020-10-08 13:36:27
# Size of source mod 2**32: 35863 bytes
...
import element_utils, elements, gsi_handlers.master_controller_handlers, gsi_handlers.sim_timeline_handlers, reset, services, sims4.log, sims4.service_manager
...
class WorkEntry:
    ...
    def __repr__(self):
        if self._debug_name is not None:
            ...
            if self._work_entry_element is not None:
...
class WorkRequest:
    __slots__ = [
     '_work_element', '_required_sims', '_additional_resources',
     '_on_accept', '_set_work_timestamp', '_debug_name']
...
class MasterController(sims4.service_manager.Service):
    ...
    def add_sim(self, sim):
        logger.assert_raise((self._enabled == True), 'Attempting to add a sim to the master controller when it is not enabled.',
          owner='sscholl')
        self._sims.add(sim)
        ...
        for required_sim in work_entry.resources:
            ...
            if required_sim in self._active_work:
                ...
                if not required_work_entry.cancelable:
                    ...
                    required_sim.is_sim or required_sim.on_requested_as_resource(work_entry)
                    continue
                self._gsi_add_log_entry(sim, 'PROCESS_WORK_ENTRY', 'Sim Resource has work entry that can be canceled added to immediate_cancels')
            ...
            try:
                ...
                for sim in self._sorted_sims(itertools.chain(self._denied_sims, sims_filtered)):
                    ...
                    if existing_entry is not None:
                        if not existing_entry.cancelable:
                            continue
                    if sim in self._denied_sims:
                        if sim.is_sim:
                            sim.queue.on_head_changed.remove(self._process)
                    ...
                    if work_request.work_element is None:
                        ...
                        continue
                    work_entry = WorkEntry(work_element=(work_request.work_element), resources=(work_request.required_sims),
                ...
                for sim in self._sims:
                    if sim not in self._active_work:
                        ...
                        if work_element_idle is not None:
                            work_entry = WorkEntry(work_element=work_element_idle, cancel_callable=cancel_callable,
                              resources=(
                             sim,),
                              owner=sim,
                              master_controller=self)
                ...
                logger.exception('Exception while processing the Master Controller.')
    ...
    def _gsi_entry_initialize(self, *sims_being_processed):
        if gsi_handlers.master_controller_handlers.archiver.enabled:
            self._gsi_entry = {'sims_with_active_work':str([str(sim) for sim in self._active_work.keys()]), 
             'last_time_stamp':str(self._last_work_timestamps)}
    ...
    def _gsi_add_log_entry(self, sim, tag, log_message, *log_message_args):
        if gsi_handlers.master_controller_handlers.archiver.enabled:
            entry = {'sim':str(sim) if sim is not None else '', 
             'tag':tag, 
        ...
        if gsi_handlers.sim_timeline_handlers.archiver.enabled:
            for resource in work_entry.resources:
                if not resource.is_sim:
                    continue
                elif resource is work_entry.owner:
    ...
    def _gsi_entry_finalize(self):
        if gsi_handlers.master_controller_handlers.archiver.enabled:
            self._gsi_entry['sims_with_active_work_after'] = str([str(sim) for sim in self._active_work.keys()])
