...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\pregnancy\pregnancy_tracker.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 41116 bytes
...
import alarms, clock, services, sims4.log, sims4.resources
...
class PregnancyTracker(SimInfoTracker):
    ...
    @property
    def is_pregnancy_supported(self):
        return self._sim_info.species in self.PREGNANCY_COMMODITY_MAP
    ...
    def start_pregnancy(self, parent_a, parent_b, pregnancy_origin=PregnancyOrigin.DEFAULT):
        if not self.is_pregnancy_supported:
            ...
            if self.is_pregnant:
                return
            return parent_a.incest_prevention_test(parent_b) or None
                ...
                if not current_zone.is_zone_running or self._sim_info.is_instanced(allow_hidden_flags=ALL_HIDDEN_REASONS):
                    if self._completion_alarm_handle is None:
                        self._completion_alarm_handle = alarms.add_alarm(self, (clock.interval_in_sim_minutes(1)),
                          (self._on_pregnancy_complete), repeating=True, cross_zone=True)
        else:
    ...
    def create_sim_info(self, offspring_data):
        ...
        parent_a, parent_b = self.get_parents()
        sim_creator = SimCreator(age=(offspring_data.age), gender=(offspring_data.gender),
    ...
    def select_traits_for_offspring(cls, offspring_data, parent_a, parent_b, num_traits, origin=PregnancyOrigin.DEFAULT, random=random):
        ...
        if origin in cls.PREGNANCY_ORIGIN_MODIFIERS:
            ...
            if parent_a is not None:
                ...
                return personality_trait_slots or traits
    ...
    def create_offspring_data(self):
        ...
        if self._offspring_count_override is not None:
            ...
            offspring_count = pop_weighted([(p.weight * p.modifiers.get_multiplier(SingleSimResolver(self._sim_info)), p.size) for p in self.MULTIPLE_OFFSPRING_CHANCES],
              random=r)
    ...
    def _get_random_first_name(self, offspring_data):
        ...
        def is_valid(first_name):
            ...
            tries_left -= 1
            if tries_left:
                if any((sim.first_name == first_name for sim in self._sim_info.household)):
                    return False
    ...
    def refresh_pregnancy_data(self, on_create=None):
        ...
        if is_pregnant:
            if not self.is_pregnancy_supported:
                logger.error('{} with id {} has an invalid pregnancy due to being species {}.', (self._sim_info), (self._sim_info.id), (self._sim_info.species), owner='kalucas')
                self.clear_pregnancy()
                return
        traits_pack_tuple = self.PREGNANCY_ORIGIN_TRAIT_MAPPING.get(self._origin)
        if traits_pack_tuple is not None:
            if not is_available_pack(traits_pack_tuple.pack):
