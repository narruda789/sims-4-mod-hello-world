...
# Size of source mod 2**32: 19318 bytes
import services, tag
...
class OutfitCASPartTagsTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    def __call__(self, test_targets, **kwargs):
        ...
        try:
            caspartid_tags_dic = get_tags_from_outfit(sim._base, *sim.get_current_outfit())
        ...
        if self.body_type is None:
            ...
            for key, value in caspartid_tags_dic.items():
                body_type = get_caspart_bodytype(key)
                if self.body_type == body_type and value.intersection(tags_to_look_for):
                    match_count += 1
