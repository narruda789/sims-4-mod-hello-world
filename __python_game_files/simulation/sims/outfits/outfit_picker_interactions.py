...
# Compiled at: 2021-05-05 14:03:40
# Size of source mod 2**32: 16420 bytes
import operator
...
class OutfitPickerSuperInteraction(PickerSuperInteraction):
            ...
            for outfit in sorted(picked_items, key=(operator.itemgetter(1)), reverse=True):
                if current_outfit[0] == outfit[0]:
                    if current_outfit[1] >= outfit[1]:
                        current_outfit = (
                         current_outfit[0], current_outfit[1] - 1)
    ...
    class _OutfitActionApplyCareerOutfit(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def get_disabled_tooltip(self):
            if self.pie_menu_test_tooltip is None:
                ...
                return filter_results or self.pie_menu_test_tooltip
        ...
        def on_choice_selected(self, interaction, picked_items, **kwargs):
            dialog = self.picker_dialog((interaction.sim), title=(lambda *_, **__: interaction.get_name(apply_name_modifiers=False)),
    ...
    class _OutfitSimInfoSelectorBusiness(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def get_outfit_sim_info(self, interaction, **_):
            ...
            if business_manager is None:
                return
            if business_manager.is_valid_employee_type(self.employee_type):
    ...
    INSTANCE_TUNABLES = {'picker_dialog':TunableUiOutfitPickerSnippet(description="\n            The interaction's outfit picker.\n            ",
     ...
     'outfit_sim_info':TunableVariant(description='\n            Define the Sim or object whose outfits are to be displayed.\n            ',
       from_participant=_OutfitSimInfoSelectorParticipant.TunableFactory(),
       from_business=_OutfitSimInfoSelectorBusiness.TunableFactory(),
       default='from_participant',
       tuning_group=GroupNames.PICKERTUNING), 
     'allow_current_outfit':OptionalTunable(description='\n            Whether or not the current outfit is a selectable entry.\n            ',
       tunable=TunableLocalizedStringFactory(description='\n                The tooltip to show on the current outfit.\n                '),
       enabled_name='Disallow',
       disabled_name='Allow',
       tuning_group=GroupNames.PICKERTUNING), 
     'outfit_actions':TunableList(description='\n            All the actions to undertake once a selection has been made.\n            ',
       tunable=TunableVariant(description='\n                The action to undertake once a selection has been made.\n                ',
       push_affordance=(_OutfitPickerActionPushInteraction.TunableFactory()),
       delete_outfit=(_OutfitActionDeleteOutfit.TunableFactory()),
       apply_career_outfit=(_OutfitActionApplyCareerOutfit.TunableFactory()),
    ...
    def has_valid_choice(cls, target, context, **kwargs):
        if not (cls._has_valid_outfit_choice)(target, context, **kwargs):
            ...
            return cls._has_valid_outfit_action() or False
        return True
    ...
    def _has_valid_outfit_choice(cls, target, context, **kwargs):
        ...
        if outfit_participant is None:
            ...
            return any((outfit_category in outfit_categories and outfit_list for outfit_category, outfit_list in outfits.get_all_outfits())) or False
        return True
        ...
        if outfit_participant is not None:
            ...
            for outfit_category, outfit_list in outfits.get_all_outfits():
                ...
                for outfit_index, _ in enumerate(outfit_list):
                    ...
                    if inst_or_cls.allow_current_outfit is not None and current_outfit == outfit_key:
                        is_enable = False
                        row_tooltip = lambda *_, **__: inst_or_cls.create_localized_string(inst_or_cls.allow_current_outfit)
                    else:
                        is_enable = True
                        row_tooltip = None
                    yield OutfitPickerRow(outfit_sim_id=(outfit_sim_info.sim_id), outfit_category=outfit_category,
