...
# Compiled at: 2021-06-10 15:25:33
# Size of source mod 2**32: 27252 bytes
...
@sims4.commands.Command('university.degree_info', pack=(Pack.EP08), command_type=(sims4.commands.CommandType.Automation))
def degree_info(opt_sim: OptionalSimInfoParam=None, _connection=None):
    ...
    if sim_info is None:
        ...
        sims4.commands.automation_output('UniversityDegreeInfo; Status:Failed, Message:No valid target for university.degree_info', _connection)
        return False
    degree_tracker = sim_info.degree_tracker
    if degree_tracker is None:
        sims4.commands.output('Sim: {} has no degree tracker in university.degree_info'.format(sim_info), _connection)
        sims4.commands.automation_output('UniversityDegreeInfo; Status:Failed, Message: Sim:{} has no degree tracker in university.degree_info'.format(sim_info), _connection)
        return False
    ...
    sims4.commands.automation_output('UniversityDegreeInfo; Status:Data, Major:{}, University:{}, GPA:{}, EnrollmentStatus:{}, CurrentDayOfTerm:{}'.format(major.__name__ if major else 'None', university.__name__ if university else 'None', degree_tracker.get_gpa(), degree_tracker.get_enrollment_status().name, current_day if current_day else 'None'), _connection)
...
@sims4.commands.Command('qa.university.enroll', pack=(Pack.EP08), command_type=(sims4.commands.CommandType.Automation))
def enroll_qe(major: TunableInstanceParam(sims4.resources.Types.UNIVERSITY_MAJOR), university: TunableInstanceParam(sims4.resources.Types.UNIVERSITY), opt_sim: OptionalSimInfoParam=None, classes: int=3, elective: TunableInstanceParam(sims4.resources.Types.UNIVERSITY_COURSE_DATA)=None, tuition_cost: int=0, total_scholarship_taken: int=0, is_using_loan: bool=False, destination_zone_id: int=None, _connection=None):
    sim_info = get_optional_target(opt_sim, target_type=OptionalSimInfoParam, _connection=_connection)
    if sim_info is None:
        sims4.commands.automation_output('UniversityEnrollInfo; Status:Failed, Message:No valid target for university.enroll', _connection)
        return False
    degree_tracker = sim_info.degree_tracker
    if degree_tracker is None:
        sims4.commands.automation_output('UniversityEnrollInfo; Status:Failed, Message:Sim {} has no degree tracker in university.enroll'.format(sim_info), _connection)
        return False
    ...
    UniversityTelemetry.send_university_housing_telemetry(destination_zone_id)
    if destination_zone_id is None:
        sims4.commands.automation_output('UniversityEnrollInfo; Status:Success', _connection)
        return True
    home_zone_id = sim_info.household.home_zone_id
    if home_zone_id == destination_zone_id:
        degree_tracker.on_enroll_in_same_housing()
