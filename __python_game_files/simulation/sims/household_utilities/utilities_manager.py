...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\household_utilities\utilities_manager.py
# Compiled at: 2021-01-07 18:31:38
# Size of source mod 2**32: 25419 bytes
import operator, sims
...
class UtilityInfo:
    ...
    def set_surplus_listeners_info(self, tracker, surplus_listener, deficit_listener):
        self._surplus_listeners_info = (
         tracker, surplus_listener, deficit_listener)
...
class ZoneUtilitiesManager:
        ...
        if ZoneUtilitiesManager.utility_surplus_threshold is None:
            ...
            for utility in Utilities:
                ...
                if utility_tuning is None:
                    ...
                    ZoneUtilitiesManager.utility_surplus_threshold[utility] = (
                     ZoneUtilitiesManager.SURPLUS_DEAD_ZONE_LOWER_BOUND, ZoneUtilitiesManager.SURPLUS_DEAD_ZONE_UPPER_BOUND_DELTA)
                    continue
                ...
                ZoneUtilitiesManager.utility_surplus_threshold[utility] = (
                 ZoneUtilitiesManager.SURPLUS_DEAD_ZONE_LOWER_BOUND, upper_bound)
    ...
    def test_utility_info(self, utilities, target, resolver, skip_safe_tests=False):
        ...
        for utility, utility_data in utilities.items():
            ...
            if utility_info is not None:
                tests = utility_info.active or None
                if target:
                    if not target.is_sim:
                        if not target.is_terrain:
                            tests = target.tests_to_bypass_utility_requirement.get(utility, None)
                return tests and tests.run_tests(resolver, skip_safe_tests=skip_safe_tests) or TestResult(False, 'Bills: Interaction requires a utility that is shut off.',
                  tooltip=(utility_info.get_priority_shutoff_tooltip(utility_data.shutoff_tooltip_override)))
    ...
    def shut_off_utility(self, utility, reason, tooltip=None, from_load=False):
        ...
        if not utility_info.add_shutoff_reason(reason, tooltip=tooltip):
            ...
            return self.is_manager_on_current_lot() or None
        if utility_info_was_active:
    ...
    def restore_utility(self, utility, reason):
        ...
        if not utility_info.remove_shutoff_reason(reason):
            ...
            return self.is_manager_on_current_lot() or None
        if not utility_info_was_active:
            if utility_info.active:
