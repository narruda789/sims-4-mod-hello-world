...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\household_utilities\utility_tests.py
# Compiled at: 2020-11-12 10:43:20
# Size of source mod 2**32: 9807 bytes
...
class UtilityTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    test_events = (
     TestEvent.UtilityStatusChanged,)
    FACTORY_TUNABLES = {'household_to_test':TunableVariant(description='\n            A variant that decides where the household to test comes from.\n            ',
       ...
       locked_args={'household_source': PARTICIPANT_HOUSEHOLD}),
       active_household=TunableTuple(description='\n                The household being controlled by the player.\n                ',
       locked_args={'household_source': ACTIVE_HOUSEHOLD}),
       active_lot_household=TunableTuple(description='\n                The household that owns the active lot.\n                ',
       ...
       tunable_type=bool,
       default=True),
       locked_args={'household_source': ACTIVE_LOT_HOUSEHOLD}),
       default='active_lot_household'), 
     'utility_states':TunableSet(description='\n            List of utilities and whether they are required to be active or not.\n            ',
    ...
    def __call__(self, test_targets=None):
        ...
        if self.household_to_test.household_source == UtilityTest.ACTIVE_LOT_HOUSEHOLD:
            ...
            if self.household_to_test.household_source == UtilityTest.ACTIVE_HOUSEHOLD:
                ...
                elif target_household is not None:
                    utilities_manager = services.get_utilities_manager_by_household_id(target_household.id)
                else:
                    if self.household_to_test.household_source == UtilityTest.ACTIVE_LOT_HOUSEHOLD and self.household_to_test.consider_non_household_lot:
...
class UtilitiesComponentTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    FACTORY_TUNABLES = {'subject':TunableEnumEntry(description='\n            The subject of the test.\n            ',
     ...
     'can_run_on_utilities':OptionalTunable(description='\n            If enabled, will verify if subject can be run\n            on tuned utilities.\n            ',
    ...
    def __call__(self, test_targets=()):
        for target in test_targets:
            ...
            if utilities_component is None:
                ...
                if self.can_run_on_utilities:
                    if not self.can_run_on_utilities.test_collection(list(utilities_component.allow_utility_usage_dict.keys())):
