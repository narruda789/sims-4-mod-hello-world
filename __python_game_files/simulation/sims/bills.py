...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\bills.py
# Compiled at: 2020-10-27 16:07:24
# Size of source mod 2**32: 89398 bytes
...
import alarms, build_buy, clock, objects.components, services
...
UtilityInfo = namedtuple('UtilityInfo', ('utility', 'cost', 'max_value', 'current_value',
                                         'utility_name', 'utility_symbol', 'rate_of_change',
                                         'selling'))
...
class BillsSourceVariant(TunableVariant):
  ...
  locked_args={'utility': ALL_BILLS_SOURCE}), 
         property_taxes=TunableTuple(description='\n                Apply this reduction to property taxes\n                ',
  locked_args={'utility': PROPERTY_TAX_SOURCE}), 
         utility=TunableTuple(description='\n                Apply this reduction to property taxes\n                ',
  utility=TunableEnumEntry(description='\n                    The utility to modify.\n                    ',
  tunable_type=Utilities,
  default=(Utilities.POWER))), 
         default='all_bills', **kwargs)
...
class Bills:
    ...
    REDUCTION_REASON_SOURCE = TunableMapping(description="\n        A mapping of reduction reason to what part of the bill it'll modify.\n        ",
      ...
      tunable_type=BillReductionEnum,
      default=(BillReductionEnum.GlobalPolicy_ControlInvasiveSpecies)),
      value_type=BillsSourceVariant(description='\n            The source of bills to apply this reduction to.\n            '))
    UTILITY_INFO = TunableMapping(key_type=Utilities,
    ...
    def get_utility_info(cls, inst, utility):
        ...
        if inst is not None and not sim_info is None:
            if sim_info.household is not inst._household:
    ...
    def _set_up_bill_timer(self, from_negative_payout=False):
        ...
        self._bill_timer_handle = alarms.add_alarm((self._household), time_until_bill_delivery, (lambda _: self.allow_bill_delivery()),
          cross_zone=True)

    def _set_up_timers(self):
        self._set_up_bill_timer()
        if self._stored_shutoff_timer_ticks == 0:
            if self._stored_warning_timer_ticks == 0:
                return
    ...
    def mailman_has_delivered_bills(self):
        if self.current_payment_owed is not None:
            if self._shutoff_handle is not None or self.is_any_utility_delinquent():
                return True
        return False
    ...
    def is_additional_bill_source_delinquent(self, additional_bill_source):
        ...
        if cost > 0:
            if any(self._utility_delinquency.values()):
                return True
        return False
        ...
        if bill_reductions:
            for reduction_reason, reduction in bill_reductions.items():
                ...
                if reduction_text:
                    ...
                    logger.error('Attempting to get reduction reason ({}) without a tuned value in the Reduction Reason Text Map.',
                      (str(reduction_reason)), owner='jjacobson')

        return (
         multipliers, bill_multiplier_descriptions)
    ...
    def _get_additional_bill_costs(self):
        ...
        return (
         total_bill_amount, bill_details)
    ...
    def trigger_bill_notifications_from_delivery--- This code section failed: ---

 L.1269         0  LOAD_FAST                'self'
                2  LOAD_METHOD              mailman_has_delivered_bills
                4  CALL_METHOD_0         0  '0 positional arguments'
                6  POP_JUMP_IF_FALSE    12  'to 12'

 L.1270         8  LOAD_CONST               None
               10  RETURN_VALUE     
             12_0  COME_FROM             6  '6'

 L.1274        12  LOAD_CONST               False
               14  LOAD_FAST                'self'
               16  STORE_ATTR               _can_deliver_bill

 L.1277        18  LOAD_FAST                'self'
               20  LOAD_ATTR                autopay_bills
               22  POP_JUMP_IF_TRUE     40  'to 40'

 L.1278        24  LOAD_FAST                'self'
               26  LOAD_ATTR                _current_payment_owed
               28  LOAD_CONST               0
               30  COMPARE_OP               ==
               32  POP_JUMP_IF_TRUE     40  'to 40'

 L.1279        34  LOAD_FAST                'self'
               36  LOAD_ATTR                _household
               38  POP_JUMP_IF_TRUE     56  'to 56'
             40_0  COME_FROM            32  '32'
             40_1  COME_FROM            22  '22'

 L.1280        40  LOAD_FAST                'self'
               42  LOAD_ATTR                pay_bill
               44  LOAD_CONST               False
               46  LOAD_CONST               ('sound',)
               48  CALL_FUNCTION_KW_1     1  '1 total positional and keyword args'
               50  POP_TOP          

 L.1281        52  LOAD_CONST               None
               54  RETURN_VALUE     
             56_0  COME_FROM            38  '38'

 L.1283        56  LOAD_FAST                'self'
               58  LOAD_METHOD              _set_next_delinquency_timers
               60  CALL_METHOD_0         0  '0 positional arguments'
               62  POP_TOP          

 L.1284        64  LOAD_FAST                'self'
               66  LOAD_METHOD              get_bills_arrival_notification
               68  CALL_METHOD_0         0  '0 positional arguments'
               70  STORE_FAST               'bills_arrival_notification'

 L.1285        72  LOAD_FAST                'bills_arrival_notification'
               74  LOAD_CONST               None
               76  COMPARE_OP               is-not
               78  POP_JUMP_IF_FALSE   102  'to 102'

 L.1286        80  LOAD_FAST                'self'
               82  LOAD_METHOD              _send_notification
               84  LOAD_FAST                'bills_arrival_notification'
               86  CALL_METHOD_1         1  '1 positional argument'
               88  STORE_FAST               'notification_shown'

 L.1287        90  LOAD_FAST                'notification_shown'
               92  POP_JUMP_IF_FALSE   102  'to 102'

 L.1288        94  LOAD_FAST                'self'
               96  LOAD_METHOD              give_bills_arrival_notification_loots
               98  CALL_METHOD_0         0  '0 positional arguments'
              100  POP_TOP          
            102_0  COME_FROM            92  '92'
            102_1  COME_FROM            78  '78'

Parse error at or near `COME_FROM' instruction at offset 102_0
    ...
    def pay_bill(self, sound=True, clear_bill=False):
        if (clear_bill or self._current_payment_owed) < 0:
            ...
            self._household.funds.add((-self._current_payment_owed), (Consts_pb2.TELEMETRY_INTERACTION_REWARD),
              count_as_earnings=False)
    ...
    def pay_source_bill(self, source, source_bill_info, lot_statistic_component=None):
        if not source_bill_info is None:
            if not source_bill_info.statistic_deltas:
                return
        ...
        for statistic, delta in source_bill_info.statistic_deltas:
            ...
            if utility_production_stat is None or statistic is not utility_production_stat:
                continue
            if delta < 0:
    ...
    def sell_excess_utility(self, utility):
        ...
        resolver = GlobalResolver()
        multiplier = utility_info.unit_cost_sell_price_multiplier.get_multiplier(resolver)
        self._household.funds.add((int(utility_info.unit_cost * (current_value - utility_info.statistic_sell_value) * multiplier)), (Consts_pb2.TELEMETRY_INTERACTION_REWARD),
          count_as_earnings=False)
    ...
    def _send_notification(self, notification):
        ...
        if self._warning_handle is not None:
            if self._warning_handle.finishing_time <= current_time:
                alarms.cancel_alarm(self._warning_handle)
                self._warning_handle = None
                play_tunable_audio(self.AUDIO.delinquency_warning_sfx)
        else:
            return self.bill_notifications_enabled or False
        ...
        additional_tokens = [
         remaining_time, self._get_colored_text(self._current_payment_owed), reduction_reasons_string]
        additional_tokens.extend(detail_tokens)
        dialog.show_dialog(icon_override=icon_override, additional_tokens=(tuple(additional_tokens)),
          tutorial_id=(Bills.BILL_TUTORIAL.guid64))
        return True
    ...
    def load_data(self, householdProto):
        ...
        for lot_unpaid_bill_item in householdProto.gameplay_data.lot_unpaid_bill_data:
            key_tuple = (
             lot_unpaid_bill_item.zone_id, lot_unpaid_bill_item.situation_id)
            sims_on_lot = []
            for sim_id in lot_unpaid_bill_item.sim_ids_on_lot:
                sims_on_lot.append(sim_id)

            self._lot_unpaid_bill[key_tuple] = (
             lot_unpaid_bill_item.money_amount, sims_on_lot)
        ...
        if self._current_payment_owed == 0:
            ...
            if hasattr(householdProto.gameplay_data, 'current_bill_details'):
            ...
            if self._current_payment_owed is None and not self._stored_shutoff_timer_ticks > 0:
                if self._stored_warning_timer_ticks > 0:
                ...
                if householdProto.gameplay_data.HasField('repo_man_due_time'):
    ...
    def show_bills_dialog(self):
        ...
        if statistic_component is not None:
            ...
            def build_item(amount, item_display_tuning, details=None):
                ...
                if item_display_tuning.detailed_tooltip:
                    if details:
