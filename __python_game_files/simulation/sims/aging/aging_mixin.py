...
# Size of source mod 2**32: 39573 bytes
import random
from careers.career_enums import CareerCategory
from sims4.localization import TunableLocalizedStringFactory
import sims4.log, sims4.telemetry
...
from sims.aging.aging_translation import *
import alarms, distributor.fields, distributor.ops, services, telemetry_helper
...
class AgingMixin:
    ...
    def _get_aging_disabled_tooltip(self):
        for trait in self.trait_tracker.equipped_traits:
            if trait.disable_aging is not None:
                if trait.can_age_up(self.age) or trait.disable_aging.tooltip is not None:
    ...
    def update_time_alive(self):
        ...
        if current_time < self._last_time_time_alive_updated:
            logger.error('Time alive going backward for {}. time_alive: {}, last time time alive updated: {}, current time: {}', self,
              (self._time_alive), (self._last_time_time_alive_updated), current_time, owner='tingyul')
            return
    ...
    def update_age_callbacks(self):
        ...
        if self._almost_can_age_handle is not None:
            alarms.cancel_alarm(self._almost_can_age_handle)
        if warn_time >= 0:
            self._almost_can_age_handle = alarms.add_alarm(self, create_time_span(days=warn_time), (self.callback_almost_ready_to_age),
              cross_zone=True)
        if self._can_age_handle is not None:
            alarms.cancel_alarm(self._can_age_handle)
        if age_time >= 0:
            self._can_age_handle = alarms.add_alarm(self, create_time_span(days=age_time), (self.callback_ready_to_age),
              cross_zone=True)
    ...
    def _age_up_pctya(self):
        if not self._high_school_student_aging_check():
            self._create_auto_age_callback(delay=(AgingMixin.TEEN_DELAY_DAYS_TILL_NEXT_AGING_ATTEMPT))
            return
    ...
    def _age_up_elder(self):
        ...
        if sim_instance is not None:
            context = InteractionContext(sim_instance, InteractionContext.SOURCE_SCRIPT, Priority.Critical)
            sim_instance.push_super_affordance(aging_data.old_age_interaction, None, context) or self._create_auto_age_callback()
        else:
            if self.is_npc:
    ...
    def suppress_aging(self):
        ...
        self._age_suppression_alarm_handle = alarms.add_alarm(self, create_time_span(minutes=(AgingTuning.AGE_SUPPRESSION_ALARM_TIME)),
          (self._suppress_aging_callback),
          cross_zone=True)
    ...
    def _high_school_student_aging_check(self):
        if not self.is_teen:
            ...
            return career_tracker is None or career_tracker.has_career or True
