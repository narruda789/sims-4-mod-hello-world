...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\rebate_manager.py
# Compiled at: 2019-03-29 21:51:18
# Size of source mod 2**32: 16213 bytes
...
import enum, services, sims4.commands, tag
    ...
    def __init__(self, **kwargs):
        super().__init__(valid_objects=TunableVariant(description='\n            The items to which the rebate will be applied.\n            ',
         ...
         tag.Tag.INVALID,))),
          ...
          buildbuy=RebateCategory(locked_args={'rebate_category_enum':RebateCategoryEnum.BUILD_BUY, 
         'cyclical':False}),
          gameplay_object=RebateCategory(locked_args={'rebate_category_enum': RebateCategoryEnum.GAMEPLAY_OBJECT}),
          default='buildbuy'))
...
class RebateManager:
        ...
        for rebate_item, rebate_data in self.REBATES.items():
            ...
            if obj is None:
                ...
                if valid_objects is not None:
                    if not obj.has_any_tag(valid_objects):
                        continue
                if self.run_tests_on_object_and_household_sims(obj, rebate_data.tests_set):
    ...
    def add_rebate_cycle(self):
        ...
        for cyclical_object_id, rebate_category in cyclical_rebate_object_ids:
            if self._rebate_cycle is None:
                self._rebate_cycle = self.REBATE_CYCLE(start_callback=(self.add_rebate_for_object_from_cycle), extra_data={'obj_id':cyclical_object_id, 
                 'category':rebate_category},
                  schedule_immediate=False)
            else:
                self._rebate_cycle.merge_schedule(self.REBATE_CYCLE(start_callback=(self.add_rebate_for_object_from_cycle), extra_data={'obj_id':cyclical_object_id, 
                 'category':rebate_category},
                  schedule_immediate=False))
    ...
    def payout_rebates(self, *_):
        ...
        self._household.funds.add(total_rebate_amount, reason=(Consts_pb2.TELEMETRY_MONEY_ASPIRATION_REWARD),
          sim=active_sim_info)
