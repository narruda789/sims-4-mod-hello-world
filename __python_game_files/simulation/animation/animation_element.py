...
def register_balloon_requests(asm, balloon_requests, repeat=False, interaction=None, overrides=None):
    ...
    if remaining_balloons:
        repeat or logger.error('Failed to schedule all requested balloons for {}', asm)
    else:
        if repeat:
        ...
        if interaction is not None:
            ...
            def do_begin(timeline):
                ...
                if begin_states:
                    ...
                    elif enable_auto_exit:
                        if asm.current_state != 'exit':
                            auto_exit_actors = {actor for actor in all_actors if actor.is_sim if not actor.asm_auto_exit.locked if not actor.asm_auto_exit.locked}
            ...
            def do_end(timeline):
                ...
                if end_states:
                    ...
                    if not requires_begin_flush:
                        sequence = arb_end.empty or create_run_animation(arb_end)
                    else:
                        pass
                    sequence = None
                if asm.current_state == 'exit':
...
class AnimationElement(HasTunableReference, elements.ParentElement, metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.ANIMATION)):
    ...
    def _set_alternative_prop_overrides(self, asm):
        ...
        for prop, prop_override in self.overrides.props.items():
            alt_prop_definitions = prop_override.alternative_prop_definitions
            if not alt_prop_definitions:
                continue
            if alt_prop_definitions.favorite_object_in_inventory:
                favorite_data = alt_prop_definitions.favorite_object_in_inventory
                inventory = True
            else:
                if alt_prop_definitions.favorite_object_by_definition:
                    favorite_data = alt_prop_definitions.favorite_object_by_definition
                    inventory = False
                else:
                    continue
                ...
                if sim is None:
                    ...
                    favorite_def, favorite_obj = favorites_utils.get_favorite_in_sim_inventory(sim, favorite_data)
                else:
                    favorite_def = favorites_utils.get_favorite_by_definition(sim, favorite_data)
                    favorite_obj = None
                if favorite_def is None:
                    continue
                self.overrides.alternative_props[prop] = favorite_def
                if favorite_obj:
                    if favorite_obj.favorite_prop_animation_overrides:
                        fav_prop_anim_overrides = favorite_obj.favorite_prop_animation_overrides.get_overrides_for_favorite_object(prop, favorite_obj)
                        if fav_prop_anim_overrides is not None:
                            self.overrides = fav_prop_anim_overrides(self.overrides)
...
class AnimationElementSet(metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.ANIMATION)):
    ...
    def __new__(cls, interaction=None, setup_asm_additional=None, setup_asm_override=DEFAULT, overrides=None, sim=DEFAULT, **animate_kwargs):
        ...
        for animation_and_overrides in cls._animation_and_overrides:
            ...
            if not best_element_supported_posture is not None or best_supported_posture is None or best_element_supported_posture < best_supported_posture:
        ...
        best_anim_element = best_anim_element_type(interaction=interaction, setup_asm_additional=setup_asm_additional, setup_asm_override=setup_asm_override, 
         overrides=overrides, **animate_kwargs)
