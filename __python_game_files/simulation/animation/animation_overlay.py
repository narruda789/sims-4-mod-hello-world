...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\animation\animation_overlay.py
# Compiled at: 2018-07-27 19:54:21
# Size of source mod 2**32: 6204 bytes
...
class AnimationOverlay(HasTunableFactory, AutoFactoryInit):
    ...
    def update_overlay(self):
        ...
        if self._overlay_interaction is None:
            return
        overlay_animation = self._overlay_animation((self._overlay_interaction), setup_asm_additional=restart_overlay_asm,
    ...
    def _on_posture_changed(self, change, dest_state, track, old_value, new_value):
        if change == PostureEvent.POSTURE_CHANGED:
            if track == PostureTrack.BODY:
                if self._overlay_interaction is not None:
...
class AnimationOverlayComponent(Component, HasTunableFactory, AutoFactoryInit, component_name=types.ANIMATION_OVERLAY_COMPONENT):
    ...
    def handle_event(self, sim_info, event_type, resolver):
        if self.owner.is_sim:
            if self.owner.sim_info is not sim_info:
                return
