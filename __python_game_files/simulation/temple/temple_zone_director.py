...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\temple\temple_zone_director.py
# Compiled at: 2020-03-05 21:42:00
# Size of source mod 2**32: 16481 bytes
...
import build_buy, camera, objects.system, services, sims4.log
...
TEMPLE_EVENTS = (
 TestEvent.SimActiveLotStatusChanged, TestEvent.InteractionComplete)
        ...
        if self._temple_data.enter_lot_loot:
            ...
            for sim in services.sim_info_manager().instanced_sims_gen(allow_hidden_flags=ALL_HIDDEN_REASONS):
                if not sim.is_on_active_lot():
                    if not arrival_spawn_point is not None or test_point_in_polygon(sim.intended_position, arrival_spawn_point.get_footprint_polygon()):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.SimActiveLotStatusChanged and self._has_enter_exit_lot_loot():
            ...
            if event == TestEvent.InteractionComplete:
                if resolver(TempleTuning.CHEST_OPEN_INTEARCTION):
                    object_manager = services.object_manager()
    ...
    def _setup_gates(self, object_manager):
        ...
        for gate in gates:
            if not gate.state_component:
                ...
                gate.has_state(gate_state) or logger.error('Trying to apply a random state to a temple gate but the gate, {}, has no state value, {} for state {}.', gate, gate_state_value, gate_state)
                continue
