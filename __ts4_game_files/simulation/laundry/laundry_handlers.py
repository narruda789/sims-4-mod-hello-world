...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\laundry\laundry_handlers.py
# Compiled at: 2017-10-24 13:39:36
    ...
    if laundry_service is not None:
        ...
        if household is None:
            ...
            entry = {'household_id':household.id, 
             ...
             'last_unload':str(household.laundry_tracker.last_unload_laundry_time)}
            laundry_data.append(entry)
            ...
            for i, hero_obj in enumerate(laundry_service.laundry_hero_objects):
                entry = {'hero_objects_id':hex(hero_obj.id), 
                 'hero_objects':hero_obj.definition.name}
                _add_entry(i, entry, laundry_data)
            ...
            for i, hamper in enumerate(laundry_service.hampers):
                entry = {'hampers_id':hex(hamper.id), 
                 'hampers':hamper.definition.name, 
                 'hampers_state':str(hamper.get_state(state_type))}
                _add_entry(i, entry, laundry_data)
