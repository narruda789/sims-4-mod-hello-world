...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\services\terrain_service.py
...
import build_buy, routing, services, sims4.log, sims4.math, sims4.reload, sims4.service_manager
...
class TerrainService(sims4.service_manager.Service):
    ...
    def setup_walkstyle_portals(self):
        ...
        if portal_component is None:
            ...
            return initial_transforms or None
        portal_ids = []
...
def try_create_ocean_object():
    ...
    if _ocean_object is not None:
        ...
        return locators or None

    def move_ocean(ocean):
        zone = services.current_zone()
        terrain_center = zone.lot.center
        location = sims4.math.Location(sims4.math.Transform(translation=terrain_center, orientation=(sims4.math.Quaternion.IDENTITY())),
          routing_surface=(SurfaceIdentifier(zone.id, 0, SurfaceType.SURFACETYPE_WORLD)))
