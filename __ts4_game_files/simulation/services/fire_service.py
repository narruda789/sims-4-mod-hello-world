...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\services\fire_service.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 88060 bytes
...
import alarms, build_buy, clock, date_and_time, placement, services, sims4.resources, tag, terrain
...
class FireService(Service):
    ...
    FIRE_PANIC_BUFFS = TunableList(TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.BUFF))),
    ...
    def is_sim_in_fire_brigade_situation(self, sim, situation_manager=None):
        ...
        if fire_brigade_situation_id is not None:
            ...
            if fire_brigade_situation is not None:
                if fire_brigade_situation.is_sim_in_situation(sim):
                    return True
        return False
    ...
    def query_quadtree_for_fire_object(self, position, radius=DEFAULT, surface_id=None):
        if self._fire_quadtree is None:
            ...
            query = sims4.geometry.SpatialQuery(sims4.geometry.QtCircle(position, radius), [
             self._fire_quadtree])
    ...
    def _query_quadtree_for_flammable_object(self, position, radius=DEFAULT, surface_id=None):
        ...
        query = sims4.geometry.SpatialQuery(sims4.geometry.QtCircle(position, radius), [
         self._flammable_objects_quadtree])
    ...
    def _query_quadtree_for_sim(self, position, surface_id, filter_type, radius=DEFAULT, flags=sims4.geometry.ObjectQuadTreeQueryFlag.IGNORE_SURFACE_TYPE):
        ...
        radius = self.FIRE_QUADTREE_RADIUS if radius is DEFAULT else radius
        return sim_quadtree.query((sims4.geometry.QtCircle(position, radius)), surface_id=surface_id,
          flags=flags,
          filter=filter_type)
    ...
    def _fire_object_state_changed_callback(self, owner, state, old_value, new_value):
        if state is self.FIRE_OBJECT_FIRE_STATE:
            if new_value is self.FIRE_OBJECT_EXTINGUISHED_STATE_VALUE:
                if not owner.get_users():
    ...
    def is_fire_allowed(self, transform, routing_surface, run_placement_tests=True):
        if not fire_enabled:
            ...
            if not services.active_lot().is_position_on_lot(transform.translation):
                ...
                venue is None or venue.allows_fire or logger.info("Trying to spawn a fire on a venue that doesn't allow fire.")
                return False
            if run_placement_tests:
                self._placement_tests(transform.translation, routing_surface) or logger.info('Trying to spawn a fire on a lot at a position that is not valid.')
                return False
        return True
    ...
    def get_fires_in_potential_targets(self, target_objects):
        ...
        return self.query_quadtree_for_fire_object((sims4.math.Vector2(on_fire_object_position.x, on_fire_object_position.z)), radius=max_radius,
          surface_id=(on_fire_object.routing_surface))
    ...
    def _spawn_fire(self, transform, routing_surface, run_placement_tests=True):
        ...
        if self._fire_spread_alarm is None:
            ...
            self._fire_spread_alarm = alarms.add_alarm(self, time_span, (self._fire_spread_alarm_callback),
              repeating=True, repeating_time_span=repeating_time_span)
        return fire_object

    def spawn_fire_at_object(self, obj, num_fires=1):
        fire_position, parent_obj = Constraint.get_validated_routing_position(obj)
        fire_transform = obj.transform
        fire_transform.translation = fire_position
        fire_object = self._spawn_fire(fire_transform, obj.routing_surface)
        if fire_object is None or num_fires <= 1:
            return
        created_fire_objects = (
         fire_object,)
    ...
    def _placement_tests(self, new_position, surface_id=None, fire_object=None):
        ...
        if fire_object is not None:
            if abs(fire_object.position.y - new_position.y) > self.FIRE_SPREAD_HEIGHT_THRESHOLD:
                return False
        if fire_object is not None:
            ...
            if plex_building_type == PlexBuildingType.FULLY_CONTAINED_PLEX or plex_building_type == PlexBuildingType.PENTHOUSE_PLEX:
                if plex_zone_id is None or plex_zone_id != zone_id:
                    return False
    ...
    def is_object_flammable(self, obj):
        return obj.has_statistic(self.FLAMMABLE_COMMODITY)

    def set_object_burning(self, obj):
        if not obj.has_statistic(self.FLAMMABLE_COMMODITY):
            return
    ...
    def start_objects_burning(self, fire_object):
        ...
        if result is not None:
            for obj in result:
                ...
                if placement_flags & build_buy.PlacementFlags.CEILING:
                    if not placement_flags & build_buy.PlacementFlags.WALL_GRAPH_PLACEMENT:
                        continue
        ...
        result = self._query_quadtree_for_sim(location, surface_id=fire_surface,
          filter_type=(placement.ItemType.SIM_POSITION))
        if result is not None:
    ...
    def _extinguish_sim(self, sim):
        ...
        if tracker is not None:
            if tracker.has_statistic(self.FLAMMABLE_COMMODITY):
    ...
    def check_for_catching_on_fire(self, sim):
        if not self.fire_is_active:
            ...
            return self.FIRE_CAN_SPREAD_TO_SIM_TESTS.run_tests(SingleSimResolver(sim.sim_info)) or False
    ...
    def remove_fire_object(self, fire_object):
        if self._burning_objects:
            ...
            self._fire_quadtree = self._fire_objects or None
            self._alerted_sims = False
    ...
    def extinguish_nearby_fires(self, subject):
        ...
        nearby_sims = self._query_quadtree_for_sim(location, surface_id=surface_id,
          filter_type=(placement.ItemType.SIM_POSITION),
          radius=(self.FIRE_EXTNIGUISH_NEARBY_RADIUS))
    ...
    def _start_fire_brigade_situation(self):
        ...
        for fire_brigade_situation in self.FIRE_BRIGADE_SITUATION(resolver=resolver):
            ...
            situation_id = situation_manager.create_situation((fire_brigade_situation.situation), guest_list=guest_list,
              user_facing=False)
    ...
    def _create_fire_situation_on_sim(self, sim, situation_manager=None):
        if sim.id in self._situation_ids:
            ...
            situation_id = situation_manager.create_situation((self.FIRE_SITUATION), guest_list=guest_list,
              user_facing=False)
            ...
            if issubclass(type(resolver.interaction), self.START_PANIC_INTERACTION):
                ...
                for sim_on_lot in services.sim_info_manager().instanced_sims_on_active_lot_gen():
                    if sim_on_lot is not sim:
                        self.is_sim_in_fire_brigade_situation(sim) or self._push_fire_reaction_affordance(sim_on_lot, resolver.interaction.target)

        if event is TestEvent.SimActiveLotStatusChanged:
            if resolver.get_resolved_arg('on_active_lot'):
                sim = sim_info.get_sim_instance()
                if sim is not None:
                    situation_manager = services.get_zone_situation_manager()
                    if not self.is_sim_in_fire_brigade_situation(sim, situation_manager=situation_manager):
    ...
    def activate_fire_alarms(self):
        ...
        self._fire_alarm_alarm = alarms.add_alarm(self, time_span, (self._fire_alarm_callback),
          repeating=True, repeating_time_span=repeating_time_span)
    ...
    def _fire_alarm_callback(self, handle):
        ...
        for deactivated_fire_alarm in deactivated_fire_alarms:
            ...
            fires_in_range = self.query_quadtree_for_fire_object((sims4.math.Vector2(alarm_position.x, alarm_position.z)), radius=(self.FIRE_ALARM_ACTIVATION_RADIUS),
              surface_id=(deactivated_fire_alarm.routing_surface))
            if fires_in_range:
    ...
    def activate_sprinkler_system(self):
        ...
        if not self._sprinkler_system_objects:
            if not self._sprinkler_floor_objects:
                return
        ...
        self._sprinkler_alarm = alarms.add_alarm(self, time_span, (self._sprinkler_alarm_callback),
          repeating=True, repeating_time_span=repeating_time_span)
    ...
    def find_and_suppress_fires_under_sprinkler(self, sprinkler, initiating_fire, new_fire_objects, suppression_radius):
        ...
        fires_under_sprinkler = self.query_quadtree_for_fire_object(sprinkler_position, radius=suppression_radius,
          surface_id=sprinkler_surface)
        for fire in fires_under_sprinkler:
            if fire == initiating_fire or fire in new_fire_objects:
        ...
        sims_under_sprinkler = self._query_quadtree_for_sim(sprinkler_position, surface_id=sprinkler_surface,
          filter_type=(placement.ItemType.SIM_POSITION),
    ...
    def _suppress_fire(self, fire, immediate=False):
        ...
        if tracker is not None:
            if tracker.has_statistic(self.FIRE_STRENGTH_COMMODITY):
                stat = tracker.get_statistic(self.FIRE_STRENGTH_COMMODITY)
    ...
    def _create_or_replace_scorch_cleanup_alarm(self):
        ...
        self._scorch_cleanup_alarm = alarms.add_alarm(self, time_span_until, (self._cleanup_scorch_marks_on_terrain),
          repeating=True, repeating_time_span=repeat_length)
    ...
    def find_cleanable_scorch_mark_locations_within_radius(self, location, level, radius):
        ...
        for scorch_mark in all_scorch_marks:
            ...
            if scorch_level == level:
                scorch_location = build_buy.is_location_natural_ground(scorch_mark[0], scorch_level) or scorch_mark[0]
