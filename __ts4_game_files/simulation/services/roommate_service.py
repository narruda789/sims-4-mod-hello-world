...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\services\roommate_service.py
# Compiled at: 2021-06-09 16:34:41
# Size of source mod 2**32: 107557 bytes
...
import alarms, build_buy, persistence_error_types, routing, services, sims4.log, tag, traits.traits
...
class RoommateService(Service):
    ...
    EXCLUDE_BED_TAGS = TunableSet(description='\n        Tags that should be excluded from the determining how many beds a lot\n        supports, even if the object also has a tag specified in BED_TAGS.\n        ',
      tunable=TunableEnumWithFilter(tunable_type=(tag.Tag),
      default=(tag.Tag.INVALID),
      filter_prefixes=BED_PREFIX_FILTER))
    ...
    ROOMMATE_SIM_FILTERS = TunableList(description='\n        List of paired tests and simfilters.\n        The first simfilter whose test passes will be used to acquire (potential)\n        roommates.\n        ',
      tunable=TunableTuple(test=(TunableTestSet()),
    ...
    ROOMMATE_SITUATION = TunablePackSafeReference(description='\n        The situation that the roommate is in.\n        ',
      ...
      class_restrictions='RoommateSituation',
      verify_tunable_callback=_verify_tunable_callback)
    ...
    ADDITIONAL_CULLING_IMMUNITY = TunableTuple(description='\n        This number will boost Roommates when the culling\n        system scores this Sim. Higher the number, lower the probability \n        of this Sim being culled.\n        ',
      active_household=TunableRange(tunable_type=int,
      default=20,
      minimum=0),
      previously_played_household=TunableRange(tunable_type=int,
      default=10,
      minimum=0),
      unplayed_household=TunableRange(tunable_type=int,
      default=2,
      minimum=0),
      no_household=TunableRange(tunable_type=int,
      default=0,
      minimum=0))
    ...
    DECORATING = TunableTuple(description='\n        Tuning related to roommate decorations\n        ',
      ...
      default_upper=3,
      minimum=0),
      distance=TunableDistanceSquared(description='\n            Maxium distance from bed objects can be placed\n            ',
      default=5),
      decorations=TunableList(description='\n            Weighted list of decorations to be placed\n            ',
      tunable=TunableTuple(item=TunableVariant(description='\n                    A reference to the item.\n                    ',
      by_tag=(TagDecoration.TunableFactory()),
      by_definition=(DefinitionDecoration.TunableFactory()),
    ...
    LEAVING = TunableTuple(description='\n        Tuning related to roommate deciding to leave\n        ',
     ...
     RoommateLeaveReason.INVALID,)),
      ...
      tunable_type=LeaveReasonTestingTime,
      default=(LeaveReasonTestingTime.HOUSEHOLD_ROOMMATES_HOME_LOT),
      invalid_enums=(LeaveReasonTestingTime.UNTESTED))),
      enabled_by_default=True),
      time_till_warning=OptionalTunable(description='\n                    Required time for LeaveReason to continue to apply before\n                    displaying warning.\n                    ',
      tunable=(TunableTimeSpan())),
      warning_notification=OptionalTunable(description='\n                    The notification to display when sim has been wanting to\n                    leave for specified reason for longer than the time till\n                    warning. \n                    ',
      tunable=(UiDialogNotification.TunableFactory())),
      time_till_leave=TunableTimeSpan(description='\n                    Required time for LeaveReason to continue to apply before\n                    roommate leaves.\n                    '),
      ...
      tunable=(UiDialogNotification.TunableFactory()))),
      callback=_populate_leave_reasons))
    ARCHETYPE_CATEGORIES = TunableList(description='\n        Categories from which archetypes are selected.  The specified\n        number of archetypes will be randomly selected from each category.\n        ',
      ...
      default_upper=1,
      minimum=0),
      archetypes=TunableList(tunable=TunableRoommateArchetypeReference(pack_safe=True))))
    ...
    def trigger_interviews(self, enabled, show_tns=False):
        if enabled:
            ...
            self._interview_schedule_alarm = alarms.add_alarm(self, time_span, (lambda _: self._schedule_interviews()), repeating=True,
    ...
    def _process_continuing_leave_reason(self, sim_info, sim_data_reason, leave_reason, time_span):
        ...
        if not sim_data_reason.been_warned:
            if reason_data.time_till_warning is None or total_time >= reason_data.time_till_warning():
                ...
                if reason_data.warning_notification is not None:
                    ...
                    notification.show_dialog()
                    return False
        if total_time >= reason_data.time_till_leave():
            resolver = SingleSimResolver(sim_info)
        ...
        for leave_reason in LEAVE_REASONS[testing_time]:
            ...
            if leave_data.tests is None:
                if leave_reason in sim_data.leave_reasons:
                    if self._process_continuing_leave_reason(sim_info, sim_data.leave_reasons[leave_reason], leave_reason, span):
                        return True
    ...
    def _trigger_interview(self):
        ...
        situation_id = services.get_zone_situation_manager().create_situation((self.INTERVIEW.situation), guest_list=guest_list,
    ...
    def generate_organization_roommate(self, org_id, sim_filter, additional_filter_terms, blacklist_sims):
        ...
        if organization_service:
            ...
            results = organization_service.generate_organization_members(org_id, amount=1,
              blacklist_sims=blacklist_sims,
              additional_filter_terms=roommate_filter_terms)
            if results:
    ...
    def conform_potential_roommate(self, roommate_sim_info):
        ...
        for category in self.ARCHETYPE_CATEGORIES:
            ...
            while count > 0 and weighted_options:
                ...
                results = services.sim_filter_service().submit_matching_filter(sim_filter=sim_filter, callback=None,
                  sim_constraints=sim_constraints,
                  allow_yielding=False,
                  conform_if_constraints_fail=True,
                  gsi_source_fn=get_sim_filter_gsi_name)
                results or logger.error('Failed to conform potential roommate {} to filter {}', roommate_sim_info, sim_filter)
                return False
    ...
    def add_roommate(self, sim_info, zone_id, assign_bed=True):
        ...
        self._update_alarms()
        if self._interview_schedule_alarm:
            if self._zone_to_roommates[zone_id].available_beds <= 0:
                self.trigger_interviews(False, show_tns=True)
    ...
    def remove_all_roommates_in_zone(self, zone_id, filter_func=None):
        ...
        for sim_id in sim_ids:
            if not filter_func is None:
                filter_func(sim_id) or self._remove_roommate(sim_id)
    ...
    def _add_roommate_to_situation(self, sim_id):
        ...
        services.get_zone_situation_manager().create_situation((self.ROOMMATE_SITUATION), guest_list=guest_list,
          spawn_sims_during_zone_spin_up=True,
          user_facing=False)
    ...
    def on_all_households_and_sim_infos_loaded(self, client):
        for roommate_id in self.get_roommate_ids():
            ...
            if not (sim_info is None or sim_info.household.home_zone_id != 0 or sim_info.can_instantiate_sim):
        ...
        for zone_id, household_data in self._zone_to_roommates.copy().items():
            ...
            if not venue is None:
                if venue.is_university_housing or household_data.household_id != persistence_service.get_household_id_from_zone_id(zone_id) or household_data.household_id == 0:
    ...
    def _do_rent(self, *_):
        ...
        if not unpaid_sim_strings:
            ...
            if not unpaid_sim_strings:
                ...
                dialog.show_dialog(additional_tokens=(rent_per_sim,
                 total_rent,
                 roommate_count - len(unpaid_sim_strings),
    ...
    def _populate_roommate_bed_assignments(self, household, beds, zone_id):
        ...
        return (
         unassigned_roommates, single_occupied_beds, double_occupied_beds)
    ...
    def _remove_excess_roommates(self, beds, bed_delta, max_possible_new, zone_id, unassigned_roommates, double_occupied_beds, single_occupied_beds):
        while not unassigned_roommates or bed_delta < 0 or max_possible_new < 0:
        ...
        while bed_delta < 0:
            ...
            if max_possible_new <= min(bed_delta, -1) * 2 and double_occupied_beds:
                ...
                if single_occupied_beds:
                    dict_to_pop = single_occupied_beds
                else:
                    if double_occupied_beds:
                        dict_to_pop = double_occupied_beds
    ...
    def get_available_roommate_count(self, beds=None, remove_extra=False):
        ...
        if household is None:
            if beds is None:
                beds = services.object_manager().get_objects_matching_tags_with_exclusion((self.BED_TAGS), (self.EXCLUDE_BED_TAGS), match_any=True)
    ...
    def assign_bed(self, sim_id, avoid_id=None):
        ...
        object_preference_tracker = household.object_preference_tracker
        old_bed = None
        for bed in services.object_manager().get_objects_matching_tags_with_exclusion((self.BED_TAGS), (self.EXCLUDE_BED_TAGS), match_any=True):
            bed_id = bed.id
    ...
    def do_decorations(self):
        ...
        if zone_id not in self._zone_to_roommates:
            ...
            return slottable_objects or None
    ...
    def _handle_queued_lock_out(self):
        ...
        services.get_zone_situation_manager().create_situation((self.LOCKED_OUT_SITUATION.situation), guest_list=guest_list,
          spawn_sims_during_zone_spin_up=True,
          user_facing=False)
        ...
        if self._auto_invite_sim_infos:
            ...
            if travelled_auto_invite_sim_infos:
                ...
                if notified_sim:
                    ...
                    dialog = self.AUTO_INVITE_DIALOG(None, target_sim_id=(notified_sim.sim_id),
                      resolver=resolver)
                    dialog.show_dialog()
        self._auto_invite_sim_infos = None
        ...
        if zone_id in self._zone_to_roommates and self._zone_to_roommates[zone_id].sim_ids:
            if self._rent_schedule_alarm is None:
                ...
                if not venue.is_university_housing:
                    self._rent_schedule_alarm = self.RENT.schedule(start_callback=(self._do_rent), schedule_immediate=False,
                      cross_zone=True)
            ...
            if zone_id in self._zone_to_roommates:
                if self._reacquire_alarm_handle is None:
                    self._reacquire_alarm_handle = alarms.add_alarm(self, (self.OFFLOT_ALARM_INTERVAL()),
                      (self._reacquire_roommates),
                      repeating=True)
            else:
                self._reacquire_alarm_handle = None