...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\animation\posture_manifest.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 50147 bytes
...
import enum, services, sims4.log, sims4.resources
...
FULL_BODY = 'FullBody'
_NOT_SPECIFIC_ACTOR = (
 None, MATCH_ANY, MATCH_NONE, HOLSTER)
_NOT_SPECIFIC_ACTOR_OR_NONE = (None, MATCH_ANY, HOLSTER)
MATCH_NONE_POSTURE_PARAM = ''
_NOT_SPECIFIC_POSTURE = (
 None, MATCH_NONE_POSTURE_PARAM, MATCH_ANY)
SORT_ORDER = {MATCH_NONE: 0, 
 UPPER_BODY: 1, 
 FULL_BODY: 2, 
 HOLSTER: 4, 
 MATCH_ANY: 5}
...
TYPE_ORDER = [
 ...
 (
  str, 2, None),
 (
  enum.Int, 3, lambda obj: (id(type(obj)), obj)),
 (
  type, 4, lambda obj: obj.__name__),
 (
  object, 5, lambda obj: obj.id)]
    ...
    if hasattr(value0, 'id'):
        if hasattr(value1, 'id'):
            if value0.id == value1.id:
                if value0.is_part:
                    if not value1.is_part:
                        return (
                         None, value0)
                if value1.is_part:
                    if not value0.is_part:
                        return (
                         None, value1)
                return (False, None)
            return (False, None)
...
PostureManifestOverrideKey = namedtuple('PostureManifestOverrideKey', ('actor', 'specific',
                                                                       'family',
                                                                       'level'))
PostureManifestOverrideValue = namedtuple('PostureManifestOverrideValue', ('left',
                                                                           'right',
                                                                           'surface'))
...
class PostureManifest(PostureManifestBase, set):
    ...
    def get_clean_manifest(self):
        immutable_self = FrozenPostureManifest((entry.intern() for entry in self if entry.posture_type_specific or entry.posture_type_family))
        return immutable_self.intern()
...
_PostureManifestEntry = namedtuple('_PostureManifestEntry', ('actor', 'specific', 'family',
                                                             'level', 'left', 'right',
                                                             'surface', 'provides',
...
class PostureManifestEntry(_PostureManifestEntry, InternMixin):
    ...
    def apply_actor_map(self, actor_map):
        ...
        return self.__class__(actor, (self.specific), (self.family), (self.level), left,
          right, surface, (self.provides), target_object_filter=(self.target_object_filter))

    def clone(self, **overrides):
        return (self._replace)(**overrides)

    def matches_override_key(self, override_key: PostureManifestOverrideKey):
        actor, specific, family, level = override_key
        if actor != MATCH_ANY:
            if actor != MATCH_NONE or self.actor is not None:
                if self.actor != actor:
                    return False
        if specific != MATCH_ANY:
            if specific != MATCH_NONE or self.specific is not None:
                if self.specific != specific:
                    return False
        if family != MATCH_ANY:
            if family != MATCH_NONE or self.family is not None:
                if self.family != family:
                    return False
        if level != MATCH_ANY:
            if level != MATCH_NONE or self.level is not None:
                if self.level != level:
                    return False
        return True

    def get_entries_with_override(self, override_value: PostureManifestOverrideValue):
        left, right, surface = override_value
        if surface is None:
            surface = self.surface
        if self.left == self.right:
            if left not in _NOT_SPECIFIC_ACTOR:
                if right not in _NOT_SPECIFIC_ACTOR:
                    return [
                     self._with_overrides(surface=surface, left=left),
                     self._with_overrides(surface=surface, right=right)]
        ...
        if (right is None or self.right) == MATCH_ANY:
            if right != MATCH_NONE:
                right = self.right
        return [
         self._with_overrides(surface=surface, left=left, right=right)]
    ...
    def __str__(self):
        if self.actor not in _NOT_SPECIFIC_ACTOR:
            format_str = '{0.actor}:{0.posture_param_value_complete}({0.left},{0.right},{0.surface})({0.target_object_filter})'
        else:
            format_str = '{0.posture_param_value_complete}({0.left},{0.right},{0.surface})({0.target_object_filter})'
        return format_str.format(self)

    @staticmethod
    def _intersect_attr(value0, value1):
        if not value0 == MATCH_ANY:
            if value0 == HOLSTER:
                return value1
            ...
            if value0 == value1:
                return value0
            if not isinstance(value0, (str, int, Definition)):
                if not isinstance(value1, (str, int, Definition)):
                    if value0.is_part:
                        if value1.parts:
                            if value0 in value1.parts:
                                return value0
                    if value1.is_part and value0.parts and value1 in value0.parts:
                        return value1
        else:
            if isinstance(value0, int):
                if value1 != MATCH_NONE:
                    return value1
    ...
    def _sort_key(self):
        return (
         SORT_ORDER.get(self.actor, SORT_ORDER_DEFAULT), _lt_get_type_order(self.actor),
         SORT_ORDER.get(self.level, SORT_ORDER_DEFAULT), _lt_get_type_order(self.level),
         SORT_ORDER.get(self.specific, SORT_ORDER_DEFAULT), _lt_get_type_order(self.specific),
         SORT_ORDER.get(self.family, SORT_ORDER_DEFAULT), _lt_get_type_order(self.family),
         SORT_ORDER.get(self.left, SORT_ORDER_DEFAULT), _lt_get_type_order(self.left),
         SORT_ORDER.get(self.right, SORT_ORDER_DEFAULT), _lt_get_type_order(self.right),
         SORT_ORDER.get(self.surface, SORT_ORDER_DEFAULT), _lt_get_type_order(self.surface),
         SORT_ORDER.get(self.provides, SORT_ORDER_DEFAULT), _lt_get_type_order(self.provides))
    ...
    def intersect(self, other_manifest_entry):
        ...
        if self.target_object_filter is MATCH_ANY:
            init_kwargs['target_object_filter'] = other_manifest_entry.target_object_filter
        else:
            if other_manifest_entry.target_object_filter is MATCH_ANY:
                init_kwargs['target_object_filter'] = self.target_object_filter
            else:
                init_kwargs['target_object_filter'] = self.target_object_filter.intersect(other_manifest_entry.target_object_filter)
        ...
        if self.specific == MATCH_ANY:
            ...
            if other_manifest_entry.specific == MATCH_ANY:
        ...
        if self.family == MATCH_ANY:
            ...
            if other_manifest_entry.family == MATCH_ANY:
                posture_type_family1 = posture_type_family0
            elif posture_type_specific0 != posture_type_specific1:
                ...
                if posture_type_family0 is None and posture_type_family1 is None:
                    ...
                    if posture_type_family0 == posture_type_family1:
                        ...
                        if posture_type_family0 is not None and posture_type_family1 is not None:
                            return
        if posture_type_family0 is not None and provides0:
            init_kwargs.family = posture_type_family0.name
        else:
            if posture_type_family1 is not None and provides1:
                ...
                if not provides0:
                    ...
                    if init_kwargs.specific:
                        if init_kwargs.family:
                            if posture_type_specific.family_name != init_kwargs.family:
                                return
                    result = (self.__class__)(**init_kwargs)
                    return result.valid or None
                return result
    ...
    @property
    def compatible_posture_types(self):
        p0 = self.posture_type_specific
        p1 = self.posture_type_family
        if p0 is not None:
            if p1 is not None:
        ...
        while open_set:
            ...
            for entry1 in open_set:
                if not entry0.actor == entry1.actor:
                    if entry0.target == entry1.target or entry0.slot == entry1.slot:
...
class _SlotManifestEntrySingle(namedtuple('_SlotManifestEntrySingle', ('actor', 'target', 'slot'))):
    ...
    def slot_types(self):
        ...
        if isinstance(slot, type):
            if issubclass(slot, SlotType):
                return {
                 slot}
        if isinstance(slot, int):
            return {
             get_slot_type_for_bone_name_hash(slot)}

    def _sort_key(self):
        return (
         SORT_ORDER.get(self.actor, SORT_ORDER_DEFAULT), _lt_get_type_order(self.actor),
         SORT_ORDER.get(self.target, SORT_ORDER_DEFAULT), _lt_get_type_order(self.target),
         SORT_ORDER.get(self.slot, SORT_ORDER_DEFAULT), _lt_get_type_order(self.slot))
...
class SlotManifestEntry(tuple, InternMixin):
    ...
    def __reduce__(self):
        return (
         self.__class__, (self.actor, self.target, self.slot, self[1:]))
    ...
    def from_entries(cls, entries):
        return cls(*entries[0], **{'additional_entries': entries[1:]})
        ...
        for other_entry in other_entries:
            ...
            for i, self_entry in enumerate(self_entries):
                if self_entry.target == other_entry.target:
                    ...
                    if self_entry.target == MATCH_ANY:
                        target = other_entry.target
                    else:
                        if other_entry.target == MATCH_ANY:
                            target = self_entry.target
                        else:
                ...
                if self_entry.actor == other_entry.actor:
                    if not self_entry.slot_types.intersection(other_entry.slot_types):
                        ...
                        needs_add = False
                        continue
                    elif self_entry.slot_types.intersection(other_entry.slot_types):
                        return
