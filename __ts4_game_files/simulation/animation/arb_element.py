...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\animation\arb_element.py
# Compiled at: 2021-01-28 14:28:10
# Size of source mod 2**32: 29328 bytes
import _math
...
import animation, distributor.ops, element_utils, elements, gsi_handlers, services, sims4.log
...
class ArbElement(distributor.ops.ElementDistributionOpMixin, elements.SubclassableGeneratorElement):
    ...
    def _log_event_records(self, log_fn, only_errors):
        ...
        if not (errors_found or only_errors):
                ...
                if sleep:
                    if timeline is not None:
                        ...
                        if self._duration_must_run or self._duration_repeat:
                            animation_sleep_element = AnimationSleepElement((self._duration_must_run),
                              (self._duration_interrupt), (self._duration_repeat),
                              enable_optional_sleep_time=(self.enable_optional_sleep_time),
                              arbs=(
                             self.arb,))
                if gsi_handlers.animation_archive_handlers.archiver.enabled or gsi_handlers.animation_archive_handlers.object_archiver.enabled:
                    gsi_handlers.animation_archive_handlers.archive_animation_request(self.arb)
            if animation_sleep_element is not None:
                yield from services.current_zone().animate_instantly or element_utils.run_child(timeline, animation_sleep_element)
    ...
    def _event_handler_client_location_capture(self, event_data):
        ...
        early_out, obj = get_animation_object_for_event(event_data,
          'target_actor_id', 'Client-only location target', asms=asms)
    ...
    def _event_handler_snap(self, event_data):
        asms = list(self._get_asms_from_arb_request_info())
        early_out, object_to_snap = get_animation_object_for_event(event_data,
          'event_actor_id', 'object to be snapped', asms=asms)
        if early_out is not None:
            return
        ...
        if snap_actor_namespace is None:
            ...
            early_out, snap_reference_object = get_animation_object_for_event(event_data,
              'snap_actor_id', 'snap reference object', asms=asms)
            if early_out is not None:
                return
                v = event_data.event_data['snap_translation']
                q = event_data.event_data['snap_orientation']
            ...
            elif object_to_snap in self._client_location_captures:
    ...
    def _event_handler_parent(self, event_data):
        ...
        early_out, child_object = get_animation_object_for_event(event_data,
          'parent_child_id', 'child', asms=asms)
        if early_out is not None:
            return
    ...
    def _event_handler_visibility(self, event_data):
        ...
        asms = self._get_asms_from_arb_request_info()
        early_out, target_object = get_animation_object_for_event(event_data,
          'target_actor_id', 'target', asms=asms, allow_obj=False)
        if early_out is not None:
            early_out, target_object = get_animation_object_for_event(event_data,
              'target_actor_id', 'target', asms=asms, allow_prop=False)
            if early_out is not None:
                return
    ...
    def _event_handler_update_flipbook(self, event_data):
        asms = list(self._get_asms_from_arb_request_info())
        early_out, target = get_animation_object_for_event(event_data,
          'event_actor_id', 'object to be snapped', asms=asms)
        if early_out is not None:
            return
