...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\animation\animation_overrides_tuning.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 5332 bytes
import animation.posture_manifest, services, sims4.resources
...
class TunableParameterMapping(TunableMapping):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(key_name='name', 
         value_type=TunableVariant(default='string',
  boolean=(Tunable(bool, False)),
  string=(Tunable(str, 'value')),
  integral=(Tunable(int, 0))), **kwargs)
...
class TunablePostureManifestOverrideKey(TunableSingletonFactory):
    ...
    def __init__(self, asm_source=None):
        if asm_source is not None:
            ...
            source_query = None
        super().__init__(actor=TunablePostureManifestCellValue(False, 'actor_name', asm_source=asm_source, source_query=source_query), specific=(TunablePostureManifestCellValue(False, 'posture_name', 'stand')),
          family=(TunablePostureManifestCellValue(False, 'posture_name', 'stand')),
          level=(TunablePostureManifestCellValue(False, 'overlay_level', 'FullBody')))
...
class TunablePostureManifestOverrideValue(TunableSingletonFactory):
    ...
    def __init__(self, asm_source=None):
        if asm_source is not None:
            ...
            source_query = None
        super().__init__(left=TunablePostureManifestCellValue(True, 'actor_name', asm_source=asm_source, source_query=source_query), right=TunablePostureManifestCellValue(True, 'actor_name', asm_source=asm_source, source_query=source_query),
          surface=TunablePostureManifestCellValue(True, 'actor_name', 'surface', asm_source=asm_source, source_query=source_query))


RequiredSlotOverride = namedtuple('RequiredSlotOverride', ('actor_name', 'parent_name',
                                                           'slot_type'))
...
class TunableRequiredSlotOverride(TunableSingletonFactory):
    ...
    def __init__(self, asm_source=None):
        if asm_source is not None:
            ...
            source_query = None
        super().__init__(actor_name=Tunable(str, None, source_location=asm_source, source_query=source_query), parent_name=Tunable(str, 'surface', source_location=asm_source, source_query=source_query),
          slot_type=(TunableReference(services.get_instance_manager(sims4.resources.Types.SLOT_TYPE))))