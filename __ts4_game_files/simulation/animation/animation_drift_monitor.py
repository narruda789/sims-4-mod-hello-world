...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\animation\animation_drift_monitor.py
# Compiled at: 2019-09-20 16:26:07
# Size of source mod 2**32: 17529 bytes
...
def _animation_drift_monitor_archive(record):
    ...
    archive_data = {'record_name':record.record_name, 
     'timestamp_sleep_start':'{} / {}'.format(_format_datetime(record.timestamp_sleep_start), _format_datetime(record.timestamp_sleep_sim_start)), 
     'timestamp_sleep_end':_format_datetime(record.timestamp_sleep_end), 
     'timestamp_client_completed':_format_datetime(record.timestamp_client_completed), 
     'duration_expected':_format_interval(record.duration_expected), 
     'duration_sleep':_format_interval(record.timestamp_sleep_end - record.timestamp_sleep_start), 
     'duration_client':'{} / {}'.format(_format_interval(record.duration_client - record.duration_offset_client), _format_interval(record.duration_offset_client)), 
     'duration_drift_server':_format_interval(record.duration_drift_server), 
     'duration_drift':_format_interval(record.timestamp_client_started - record.timestamp_sleep_sim_start), 
     'Resources':[{'resource_name':str(resource),  'resource_animation_properties':'Blocker' if resource in relevant_objects else 'Non-blocker',  'resource_master_controller_properties':'Required' if resource.id in record.resource_ids else 'Not required',  'resource_time_debt':_format_interval(arb_accumulator_service.get_time_debt((resource,))),  'resource_added_time_debt':_format_interval(record.additional_actor_debt if resource in relevant_objects else 0)} for resource in all_resources], 
...
def build_animation_drift_monitor_sequence(animation_sleep_element, sleep_element):
    sequence = build_element((
     lambda _: _animation_drift_monitor_start_sleep_element(animation_sleep_element),
     sleep_element))
    return sequence
