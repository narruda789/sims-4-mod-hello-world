...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\weather\weather_tests.py
# Compiled at: 2020-11-24 23:35:22
# Size of source mod 2**32: 17148 bytes
...
class WeatherTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
  ...
  default=False), **kwargs)
    ...
    FACTORY_TUNABLES = {'temperature':OptionalTunable(TunableTuple(temperature=TunableEnumSet(description='\n                    The temperature(s) we must be in to pass the test\n                    ',
       ...
       tunable_type=bool,
       default=False))), 
     'precipitation':OptionalTunable(TunableWeatherTestTuple(description='\n                Specify amount and type of precipitation\n                ',
     ...
     'lightning':OptionalTunable(Tunable(description='\n                If checked must be lightning.\n                If unchecked must not be.\n                ',
       tunable_type=bool,
       default=True)), 
     'icy_conditions':OptionalTunable(Tunable(description='\n                If checked must be Icy Conditions.\n                If unchecked must not be.\n                ',
       tunable_type=bool,
       default=True)), 
     'thunder':OptionalTunable(Tunable(description='\n                If checked must be thundering.\n                If unchecked must not be.\n                ',
       tunable_type=bool,
       default=True)), 
     'water_freeze':OptionalTunable(TunableWeatherTestTuple(description='\n                Specify amount water (ponds, etc.) is frozen\n                ')), 
     ...
     'cloud_state':OptionalTunable(TunableTuple(whitelist=TunableEnumSet(description='\n                    If any are specified, the value of one must be > the\n                    whitelist threshold for the test to pass.\n                    \n                    If weather_service is not running, test will pass if\n                    partly_cloudy is in white list (or whitelist is empty) but \n                    not black list.\n                    ',
       enum_type=CloudType),
       whitelist_threshold=Tunable100ConvertRange(description='\n                    The value of a whitelist cloudtype must be above this \n                    threshold to pass.\n                    ',
       default=0,
       minimum=0,
       maximum=100),
       blacklist=TunableEnumSet(description='\n                    If any are specified, the value of all must be <= the\n                    blacklist threshold for the test to pass.\n                    \n                    If weather_service is not running, test will pass if\n                    partly_cloudy is in white list (or whitelist is empty) but \n                    not black list.\n                    ',
       enum_type=CloudType),
       blacklist_threshold=Tunable100ConvertRange(description='\n                    The maximum value for each of the blacklist cloud types for \n                    the test to pass.\n                    ',
       default=0,
       minimum=0,
       maximum=100))), 
     'ground_cover':OptionalTunable(TunableWeatherTestTuple(description='\n                Specify amount and type of ground cover\n                ',
       minimum_range=(-100),
       cover_type=TunableEnumEntry(description='\n                    The type of precipitation we are testing\n                    ',
    ...
    def __call__(self):
        ...
        elif self.cloud_state is not None:
            if weather_service is None:
                if self.cloud_state.whitelist:
                    if CloudType.PARTLY_CLOUDY not in self.cloud_state.whitelist:
        ...
        if self.precipitation is not None:
            ...
            if precip == 0:
                return self.precipitation.zero_is_true or TestResult(False, 'Must not be 0 precipitation', tooltip=(self.tooltip))
            else:
                if precip not in self.precipitation.range:
        ...
        if self.icy_conditions is not None:
            icy_conditions = self._get_processed_weather_element_value(WeatherEffectType.SNOW_ICINESS, weather_service, time)
            if icy_conditions == 0:
                if self.icy_conditions:
                    return TestResult(False, 'Must not be 0 Icy conditions', tooltip=(self.tooltip))
            elif not self.icy_conditions:
                return TestResult(False, 'Must be 0 conditions', tooltip=(self.tooltip))
        ...
        if self.water_freeze is not None:
            ...
            if freeze == 0:
                return self.water_freeze.zero_is_true or TestResult(False, 'Must not be 0 water freeze', tooltip=(self.tooltip))
            else:
                if freeze not in self.water_freeze.range:
        ...
        if self.wind is not None:
            ...
            if wind == 0:
                return self.wind.zero_is_true or TestResult(False, 'Must not be 0 wind', tooltip=(self.tooltip))
            else:
                if wind not in self.wind.range:
        ...
        if self.ground_cover is not None:
            ...
            if cover == 0:
                return self.ground_cover.zero_is_true or TestResult(False, 'Must not be 0 ground cover', tooltip=(self.tooltip))
            else:
                if cover not in self.ground_cover.range:
...
class WeatherTypeTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
        ...
        if weather_service is None:
            if WeatherType.UNDEFINED in self.weather_types:
                if self.invert:
                    ...
                    return self.invert or TestResult(False, "No WeatherService: WeatherType must be UNDEFINED and isn't", tooltip=(self.tooltip))
        else:
            if weather_service.has_any_weather_type(self.weather_types):
                if self.invert:
    ...
    def qualifies_for_cache_clear(self):
        ...
        if weather_service is None:
            return False
        return weather_service.has_weather_changed()
