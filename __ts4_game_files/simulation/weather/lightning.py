...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\weather\lightning.py
# Compiled at: 2020-04-20 19:43:26
...
import build_buy, clock, objects, placement, routing, services, sims4.log, sims4.math, sims4.random
...
class LightningStrike:
    ...
    def perform_active_lightning_strike():
        ...
        weighted_strike_fns = ((lightning_weights.weight_terrain, LightningStrike.strike_terrain),
         (
          lightning_weights.weight_object, LightningStrike.strike_object),
         (
          lightning_weights.weight_sim, LightningStrike.strike_sim))
    ...
    def _get_terrain_position_and_routing_surface_for_lightning_strike():
        ...
        while count:
            position, routing_surface = build_buy.is_location_outside(position, routing_surface.secondary_id) or _get_random_position_and_routing_surface()
            count -= 1
    ...
    def strike_sim(sim_to_strike=None):
        ...
        if sim_to_strike is None:
            ...
            specific_sim = True
        if sim_to_strike is not None:
            if lightning_strike_tuning.affordance is not None:
                context = InteractionContext(sim_to_strike, (InteractionSource.SCRIPT), priority=(Priority.Critical),
