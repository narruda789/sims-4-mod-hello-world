...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\weather\weather_forecast.py
# Compiled at: 2020-10-23 18:18:19
...
class WeatherForecast(HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(Types.WEATHER_FORECAST)):
    INSTANCE_TUNABLES = {'calendar_icon':TunableIcon(description='\n            The small icon for this forecast.\n            ',
     ...
     'calendar_icon_large':TunableIcon(description='\n            The large icon for this forecast.\n            ',
       export_modes=ExportModes.All), 
     'calendar_icon_mascot':TunableIcon(description='\n            Optional icon to use as the forecast mascot in the calendar.\n            ',
       allow_none=True,
       export_modes=ExportModes.All), 
     'forecast_description':TunableLocalizedString(description='\n            The description for this forecast.\n            ',
       export_modes=ExportModes.All), 
     'forecast_name':TunableLocalizedString(description='\n            The name for this forecast.\n            ',
       export_modes=ExportModes.All), 
     'prescribed_weather_type':OptionalTunable(description='\n            The types of prescribed weather this forecast counts as\n            ',
       tunable=TuningPrescribedWeatherType()), 
     'weather_event_time_blocks':TunableMapping(description='\n            The weather events that make up this forecast.  Key is hour of day\n            that event would start, value is a list of potential events\n            ',
       ...
       tunable_type=int,
       default=1)))), 
     'weather_ui_override':TunableMapping(description='\n            If set, this overrides the weather type that is shown for the\n            specified group.\n            ',
       key_type=TunableEnumEntry(tunable_type=WeatherTypeGroup,
       default=(WeatherTypeGroup.UNGROUPED),
       invalid_enums=(
      WeatherTypeGroup.UNGROUPED,)),
       value_type=TunableEnumEntry(tunable_type=WeatherType,
    ...
    def get_weather_event(cls, options, thunder_snow_storms_option):
        ...
        if chosen_weather_event is None:
            return (None, None)
        return (
         chosen_weather_event.weather_event, chosen_weather_event.duration.random_float())
...
TunableWeatherSeasonalForecastsReference, TunableWeatherSeasonalForecastsSnippet = define_snippet('weather_seasonal_forecasts', TunableMapping(key_type=TunableEnumEntry(description='\n            The part of the season.\n            ',
  ...
  value_type=TunableWeatherForecastListReference(description='\n            Potential forecasts for this part of the season.\n            ',
  pack_safe=True)))
...
def is_prescribed_weather_supported(prescribed_weather_type, options, thunder_snow_storms_enabled, snow_safe=True, rain_safe=True):
    if prescribed_weather_type is None:
        return True
        if prescribed_weather_type.rain:
            if rain_safe:
                if options[PrecipitationType.RAIN] == WeatherOption.WEATHER_DISABLED:
                    return False
        if prescribed_weather_type.snow:
            if snow_safe:
                if options[PrecipitationType.SNOW] == WeatherOption.WEATHER_DISABLED:
                    return False
        if prescribed_weather_type.storm:
            if rain_safe:
                if options[PrecipitationType.RAIN] == WeatherOption.DISABLE_STORMS:
                    return False
    else:
        if prescribed_weather_type.blizzard:
            if snow_safe:
                if options[PrecipitationType.SNOW] == WeatherOption.DISABLE_STORMS:
                    return False
        if prescribed_weather_type.thunder_snow_storms:
            return snow_safe and thunder_snow_storms_enabled or False
    return True