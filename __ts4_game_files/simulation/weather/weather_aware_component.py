...
# Compiled at: 2020-04-20 18:05:42
  ...
  pack_safe=True))), **kwargs)
        ...
        def load_etree_node(self, node, source, expect_error):
            ...
            for weather_type, loots in value.items():
                if loots.start_loot or loots.end_loot:
    ...
    def provide_route_events(self, route_event_context, sim, path, start_time=0, end_time=0, **kwargs):
        if not self.enabled:
                ...
                if not force_no_carry:
                    ...
                    if node is prev_node:
                        ...
                        if not force_no_carry:
                            ...
                            is_curr_point_outside = is_location_outside(transform.translation, level)
                            if is_prev_point_outside is None:
                                is_prev_point_outside = is_curr_point_outside
                                prev_time = time
                                continue
                            if can_carry_umbrella and not added_enter_carry:
                                ...
                                if not added_exit_carry:
                                    if not is_curr_point_outside:
                                        if is_prev_point_outside or force_no_carry:
            ...
            if self._safety_umbrella_putaway_event is None:
                location = path.final_location
                if not is_location_outside(location.transform.translation, location.routing_surface.secondary_id):
