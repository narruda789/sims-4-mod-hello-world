...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\privacy.py
# Compiled at: 2019-09-11 13:53:51
# Size of source mod 2**32: 44797 bytes
...
from sims4.tuning.tunable import TunableFactory, Tunable, TunableReference, OptionalTunable, TunableTuple, TunableRange, TunableList, TunableEnumFlags
...
import enum, placement, routing, services, sims4.geometry, sims4.log, snippets
    ...
    def check_for_late_violators(self, sim):
        for privacy in self.privacy_instances:
            if not privacy.privacy_violators & PrivacyViolators.SIM:
                continue
            if sim in privacy.violators or sim in privacy.late_violators:
                continue
            if privacy.is_sim_shoo_exempt(sim):
                privacy.persistent_instance or privacy.add_exempt_sim(sim)
                continue
...
class Privacy(LineOfSight):
    ...
    def _is_sim_allowed(self, sim):
        if self._tests:
            ...
            if self._tests:
                if self._tests.run_tests(resolver):
                    return True
    ...
    def build_privacy(self, target=None):
        ...
        if self.central_object is None:
            ...
            for poly in self.constraint.geometry.polygon:
                self._privacy_constraints.append(PolygonFootprint(poly, routing_surface=routing_surface, cost=(self.privacy_discouragement_cost),
                  footprint_type=(FootprintType.FOOTPRINT_TYPE_PATH),
                  enabled=True))
        ...
        if vehicle.objectrouting_component is None:
            ...
            if self._vehicle_tests is not None:
                ...
                if self._vehicle_tests.run_tests(resolver):
                    return False
            return self.intersects_with_object(vehicle) or False
        return True
    ...
    def find_violating_sims(self, consider_exempt=True):
        if not self.is_active:
            ...
            return self.privacy_violators & PrivacyViolators.SIM or []
        check_all_surfaces_on_level = not self._routing_surface_only
        nearby_sims = placement.get_nearby_sims_gen((self.central_object.position), (self._routing_surface), radius=(self.max_line_of_sight_radius),
          exclude=(self._allowed_sims),
          only_sim_position=True,
          check_all_surfaces_on_level=check_all_surfaces_on_level)
        violators = []
    ...
    def _add_overrides_and_constraints_if_needed(self, violating_sims):
        ...
        for sim in violating_sims:
            ...
            liabilities = (
             (
              SHOO_LIABILITY, ShooLiability(self, sim)),)
    ...
    def _route_sim_away(self, sim, liabilities=()):
        ...
        result = sim.push_super_affordance(BuildAndForceSatisfyShooConstraintInteraction, None, context, liabilities=liabilities, privacy_inst=self,
    ...
    def handle_late_violator(self, sim):
        ...
        liabilities = (
         (
          LATE_SHOO_LIABILITY, LateShooLiability(self, sim)),)
        result = self._route_sim_away(sim, liabilities=liabilities)
        if not result:
            return
        if not self._violators:
            ...
            if not result:
                if not services.sim_spawner_service().sim_is_leaving(sim):
...
class PrivacyViolators(enum.IntFlags):
    SIM = 1
    VEHICLES = 2
...
class TunablePrivacy(TunableFactory):
  ...
  enum_type=PrivacyViolators,
  allow_no_flags=True,
  default=(PrivacyViolators.SIM | PrivacyViolators.VEHICLES)), 
         verify_tunable_callback=TunablePrivacy.verify_tunable_callback, 
