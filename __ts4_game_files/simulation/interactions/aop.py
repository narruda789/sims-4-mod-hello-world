...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\aop.py
# Compiled at: 2017-12-08 20:46:19
# Size of source mod 2**32: 19860 bytes
from weakref import WeakKeyDictionary
from animation.posture_manifest_constants import SIT_NO_CARRY_ANY_SURFACE_CONSTRAINT, SIT_INTIMATE_CONSTRAINT
...
import caches, elements, interactions.constraints, services, sims4.commands, sims4.log, telemetry_helper
...
class AffordanceObjectPair:
    ...
    def constraint_intersection(self, sim=DEFAULT, target=DEFAULT, posture_state=DEFAULT, participant_type=ParticipantType.Actor, include_super_affordance_constraint=False, **kwargs):
        ...
        constraint = (self.affordance.constraint_intersection)(sim=sim, target=self.target, posture_state=posture_state, 
         participant_type=participant_type, **kwargs)
        if include_super_affordance_constraint:
            si_constraint = (self.super_affordance.constraint_intersection)(sim=sim, target=self.target, posture_state=posture_state, 
             participant_type=participant_type, **kwargs)
    ...
    def test(self, context, skip_safe_tests=DEFAULT, **kwargs) -> TestResult:
        if self.super_interaction is not None:
            if not self.super_interaction.can_run_subinteraction(self, context=context):
        ...
        if interaction:
            ...
            if interaction.affordance.immediate:
                ...
                if immediate_timeline.heap:
                    logger.error('On immediate execute_interaction of {}, immediate timeline is not empty, heap:{}', interaction, immediate_timeline.heap)
    ...
    def interaction_factory(self, context) -> ExecuteResult:
        ...
        if si is not None:
            if not si.can_run_subinteraction(self, context):
    ...
    def compatible_with_current_posture_state(self, sim):
        ...
        if self.affordance.is_social and self.target is not None and self.target.is_sim:
            ...
            target_sim_constraint = self.constraint_intersection(sim=target_sim, posture_state=None, participant_type=(ParticipantType.TargetSim),
              include_super_affordance_constraint=True)
            listener_constraint = self.constraint_intersection(sim=target_sim, posture_state=None, participant_type=(ParticipantType.Listeners),
              include_super_affordance_constraint=True)
            ...
            if not sim.posture.mobile:
                ...
                if sim_social_constraint.valid:
                    if not (sim_social_constraint.geometry.contains_point(target_sim.position) and sim.can_see(target_sim)):
                        return False
            if sim_constraint.intersect(SIT_INTIMATE_CONSTRAINT).valid:
                if self.target is not None:
                    if self.target.is_sim:
                        if sim.posture.target is not None:
                            if self.target.posture.target is not None:
                                if sim.posture.target.is_part:
                                    if target_sim.posture.target.is_part:
                                        if sim.posture.target in list(target_sim.posture.target.adjacent_parts_gen()):
                                            if sim_posture_state_constraint.intersect(SIT_NO_CARRY_ANY_SURFACE_CONSTRAINT).valid:
                                                if target_posture_state_constraint.intersect(SIT_NO_CARRY_ANY_SURFACE_CONSTRAINT).valid:
                                                    return True
            if sim_constraint.intersect(SIT_NO_CARRY_ANY_SURFACE_CONSTRAINT).valid and self.target is not None and self.target.is_sim and sim_posture_state_constraint.intersect(SIT_INTIMATE_CONSTRAINT).valid:
                if target_posture_state_constraint.intersect(SIT_INTIMATE_CONSTRAINT).valid:
                    return True
        else:
            if not sim.posture.mobile:
            ...
            return sim_intersection.valid or False
        if self.affordance.is_social:
            if self.target is not None:
                if self.target.is_sim:
