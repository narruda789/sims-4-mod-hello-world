...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\constraint_variants.py
# Compiled at: 2020-04-02 15:18:54
# Size of source mod 2**32: 6618 bytes
...
class TunableGeometricConstraintVariant(TunableVariant):
    ...
    def __init__(self, constraint_locked_args=frozendict(), circle_locked_args=frozendict(), disabled_constraints=(), default='circle', **kwargs):
        ...
        available_constraints = {'facing':TunableFacing(description='\n                Existential tunable that requires the sim to face the object.\n                '), 
         ...
         'spawn_points':TunableSpawnPoint(description='\n                A constraint that represents all of the spawn locations on the lot.\n                '), 
         'spawn_points_with_backup':TunableSpawnPointWithBackup(description='\n                A constraint that represents primary spawn points with backup secondary ones.\n                '), 
         'relative_circle':RelativeCircleConstraint.TunableFactory(locked_args=constraint_locked_args), 
         'current_position':CurrentPosition.TunableFactory(), 
         'portal':PortalConstraint.TunableFactory(), 
         'position':TunablePosition(description='\n                The relative position geometry required for a sim/posture to use the object.\n                ',
           relative_position=sims4.math.Vector3(0, 0, 0)), 
         'water_depth':WaterDepthConstraint.TunableFactory(), 
         'water_depth_interval':WaterDepthIntervalConstraint.TunableFactory(), 
         'terrain_material':TerrainMaterialConstraint.TunableFactory(), 
         'ocean_loc':TunableOceanStartLocationConstraint(description='\n                The circle geometry relative to the nearest ocean locator.',
           radius=1,
           locked_args=circle_locked_args), 
         'default':default}
...
class TunedConstraintSet(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def constraints_override(disabled_constraints, default):
        return {'constraints': TunableList(description='\n                A set of constraints, of which one must be valid for this set\n                to be valid.\n                ',
                          minlength=1,
                          tunable=TunableGeometricConstraintVariant(disabled_constraints=disabled_constraints, default=default))}
...
class TunableConstraintVariant(TunableGeometricConstraintVariant):
    ...
    def __init__(self, disabled_constraints=frozenset(), default='circle', **kwargs):
        (super().__init__)(posture=PostureConstraintFactory.TunableFactory(), 
         welcome=TunableWelcomeConstraint(description='\n                A constraint that requires the sim be at the object with the highest scoring Welcome Component\n                ',
  radius=1), 
         front_door=TunableFrontDoorConstraint(), 
         jig=JigConstraint.TunableFactory(), 
         animation=TunableAnimationConstraint(), 
         routing_slot=TunableRoutingSlotConstraint(), 
         object_placement=ObjectPlacementConstraint.TunableFactory(), 
         fire_or_lot_facing=TunableFireOrLotFacingConstraint(), 
         plex=PlexConstraint.TunableFactory(), 
         constraint_set=TunedConstraintSet.TunableFactory(constraints_override=(disabled_constraints, default)), 
