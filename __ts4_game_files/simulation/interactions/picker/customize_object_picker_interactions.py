...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\picker\customize_object_picker_interactions.py
# Compiled at: 2020-04-01 03:32:42
# Size of source mod 2**32: 7137 bytes
...
class CustomizeObjectDefinitionPicker(PickerSuperInteraction):
    ...
    def picker_rows_gen(cls, inst, target, context, **kwargs):
        ...
        for index, obj_def in enumerate(inst_or_cls.object_definitions):
            ...
            yield ObjectPickerRow(name=name, icon_info=IconInfoData(obj_def_id=(obj_def.id), obj_geo_hash=(inst_or_cls.object_thumbnail_geo_state_override)),
              tag=obj_def,
              is_selected=(index == 0))
...
class CustomizeObjectGeoMatPicker(PickerSuperInteraction):
    INSTANCE_TUNABLES = {'geo_mat_combinations': TunableMapping(description='\n            A mapping of geometry state to a list of material states. So we can\n            use them to generate a list of different objects which have one object\n            definition id and different geometry and material states.\n            ',
                               ...
                               value_name='material_state_list',
                               value_type=TunableList(tunable=TunableReference(description='\n                    An object state that has material state to apply.\n                    ',
                               manager=(services.get_instance_manager(sims4.resources.Types.OBJECT_STATE)),
                               class_restrictions='ObjectStateValue')),
                               tuning_group=(GroupNames.PICKERTUNING))}
        ...
        for geo_stat, mat_states in cls.geo_mat_combinations.items():
            ...
            if not geo_op is None:
                ...
                for mat_state in mat_states:
                    ...
                    if mat_op is None or mat_op is UNSET:
    ...
    def picker_rows_gen(cls, inst, target, context, obj_def, **kwargs):
        ...
        for geo_stat, mat_states in inst_or_cls.geo_mat_combinations.items():
            for mat_state in mat_states:
                yield ObjectPickerRow(name=name, icon_info=IconInfoData(obj_def_id=(obj_def.id), obj_geo_hash=(sims4.hash_util.hash32(geo_stat.new_client_state.ops['geometry_state'])),
                  obj_material_hash=(mat_state.new_client_state.ops['material_state'].state_name_hash)),
                  tag=(
                 obj_def, geo_stat, mat_state))
