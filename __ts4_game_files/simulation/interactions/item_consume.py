...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\item_consume.py
# Compiled at: 2020-08-04 14:10:11
...
class ItemCostBase(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def build_element_for_interaction(self, sequence, interaction):
        ...
        if self.timing.timing == self.AT_END:
        ...
        def do_consume(*args, **kwargs):
            for item in self.ingredients:
                for _ in range(item.quantity):
                    if not interaction.sim.inventory_component.try_destroy_object_by_definition((item.ingredient), source=interaction,
                      cause='Consuming the cost of the interaction'):
                        return False
...
class SpellCost(ItemCostBase):
    FACTORY_TUNABLES = {'from_spell': TunableReference(description='The spell to pull ingredients from.',
...
class InteractionItemCostVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, explicit_item_cost=ItemCost.TunableFactory(), 
         spell_cost=SpellCost.TunableFactory(), 
         default='explicit_item_cost', **kwargs)