...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\choices.py
# Compiled at: 2020-05-21 20:02:03
...
import gsi_handlers.sim_handlers_log, sims4.log
__all__ = [
 'ChoiceMenu']
...
class ChoiceMenu:
    ...
    def add_potential_aops(self, target, context, potential_aops, scoring_gsi_handler):
        for aop in potential_aops:
            ...
            if result:
                result = DEFAULT
            potentials = (aop.affordance.potential_pie_menu_sub_interactions_gen)(
             (aop.target), context, scoring_gsi_handler, **aop.interaction_parameters)
            ...
            if do_test:
                if user_pick_target is not None and user_pick_target.check_affordance_for_suppression((context.sim), aop, user_directed=True):
                    ...
                    if gsi_handlers.sim_handlers_log.pie_menu_generation_archiver.enabled:
                        ...
                        result or logger.info('Test Failure: {}: {}', aop, result.reason)
                    if not result:
    ...
    def add_aop(self, aop, context, user_pick_target=None, result_override=DEFAULT, do_test=True):
        result = ChoiceMenu.is_valid_aop(aop, context,
    ...
    def select(self, choice_id):
        ...
        if context.sim is not None:
            if not selection.aop.affordance.immediate:
                if context.sim.queue.visible_len() >= context.sim.queue.max_interactions:
                    return EnqueueResult.NONE
        if selection is not None:
            if selection.result:
                if not selection.target_invalid:
