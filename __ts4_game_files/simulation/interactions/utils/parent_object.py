...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\utils\parent_object.py
# Compiled at: 2020-10-19 15:50:16
# Size of source mod 2**32: 15487 bytes
...
import itertools, services, sims4.log, sims4.resources
...
def parent_object(child_object, parent_object, slot_type=None, bone_name_hash=None, ignore_object_placmenent_verification=False):
    ...
    if slot_type is not None:
        for runtime_slot in parent_object.get_runtime_slots_gen(slot_types={slot_type}):
            ...
            if not ignore_object_placmenent_verification:
                if result:
                    runtime_slot.add_child(child_object)
                    return True
                logger.warn("runtime_slot isn't valid for placement: {}", result, owner='nbaker')

        logger.error('The parent object: ({}) does not have the requested slot type: ({}) required for this parenting, or the child ({}) is not valid for this slot type.', parent_object,
          slot_type, child_object, owner='nbaker')
    else:
        if bone_name_hash is not None:
            if parent_object.has_slot(bone_name_hash):
                if current_child_object_parent_slot is not None:
                    if current_child_object_parent_slot.slot_name_hash == bone_name_hash:
                        return True
                ...
                if ignore_object_placmenent_verification or runtime_slot.empty:
                    ...
                    return True
                logger.error('The parent object: ({}) does not have the requested slot type: ({}) required for this parenting, or the child ({}) is not valid for this slot type.  Slot is empty: {}', parent_object,
                  bone_name_hash, child_object, (runtime_slot.empty), owner='nbaker')
            else:
                try:
                    child_object.set_parent(parent_object, joint_name_or_hash=bone_name_hash)
                except (ValueError, KeyError):
                    logger.error('Error setting the location of ({}) to be a child of ({}) by referencing bone-name ({})', child_object,
                      parent_object, bone_name_hash, owner='shipark')
...
class ParentObjectWithRoutingFormationSlave(ParentObjectElement):
    ...
    def _do_behavior(self):
        ...
        if actor_routing_component is None:
            ...
            return should_parent_immediately or True
        return self._parent_object_fn()