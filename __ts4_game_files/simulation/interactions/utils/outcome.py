...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\utils\outcome.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 96068 bytes
import random, telemetry_helper
...
import animation.arb, element_utils, elements, enum, event_testing, services, sims.sim_log, sims4.log
logger = sims4.log.Logger('Interactions')
TELEMETRY_GROUP_INTERACTION = 'INTR'
TELEMETRY_HOOK_INTERACTION_OUTCOME_RESULT = 'RSLT'
TELEMETRY_FIELD_INTERACTION = 'idix'
TELEMETRY_FIELD_INTERACTION_RESULT_INDEX = 'iidx'
TELEMETRY_FIELD_INTERACTION_TARGET = 'itid'
TELEMETRY_FIELD_TARGET_RESULT_INDEX = 'tidx'
TELEMETRY_FIELD_INTERACTION_FINISH = 'ifns'
TELEMETRY_FIELD_USER_DIRECTED = 'udir'
TELEMETRY_FIELD_DURATION = 'dura'
outcome_telemetry_writer = sims4.telemetry.TelemetryWriter(TELEMETRY_GROUP_INTERACTION)
...
def _debug_outcome_result():
    ...
    if debug_outcome_style == DebugOutcomeStyle.AUTO_SUCCEED:
        ...
        if debug_outcome_style == DebugOutcomeStyle.AUTO_FAIL:
            previous_debug_outcome = OutcomeResult.FAILURE
        else:
            if debug_outcome_style == DebugOutcomeStyle.ROTATE:
...
def _is_debug_outcome_style_cheat_active():
    if debug_outcome_style != DebugOutcomeStyle.NONE:
        if debug_outcome_style is not False:
            return True
    return False
...
def build_outcome_actions(interaction, actions, setup_asm_override=DEFAULT):
    ...
    if actions.continuation:
        ...
        if not interaction.user_canceled:
            ...
            def parameterized_autonomy_gen(timeline):
                ...
                for participant_type, request_data in actions.parameterized_autonomy.items():
                    yield from interaction.super_interaction._parameterized_autonomy_helper_gen(timeline, (request_data.requests), (InteractionBucketType.DEFAULT),
                      participant_type=participant_type, fallback_notification=(request_data.fallback_notification),
                      push_on_success_or_fail=(request_data.push_on_success_or_fail))
        ...
        if actions.loot_list is not None:
            ...
            loot_list = []
    if actions.add_target_affordance_loot:
    ...
    if actions.response is not None:
        ...
        end_animation = actions.response(interaction, setup_asm_override=setup_asm_override,
          setup_asm_additional=setup_asm_end_animation)
    ...
    sequence = build_element((start_animation,
     set_last_animation_element,
     social_animation,
     end_animation,
     ...
     send_events,
     cancel_si_element,
     parameterized_autonomy_gen), CleanupType.RunAll)
...
class InteractionOutcome:
    ...
    def _telemetry_valid(self, interaction):
        if interaction.sim is None or interaction.provided_posture_type is not None:
            return False
        else:
            if not interaction.is_super:
                if not interaction.is_social:
                    return False
            return hasattr(interaction, 'guid64') or False
        return interaction.sim.is_selected

    def build_elements(self, interaction, update_global_outcome_result=False, interaction_outcome=None, send_telemetry=True):
        ...
        def _do(timeline):
            if interaction.global_outcome_result is None:
                ...
                if interaction.get_result_for_outcome(self) is None:
                    ...
                    try:
                        if send_telemetry:
                            if self._telemetry_valid(interaction):
                                if interaction_outcome is None:
                                    interaction_outcome_index = self._get_outcome_index(interaction)
                                    target_outcome_index = -1
                                    target_guid64 = 0
                                else:
                                    interaction_outcome_index = interaction_outcome._get_outcome_index(interaction)
                                    target_outcome_index = self._get_outcome_index(interaction)
                                    target_guid64 = interaction.target.guid64
                                if interaction_outcome_index is None:
                                    interaction_outcome_index = -1
                                if target_outcome_index is None:
                                    target_outcome_index = -1
                                with telemetry_helper.begin_hook(outcome_telemetry_writer, TELEMETRY_HOOK_INTERACTION_OUTCOME_RESULT, sim=(interaction.sim)) as (hook):
                                    hook.write_guid(TELEMETRY_FIELD_INTERACTION, interaction.guid64)
                                    hook.write_int(TELEMETRY_FIELD_INTERACTION_RESULT_INDEX, interaction_outcome_index)
                                    hook.write_guid(TELEMETRY_FIELD_INTERACTION_TARGET, target_guid64)
                                    hook.write_int(TELEMETRY_FIELD_TARGET_RESULT_INDEX, target_outcome_index)
                                    hook.write_enum(TELEMETRY_FIELD_INTERACTION_FINISH, interaction.telemetry_finishing_type)
                                    hook.write_bool(TELEMETRY_FIELD_USER_DIRECTED, interaction.is_user_directed)
                                    hook.write_float(TELEMETRY_FIELD_DURATION, abs(interaction.consecutive_running_time_span.in_minutes()))
                    except:
                        pass
    ...
    def _get_outcome_index(self, interaction):
        pass
    ...
    def get_reactionlet(self, interaction, **kwargs):
        if interaction.get_result_for_outcome(self) is None:
            self.decide(interaction)
        trait_reactionlet = self._get_trait_reactionlet_override(interaction, kwargs.get('sim'))
        if trait_reactionlet is not None:
            trait_reactionlet = trait_reactionlet(interaction, use_asm_cache=False, 
             enable_auto_exit=False, **kwargs)
            if trait_reactionlet is None:
                logger.error('Reactionlet in outcome {} has invalid selector', self)
            return trait_reactionlet
        reactionlet = self._get_reactionlet(interaction)
        if reactionlet is not None:
            reactionlet = reactionlet(interaction, use_asm_cache=False, 
             enable_auto_exit=False, **kwargs)
            if reactionlet is None:
                logger.error('Reactionlet in outcome {} has invalid selector', self)
            return reactionlet

    def _get_trait_reactionlet_override(self, interaction, sim):
        if interaction is None or sim is None:
            return
        for trait in sim.sim_info.get_traits():
            if trait.reactionlet_overrides is not None:
                for affordance, override in trait.reactionlet_overrides.override_map.items():
                    if affordance is interaction.affordance:
                        return override

                return trait.reactionlet_overrides.default_override
        ...
        for loot in self._get_loot_gen():
            ...
            for loot_op, _ in loot.get_loot_ops_gen():
                if loot_op is None:
                    ...
                    continue
                if loot_op.loot_type == LootType.SKILL:
                    stat = loot_op.stat
                    if stat.is_skill:
                        return stat
...
class TunableOutcomeSingle(TunableSingletonFactory):
    ...
    def __init__(self, outcome_action_kwargs=None, **kwargs):
        ...
        (super().__init__)(description='There is a single, guaranteed outcome for this interaction.', actions=TunableOutcomeActions(**outcome_action_kwargs), 
         callback=InteractionOutcome.tuning_loaded_callback, **kwargs)
        ...
        if debug_outcome_index:
            ...
            if debug_outcome_style != DebugOutcomeStyle.NONE:
                if debug_outcome_style is not False:
                    ...
                    elif interaction.get_result_for_outcome(self) == OutcomeResult.SUCCESS:
...
class TunableOutcomeDual(TunableSingletonFactory):
    ...
    def __init__(self, outcome_action_kwargs=None, **kwargs):
        ...
        (super().__init__)(description='\n                            There are one success and one failure outcome for\n                            this interaction.\n                            ', 
         ...
         success_actions=TunableOutcomeActions(**outcome_action_kwargs), 
         failure_actions=TunableOutcomeActions(**outcome_action_kwargs), 
         callback=InteractionOutcome.tuning_loaded_callback, **kwargs)


class InteractionOutcomeTestBased(InteractionOutcome):

    def __init__(self, tested_outcomes, fallback_outcomes, prioritized_tested_outcomes, use_fallback_as_default, **kwargs):
        super().__init__()
        self._tested_outcomes = tested_outcomes
        self._fallback_outcomes = fallback_outcomes
        self._prioritized_tested_outcomes = prioritized_tested_outcomes
        self._use_fallback_as_default = use_fallback_as_default
        self._selected_outcome = None
        self._selected_outcome_index = None

    @property
    def consumes_object(self):
        for outcome_tuple in self._prioritized_tested_outcomes:
            if outcome_tuple.outcome.consume_object:
                return True
    ...
    def print_outcome_index(self):
        ...
        prioritized_tested_outcome_str = '\n'
        prioritized_tested_outcomes_num = len(self._prioritized_tested_outcomes)
        if prioritized_tested_outcomes_num > 0:
            prioritized_tested_outcomes = [outcome for _, outcome in self._prioritized_tested_outcomes]
            prioritized_tested_outcome_str += 'Prioritized Tested Outcomes: ({})\n'.format(prioritized_tested_outcomes)
        tested_outcome_str = '('
        test_counter = 0
        ...
        total_outcome_num = total_potential_outcomes + fallback_outcomes + prioritized_tested_outcomes_num
        return 'Total Outcome Count:({})\n'.format(total_outcome_num) + fallback_outcome_str + prioritized_tested_outcome_str + 'Tested Outcomes: ({})\n'.format(total_potential_outcomes) + tested_outcome_str

    def _get_outcome_index(self, interaction):
        if self._selected_outcome_index is None:
            return super()._get_outcome_index(interaction)
    ...
    def _set_debug_outcome(self, debug_outcome_index, resolver):
        fallback_outcomes_num = len(self._fallback_outcomes)
        prioritized_outcomes_num = len(self._prioritized_tested_outcomes)
        total_potential_outcomes = fallback_outcomes_num + prioritized_outcomes_num + sum((len(x.potential_outcomes) for x in self._tested_outcomes))
        if debug_outcome_index < 0:
            logger.error('Attempting to set a negative value {} for the debug outcome index.', debug_outcome_index, owner='amwu')
            return (None, None)
        if debug_outcome_index > total_potential_outcomes:
            ...
            return (None, None)
        if self._fallback_outcomes:
            if debug_outcome_index <= fallback_outcomes_num:
                return (
                 None, self._fallback_outcomes[(debug_outcome_index - 1)].outcome)
        if self._prioritized_tested_outcomes:
            if debug_outcome_index <= fallback_outcomes_num + prioritized_outcomes_num:
                return (
                 None, self._prioritized_tested_outcomes[(debug_outcome_index - fallback_outcomes_num - 1)].outcome)
        if self._tested_outcomes:
            counter = fallback_outcomes_num + prioritized_outcomes_num
            for outcome_tuple in self._tested_outcomes:
                ...
                for potential_outcome in outcome_tuple.potential_outcomes:
                    ...
                    if debug_outcome_index == counter:
                        return (
                         result, potential_outcome.outcome)
    ...
    def decide(self, interaction, update_global_outcome_result=False):
        ...
        if debug_outcome_index:
            ...
            selected_prioritized_outcome = None
            index = len(self._fallback_outcomes)
            for outcome_and_tests in self._prioritized_tested_outcomes:
                index += 1
                if cheat_active:
                    if not self.cheat_outcome_style_test(curr_debug_style, outcome_and_tests.outcome):
                        continue
                    else:
                        if not outcome_and_tests.tests.run_tests(resolver):
                            continue
                    if interaction.allow_outcomes or outcome_and_tests.outcome.force_outcome_on_exit:
                        selected_prioritized_outcome = outcome_and_tests.outcome
                        break

            if selected_prioritized_outcome:
                self._selected_outcome, self._selected_outcome_index = selected_prioritized_outcome, index
                ...
                if update_global_outcome_result:
                    ...
                    if sims.sim_log.archiver.enabled:
                        sims.sim_log.log_interaction('Tested Outcome: Prioritized List', interaction, msg=(sims.sim_log.create_tested_outcome_message(self._selected_outcome)))
            else:
                if sims.sim_log.outcome_archiver.enabled or sims.sim_log.sim_outcome_archiver.enabled:
                    sims.sim_log.log_interaction_outcome(interaction, self, 'Tested Outcome: Prioritized List', is_tested_outcome=True,
                      msg=(sims.sim_log.create_tested_outcome_message(self._selected_outcome)))
                return
                index = len(self._fallback_outcomes) + len(self._prioritized_tested_outcomes)
                for outcome_tuple in self._tested_outcomes:
                    if cheat_active or outcome_tuple.tests.run_tests(resolver):
                        for outcome in outcome_tuple.potential_outcomes:
                            index += 1
                            if cheat_active:
                                if not self.cheat_outcome_style_test(curr_debug_style, outcome.outcome):
                                    continue
                                if interaction.allow_outcomes or outcome.outcome.force_outcome_on_exit:
                                    pass
                                weight = outcome.weight.get_multiplier(interaction.get_resolver())
                                if weight > 0:
                                    weights.append((weight, (outcome.outcome, index)))

                    else:
                        index += len(outcome_tuple.potential_outcomes)

                index = 0
                if not self._use_fallback_as_default:
                    if not weights or cheat_active:
                    ...
                    if weights:
                        self._selected_outcome, self._selected_outcome_index = sims4.random.weighted_random_item(weights)
                        interaction.store_result_for_outcome(self, self._selected_outcome.outcome_result)
                        if update_global_outcome_result:
                            interaction.global_outcome_result = self._selected_outcome.outcome_result
                            if sims.sim_log.archiver.enabled:
    ...
    def _get_loot_gen(self):
        ...
        for outcome_tuple in self._prioritized_tested_outcomes:
            yield outcome_tuple.outcome.loot_list
    ...
    def get_basic_extras_gen(self):
        for outcome_tuple in self._prioritized_tested_outcomes:
            yield from outcome_tuple.outcome.basic_extras
...
class TunableOutcomeTestBased(TunableSingletonFactory):
    ...
    def __init__(self, outcome_action_kwargs=None, **kwargs):
        ...
        super().__init__(description='\n            Based on the tests and the weights of the outcomes, a\n            single outcome will be selected.\n            ',
          tested_outcomes=TunableList(description='\n                If all of the tests pass, the outcomes will be randomly selected\n                based on the provided weight.\n                ',
          tunable=TunableTuple(tests=(event_testing.tests.TunableTestSet()),
          potential_outcomes=TunableList(description='\n                        List of weight/outcome pairs.\n                        ',
          tunable=TunableTuple(outcome=TunableOutcomeActions(**outcome_action_kwargs),
          ...
          prioritized_tested_outcomes=TunableList(description='\n                A higher-prioritized list of tested outcomes. These are NOT\n                weighted and instead will select by the first test that passes;\n                if any item from this list passes then it will be the only\n                outcome, and if none pass we will move on to choosing a \n                tested outcome as normal.\n                ',
          tunable=TunableTuple(tests=(event_testing.tests.TunableTestSet()),
          outcome=TunableOutcomeActions(**outcome_action_kwargs))),
          fallback_outcomes=TunableList(description='\n                These outcomes will only be considered if none of the Tested Outcomes pass their tests.\n                If Use Fallback As Default is checked, these outcomes will be considered along with any TestedOutcomes that passed.\n                ',
          tunable=TunableTuple(outcome=TunableOutcomeActions(**outcome_action_kwargs),
          weight=TunableMultiplier.TunableFactory(description='\n                        A tunable list of tests and multipliers to apply to the \n                        weight of the outcome.\n                        '))),
...
class InteractionOutcomePartial(InteractionOutcome):
    ...
    def _get_error_and_string(self, interaction, sim):
        ...
        for test_string_pair in self._test_and_results:
            if test_string_pair.test.run_tests(resolver):
                return (
                 False, test_string_pair.result_string(sim))
    ...
    def decide--- This code section failed: ---

 L.1244         0  LOAD_FAST                'self'
                2  LOAD_ATTR                _default_actions
                4  LOAD_FAST                'self'
                6  STORE_ATTR               _actions

 L.1246         8  LOAD_FAST                'interaction'
               10  LOAD_METHOD              get_participants
               12  LOAD_GLOBAL              ParticipantType
               14  LOAD_ATTR                PickedSim
               16  CALL_METHOD_1         1  '1 positional argument'
               18  STORE_FAST               'picked_sims'

 L.1247        20  LOAD_FAST                'picked_sims'
            22_24  POP_JUMP_IF_FALSE   594  'to 594'

 L.1248        26  LOAD_GLOBAL              set
               28  CALL_FUNCTION_0       0  '0 positional arguments'
               30  STORE_FAST               'valid_sim_ids'

 L.1249        32  LOAD_GLOBAL              len
               34  LOAD_FAST                'picked_sims'
               36  CALL_FUNCTION_1       1  '1 positional argument'
               38  STORE_FAST               'picked_sim_count'

 L.1250        40  LOAD_FAST                'interaction'
               42  LOAD_ATTR                sim
               44  STORE_FAST               'actor'

 L.1251        46  LOAD_FAST                'picked_sim_count'
               48  LOAD_CONST               1
               50  COMPARE_OP               ==
               52  POP_JUMP_IF_FALSE   234  'to 234'

 L.1252        54  LOAD_GLOBAL              next
               56  LOAD_GLOBAL              iter
               58  LOAD_FAST                'picked_sims'
               60  CALL_FUNCTION_1       1  '1 positional argument'
               62  CALL_FUNCTION_1       1  '1 positional argument'
               64  STORE_FAST               'sim'

 L.1253        66  LOAD_FAST                'self'
               68  LOAD_METHOD              _get_error_and_string
               70  LOAD_FAST                'interaction'
               72  LOAD_FAST                'sim'
               74  CALL_METHOD_2         2  '2 positional arguments'
               76  UNPACK_SEQUENCE_2     2 
               78  STORE_FAST               'return_val'
               80  STORE_FAST               'error_string'

 L.1254        82  LOAD_FAST                'return_val'
               84  POP_JUMP_IF_TRUE    152  'to 152'

 L.1255        86  LOAD_GLOBAL              sims
               88  LOAD_ATTR                sim_log
               90  LOAD_ATTR                outcome_archiver
               92  LOAD_ATTR                enabled
               94  POP_JUMP_IF_TRUE    106  'to 106'

 L.1256        96  LOAD_GLOBAL              sims
               98  LOAD_ATTR                sim_log
              100  LOAD_ATTR                sim_outcome_archiver
              102  LOAD_ATTR                enabled
              104  POP_JUMP_IF_FALSE   122  'to 122'
            106_0  COME_FROM            94  '94'

 L.1257       106  LOAD_GLOBAL              sims
              108  LOAD_ATTR                sim_log
              110  LOAD_METHOD              log_interaction_outcome
              112  LOAD_FAST                'interaction'
              114  LOAD_FAST                'self'
              116  LOAD_STR                 'partial Outcome: single fail'
              118  CALL_METHOD_3         3  '3 positional arguments'
              120  POP_TOP          
            122_0  COME_FROM           104  '104'

 L.1258       122  LOAD_FAST                'self'
              124  LOAD_ATTR                _single_failure_string
              126  POP_JUMP_IF_FALSE   230  'to 230'

 L.1259       128  LOAD_FAST                'self'
              130  LOAD_METHOD              _single_failure_string
              132  LOAD_FAST                'error_string'
              134  CALL_METHOD_1         1  '1 positional argument'
              136  STORE_FAST               'text'

 L.1260       138  LOAD_FAST                'self'
              140  LOAD_METHOD              _send_notification
              142  LOAD_FAST                'text'
              144  LOAD_FAST                'actor'
              146  CALL_METHOD_2         2  '2 positional arguments'
              148  POP_TOP          
              150  JUMP_FORWARD        556  'to 556'
            152_0  COME_FROM            84  '84'

 L.1262       152  LOAD_GLOBAL              sims
              154  LOAD_ATTR                sim_log
              156  LOAD_ATTR                outcome_archiver
              158  LOAD_ATTR                enabled
              160  POP_JUMP_IF_TRUE    172  'to 172'

 L.1263       162  LOAD_GLOBAL              sims
              164  LOAD_ATTR                sim_log
              166  LOAD_ATTR                sim_outcome_archiver
              168  LOAD_ATTR                enabled
              170  POP_JUMP_IF_FALSE   188  'to 188'
            172_0  COME_FROM           160  '160'

 L.1264       172  LOAD_GLOBAL              sims
              174  LOAD_ATTR                sim_log
              176  LOAD_METHOD              log_interaction_outcome
              178  LOAD_FAST                'interaction'
              180  LOAD_FAST                'self'
              182  LOAD_STR                 'partial Outcome: single success'
              184  CALL_METHOD_3         3  '3 positional arguments'
              186  POP_TOP          
            188_0  COME_FROM           170  '170'

 L.1265       188  LOAD_FAST                'valid_sim_ids'
              190  LOAD_METHOD              add
              192  LOAD_FAST                'sim'
              194  LOAD_ATTR                id
 ...
 L.1266       200  LOAD_FAST                'self'
              202  LOAD_ATTR                _single_success_string
              204  POP_JUMP_IF_FALSE   230  'to 230'

 L.1267       206  LOAD_FAST                'self'
              208  LOAD_METHOD              _single_success_string
              210  LOAD_FAST                'actor'
              212  LOAD_FAST                'sim'
              214  CALL_METHOD_2         2  '2 positional arguments'
              216  STORE_FAST               'text'

 L.1268       218  LOAD_FAST                'self'
              220  LOAD_METHOD              _send_notification
              222  LOAD_FAST                'text'
              224  LOAD_FAST                'actor'
              226  CALL_METHOD_2         2  '2 positional arguments'
              228  POP_TOP          
            230_0  COME_FROM           204  '204'
            230_1  COME_FROM           126  '126'
          230_232  JUMP_FORWARD        556  'to 556'
            234_0  COME_FROM            52  '52'

 L.1270       234  BUILD_LIST_0          0 
              236  STORE_FAST               'bullet_points'

 L.1271       238  SETUP_LOOP          298  'to 298'
              240  LOAD_FAST                'picked_sims'
              242  GET_ITER         
              244  FOR_ITER            296  'to 296'
              246  STORE_FAST               'sim'

 L.1272       248  LOAD_FAST                'self'
              250  LOAD_METHOD              _get_error_and_string
              252  LOAD_FAST                'interaction'
              254  LOAD_FAST                'sim'
              256  CALL_METHOD_2         2  '2 positional arguments'
              258  UNPACK_SEQUENCE_2     2 
              260  STORE_FAST               'return_val'
              262  STORE_FAST               'error_string'

 L.1273       264  LOAD_FAST                'return_val'
          266_268  POP_JUMP_IF_TRUE    282  'to 282'

 L.1274       270  LOAD_FAST                'bullet_points'
              272  LOAD_METHOD              append
              274  LOAD_FAST                'error_string'
              276  CALL_METHOD_1         1  '1 positional argument'
              278  POP_TOP          
              280  JUMP_BACK           244  'to 244'
            282_0  COME_FROM           266  '266'

 L.1276       282  LOAD_FAST                'valid_sim_ids'
              284  LOAD_METHOD              add
              286  LOAD_FAST                'sim'
              288  LOAD_ATTR                id
              ...
              296  POP_BLOCK        
            298_0  COME_FROM_LOOP      238  '238'

 L.1277       298  LOAD_GLOBAL              len
              300  LOAD_FAST                'valid_sim_ids'
              302  CALL_FUNCTION_1       1  '1 positional argument'
              304  STORE_FAST               'valid_sim_count'

 L.1278       306  LOAD_FAST                'valid_sim_count'
              308  LOAD_FAST                'picked_sim_count'
              310  COMPARE_OP               ==
          312_314  POP_JUMP_IF_FALSE   390  'to 390'

 L.1279       316  LOAD_GLOBAL              sims
              318  LOAD_ATTR                sim_log
              320  LOAD_ATTR                outcome_archiver
              322  LOAD_ATTR                enabled
          324_326  POP_JUMP_IF_TRUE    340  'to 340'

 L.1280       328  LOAD_GLOBAL              sims
              330  LOAD_ATTR                sim_log
              332  LOAD_ATTR                sim_outcome_archiver
              334  LOAD_ATTR                enabled
          336_338  POP_JUMP_IF_FALSE   356  'to 356'
            340_0  COME_FROM           324  '324'

 L.1281       340  LOAD_GLOBAL              sims
              342  LOAD_ATTR                sim_log
              344  LOAD_METHOD              log_interaction_outcome
              346  LOAD_FAST                'interaction'
              348  LOAD_FAST                'self'
              350  LOAD_STR                 'partial Outcome: multiple total success'
              352  CALL_METHOD_3         3  '3 positional arguments'
              354  POP_TOP          
            356_0  COME_FROM           336  '336'

 L.1282       356  LOAD_FAST                'self'
              358  LOAD_ATTR                _all_success_string
          360_362  POP_JUMP_IF_FALSE   556  'to 556'

 L.1283       364  LOAD_FAST                'interaction'
              366  LOAD_METHOD              create_localized_string
              368  LOAD_FAST                'self'
              370  LOAD_ATTR                _all_success_string
              372  CALL_METHOD_1         1  '1 positional argument'
              374  STORE_FAST               'text'

 L.1284       376  LOAD_FAST                'self'
              378  LOAD_METHOD              _send_notification
              380  LOAD_FAST                'text'
              382  LOAD_FAST                'actor'
              384  CALL_METHOD_2         2  '2 positional arguments'
              386  POP_TOP          
              388  JUMP_FORWARD        556  'to 556'
            390_0  COME_FROM           312  '312'

 L.1286       390  LOAD_GLOBAL              LocalizationHelperTuning
              392  LOAD_ATTR                get_bulleted_list
              394  LOAD_CONST               (None,)
              396  LOAD_FAST                'bullet_points'
              398  BUILD_TUPLE_UNPACK_WITH_CALL_2     2 
              400  CALL_FUNCTION_EX      0  'positional arguments only'
              402  STORE_FAST               'bullet_text'

 L.1287       404  LOAD_FAST                'valid_sim_count'
              406  LOAD_CONST               0
              408  COMPARE_OP               ==
          410_412  POP_JUMP_IF_FALSE   486  'to 486'

 L.1288       414  LOAD_GLOBAL              sims
              416  LOAD_ATTR                sim_log
              418  LOAD_ATTR                outcome_archiver
              420  LOAD_ATTR                enabled
          422_424  POP_JUMP_IF_TRUE    438  'to 438'

 L.1289       426  LOAD_GLOBAL              sims
              428  LOAD_ATTR                sim_log
              430  LOAD_ATTR                sim_outcome_archiver
              432  LOAD_ATTR                enabled
          434_436  POP_JUMP_IF_FALSE   454  'to 454'
            438_0  COME_FROM           422  '422'

 L.1290       438  LOAD_GLOBAL              sims
              440  LOAD_ATTR                sim_log
              442  LOAD_METHOD              log_interaction_outcome
              444  LOAD_FAST                'interaction'
              446  LOAD_FAST                'self'
              448  LOAD_STR                 'partial Outcome: multiple total failure'
              450  CALL_METHOD_3         3  '3 positional arguments'
              452  POP_TOP          
            454_0  COME_FROM           434  '434'

 L.1291       454  LOAD_FAST                'self'
              456  LOAD_ATTR                _all_failure_string
          458_460  POP_JUMP_IF_FALSE   556  'to 556'

 L.1292       462  LOAD_FAST                'self'
              464  LOAD_METHOD              _all_failure_string
              466  LOAD_FAST                'bullet_text'
              468  CALL_METHOD_1         1  '1 positional argument'
              470  STORE_FAST               'text'

 L.1293       472  LOAD_FAST                'self'
            474_0  COME_FROM           150  '150'
              474  LOAD_METHOD              _send_notification
              476  LOAD_FAST                'text'
              478  LOAD_FAST                'actor'
              480  CALL_METHOD_2         2  '2 positional arguments'
              482  POP_TOP          
              484  JUMP_FORWARD        556  'to 556'
            486_0  COME_FROM           410  '410'

 L.1295       486  LOAD_GLOBAL              sims
              488  LOAD_ATTR                sim_log
              490  LOAD_ATTR                outcome_archiver
              492  LOAD_ATTR                enabled
          494_496  POP_JUMP_IF_TRUE    510  'to 510'

 L.1296       498  LOAD_GLOBAL              sims
              500  LOAD_ATTR                sim_log
              502  LOAD_ATTR                sim_outcome_archiver
              504  LOAD_ATTR                enabled
          506_508  POP_JUMP_IF_FALSE   526  'to 526'
            510_0  COME_FROM           494  '494'

 L.1297       510  LOAD_GLOBAL              sims
              512  LOAD_ATTR                sim_log
              514  LOAD_METHOD              log_interaction_outcome
              516  LOAD_FAST                'interaction'
              518  LOAD_FAST                'self'
              520  LOAD_STR                 'partial Outcome: partial success'
              522  CALL_METHOD_3         3  '3 positional arguments'
              524  POP_TOP          
            526_0  COME_FROM           506  '506'

 L.1298       526  LOAD_FAST                'self'
              528  LOAD_ATTR                _partial_success_string
          530_532  POP_JUMP_IF_FALSE   556  'to 556'

 L.1299       534  LOAD_FAST                'self'
              536  LOAD_METHOD              _partial_success_string
              538  LOAD_FAST                'bullet_text'
              540  CALL_METHOD_1         1  '1 positional argument'
              542  STORE_FAST               'text'

 L.1300       544  LOAD_FAST                'self'
              546  LOAD_METHOD              _send_notification
              548  LOAD_FAST                'text'
              550  LOAD_FAST                'actor'
            ...
            556_2  COME_FROM           458  '458'
            556_3  COME_FROM           388  '388'
            556_4  COME_FROM           360  '360'
            556_5  COME_FROM           230  '230'

 L.1302       556  LOAD_FAST                'valid_sim_ids'
              558  LOAD_FAST                'interaction'
              560  LOAD_ATTR                interaction_parameters
              562  LOAD_STR                 'picked_item_ids'
              564  STORE_SUBSCR     

 L.1303       566  LOAD_FAST                'valid_sim_ids'
          568_570  POP_JUMP_IF_TRUE    634  'to 634'

 L.1304       572  LOAD_FAST                'self'
              574  LOAD_ATTR                _total_failure
              576  LOAD_CONST               None
              578  COMPARE_OP               is-not
          580_582  POP_JUMP_IF_FALSE   634  'to 634'

 L.1305       584  LOAD_FAST                'self'
              586  LOAD_ATTR                _total_failure
              588  LOAD_FAST                'self'
              590  STORE_ATTR               _actions
              592  JUMP_FORWARD        634  'to 634'
            594_0  COME_FROM            22  '22'

 L.1306       594  LOAD_GLOBAL              sims
              596  LOAD_ATTR                sim_log
              598  LOAD_ATTR                outcome_archiver
              600  LOAD_ATTR                enabled
          602_604  POP_JUMP_IF_TRUE    618  'to 618'

 L.1307       606  LOAD_GLOBAL              sims
              608  LOAD_ATTR                sim_log
              610  LOAD_ATTR                sim_outcome_archiver
              612  LOAD_ATTR                enabled
          614_616  POP_JUMP_IF_FALSE   634  'to 634'
            618_0  COME_FROM           602  '602'

 L.1308       618  LOAD_GLOBAL              sims
              620  LOAD_ATTR                sim_log
              622  LOAD_METHOD              log_interaction_outcome
              624  LOAD_FAST                'interaction'
              626  LOAD_FAST                'self'
              628  LOAD_STR                 'partial Outcome: No picked sims'
              630  CALL_METHOD_3         3  '3 positional arguments'
              632  POP_TOP          
            634_0  COME_FROM           614  '614'
            634_1  COME_FROM           592  '592'
            634_2  COME_FROM           580  '580'
            634_3  COME_FROM           568  '568'

 L.1310       634  LOAD_FAST                'interaction'
              636  LOAD_METHOD              store_result_for_outcome
              638  LOAD_FAST                'self'
              640  LOAD_FAST                'self'
              642  LOAD_ATTR                _actions
              644  LOAD_ATTR                outcome_result
              646  CALL_METHOD_2         2  '2 positional arguments'
              648  POP_TOP          

 L.1311       650  LOAD_FAST                'update_global_outcome_result'
          652_654  POP_JUMP_IF_FALSE   676  'to 676'

 L.1312       656  LOAD_FAST                'self'
              658  LOAD_ATTR                _actions
              660  LOAD_ATTR                outcome_result
              662  LOAD_FAST                'interaction'
              664  STORE_ATTR               global_outcome_result

 L.1313       666  LOAD_FAST                'self'
              668  LOAD_ATTR                _actions
              670  LOAD_ATTR                display_message
              672  LOAD_FAST                'interaction'
              674  STORE_ATTR               outcome_display_message
            676_0  COME_FROM           652  '652'

Parse error at or near `LOAD_METHOD' instruction at offset 474
...
class TunableOutcomePartial(TunableSingletonFactory):
    ...
    def __init__(self, outcome_action_kwargs=None, **kwargs):
        ...
        (super().__init__)(description='\n                            Filters the list of picked sims based on tests before \n                            applying the action.  Also sends notifications with \n                            multiple options for creating the text based on sims\n                            failing to pass the filter/test.\n                            \n                            Example usage:\n                            Travel_DummyInteractions_Phone_TravelWith\n                            \n                            The player selects two Sims, one asleep and one\n                            awake, to travel with. We want the travel to still\n                            proceed but with the asleep Sim removed from the\n                            list of picked Sims and a TNS to show telling the\n                            player that the asleep Sim will not travel with\n                            them. This can be accomplished by tuning the partial\n                            outcome to have a filter for sleeping and the string\n                            "{0.SimName} is asleep!" to show for any picked Sim\n                            that matches the filter. Now say we want to widen\n                            the criteria for removing Sims to travel with to\n                            also include low relationship, busy due to being at\n                            work, etc. We can add more filters and strings to\n                            the partial outcome to remove those Sims, e.g.\n                            adding the filter and string pairs of: low rel\n                            filter and "{0.SimName} would rather not come.",\n                            busy filter and\n                            "{0.SimName}" is busy!", etc.\n                            ', 
         default_actions=TunableOutcomeActions(**outcome_action_kwargs), 
         total_failure=OptionalTunable(description='\n                            If specified, then if all picked sims fail to pass\n                            these actions will replace the default\n                            ',
  tunable=TunableOutcomeActions(**outcome_action_kwargs)), 
         callback=InteractionOutcome.tuning_loaded_callback, 
         single_success_string=TunableLocalizedStringFactory(description='\n                             The text of the notification that is displayed when the only sim\n                             in the list of picked sims passes.  Arg 0 is actor, 1 is picked sim\n                             '), 
...
class TunableResponseSelector(TunableVariant):
    ...
    class TunableResponsePaired(TunableFactory):
        ...
        def __init__(self, description='A response that plays on both Sims.', **kwargs):
            (super().__init__)(animation=TunableAnimationReference(interaction_asm_type=(InteractionAsmType.Response), participant_enum_override=(
 ParticipantTypeResponsePaired, ParticipantTypeResponsePaired.TargetSim),
  description='The response animation to play.'), 
    ...
    class TunableResponseIndividual(TunableFactory):
        ...
        def _on_tunable_loaded_callback(cls, fields, source, *, animations):
            for participant_type, animation_element in animations.items():
                ...
                add_auto_constraints(cls, fields, source, actor_participant_type=participant_type,
                  target_participant_type=None,
                  factory=(animation_element.factory),
                  overrides=(animation_element.overrides))
        ...
        def _create_response(interaction, *, animations, setup_asm_override=DEFAULT, **kwargs):
                ...
                with arb_accumulator.parallelize():
                    for sim in sims_ordered:
                        ...
                        if response_animation is None:
                            continue
                        elif sim is not interaction.sim or setup_asm_override is DEFAULT:
            ...
            if work:
                if not actor_in_sims:
                    work = (
                     flush_all_animations, work)
            return work

        FACTORY_TYPE = _create_response

        def __init__(self, description='A response that plays distinct animations on Actor and TargetSim.', **kwargs):
            (super().__init__)(callback=self._on_tunable_loaded_callback, 
             animations=TunableMapping(key_type=(TunableEnumEntry(ParticipantTypeResponse, ParticipantType.Actor)), key_name='target',
  value_type=TunableAnimationReference(callback=None, interaction_asm_type=(InteractionAsmType.Response),
  participant_enum_override=(
 ParticipantTypeResponse, ParticipantTypeResponse.Invalid)),
  value_name='animation',
  description='A mapping of participants to the animation they should play. If a Sim is found more than onceit will only have the first animation played on it. Tuning this field will override animation_actorand animation_target.'), 
             description=description, **kwargs)
    ...
    def __init__(self, description='\n        Responses are short, one-shot animations \n        that we play after the conclusion of interactions. They are usually \n        meant to convey how the Sims felt about how the interaction went and \n        are thus almost always related to emotions.\n        These can be configured to play on a specific participant or can be \n        paired between actor and target.\n        ', participant_enum_override=DEFAULT, **kwargs):
        (super().__init__)(paired=self.TunableResponsePaired(), 
         individual=self.TunableResponseIndividual(), 
         description=description, **kwargs)
...
class TunableOutcomeActions(TunableTuple):
    ...
    def __init__(self, animation_callback=DEFAULT, allow_route_events=False, allow_multi_si_cancel=False, allow_social_animation=False, locked_args=None, **kwargs):
        ...
        animation_ref = TunableAnimationReference(callback=animation_callback, interaction_asm_type=(InteractionAsmType.Outcome),
          description='The one-shot animation ref to play')
        ...
        if allow_multi_si_cancel:
            ...
            cancel_si = TunableVariant(description="\n                                        When enabled, the outcome will cancel the owning\n                                        interaction's parent SI.\n                                        ",
              locked_args={'enabled':ParticipantType.Actor, 
             'disabled':None},
              default='disabled')
        ...
        if locked_args is None:
            ...
            if TunableOutcomeActions.ADD_TARGET_AFFORDANCE_LOOT_KEY not in locked_args:
  ...
  tunable_type=bool,
  default=False), 
         continuation=TunableContinuation(description='An affordance to be pushed as part of an outcome.'), 
         cancel_si=cancel_si, 
         events_to_send=TunableList(TunableEnumEntry(TestEvent, (TestEvent.Invalid), description='events types to send')), 
         display_message=OptionalTunable(description='\n                             If set, flyaway text will be shown.\n                             ',
  tunable=TunableLocalizedString(default=None, description='Localized string that is shown as flyaway text')), 
         parameterized_autonomy=TunableMapping(description='\n                             Specify parameterized autonomy for the participants of the interaction.\n                             ',
  key_type=TunableEnumEntry(description='\n                                 The participant to run parameterized autonomy for.\n                                 ',
  tunable_type=ParticipantType,
  default=(ParticipantType.Actor)),
  value_type=TunableTuple(requests=TunableList(description='\n                                     A list of parameterized autonomy requests to run.\n                                     ',
