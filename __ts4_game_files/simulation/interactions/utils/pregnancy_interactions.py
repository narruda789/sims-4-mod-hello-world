...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\utils\pregnancy_interactions.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 29937 bytes
...
import element_utils, interactions, services, sims4
...
class DeliverBabySuperInteraction(SuperInteraction, NameOffspringSuperInteractionMixin):
    ...
    def _name_and_create_babies_gen(self, timeline):
        ...
        if not pregnancy_tracker.is_pregnant:
            ...
            result = yield from self.sim.is_npc or self._do_renames_gen(timeline, list(pregnancy_tracker.get_offspring_data_gen()))
    ...
    def _complete_pregnancy_gen(self, timeline, pregnancy_tracker):
        ...
        if not pregnant_sim.is_npc:
            if self.inherited_loots:
    ...
    def _apply_inherited_loots(self, sim_infos, pregnancy_tracker):
        ...
        for inherited_loot in inherited_loots:
            if birther:
                if not inherited_loot.birther_test.run_tests(SingleSimResolver(birther)):
                    continue
            if non_birther:
                if not inherited_loot.non_birther_test.run_tests(SingleSimResolver(non_birther)):
                    continue
    ...
    def _complete_pregnancy_gen(self, timeline, pregnancy_tracker):
        ...
        for offspring_data in pregnancy_tracker.get_offspring_data_gen():
            ...
            if current_zone.id == sim_info.zone_id:
                ...
                assign_bassinet_for_baby(sim_info) or create_and_place_baby(sim_info)
            else:
        ...
        if is_off_lot_birth:
            ...
            travel_dialog_element = UiDialogElement((self.sim), (self.get_resolver()),
...
class DeliverBabyOnSurgeryTableInteraction(DeliverBabySuperInteraction):
    INSTANCE_TUNABLES = {'bassinet_to_use':TunableReference(description='\n            Bassinet with Baby object definition id.\n            ',
     ...
     'bassinet_slot_type':SlotType.TunableReference(description='\n            SlotType used to place the bassinet when it is created.\n            '), 
    ...
    def _build_outcome_sequence(self):
        ...
        return build_element((self._handle_npc_bassinet_fade_and_effect,
         sequence))
    ...
    def _push_post_delivery_interaction(self, sim_infos):
        if self._baby_object is not None:
            if self.after_delivery_interaction is not None:
                ...
                liabilities = ((
                  ...
                  CleanupInteractionCallbackLiability(cleanup_interaction_callback=_destroy_all_bassinets)),)
                context = self.context.clone_for_continuation(self)
                self.sim.push_super_affordance((self.after_delivery_interaction), (self._baby_object),
