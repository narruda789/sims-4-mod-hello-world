...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\utils\adventure.py
# Compiled at: 2021-04-28 13:08:36
# Size of source mod 2**32: 57462 bytes
...
import clock, services, sims4.reload
...
class AdventureMoment(HasTunableFactory, AutoFactoryInit):
    ...
    FACTORY_TUNABLES = {'description':'\n            A phase of an adventure. Adventure moments may present\n            some information in a dialog form and for a choice to be\n            made regarding how the overall adventure will branch.\n            ', 
     ...
     '_finish_actions':TunableList(description='\n            A list of choices that can be made by the player to determine\n            branching for the adventure. They will be displayed as buttons\n            in the UI. If no dialog is displayed, then the first available\n            finish action will be selected. If this list is empty, the\n            adventure ends.\n            ',
       ...
       locked_args={'cost_type': COST_TYPE_SIMOLEONS},
       amount=TunableRange(description='How many Simoleons to\n                            deduct.\n                            ',
       tunable_type=int,
       default=0,
       minimum=0)),
       item_cost=TunableTuple(description="The specified items will \n                        be removed from the Sim's inventory.\n                        ",
       locked_args={'cost_type': COST_TYPE_ITEMS},
       item_cost=(ItemCost.TunableFactory())),
       default=None),
       action_results=TunableList(description='\n                    A list of possible results that can occur if this finish\n                    action is selected. Action results can award loot, display\n                    notifications, and control the branching of the adventure by\n                    selecting the next adventure moment to run.\n                    ',
       ...
       test_locked_args={'tooltip': None}),
       weight_multiplier=Tunable(description='\n                                    The weight multiplier to apply if the\n                                    associated test passes.\n                                    ',
       tunable_type=float,
       default=1))),
       ...
       events_to_send=TunableList(description='\n                            A list of events to send.\n                            ',
    ...
    def _run_action_from_cheat(self, action_index):
        ...
        if cheat_index == self.CHEAT_PREVIOUS_INDEX:
            ...
            if cheat_index == self.CHEAT_NEXT_INDEX:
    ...
    def _get_action_result_weight(self, action_result):
        ...
        for modifier in action_result.weight_modifiers:
            if modifier.test is None or interaction_resolver(modifier.test):
    ...
    def _apply_action_cost(self, action):
        if action.cost is not None:
            if action.cost.cost_type == self.COST_TYPE_SIMOLEONS:
                return self._sim.family_funds.try_remove((action.cost.amount), (Consts_pb2.TELEMETRY_INTERACTION_COST),
                  sim=(self._sim)) or False
            else:
                if action.cost.cost_type == self.COST_TYPE_ITEMS:
    ...
    def _run_action_from_index(self, action_index):
        ...
        if not weight_pairs:
            ...
            if not action_result is not None:
                if not finish_action.action_results:
                    if not self._apply_action_cost(finish_action):
                        return
    ...
    def _on_dialog_response(self, dialog):
        ...
        if response_index is None:
            return
        if False:
            if self._is_cheat_response(response_index):
    ...
    def _get_dialog(self):
        ...
        for action_id, finish_action in enumerate(self._finish_actions):
            ...
            if result or finish_action.disabled_text is not None:
                disabled_text = finish_action.disabled_text if not result else None
                responses.append(UiDialogResponse(dialog_response_id=action_id, text=(self._get_action_display_text(finish_action)),
                  subtext=(self._interaction.create_localized_string(finish_action.display_subtext)),
                  disabled_text=(disabled_text() if disabled_text is not None else None)))

        if not responses:
            return
        if False:
            if _show_all_adventure_moments:
            ...
            for result in action.action_results:
                ...
                if result.next_moments:
                    ...
                    for modifier in result.weight_modifiers:
                        ...
                        if resolver is not None and modifier.test:
                            try:
...
class Adventure(XevtTriggeredElement, HasTunableFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'description':'\n            A series of individual moments linked together in a game-like\n            fashion.\n            ', 
     ...
     '_initial_moments':TunableList(description='\n            A list of adventure moments that are valid as initiating moments for\n            this adventure.\n            ',
       ...
       tunable_type=AdventureMomentKey,
       default=(AdventureMomentKey.INVALID)),
    ...
    def get_cheat_responses(self, last_action_id):
        ...
        responses.append(UiDialogResponse(dialog_response_id=(last_action_id + AdventureMoment.CHEAT_PREVIOUS_INDEX), text=(self._get_cheat_display_text(AdventureMoment.CHEAT_TEXT.previous_display_text, curr_index, total_moments)),
          disabled_text=(disabled_text() if curr_index <= 0 else None)))
        responses.append(UiDialogResponse(dialog_response_id=(last_action_id + AdventureMoment.CHEAT_NEXT_INDEX), text=(self._get_cheat_display_text(AdventureMoment.CHEAT_TEXT.next_display_text, curr_index + 2, total_moments)),
          disabled_text=(disabled_text() if curr_index >= total_moments - 1 else None)))
        return responses
    ...
    def queue_adventure_moment(self, adventure_moment_key):
        if self._maximum_triggers:
            if self._adventure_moment_count >= self._maximum_triggers:
                return
    ...
    def _get_initial_adventure_moment_key(self):
        ...
        if initial_adventure_moment_key is not None:
            if self.is_adventure_moment_available(initial_adventure_moment_key):
    ...
    def display_adventure_enums(tuned_values, display_moment_data, resolver, _connection=None):
        ...
        for name, tuning in tuned_values._adventure_moments.items():
            ...
            output('        Moment Tuning: {}'.format(tuning.aventure_moment))
            if display_moment_data:
                output('          sim specific cooldown:  {}'.format(tuning.sim_specific_cooldown))
                tuning.aventure_moment.factory.display_moment_data_(tuning.aventure_moment._tuned_values, '          ', resolver, _connection)
    ...
    def find_moment_path(tuned_values, adventure_moment, moment_key, explored_keys):
        for followup_key in adventure_moment.factory.get_folloup_moments_gen(adventure_moment._tuned_values):
            if followup_key is moment_key:
                ...
                result = Adventure.find_moment_path(tuned_values, tuning.aventure_moment, moment_key, explored_keys)
    ...
    def find_moment_gen(tuned_values, tuning_name):
        for moment_key, tuning in tuned_values._adventure_moments.items():
            if tuning_name in str(tuning.aventure_moment).lower():
                explored_keys = set()
                result = []
                for initial_moment in tuned_values._initial_moments:
                    ...
                    result = Adventure.find_moment_path(tuned_values, target_tuning.aventure_moment, moment_key, explored_keys)
                    if result is not None:
                        result.append(followup_key)
                        result.reverse()
                        break
                ...
                yield (
                 moment_key, tuning.aventure_moment, result)