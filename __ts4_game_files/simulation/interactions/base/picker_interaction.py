...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\base\picker_interaction.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 205820 bytes
...
import build_buy, element_utils, enum, event_testing.results, gsi_handlers, services, sims, sims4.resources, tag
...
class PickerSuperInteractionMixin:
    INSTANCE_TUNABLES = {'picker_dialog':TunablePickerDialogVariant(description='\n            The object picker dialog.\n            ',
     ...
     'pie_menu_option':OptionalTunable(description='\n            Whether use Pie Menu to show choices other than picker dialog.\n            ',
       tunable=_TunablePieMenuOptionTuple(),
       disabled_name='use_picker',
       enabled_name='use_pie_menu',
       tuning_group=GroupNames.PICKERTUNING), 
...
class PickerSuperInteraction(PickerSuperInteractionMixin, ImmediateSuperInteraction):
    ...
    def _verify_tuning_callback(cls):
        if not cls.allow_user_directed:
            if cls.pie_menu_option is not None:
    ...
    def __init__(self, *args, choice_enumeration_strategy=None, **kwargs):
        ...
        if self.allow_autonomous:
            if self._choice_enumeration_strategy is None:
...
class PickerSingleChoiceSuperInteraction(PickerSuperInteraction):
    ...
    def _get_id_from_picked_item(self, picked_item):
        return picked_item

    def _show_picker_dialog(self, owner, target_sim=None, target=None, choices=(), **kwargs):
        if self.use_pie_menu():
            return
        picked_items = self.interaction_parameters.get('picked_item_ids')
        picked_items = list(picked_items) if picked_items is not None else None
        if len(choices) == 1:
            if picked_items:
                picked_item = picked_items[0]
                picked_id = self._get_id_from_picked_item(picked_item)
                choice_id = self._get_id_from_choice(choices[0])
                if choice_id == picked_id:
                    self.on_choice_selected(picked_item)
                    return
...
class SimPickerMixin:
 ...
 L. 704         0  LOAD_GLOBAL              super
                2  CALL_FUNCTION_0       0  '0 positional arguments'
                4  LOAD_METHOD              _verify_tuning_callback
                6  CALL_METHOD_0         0  '0 positional arguments'
                8  POP_TOP          

 L. 705        10  LOAD_FAST                'cls'
               12  LOAD_ATTR                include_instantiated_sims
               14  POP_JUMP_IF_FALSE    22  'to 22'
               16  LOAD_FAST                'cls'
               18  LOAD_ATTR                include_uninstantiated_sims
 ...
 L. 706        32  LOAD_GLOBAL              logger
               34  LOAD_METHOD              error
               36  LOAD_STR                 'Tuning: If include_instantiated_sims is False or include_uninstantiated_sims is True, radius should be disabled: {}'
               38  LOAD_FAST                'cls'
...
Parse error at or near `COME_FROM' instruction at offset 44_0
    ...
    def _get_valid_sim_choices_gen(cls, inst, target, context, test_function=None, min_required=1, **kwargs):
        ...
        for sim_info in sim_infos:
            if not sim_info.can_instantiate_sim:
                ...
                if not inst_or_cls.include_actor_sim:
                    if sim_info is actor_sim_info:
                        continue
                    else:
                        if (inst_or_cls.include_target_sim or target) is not None:
                            if target.is_sim:
                                if sim_info is target.sim_info:
                                    continue
                        if not inst_or_cls.include_uninstantiated_sims:
                            if not sim_info.is_instanced():
                                continue
                        if not inst_or_cls.include_instantiated_sims:
                            if sim_info.is_instanced():
                                continue
                    if inst_or_cls.radius is not None:
                        sim = sim_info.get_sim_instance()
                        if sim is None or actor_sim_info is None:
                            continue
                        ...
                        if delta.magnitude() > inst_or_cls.radius:
                            continue
            results = services.sim_filter_service().submit_filter((inst_or_cls.sim_filter),
              None, sim_constraints=(sim_info.sim_id,), requesting_sim_info=(requesting_sim_info.sim_info),
              allow_yielding=False,
              household_id=household_id,
              gsi_source_fn=(inst_or_cls.get_sim_filter_gsi_name),
              include_rabbithole_sims=(inst_or_cls.include_rabbithole_sims))
            if not results:
                continue
            if inst_or_cls.sim_tests:
                ...
                if not inst_or_cls.sim_tests.run_tests(resolver):
                    continue
            if test_function is not None:
                sim = sim_info.get_sim_instance()
                if not (sim is None or test_function(sim)):
                    continue
                valid_sims_found += 1
                yield results[0]

        number_required_sims_not_found = min_required - valid_sims_found
        if inst_or_cls.create_sim_if_no_valid_choices:
            if number_required_sims_not_found > 0:
                results = services.sim_filter_service().submit_matching_filter(number_of_sims_to_find=number_required_sims_not_found, sim_filter=(inst_or_cls.sim_filter), requesting_sim_info=(requesting_sim_info.sim_info),
                  allow_yielding=False,
                  household_id=household_id,
                  gsi_source_fn=(inst_or_cls.get_sim_filter_gsi_name))
                if results:
    ...
    def _push_continuation(self, sim, tunable_continuation, sim_ids, insert_strategy, picked_zone_set):
        continuation = None
        picked_item_set = {target_sim_id for target_sim_id in sim_ids if target_sim_id is not None if target_sim_id is not None}
    ...
    def _push_continuations(self, sim_ids, zone_datas=None):
        ...
        if zone_datas is not None:
            try:
                picked_zone_set = {zone_data.zone_id for zone_data in zone_datas if zone_data is not None if zone_data is not None}
            except TypeError:
                picked_zone_set = {
                 zone_datas.zone_id}
                ...
                if target_sim_id is None:
                    ...
                    if self.continuations_are_sequential:
                        if index < num_continuations:
                            continuation = (
                             self.picked_continuation[index],)
                        else:
        ...
        if link_type != SimPickerLinkContinuation.NEITHER:
            ...
            if actor_continuation is not None:
                if target_continuation is not None and not link_type == SimPickerLinkContinuation.TARGET:
...
class SimPickerInteraction(SimPickerMixin, PickerSingleChoiceSuperInteraction):
    ...
    def _run_interaction_gen(self, timeline):
        ...
        if len(choices) < self.picker_dialog.min_choices:
            return False
        if self.picker_dialog.min_selectable == 0:
            if not choices:
                self._on_successful_picker_selection()
                return True
        if self.single_choice_display_name is not None:
            if len(choices) == 1:
                self._kwargs['picked_item_ids'] = (
                 choices[0].sim_info.sim_id,)
    ...
    def create_row(cls, inst, tag, select_default=False):
        ...
        if inst_or_cls.requires_age_romance_check:
            ...
            if inst is not None:
                ...
                if actor_sim_info.age <= Age.TEEN and target_sim_info.age >= Age.YOUNGADULT or actor_sim_info.age >= Age.YOUNGADULT:
                    if target_sim_info.age <= Age.TEEN:
                        allow_romance = False
                second_tag_list = (1, ) if not allow_romance else None
    ...
    def get_single_choice_and_row(cls, inst, context=None, target=None, **kwargs):
        ...
        if single_choice is not None:
            ...
            return (
             single_choice.sim_info, row)
        return (None, None)

    @classmethod
    def has_valid_choice(cls, target, context, **kwargs):
        if cls.picker_dialog is not None:
            if cls.picker_dialog.min_selectable == 0:
                if cls.picker_dialog.min_choices == 0:
                    return True
        choice_count = 0
    ...
    def _get_name(cls, inst, target=DEFAULT, context=DEFAULT, **interaction_parameters):
        ...
        if inst_or_cls.get_display_name_from_destination:
            ...
            if venue_instance is not None:
                if venue_instance.travel_with_interaction_name is not None:
...
class LotPickerMixin:
    INSTANCE_TUNABLES = {'default_inclusion':TunableVariant(description='\n            This defines which venue types are valid for this picker.\n            ',
       ...
       include_all_by_default=(Tunable(bool, True)),
       exclude_venues=TunableList(tunable=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.VENUE)),
       tuning_group=(GroupNames.VENUES),
       pack_safe=True),
       display_name='Blacklist Items'),
       exclude_lists=TunableList((TunableVenueListReference()), display_name='Blacklist Lists'),
       locked_args={'include_all_by_default': True}),
       exclude_all=TunableTuple(description='\n                This will prevent all venue types from being valid, except those whitelisted.\n                ',
       include_all_by_default=(Tunable(bool, False)),
       include_venues=TunableList(tunable=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.VENUE)),
       tuning_group=(GroupNames.VENUES),
       pack_safe=True),
       display_name='Whitelist Items'),
       include_lists=TunableList((TunableVenueListReference()), display_name='Whitelist Lists'),
       locked_args={'include_all_by_default': False}),
       default='include_all',
       tuning_group=GroupNames.PICKERTUNING), 
     'building_types_excluded':TunableSet(description='\n            A set of building types to exclude for this map view picker. This\n            allows us to do things like exclude apartments or penthouses.\n            ',
       ...
       tunable_type=PlexBuildingType,
       default=(PlexBuildingType.FULLY_CONTAINED_PLEX),
       invalid_enums=(
      PlexBuildingType.INVALID,)),
       tuning_group=GroupNames.PICKERTUNING), 
    ...
    def _get_valid_lot_choices(cls, inst, target, context, target_list=None):
        ...
        if actor is None:
            ...
            if target_list is None:
                if target is not None and target.is_sim and target.household is not None:
        ...
        for zone_data in persistence_service.zone_proto_buffs_gen():
            ...
            if not inst_or_cls.include_active_lot:
            ...
            if inst_or_cls.test_region_compatibility:
                ...
                if not current_region.is_region_compatible(dest_region):
                    continue
            if inst_or_cls.building_types_excluded:
                if plex_service.get_plex_building_type(zone_id) in inst_or_cls.building_types_excluded:
                    continue
            ...
            if venue_tuning is None:
                ...
                if inst_or_cls.exclude_rented_zones and venue_tuning.is_vacation_venue:
                    if not travel_group_manager.is_zone_rentable(zone_id, venue_tuning):
                        continue
                else:
                    ...
                    if inst_or_cls.default_inclusion.include_all_by_default:
                        if venue_tuning in default_inclusion.exclude_venues:
                            continue
                        if any((venue_tuning in venue_list for venue_list in default_inclusion.exclude_lists)):
                            continue
                        results.append(zone_data)
                if venue_tuning in default_inclusion.include_venues:
                    results.append(zone_data)
            if any((venue_tuning in venue_list for venue_list in default_inclusion.include_lists)):
                results.append(zone_data)
                continue
...
class MapViewPickerInteraction(LotPickerMixin, PickerSuperInteraction):
    INSTANCE_TUNABLES = {'picker_dialog':TunablePickerDialogVariant(description='\n            The object picker dialog.\n            ',
       available_picker_flags=ObjectPickerTuningFlags.MAP_VIEW,
       tuning_group=GroupNames.PICKERTUNING,
       dialog_locked_args={'text_cancel':None, 
      'text_ok':None, 
      'title':None, 
      'text':None, 
      'text_tokens':DEFAULT, 
     ...
     'actor_continuation':TunableContinuation(description='\n            If specified, a continuation to push on the actor when a picker \n            selection has been made.\n            ',
       locked_args={'actor': ParticipantType.Actor},
       tuning_group=GroupNames.PICKERTUNING), 
     'target_continuation':TunableContinuation(description='\n            If specified, a continuation to push on the sim targetted',
       tuning_group=GroupNames.PICKERTUNING), 
     'bypass_picker_on_single_choice':Tunable(description='\n            If this is checked, bypass the picker if only one option is available.\n            ',
       tunable_type=bool,
       default=False,
       tuning_group=GroupNames.PICKERTUNING)}
    ...
    def _run_interaction_gen(self, timeline):
        ...
        if self.bypass_picker_on_single_choice:
            if len(choices) == 1:
...
class ObjectPickerMixin:
    ...
    def _push_picked_continuation(self, picked_items):
        ...
        self.push_tunable_continuation((self.continuation), multi_push=(not self.single_push_continuation),
            ...
            for local_actor in local_actors:
                if isinstance(local_actor, sims.sim_info.SimInfo):
                    ...
                    if affordance.is_super:
                        result = local_actor.test_super_affordance(affordance, local_target, local_context, picked_object=target,
                        ...
                        aop = AffordanceObjectPair(affordance, local_target, super_affordance, super_interaction, picked_object=target,
    ...
    def _test_continuation(cls, inst, row_data, **kwargs):
        ...
        if inst_or_cls.continuation is None:
            ...
            return row_data.object_id or None
...
class ObjectPickerInteraction(ObjectPickerMixin, PickerSingleChoiceSuperInteraction):
    ...
    class _DescriptionFromTooltip(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'field': TunableEnumEntry(description='\n                Use an existing field from the tooltip component\n                ',
                    ...
                    default=(TooltipFields.recipe_description))}
    ...
    def create_row(cls, inst, row_obj, context=DEFAULT, target=DEFAULT, is_selected=False):
        ...
        if row_obj.has_custom_name() and row_obj.animalobject_component is None:
            ...
            if row_obj.crafting_component is not None:
                ...
                if row_obj.animalobject_component is not None:
                    ...
                    if row_description is None:
                        if row_obj.has_custom_description():
                            ...
                            if inst_or_cls.fallback_description is not None:
                    ...
                    row = ObjectPickerRow(object_id=(row_obj.id), name=name,
                      row_description=row_description,
                      icon=icon,
                      def_id=(row_obj.definition.id),
                      count=(inst_or_cls.get_stack_count(row_obj)),
                      icon_info=icon_info,
                      tag_list=tag_list,
                      tag=row_obj,
    ...
    def has_valid_choice(cls, target, context, **kwargs):
        ...
        for _ in cls._get_objects_gen(target, context):
            obj_count += 1
            if not cls.picker_dialog or obj_count >= cls.picker_dialog.min_selectable:
                return True
    ...
    def _get_current_selected_count(self):
        ...
        if target is not None:
            if animal_service is not None:
                if animal_service.is_registered_home(target.id):
...
class ObjectInInventoryPickerMixin:
        ...
        if cls.suppress_duplicate_objects is not None:
            if obj.inventoryitem_component is not None:
                if cls.suppress_duplicate_objects == DuplicateObjectsSuppressionType.BY_STACK_ID:
                    ...
                    if cls.suppress_duplicate_objects == DuplicateObjectsSuppressionType.BY_DEFINITION_ID_AND_STACK_ID:
                        key = (
                         obj.inventoryitem_component.get_stack_id(), obj.definition.id)
        if key is None:
    ...
    def _get_objects_internal_gen(cls, inst, target, context, **kwargs):
        inst_or_cls = inst if inst is not None else cls
        subject = (inst_or_cls.get_participant)(participant_type=inst_or_cls.inventory_subject, sim=context.sim, 
         target=target, **kwargs)
        if subject is None:
            return
            actor_sim = (inst_or_cls.get_participant)(participant_type=ParticipantType.Actor, sim=context.sim, 
             target=target, **kwargs)
            actor_sim_info = actor_sim.sim_info if actor_sim is not None else None
            target_sim = (inst_or_cls.get_participant)(participant_type=ParticipantType.TargetSim, sim=context.sim, 
             target=target, **kwargs)
                ...
                if inst_or_cls.hidden_inventory_test != InventoryTest.TestHiddenInventories.ANY:
                    ...
                    if inst_or_cls.additional_item_test:
                        ...
                        if not inst_or_cls.additional_item_test.run_tests(resolver):
                            continue
                    if inst_or_cls.suppress_duplicate_objects is not None:
...
class ObjectInHouseholdInventoryPickerMixin:
    ...
    def _get_object_ids_from_household_inventory(cls, inst, target, context, **kwargs):
        inst_or_cls = inst if inst is not None else cls
        inventory_subject = (inst_or_cls.get_participant)(participant_type=inst_or_cls.inventory_subject, sim=context.sim, 
         target=target, **kwargs)
        ...
        if household.home_zone_id == 0:
            return (None, None)
        return (
         build_buy.get_object_ids_in_household_inventory(household_id), household_id)
...
class ObjectInSlotPickerMixin:
    ...
    def _get_objects_internal_gen(cls, inst, target, context, **kwargs):
        inst_or_cls = inst if inst is not None else cls
        slot_subject = (inst_or_cls.get_participant)(participant_type=inst_or_cls.subject_with_slots, sim=context.sim, 
         target=target, **kwargs)
        if slot_subject is not None:
            if slot_subject.children:
                for child in slot_subject.children:
                    if inst_or_cls.required_slot_types is not None:
                        if (child.location.slot_hash or child.location.joint_name_hash) not in inst_or_cls.required_slot_types:
                            continue
...
class AutonomousObjectTaggedPickerInteraction(AutonomousObjectPickerInteraction):
    INSTANCE_TUNABLES = {'whitelist_filter':ObjectDefinitonsOrTagsVariant(description='\n            Either a list of tags or definitions that objects can be considered.\n            ',
       tuning_group=GroupNames.PICKERTUNING), 
     'blacklist_filter':OptionalTunable(ObjectDefinitonsOrTagsVariant(description='\n            Either a list of tags or definitions that objects should be ignored.\n            '),
       tuning_group=GroupNames.PICKERTUNING), 
    ...
    def _get_objects_internal_gen(cls, inst, target, context, **kwargs):
        ...
        skipped = []
        if not inst_or_cls.counts_as_inside:
        ...
        should_do_outside_later = not inst_or_cls.supress_outside_if_modifier and not prevent_outside
        for obj in object_manager.get_objects_with_filter_gen(inst_or_cls.whitelist_filter):
            if sim_level != obj.level:
                if not inst_or_cls.allow_different_level:
                    continue
                elif inst_or_cls.blacklist_filter is not None:
                    if inst_or_cls.blacklist_filter.matches(obj):
                        continue
                else:
                    if inst_or_cls.radius is not None:
                        ...
                        if delta.magnitude() > inst_or_cls.radius:
                            continue
                    if handle_outside and not obj.is_in_sim_inventory(sim):
                        if obj.is_outside:
    ...
    def _passes_post_skip_tests(cls, inst, sim, obj):
        inst_or_cls = inst if inst is not None else cls
        if inst_or_cls.require_on_offlot_parity:
            if sim.is_on_active_lot(tolerance=(StatisticComponentGlobalTuning.DEFAULT_OFF_LOT_TOLERANCE)):
                if not obj.is_on_active_lot(tolerance=(StatisticComponentGlobalTuning.DEFAULT_OFF_LOT_TOLERANCE)):
                    return False
        if inst_or_cls.object_filter_test:
            ...
            return obj.is_connected(sim) or False
        return True


lock_instance_tunables(AutonomousObjectTaggedPickerInteraction, pre_add_autonomy_commodities=None,
...
lock_instance_tunables(AutonomousPreferredObjectPickerInteraction, pre_add_autonomy_commodities=None,
  pre_run_autonomy_commodities=None,
  post_guaranteed_autonomy_commodities=None,
  post_run_autonomy_commodities=None,
  basic_content=None,
...
class DefinitionsFromTags(PurchaseListOption):
    FACTORY_TUNABLES = {'filter_tags': OptionalTunable(description='\n            An optional filter that if enabled will filter out the allowed items\n            based on the filter.\n            ',
                      ...
                      default=(tag.Tag.INVALID)),
                      minlength=1),
                      disabled_name='all_definitions',
                      enabled_name='specific_definitions')}
...
class DefinitionsRandom(PurchaseListOption):
    FACTORY_TUNABLES = {'item_list':TunableList(description='\n            The list of items available for purchase.\n            ',
       ...
       default=1)),
       minlength=1), 
     'items_avaiable':TunableRange(description='\n            The number of items available.\n            ',
       tunable_type=int,
       minimum=1,
       default=1)}
...
class InventoryItems(PurchaseListOption):
    FACTORY_TUNABLES = {'participant_type':TunableEnumEntry(description="\n            The participant type who's inventory will be used to list objects.\n            ",
     ...
     'object_requirement':OptionalTunable(description='\n            The requirements the object must meet to be added to the picker. If\n            this is disabled, all inventory items will be displayed.\n            ',
       ...
       locked_args={'requirement_type': OBJ_REQUIREMENT_PURCHASABLE}),
       state_threshold=TunableTuple(description='\n                    Only show items that meet this state threshold.\n                    ',
       threshold=TunableThreshold(description='\n                        The state threshold the object must meet.\n                        ',
       value=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.OBJECT_STATE)),
       class_restrictions='ObjectStateValue')),
       locked_args={'requirement_type': OBJ_REQUIREMENT_STATE_THRESHOLD}))), 
     'purchase_option':TunableVariant(description='\n            Options for how we we want to purchase the items we found in the\n            inventory.\n            ',
       ...
       locked_args={'purchase_type': PURCHASE_BY_DEFINITION}),
       by_copy=TunableTuple(description='\n                Purchase the item by actually copying one of the instances of\n                the items in the inventory.  Objects will be grouped by price\n                within the picker.\n                ',
       price_option=TunableEnumEntry(description='\n                    The option for determining the price of the item.\n                    ',
       tunable_type=PriceOption,
       default=(PriceOption.USE_CURRENT_VALUE)),
       locked_args={'purchase_type': PURCHASE_BY_ITEM_COPY}),
       default='by_definition')}
    ...
    def _get_items_internal_gen(self, inst_or_cls, **interaction_kwargs):
        ...
        if inventory_component is not None:
            if self.object_requirement is None:
                ...
                if self.object_requirement.requirement_type == OBJ_REQUIREMENT_PURCHASABLE:
                    yield from inventory_component.purchasable_objects.objects
                else:
                    if self.object_requirement.requirement_type == OBJ_REQUIREMENT_STATE_THRESHOLD:
    ...
    def add_objects_to_purchase_picker(self, inst_or_cls, purchase_picker_data, **interaction_kwargs):
        ...
        if self.purchase_option.purchase_type == PURCHASE_BY_DEFINITION:
            ...
            if self.purchase_option.purchase_type == PURCHASE_BY_ITEM_COPY:
                participant = (inst_or_cls.get_participant)(participant_type=self.participant_type, **interaction_kwargs)
                ...
                for obj in objects_to_purchase:
                    if self.purchase_option.price_option == PriceOption.USE_CURRENT_VALUE:
                        ...
                        if self.purchase_option.price_option == PriceOption.USE_RETAIL_VALUE:
                            ...
                            break
                    purchase_picker_data.add_definition_to_purchase((obj.definition), custom_price=price,
                      obj=obj)
...
class ParticipantInventoryCount(TunableFactory):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(participant_type=TunableEnumEntry(description="\n                The participant type who's inventory will be used to count the\n                number of objects owned of a specific definition.\n                ",
                             tunable_type=ParticipantType,
                             default=(ParticipantType.Object)), **kwargs)
...
class InventoryTypeCount(TunableFactory):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(inventory_type=TunableEnumEntry(description='\n                The type of inventory that is used to count the number of\n                objects owned of a specific definition.\n                ',
                           ...
                           default=(InventoryType.UNDEFINED)), **kwargs)
...
class SellPickerInteraction(ObjectInInventoryPickerMixin, TransactionPickerMixin, PickerSuperInteraction):
    ...
    def _setup_dialog(self, dialog, **kwargs):
        ...
        for obj in (self._get_objects_internal_gen)((self.target), (self.context), **kwargs):
            ...
            if multiplier != 1:
                ...
                tooltip = None
            row = PurchasePickerRow(def_id=(definition.id), is_enable=True,
              num_available=(self.get_stack_count(obj)),
              tags=(definition.build_buy_tags),
              custom_price=custom_price,
              objects={
             obj},
              row_tooltip=tooltip,
              show_discount=(self.show_discount and multiplier != 1),
              prediscounted_price=(obj.current_value))
            dialog.add_row(row)
    ...
    def _remove_sold_objects_from_inventory(self, dialog):
        ...
        for obj_id, amount, price in zip(ids_and_amounts_and_price[::3], ids_and_amounts_and_price[1::3], ids_and_amounts_and_price[2::3]):
            ...
            potential_objects = self._stack_key_objects[key] if self.suppress_duplicate_objects is not None else [
             row_obj]
...
class PurchasePickerMixin(TransactionPickerMixin):
    INSTANCE_TUNABLES = {'purchase_list_option':TunableList(description='\n            A list of methods that will be used to generate the list of objects that are available in the picker.\n            ',
     ...
     'object_count_option':OptionalTunable(description='\n            If enabled then we will display a count next to each item of the\n            number owned.\n            ',
       ...
       default='participant_inventory_count')),
       tuning_group=GroupNames.PICKERTUNING), 
     'delivery_method':TunableVariant(description='\n            Where the objects purchased will be delivered.\n            ',
    ...
    def _setup_dialog(cls, inst, dialog, **kwargs):
        ...
        for (definition, custom_price), item_data in purchase_picker_data.items_to_purchase.items():
            if multiplier != 1:
                ...
                tooltip = None
            row = PurchasePickerRow(def_id=(definition.id), is_enable=(inst_or_cls._get_enabled_option(definition)),
              num_owned=(inst_or_cls._get_count_option(definition)),
              num_available=(item_data.num_available or inst_or_cls._get_availability_option(definition)),
              tags=(definition.build_buy_tags),
              custom_price=custom_price,
              fashion_trend=(item_data.fashion_trend),
              objects=(item_data.objects),
              row_tooltip=tooltip,
              show_discount=(inst_or_cls.show_discount),
              icon_info_data_override=(inst_or_cls._get_icon_info_data_override_option(definition)),
              row_description=(inst_or_cls._get_description_override_option(definition)))
            dialog.add_row(row)
...
class ObjectsInMultipleInventoriesMixin:
    ...
    def _get_objects_from_household_inventory(cls, inst, participant_type, target, context, **kwargs):
        inst_or_cls = inst if inst is not None else cls
        inventory_subject = (inst_or_cls.get_participant)(participant_type=participant_type, sim=context.sim, 
         target=target, **kwargs)
        if inventory_subject is None:
            return
        household_id = None
        ...
        for object_id in object_ids:
            ...
            if not obj is None:
                if obj.is_sim:
                    continue
                if inst_or_cls.inventory_item_test:
                    if not inst_or_cls.inventory_item_test(obj):
                        continue
                objects.append(obj)
                yield obj
    ...
    def _get_object_count(cls, inst, definition, target, context, **kwargs):
        ...
        if inst_or_cls.use_sim_inventory is not None:
            inventory_subject = (inst_or_cls.get_participant)(participant_type=inst_or_cls.use_sim_inventory, sim=context.sim, 
             target=target, **kwargs)
            if inventory_subject is not None:
                if inventory_subject.inventory_component is not None:
    ...
    def _get_household_id_for_participant(cls, inst, target, context, **kwargs):
        inst_or_cls = inst if inst is not None else cls
        if inst_or_cls.use_household_inventory is not None:
            inventory_subject = (inst_or_cls.get_participant)(participant_type=inst_or_cls.use_household_inventory, sim=context.sim, 
             target=target, **kwargs)
            if inventory_subject is not None:
                if isinstance(inventory_subject, sims.household.Household):
...
class ObjectsInMultipleInventoriesPurchasePickerInteraction(ObjectsInMultipleInventoriesMixin, PurchasePickerInteraction):
    ...
    def _setup_dialog(self, dialog, **kwargs):
        ...
        for (definition, custom_price), item_data in purchase_picker_data.items_to_purchase.items():
            ...
            if multiplier != 1:
                ...
                custom_price = int(original_price * multiplier)
                tooltip = None
            row = PurchasePickerRow(def_id=(definition.id), num_available=count,
              custom_price=custom_price,
              tags=(definition.build_buy_tags),
              objects=(item_data.objects),
              row_tooltip=tooltip,
    ...
    def _add_picked_items(self, picked_items, definition, total_count):
        if total_count <= 0:
            ...
            if self.use_sim_inventory is not None:
                inventory_subject = self.get_participant(participant_type=(self.use_sim_inventory), sim=(self.context.sim),
                  target=(self.target))
    ...
    def _push_picked_continuation(self, picked_items):
        if self.use_compressed_multiple_inventory_loot:
            ...
            self.push_tunable_continuation((self.continuation), compressed_multiple_inventory_items=picked_items,
              insert_strategy=(QueueInsertStrategy.LAST))
        else:
            self.interaction_parameters['picked_item_ids'] = picked_items
            self.push_tunable_continuation((self.continuation), picked_item_ids=picked_items,
              insert_strategy=(QueueInsertStrategy.LAST))
