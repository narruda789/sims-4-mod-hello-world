...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\base\super_interaction.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 300992 bytes
from _sims4_collections import frozendict
from _weakrefset import WeakSet
from contextlib import contextmanager
import itertools, operator, weakref
from animation.animation_constants import InteractionAsmType
from animation.animation_utils import flush_all_animations, get_tested_animation_override
...
from interactions.base.interaction import Interaction, CancelGroupInteractionType, InteractionQueueVisualType
...
from interactions.utils.line_utils import LineUtils, start_wait_in_line_together_situation, get_wait_in_line_together_situation, WaitingLineInteractionChainLiability, LineUpdateTiming
...
from sims4.tuning.tunable import Tunable, TunableList, TunableReference, TunableTuple, OptionalTunable, TunableEnumEntry, TunableVariant, TunableSet, TunableMapping, TunableAngle, HasTunableSingletonFactory, AutoFactoryInit, TunableEnumSet, TunableSimMinute, TunablePackSafeReference
...
import autonomy.autonomy_request, caches, element_utils, elements, enum
import event_testing.test_events as test_events
import event_testing.tests as tests
import gsi_handlers.interaction_archive_handlers, interactions.priority, postures, services, sims4.log, situations
...
class SuperInteraction(RouteEventProviderMixin, Interaction, StageControllerElement):
    ...
    DEFAULT_POSTURE_TARGET_PREFERENCES = TunableMapping(description='\n            A tunable mapping of posture tags to a goal score bonus in meters.\n            This is used to make some objects more attractive than others for\n            the purposes of posture preference scoring.  That means that higher\n            numbers are good; the Sim will go x meters out of their way to use\n            these objects, where x is the amount tuned.\n            \n            For example, if one object has a score of 3 and another object has\n            a score of 0, the object that scores 0 will need to be more than 3\n            meters closer than the object that scores 3 for the Sim to choose\n            it.\n            \n            Example: Let\'s say you want to make couches more desirable for\n            watching TV.  To do this, you would create a new tag in\n            PostureTransitionTargetPreferenceTag (found in Tuning->postures)\n            called "ComfortableSeating".  Then you would tag all appropriate\n            objects with that tag by adding it to PosturePreferenceTagList on\n            the object.  Next, you would come in here and add a new item with a\n            key of that tag and a value of 10 or so, which is about the size of\n            the constraint to watch TV. Thus they will tend to use couches in\n            the TV cone at the expense of other factors. One example downside\n            of this is they will be less inclined to consider how centered they\n            are in the TV cone and what direction the sofa is facing.\n            ',
      key_type=(TunableEnumEntry(PostureTransitionTargetPreferenceTag, PostureTransitionTargetPreferenceTag.INVALID)),
    ...
    INSTANCE_TUNABLES = {'waiting_line':OptionalTunable(description='\n            Enable a waiting line for sims wanting to use this object. \n            ',
       ...
       tunable_type=float,
       default=4),
       route_near_affordance_override=OptionalTunable(description='\n                    If enabled, a reference to an interaction used for getting\n                    Sims to route closer to the target before running the wait \n                    in line interaction. If disabled, the sim will run the \n                    default "route nearby affordance" specified in \n                    interactions.utils.line_utils module tuning.\n                    ',
       tunable=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION)),
       class_restrictions='SitOrStandSuperInteraction')),
       autonomous_waiting_line_prefence_curve=OptionalTunable(description='\n                    Tune how autonomy scores interactions with waiting lines.\n                    ',
       ...
       tunable_type=int,
       default=0),
       enabled_name='specify_subroot',
       disabled_name='use_object_root'),
       use_origin_override_for_constraints=Tunable(description='\n                    If enabled, generate the line constraints relative to the\n                    tuned line origin override. If disabled, generate \n                    constraints relative to the object root. \n                    ',
       tunable_type=bool,
       default=False),
       allow_line_on_same_target=Tunable(description='\n                    If checked the interaction will consider using the same\n                    target to go in line again.  This is for cases like the \n                    slippy slide where the sim will queue up again after \n                    using the object.\n                    ',
       tunable_type=bool,
       default=False),
       wait_in_line_with_picked_sims=Tunable(description='\n                    If enabled, the interaction will start a \n                    WaitInLineTogetherSituation to emulate the visual of \n                    picked sims waiting in line together with the actor.\n                    ',
       tunable_type=bool,
       default=False),
       wait_for_continuations=OptionalTunable(description='\n                    If enabled, the waiting line will not move forward until\n                    the sim in front finishes their continuation chain.\n                    ',
       tunable=TunableSet(description='\n                        A set of interactions that are considered to be the last\n                        step in the interaction chain. These interactions will\n                        register stand slot released callbacks to remove the sim\n                        from the line and push the next sim to consider using \n                        the line target. \n                        ',
       tunable=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION))))),
       line_update_timing=TunableVariant(description='\n                    Timing for when to notify the next sim in line that \n                    they can go.\n                    ',
       locked_args={'stand_slot_released':LineUpdateTiming.STAND_SLOT_RELEASED, 
      'path_planned':LineUpdateTiming.PATH_PLANNED},
       default='stand_slot_released')),
       tuning_group=GroupNames.SPECIAL_CASES), 
     'maximum_time_to_wait_for_other_sims':TunableSimMinute(description='\n            The number of Sim minutes to wait for other Sims in a multi-Sim\n            interaction before giving up and canceling the interaction.\n            ',
       default=8,
       tuning_group=GroupNames.SPECIAL_CASES), 
     '_transition_constraints':OptionalTunable(description='\n            If enabled, constraints that will be used for generating the\n            transition to this interaction.  I.e. if you want a sim drinking coffee\n            to drink the coffee wherever the coffee happens to be, you would\n            put in a circle constraint with the object being the constraint actor.\n            If a player later decides to drink someplace else, (or later autonomy requires\n            moving elsewhere) the sim would still be able to do so while continuing\n            to drink.\n            \n            This creates an entry in a constraint set for each\n            target object/list of constraints\n            \n            This is primarily intended as a performance optimization.\n            ',
       ...
       default=(ParticipantType.Object)),
       locked_args={'is_participant': True}),
       tag_based=TunableTuple(tags=TunableSet(description='\n                                            The tag for objects used to generate _constraints relative to.\n                                            ',
      ...
      Tag.INVALID,),
       pack_safe=True),
       minlength=1),
       locked_args={'is_participant': False}),
     ...
     '_preserve_held_props':TunableTuple(description='\n            Define how props are preserved for this interaction. Normally, props\n            from one SI are hidden whenever mixers from another SI are run.\n            Also, props are hidden whenever a Sim routes.\n            \n            Use this tuning to override this behavior such that props are never\n            hidden in either or both of those cases.\n            \n            NOTE: Double check with your animator that this is correct. This is\n            normally not required in regular circumstances.\n            ',
       ...
       tunable_type=bool,
       default=False),
       preserve_during_route=Tunable(description='\n                If checked, props held as part of this SI are not hidden when\n                the Sim routes.\n                ',
       tunable_type=bool,
       default=False),
       tuning_group=GroupNames.ANIMATION), 
     'animation_stat':OptionalTunable(description='\n                If enabled, specify a statistic to drive the animation content\n                for this interaction.\n                ',
       tunable=TunableTuple(description='\n                    Which stat to use for the actor and target.\n                    ',
       actor_stat=OptionalTunable(description='\n                        If enabled, specify a statistic to drive the animation\n                        context for the actor of this interaction.\n                        ',
     ...
     'rallyable':TunableList(description='\n                Interactions in this list will be generated in the Pie Menu\n                when the Sim is in a Party with other Sims. All Sims will have\n                an interaction pushed in order to keep the Party together.\n                ',
       ...
       manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION)),
       allow_none=True),
       preserve_preferred_objects=Tunable(description='\n                                If enabled, the affordance we push will preserve\n                                the preferred objects from the original\n                                interaction. \n                                ',
       tunable_type=bool,
       default=True)),
       solve_static_commodity=TunableTuple(description='\n                            Bring the Party along and try to solve for the \n                            specified static commodity for all members.\n                            ',
       loc_display_name=TunableLocalizedStringFactory(default=180956154),
       ...
       manager=(services.get_instance_manager(sims4.resources.Types.STATIC_COMMODITY)))),
       default='push_affordance'),
       push_social=TunableReference(description='\n                        When rallied Sims finish their transition they will\n                        push this affordance if they are no longer in a social\n                        group. e.g. If you run GoHereTogether while your Sims\n                        are in sit_intimate, sit_intimate will cancel, so we\n                        want to put them in chat at the end.\n                        ',
       manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION)),
       allow_none=True),
       rally_allow_forward=Tunable(description='\n                        If checked then we will skip testing the Sims we will\n                        rally, with the tests of this interaction.\n                        ',
       tunable_type=bool,
       default=False)),
       tuning_group=GroupNames.SPECIAL_CASES), 
     'only_available_as_rally':Tunable(description='\n                If checked then this interaction will only be available in its\n                rallyable form.\n                ',
       tunable_type=bool,
       default=False,
       tuning_group=GroupNames.SPECIAL_CASES), 
     'allow_forward_from_inventory':Tunable(description="\n                Some inventory objects can make all of their interactions to\n                be available when clicking on inventory owner object.\n                \n                If checked, this interaction will allow to be forwarded \n                from inventory to the inventory owner.\n                \n                Example: All earbuds interactions will be available on Sims who has \n                earbuds in their inventory. If this interaction is available on \n                earbuds but we don't want to forward it to the Sim, uncheck this value.\n                ",
       tunable_type=bool,
       default=True,
       tuning_group=GroupNames.SPECIAL_CASES), 
     'autonomy_preference':OptionalTunable(description='\n                Autonomy Preference related tuning options for this super\n                interaction. You can make a sim always use the same object, or\n                use the tuned preference score for certain SIs.\n                ',
       tunable=TunableVariant(use_preference=TunableTuple(preference=TunableAutonomyPreference(is_scoring=False)),
       scoring_preference=TunableTuple(preference=TunableAutonomyPreference(is_scoring=True),
       autonomy_score=Tunable(description='\n                            The amount to multiply the autonomous aop score by\n                            when the Sim prefers this object.\n                            ',
       tunable_type=float,
       default=1)),
       use_only=TunableTuple(description='\n                        Preference type that specifies sim should ONLY use this\n                        object, and no other sim can use it via autonomy  \n                        Restriction is zone specific, so not for inventoryable \n                        objects.\n                        \n                        There is a test in module tuning on a per preference \n                        tag to bypass the lockout.\n                        ',
       preference=TunableAutonomyPreference(is_scoring=False, use_only=True)),
       default='use_preference'),
       tuning_group=GroupNames.AUTONOMY), 
     'disable_autonomous_multitasking_if_user_directed':Tunable(description="\n                If this is checked, if this interaction is user directed and\n                guaranteed, sim will not consider running full autonomy and sim\n                cannot be a target of an full autonomy ping.\n                \n                For Example, if sim started a user directed painting, but don't\n                want sim to be interrupted by a social have this checked.\n                ",
       tunable_type=bool,
       default=False,
       tuning_group=GroupNames.AUTONOMY), 
     'use_best_scoring_aop':Tunable(description="\n                If checked, autonomy will always use the best scoring aop when\n                there are similar aops. For example, checking this on\n                view_painting will cause only the best scoring painting to be\n                considered.  If you uncheck this for painting, autonomy will\n                consider all paintings, but will use the best scoring painting\n                when scoring against other aops. In other words, if you uncheck\n                this for view_painting and have 10,000 paintings on the lot,\n                the Sim will consider those paintings, but it won't skew the\n                probability.\n                ",
       tunable_type=bool,
       default=True,
       needs_tuning=True,
       tuning_group=GroupNames.AUTONOMY), 
     'pre_route_buff':OptionalTunable(description='\n                The buff takes effect on the first node of routing until transition ends.\n                ',
       tunable=TunableTuple(description='\n                    ',
       buff_type=TunablePackSafeReference(description='\n                        A buff that is fired when the route event fires and is\n                        destroyed when the transition ends. \n                        ',
       manager=(services.get_instance_manager(sims4.resources.Types.BUFF))),
       buff_reason=TunableLocalizedString(description='\n                        Reason for the buff.\n                        '))), 
    ...
    def _tuning_loaded_callback(cls):
        if cls.basic_content is not None:
            ...
            if cls._content_sets.phase_tuning is not None:
                if cls._content_sets.num_phases > 0:
                    ...
                    def condition_factory(*args, **kwargs):
                        condition = StatisticCondition(who=cls._content_sets.phase_tuning.target, stat=cls._content_sets.phase_tuning.turn_statistic, 
                         threshold=threshold, 
    ...
    def _verify_tuning_callback(cls):
        ...
        for affordance in cls.all_affordances_gen():
            if affordance.allow_user_directed:
                ...
                if affordance._auto_constraints is not None:
                ...
                if cls.target_type == TargetType.ACTOR and cls.target_type != affordance.target_type:
            ...
            if not affordance.optional or has_valid_intersection:
                for constraint_error in constraint_errors:
                    logger.error(constraint_error)

        if cls.score_additional_sims_for_in_use is not None:
            if cls.basic_reserve_object is None:
                logger.error('Interaction {} has scoring for additional Sims enabled, but no basic reserve object is set.', (cls.__name__),
                  owner='jdimailig')

    @classmethod
    def _get_tuning_suggestions(cls, print_suggestion):
        if cls._provided_posture_type is not None:
            if 'generic' not in str(cls).lower():
                print_suggestion("Interaction is tuned as a generic posture provider, but does not respect the appropriate 'generic' naming convention. This might suggest it was setup incorrectly. Please look at PostureProviderGenericInteraction tuning for reference.")
            elif not cls.visible:
                print_suggestion('Interaction is tuned as a generic posture provider, but is not visible. This may be unintended and will prevent a posture icon from being present in the interaction queue.')
            else:
                if cls.visual_type_override != InteractionQueueVisualType.POSTURE:
                    print_suggestion('Interaction is tuned as a visible generic posture provider but does not use a posture visual type override. This may cause its interaction queue icon to display incorrectly.')
        if cls.privacy:
            print_suggestion('This interaction is tuned to require privacy. This may cause strange behavior like sims failing to transition to interactions or prematurely canceling.')

    @classmethod
    def has_slot_constraint(cls, *args, **kwargs):
        if cls.provided_posture_type is not None:
            if not cls.provided_posture_type.mobile:
                return True
    ...
    def additional_mixers_to_cache(cls):
        ...
        if basic_content is not None:
            if basic_content.content_set is not None:
                if hasattr(basic_content.content_set, 'additional_mixers_to_cache'):
    ...
    def potential_interactions(cls, target, context, **kwargs):
        ...
        if not cls.only_available_as_rally:
            is_forward_from_inventory = kwargs.get('from_inventory_to_owner', False)
            if not is_forward_from_inventory or cls.allow_forward_from_inventory:
                aop = (cls.generate_aop)(target, context, **kwargs)
                yield aop

    @classmethod
    def get_rallyable_aops_gen(cls, target, context, rally_constraint=None, affordance=None, **kwargs):
        if cls._can_rally(context):
            for entry in cls.rallyable:
                from interactions.base.rally_interaction import RallyInteraction
                affordance = affordance if affordance is not None else cls
                rally_interaction = RallyInteraction.generate(affordance, rally_tag=(entry.tag), rally_level=0, rally_data=(entry.behavior),
                  rally_push_social=(entry.push_social),
                  rally_constraint=rally_constraint,
                  rally_sources=(entry.sources),
                  rally_pie_menu_icon=(entry.pie_menu_icon),
                  rally_allow_forward=(entry.rally_allow_forward))
    ...
    def potential_pie_menu_sub_interactions_gen(cls, target, context, scoring_gsi_handler=None, **aop_kwargs):
        if cls.generate_content_set_as_potential_aops:
            content_set = generate_content_set((context.sim),
              cls, None, context, potential_targets=(target,), scoring_gsi_handler=scoring_gsi_handler,
    ...
    def is_allowed_to_forward(cls, obj):
        if cls.is_rally_interaction:
            if cls.rally_allow_forward:
                return True
    ...
    @classproperty
    def provided_posture_type_species(cls):
        return cls._provided_posture_type_species

    @classmethod
    def get_provided_posture_change(cls, aop):
        if cls._provided_posture_type is not None:
            return PostureOperation.BodyTransition((cls._provided_posture_type), (enumdict(Species, {cls._provided_posture_type_species: aop})),
              target=(aop.target),
              disallowed_ages=(cls._provided_posture_type_disallowed_ages))
    ...
    def get_supported_posture_types(cls, posture_type_filter=None):
        ...
        for affordance in itertools.chain(cls.all_affordances_gen(), (cls,)):
            for participant_type, supported_posture_manifest in affordance._define_supported_postures().items():
    ...
    def supports_posture_state(cls, inst, posture_state, participant_type=ParticipantType.Actor, posture_type_filter=None, target=DEFAULT):
        ...
        if not inst_or_cls.supports_posture_type((body.posture_type), participant_type=participant_type, posture_type_filter=posture_type_filter):
            ...
            interaction_constraint = inst_or_cls.constraint_intersection(sim=sim, target=target, posture_state=None,
            ...
            return interaction_constraint.valid or TestResult(False, "Interaction's constraint doesn't support body posture: {} and {}", interaction_constraint, body)
    ...
    def _cache_supported_posture_types(cls):
        ...
        for supported_posture_type_filter in cls.supported_posture_type_filter:
            if supported_posture_type_filter.participant_type in supported_posture_type_filters:
                ...
                supported_posture_type_filters[supported_posture_type_filter.participant_type] = (
                 supported_posture_type_filter.posture_type, supported_posture_type_filter.force_carry_state)
    ...
    def transition_constraint_intersection(self, sim, participant_type, final_constraint):
        ...
        for constraint_tuple in self._transition_constraints.get(participant_type, ()):
            if constraint_tuple.autonomous_only:
    ...
    def all_affordances_gen(cls, inst, context=None, **kwargs):
        ...
        if sim is not None:
            ...
            actor_mixers |= sim.career_tracker.get_cached_actor_mixers(class_to_check)
        ...
        if target_id is not None:
            ...
            if target_info is not None:
                ...
                yield from target_info.career_tracker.get_cached_provided_mixers_gen(class_to_check)
        ...
        if inst is not None:
            if inst.target is not None:
                ...
                if target_basic_content is not None:
                    target_content_set = target_basic_content.content_set
                    if target_content_set is not None:
                        if target_content_set().has_affordances():
                            return True
        return False
    ...
    def _test(cls, target, context, **interaction_parameters):
        ...
        for si in context.sim.si_state:
            ...
            if not (test_self and si_affordance.get_interaction_type() is cls_interaction_type):
                if si_affordance in cls.test_disallow_while_running.affordances:
                    if 'interaction_starting' in interaction_parameters:
                        is_starting = interaction_parameters['interaction_starting']
                    else:
                        is_starting = False
                    if not is_starting:
                        if cls.test_disallow_while_running.skip_target_check:
                            return TestResult(False, 'Currently running interaction')
                    if si.target is target:
                        if not is_starting:
                            return TestResult(False, 'Currently running interaction')
                if si.target is not None and si.target.is_part and si.target.part_owner is target:
                    return is_starting or TestResult(False, 'Currently running interaction')

        return TestResult.TRUE

    @classmethod
    def _should_test_affordance_filters(cls, context):
        if context.source == context.SOURCE_PIE_MENU:
            if not cls.simless:
                return True
        return False
    ...
    def _is_linked_to(cls, super_affordance):
        if cls.super_affordance_compatibility is not None:
            if cls.super_affordance_compatibility(super_affordance, allow_ignore_exclude_all=True):
                return True
        return False
    ...
    def is_linked_to(cls, inst, super_affordance):
        inst_or_cls = inst if inst is not None else cls
        if inst_or_cls.provided_posture_type is not None:
            if inst_or_cls.provided_posture_type.IS_BODY_POSTURE:
                if super_affordance.provided_posture_type is not None:
                    if super_affordance.provided_posture_type.IS_BODY_POSTURE:
                        return False
        if inst_or_cls.provided_posture_type is not None:
            if super_affordance.provided_posture_type is None:
                return cls._is_linked_to(super_affordance)
        if super_affordance.provided_posture_type is not None:
            if inst_or_cls.provided_posture_type is None:
    ...
    def _constraint_gen(cls, inst, sim, target, participant_type=ParticipantType.Actor, **kwargs):
        if inst is not None:
            if inst.is_finishing:
                return
        else:
            inst_or_cls = inst if inst is not None else cls
            found_constraint = False
            for constraint in (super(SuperInteraction, inst_or_cls)._constraint_gen)(sim, inst_or_cls.get_constraint_target(target), participant_type=participant_type, **kwargs):
                found_constraint = True
                yield constraint

            if inst is not None:
                if inst.sim.posture.source_interaction is inst:
                    if not inst.sim.posture.mobile:
                        if not inst.sim.posture.is_universal:
            ...
            if target is not None and target.additional_interaction_constraints is not None:
                for tuned_additional_constraint in target.additional_interaction_constraints:
                    ...
                    found_constraint = True
                    if affordance_links is None or affordance_links(cls):
    ...
    def combined_posture_preferences(self):
        if self.combinable_interactions:
            if self.combinable_interactions != self.get_combinable_interactions_with_safe_carryables():
    ...
    def combined_posture_target_preference(self):
        ...
        posture_target_preferences = None
        if self.posture_target_preference is not None:
            posture_target_preferences = self.posture_target_preference.copy()
        if self.tested_posture_target_preferences is not None:
            if posture_target_preferences is None:
                posture_target_preferences = {}
            for posture_preference_tag, tested_score_sum in self.tested_posture_target_preferences.items():
                posture_target_preferences[posture_preference_tag] = tested_score_sum.get_modified_value(self.get_resolver())

        return posture_target_preferences
    ...
    def _generate_connectivity(self, ignore_all_other_sis=False):
        if not self.transition is None:
            if self.transition.ignore_all_other_sis != ignore_all_other_sis:
            ...
            was_locked or self.unlock_required_sims()
        ...
        if self.is_cancel_aop:
            ...
            if self._prepare_liabilities is not None:
                for liability in self._prepare_liabilities:
                    if liability.path_generation_deferred():
                        return set()
    ...
    def _estimate_distance_cache_key(self):
        ...
        all_potentially_included_sis = frozenset(((si, si.is_guaranteed) for sim in required_sims for si in sim.si_state))
        return (self.constraint_intersection(posture_state=None).estimate_distance_cache_key(),
         self.super_affordance_compatibility,
         self.should_rally,
         self.sim,
         required_sims,
         all_potentially_included_sis)
    ...
    def get_affordance_weight_from_group_size(cls, party_size):
        ...
        logger.error(("Attempting to call get_affordance_weight_from_group_size() on affordance {} with 'Relationship Scoring' enabled but no tuned 'Party Size Weight Tuning' curve.".format(cls)), owner='rez')
        return 1

    def __init__(self, aop, context, *args, exit_functions=(), force_inertial=False, additional_post_run_autonomy_commodities=None, cancel_incompatible_with_posture_on_transition_shutdown=True, disable_saving=False, pushed_waiting_line=False, **kwargs):
        ...
        self._pushed_waiting_line = pushed_waiting_line
    ...
    def transition(self, value):
        ...
        if old_transition is not None:
            if old_transition.interaction is self:
    ...
    @property
    def continuation_affordance_chain(self):
        return self.context.continuation_affordance_chain
    ...
    def _setup_animation_actors(self):
        ...
        if carry_participant is not None:
            ...
            elif self.context.carry_target is not None and carry_target != self.context.carry_target:
        ...
        elif carry_participant is None:
    ...
    def map_create_target(self, created_obj):
        ...
        if carry_participant == ParticipantType.CreatedObject:
            ...
            if created_obj is not None:
                ...
                self.interaction_parameters.pop('created_target_id', None)
    ...
    def guaranteed_watcher(self):
        ...
        try:
            ...
            is_guaranteed = self.is_guaranteed()
            if is_guaranteed != was_guaranteed:
        ...
        for combinable in self.combinable_interactions:
            if combinable is self:
                ...
                if combinable_carry is not None and combinable_carry not in combined_carry_targets:
                    if not isinstance(combinable_carry, Definition):
                        if combinable_carry.is_sim:
                            continue
    ...
    def get_idle_behavior(self):
        ...
        if self.idle_animation_overrides is not None:
            idle_animation_override = get_tested_animation_override(self._sim, self.idle_animation_overrides)
            if idle_animation_override is not None:
                idle_animation = idle_animation_override
        if idle_animation is None:
            return
        return idle_animation(self)
    ...
    def _pre_perform(self):
        ...
        if self.staging:
            ...
            result = super()._pre_perform()
        if self.sim.get_autonomy_state_setting() == autonomy.settings.AutonomyState.MEDIUM:
            if self.is_user_directed:
    ...
    def _check_if_push_affordance_on_run(self):
        ...
        for actor in self.get_participants(push_affordance_on_run.actor):
            ...
            if actor is self.sim:
                ...
                elif push_affordance_on_run.carry_target is not None:
    ...
    def _setup_phase_tuning(self):
        if self._content_sets.phase_tuning is not None:
            if self._content_sets.num_phases > 0:
                participant = self.get_participant(self._content_sets.phase_tuning.target)
                tracker = participant.get_tracker(self._content_sets.phase_tuning.turn_statistic)
    ...
    def _post_perform(self):
        if self.suspended:
            if self.staging:
                ...
                services.get_event_manager().process_event((test_events.TestEvent.InteractionStaged), sim_info=(self.sim.sim_info),
                  interaction=self)
            return
        if not self.staging:
            self._update_autonomy_timer()
    ...
    def _parameterized_autonomy_helper_gen(self, timeline, commodity_info_list, context_bucket, participant_type=ParticipantType.Actor, fallback_notification=None, push_on_success_or_fail=None, as_continuation=True):
        ...
        for commodity_info in reversed(commodity_info_list):
            ...
            if self.carry_target is not None:
                ...
                if commodities or static_commodities:
                    request = ParameterizedAutonomyRequestInfo(commodities, static_commodities,
                      objects,
                      (commodity_info.retain_priority),
                      (commodity_info.retain_carry_target),
                      objects_to_ignore=objects_to_ignore,
                      randomization_override=(commodity_info.randomization_override),
                      radius_to_consider=(commodity_info.radius_to_consider),
                      consider_scores_of_zero=(commodity_info.consider_scores_of_zero),
                      retain_context_source=(commodity_info.retain_context_source),
                      test_connectivity_to_target=(commodity_info.test_connectivity_to_target),
                      ignore_user_directed_and_autonomous=(commodity_info.ignore_user_directed_and_autonomous))
    ...
    def _process_parameterized_autonomy_request_gen(self, timeline, sim, parameterized_requests, context_bucket, as_continuation=True):
        ...
        for parameterized_request in parameterized_requests:
            ...
            if parameterized_request.retain_carry_target:
                if not as_continuation:
            ...
            context = context.clone_for_parameterized_autonomy(self, source=context_source,
              priority=priority,
              bucket=context_bucket,
              group_id=group_id,
              continuation_id=continuation_id,
              visual_continuation_id=visual_continuation_id)
            autonomy_request = autonomy.autonomy_request.AutonomyRequest(sim, FullAutonomy, commodity_list=(parameterized_request.commodities),
              static_commodity_list=(parameterized_request.static_commodities),
              object_list=(parameterized_request.objects),
              ignored_object_list=(parameterized_request.objects_to_ignore),
              affordance_list=(parameterized_request.affordances),
              apply_opportunity_cost=False,
              is_script_request=True,
              ignore_user_directed_and_autonomous=(parameterized_request.ignore_user_directed_and_autonomous),
              context=context,
              si_state_view=(sim.si_state),
              limited_autonomy_allowed=True,
              radius_to_consider=(parameterized_request.radius_to_consider),
              consider_scores_of_zero=(parameterized_request.consider_scores_of_zero),
              autonomy_mode_label_override='ParameterizedAutonomy',
              test_connectivity_to_target_object=(parameterized_request.test_connectivity_to_target))
            selected_interaction = yield from autonomy_service.find_best_action_gen(timeline, autonomy_request,
              randomization_override=randomization_override)
        ...
        return (
         action_selected, selected_interaction)
        if False:
            yield None
    ...
    def provide_route_events(self, route_event_context, sim, path, failed_types=None, **kwargs):
        ...
        for route_event_tuple in self.route_events:
            ...
            for participant in participants:
                if participant is sim and self.can_provide_route_event(route_event_tuple.route_event, failed_types, resolver):
                    route_event_context.route_event_already_scheduled(route_event_tuple.route_event) or route_event_context.add_route_event(RouteEventType.INTERACTION_PRE, route_event_tuple.route_event(provider=self))
    ...
    def prepare_gen(self, timeline, cancel_incompatible_carry_interactions=False):
        ...
        if self.waiting_line is not None:
            if not self._pushed_waiting_line:
                if not self.target is not current_posture_target:
                    if self.waiting_line.allow_line_on_same_target:
                        self.push_route_nearby_and_wait_in_line()
                        self.cancel((FinishingType.WAIT_IN_LINE), cancel_reason_msg='Pushed a Waiting Line interaction.')
                        return InteractionQueuePreparationStatus.PUSHED_REPLACEMENT
        if cancel_incompatible_carry_interactions:
            if not self.cancel_incompatible_carry_interactions():
    ...
    def push_route_nearby_and_wait_in_line(self):
        ...
        if self.waiting_line.line_origin_override is not None:
            ...
            if destination_part is None:
                ...
                routing_surface = destination_part.routing_surface
        else:
            if self.target.has_component(WAITING_LINE_COMPONENT):
                ...
                if waiting_line is not None:
                    last_interaction_in_line = waiting_line.last_interaction_in_line
                    if not last_interaction_in_line.is_finishing:
                        last_sim_in_line = last_interaction_in_line.sim
                ...
                return constraint_circle.valid or False
            constraint_circle = constraint_circle.intersect(STAND_OR_SIT_CONSTRAINT)
            interaction_data_to_store = (
             self.aop, self.context, self.waiting_line.waiting_line_key)
            if self.waiting_line.wait_in_line_with_picked_sims:
                target = self.aop.target
                wait_in_line_together_situation = get_wait_in_line_together_situation(self.sim, target.id if target is not None else None, self.waiting_line.waiting_line_key)
                if wait_in_line_together_situation:
                    wait_in_line_together_situation.change_to_route_to_waiting_line_state()
                    waiting_nearby_sim_ids = wait_in_line_together_situation.waiting_nearby_sim_ids
                    self.aop.interaction_parameters['picked_item_ids'] = waiting_nearby_sim_ids
                else:
                    wait_together_participants = self.get_participants(ParticipantType.PickedSim)
                    if wait_together_participants is None:
                        logger.error('Unable to get wait in line with participants {} to start the WaitInLineTogetherSituation for {}. Canceling.', wait_together_participants, self)
                        self.cancel(FinishingType.TRANSITION_FAILURE, 'Failed to get participants for WaitInLineTogetherSituation.')
                        return
                    situation_id = start_wait_in_line_together_situation(self.sim, wait_together_participants, interaction_data_to_store)
                    if situation_id is None:
                        logger.error('Failed to start WaitInLineTogetherSituation for {}. Canceling {}.', self.sim, self)
                        self.cancel(FinishingType.TRANSITION_FAILURE, 'Failed to start WaitInLineTogetherSituation.')
                        return
                wait_in_line_together_situation = services.get_zone_situation_manager().get(situation_id)
        route_near_override = self.waiting_line.route_near_affordance_override
        route_near_affordance = route_near_override if route_near_override is not None else LineUtils.ROUTE_TO_WAITING_IN_LINE
        route_near_context = InteractionContext((self.sim), (self.context.source),
          (self.priority),
          insert_strategy=(QueueInsertStrategy.FIRST),
          cancel_if_incompatible_in_queue=True)

        def push_wait_in_line(_):
            self._aop.interaction_parameters['pushed_waiting_line'] = True
            if self.waiting_line.use_origin_override_for_constraints:
                constrain_target = destination_part
            else:
                constrain_target = self.target
            line_head_data_to_store = (
             self.waiting_line.line_head_position,
             self.waiting_line.line_head_angle,
             self.waiting_line.line_cone,
             self.waiting_line.line_head_los_constraint,
             constrain_target,
             self.waiting_line.line_update_timing)
            ...
            self.sim.push_super_affordance(wait_in_line_affordance, constrain_target,
              continuation_context,
              allow_posture_changes=False,
              must_run_next=True,
              interaction_data=interaction_data_to_store,
              line_head_data=line_head_data_to_store)

        def route_nearby_fn():
            target = destination_part if destination_part is not None else self.target
            affordances_to_push = (
             route_near_affordance,)
            if self.should_rally:
                rallyable_route_near_affordances = {aop.affordance for aop in self.get_rallyable_aops_gen(target, route_near_context,
                  affordance=route_near_affordance)}
                if rallyable_route_near_affordances:
                    affordances_to_push = rallyable_route_near_affordances
            for affordance in affordances_to_push:
                self.sim.push_super_affordance(affordance, target,
                  route_near_context,
                  constraint_to_satisfy=constraint_circle,
                  allow_posture_changes=True,
                  run_element=(element_utils.build_element(push_wait_in_line)),
                  name_override='RouteToWaitingLine',
                  display_name_override=(self.display_name))

        if not self.waiting_line.wait_in_line_with_picked_sims:
            route_nearby_fn()
        else:
            wait_in_line_together_situation.route_nearby_fn = route_nearby_fn
            wait_in_line_together_situation.set_route_nearby_affordance(route_near_affordance)
    ...
    def run_pre_transition_behavior(self):
        if self.is_user_directed:
            ...
            if target_sim is not None:
                if self.is_user_directed and target_sim.is_npc:
                    if not target_sim.sim_info.lives_here:
                        if self.sim.sim_info.lives_here or services.get_zone_situation_manager().is_player_greeted():
                            if self.sim.is_on_active_lot():
    ...
    def on_added_to_queue(self, *args, **kwargs):
        if self.target is not None:
            ...
            if self.waiting_line is not None:
                wait_for_continuations = self.waiting_line.wait_for_continuations
                if wait_for_continuations is not None:
                    self.add_liability(WaitingLineInteractionChainLiability.LIABILITY_TOKEN, WaitingLineInteractionChainLiability(self, wait_for_continuations))
                if self.target.has_component(WAITING_LINE_COMPONENT):
                    is_not_chosen_dest = lambda dest:                     if dest not in self.target.waiting_line_component.chosen_destinations:
True # Avoid dead code: False
                    self.additional_destination_validity_tests.append(is_not_chosen_dest)
    ...
    def _maybe_acquire_posture_ownership(self):
        if not self.can_acquire_posture_ownership:
            ...
            if self.is_finishing:
                return
        elif not self.disable_transitions:
            if not self.is_compatible_with_stand_at_none:
                if self.target is not None:
    ...
    def run_direct_gen(self, timeline, source_interaction=None, pre_run_behavior=None, included_sis=None, acquire_posture_ownership=True):
        ...
        if result:
            if self.pipeline_progress < PipelineProgress.PREPARED:
                ...
                if status == InteractionQueuePreparationStatus.NEEDS_DERAIL:
                    if self.transition is not None:
    ...
    def should_replace_posture_source_interaction(new_interaction):
        if new_interaction.simless:
            return False
        if new_interaction.sim.posture.posture_type is new_interaction.provided_posture_type:
            if new_interaction.sim.posture.source_interaction is not new_interaction:
                return new_interaction.sim.posture.multi_sim and new_interaction.sim.posture.is_puppet or True
        return False
    ...
    def acquire_posture_ownership(self, posture):
        if posture.target is None:
            if posture.mobile:
                if not posture.skip_route:
                    return
        if posture.track == PostureTrack.BODY:
            if self.is_compatible_with_stand_at_none:
                return
    ...
    def _run_interaction_gen(self, timeline):
        if self.should_replace_posture_source_interaction(self):
            ...
            if posture_source_interaction is not None:
                if not posture_source_interaction.is_finishing:
    ...
    def kill(self):
        if self.has_been_killed:
            return False
        on_cancel_aops = self.get_cancel_replacement_aops_contexts_postures()
        if on_cancel_aops:
    ...
    def _get_cancel_replacement_context_for_posture(self, posture, affordance, always_run, enqueue_first):
        if posture is None:
            context_source = InteractionContext.SOURCE_AUTONOMY
            continuation_id = None
            priority = self.priority
        else:
            if posture.is_vehicle:
                context_source = InteractionContext.SOURCE_VEHICLE_CANCEL_AOP
                continuation_id = None
        ...
        if enqueue_first:
            priority = interactions.priority.Priority.High
            insert_strategy = QueueInsertStrategy.FIRST
        elif affordance.is_basic_content_one_shot:
            run_priority = None
        else:
        ...
        context = InteractionContext((self.sim), source=context_source, priority=priority,
          carry_target=(self.carry_target),
    ...
    def get_cancel_replacement_aops_contexts_postures(self, can_transfer_ownership=True, carry_cancel_override=None):
        ...
        if can_transfer_ownership:
            if carry_target is not None:
                if carry_target.parent is self.sim:
                    if not self.is_putdown:
                        head_interaction = self.sim.queue.get_head()
                        if head_interaction is not None:
                            if head_interaction.is_super:
                                if head_interaction is not self:
        ...
        for posture in reversed(self.sim.posture_state.aspects):
            if new_carry_owner is not None:
                if posture.target is carry_target:
                    ...
                    if self.cancel_replacement_affordances:
                        for posture_track_group, cancel_affordance_info in self.cancel_replacement_affordances.items():
                            ...
                            if cancel_affordance_info.target is None:
                                ...
                                enqueue_first = cancel_affordance_info.enqueue_first
                                break
                        else:
                    ...
                    if posture.last_owning_interaction(self):
                        if proxy_posture_owner is None:
                            if cancel_affordance is not None:
                                ...
                                context = self._get_cancel_replacement_context_for_posture(posture, cancel_affordance, always_run, enqueue_first)
                                cancel_aops_contexts_postures.append((cancel_replacement_aop, context, posture))
                                continue
                            if posture.source_interaction is not None:
                                if posture.source_interaction is not self:
                                    cancel_aops = posture.source_interaction.get_cancel_replacement_aops_contexts_postures(can_transfer_ownership=False, carry_cancel_override=carry_cancel_override)
                ...
                if posture.source_interaction is self:
                    if cancel_affordance is not None:
                        ...
                        if posture.track == PostureTrack.BODY:
                            ...
                            if posture.target is not None and posture.target.valid_for_distribution and posture.target.reset_reason != ResetReason.BEING_DESTROYED:
                                if posture.target in self.get_participants(ParticipantType.All):
                                    affordance = self.CARRY_POSTURE_REPLACEMENT_AFFORDANCE
                                    if carry_cancel_override is not None:
                                        ...
                                        continue
                            context = self._get_cancel_replacement_context_for_posture(posture, replacement_aop.affordance, always_run, enqueue_first)
    ...
    def _try_exit_via_cancel_aop(self, carry_cancel_override=None):
        ...
        if sim is None or sim.is_being_destroyed:
            ...
            on_cancel_aops_contexts_postures = self.get_cancel_replacement_aops_contexts_postures(carry_cancel_override=carry_cancel_override)
            return on_cancel_aops_contexts_postures or sim.posture_state.is_source_interaction(self)
        ...
        for on_cancel_aop, context, posture in on_cancel_aops_contexts_postures:
            if on_cancel_aop.affordance is None:
                ...
                if context.source == InteractionSource.BODY_CANCEL_AOP:
                    if sim.parent is not None:
                        if sim.parent.is_sim:
                            continue
                else:
                    if self in sim.si_state:
                        if posture.source_interaction is self or posture is sim.posture:
                            if self.staging:
                                prevent_from_canceling = not self.satisfied
                    if not sim.queue.needs_cancel_aop(on_cancel_aop, context):
                        continue
                    if posture.has_linked_sim_pushed_cancel(carry_cancel_override=carry_cancel_override) and not context.insert_strategy == QueueInsertStrategy.FIRST:
                        continue
                if posture in self._has_pushed_cancel_aop:
                    continue
                cancel_aop_result = on_cancel_aop.test(context)
                cancel_aop_result or logger.warn('Failed to push the cancelation replacement effect ({} -> {}) for {}: {}.', posture, on_cancel_aop, self, cancel_aop_result)
            ...
            if not context.source == InteractionContext.SOURCE_BODY_CANCEL_AOP:
                if context.source == InteractionContext.SOURCE_VEHICLE_CANCEL_AOP:
    ...
    def _cancel_eventually(self, *args, immediate=False, **kwargs):
        if not self.is_finishing:
            if self._cancel_deferred is not None:
                if not immediate:
    ...
    def _cancel(self, finishing_type, cancel_reason_msg, notify_UI=False, lifetime_state=None, log_id=None, ignore_must_run=False, carry_cancel_override=None):
        ...
        if user_cancel_liability is not None:
        ...
        try:
            try:
                ...
                if self.sim is not None:
                    if notify_UI and not self.is_finishing_naturally:
                        if not self._finisher.has_pending_natural_finisher:
                ...
                if self._interrupt_active_work(finishing_type=finishing_type, cancel_reason_msg=cancel_reason_msg):
                    if not self._try_exit_via_cancel_aop(carry_cancel_override=carry_cancel_override):
                        self._active = False
                        ...
                        if self.visual_type_override_data.group_cancel_behavior == CancelGroupInteractionType.ALL or self.user_canceled:
                            for grouped_int_info in self.sim.ui_manager.get_grouped_interaction_gen(self.id):
                                ...
                                if grouped_interaction is not None:
                                    grouped_interaction.cancel(finishing_type=finishing_type, cancel_reason_msg='Canceled by group interaction being canceled',
    ...
    def displace(self, displaced_by, cancel_reason_msg=None, **kwargs):
        if not displaced_by.source == InteractionContext.SOURCE_BODY_CANCEL_AOP:
            if displaced_by.source == InteractionContext.SOURCE_CARRY_CANCEL_AOP:
                if self.transition is not None:
                    if not self.transition.succeeded:
                        actor = self.get_participant(ParticipantType.Actor)
                        self.transition.derail(DerailReason.DISPLACE, actor)
                        return False
        else:
            notify_ui = True
            if self.source == InteractionContext.SOURCE_AUTONOMY:
        ...
        return self._cancel_eventually((FinishingType.DISPLACED), cancel_reason_msg=cancel_reason_msg,
          notify_UI=notify_ui,
          log_id='Displace_SI',
          carry_cancel_override=carry_cancel_override)
    ...
    def _auto_complete(self, force_satisfy=False):
        if force_satisfy:
            self.satisfied = True
            self.force_inertial = True
        return self._cancel_eventually((FinishingType.NATURAL), cancel_reason_msg='Interaction Auto Completed',
          log_id='Complete_SI',
    ...
    def cancel(self, finishing_type, cancel_reason_msg, **kwargs):
        ...
        return (self._cancel_eventually)(finishing_type,
 cancel_reason_msg, notify_UI=True, 
         lifetime_state=LifetimeState.CANCELED, 
         log_id='Cancel_SI', **kwargs)
    ...
    def _interrupt_active_work(self, kill=False, finishing_type=None, cancel_reason_msg=None):
        ...
        elif self._post_guaranteed_autonomy_element is not None:
            if kill:
    ...
    def _exit(self, timeline, allow_yield):
        exit_functions = self.affordance.immediate or self._exit_functions
        self._exit_functions = None
        exit_si_element = None
        self._lifetime_state = LifetimeState.EXITED
        if allow_yield:
            while self.started:
                exit_si_element = self.stopped or self.next_stage()
    ...
    def _exited_pipeline(self, *args, **kwargs):
        ...
        if self.transition is not None:
            ...
            self.transition = None
            if not self.owning_transition_sequences or transition not in self.owning_transition_sequences:
    ...
    def attach_interaction(self, interaction, cancel_continuations=False):
        ...
        liability.add_cancel_entry((interaction.context.sim), interaction, cancel_continuations=cancel_continuations)
        if interaction.source != InteractionSource.REACTION:
    ...
    def get_interaction_locked_params(self, actor_name, target_name):
        if self.animation_stat is None:
            return {}

        def get_asm_param--- This code section failed: ---

 L.5546         0  LOAD_FAST                'skill_sim'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_TRUE     30  'to 30'
                8  LOAD_FAST                'skill_sim'
               10  LOAD_ATTR                is_sim
               12  POP_JUMP_IF_FALSE    30  'to 30'
               14  LOAD_FAST                'skill_stat'
               16  LOAD_CONST               None
               18  COMPARE_OP               is
               20  POP_JUMP_IF_TRUE     30  'to 30'
               22  LOAD_FAST                'skill_actor_name'
               24  LOAD_CONST               None
               26  COMPARE_OP               is
               28  POP_JUMP_IF_FALSE    34  'to 34'
             30_0  COME_FROM            20  '20'
             30_1  COME_FROM            12  '12'
             30_2  COME_FROM             6  '6'

 L.5547        30  BUILD_MAP_0           0 
               32  RETURN_VALUE     
             34_0  COME_FROM            28  '28'

 L.5549        34  LOAD_FAST                'skill_stat'
               36  LOAD_DEREF               'self'
               38  LOAD_METHOD              get_resolver
               40  CALL_METHOD_0         0  '0 positional arguments'
               42  LOAD_CONST               ('resolver',)
               44  CALL_FUNCTION_KW_1     1  '1 total positional and keyword args'
               46  STORE_FAST               'tested_result'

 L.5550        48  LOAD_FAST                'tested_result'
               50  LOAD_CONST               None
               52  COMPARE_OP               is
               54  POP_JUMP_IF_FALSE    60  'to 60'

 L.5551        56  BUILD_MAP_0           0 
               58  RETURN_VALUE     
             60_0  COME_FROM            54  '54'

 L.5553        60  LOAD_FAST                'skill_sim'
               62  LOAD_METHOD              get_stat_instance
               64  LOAD_FAST                'tested_result'
               66  CALL_METHOD_1         1  '1 positional argument'
               68  STORE_FAST               'animation_stat'

 L.5554        70  LOAD_FAST                'animation_stat'
               72  LOAD_CONST               None
               74  COMPARE_OP               is-not
               76  POP_JUMP_IF_FALSE   120  'to 120'

 L.5555        78  LOAD_FAST                'animation_stat'
               80  LOAD_METHOD              get_asm_param
               82  CALL_METHOD_0         0  '0 positional arguments'
               84  UNPACK_SEQUENCE_2     2 
               86  STORE_FAST               'asm_param_name'
               88  STORE_FAST               'asm_param_value'

 L.5556        90  LOAD_FAST                'asm_param_name'
               92  LOAD_CONST               None
               94  COMPARE_OP               is-not
               96  POP_JUMP_IF_FALSE   180  'to 180'
               98  LOAD_FAST                'asm_param_value'
              100  LOAD_CONST               None
              102  COMPARE_OP               is-not
              104  POP_JUMP_IF_FALSE   180  'to 180'

 L.5557       106  LOAD_FAST                'asm_param_name'
              108  LOAD_FAST                'skill_actor_name'
              110  BUILD_TUPLE_2         2 
              112  LOAD_FAST                'asm_param_value'
              114  BUILD_MAP_1           1 
              116  RETURN_VALUE     
              118  JUMP_FORWARD        180  'to 180'
            120_0  COME_FROM            76  '76'

 L.5558       120  LOAD_FAST                'tested_result'
              122  LOAD_ATTR                stat_asm_param
              124  LOAD_CONST               None
              126  COMPARE_OP               is-not
              128  POP_JUMP_IF_FALSE   180  'to 180'

 L.5560       130  LOAD_FAST                'tested_result'
              132  LOAD_ATTR                default_value
              134  STORE_FAST               'default_value'

 L.5561       136  LOAD_FAST                'tested_result'
              138  LOAD_ATTR                stat_asm_param
              140  LOAD_METHOD              get_asm_param_for_value
              142  LOAD_FAST                'default_value'
              144  CALL_METHOD_1         1  '1 positional argument'
              146  UNPACK_SEQUENCE_2     2 
              148  STORE_FAST               'asm_param_name'
              150  STORE_FAST               'asm_param_value'

 L.5562       152  LOAD_FAST                'asm_param_name'
              154  LOAD_CONST               None
              156  COMPARE_OP               is-not
              158  POP_JUMP_IF_FALSE   180  'to 180'
              160  LOAD_FAST                'asm_param_value'
              162  LOAD_CONST               None
              164  COMPARE_OP               is-not
              166  POP_JUMP_IF_FALSE   180  'to 180'

 L.5563       168  LOAD_FAST                'asm_param_name'
              170  LOAD_FAST                'skill_actor_name'
              172  BUILD_TUPLE_2         2 
              174  LOAD_FAST                'asm_param_value'
              176  BUILD_MAP_1           1 
              178  RETURN_VALUE     
            180_0  COME_FROM           166  '166'
            180_1  COME_FROM           158  '158'
            180_2  COME_FROM           128  '128'
            180_3  COME_FROM           118  '118'
            180_4  COME_FROM           104  '104'
            180_5  COME_FROM            96  '96'

 L.5565       180  BUILD_MAP_0           0 
              182  RETURN_VALUE     
               -1  RETURN_LAST      
...
Parse error at or near `RETURN_VALUE' instruction at offset 182
        ...
        locked_params += get_asm_param(self.animation_stat.actor_stat, self.sim, actor_name)
        locked_params += get_asm_param(self.animation_stat.target_stat, self.target, target_name)
    ...
    def set_stat_asm_parameter(self, asm, actor_name, target_name, sim, target):
        ...
        def set_asm_param--- This code section failed: ---

 L.5584         0  LOAD_FAST                'skill_sim'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_TRUE     30  'to 30'
                8  LOAD_FAST                'skill_sim'
 ...
 L.5585        30  LOAD_CONST               None
               32  RETURN_VALUE     
             34_0  COME_FROM            28  '28'

 L.5587        34  LOAD_FAST                'skill_stat'
               36  LOAD_DEREF               'self'
               38  LOAD_METHOD              get_resolver
               40  CALL_METHOD_0         0  '0 positional arguments'
               42  LOAD_CONST               ('resolver',)
 ...
 L.5588        48  LOAD_FAST                'tested_result'
               50  LOAD_CONST               None
               52  COMPARE_OP               is
               54  POP_JUMP_IF_FALSE    60  'to 60'

 L.5589        56  LOAD_CONST               None
               58  RETURN_VALUE     
             60_0  COME_FROM            54  '54'

 L.5591        60  LOAD_FAST                'skill_sim'
               62  LOAD_ATTR                get_stat_instance
               64  LOAD_FAST                'tested_result'
               66  LOAD_CONST               True
               68  LOAD_CONST               ('add',)
               70  CALL_FUNCTION_KW_2     2  '2 total positional and keyword args'
               72  STORE_FAST               'animation_stat'

 L.5592        74  LOAD_FAST                'animation_stat'
               76  LOAD_CONST               None
               78  COMPARE_OP               is-not
               80  POP_JUMP_IF_FALSE   126  'to 126'

 L.5593        82  LOAD_FAST                'animation_stat'
               84  LOAD_METHOD              get_asm_param
               86  CALL_METHOD_0         0  '0 positional arguments'
               88  UNPACK_SEQUENCE_2     2 
               90  STORE_FAST               'asm_param_name'
               92  STORE_FAST               'asm_param_value'

 L.5594        94  LOAD_FAST                'asm_param_name'
               96  LOAD_CONST               None
               98  COMPARE_OP               is-not
              100  POP_JUMP_IF_FALSE   126  'to 126'
              102  LOAD_FAST                'asm_param_value'
              104  LOAD_CONST               None
              106  COMPARE_OP               is-not
              108  POP_JUMP_IF_FALSE   126  'to 126'

 L.5595       110  LOAD_DEREF               'asm'
              112  LOAD_METHOD              set_actor_parameter
              114  LOAD_FAST                'skill_actor_name'
              116  LOAD_FAST                'skill_sim'
              118  LOAD_FAST                'asm_param_name'
              120  LOAD_FAST                'asm_param_value'
              122  CALL_METHOD_4         4  '4 positional arguments'
              124  POP_TOP          
...
Parse error at or near `COME_FROM' instruction at offset 126_1
        ...
        set_asm_param(self.animation_stat.actor_stat, sim, actor_name)
        if target is not None:
            set_asm_param(self.animation_stat.target_stat, target, target_name)

    def set_walkstyle_asm_parameter(self, asm, actor_name, target_name, sim, target):
 ...
 L.5615         0  LOAD_FAST                'walkstyle_sim'
                2  LOAD_CONST               None
                4  COMPARE_OP               is
                6  POP_JUMP_IF_TRUE     30  'to 30'
                8  LOAD_FAST                'walkstyle_sim'
               10  LOAD_ATTR                is_sim
               12  POP_JUMP_IF_FALSE    30  'to 30'

 L.5616        14  LOAD_FAST                'walkstyle'
               16  LOAD_CONST               None
               18  COMPARE_OP               is
               20  POP_JUMP_IF_TRUE     30  'to 30'
               22  LOAD_FAST                'walkstyle_actor_name'
               24  LOAD_CONST               None
               26  COMPARE_OP               is
               28  POP_JUMP_IF_FALSE    34  'to 34'
             30_0  COME_FROM            20  '20'
 ...
 L.5617        30  LOAD_CONST               None
               32  RETURN_VALUE     
             34_0  COME_FROM            28  '28'

 L.5619        34  LOAD_FAST                'walkstyle'
               36  LOAD_DEREF               'resolver'
               38  LOAD_CONST               ('resolver',)
               40  CALL_FUNCTION_KW_1     1  '1 total positional and keyword args'
               42  STORE_FAST               'tested_result'

 L.5620        44  LOAD_FAST                'tested_result'
               46  LOAD_CONST               None
               48  COMPARE_OP               is
               50  POP_JUMP_IF_FALSE    56  'to 56'

 L.5621        52  LOAD_CONST               None
               54  RETURN_VALUE     
             56_0  COME_FROM            50  '50'

 L.5625        56  LOAD_DEREF               'asm'
               58  LOAD_METHOD              set_actor_parameter
               60  LOAD_FAST                'walkstyle_actor_name'
               62  LOAD_FAST                'walkstyle_sim'

 L.5626        64  LOAD_STR                 'walkstyle'
               66  LOAD_FAST                'tested_result'
               68  LOAD_ATTR                animation_parameter
               70  CALL_METHOD_4         4  '4 positional arguments'
               72  POP_TOP          

 L.5627        74  LOAD_DEREF               'asm'
               76  LOAD_METHOD              set_actor_parameter
               78  LOAD_FAST                'walkstyle_actor_name'
               80  LOAD_FAST                'walkstyle_sim'

 L.5628        82  LOAD_STR                 'walkstyle_override'
               84  LOAD_FAST                'tested_result'
               86  CALL_METHOD_4         4  '4 positional arguments'
               88  POP_TOP          

Parse error at or near `CALL_METHOD_4' instruction at offset 86

        set_asm_param(self.animation_walkstyle_override.actor_walkstyle, sim, actor_name)
        set_asm_param(self.animation_walkstyle_override.target_walkstyle, target, target_name)
    ...
    def perform_gen(self, timeline):
        ...
        if not self.is_guaranteed():
            if self.force_exit_on_inertia or self.one_shot:
                self._auto_complete()
        return result
        if False:
            yield None
    ...
    def create_load_context(cls, sim, source, priority):
        context = interactions.context.InteractionContext(sim, source,
          priority,
          restored_from_load=True)
        return context
    ...
    def get_on_route_change(self, **kwargs):
        if self.outfit_change is not None:
            if self.outfit_change.on_route_change is not None:
    ...
    def get_tuned_outfit_changes(self, include_exit_changes=True):
        ...
        if pre_route_change is not None:
            ...
            return overrides or outfit_changes
    ...
    def can_reserve_target(cls, inst, *, target, context=DEFAULT):
        ...
        sim = context.sim
        if target.is_prop:
            logger.error('Sim {} is checking has_reservation_tests from prop object {} in interaction {}.', sim,
              target, inst_or_cls, owner='mkartika')
            return False
        else:
            if not target._has_reservation_tests:
                return ReservationResult.TRUE
            return target.is_part or ReservationResult.TRUE
            ...
            for using_sim in part.get_users(sims_only=True):
                ...
                for si in using_sim.si_state:
                    ...
                    if reserve_object_tests:
                        ...
                        return reserve_result or ReservationResult(reserve_result, result_obj=using_sim)
