...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\base\teleport_interaction.py
# Compiled at: 2020-12-07 20:36:38
# Size of source mod 2**32: 22653 bytes
...
import element_utils, placement, routing, services, sims4.log, sims4.math
...
class TeleportHereInteraction(TerrainSuperInteraction):
    ...
    def _test(cls, target, context, **kwargs):
        ...
        if position is None or surface is None:
            ...
            return routing.test_connectivity_permissions_for_handle(routing.connectivity.Handle(location), context.sim.routing_context) or TestResult(False, 'Cannot TeleportHere! Unroutable area.')
...
class TeleportInteraction(SuperInteraction):
    ...
    def _run_interaction_gen(self, timeline):
        for goal in self.dest_goals:
            ...
            if self.use_fgl:
                if goal_surface.type != SurfaceType.SURFACETYPE_WORLD:
                    goal_surface = SurfaceIdentifier(goal_surface.primary_id, goal_surface.secondary_id, SurfaceType.SURFACETYPE_WORLD)
                starting_loc = placement.create_starting_location(location=goal_location)
                fgl_context = placement.create_fgl_context_for_sim(starting_loc, self.sim)
                position, orientation = placement.find_good_location(fgl_context)
                end_transform = sims4.math.Transform(position, orientation)
                goal_location = sims4.math.Location(end_transform, goal_surface)
...
class TeleportToActiveSimInteraction(TeleportInteraction):
...
lock_instance_tunables(TeleportToActiveSimInteraction, _constraints_actor=(ParticipantType.Invalid))
...
class TeleportStyleSuperInteraction(TerrainSuperInteraction):
    ...
    def _test(cls, target, context, **kwargs):
        ...
        if not result:
            ...
            location = routing.Location(position, sims4.math.Quaternion.IDENTITY(), surface)
            return routing.test_connectivity_permissions_for_handle(routing.connectivity.Handle(location), context.sim.routing_context) or TestResult(False, 'Unroutable area.')
        return TestResult.TRUE
            ...
            if starting_position is not None:
                if routing_surface_id is not None:
                    if desired_level is not None:
                        ...
                        sim_head_room = get_required_height_clearance((self.sim), override_tuning=(self.min_height_clearance))
                        if self.destination_jig is not None:
                            ...
                            if self._destination_jig_object is not None:
                                position, orientation = TeleportHelper.get_fgl_at_destination_for_teleport(starting_location, (self._destination_jig_object),
                                  destination_must_be_outside=(self.destination_must_be_outside),
                                  min_head_room=sim_head_room)
                                if position is not None and orientation is not None:
                                    self._destination_jig_object.move_to(translation=position, orientation=orientation,
                                    ...
                                    self._teleport_location = routing.Location((jig_slot_concat_transform.translation), orientation=(jig_slot_concat_transform.orientation),
                                      routing_surface=routing_surface_id)
                        else:
                            final_position, _ = TeleportHelper.get_fgl_at_destination_for_teleport(starting_location, (self.sim),
                              ignore_connectivity=True,
                              destination_must_be_outside=(self.destination_must_be_outside),
                              min_head_room=sim_head_room)
                        if final_position is not None:
                            position, orientation = TeleportHelper.get_fgl_at_destination_for_teleport(starting_location, (self.sim),
                              destination_must_be_outside=(self.destination_must_be_outside),
                              min_head_room=sim_head_room)
                            if position is None:
                                if services.current_zone().lot.is_position_on_lot(starting_position):
                                    ...
                                    if front_door:
                                        ...
                                        door_location = routing.Location((front_door.position), routing_surface=routing_surface_id)
                                        position, orientation = TeleportHelper.get_fgl_at_destination_for_teleport(door_location, (self.sim), destination_must_be_outside=(self.destination_must_be_outside),
                                          ignore_connectivity=True,
                                          min_head_room=sim_head_room)
                            if position is not None:
                                if orientation is not None:
                                    self._teleport_location = routing.Location(position, orientation=orientation,
                                      routing_surface=routing_surface_id)
        except Exception as exception:
            try:
                logger.exception('Exception while getting teleport location for TeleportStyleSuperInteraction: ', exc=exception,
    ...
    def build_basic_content(self, sequence=(), *args, **kwargs):
        ...
        def _perform_teleport(timeline):
            teleport = yield from TeleportHelper.perform_teleport_gen(timeline, (self.sim),
              (self._teleport_location),
              style_tuning=(self.teleport_style_tuning))
            return teleport
            if False:
                yield None
