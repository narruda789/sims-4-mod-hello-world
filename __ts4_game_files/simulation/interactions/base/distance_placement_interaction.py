...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\base\distance_placement_interaction.py
# Compiled at: 2021-03-22 20:29:56
# Size of source mod 2**32: 25476 bytes
...
import element_utils, interactions, routing, services, sims4.geometry, sims4.math
...
class DistancePlacementMixin:
    ...
    def _get_distance_placement_constraint(cls, inst, sim, target, participant_type=ParticipantType.Actor):
        ...
        if inst is None or participant_type != ParticipantType.Actor:
            ...
            constraint = ANYWHERE
            if not inst._distance_placement_transform is None:
                if inst._bounce_transform is None:
                ...
                constraint = constraint.intersect(interactions.constraints.Circle((inst_or_cls._distance_placement_transform.translation), (inst_or_cls.placement_distance.upper_bound + DistancePlacementMixin.CONSTRAINT_RADIUS_BUFFER),
    ...
    def find_starting_location(self):
        if self.carry_target is not None and self.minimum_requirement_jig is not None:
            ...
            fgl_kwargs = {'ignored_object_ids':{sim.id for sim in self.required_sims()}, 
            ...
            def find_a_starting_location():
                for transform_a, transform_b, routing_surface, _ in self.minimum_requirement_jig.get_transforms_gen((self.sim), (self.carry_target), actor_loc=loc_a, target_loc=loc_b, fgl_kwargs=fgl_kwargs):
                    if check_on_lot:
                        if lot.is_position_on_lot(transform_a.translation):
            ...
            is_in_pond = bool(build_buy.get_pond_id(loc_a.transform.translation))
            use_pool_surface = loc_a.routing_surface.type == SurfaceType.SURFACETYPE_POOL or 0 < get_water_depth_at_location(loc_a) and not is_in_pond
            if use_pool_surface:
    ...
    def _entered_pipeline(self):
        if self.carry_target is not None:
            ...
            fgl_kwargs = {'raytest_radius':self.raytest_radius,  'raytest_start_offset':self.raytest_offset.lower_bound, 
             'raytest_end_offset':self.raytest_offset.upper_bound, 
             'ignored_object_ids':{sim.id for sim in self.required_sims()}, 
             'positioning_type':JigPositioning.RelativeToSimA}
            ...
            if pick is not None and pick.routing_surface.type == SurfaceType.SURFACETYPE_POOL:
                ...
                fgl_kwargs.update({'search_flags':fgl_flags,  'restrictions':(
                  sims4.geometry.RelativeFacingRange(self.sim.position, 0),)})
                ...
                if self._starting_location is not None:
                    ...
                    fgl_kwargs.update({'max_distance':self.placement_distance.upper_bound,  'min_distance':self.placement_distance.lower_bound, 
                     'restrictions':(
                      sims4.geometry.RelativeFacingRange(loc_a.transform.translation, 0),), 
        ...
        if self._distance_placement_transform is None:
            ...
            if self.carry_target is None:
                ...
                if not asm.set_actor_parameter(self.thrown_object_actor_name, self.carry_target, animation_constants.ASM_TARGET_TRANSLATION, self._distance_placement_transform.translation):
                    return False
                return asm.set_actor_parameter(self.thrown_object_actor_name, self.carry_target, animation_constants.ASM_TARGET_ORIENTATION, self._distance_placement_transform.orientation) or False
    ...
    def build_basic_content(self, *args, **kwargs):
        ...
        def plan_fetch_paths(timeline):
            for target_sim in self.get_participants(ParticipantType.TargetSim | ParticipantType.Listeners):
                fetch_constraint = self.fetch_constraint.create_constraint(target_sim, target_position=(self._distance_placement_transform.translation),
                  routing_surface=(self._routing_surface if self._routing_surface is not None else DEFAULT))
