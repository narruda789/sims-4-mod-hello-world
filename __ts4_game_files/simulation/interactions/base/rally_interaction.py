...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\base\rally_interaction.py
# Compiled at: 2020-10-06 19:10:24
# Size of source mod 2**32: 16681 bytes
...
class RallyInteraction(ProxyInteraction):
    ...
    def generate_continuation_affordance(cls, affordance, **kwargs):
        return (RallyInteraction.generate)(affordance, rally_tag=cls.rally_tag, 
         rally_level=cls.rally_level + 1, 
         rally_data=None, 
         rally_sources=cls.rally_sources, **kwargs)
    ...
    def _run_interaction_gen(self, timeline):
        ...
        if self._push_social is not None:
    ...
    def _do_rally_behavior(self, sim, constraint):
        if self.rally_data is not None:
            ...
            if self.rally_constraint is not None:
                ...
                if hasattr(self.rally_data, 'preserve_preferred_objects'):
                ...
                if affordance is None or static_commodity is not None:
                    if static_commodity is not None:
                        request = AutonomyRequest(sim, static_commodity_list=(static_commodity,), skipped_static_commodities=None,
                          object_list=(self._rally_targets),
                          constraint=constraint,
                          context=context,
                          autonomy_mode=FullAutonomy,
                          autonomy_mode_label_override='RallyBehavior')
                        ...
                        if self._rally_targets is None:
                            ...
                            for scored_interaction_data in autonomy_result:
                                ...
                                if possible_target is not None:
                                    if possible_target.parts is not None:
                                        for part in possible_target.parts:
                                            if not part.in_use or part.in_use_by(sim):
            ...
            if affordance is not None:
                ...
                return (sim.push_super_affordance)(affordance, target, context, from_rally_interaction=self, 
                 push_social=self.rally_push_social, **interaction_parameters)
            return False
    ...
    def maybe_bring_group_along(self, **kwargs):
        if not self.should_rally:
            ...
            elif RallySource.SOCIAL_GROUP in self.rally_sources:
                main_group = self.sim.get_visible_group()
                if main_group:
