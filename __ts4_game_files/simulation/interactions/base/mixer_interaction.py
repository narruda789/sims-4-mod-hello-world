...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\interactions\base\mixer_interaction.py
# Compiled at: 2021-06-15 19:03:25
# Size of source mod 2**32: 41231 bytes
import weakref
...
import gsi_handlers.interaction_archive_handlers, gsi_handlers.sim_timeline_handlers, interactions.constraints, performance.counters, services, sims4.log, sims4.resources
...
class MixerInteraction(Interaction):
    ...
    def _test(cls, target, context, **kwargs):
        if cls.optional:
            if not cls.is_mixer_compatible((context.sim), target, participant_type=(ParticipantType.Actor)):
    ...
    def filter_mixer_targets(cls, super_interaction, potential_targets, actor, affordance=None):
        if cls.target_type & TargetType.ACTOR:
            ...
            if cls.target_type & TargetType.TARGET or cls.target_type & TargetType.OBJECT:
                ...
                if cls.target_type & TargetType.GROUP:
    ...
    def should_cancel_on_si_cancel(self, interaction):
        if self.interruptible:
            return True
        if self.super_interaction is interaction:
            return self.looping
        return False
    ...
    def _must_push_super_interaction(self):
        ...
        for interaction in self.sim.running_interactions_gen(self.super_affordance):
            if interaction.is_finishing:
                continue
            if self.target is interaction.target or self.target in interaction.get_potential_mixer_targets():
    ...
    def notify_queue_head(self):
        if self.is_finishing:
            ...
            if self._must_push_super_interaction():
                ...
                context = InteractionContext((self.sim), (self.source), (self.priority), insert_strategy=(QueueInsertStrategy.FIRST),
    ...
    def prepare_gen(self, timeline):
        if not self.allow_with_unholsterable_carries:
            if not self.cancel_incompatible_carry_interactions(can_defer_putdown=False):
    ...
    def _get_required_sims(self, *args, **kwargs):
        ...
        if self.target_type & TargetType.GROUP:
            ...
            if self.target_type & TargetType.TARGET:
                ...
                if self.target_type & TargetType.ACTOR or self.target_type & TargetType.OBJECT:
                    sims.update(self.get_participants(ParticipantType.Actor))
        return sims
    ...
    def _generate_suspended_modifiers_dict(self):
        ...
        for sim in self.required_sims():
            for handle, autonomy_modifier_entry in sim.sim_info.get_statistic_modifiers_gen():
                ...
                if autonomy_modifier.exclusive_si:
                    if autonomy_modifier.exclusive_si is not self.super_interaction:
    ...
    def is_mixer_compatible(cls, inst, sim, target, error_on_fail=False, participant_type=DEFAULT):
        ...
        if not ret:
            if error_on_fail:
                if no_geometry_posture_state.valid:
            ...
            if not geometry_intersection.valid:
                ...
                if isinstance(constraint, RequiredSlotSingle):
                    ...
                    if sims4.math.vector3_almost_equal_2d((sim.transform.translation), containment_transform, epsilon=ANIMATION_SLOT_EPSILON):
                        continue
                logger.warn("Interaction Constraint Error: Interaction's constraint is incompatible with the Sim's current position \n                    Interaction: {}\n                    Sim: {}, \n                    Constraint: {}\n                    Sim Position: {}\n                    Interaction Target Position: {},\n                    Target Containment Transform: {}",
                  self,
                  sim, constraint, (sim.position), (self.target.position if self.target is not None else None), containment_transform, owner='MaxR', trigger_breakpoint=True)
                return False
