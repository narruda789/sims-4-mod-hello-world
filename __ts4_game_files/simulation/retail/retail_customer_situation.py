...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\retail\retail_customer_situation.py
# Compiled at: 2018-07-25 16:02:55
# Size of source mod 2**32: 41145 bytes
import random
...
import distributor.system, event_testing, interactions, objects.object_tests, services, sims4.log, situations.bouncer, zone_tests
...
class TunableCustomerSituationInitiationTestVariant(TunableVariant):
    ...
    def __init__(self, description='A single tunable test.', **kwargs):
        (super().__init__)(object_criteria=objects.object_tests.ObjectCriteriaTest.TunableFactory(locked_args={'tooltip': None}), 
...
class RetailCustomerSituation(BusinessSituationMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'customer_job':SituationJob.TunableReference(description='\n            The situation job for the customer.\n            '), 
     ...
     'role_state_browse':OptionalTunable(description='\n            If enabled, the customer will be able to browse items.\n            ',
       ...
       default=10),
       browse_time_max=TunableSimMinute(description='\n                    The maximum amount of time, in sim minutes, the customer\n                    will browse before moving on to the next state. When the\n                    customer begins browsing, a random time will be chosen\n                    between the min and max browse time.\n                    ',
       default=20),
       browse_time_extension_tunables=(OptionalTunable(TunableTuple(description='\n                    A set of tunables related to browse time extensions.\n                    ',
    ...
    def adjust_total_shop_time(self, multiplier):
        if multiplier == 0:
            ...
            if type(self._cur_state) is _GoToStoreState:
    ...
    def _get_min_max_price_range(self):
        ...
        return (
         max(0, price_range.lower_bound * self._min_price_range_multiplier),
         max(1, price_range.upper_bound * self._max_price_range_multiplier))

    def _initialize_purchase_intent(self):
        if self.role_state_buy is None:
            return
        elif self._saved_purchase_intent is None:
    ...
    def _on_social_group_members_changed(self, group):
        if self._customer is not None:
            ...
            if self._customer in group:
                for sim in group:
                    if business_manager.is_household_owner(sim.household_id) or business_manager.is_employee(sim.sim_info):
                        employee_still_in_group = True
                        break

            elif employee_still_in_group:
    ...
    def _set_purchase_intent_visibility(self, toggle):
        if self._showing_purchase_intent is not toggle:
            if not toggle or isinstance(self._cur_state, _BrowseState):
    ...
    def _purchase_intent_watcher(self, stat_type, old_value, new_value):
        if stat_type is not self.PURCHASE_INTENT_STATISTIC:
            ...
            if new_value == self.PURCHASE_INTENT_STATISTIC.max_value:
                ...
                if new_value == self.PURCHASE_INTENT_STATISTIC.min_value:
...
lock_instance_tunables(RetailCustomerSituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.NORMAL),
...
class _BrowseState(SituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm((_BrowseState.BROWSE_STATE_TIMEOUT), browse_time,
    ...
    def adjust_timeout(self, multiplier):
        if multiplier == 0:
            ...
            self._cancel_alarm(_BrowseState.BROWSE_STATE_TIMEOUT)
            self._create_or_load_alarm((_BrowseState.BROWSE_STATE_TIMEOUT), minutes_remaining,
              (lambda _: self._timer_expired()),
              should_persist=True)
...
class _BuyState(SituationState):
    ...
    def on_activate(self, reader=None):
        ...
        if reader is not None:
            ...
            if self.object_id is not None:
                ...
                object_to_buy is None or result or self.owner._self_destruct()
            else:
                self._create_or_load_alarm((_BuyState.BUY_STATE_VERIFICATION), (_BuyState.BUY_STATE_VERIFICATION_TIME),
    ...
    def _test_execute_buy_affordance(self, object_to_buy):
        ...
        if retail_component is None:
            return (
             object_to_buy, TestResult(False, '{} missing retail component'))
        buy_affordance = retail_component.get_buy_affordance()
        if buy_affordance is None:
            return (
             object_to_buy, TestResult(False, '{} missing buy affordance'))
        ...
        return (
         object_to_buy, EnqueueResult(test_result, execute_result))
    ...
    def _try_find_object_and_push_buy_affordance(self):
        ...
        for found_object in RetailUtils.all_retail_objects_gen(allow_sold=False):
            if found_object.in_use:
                if not found_object.in_use_by(self.owner._customer):
                    continue
    ...
    def _verify_buy_state_interactions(self):
        ...
        if self.object_id is not None:
            ...
            should_restart = object_to_buy is None or any((interaction.sim is self.owner._customer for interaction in list(object_to_buy.interaction_refs))) or True
        elif should_restart:
            new_buy_state = _BuyState()
