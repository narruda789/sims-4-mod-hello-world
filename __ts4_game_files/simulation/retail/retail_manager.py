...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\retail\retail_manager.py
# Compiled at: 2018-04-27 15:23:08
# Size of source mod 2**32: 17167 bytes
...
class RetailManager(BusinessManager):
    ...
    def _fixup_placard_if_necessary(self, obj):
        ...
        if obj_retail_component is not None:
            if obj_retail_component.is_sold:
    ...
    def _update_for_sale_vfx_for_object(self, obj, toggle_value):
        if obj not in self._objs_with_for_sale_vfx and toggle_value:
            ...
            if obj in self._objs_with_for_sale_vfx:
                if not toggle_value:
    ...
    def _open_pure_npc_store(self, is_premade):
        ...
        for retail_obj in RetailUtils.all_retail_objects_gen(allow_not_for_sale=True):
            ...
            if not obj_retail_component.is_for_sale:
                if retail_obj.is_in_inventory():
                    has_retail_obj = True
                    continue
                if is_premade:
    ...
    def _open_household_owned_npc_store(self):
        ...
        for retail_obj in RetailUtils.all_retail_objects_gen(allow_not_for_sale=True):
            self._adjust_commodities_if_necessary(retail_obj)
            if not should_open:
                should_open = retail_obj.retail_component.is_not_for_sale or True
