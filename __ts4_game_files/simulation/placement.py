...
# Size of source mod 2**32: 61811 bytes
import time
...
def get_nearby_sims_gen(position, surface_id, radius=None, exclude=None, stop_at_first_result=False, only_sim_position=False, only_sim_intended_position=False, check_all_surfaces_on_level=False):
    ...
    if check_all_surfaces_on_level:
            ...
            if search_result == FGLSearchResult.SUCCESS:
                ...
                if temp_list is not None:
                    if len(temp_list) > 0:
                        ...
                        return (
                         fgl_pos, fgl_loc.orientation)
            else:
                if search_result == FGLSearchResult.FAIL_NO_RESULTS:
...
class FindGoodLocationContext:
    ...
    def __init__(self, starting_routing_location, object_id, object_def_id, object_def_state_index, object_footprints, object_polygons, routing_context, ignored_object_ids, min_distance, max_distance, position_increment_info, additional_avoid_sim_radius, restrictions, scoring_functions, offset_distance, offset_restrictions, random_range_weighting, max_results, max_steps, height_tolerance, terrain_tags, raytest_start_offset, raytest_end_offset, raytest_radius, raytest_ignore_flags, raytest_start_point_override, search_flags, min_water_depth, max_water_depth, min_pond_water_depth, max_pond_water_depth, connectivity_group_override_point, min_head_room=NoneNoneNoneNoneNoneNoneNoneNoneNoneNone0NoneNoneNoneNoneNone01NoneNoneNoneNoneNoneNoneNoneFGLSearchFlagsDefaultNoneNoneNoneNoneNoneNone, **kwargs):
        ...
        elif offset_distance is not None and offset_distance > 0:
        ...
        if search_flags is not None:
            ...
            self.search_strategy.enclosed_room_only = search_flags & FGLSearchFlag.ENCLOSED_ROOM_ONLY
            self.search_strategy.lot_terrain_only = search_flags & FGLSearchFlag.LOT_TERRAIN_ONLY
        ...
        if min_head_room is not None:
            ...
            if max_pond_water_depth is not None:
                ...
                self.search_strategy.max_pond_water_depth = max_pond_water_depth
            if min_pond_water_depth is not None:
                self.search_strategy.min_pond_water_depth = min_pond_water_depth
            if gsi_handlers.routing_handlers.FGL_archiver.enabled:
...
def create_fgl_context_for_object(starting_location, obj_to_place, search_flags=FGLSearchFlagsDefault, test_buildbuy_allowed=True, **kwargs):
    ...
    return FindGoodLocationContext(starting_location, object_id=obj_to_place.id, 
...
def create_fgl_context_for_sim(starting_location, sim_to_place, search_flags=FGLSearchFlagsDefaultForSim, **kwargs):
    return FindGoodLocationContext(starting_location, object_id=sim_to_place.id, 
     search_flags=search_flags, **kwargs)
