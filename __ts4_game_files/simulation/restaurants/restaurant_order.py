...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\restaurants\restaurant_order.py
# Compiled at: 2019-12-03 21:31:32
# Size of source mod 2**32: 15806 bytes
import weakref
...
class GroupOrder:
    ...
    def add_sim_order(self, sim_id, food_recipe_id=None, drink_recipe_id=None, recommendation_state=OrderRecommendationState.NO_RECOMMENDATION, order_status=OrderStatus.MENU_READY):
        if food_recipe_id is None:
            if drink_recipe_id is None:
                return False
        ...
        if self.has_food_order():
            for eat_object in self.slotted_objects_gen({RestaurantTuning.TABLE_FOOD_SLOT_TYPE}):
                if not eat_object.is_prop:
                    if eat_object.state_value_active(RestaurantTuning.CONSUMABLE_EMPTY_STATE_VALUE) or self._order_status == OrderStatus.ORDER_READY:
        ...
        if self.has_drink_order():
            for drink_object in self.slotted_objects_gen({RestaurantTuning.TABLE_DRINK_SLOT_TYPE}):
                if not drink_object.is_prop:
                    if drink_object.state_value_active(RestaurantTuning.CONSUMABLE_EMPTY_STATE_VALUE) or self._order_status == OrderStatus.ORDER_READY:
