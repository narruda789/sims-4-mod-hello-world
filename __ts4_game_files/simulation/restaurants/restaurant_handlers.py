...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\restaurants\restaurant_handlers.py
...
def generate_restaurant_order_data(zone_id: int=None):
    ...
    for group_order in zone_director._group_orders.values():
        ...
        for sim_id, sim_order in group_order._sim_orders.items():
            ...
            sim_order_data.append({'sim_name':str(sim_info), 
             'food_recipe':food_str, 
             'drink_recipe':drink_str, 
             'recommendation_state':str(sim_order.recommendation_state)})
        ...
        all_orders.append({'order_id':str(group_order.order_id), 
         'situation_id':str(group_order._situation_id), 
         'order_status':str(group_order.order_status), 
         'current_bucket':str(group_order.current_bucket), 
         'table_ids':','.join([str(table_id) for table_id in group_order._table_ids]), 
         'sim_order_count':str(group_order.sim_order_count), 
         'assigned_waiter':str(assigned_waiter), 
         'assigned_chef':str(assigned_chef), 
         'food_platter':str(food_platter), 
         'complimentary':str(group_order.is_complimentary), 
         'SimOrders':sim_order_data})
...
def generate_restaurant_seat_data(zone_id: int=None):
    ...
    for table_id, dining_spots in zone_director._dining_spots.items():
        ...
        for part_index, dining_spot in dining_spots.items():
            ...
            dining_spot_data.append({'part_index':str(part_index), 
             'seat':str(seat_object) + chair_part_str, 
             'seated_sim':sim_name, 
             'food_and_drink':','.join(food_and_drink_objects)})

        all_tables.append({'table':str(table), 
         'dining_spots_num':str(len(dining_spots)), 
         'situation':situation_str, 
         'seated_sim_num':str(seated_sim_num), 
         'DiningSpots':dining_spot_data})
