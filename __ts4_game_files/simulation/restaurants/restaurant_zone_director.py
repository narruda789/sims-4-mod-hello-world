...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\restaurants\restaurant_zone_director.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 101171 bytes
...
import build_buy, services, sims4.log, zone_types
...
class RestaurantZoneDirector(BusinessZoneDirectorMixin, VisitorSituationOnArrivalZoneDirectorMixin, SchedulingZoneDirector):
    INSTANCE_TUNABLES = {'food_reaction_affordance':TunableReference(description='\n            The reaction interaction that gets pushed on Sims when their food is \n            created at the table. This interaction should have the constraints to\n            make sure the Sims is sitting at the table before reacting or they\n            might react from somewhere other than their table.\n            ',
     ...
     'all_in_one_table_parts':TunableList(description='\n            A list of parts that will be considered dining spots by this zone \n            director. This tuning should be used for objects that are their\n            own seats (like picnic tables) and therefore will be treated\n            incorrectly by the normal dining spot tagging system). \n            ',
       tunable=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.OBJECT_PART)),
       pack_safe=True)), 
     'not_a_table_tag':TunableTag(description='\n            Any object with this tag will not be considered a table for\n            restaurant purposes. The "is a table" criteria is determine by\n            having one of the parent tags of this venues required objects. This\n            tag can be given to any object that would be otherwise considered a\n            table by this criteria.\n            ',
       filter_prefixes=('Func', )), 
     'customer_situation_type_curve':SituationCurve.TunableFactory(description="\n            When customer situations are being generated, they'll be pulled\n            based on the tuning in this.\n            ",
       tuning_group=GroupNames.BUSINESS,
       get_create_params={'user_facing': False}), 
     'business_chef_employee_type_and_situation':TunableTuple(description='\n            The Business Employee Type of the Chef as well as the list of\n            situations that will be randomly chosen from when we spawn a hired\n            chef.\n            ',
     ...
     'business_host_employee_type_and_situation':TunableTuple(description='\n            The Business Employee Type of the Host as well as the list of\n            situations that will be randomly chosen from when we spawn a hired\n            host.\n            ',
      ...
      BusinessEmployeeType.INVALID,)),
       ...
       tuning_group=GroupNames.BUSINESS), 
     'business_waitstaff_employee_type_and_situation':TunableTuple(description='\n            The Business Employee Type of the waitstaff as well as the list of\n            situations that will be randomly chosen from when we spawn a hired\n            waitstaff.\n            ',
    ...
    def on_startup(self):
        ...
        if business_manager is not None:
            if business_manager.is_owner_household_active:
                self.destroy_shifts()
    ...
    def _update_restaurant_config(self):
        ...
        if config_data is None:
            ...
            elif self._business_manager is not None:
                self._preset_id = MenuPresets.CUSTOMIZE
            else:
                if restaurant_config.HasField('preset_id'):
        ...
        if len(restaurant_config.outfits) > 0:
            for outfit_data in restaurant_config.outfits:
                ...
                if mannequin_data.HasField('mannequin_id'):
                    if mannequin_data.HasField('gender'):
                        sim_info_wrapper = SimInfoBaseWrapper()
    ...
    def _update_dining_spots(self):
        ...
        for test in dining_spot_tests:
            ...
            for table in (object_manager.get_objects_with_tags_gen)(*test.parent_tags.tag_set):
                if table not in table_list and table.is_on_active_lot():
                    table.has_tag(self.not_a_table_tag) or table_list.append(table)
        ...
        for table in table_list:
            ...
            for part in table.parts:
                ...
                seat_found = False
                if part.part_definition in self.all_in_one_table_parts:
                    seat_found = True
                    ...
                    for child in part.children:
                        if child.has_any_tag(child_tags):
                            ...
                            if not dining_spot is not None or dining_spot.seat_id != child.id or dining_spot.chair_part_index != child_part_index:
                ...
                if (seat_found or dining_spot) is not None:
                    ...
                    if part.part_group_index in dining_spot_dict:
    ...
    def _try_choose_daily_special(self):
        if self._business_manager is not None:
            if self._business_manager.is_owner_household_active:
        ...
        for course, items in menu.items():
            ...
            if day_unchanged:
                if course in self._daily_special_recipe_ids.keys():
                    ...
                    if any((valid_item.guid64 == course_recipe_id for valid_item in valid_items)):
                        continue
            if valid_items and len(valid_items) >= self.MIN_REQUIRED_ITEMS_FOR_DAILY_SPECIAL:
    ...
    def is_all_in_one_table(self, obj_or_part):
        if obj_or_part.is_part:
            return obj_or_part.part_definition in self.all_in_one_table_parts
        else:
            return obj_or_part.parts or False
    ...
    def order_for_table(self, sim_orders, recommendation_state=OrderRecommendationState.NO_RECOMMENDATION, order_status=OrderStatus.MENU_READY, group_order=None, send_order=True, complimentary=False):
        ...
        for sim_id, recipe_id in sim_orders:
            ...
            food_recipe_id, drink_recipe_id = GroupOrder.get_food_drink_recipe_id_tuple(recipe)
            group_order.add_sim_order(sim_id, food_recipe_id=food_recipe_id, drink_recipe_id=drink_recipe_id,
    ...
    def on_order_update(self, group_order):
        if not group_order is None:
            if not group_order.need_to_update_bucket():
                return
            ...
            if not current_bucket_list:
                if group_order.current_bucket != OrderStatus.ORDER_INIT:
    ...
    def _create_single_recipe_and_slot(self, recipe, slot_type, table_part, chef_sim, push_reaction=True):
        ...
        if CraftingTuning.FOOD_POISONING_STATE is not None:
            if recipe.food_poisoning_chance:
    ...
    def cancel_group_order(self, group_order, refund_cost=False):
        if group_order.order_status == OrderStatus.ORDER_READY or group_order.order_status == OrderStatus.ORDER_READY_DELAYED:
            if group_order.serving_from_chef is not None:
                if group_order.serving_from_chef.parent is not None:
                    if not group_order.serving_from_chef.parent.is_sim:
        ...
        if not self.has_claimed_table(sim):
            if dining_groups:
                ...
                if table_to_claim is not None:
                    if table_to_claim.id not in self._dining_spots:
                ...
                if sim_num_left > 0:
                    _, released_extra_tables = self._find_tables_by_sim_num(sim_num_left, table_id_to_claim,
                      tables_found,
                      consider_occupied=True)
    ...
    def release_table(self, sim=None, release_table_id=None, from_situation_destroy=False):
        ...
        table_released = []
        if release_table_id is not None:
            if release_table_id in self._dining_spots:
                if not self.table_has_been_claimed(release_table_id):
    ...
    def _find_tables_by_sim_num(self, sim_num, table_id_to_claim=None, tables_found=(), consider_occupied=False, search_only=False):
        ...
        for dining_num in dining_number_list:
            if dining_num < sim_num:
                continue
            closest_table_id = self._find_closest_table(table_id_to_claim, (self._table_dict_by_dining_num[dining_num]),
              tables_found,
              consider_occupied=consider_occupied,
        ...
        for dining_num in reversed(dining_number_list):
            ...
            if closest_table_id is not None:
                ...
                sim_num_left, small_tables_found = self._find_tables_by_sim_num((sim_num - dining_num), table_id_to_claim,
                  temp_tables_found,
                  consider_occupied=consider_occupied)
    ...
    def _find_closest_table(self, original_table_id, table_ids, tables_found, consider_occupied=False, search_only=False):
        ...
        for table_id in table_ids:
            ...
            if table_occupied:
                if not consider_occupied:
                    ...
                    if dining_groups is not None:
                        if any((group.is_player_group() for group in dining_groups)):
                            continue
                    elif original_table is None:
                        if table_occupied:
        ...
        if found_occupied_table_id is not None:
            if not search_only:
    ...
    def seat_claimed_by_sim(self, sim, seat):
        if self.is_all_in_one_table(seat):
            table_part = seat
        else:
                ...
                if table_id == found_table_id:
                    for part_id, dining_spot in data.items():
                        if part_id == found_part_id:
                            if dining_spot.seat_id is None:
                                if self.is_all_in_one_table(services.object_manager().get(found_table_id)):
                                    return (
                                     dining_spot.table_id, dining_spot.table_part_index)
                            return (
                             dining_spot.seat_id, dining_spot.chair_part_index)
            ...
            for dining_spot in dining_spots.values():
                if not seat_id == dining_spot.seat_id:
                    if use_table_as_seat:
                        if seat_id == dining_spot.table_id:
                            if chair_part_index is not None:
                                if use_table_as_seat and not dining_spot.table_part_index == chair_part_index:
                                    continue
                                else:
                                    if not dining_spot.chair_part_index == chair_part_index:
                                        continue
                        return dining_spot
    ...
    def show_menu(self, sim, chef_order=False, is_recommendation=False):
        ...
        if int(now.absolute_days()) != self._last_daily_special_aboslute_day:
            self._try_choose_daily_special()
        show_menu_message = get_menu_message((self.get_current_menu().items()), group_sim_ids,
          chef_order=chef_order,
          is_recommendation=is_recommendation,
          daily_special_ids_map=(self._daily_special_recipe_ids))
    ...
    def zone_director_specific_destination_tests(self, sim, obj):
        if self.get_dining_spot_by_seat((obj.id), use_table_as_seat=(self.is_all_in_one_table(obj))):
            if not self.seat_claimed_by_sim(sim, obj):
                return False
        return True
    ...
    def additional_social_picker_tests(self, actor, target):
        ...
        if actor_seated:
            if actor.posture.posture_type is PostureConstants.SIT_POSTURE_TYPE:
                if target.posture.posture_type is not PostureConstants.SIT_POSTURE_TYPE:
