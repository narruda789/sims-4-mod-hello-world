...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\civic_policies\base_civic_policy_provider.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 51755 bytes
...
import alarms, autonomy, objects.components.types, services, random
...
class BaseCivicPolicyProvider(ComponentContainer, HasStatisticComponent, HasTunableFactory, AutoFactoryInit):
    ...
    def _reset_voting_statistics(self):
        for policy in self._civic_policies:
            if policy.vote_count_statistic is None:
                ...
                self.statistic_tracker.clear_statistic(policy.vote_count_statistic)
    ...
    def vote_by_instance(self, policy_instance, count=1, user_directed=False, lobby_interaction=False):
        if policy_instance.vote_count_statistic is not None:
            ...
            def get_current_rank():
                policy_list.sort(key=(lambda policy: (
                 self.get_stat_value(policy.vote_count_statistic), policy.guid64)),
                  reverse=True)
            ...
            if user_directed:
                ...
                if lobby_interaction:
                    ...
                    if user_directed and policy_list is not None:
    ...
    def add_to_ballot(self, policy_instance):
        if policy_instance.vote_count_statistic is not None:
            if policy_instance not in self._balloted_policies:
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.CivicPolicyDailyRandomVoting:
            ...
            if event == TestEvent.CivicPolicyOpenVoting:
                self.open_voting()
            else:
                if event == TestEvent.CivicPolicyCloseVoting:
                    self.close_voting()
    ...
    def create_add_policy_picker--- This code section failed: ---

 L. 891         0  LOAD_GLOBAL              SingleSimResolver
                2  LOAD_DEREF               'sim_info'
                4  CALL_FUNCTION_1       1  '1 positional argument'
                6  STORE_FAST               'resolver'

 L. 892         8  LOAD_FAST                'self'
               10  LOAD_ATTR                COMMUNITY_BOARD_TEXT
               12  LOAD_ATTR                add_policy_picker
               14  LOAD_DEREF               'sim_info'
               16  LOAD_FAST                'resolver'
               18  LOAD_CONST               ('resolver',)
               20  CALL_FUNCTION_KW_2     2  '2 total positional and keyword args'
               22  STORE_FAST               'dialog'

 L. 893        24  SETUP_LOOP          104  'to 104'
               26  LOAD_FAST                'self'
               28  LOAD_METHOD              get_dormant_policies
               30  CALL_METHOD_0         0  '0 positional arguments'
               32  GET_ITER         
             34_0  COME_FROM            46  '46'
               34  FOR_ITER            102  'to 102'
               36  STORE_FAST               'policy'

 L. 894        38  LOAD_FAST                'policy'
               40  LOAD_ATTR                guid64
               42  LOAD_FAST                'used_policy_ids'
               44  COMPARE_OP               not-in
               46  POP_JUMP_IF_FALSE    34  'to 34'

 L. 895        48  LOAD_FAST                'policy'
               50  LOAD_ATTR                display_description
               52  LOAD_CONST               ('tooltip',)
               54  BUILD_CONST_KEY_MAP_1     1 
               56  LOAD_CLOSURE             'sim_info'
               58  BUILD_TUPLE_1         1 
               60  LOAD_LAMBDA              '<code_object <lambda>>'
               62  LOAD_STR                 'BaseCivicPolicyProvider.create_add_policy_picker.<locals>.<lambda>'
               64  MAKE_FUNCTION_10         'keyword-only, closure'
               66  STORE_FAST               'tooltip'

 L. 896        68  LOAD_FAST                'dialog'
               70  LOAD_METHOD              add_row
               72  LOAD_GLOBAL              BasePickerRow
               74  LOAD_FAST                'policy'
               76  LOAD_METHOD              display_name
               78  LOAD_DEREF               'sim_info'
               80  CALL_METHOD_1         1  '1 positional argument'

 L. 897        82  LOAD_FAST                'policy'
               84  LOAD_ATTR                display_icon

 L. 898        86  LOAD_FAST                'policy'
               88  LOAD_ATTR                guid64

 L. 899        90  LOAD_FAST                'tooltip'
               92  LOAD_CONST               ('name', 'icon', 'tag', 'row_tooltip')
              ...
              102  POP_BLOCK        
            104_0  COME_FROM_LOOP       24  '24'

 L. 901       104  LOAD_GLOBAL              min
              106  LOAD_GLOBAL              len
              108  LOAD_FAST                'dialog'
              110  LOAD_ATTR                picker_rows
              112  CALL_FUNCTION_1       1  '1 positional argument'
              114  LOAD_FAST                'self'
              116  LOAD_ATTR                max_balloted_policy_count
              118  LOAD_GLOBAL              len
              120  LOAD_FAST                'self'
              122  LOAD_ATTR                _balloted_policies
              124  CALL_FUNCTION_1       1  '1 positional argument'
              126  BINARY_SUBTRACT  
              128  LOAD_GLOBAL              len
              130  LOAD_FAST                'used_policy_ids'
              132  CALL_FUNCTION_1       1  '1 positional argument'
              134  BINARY_SUBTRACT  
              136  CALL_FUNCTION_2       2  '2 positional arguments'
              138  LOAD_FAST                'dialog'
              140  LOAD_ATTR                max_selectable
              142  STORE_ATTR               number_selectable

 L. 902       144  LOAD_FAST                'dialog'
              146  LOAD_METHOD              set_target_sim
              148  LOAD_DEREF               'sim_info'
              150  CALL_METHOD_1         1  '1 positional argument'
              152  POP_TOP          

 L. 903       154  LOAD_FAST                'dialog'
              156  LOAD_METHOD              add_listener
              158  LOAD_FAST                'self'
              160  LOAD_ATTR                _on_add_picker_selected
              162  CALL_METHOD_1         1  '1 positional argument'
              164  POP_TOP          

 L. 904       166  LOAD_FAST                'dialog'
              168  LOAD_METHOD              show_dialog
...
Parse error at or near `STORE_FAST' instruction at offset 66
    ...
    def handle_vote_interaction(self, sim_info, target_id, push_continuation):
        ...
        if push_continuation:
            ...
            autonomy_request = autonomy.autonomy_request.AutonomyRequest(sim,
              FullAutonomy, static_commodity_list=(self.VOTING_CONTINUATION_AUTONOMY_COMMODITIES),
              object_list=(
             target_object,),
...
class BaseCivicPolicyProviderServiceMixin:
    ...
    def _do_daily_random_voting(self):
        if self.enable_automatic_voting:
            if self.voting_open:
    ...
    def _start_voting_timer(self, voting_time_of_week, callback):
        ...
        voting_alarm = alarms.add_alarm(self, voting_time, callback,
          cross_zone=True)
    ...
    def _set_next_daily_random_voting_alarm(self):
        if self._voting_daily_random_alarm is not None:
            ...
            self._voting_daily_random_alarm = self.voting_open or None
            return
        ...
        self._voting_daily_random_alarm = alarms.add_alarm(self, daily_random_voting_span, (lambda _: self._do_daily_random_voting()),
          cross_zone=True)
    ...
    def _set_next_warn_alarm(self):
        if self._voting_warn_alarm is not None:
        ...
        elif self._voting_close_time is None:
            return
            now = services.time_service().sim_now
            ...
            if TimeSpan.ZERO < time_until_warning:
                self._voting_warn_alarm = alarms.add_alarm(self, time_until_warning, (lambda _: self._warn_voting()),
                  cross_zone=True)
        else:
            self._voting_warn_alarm = None
