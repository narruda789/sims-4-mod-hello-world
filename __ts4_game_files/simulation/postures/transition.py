...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\postures\transition.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 31510 bytes
import weakref
...
import animation.arb, element_utils, elements, enum, mutex, routing, sims, sims4.log
...
class PostureStateTransition(elements.SubclassableGeneratorElement):
    ...
    def _run_gen(self, timeline):
        ...
        if dest_aspect is None:
            if source_state.body.mobile:
                if dest_state.body.mobile:
...
class PostureTransition(elements.SubclassableGeneratorElement):
    ...
    def get_entry_exit_mutex_key(self):
        if self._source.mutex_entry_exit_animations:
            if self._dest.mutex_entry_exit_animations:
    ...
    def _get_unholster_predicate(self, sim, interaction):
        ...
        def unholster_predicate(obj):
            if self._transition_spec is not None:
                ...
                if path is not None:
                    if obj.should_unholster(sim=sim, path=path):
                        return True
        ...
        def unholster_after_predicate(obj):
            if interaction is not None:
                if interaction.should_unholster_carried_object(obj):
                    return True
                if sim.is_required_to_holster_while_routing(obj):
                    return False
            return True
    ...
    def calculate_distance_param(cls, source, dest):
        if source is None or dest is None:
            ...
            if distance_from_pos < math.EPSILON:
                ...
                if distance_from_pos >= cls.DISTANCE_HIGH:
                    distance_param = 'high'
                else:
                    if distance_from_pos >= cls.DISTANCE_MID and distance_from_pos < cls.DISTANCE_HIGH:
                        distance_param = 'mid'
                    else:
                        distance_param = 'low'
    ...
    def _do_transition(self, timeline) -> bool:
        ...
        try:
                        ...
                        if self._transition_spec.is_failure_path:
                            ...
                            if failure_reason is not None or failure_target is not None:
                                if self._interaction is not None:
                                    yield from element_utils.run_child(timeline, handle_transition_failure(sim, (self._interaction.target), (self._interaction), failure_reason=failure_reason,
            ...
            if self._transition_spec is not None and dest.track == PostureTrack.BODY and not source.mobile:
                ...
                elif sim.routing_surface is not None and new_routing_surface is not None:
                    source_locked_params += routing.get_routing_surface_asm_params(sim.routing_surface, new_routing_surface)
                    dest_locked_params += routing.get_routing_surface_asm_params(sim.routing_surface, new_routing_surface)
                ...
                def do_transition_animation(timeline):
                    yield from element_utils.run_child(timeline, do_auto_exit)
                    if PostureTrack.is_carry(dest.track):
                        if dest.target is not None:
                            if dest.target.is_sim:
                ...
                if dest.track.is_carry(dest.track):
                    ...
                    sequence = interact_with_carried_object(sim, carry_target, posture_state=carry_posture_state, interaction=(dest.source_interaction),
                ...
                if sim.is_required_to_holster_for_transition(source, dest):
                    ...
                    sequence = holster_carried_object(sim, (dest.source_interaction), (self._get_unholster_predicate(sim, dest.source_interaction)),
        ...
        if not dest.unconstrained:
            if sim.transition_controller is not None:
                if not sims4.math.vector3_almost_equal((sim.position), starting_position, epsilon=(sims4.geometry.ANIMATION_SLOT_EPSILON)):
