...
# Size of source mod 2**32: 37298 bytes
...
class PostureState:
    ...
    def __init__(self, sim, current_posture_state, posture_spec, var_map, invalid_expected=False, body_state_spec_only=False, carry_posture_overrides=frozendict(), is_throwaway=False):
        ...
        if not current_posture_state is None:
            if spec_body.posture_type != current_posture_state.body.posture_type or spec_body.target != current_posture_state.body.target:
                ...
                self._aspect_body = postures.create_posture((spec_body.posture_type), (self.sim),
                  (self.body_target),
        ...
        if body_slot_constraint is not None:
            if self._aspect_body.is_vehicle:
                if not (current_posture_state is not None and current_posture_state.body.is_vehicle):
        ...
        if current_posture_state is not None:
            if spec_carry_target != curr_spec_carry_target:
                ...
                if spec_carry.hand in var_map:
                    ...
                    new_carry_aspect = postures.create_posture(carry_posture_type, (self.sim),
                      carry_target,
                    ...
                    if hand == Hand.LEFT:
                        ...
                        if hand == Hand.RIGHT:
                            ...
                            if current_posture_state is not None:
                                ...
                                self._aspect_carry_right = _get_default_carry_aspect(PostureTrack.LEFT)
                        else:
            ...
            if current_posture_state is not None:
                ...
                if spec_carry_target is not None:
                    carry_target = var_map[spec_carry_target]
                    spec_carry_posture_type = spec_carry.posture_type
