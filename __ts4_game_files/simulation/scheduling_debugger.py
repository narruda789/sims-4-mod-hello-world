...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\scheduling_debugger.py
# Compiled at: 2014-06-05 15:41:32
...
class _ElementDebugData:
    ...
    def should_break(self, event):
        ...
        if breakpoint is None:
            ...
            return breakpoint.enabled or False
...
class TimelineDebugger:
    ...
    def break_on_element_event(self, element, event):
        ...
        if debug_data is None:
            ...
            return debug_data.should_break(event) or None
    ...
    def _logging(self, e, what, result=None):
        if not self._enable_call_logging:
            ...
            if getattr(e, 'generator', None):
                if e.generator.gi_code is not None and result is self._timeline._child:
...
def childmost(element):
    while hasattr(element, '_child_handle') and element._child_handle is not None and element._child_handle.element is not None:
...
def parentmost(element):
    while element._parent_handle is not None and element._parent_handle.element is not None:
        element = element._parent_handle.element
