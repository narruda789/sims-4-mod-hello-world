...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\ensemble\ensemble_service.py
# Compiled at: 2020-07-29 18:27:51
# Size of source mod 2**32: 17808 bytes
...
import persistence_error_types, services, sims4.resources
...
class EnsembleService(Service):
    ...
    def create_ensemble(self, new_ensemble_type, potential_sims):
        ...
        if new_ensemble_type.max_limit is not None:
            if len(ensemble_sims) > new_ensemble_type.max_limit:
            ...
            if ensembles_to_merge:
                ...
                for ensemble in ensembles_to_merge:
                    if new_ensemble_type.max_limit is not None:
                        if len(final_ensemble) + len(ensemble) > new_ensemble_type.max_limit:
    ...
    def should_start_travel_ensemble(self, traveled_sim_infos):
        if len(traveled_sim_infos) <= 1:
            return False
            ...
            if travel_group is not None:
                if travel_group.zone_id == current_zone_id and services.venue_service().active_venue.is_rental:
                    return False
