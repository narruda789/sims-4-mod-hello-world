...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\server_commands\aspiration_commands.py
# Compiled at: 2020-04-25 03:45:52
            ...
            for aspiration in services.get_instance_manager(sims4.resources.Types.ASPIRATION).types.values():
                if aspiration.aspiration_type == AspriationType.FULL_ASPIRATION:
                    if aspiration.do_not_register_events_on_load:
                        if not aspiration_tracker.aspiration_in_sequence(aspiration):
                            continue
                for asp_objective in aspiration_tracker.get_objectives(aspiration):
                    if asp_objective == objective:
                        aspiration_tracker.handle_event(aspiration, None,
                          (DataResolver(sim.sim_info)),
                          debug_objectives_to_force_complete=[
                         objective])
