...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\server_commands\cheat_commands.py
# Compiled at: 2017-10-26 18:18:44
# Size of source mod 2**32: 5388 bytes
from sims4.commands import get_command_info_gen
import paths, services, sims4.commands, ui.ui_dialog
...
def testing_cheats(enable: bool=False, _connection=None):
    if paths.IS_DESKTOP:
        ...
        if not cheat_service.cheats_ever_enabled:
            if not enable:
                _testing_cheats_common(enable=enable, _connection=_connection)
        else:
