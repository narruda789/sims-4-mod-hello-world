...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\server_commands\relationship_commands.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 40702 bytes
...
import relationships.relationship_track, services, sims4.commands, sims4.log
...
def create_friends_for_sim(opt_sim: OptionalTargetParam=None, _connection=None):
    ...
    services.sim_filter_service().submit_matching_filter(number_of_sims_to_find=quantity, sim_filter=friend_filter,
    ...
    for sim_a in services.active_household():
        ...
        for sim_b in all_sims:
            if sim_a.id == sim_b.id:
                continue
            elif track.track_type == RelationshipTrackType.SENTIMENT:
...
def set_average_relationships(avg_relationships: float, _connection=None):
    ...
    if needed_relationships > 0:
        ...
        for sim_info_id_a, sim_info_id_b in sim_info_combinations:
            ...
            if sim_info_a.is_npc and sim_info_a.lod != SimInfoLODLevel.MINIMUM and sim_info_b.is_npc and sim_info_b.lod != SimInfoLODLevel.MINIMUM:
                sim_info_a.relationship_tracker.has_relationship(sim_info_b.sim_id) or sim_info_a.relationship_tracker.set_default_tracks(sim_info_b, update_romance=False)
...
def check_culling_alarms(_connection=None):
    ...
    for relationship in services.relationship_service():
        if relationship.find_sim_info_a().is_player_sim or relationship.find_sim_info_b().is_player_sim:
...
def _get_sim_ids_from_string_list(sim_id_list, _connection):
    if not sim_id_list:
        ...
        output_list or sims4.commands.output('No valid sim ids in _get_sim_ids_from_string_list() command.', _connection)
        return
    return output_list