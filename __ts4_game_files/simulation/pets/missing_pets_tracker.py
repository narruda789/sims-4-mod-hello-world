...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\pets\missing_pets_tracker.py
# Compiled at: 2019-05-07 02:39:17
# Size of source mod 2**32: 28472 bytes
...
import alarms, clock, routing, services, sims4
...
class MissingPetsTracker(HouseholdTracker):
    ...
    DEFAULT_AWAY_ACTIONS = TunableMapping(description='\n        Map of species to the default away action to run when a pet is missing.\n        ',
     ...
     Species.INVALID,)),
      ...
      manager=(services.get_instance_manager(sims4.resources.Types.AWAY_ACTION)),
      pack_safe=True))
    ...
    def restore_missing_state(self):
        if self._missing_pet_id == 0:
            ...
            if zone_restored_sis:
                if missing_pet_info.has_loaded_si_state:
                    return
    ...
    def _run_motive_tests(self, pet_info):
        ...
        if pet_info.species == Species.CAT:
    ...
    def run_away(self, pet_info):
        if self.missing_pet_id != 0 or self._running_away:
            ...
            return routing.test_connectivity_pt_pt(pet.routing_location, routing_location, pet.routing_context) or None
    ...
    def _push_run_away_affordance(self, pet_sim):
        if pet_sim is None or self.RUN_AWAY_INTERACTION is None:
            ...
            return pet_sim.queue.can_queue_visible_interaction() or EnqueueResult.NONE
        if pet_sim.queue.has_duplicate_super_affordance(self.RUN_AWAY_INTERACTION, pet_sim, pet_sim):
            return EnqueueResult.NONE
        context = InteractionContext(pet_sim, (InteractionContext.SOURCE_SCRIPT),
          (Priority.High),
    ...
    def _return_pet(self, pet_info):
        ...
        if pet is not None:
            if self.RETURN_INTERACTION is not None:
                context = InteractionContext(pet, (InteractionContext.SOURCE_SCRIPT),
                  (Priority.High),
    ...
    def fix_up_data(self):
        if self._missing_pet_id != 0:
            if self._missing_pet_id is not None:
                if self._household.get_sim_info_by_id(self._missing_pet_id) is None:
                    self._clear_missing_pet_data(self._missing_pet_id)

    def _add_test_alarm(self, interval):
        repeat_interval = clock.interval_in_sim_minutes(self.TEST_INTERVAL)
        self._test_alarm = alarms.add_alarm(self, interval,
          (lambda _: self._run_tests()),
          repeating=True,
          repeating_time_span=repeat_interval,
          cross_zone=True)

    def _add_return_alarm(self, interval):
        self._return_alarm = alarms.add_alarm(self, interval,
          (lambda _: self.cancel_run_away_interaction()),
          cross_zone=True)

    def _add_cooldown_alarm(self, interval):
        self._cooldown_alarm = alarms.add_alarm(self, interval,
          (lambda _: self._cooldown_complete()),
          cross_zone=True)
