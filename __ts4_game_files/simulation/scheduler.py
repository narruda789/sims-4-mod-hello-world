...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\scheduler.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 31600 bytes
import collections, functools, random
...
class WeeklySchedule(HasTunableFactory):
    ...
    def __init__(self, schedule_entries, start_callback=None, schedule_immediate=True, min_alarm_time_span=None, min_duration_remaining=None, early_warning_callback=None, early_warning_time_span=None, extra_data=None, init_only=False, required_start_time=None, schedule_shift_type=CareerShiftType.ALL_DAY, cross_zone=False):
        ...
        for entry in self._schedule_entry_tuning:
            if will_not_be_empty:
                if entry.schedule_shift_type != schedule_shift_type:
                    continue
            for start_time, end_time in entry.get_start_and_end_times():
                is_random = entry.random_start
                if required_start_time is not None:
                    if now.time_to_week_time(required_start_time) != now.time_to_week_time(start_time):
                        continue
    ...
    def get_random_alarm_handles(self):
        return self._random_alarm_handles

    def populate_scheduler_msg(self, schedule_msg, schedule_shift_type=None):
        for schedule_entry in self._schedule_entry_tuning:
            if schedule_shift_type is not None:
                if schedule_entry.schedule_shift_type != schedule_shift_type:
                    continue
    ...
    def supports_shift(self, shift):
        return any((entry.schedule_shift_type == shift for entry in self._schedule_entry_tuning))
                    ...
                    if self._early_warning_time_span is not None:
                        if self._early_warning_time_span > date_and_time.TimeSpan.ZERO:
                            ...
                            if warning_time_span > date_and_time.TimeSpan.ZERO:
                                ...
                                self._early_warning_alarm_handle = alarms.add_alarm(self, warning_time_span,
    ...
    def time_until_next_scheduled_event(self, current_date_and_time, schedule_immediate=False, min_duration_remaining=None):
        ...
        for alarm_data in self._schedule_entries:
            ...
            cur_time = current_date_and_time.time_till_timespan_of_week(start_time, optional_end_time=(end_time if schedule_immediate else None),
...
class SituationWeeklySchedule(WeeklySchedule):
    ...
    def schedule_entry_data(pack_safe=False, affected_object_cap=False):
        schedule_entry_tuning = {'tuning_name':'situation', 
         'tuning_type':TunableReference(description='\n                The situation to start according to the tuned schedule.\n                ',
...
class WeightedSituationsWeeklySchedule(WeeklySchedule):
    ...
    @staticmethod
    def get_weighted_situations(cls, predicate=lambda _: True, resolver=None):
        if resolver is None:
        ...
        def get_weight(item):
            if not predicate(item.situation):
                ...
                return item.tests.run_tests(resolver) or 0
    ...
    @staticmethod
    def get_situation_and_params(cls, predicate=lambda _: True, additional_situations=None, resolver=None):
        weighted_situations = WeightedSituationsWeeklySchedule.get_weighted_situations(cls, predicate=predicate, resolver=resolver)
...
class SituationWeeklyScheduleVariant(TunableVariant):
    ...
    def __init__(self, *args, pack_safe=False, affected_object_cap=False, **kwargs):
        ...
        (super().__init__)(args, situation=SituationWeeklySchedule.TunableFactory(schedule_entry_data=schedule_entry_data), 
         weighted_situations=WeightedSituationsWeeklySchedule.TunableFactory(schedule_entry_data=schedule_entry_data), 
         default='situation', **kwargs)
...
class ObjectLayerWeeklySchedule(WeeklySchedule):
    ...
    def schedule_entry_data(pack_safe=False):
        return {'schedule_entries': TunableList(description='\n                A list of schedule entries. Each entry is a mapping of days of the\n                week to a start_time and duration.\n                ',
                               tunable=ScheduleEntry.TunableFactory(schedule_entry_data={'tuning_name':'conditional_layer', 
