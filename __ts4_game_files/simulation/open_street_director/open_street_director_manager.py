...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\open_street_director\open_street_director_manager.py
# Compiled at: 2021-05-26 22:00:33
...
class OpenStreetDirectorManager:
    ...
    def on_loading_screen_animation_finished(self):
        open_street_director = self.open_street_director
        if open_street_director is not None:
            open_street_director.on_loading_screen_animation_finished()
    ...
    def deactivate(self, from_load=False):
        if from_load:
            ...
            self._active or logger.error('Trying to deactivate open street manager that is already inactive.')
            return
        self._active = False
    ...
    def _set_active_request(self, request, from_load=False):
        if not self._active:
            ...
            if request is None:
                ...
                return
            request.validate_request() or logger.error('Trying to set request {} as the active request when it is not valid.', request)
            return
