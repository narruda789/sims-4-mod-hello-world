...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\relationships\relationship_handlers.py
# Compiled at: 2020-10-08 18:03:04
...
def build_track_entry(track, callback_info, sim_name, target_name):
    ...
    track_entry = {'type':track_name,  'score':track.get_user_value(), 
     'decay':track.get_decay_rate(), 
     'decay_enabled':track.decay_enabled, 
     'source_sim':sim_name, 
     'target_sim':target_name, 
     'progress':track.get_normalized_value(), 
     'time_till_decay':track.get_decay_time(Threshold(0)), 
     'is_sentiment':track.track_type == RelationshipTrackType.SENTIMENT}
...
def generate_relationship_view_data(sim_id: int=None):
    ...
    for rel in relationship_service.get_all_sim_relationships(sim_id):
        ...
        entry = {'relationship_id':str(target_sim_id),  'depth':rel.get_relationship_depth(sim_id), 
         'prevailing_stc':str(rel.get_prevailing_short_term_context_track(sim_id)), 
        ...
        for rel_lock in rel.get_all_relationship_bit_locks(sim_id):
            entry['relationship_bit_locks'].append({'lock':str(rel_lock), 
             'lock_group':str(rel_lock.group_id), 
             'lock_end_time':str(rel_lock.end_time)})
    ...
    for rel in relationship_service.get_all_sim_object_relationships(sim_id):
        ...
        entry = {'relationship_id':str(rel.sim_id_b),  'depth':rel.get_relationship_depth(sim_id), 
         'sim_id':str(sim_info.sim_id)}
        target_name = None
