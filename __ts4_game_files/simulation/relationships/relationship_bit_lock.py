...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\relationships\relationship_bit_lock.py
# Compiled at: 2017-06-15 19:22:44
...
class RelationshipBitLock(metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.RELATIONSHIP_LOCK)):
    INSTANCE_TUNABLES = {'group_id':TunableEnumEntry(description='\n            The group that this lock applies to.  No two locks can belong to\n            the same group.\n            ',
       ...
       invalid_enums=(
      RelationshipBitType.Invalid, RelationshipBitType.NoGroup)), 
    ...
    def try_and_aquire_lock_permission(self):
        ...
        if now < self.end_time:
            return False
        if sims4.random.random_chance(self.relock_percentage * 100):
            self.lock()
            return False
        return True
