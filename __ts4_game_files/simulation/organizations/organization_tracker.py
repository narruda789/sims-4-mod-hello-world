...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\organizations\organization_tracker.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 21885 bytes
import random
...
class OrganizationTracker(SimInfoTracker):
    ...
    def update_organization_task(self, task_data_org_info, timed_out=False):
        ...
        if not self._organization_active_tasks.get(task_data_org_info.org_id):
            if timed_out:
                snippet_manager = services.snippet_manager()
    ...
    def join_organization(self, organization_id):
        if self._organization_status.get(organization_id) == ACTIVE:
            ...
            self._sim_info.is_npc or self._assign_organization_tasks(organization_instance)
    ...
    def on_lod_update(self, old_lod, new_lod):
        if not self._organization_status:
            ...
            if old_lod > new_lod:
                if new_lod < SimInfoLODLevel.FULL:
