...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\organizations\organization_service.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 42565 bytes
...
import alarms, persistence_error_types, services, sims4
...
class OrganizationService(Service):
        ...
        if save_slot_data_msg.gameplay_data.HasField('organization_service'):
            ...
            for schedule_cancelled_event_data in data.schedule_cancelled_event_data:
                if schedule_cancelled_event_data.HasField('schedule_venue_event_time') and schedule_cancelled_event_data.HasField('org_event_id'):
                    alarm_handle = alarms.add_alarm(self, (TimeSpan(schedule_cancelled_event_data.schedule_venue_event_time)),
                      (lambda *_: self._schedule_cancelled_organization_event(schedule_cancelled_event_data.org_event_id)),
                      cross_zone=True)
    ...
    def _schedule_venue_organization_event(self, org_drama_node, zone_id):
        ...
        uid = drama_scheduler.schedule_node(org_drama_node, resolver,
    ...
    def schedule_org_events(self):
        ...
        for org_drama_node in self.ORGANIZATION_EVENTS:
            if org_drama_node in scheduled_org_events or org_drama_node in active_org_events:
                continue
            org_start_time = self.verify_valid_time(org_drama_node)
            if org_start_time is None:
                continue
    ...
    def add_organization_member(self, sim_info, org_id):
        ...
        if organization_tracker is None:
            ...
            members_list = self._organization_members.get(org_id)
            if members_list is None:
                self._organization_members[org_id] = [
                 sim_info.id]
            else:
                if sim_info.id not in members_list:
    ...
    def generate_organization_members(self, org_id, amount=0, blacklist_sims=(), additional_filter_terms=None, minimum=None):
        ...
        if member_result_ids is None:
            ...
            for org_member_id in member_result_ids:
                if filter_service.does_sim_match_filter(org_member_id, sim_filter=(organization_snippet.organization_filter),
                  gsi_source_fn=(lambda : str(self) + ': ' + str(organization_snippet))):
            ...
            if invalid_org_member_ids:
                for invalid_org_member_id in invalid_org_member_ids:
                    ...
                    if invalid_org_member_info is not None:
                        if invalid_org_member_info.is_played_sim:
                            continue
            ...
            for org_member_id in valid_org_member_ids:
                if org_member_id not in blacklist_sims and filter_service.does_sim_match_filter(org_member_id, sim_filter=(organization_snippet.organization_filter),
        ...
        if members_needed > 0:
            new_members = filter_service.submit_matching_filter(number_of_sims_to_find=members_needed, sim_filter=(organization_snippet.organization_filter),
              callback=None,
    ...
    def _reschedule_org_event(self, drama_node, **setup_kwargs):
        ...
        uid = drama_scheduler.schedule_node((type(drama_node)), resolver,
    ...
    def _reschedule_festival_org_event(self, drama_node, from_shutdown=False):
        ...
        if from_shutdown:
            if services.time_service().sim_timeline.is_frozen:
                return
        setup_kwargs = {}
        if is_scoring_archive_enabled():
            gsi_data = GSIDramaScoringData()
