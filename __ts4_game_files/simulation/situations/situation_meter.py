...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\situation_meter.py
# Compiled at: 2020-11-09 18:32:13
...
class SituationMeterData(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'_meter_id':Tunable(description='\n            Meter ID used by UI to differentiate meters from each other\n            (when multiple meters exist in a situation).\n            \n            This will typically be assigned by a GPE in code as opposed to tuned.\n            ',
       tunable_type=bool,
       default=True), 
     '_threshold_data':TunableList(description='\n            List of thresholds for this meter.\n            ',
       ...
       tunable_type=int,
       default=0))), 
     '_display_text':OptionalTunable(description='\n            Optional meter display text.\n            ',
       tunable=TunableLocalizedString()), 
     '_tooltip':OptionalTunable(description='\n            Optional tooltip text.\n            ',
       tunable=TunableLocalizedString())}
...
class ValueBasedSituationMeterData(SituationMeterData):
    FACTORY_TUNABLES = {'_min_value':Tunable(description='\n            Minimum value of the meter.\n            ',
       tunable_type=int,
       default=0), 
     '_max_value':Tunable(description='\n            Maximum value of the meter.\n            ',
       tunable_type=int,
       default=100)}
...
class StatBasedSituationMeterData(SituationMeterData):
    ...
    def create_meter_with_statistic_tracker(self, situation, statistic_tracker):
        return StatBasedSituationMeter(situation, (self._meter_id), statistic_tracker.get_statistic((self.stat), add=True), auto_update=(self.auto_update_on_stat_change))
...
class SituationMeter:
    ...
    def send_update(self):
        if self._situation is None:
            return
        if self._situation.is_user_facing:
            if services.get_zone_situation_manager().is_distributed(self._situation):
...
class StatBasedSituationMeter(SituationMeter):
    ...
    def __init__(self, situation, meter_id, stat_inst, *args, auto_update=True, **kwargs):
        ...
        self._stat_tracker = self._stat_inst.tracker
        self._watcher_handle = self._stat_tracker.add_watcher(self._on_stat_change)

    def destroy(self):
        super().destroy()
        if self._stat_tracker is not None:
            self._stat_tracker.remove_watcher(self._watcher_handle)
