...
# Size of source mod 2**32: 60082 bytes
import random
...
from event_testing.resolver import GlobalResolver, DoubleSimResolver, SingleSimResolver
...
class SituationGoal(SituationGoalDisplayMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.SITUATION_GOAL)):
    ...
    def __init__(self, sim_info=None, situation=None, scenario=None, goal_id=0, count=0, locked=False, completed_time=None, secondary_sim_info=None, **kwargs):
    ...
    def reevaluate_goal_completion(self):
        for sim_info in self.all_sim_infos_interested_in_goal_gen():
            if self._run_goal_completion_tests(sim_info, None, SingleSimResolver(sim_info_to_test=sim_info, additional_metric_key_data=self)) and self._increment_completion_count():
                return
