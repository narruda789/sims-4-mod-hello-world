...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\tunable.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 40233 bytes
import itertools
...
from situations.situation_types import SituationCategoryUid
...
from tag import Tag
import services, sims4.log, sims4.resources, venues.venue_constants
logger = sims4.log.Logger('Situations')

class TunableSituationCreationUI(TunableFactory):

    @staticmethod
    def _factory(resolver, targeted_situation_participant, situations_available, situation_category, **kwargs):

        def craft_situation(resolver, targeted_situation_participant, situations_available, situation_category):
            ...
            services.get_zone_situation_manager().send_situation_start_ui(actor, target=target, situations_available=situations_available, situation_category=situation_category)
            return True

        return lambda : craft_situation(resolver, targeted_situation_participant, situations_available, situation_category)
    ...
    def __init__(self, **kwargs):
        super().__init__(description='\n            Triggers the Situation Creation UI.\n            ',
          ...
          situation_category=TunableEnumEntry(description='\n                The category for the available Situations.\n                ',
    ...
    def _factory(resolver, situation, user_facing=True, invite_participants=None, invite_actor=True, actor_init_job=None, invite_picked_sims=True, invite_target_sim=True, target_init_job=None, invite_household_sims_on_active_lot=False, situation_default_target=None, situation_created_callback=None, linked_sim_participant=None, situation_guest_info=None, **kwargs):
        ...
        def start_situation(resolver, situation, user_facing=True, invite_participants=None, invite_actor=True, actor_init_job=None, invite_picked_sims=True, invite_target_sim=True, target_init_job=None, invite_household_sims_on_active_lot=False, situation_default_target=None, situation_created_callback=None, linked_sim_participant=None, situation_guest_info=None, **kwargs):
            ...
            if guest_list is None:
                ...
                if situation.targeted_situation is not None:
                    ...
                    for sim_id, job_type_id in job_assignments:
                        job_type = services.get_instance_manager(sims4.resources.Types.SITUATION_JOB).get(job_type_id)
            ...
            situation_id = (situation_manager.create_situation)(situation, guest_list=guest_list, 
             ...
             linked_sim_id=linked_sim_id, **kwargs)
        ...
        return lambda : start_situation(resolver, situation, user_facing=user_facing, 
         invite_participants=invite_participants, 
         invite_actor=invite_actor, 
         actor_init_job=actor_init_job, 
         invite_picked_sims=invite_picked_sims, 
         invite_target_sim=invite_target_sim, 
         target_init_job=target_init_job, 
         invite_household_sims_on_active_lot=invite_household_sims_on_active_lot, 
         situation_default_target=situation_default_target, 
         situation_created_callback=situation_created_callback, 
         linked_sim_participant=linked_sim_participant, 
         situation_guest_info=situation_guest_info, **kwargs)
...
class CreateSituationElement(XevtTriggeredElement):
    FACTORY_TUNABLES = {'create_situation': TunableVariant(description='\n            Determine how to create a specific situation.\n            ',
                           situation_creation_ui=(TunableSituationCreationUI()),
                           situation_start=(TunableSituationStart()))}
...
class DestroySituationsByTagsMixin:
    ...
    def _destroy_situations_by_tags(self, resolver):
        ...
        if situations:
            participant = None
            if self.required_participant is not None:
                if self.required_participant != ParticipantType.Invalid:
                    participant = resolver.get_participant(self.required_participant)
                    return participant is None or participant.is_sim or False
            for situation in situations:
                if participant:
                    if not situation.is_sim_info_in_situation(participant.sim_info):
                        continue
...
class JoinSituationElement(XevtTriggeredElement):
    ...
    def _do_behavior(self, *args, **kwargs):
        ...
        if not (subject is None or subject.is_sim):
            ...
            return False
        situation.invite_sim_to_job(subject, job=(self.situation_job), reassign_within_situation=(self.reassign_within_situation))
        return True
...
class TunableMakeNPCLeaveMustRun(TunableFactory):
    ...
    def __init__(self, **kwargs):
        super().__init__(subject=TunableEnumEntry(description='\n                                     Who to ask to leave.\n                                     ',
          tunable_type=ParticipantType,
          default=(ParticipantType.Actor)),
          description="Make the subject leave the lot proto. E.g. for motive distress. Only applies to NPCs who don't live here.")
...
class TunableSituationCondition(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, time_based=TunableTimeRangeCondition(description='The minimum and maximum amount of time required to satisify this condition.'), 
         event_based=TunableEventBasedCondition(description='A condition that is satsified by some event'), 
         default='time_based', **kwargs)
...
class TunableSummonNpc(TunableFactory):
  ...
  default=(venues.venue_constants.NPCSummoningPurpose.DEFAULT)), **kwargs)
...
class TunableQualityMultiplier(TunableFactory):
    ...
    def __init__(self, **kwargs):
        super().__init__(stat_to_check=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.STATISTIC))), threshold=TunableThreshold(description='Stat should be greater than this value for object creation to score.'),
          multiplier=Tunable(float, 1, description='Multiplier to be applied to score if object is created with this quality'))
...
class TunableSituationPhase(TunableSingletonFactory):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(job_list=TunableMapping(description='A list of roles associated with the situation.', key_type=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.SITUATION_JOB)), description='Job reference'),
  value_type=TunableReference((services.get_instance_manager(sims4.resources.Types.ROLE_STATE)), description='Role the job will perform'),
  key_name='job',
  value_name='role'), 
         exit_conditions=TunableList(TunableTuple(conditions=TunableList(TunableSituationCondition(description='A condition for a situation or single phase.'),
  description='A list of conditions that all must be satisfied for the group to be considered satisfied.')),
  description='A list of condition groups of which if any are satisfied, the group is satisfied.'), 
