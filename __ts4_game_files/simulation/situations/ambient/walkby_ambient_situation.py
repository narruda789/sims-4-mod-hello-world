...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\walkby_ambient_situation.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 18477 bytes
import random
...
from tag import Tag
import alarms, clock, interactions, services, sims4.log, sims4.tuning.tunable, situations.bouncer, terrain
...
class WalkbyAmbientSituation(WalkbyLimitingTagsMixin, SituationComplexCommon):
    ...
    def get_sim_available_for_walkby_flavor(self):
        if not self._cur_state is None:
            if not self._cur_state._is_available_for_interruption():
                return
            if services.time_service().sim_now < self._flavor_cooldown_until:
                return
        else:
            if self._walker is not None:
                if self._walker.opacity < 1.0:
                    return
    ...
    def get_sim_available_for_social(self):
        if not self._cur_state is None:
            return self._cur_state._is_available_for_interruption() or None
        else:
...
sims4.tuning.instances.lock_instance_tunables(WalkbyAmbientSituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.WALKBY),
  creation_ui_option=(situations.situation_types.SituationCreationUIOption.NOT_AVAILABLE),
  duration=0,
  duration_randomizer=0,
...
class _LeaveState(SituationState):
    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        return (
         self.owner.walker_job.walkby_state, role_affordance_target)
...
class _SocialState(SituationState):
    ...
    def _on_set_sim_role_state(self, sim, *args, **kwargs):
        ...
        if self._interaction is not None:
            ...
            if target_sim is None:
                ...
                if self._interaction is not None:
                    self._interaction.register_on_finishing_callback(self._on_finishing_callback)
                    self._other_situation.start_social(self.owner, self._interaction)
            else:
                self.owner._on_social_finished()
    ...
    def on_deactivate(self):
        if self._interaction is not None:
            ...
            if self._sim_id is not None:
                ...
                if sim is not None:
                    if self._on_social_group_changed in sim.on_social_group_changed:
    ...
    def _on_finishing_callback(self, interaction):
        ...
        if self._sim_id is not None:
            sim = services.object_manager().get(self._sim_id)
            if sim is not None:
                if tuple(sim.get_groups_for_sim_gen()):
                    self._interaction = None
    ...
    def _on_social_group_changed(self, sim, group):
        if self._sim_id is not None:
            sim = services.object_manager().get(self._sim_id)
            if sim is not None:
                if not tuple(sim.get_groups_for_sim_gen()):
                    self.owner._on_social_finished()
