...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\ambient_service.py
# Compiled at: 2021-05-28 17:23:36
# Size of source mod 2**32: 28558 bytes
...
import alarms, clock, enum, gsi_handlers, persistence_error_types, services, sims.ghost, sims4.log, sims4.service_manager, sims4.tuning.tunable, situations.situation_guest_list, terrain, world.lot_tuning
...
class _AmbientSourceStreet(_AmbientSource):
    ...
    def walkby_schedule(self):
        return self._walkby_schedule
...
class AmbientService(sims4.service_manager.Service):
    ...
    def begin_walkbys(self):
        ...
        self._update_alarm_handle = alarms.add_alarm(self, (clock.interval_in_sim_minutes(5)),
          (self._update_alarm_callback),
          repeating=True,
          use_sleep_time=False)
        self._flavor_alarm_handle = alarms.add_alarm(self, (clock.interval_in_sim_minutes(1)),
          (self._flavor_alarm_callback),
          repeating=True,
          use_sleep_time=False)
        walkby_director = self.get_walkby_director()
        if walkby_director:
            walkby_director.create_situations_during_zone_spin_up()
    ...
    def _flavor_alarm_callback(self, _):
        ...
        if available_social_pairs:
            if sims4.random.random_chance(len(available_social_pairs) * self.SOCIAL_CHANCE_TO_START * 100):
    ...
    def get_walkby_director(self):
        for source in self._sources:
            if source.source_type == AmbientSourceType.SOURCE_STREET:
                return source.walkby_schedule()
