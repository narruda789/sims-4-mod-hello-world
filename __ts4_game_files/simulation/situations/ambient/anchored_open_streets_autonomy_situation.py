...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\anchored_open_streets_autonomy_situation.py
# Compiled at: 2022-03-29 18:15:50
# Size of source mod 2**32: 17062 bytes
...
import filters, services, sims4.log, sims4.tuning.tunable, situations.bouncer, tag
...
class GroupAnchoredAutonomySituationCommon(SituationComplexCommon):
    ...
    INSTANCE_TUNABLES = {'object_anchor_tag':sims4.tuning.tunable.TunableEnumEntry(description="\n                                    The tag that defines the objects that are valid for the anchor point.\n                                    The situation will search the object manager for any object with this \n                                    tag and will choose one at random.  This object's position will be the \n                                    anchor point.  It is assumed that the roles in this situation will \n                                    have an autonomy modifier with off_lot_autonomy_rule set to ANCHORED.\n                                    ",
     ...
     'group_filter':sims4.tuning.tunable.TunableReference(description='\n                                The group filter for this visit.  \n                            ',
    ...
    def get_new_anchor_position(self, tags, test_func=None):
        tagged_objects = [obj for obj in services.object_manager().get_objects_with_tag_gen(tags) if not test_func is None if test_func(obj)]
...
class AnchoredOpenStreetsAutonomySituation(WalkbyLimitingTagsMixin, GroupAnchoredAutonomySituationCommon):
    INSTANCE_TUNABLES = {'role':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                          The situation job for all sims in this situation.\n                          '),
     ...
     'group_filter':sims4.tuning.tunable.TunableReference(description="\n                                The group filter for this walkby.  If set, this filter will be used \n                                instead of the filter tuned in the walker_job.  If it's None, the \n                                filter in the walker_job will be used.  Note that all sims spawned \n                                with this filter will be put into the walker_job job.\n                            ",
       ...
       class_restrictions=filters.tunable.TunableAggregateFilter,
       tuning_group=GroupNames.ROLES), 
     'wait_for_arrival_timeout':sims4.tuning.tunable.TunableSimMinute(description='\n                                        The amount of time the sim waits at the spawn point before doing\n                                        stuff.\n                                        ',
       default=30,
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _ArrivingState),
         SituationStateData(2, _DoStuffState),
         SituationStateData(3, _LeaveState))
    ...
    def _on_set_sim_job(self, sim, job_type):
        super()._on_set_sim_job(sim, job_type)
        if self._cur_state.transition_to_doing_stuff_when_full():
            if self.group_filter is None or len(self._guests) >= self.group_filter.get_filter_count():
...
sims4.tuning.instances.lock_instance_tunables(AnchoredOpenStreetsAutonomySituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.WALKBY),
...
class _ArrivingState(_AnchoredOpenStreetsAutonomySituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm((self._WAIT_FOR_ARRIVAL_TIMEOUT), (self.owner.wait_for_arrival_timeout), (self._on_waited_too_long),
...
class _DoStuffState(_AnchoredOpenStreetsAutonomySituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm((self._DO_STUFF_TIMEOUT), (self.owner.do_stuff_timeout), (self._on_done_doing_stuff),
          should_persist=True, reader=reader)
