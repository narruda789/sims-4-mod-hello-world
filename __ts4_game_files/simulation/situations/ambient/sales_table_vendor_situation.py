...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\sales_table_vendor_situation.py
# Compiled at: 2020-03-12 16:11:10
...
class SalesTableVendorSituationMixin:
    INSTANCE_TUNABLES = {'setup_state':SalesTableSetupState.TunableFactory(tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP, display_name='01_setup_state'), 
     'tend_state':TendSalesTableState.TunableFactory(tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP, display_name='02_tend_state'), 
     'teardown_state':SalesTableTeardownState.TunableFactory(tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP, display_name='03_teardown_state'), 
     'vendor_job_and_role_state':TunableSituationJobAndRoleState(description='\n            Job and Role State for the vendor.\n            '), 
     'sale_object_tags':TunableList(description='\n            A list of tags that tell us the object comes from the vendor. We\n            use these tags to find objects and destroy them when the situation\n            ends or the sim is removed.\n            ',
       ...
       tunable_type=Tag,
       default=(Tag.INVALID))), 
     'number_of_sale_objects':TunableInterval(description='\n            ',
       tunable_type=int,
       default_lower=7,
       default_upper=10,
       minimum=1,
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, SalesTableSetupState, factory=(cls.setup_state)),
         SituationStateData(2, TendSalesTableState, factory=(cls.tend_state)),
         SituationStateData(3, SalesTableTeardownState, factory=(cls.teardown_state)))
...
lock_instance_tunables(SalesTableVendorSituation, exclusivity=(BouncerExclusivityCategory.WALKBY),
