...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\ambient\walkby_tandem_sledding_situation.py
# Compiled at: 2021-05-14 16:15:05
# Size of source mod 2**32: 11542 bytes
import itertools
...
import filters.tunable, services, sims4.log, sims4.resources
...
class WalkbyTandemSleddingSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'group_filter':TunableAggregateFilter.TunableReference(description='\n            The aggregate filter that we use to find the sims for this\n            situation.\n            '), 
     'go_to_sport_slope_state':GoToSportSlopeSituationState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':True},
       tuning_group=GroupNames.STATE), 
     'sled_state':TandemSledSituationState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':True},
       tuning_group=GroupNames.STATE), 
     'sled_leader':TunableTuple(situation_job=SituationJob.TunableReference(description='\n                The Situation Job of the sled leader.\n                '),
    ...
    def _states(cls):
        return (SituationStateData(1, GetSimsState),
         SituationStateData.from_auto_factory(2, cls.go_to_sport_slope_state),
         SituationStateData.from_auto_factory(3, cls.sled_state))
    ...
    def get_predefined_guest_list(cls):
        ...
        filter_results = services.sim_filter_service().submit_matching_filter(sim_filter=(cls.group_filter), allow_yielding=False,
...
sims4.tuning.instances.lock_instance_tunables(WalkbyTandemSleddingSituation, exclusivity=(BouncerExclusivityCategory.WALKBY),
