...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\situation_excursion.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 23785 bytes
import random, services, sims4, situations, telemetry_helper
...
class ExcursionSituation(BaseCustomStatesSituation):
    INSTANCE_TUNABLES = {'_activities':TunableMapping(description='\n            A tunable mapping between activity keys and activities.\n            ',
       ...
       default=None),
       value_name='Activity',
       value_type=TunableExcursionActivitySnippet(description='\n                The activity that is tied to this key.\n                '),
       tuning_group=GroupNames.CORE), 
     '_starting_activity':TunableVariant(description='\n            The starting activity of this situation.\n            ',
       random_starting_activity=RandomWeightedSituationStateKey.TunableFactory(),
       time_based=TimeBasedSituationStateKey.TunableFactory(),
       default='random_starting_activity',
       tuning_group=GroupNames.CORE)}
    ...
    def is_venue_location_valid(cls, zone_id):
        return zone_id in cls.get_possible_zone_ids_for_situation()
    ...
    def _gsi_additional_data_gen(self):
        yield (
         'Current Activity Key', self._current_activity_key)
        if self.current_activity:
            yield (
             'Current Activity Score', str(self.current_activity.score))
        if self._applied_activity_setups:
            for cluster_key, setup_key in self._applied_activity_setups:
                yield (
                 'Activity Setup', cluster_key + ' -> ' + setup_key)
    ...
    def _try_apply_activity_setups(self):
        if self._applied_activity_setups is None:
            if self._in_activity_zone():
                resolver = SingleSimResolver(self.initiating_sim_info)
