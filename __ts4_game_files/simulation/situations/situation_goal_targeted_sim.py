...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\situation_goal_targeted_sim.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 25930 bytes
import random
...
import enum, event_testing.test_variants, event_testing.tests_with_data, services, sims.sim_info_tests, sims4.tuning.tunable, statistics.skill_tests
...
class TunableTargetedSimTestVariant(sims4.tuning.tunable.TunableVariant):
    ...
    def __init__(self, description='A single tunable test.', **kwargs):
        (super().__init__)(statistic=event_testing.statistic_tests.StatThresholdTest.TunableFactory(locked_args={'who':ParticipantType.TargetSim,  'tooltip':None}), relative_statistic=event_testing.statistic_tests.RelativeStatTest.TunableFactory(locked_args={'source':ParticipantType.Actor,  'target':ParticipantType.TargetSim}), 
         skill_tag=statistics.skill_tests.SkillTagThresholdTest.TunableFactory(locked_args={'who':ParticipantType.TargetSim,  'tooltip':None}), 
         mood=sims.sim_info_tests.MoodTest.TunableFactory(locked_args={'who':ParticipantTypeSim.TargetSim,  'tooltip':None}), 
         sim_info=sims.sim_info_tests.SimInfoTest.TunableFactory(locked_args={'who':ParticipantType.TargetSim,  'tooltip':None}), 
         trait=sims.sim_info_tests.TraitTest.TunableFactory(locked_args={'subject':ParticipantType.TargetSim,  'tooltip':None}), 
         ...
         buff=sims.sim_info_tests.BuffTest.TunableFactory(locked_args={'subject':ParticipantType.TargetSim,  'tooltip':None}), 
         relationship=TunableRelationshipTest(locked_args={'subject':ParticipantType.Actor,  'target_sim':ParticipantType.TargetSim,  'tooltip':None}), 
         motive=event_testing.statistic_tests.MotiveThresholdTest.TunableFactory(locked_args={'who':ParticipantType.TargetSim,  'tooltip':None}), 
         filter=sims.sim_info_tests.FilterTest.TunableFactory(locked_args={'filter_target':ParticipantType.TargetSim,  'relative_sim':ParticipantType.Actor,  'tooltip':None}), 
         situation_job=event_testing.test_variants.TunableSituationJobTest(locked_args={'participant':ParticipantType.TargetSim,  'tooltip':None}), 
         gender_preference=sims.sim_info_tests.GenderPreferenceTest.TunableFactory(locked_args={'subject':ParticipantType.Actor,  'target_sim':ParticipantType.TargetSim,  'tooltip':None}), 
         club=ClubTest.TunableFactory(locked_args={'club':ClubTest.CLUB_USE_ANY,  'tooltip':None}), 
         career=event_testing.test_variants.TunableCareerTest.TunableFactory(locked_args={'subjects':ParticipantType.TargetSim,  'tooltip':None}), 
...
class SituationGoalTargetedSim(SituationGoal):
    ...
    INSTANCE_TUNABLES = {'_target_tests':TunableTargetedSimTestSet(description='\n                A set of tests that a sim must to be a target of this goal.\n                ',
     ...
     '_target_option':sims4.tuning.tunable.TunableEnumEntry(SituationGoalSimTargetingOptions,
       SituationGoalSimTargetingOptions.PlayerChoice,
       description='How to apply the target tests. See design document for Event Goals'), 
    ...
    def _can_sim_pass_test(cls, target_sim_info, actor_sim_info, inherited_target_sim_info):
        if cls._target_option == SituationGoalSimTargetingOptions.GoalSystemChoiceExcludingInherited:
            if target_sim_info is inherited_target_sim_info:
                return False
    ...
    @classmethod
    def can_be_given_as_goal(cls, actor, situation, scenario=None, inherited_target_sim_info=None, **kwargs):
        ...
        if cls._target_option == SituationGoalSimTargetingOptions.DebugChoice:
            return TestResult.TRUE
        for sim_info in SituationGoal.all_sim_infos_interested_in_goal_gen(sim_info=None, situation=None, scenario=None, all_instanced_sim_infos_including_babies_are_interested=(cls._select_all_instantiated_sims)):
    ...
    def _run_goal_completion_tests(self, sim_info, event, resolver):
        if self._target_option == SituationGoalSimTargetingOptions.PlayerChoice:
            if self._situation is not None:
                target = self._select_all_instantiated_sims or self._get_target_sim_info_from_resolver(resolver)
                if target is None:
                    return False
        else:
        ...
        if not self._target_tests.run_tests(resolver):
            ...
            if self._get_target_sim_info_from_resolver(resolver) is not self._required_target_sim_info:
                return False
...
class SituationGoalEventRanSuccessfullyTargetedSim(SituationGoalTargetedSim):
    INSTANCE_TUNABLES = {'test_events_to_score': TunableMapping(description='\n            A mapping of test event -> score achieved when successfully ran.\n            ',
                               ...
                               tunable_type=int,
                               default=100),
                               status=TunableEnumEntry(description='\n                    The status of the goal result. This will only change the \n                    visual when removing this goal from the list of goals.\n                    ',
                               tunable_type=UiSituationGoalStatus,
                               default=(UiSituationGoalStatus.COMPLETED)),
    ...
    def _run_goal_completion_tests(self, sim_info, event, resolver):
        ...
        if event_score.interaction is not None:
            if event_score.interaction is not resolver.interaction.affordance:
                return False
