...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\visiting\greeted_player_visiting_npc_situation.py
# Compiled at: 2018-09-07 18:25:33
# Size of source mod 2**32: 15845 bytes
...
import interactions, role.role_state, services, sims4.tuning.tunable, situations.bouncer.bouncer_types
...
class _GreetedPlayerVisitingNPCState(CommonSituationState):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionComplete:
            if resolver(self.scolding_interactions):
    ...
    def _handle_scolding_interaction(self, sim_info, event, resolver):
        ...
        if resolver.interaction.sim.sim_info is not sim_info:
            ...
            return self.owner.is_sim_in_situation(target) or None
        self._scold_count += 1
...
class GreetedPlayerVisitingNPCSituation(VisitingNPCSituation):
    INSTANCE_TUNABLES = {'greeted_player_sims':sims4.tuning.tunable.TunableTuple(situation_job=situations.situation_job.SituationJob.TunableReference(description='\n                    The job given to player sims in the visiting situation.\n                    '),
     ...
     '_line_of_sight_generation_distance':Tunable(description='\n                The distance in front of the sim that rang the doorbell that we\n                generate the line of sight constraint.\n                ',
       tunable_type=float,
       default=2.0), 
     'greeted_player_visiting_npc_state':_GreetedPlayerVisitingNPCState.TunableFactory(description='\n            The state in which a greeted player is visiting an NPC. \n            ',
       display_name='1. Greeted Player Visiting NPC State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       locked_args={'time_out': None}), 
     'leave_npc_house_state':_LeaveNPCHouseState.TunableFactory(description='\n            The state in which an ungreeted household leaves the npc\n            house.   \n            ',
       display_name='2. Leave NPC House State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'ungreeted_player_visiting_npc_situation':TunableReference(description='\n            The situation that will be created after a previously greeted\n            player sim is kicked out by a npc. \n            ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION),
       class_restrictions=(
      UngreetedPlayerVisitingNPCSituation,))}

    @classmethod
    def _states(cls):
        return (SituationStateData(1, _GreetedPlayerVisitingNPCState, factory=(cls.greeted_player_visiting_npc_state)),
         SituationStateData(2, _LeaveNPCHouseState, factory=(cls.leave_npc_house_state)))
    ...
    def _issue_requests(self):
        request = SelectableSimRequestFactory(self, callback_data=_RequestUserData(role_state_type=(self.greeted_player_sims.role_state)),
          job_type=(self.greeted_player_sims.situation_job),
    ...
    def _on_add_sim_to_situation(self, sim, job_type, role_state_type_override=None):
        ...
        if self._line_of_sight is not None:
            if sim.species == Species.HUMAN:
                ...
                sim.push_super_affordance(ForceSatisfyConstraintSuperInteraction, None, context, constraint_to_satisfy=constraint_to_satisfy,
                  name_override='MoveInsideHouseFromGreetedSituation')
...
lock_instance_tunables(GreetedPlayerVisitingNPCSituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.VISIT),
