...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\single_sim_leave_situation.py
...
from situations.situation_complex import _StateAlarm, SituationStateData
import alarms, clock, objects, role.role_state, services, sims4.tuning.tunable, situations.situation_complex, situations.situation_job
...
class SingleSimLeaveSituation(situations.situation_complex.SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, LeaveState),
         SituationStateData(2, ImprisonedState),
         SituationStateData(3, DelayState))
...
class LeaveState(situations.situation_complex.SituationState):
    ...
    def on_activate(self, reader):
        ...
        for custom_key in self.owner.affordances_to_monitor.custom_keys_gen():
            self._test_event_register(TestEvent.InteractionExitedPipeline, custom_key)

        self._handle = alarms.add_alarm(self, (clock.interval_in_sim_minutes(self.owner.FAILSAFE_TIMEOUT)),
          (lambda _: self.timer_expired()),
          repeating=True,
          repeating_time_span=(clock.interval_in_sim_minutes(self.owner.FAILSAFE_TIMEOUT)))
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionExitedPipeline:
            if self.owner._leaver is not None:
                if sim_info is self.owner._leaver.sim_info:
                    if resolver(self.owner.affordances_to_monitor):
                        interaction = resolver.interaction
                        leaver = self.owner._leaver
                        if interaction is not None:
                            if not isinstance(interaction, objects.base_interactions.AggregateSuperInteraction):
                                if interaction.transition_failed:
                                    if leaver.is_on_active_lot():
                                        self.owner._num_on_lot_failures += 1
                                        ...
                                        self.owner._num_open_streets_failures += 1
