...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\star_placement_ceremony.py
# Compiled at: 2018-07-25 20:13:07
...
class StarPlacementCeremonySituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'honoree_job_and_role_state':TunableSituationJobAndRoleState(description='\n            The job and role state for the honoree.\n            '), 
     ...
     'impatient_gather_state':_ImpatientGatherState.TunableFactory(description="\n            The crowd grows restless after they've gathered and the star has not been\n            placed.\n            ",
       display_name='2. Impatient Gather State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'start_ceremony_state':_StartCeremonyState.TunableFactory(description='\n            The crowd reacts to the honoree having placed the star, and the honoree\n            responds with an excited reaction, like a fist pump.\n            ',
       display_name='3. Start Ceremony State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP)}
    REMOVE_INSTANCE_TUNABLES = Situation.NON_USER_FACING_REMOVE_INSTANCE_TUNABLES

    @classmethod
    def _states(cls):
        return (SituationStateData(1, _GatherState, factory=(cls.gather_state)),
         SituationStateData(2, _ImpatientGatherState, factory=(cls.impatient_gather_state)),
         SituationStateData(3, _StartCeremonyState, factory=(cls.start_ceremony_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.honoree_job_and_role_state.job, cls.honoree_job_and_role_state.role_state),
         (
          cls.crowd_member_job_and_role_state.job, cls.crowd_member_job_and_role_state.role_state)]
...
lock_instance_tunables(StarPlacementCeremonySituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
