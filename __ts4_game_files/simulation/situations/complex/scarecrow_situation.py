...
# Compiled at: 2018-05-14 18:33:09
# Size of source mod 2**32: 10310 bytes
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.InteractionComplete:
            if self.owner.is_sim_info_in_situation(sim_info):
                if resolver(self._leave_interaction):
...
class ScarecrowSituation(ObjectBoundSituationMixin, SituationComplexCommon):
    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        return (
         role_state_type, services.object_manager().get(self.bound_object_id))
    ...
    def _gsi_additional_data_gen(self):
        if isinstance(self._cur_state, _DoStuffState):
            yield (
             'Time till Leave State', str(self._cur_state.get_time_remaining()))


sims4.tuning.instances.lock_instance_tunables(ScarecrowSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
