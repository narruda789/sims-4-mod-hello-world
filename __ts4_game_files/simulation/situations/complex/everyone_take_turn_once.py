...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\everyone_take_turn_once.py
# Compiled at: 2018-12-06 14:34:58
# Size of source mod 2**32: 15756 bytes
...
class GatherTogetherState(CommonInteractionCompletedSituationState):
    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        return (
         role_state_type, self.owner.target_object)
...
class EveryoneTakeATurnOnceSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'gather_together_state':GatherTogetherState.TunableFactory(description='\n            The state of the situation when the situation first starts,\n            which lasts until every sim is in place and ready to take a turn.\n            ',
     ...
     'taking_turns_state':TakingTurnsState.TunableFactory(description='\n            The state that means all Sims have gathered and are taking turns.\n            ',
       tuning_group=GroupNames.STATE), 
     'job_and_role_state':TunableSituationJobAndRoleState(description='\n            Job and Role State.\n            '), 
    ...
    def _states(cls):
        return [
         SituationStateData(1, GatherTogetherState, factory=(cls.gather_together_state)),
         SituationStateData(2, TakingTurnsState, factory=(cls.taking_turns_state))]
    ...
    def _on_set_sim_job(self, sim, job_type):
        ...
        sub_situation_id = situation_manager.create_situation((self.sub_situation), guest_list=guest_list,
...
lock_instance_tunables(EveryoneTakeATurnOnceSituation, creation_ui_option=(SituationCreationUIOption.NOT_AVAILABLE),
...
class WaitState(CommonSituationState):
    ...
    def _get_role_state_overrides(self, sim, job_type, role_state_type, role_affordance_target):
        return (
         role_state_type, self.owner.background_situation.target_object)
...
class EveryoneTakeATurnOnceSubSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'sim_wait_state':WaitState.TunableFactory(description='\n            The state of Sims who are not currently taking a turn.\n            ',
       tuning_group=GroupNames.STATE), 
     'take_turn_state':TakeTurnState.TunableFactory(description='\n            The state of Sims who are taking a turn.\n            ',
       tuning_group=GroupNames.STATE), 
     'take_last_turn_state':TakeTurnState.TunableFactory(description='\n            The state of the Sim who are taking a last turn.\n            ',
       tuning_group=GroupNames.STATE,
       locked_args={'last_turn': True}), 
     'job_and_role_state':TunableSituationJobAndRoleState(description='\n            Job and Role State.\n            ')}
    REMOVE_INSTANCE_TUNABLES = Situation.NON_USER_FACING_REMOVE_INSTANCE_TUNABLES
    ...
    def _states(cls):
        return [
         SituationStateData(1, WaitState, factory=(cls.sim_wait_state)),
         SituationStateData(2, TakeTurnState, factory=(cls.take_turn_state)),
         SituationStateData(3, TakeTurnState, factory=(cls.take_last_turn_state))]
