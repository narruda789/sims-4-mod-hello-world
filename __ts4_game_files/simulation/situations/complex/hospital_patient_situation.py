...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\hospital_patient_situation.py
    ...
    def should_state_type_load_after_time_jump(cls, state_type):
        if not super().should_state_type_load_after_time_jump(state_type):
            ...
            if state_type is _AdmittedState:
                ...
                if state_type is _DiagnosedState:
        ...
        if event is TestEvent.InteractionComplete:
            if patient is not None:
                if sim_info is patient.sim_info:
                    if not resolver.interaction.has_been_reset:
                        if resolver(self.owner.go_to_diagnosed_interactions):
                            self._change_state(_DiagnosedState())
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event is TestEvent.InteractionComplete:
            if patient is not None:
                if sim_info is patient.sim_info:
                    if not resolver.interaction.has_been_reset:
                        if resolver(self.owner.go_to_treated_interactions):
