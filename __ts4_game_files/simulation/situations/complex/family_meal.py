...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\family_meal.py
# Compiled at: 2016-10-24 20:32:48
# Size of source mod 2**32: 13960 bytes
...
import alarms, clock, interactions, services, sims4.tuning.tunable, situations.bouncer.bouncer_types
...
class FamilyMealSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'chef':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                    The SituationJob for the sim making the meal.'),
       chef_cooking_role_state=RoleState.TunableReference(description="\n                    Chef's role state while making food."),
       chef_eating_role_state=RoleState.TunableReference(description="\n                    Chef's role state when eating."),
       tuning_group=GroupNames.ROLES), 
     'household_eater':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description='\n                    The SituationJob for an eater (a non-chef) sim.'),
       household_eater_cooking_role_state=RoleState.TunableReference(description="\n                    Eater's role state while food is being prepared."),
       household_eater_eating_role_state=RoleState.TunableReference(description="\n                    Eater's role state when eating."),
       tuning_group=GroupNames.ROLES), 
     'guest_eater':sims4.tuning.tunable.TunableTuple(situation_job=SituationJob.TunableReference(description="\n                    The SituationJob for an eater (a non-chef) sim who doesn't live here."),
       guest_eater_cooking_role_state=RoleState.TunableReference(description="\n                    Guest eater's role state while food is being prepared."),
       guest_eater_eating_role_state=RoleState.TunableReference(description="\n                    Guest eater's role state when eating."),
       tuning_group=GroupNames.ROLES), 
     'cook_group_meal_interaction':TunableInteractionOfInterest(description='\n                                            When this interaction is started, the chef has successfully\n                                            begun preparing the meal.',
       tuning_group=GroupNames.TRIGGERS), 
     'meal_is_done_interaction':TunableInteractionOfInterest(description='\n                                            When this interaction has been completed by the chef, it will\n                                            signal the end of the eating phase.',
       tuning_group=GroupNames.TRIGGERS), 
     'call_to_meal_interaction':sims4.tuning.tunable.TunableReference(description='\n                                            Interaction that is pushed on the chef sim with the crafted \n                                            object as the target. \n                                            ',
       manager=services.get_instance_manager(sims4.resources.Types.INTERACTION)), 
     'trying_to_cook_timeout':TunableSimMinute(description='\n                        The amount of time the sim will attempt to try to start cooking.',
       default=30,
       tuning_group=GroupNames.TRIGGERS), 
     'cooking_timeout':TunableSimMinute(description='\n                        The amount of time the sim will attempt to actually cook.',
       default=60,
       tuning_group=GroupNames.TRIGGERS), 
     'meal_timeout':TunableSimMinute(description='\n                        The amount of time the meal will last, assuming not all sims made it to the meal.',
       default=30,
       tuning_group=GroupNames.TRIGGERS)}
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, TryingToCookState),
         SituationStateData(2, CookState),
         SituationStateData(3, EatState))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.chef.situation_job, cls.chef.chef_cooking_role_state),
         (
          cls.household_eater.situation_job, cls.household_eater.household_eater_cooking_role_state),
         (
          cls.guest_eater.situation_job, cls.guest_eater.guest_eater_cooking_role_state)]
...
lock_instance_tunables(FamilyMealSituation, exclusivity=(situations.bouncer.bouncer_types.BouncerExclusivityCategory.NORMAL),
...
class CookState(SituationState):
    ...
    def handle_event(self, sim_info, event, resolver):
        if event == TestEvent.ItemCrafted:
            ...
            if self.owner._was_cooking_interaction_canceled(event, resolver):
                self.owner._self_destruct()
