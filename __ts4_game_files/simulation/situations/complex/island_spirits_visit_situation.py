...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\island_spirits_visit_situation.py
# Compiled at: 2019-02-21 16:56:30
...
class IslandSpiritsVisitSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'visit_state':VisitState.TunableFactory(description='\n            The state in which spirits will perform a set tuned of interactions.\n            ',
       display_name='01_visit_state',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'island_spirit':TunableTuple(situation_job=SituationJob.TunableReference(description="\n                Island spirit's job during the visit.\n                "),
     ...
     'island_elemental':TunableTuple(situation_job=SituationJob.TunableReference(description="\n                Island elemental's job during the visit.\n                "),
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.island_elemental.situation_job, cls.island_elemental.situation_role_state),
         (
          cls.island_spirit.situation_job, cls.island_spirit.situation_role_state)]
    ...
    def _expand_guest_list_based_on_tuning(self):
        ...
        if sim_info == None:
            ...
            return self.island_elemental.spawn_tests.run_tests(resolver) or None
        guest_info = SituationGuestInfo(0, (self.island_elemental.situation_job), spawning_option=(RequestSpawningOption.DONT_CARE),
