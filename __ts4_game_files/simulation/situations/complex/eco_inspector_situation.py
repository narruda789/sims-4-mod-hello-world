...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\eco_inspector_situation.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 11472 bytes
...
class EcoInspectorSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'inspector_job_and_role_state':TunableSituationJobAndRoleState(description='\n            The job and role state for the eco-inspector.\n            ',
     ...
     'eco_inspect_state':_EcoInspectState.TunableFactory(description='\n           Inspection + Vacuuming state\n            ',
       display_name='1. Eco Inspect',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'expelling_smog_state':_ExpellingSmogState.TunableFactory(description='\n           Expelling state\n            ',
       display_name='2. Expelling Smog',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'leaving_state':_LeavingState.TunableFactory(description='\n            Inspector is leaving after Vacuuming.\n            ',
       display_name='3. Leaving State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'expel_smog':TunableInteractionOfInterest(description='\n            Social: listen to move to expelling_smog state\n            '), 
     'delta_expel':Tunable(description='\n            How much to increase the pollution level by?  \n            ',
       tunable_type=float,
       default=0.0), 
     'leave':TunableInteractionOfInterest(description='\n            Listen for this enter the leave state.\n            '), 
    ...
    def _states(cls):
        return (
         ...
         SituationStateData(2, _EcoInspectState, factory=(cls.eco_inspect_state)),
         SituationStateData(3, _ExpellingSmogState, factory=(cls.expelling_smog_state)),
         SituationStateData(4, _LeavingState, factory=(cls.leaving_state)))
