...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\relaxation_center_visitor_situation.py
...
import filters, services, sims4.tuning
...
class _ArrivingState(CommonInteractionCompletedSituationState, AnchoredAutonomySituationStateMixin):
    ...
    def on_activate(self, reader=None):
        ...
        if self._arrival_timeout is not None:
            self._create_or_load_alarm(ARRIVING_TIMEOUT, (self._arrival_timeout), (lambda _: self.timer_expired()),
    ...
    def _additional_tests(self, sim_info, event, resolver):
        if not self.owner.sim_of_interest(sim_info):
            ...
            return self.owner.all_sims_of_interest_arrived(self._arrived_sims) or False
        return True
...
class _DoStuffState(CommonSituationState, AnchoredAutonomySituationStateMixin):
    ...
    def on_activate(self, reader=None):
        ...
        if self._do_stuff_timeout is not None:
            ...
            self._create_or_load_alarm(DO_STUFF_TIMEOUT, duration, (lambda _: self.timer_expired()),
              should_persist=True, reader=reader)
...
class _ChangeClothesLeave(CommonInteractionCompletedSituationState, AnchoredAutonomySituationStateMixin):
    ...
    def on_activate(self, reader=None):
        ...
        if self._departing_timeout is not None:
            self._create_or_load_alarm(DEPARTING_TIMEOUT, (self._departing_timeout), (lambda _: self.timer_expired()),
              should_persist=True, reader=reader)
    ...
    def _additional_tests(self, sim_info, event, resolver):
        if not self.owner.sim_of_interest(sim_info):
            ...
            return resolver.interaction.is_finishing_naturally or False
        return True
...
class RelaxationCenterVisitorSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'situation_default_job':SituationJob.TunableReference(description='\n            The default job that a visitor will be in during the situation.\n            '), 
     ...
     'do_stuff_situation_state':_DoStuffState.TunableFactory(description='\n            The main state of the situation. This is where Sims will do \n            everything except for arrive and leave.\n            ',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       display_name='02_do_stuff_situation_state'), 
     'change_clothes_leave_situation_state':_ChangeClothesLeave.TunableFactory(description='\n            The state that is used to get the Sim to change clothes before \n            ending the situation and ending up in the leave lot situation.\n            ',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       display_name='03_change_clothes_leave_situation_state')}
    ...
    @classmethod
    def _states(cls):
        return [SituationStateData(1, _ArrivingState, factory=(cls.arriving_situation_state)),
         SituationStateData(2, _DoStuffState, factory=(cls.do_stuff_situation_state)),
         SituationStateData(3, _ChangeClothesLeave, factory=(cls.change_clothes_leave_situation_state))]
    ...
    def _on_set_sim_job(self, sim, job_type):
        ...
        if self.apply_situation_outfit:
            ...
            sim.sim_info.try_set_current_outfit((OutfitCategory.SITUATION, 0), arb=arb)
    ...
    def sim_of_interest(self, sim_info):
        if self._visitor is not None:
            if self._visitor.sim_info is sim_info:
                return True
        return False
