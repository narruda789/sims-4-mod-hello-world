...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\skeleton_npc_situation.py
# Compiled at: 2018-07-22 17:25:27
# Size of source mod 2**32: 5874 bytes
...
class EndSkeleton(CommonInteractionCompletedSituationState):
    ...
    def _additional_tests(self, sim_info, event, resolver):
        ...
        if skeleton is not None:
            if self.owner.get_skeleton().sim_info is sim_info:
                return True
        return False
...
class SkeletonNPCSituation(situations.situation_complex.SituationComplexCommon):
    INSTANCE_TUNABLES = {'skeleton_job_and_role_state':TunableSituationJobAndRoleState(description='\n            Job and Role State for the Skeleton Sim in this situation.\n            '), 
     ...
     'end_skeleton_state':EndSkeleton.TunableFactory(description='\n            The state that the skeleton goes into when then situation ends.\n            ',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP), 
     'skeleton_spawn_point':DynamicSpawnPointElement.TunableFactory(), 
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, BeSkeleton, factory=(cls.be_skeleton_state)),
         SituationStateData(2, EndSkeleton, factory=(cls.end_skeleton_state)))
