...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\wait_in_line_together_situation.py
# Compiled at: 2020-09-21 23:59:03
# Size of source mod 2**32: 17164 bytes
import services
...
class _WaitingInLineState(CommonInteractionCompletedSituationState):
    ...
    def _on_interaction_of_interest_complete(self, sim_info=None, resolver=None):
        if self.owner.is_sim_in_situation(sim_info.get_sim_instance()):
...
class _RouteToWaitingLineState(_WaitingInLineState):
    ...
    def _on_interaction_of_interest_complete(self, sim_info=None, resolver=None):
        if not self.owner.is_sim_in_situation(sim_info.get_sim_instance()):
            return
        elif resolver.interaction.has_been_canceled:
            self.owner._self_destruct()
        else:
...
class WaitInLineTogether(SituationComplexCommon):
    INSTANCE_TUNABLES = {'_route_to_waiting_line_state':_RouteToWaitingLineState.TunableFactory(description='\n            The state where sims route to the waiting line together.\n            ',
       display_name='1. Route to Line State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       locked_args={'allow_join_situation':False, 
      'time_out':None}), 
     '_waiting_in_line_state':_WaitingInLineState.TunableFactory(description='\n            The wait in line state, which has the sims wait in the line after\n            routing there.\n            ',
       display_name='2. Waiting In Line State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       locked_args={'allow_join_situation':False, 
      'time_out':None}), 
     '_run_stored_interaction_state':_RunStoredInteractionState.TunableFactory(description='\n            The run stored interaction state, which encapsulates the host sim \n            running the interaction they were lined up for after becoming\n            first in line.\n            ',
       display_name='3. Run Stored Interaction State',
       tuning_group=SituationComplexCommon.SITUATION_STATE_GROUP,
       locked_args={'allow_join_situation':False, 
      'time_out':None}), 
     'waiting_in_line_sim_job_and_role_state':TunableSituationJobAndRoleState(description='\n            The job and role state for the sim waiting in the line.\n            ',
       tuning_group=GroupNames.ROLES), 
     'waiting_near_line_sim_job_and_role_state':TunableSituationJobAndRoleState(description='\n            The job and role state for a sim waiting near the sim waiting in \n            the line.\n            ',
       tuning_group=GroupNames.ROLES)}
    ...
    def _states(cls):
        return (SituationStateData(1, GetSimsState),
         SituationStateData(2, _RouteToWaitingLineState, factory=(cls._route_to_waiting_line_state)),
         SituationStateData(3, _WaitingInLineState, factory=(cls._waiting_in_line_state)),
         SituationStateData(4, _RunStoredInteractionState, factory=(cls._run_stored_interaction_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.waiting_in_line_sim_job_and_role_state.job, cls.waiting_in_line_sim_job_and_role_state.role_state),
         (
          cls.waiting_near_line_sim_job_and_role_state.job, cls.waiting_near_line_sim_job_and_role_state.role_state)]
    ...
    def _destroy(self):
        ...
        if self.route_nearby_affordance is not None:
    ...
    def _on_routing_formation_started(self, slave=None):
        if self.is_sim_in_situation(slave):
            if slave is None:
                return
    ...
    def try_to_push_routing_formation(self):
        ...
        for nearby_sim_id in self.waiting_nearby_sim_ids:
            ...
            if nearby_sim is None:
                self._self_destruct()
                return
                wait_near_context = InteractionContext(nearby_sim, (InteractionContext.SOURCE_SCRIPT),
                  (Priority.High),
                  insert_strategy=(QueueInsertStrategy.FIRST))
                success = nearby_sim.push_super_affordance((LineUtils.WAIT_IN_LINE_TOGETHER_ROUTING_FORMATION), waiting_in_line_sim,
                  wait_near_context,
                  allow_posture_changes=True,
                  must_run_next=True,
                  name_override='WaitNearSimInLineRoutingFormation')
                success or self._self_destruct()