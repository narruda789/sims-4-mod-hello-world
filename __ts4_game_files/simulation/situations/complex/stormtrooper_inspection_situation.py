...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\complex\stormtrooper_inspection_situation.py
# Compiled at: 2020-05-15 16:59:30
# Size of source mod 2**32: 11187 bytes
import random
    ...
    def _on_interaction_of_interest_complete(self, resolver=None, **kwargs):
        if resolver is not None:
            if resolver.interaction is not None:
                ...
                suspect_sim = self.owner.get_suspect()
                if ko_sim is not None:
                    if inspector_sim is not None:
                        if suspect_sim is not None:
                            if ko_sim.sim_id == inspector_sim.sim_id:
                                if suspect_sim.sim_info.is_npc:
                                    services.get_zone_situation_manager().make_sim_leave_now_must_run(suspect_sim)
        self._change_state(self.owner.fallback_state())
...
class StormtrooperInspectionSituation(SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _FleeState, factory=(cls.flee_state)),
         SituationStateData(2, _SurrenderState, factory=(cls.surrender_state)),
         SituationStateData(3, _FightState, factory=(cls.fight_state)),
         SituationStateData(4, _FallbackState, factory=(cls.fallback_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.suspect_job_and_role_state.job, cls.suspect_job_and_role_state.role_state),
         (
          cls.inspector_job_and_role_state.job, cls.inspector_job_and_role_state.role_state)]
