...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\situation_guest_list.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 20600 bytes
...
import enum, services, situations.situation_types
...
class SituationGuestInfo:
    ...
    def construct_from_purpose(cls, sim_id, job_type, invitation_purpose):
        if not invitation_purpose == SituationInvitationPurpose.INVITED:
            if invitation_purpose == SituationInvitationPurpose.CAREER or invitation_purpose == SituationInvitationPurpose.PREFERRED:
            ...
            if invitation_purpose == SituationInvitationPurpose.HIRED or invitation_purpose == SituationInvitationPurpose.AUTO_FILL:
                request_priority = job_type.sim_auto_invite_allow_priority
            else:
                if invitation_purpose == SituationInvitationPurpose.WALKBY:
                    request_priority = BouncerRequestPriority.BACKGROUND_LOW
                else:
                    if invitation_purpose == SituationInvitationPurpose.LEAVE:
        ...
        if not sim_id == 0 or invitation_purpose == SituationInvitationPurpose.HIRED or invitation_purpose == SituationInvitationPurpose.AUTO_FILL or invitation_purpose == SituationInvitationPurpose.WALKBY:
            if invitation_purpose == SituationInvitationPurpose.AUTO_FILL:
                if not job_type.sim_auto_invite_allow_instanced_sim:
                ...
                if sim_id != 0 and job_type.no_show_action == situations.situation_types.JobHolderNoShowAction.REPLACE_THEM:
            ...
            guest_info = SituationGuestInfo(sim_id, job_type,
              ...
              accept_alternate_sim,
              common_blacklist_categories=common_blacklist_categories,
              elevated_importance_override=elevated_importance_override)
            if invitation_purpose == SituationInvitationPurpose.HIRED:
    ...
    def clone_for_guest_list(self):
        return SituationGuestInfo((self.sim_id), (self.job_type),
          (self.spawning_option),
          (self.request_priority),
          expectation_preference=(self.expectation_preference),
          accept_alternate_sim=(self.accept_alternate_sim),
          common_blacklist_categories=(self.common_blacklist_categories),
          elevated_importance_override=(self.elevated_importance_override),
...
class SituationGuestList:
    ...
    @property
    def job_type_to_guest_infos(self):
        return self._job_type_to_guest_infos

    def clone(self):
        new_guest_list = SituationGuestList(invite_only=(self._invite_only), host_sim_id=(self._host_sim_id),
          filter_requesting_sim_id=(self.filter_requesting_sim_id))
    ...
    def get_traveler_id(self, allow_uninstanced=False):
        if self.host_sim_info is not None:
            if self.host_sim_info.is_instanced(allow_hidden_flags=ALL_HIDDEN_REASONS) or allow_uninstanced and self.host_sim_info.is_selectable:
                return self.host_sim_id
        object_manager = services.object_manager()
        sim_info_manager = services.sim_info_manager()
        selectable_sims = []
        for guest_infos in self._job_type_to_guest_infos.values():
            for guest_info in guest_infos:
                guest_id = guest_info.sim_id
                if not guest_id:
                    continue
                if allow_uninstanced:
                    guest_sim_info = sim_info_manager.get(guest_id)
                    if guest_sim_info is not None:
                        if guest_sim_info.is_selectable:
                            selectable_sims.append(guest_id)
                            continue
                        sim = object_manager.get(guest_id)
                        if sim is not None and sim.is_selectable:
                            return guest_id

        if selectable_sims:
            return selectable_sims.pop()

    def get_other_traveler_ids(self, traveling_sim_id):
        traveling_sim_ids = set()
        npc_guest_infos = []
        traveling_sim_info = services.sim_info_manager().get(traveling_sim_id)
        sim_info_manager = services.sim_info_manager()
        for guest_infos in self._job_type_to_guest_infos.values():
            for guest_info in guest_infos:
                if guest_info.sim_id == 0 or guest_info.sim_id == traveling_sim_id:
                    continue
        ...
        potential_toddler_caregivers = traveling_sim_info.lives_here or set()
        potential_pet_caregivers = set()
        travel_toddlers_ids = set()
        travel_pet_ids = set()
        for active_sim in services.active_household().instanced_sims_gen():
            if active_sim.id == traveling_sim_info.id or active_sim.sim_info.id in traveling_sim_ids:
                continue
            if active_sim.sim_info.is_pet:
                if not active_sim.sim_info.lives_here:
