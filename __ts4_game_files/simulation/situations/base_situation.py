...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\base_situation.py
# Compiled at: 2022-08-26 18:13:12
# Size of source mod 2**32: 133331 bytes
...
import build_buy, distributor.ops, enum, gsi_handlers, id_generator, interactions.context, services, sims4.log, tag, telemetry_helper
...
class BaseSituation(IBouncerClient, SituationScoringMixin):
    ...
    AUTOMATIC_BRONZE_TRAITS = TunableList(description='\n        An optional collection of traits that, if possessed by the host, will automagically promote the situation to bronze on start.',
      tunable=TunableReference(description='\n            A trait that if possessed by the host will start a given situation at bronze.',
    ...
    def start_situation(self):
        ...
        services.get_event_manager().process_events_for_household((test_events.TestEvent.SituationStarted), (services.active_household()),
          situation=self)
    ...
    def should_seed_be_loaded(cls, seed, load_open_street_situation_with_selectable_sim=False):
        ...
        if cls.situation_serialization_option == SituationSerializationOption.LOT and not cls.time_jump.should_load(seed):
            logger.debug("Don't load lot situation:{} due to Sim Time passing", (seed.situation_type), owner='sscholl')
            return False
            if not cls.survives_active_household_change:
            ...
            if zone.venue_changed_between_save_and_load():
                ...
                return False
            if zone.lot_owner_household_changed_between_save_and_load():
    ...
    def _self_destruct(self):
        if self._stage >= SituationStage.DYING:
            ...
            return self.manager._request_destruction(self) or None
    ...
    def on_remove(self):
        ...
        if self.should_give_rewards:
            if services.current_zone().is_zone_shutting_down == False:
                ...
                for sim, situation_sim in self._situation_sims.items():
                    ...
                    if sim.is_selectable or job_type.give_rewards_to_npc:
                        if job_type.rewards:
                ...
                if self.is_user_facing:
                    with telemetry_helper.begin_hook(writer, TELEMETRY_HOOK_STOP_SITUATION, sim=(self._guest_list.host_sim)) as (hook):
                        ...
                        if venue_type is not None:
                            hook.write_int(TELEMETRY_FIELD_SITUATION_VENUE_TYPE, venue_type)
                elif not self.visible_to_client:
                    ...
                    for sim in self.all_sims_in_situation_gen():
                        services.get_event_manager().process_event((test_events.TestEvent.SituationEnded), sim_info=(sim.sim_info),
                          situation=self,
                          custom_keys=(self.custom_event_keys))
                    ...
                    if level_data.medal == SituationMedal.GOLD:
        ...
        if False:
            if gsi_handlers.situation_handlers.situation_archiver.enabled:
    ...
    def _create_standard_save_seed(self):
        ...
        seed = SituationSeed((type(self)), (SeedPurpose.PERSISTENCE), (self.id), guest_list,
          (self.is_user_facing), duration_override=(self._get_remaining_time_in_minutes()),
          zone_id=(services.current_zone().id),
          start_time=(self._start_time),
          scoring_enabled=(self.scoring_enabled),
          main_goal_visiblity=(self._main_goal_visibility),
          linked_sim_id=(self.linked_sim_id),
          guest_attire_color=(self.guest_attire_color),
          guest_attire_style=(self.guest_attire_style))
    ...
    def handle_event(self, sim_info, event, resolver):
        if self.scoring_enabled:
            if not self._main_goal_visibility:
                if self.main_goal_visibility_test is not None:
                    if event in self.main_goal_visibility_test.test_events:
    ...
    def _get_relationship_bits_to_add_to_sims(self, sim, job_type):
        ...
        for relationship_data in self.relationship_between_job_members:
            ...
            if job_type == relationship_data.job_x:
                ...
                if job_type == relationship_data.job_y:
    ...
    def _on_add_sim_to_situation(self, sim, job_type, role_state_type_override=None):
        ...
        if not (job_type.ignore_on_creation_restrictions and sim.is_npc):
            ...
            if ensemble_data.ensemble_option == EnsembleOption.ADD_TO_HOST:
    ...
    def _on_remove_sim_from_situation(self, sim):
        ...
        ensemble_data = self._ensemble_data
        if ensemble_data is not None:
            if not ensemble_data.ignore_situation_removal:
    ...
    def _remove_situation_buff_from_sim(self, sim, situation_sim):
        if sim is not None:
            if situation_sim.buff_handle is not None:
    ...
    def is_sim_info_in_situation(self, sim_info):
        sim = sim_info.get_sim_instance()
        return sim is None or self.is_sim_in_situation(sim) or False
        return True
    ...
    def on_sim_unassigned_from_request(self, sim, request):
        ...
        if job_type.died_or_left_action == JobHolderDiedOrLeftAction.END_SITUATION:
            ...
            if job_type.died_or_left_action == JobHolderDiedOrLeftAction.REPLACE_THEM:
    ...
    def on_failed_to_spawn_sim_for_request(self, request):
        ...
        if job_type.no_show_action == JobHolderNoShowAction.END_SITUATION:
            ...
            if job_type.no_show_action == JobHolderNoShowAction.REPLACE_THEM:
    ...
    def _make_late_auto_fill_request(self, job_type):
        ...
        request = BouncerRequest(self, callback_data=(_RequestUserData()),
          job_type=job_type,
          request_priority=request_priority,
          user_facing=(self.is_user_facing),
          exclusivity=(self.exclusivity),
          common_blacklist_categories=(SituationCommonBlacklistCategory.ACTIVE_HOUSEHOLD | SituationCommonBlacklistCategory.ACTIVE_LOT_HOUSEHOLD),
    ...
    def _expand_guest_list_based_on_tuning(self):
        ...
        if self.resident_job() is not None:
            if host_sim_id != 0:
                if self._guest_list.get_guest_info_for_sim_id(host_sim_id) is None:
    ...
    def _create_reservation_request_from_guest_info(self, guest_info):
        request = SimReservationRequest(self, (guest_info.sim_id),
          (self.exclusivity),
          job_type=(guest_info.job_type),
          request_priority=(guest_info.request_priority),
          spawning_option=(guest_info.spawning_option),
          expectation_preference=(guest_info.expectation_preference))
        return request

    def _create_guest_info_from_reservation_request(self, request):
        return SituationGuestInfo((request.sim_id), (request.job_type),
          (request.spawning_option),
          (request.request_priority),
          (request.expectation_preference),
          reservation=True)
    ...
    def _create_request_from_guest_info(self, guest_info, spawn_point_override=None, location_override=None, reassign_within_situation=False):
        request = BouncerRequest(self, callback_data=(_RequestUserData(guest_info.persisted_role_state_type)),
          job_type=(guest_info.job_type),
          request_priority=(guest_info.request_priority),
          user_facing=(self.is_user_facing),
          exclusivity=(self.exclusivity),
          requested_sim_id=(guest_info.sim_id),
          accept_alternate_sim=(guest_info.accept_alternate_sim),
          spawning_option=(guest_info.spawning_option),
          requesting_sim_info=(self.requesting_sim_info),
          expectation_preference=(guest_info.expectation_preference),
          common_blacklist_categories=(guest_info.common_blacklist_categories),
          for_persisted_sim=(guest_info.for_persisted_sim),
          elevated_importance_override=(guest_info.elevated_importance_override),
          accept_looking_for_more_work=(guest_info.job_type.accept_looking_for_more_work),
          specific_spawn_point=spawn_point_override,
          specific_location=location_override,
          reassign_within_situation=reassign_within_situation)
        return request
    ...
    def _create_guest_info_from_request(self, request):
        ...
        if sim is not None:
            guest_info = SituationGuestInfo((sim.id), (request.job_type),
              (request.spawning_option),
              (request.request_priority),
              (request.expectation_preference),
              (request.accept_alternate_sim),
              (request.common_blacklist_categories),
              elevated_importance_override=(request.elevated_importance_override))
            guest_info._set_persisted_role_state_type(self.get_current_role_state_for_sim(sim))
        else:
            if request.is_factory == False:
                guest_info = SituationGuestInfo((request.requested_sim_id), (request.job_type),
                  (request.spawning_option),
                  (request.request_priority),
                  (request.expectation_preference),
                  (request.accept_alternate_sim),
        ...
        if self.resident_job() is not None:
            if host_sim is not None:
                if host_sim.sim_info.lives_here:
                    request = BouncerHostRequestFactory(self, callback_data=(_RequestUserData()),
                      job_type=(self.resident_job()),
                      user_facing=(self.is_user_facing),
                      exclusivity=(self.exclusivity),
                      requesting_sim_info=(self.requesting_sim_info))
                    self.manager.bouncer.submit_request(request)
    ...
    def _create_uninvited_request(self):
        if not self._is_invite_only:
            if self.default_job() is not None:
                request = BouncerFallbackRequestFactory(self, callback_data=(_RequestUserData()),
                  job_type=(self.default_job()),
                  user_facing=(self.is_user_facing),
                  exclusivity=(self.exclusivity))
                self.manager.bouncer.submit_request(request)
    ...
    def invite_sim_to_job(self, sim, job=DEFAULT, reassign_within_situation=False):
        ...
        if job is None:
            logger.error('Requesting invitation to a None job on a situation ({}).', self,
              owner='manus')
            return
        guest_info = SituationGuestInfo((sim.id), job,
    ...
    def _get_weather_uniform(self, sim):
        if not self.manager.sim_being_created is sim:
            weather_service = services.current_zone().is_zone_running or services.weather_service()
    ...
    def set_job_uniform(self, sim, job, outfit_for_clothing_change=None):
        ...
        if job_uniform is None:
            ...
            if weather_uniform is not None:
                if job.apply_weather_based_uniform:
                    sim.sim_info.set_current_outfit(weather_uniform)
                    return
            if self.try_apply_trailblazer_outfit(sim, resolver):
                return
            if self.manager.sim_being_created is sim:
                services.get_style_service().try_set_style_outfit(sim, resolver)
            return
        if sim.is_selectable:
            if not job_uniform.playable_sims_change_outfits:
                return
        ...
        if job_uniform.use_sold_fashion_outfit:
            ...
            if fashion_trend_service is not None:
                fashion_trend_service.apply_sold_fashion_outfit(target_sim=sim)
                return
        outfit_generators = job_uniform.situation_outfit_generators
        outfit_extra_tag_set = None
        if self._seed:
            if self._seed.has_user_defined_outfit:
                outfit_extra_tag_set = self.get_job_outfit_extra_tag_set(job)
        found_outfit = True
        use_existing_outfit = False
        if outfit_generators:
            should_generate_outfit = self.permit_outfit_generation(job)
            if self._seed.is_loadable or outfit_extra_tag_set is not None:
                outfit_category_for_tags = self.get_preferred_outfit_category()
                try:
                    combined_sim_tags = get_tags_from_outfit(sim.sim_info._base, outfit_category_for_tags, 0)
                    sim_tags = set()
                    for tags in combined_sim_tags.values():
                        sim_tags.update(tags)

                    sim_tags = frozenset(sim_tags)
                    if outfit_extra_tag_set is not None:
                        if any((extra_tag in sim_tags for extra_tag in outfit_extra_tag_set)):
                            use_existing_outfit = True
            ...
            if should_generate_outfit:
                ...
                while outfit_generators_test:
                    outfit_generator = found_outfit or random.choice(outfit_generators_test)
                ...
                if found_outfit:
                    OutfitGenerator.generate_outfit(outfit_generator, (sim.sim_info), (OutfitCategory.SITUATION), outfit_extra_tag_set=outfit_extra_tag_set, outfit_index=0)
        if not found_outfit:
            return
            if outfit_extra_tag_set is not None:
                if use_existing_outfit:
                    outfit_change_reason = self.get_default_job_outfit_change_reason()
                else:
                    outfit_change_reason = self.get_job_outfit_change_reason(job)
            else:
                outfit_priority_handle = sim.sim_info.add_default_outfit_priority(None, outfit_change_reason, job_uniform.outfit_change_priority)
                self._situation_sims[sim].outfit_priority_handle = outfit_priority_handle
                if self.manager.sim_being_created is sim or services.current_zone().is_zone_running or outfit_for_clothing_change is None:
                    new_outfit = sim.sim_info.get_outfit_for_clothing_change(None, (OutfitChangeReason.DefaultOutfit),
                      resolver=resolver)
                else:
    ...
    def get_num_sims_in_job(self, job_type=None):
        ...
        for _, situation_sim in self._situation_sims.items():
            if job_type is None or situation_sim.current_job_type is job_type:
                count += 1
    ...
    def get_default_job_outfit_change_reason(self):
        return OutfitChangeReason.DefaultOutfit
    ...
    def gsi_additional_data(self, gsi_key_name, gsi_value_name):
        ...
        for key, value in self._gsi_additional_data_gen():
            gsi_additional_data.append({gsi_key_name: key, 
             gsi_value_name: value})
    ...
    def build_situation_start_message(self):
        ...
        if self.scoring_enabled:
            ...
            if self.suppress_scoring_progress_bar:
                if not self.show_timer_in_scored_situation:
                    start_msg.situation_display_flags &= ~SituationDisplayFlags.SHOW_TIMER
            if self.situation_end_time_string is not None and not self.user_facing_type == SituationUserFacingType.CAREER_EVENT:
                if self.user_facing_type == SituationUserFacingType.ACTING_CAREER_EVENT:
    ...
    def add_situation_sim_joined_message(self, sim):
        ...
        elif self.is_user_facing and self.manager.is_distributed(self):
    ...
    def try_apply_trailblazer_outfit(self, sim, resolver):
        if FameTunables.TRAILBLAZER_PERK is None:
            ...
            return FameTunables.TRAILBLAZER_EFFECT_AVAILABLE_TESTS.run_tests(resolver) or False
        ...
        for target_sim in active_household.instanced_sims_gen():
            ...
            if target_sim_info.species == sim_info.species:
                if sim_info.clothing_preference_gender == target_sim_info.clothing_preference_gender and bucks_tracker.is_perk_unlocked(FameTunables.TRAILBLAZER_PERK) and random.random() <= FameTunables.CHANCE_TO_WEAR_TRAILBLAZER_OUTFIT:
                    ...
                    self.manager.sim_being_created is sim or services.current_zone().is_zone_running or sim.set_current_outfit((OutfitCategory.SITUATION, 0))
                    return True
                context = InteractionContext(sim, (InteractionContext.SOURCE_SCRIPT),
                  (interactions.priority.Priority.High),
                  insert_strategy=(QueueInsertStrategy.NEXT),
