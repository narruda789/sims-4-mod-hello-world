...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\npc_hosted\need_something_fixed.py
# Compiled at: 2019-02-03 17:03:11
# Size of source mod 2**32: 12627 bytes
import random
...
import enum, event_testing, services, sims4
        ...
        if reader is None:
            if self._done_reason is not None:
                ...
                if self._done_reason == RepairNeighborLeaveReason.FINISHED_WORK:
                    ...
                    if self._done_reason == RepairNeighborLeaveReason.ASKED_TO_LEAVE:
...
class NeedSomethingFixedSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'repair_neighbor_job':SituationJob.TunableReference(description='\n            The job for repair neighbor sim in this situation and the corresponding\n            starting role state for service sim.\n            ',
     ...
     'front_door_repair_arrival':RepairArrivalFrontDoorState.TunableFactory(description='\n            The state in which the repair neighbor arrives when there is not a front door.\n            ',
       display_name='1. Has Front Door Arrival State',
       tuning_group=GroupNames.STATE,
       locked_args={'allow_join_situation': True}), 
     'no_front_door_repair_arrival':RepairArrivalNoDoorState.TunableFactory(description='\n            The state in which the repair neighbor arrives when there is not a front door.\n            ',
       display_name='1. Has No Front Door Arrival State',
       tuning_group=GroupNames.STATE,
       locked_args={'allow_join_situation': True}), 
     'wait_to_be_let_in_state':RepairWaitToBeLetInState.TunableFactory(description='\n            Second state of the situation.  In this state the repair neighbor will\n            wait to be let into the house.\n            ',
       display_name='2. Wait to Be Let in State',
       tuning_group=GroupNames.STATE,
       locked_args={'allow_join_situation': True}), 
     'repair_state':RepairSituationState.TunableFactory(description='\n            ',
       display_name='3. Repair Situation State',
       tuning_group=GroupNames.STATE,
       locked_args={'allow_join_situation': True}), 
     'repair_job_done_tests':TunableMapping(description='\n            Tune pairs of job finish role states with job finish tests. When\n            those tests pass, the repair neighbor will transition to the paired\n            role state.\n            ',
       display_name='3. Repair State Job Done Tests',
       key_name='Leave Reason',
       key_type=RepairNeighborLeaveReason,
       value_type=TunableFinishJobStateAndTest(),
       value_name='Finish Test and Role State',
       tuning_group=GroupNames.STATE)}
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData.from_auto_factory(1, cls.no_front_door_repair_arrival),
         SituationStateData.from_auto_factory(2, cls.front_door_repair_arrival),
         SituationStateData.from_auto_factory(3, cls.wait_to_be_let_in_state),
    ...
    def get_predefined_guest_list(cls):
        ...
        repair_neighbor_results = services.sim_filter_service().submit_filter((cls.repair_neighbor_job.filter), callback=None,
        ...
        guest_list = SituationGuestList(invite_only=True, host_sim_id=(repair_neighbor.sim_info.sim_id),
