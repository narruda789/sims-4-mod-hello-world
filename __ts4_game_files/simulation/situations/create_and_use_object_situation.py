...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\create_and_use_object_situation.py
# Compiled at: 2019-08-01 18:57:26
...
class CreateAndUseObjectSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (
         SituationStateData(1, _CreateAndUseObjectSituationInitialState, factory=(cls.initial_state)),
         SituationStateData(2, _CreateAndUseObjectSituationFinalState, factory=(cls.final_state)))
    ...
    def _on_set_sim_job(self, sim, job):
        ...
        if self._target_object:
            if self.grant_relationship_with_target_object:
                if self._target_object:
                    ObjectRelationshipComponent.setup_relationship(sim, self._target_object)
        if self._created_object:
            if self.grant_relationship_with_created_object:
...
lock_instance_tunables(CreateAndUseObjectSituation, creation_ui_option=(SituationCreationUIOption.NOT_AVAILABLE),
