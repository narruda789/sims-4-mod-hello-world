...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\situations\service_npcs\modify_lot_items_tuning.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 20582 bytes
...
import event_testing.results, event_testing.state_tests, event_testing.test_base, event_testing.test_events, event_testing.tests, objects.object_tests, seasons, services, world.world_tests
...
class TunableObjectModifyGlobalTestVariant(TunableVariant):
    ...
    def __init__(self, additional_tests=None, **kwargs):
        ...
        (super().__init__)(season=seasons.season_tests.SeasonTest.TunableFactory(locked_args={'tooltip': None}), 
         elapsed_time=TimeElapsedZoneTest.TunableFactory(locked_args={'tooltip': None}), 
         zone=ZoneTest.TunableFactory(locked_args={'tooltip': None}), 
         object_criteria=objects.object_tests.ObjectCriteriaTest.TunableFactory(locked_args={'tooltip': None}), **kwargs)
...
class TunableObjectModifyTestVariant(TunableVariant):
    ...
    def __init__(self, description='A single tunable test.', additional_tests=None, **kwargs):
        ...
        (super().__init__)(description=description, 
         ...
         object_definition=TunableObjectMatchesDefinitionOrTagTest(), 
         inventory=objects.object_tests.InventoryTest.TunableFactory(locked_args={'tooltip': None}), 
         custom_name=objects.object_tests.CustomNameTest.TunableFactory(locked_args={'tooltip': None}), 
         consumable_test=objects.object_tests.ConsumableTest.TunableFactory(locked_args={'tooltip': None}), 
         existence=objects.object_tests.ExistenceTest.TunableFactory(locked_args={'tooltip':None,  'require_instantiated':False}), 
         game_option=event_testing.game_option_tests.SimInfoGameplayOptionsTest.TunableFactory(), 
         location=world.world_tests.LocationTest.TunableFactory(locked_args={'tooltip':None,  'subject':ParticipantType.Object}), 
         season=seasons.season_tests.SeasonTest.TunableFactory(locked_args={'tooltip': None}), 
         street_civic_policy_test=StreetCivicPolicyTest.TunableFactory(locked_args={'tooltip': None}), 
         object_preference=objects.object_tests.ObjectScoringPreferenceTest.TunableFactory(locked_args={'tooltip': None}), **kwargs)
...
class ObjectMatchesDefinitionOrTagTest(event_testing.test_base.BaseTest):
    FACTORY_TUNABLES = {'description':'Check to see if the specified object matches either a static definition or a set of tags', 
     'item':TunableVariant(actual_item=objects.object_tests.CraftActualItemFactory(), tagged_item=objects.object_tests.CraftTaggedItemFactory(),
       default='tagged_item',
       description='Whether to test for a specific item or item that has a set of tags')}
...
class ModifyAllLotItems(HasTunableFactory, AutoFactoryInit):
    ...
    FACTORY_TUNABLES = {'description':'\n        Tune modifications to apply to all objects on a lot.\n        Can do state changes, destroy certain items, etc.\n        \n        EX: for auto cleaning, tune to have objects with Dirtiness state that\n        equals dirty to be set to the clean state and tune to have dirty dishes\n        and spoiled food to be deleted\n        ', 
     'modifications':TunableList(description="\n            A list of where the elements define how to modify objects on the\n            lot. Each entry is a triplet of an object modification action\n            (currently either destroy the object or set its state), a list of\n            tests to run on the object to determine if we should actually apply\n            the modification, and a priority in case some modifications should\n            take precedence over other ones when both of their tests pass.\n            \n            EX: test list: object's dirtiness state != dirtiness clean\n            action: set state to Dirtiness_clean\n            \n            So dirty objects will become clean\n            ",
       tunable=TunableTuple(action=TunableVariant(set_state=TunableTuple(action_value=TunableStateValueReference(description='An object state to set the object to', pack_safe=True),
       locked_args={'action_type': SET_STATE}),
       destroy_object=TunableTuple(locked_args={'action_type': DESTROY_OBJECT}),
       inventory_transfer=TunableTuple(action_value=(InventoryTransferFakePerform.TunableFactory()),
       locked_args={'action_type': INVENTORY_TRANSFER}),
       deliver_bills=TunableTuple(action_value=(DeliverBillFakePerform.TunableFactory()),
       locked_args={'action_type': DELIVER_BILLS}),
       set_on_fire=TunableTuple(locked_args={'action_type': SET_ON_FIRE}),
       cleanup_vehicle=TunableTuple(description='\n                        Cleanup vehicles that are left around.\n                        ',
       locked_args={'action_type': CLEANUP_VEHICLE}),
       loot=TunableTuple(description='\n                        Apply loots to the object.\n                        ',
    ...
    def modify_objects(self, object_criteria=None):
        ...
        for mod in self.modifications:
            if not random_chance(mod.chance * 100):
                continue
            if mod.global_tests:
                if not mod.global_tests.run_tests(GlobalResolver()):
                    continue
        ...
        for obj in all_objects:
            if obj.is_sim:
                continue
            if object_criteria is not None:
                if not object_criteria(obj):
                    continue
            ...
            for action, tests in modifications.copy().items():
                if not tests.run_tests(resolver):
                    continue
                if action in num_modified_required:
    ...
    def _should_cleanup_vehicle(self, obj):
        ...
        if household_owner_id is not None:
            if household_owner_id != 0:
                return False
        if obj.interaction_refs:
            return False
        return True