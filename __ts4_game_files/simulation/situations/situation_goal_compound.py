...
class _EvalModeAny(_EvalModeBase):
    ...
    def get_max_iterations(self, goals, compound_goal_name='name_missing'):
        ...
        if any((goal.max_iterations != iterations for goal in goals)):
            logger.error("Compound any-goal {} has sub goals with different iterations. Listing the compound goal's iterations as the first sub goal's iterations.", compound_goal_name)
        return iterations
