...
# Size of source mod 2**32: 65366 bytes
...
class WhimsTracker(SimInfoTracker, AffordanceCacheMixin):
    ...
    def get_active_whimset_data(self):
        ...
        for target_sim in target_sims:
            for targets_bit in target_sim.relationship_tracker.get_all_bits(sim_id):
                if targets_bit.whim_set is not None:
                    insert_data(targets_bit.whim_set, target_sim)
    ...
    def _get_currently_active_whim_guids(self):
        return {whim_slot.whim.guid64 for whim_slot in self._whim_slots if not whim_slot.is_empty() if not whim_slot.is_empty()}

    def _get_currently_used_whimsets(self):
        return {whim_slot.whimset for whim_slot in self._whim_slots if not whim_slot.is_empty() if whim_slot.whimset is not None if whim_slot.whimset is not None}
