...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\snippets.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 18957 bytes
...
import interactions, services, sims4.reload, sims4.resources
...
def get_defined_snippets_gen(snippet_type):
    for snippet in services.snippet_manager().types.values():
        if isinstance(snippet, SnippetInstanceMetaclass) and snippet.snippet_type is snippet_type:
            yield snippet
...
def define_snippet(snippet_type, snippet, use_list_reference=False):
    ...
    if use_list_reference:
        ...
        SnippetVariantListInstance = SnippetInstanceMetaclass.__new__(SnippetInstanceMetaclass,
          list_name,
          bases,
          class_dict,
...
def is_snippet_list(snippet):
    if isinstance(snippet, SnippetInstanceMetaclass):
        if snippet.is_list:
            return True
    return False
...
class TunableSnippet(TunableVariant):
    ...
    def __init__(self, snippet_type, description=None, allow_list_reference=False, pack_safe=False, **kwargs):
        ...
        (super().__init__)(literal=SNIPPETS[snippet_type], reference=SNIPPET_REFERENCES[snippet_type](pack_safe=pack_safe), 
         default='literal', 
...
class TunableSnippetReference(TunableReference):
    ...
    def __init__(self, snippet_class, **kwargs):
        (super().__init__)(manager=services.get_instance_manager(sims4.resources.Types.SNIPPET), class_restrictions=snippet_class, **kwargs)
...
class TunableSnippetVariantListReference(TunableReference):
    ...
    def __init__(self, snippet_variant_list_instance, **kwargs):
        (super().__init__)(manager=services.get_instance_manager(sims4.resources.Types.SNIPPET), class_restrictions=snippet_variant_list_instance, **kwargs)
...
EXCURSION_ACTIVITY = 'excursion_activity'
EXCURSION_SITUATION_STATE = 'excursion_situation_state'
WAYPOINT_GRAPH = 'waypoint_graph'
ROUTING_FORMATION_LIST = 'routing_formation_list'
TunableAffordanceListReference, TunableAffordanceListSnippet = define_snippet(AFFORDANCE_LIST, TunableList(TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.INTERACTION)), pack_safe=True)))
TunableVenueListReference, TunableVenueListSnippet = define_snippet(VENUE_LIST, TunableList(TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.VENUE)),
  pack_safe=True,
...
class _TunableAffordanceFilter(TunableFactory, is_fragment=True):
    ...
    def __init__(self, description='An affordance filter.', **kwargs):
        ...
        (super().__init__)(default_inclusion=TunableVariant(include_all=TunableTuple(include_all_by_default=Tunable(bool, True, description=''),
  include_affordances=TunableList(AffordanceReference, display_name='Filter Exception Items'),
  exclude_affordances=TunableList(AffordanceReference, display_name='Blacklist Items'),
  include_lists=TunableList(TunableAffordanceListReference(pack_safe=True), display_name='Filter Exception Lists'),
  exclude_lists=TunableList(TunableAffordanceListReference(pack_safe=True), display_name='Blacklist Lists'),
  ...
  exclude_all=TunableTuple(include_all_by_default=Tunable(bool, False, description=''), include_affordances=TunableList(AffordanceReference, display_name='Whitelist Items'),
  exclude_affordances=TunableList(AffordanceReference, display_name='Filter Exception Items'),
  include_lists=TunableList(TunableAffordanceListReference(pack_safe=True), display_name='Whitelist Lists'),
  exclude_lists=TunableList(TunableAffordanceListReference(pack_safe=True), display_name='Filter Exception Lists'),
