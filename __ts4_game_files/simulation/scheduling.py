...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\scheduling.py
...
class Timeline:
    ...
    def _schedule(self, element, when=None, asap=False):
        ...
        if when is None:
            ...
            elif self._active is not None and self._active[0] is element:
    ...
    def soft_stop(self, handle):
        ...
        while pending:
            ...
            if element is None or id(element) in visited:
                continue
            visited[id(element)] = element
            element._soft_stop() or pending.extend(element._get_child_handles())
    ...
    def _stop_element_tree(self, handle):
        ...
        for element in elements:
            if not self._active is not None or self._active[1] is element._element_handle or self._active[0] is element:
                self._active = None
...
class ElementHandle(list):
    ...
    def is_active(self):
        ...
        if timeline is not None:
            if timeline._active is not None:
