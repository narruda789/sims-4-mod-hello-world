...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sickness\sickness_tests.py
...
class _DiagnosticTestBase:
    ...
    def test_result(self, affordance, target, invert, tooltip):
        if self.requires_sick_sim:
            if not target.is_sick():
        ...
        return TestResult(False, '{} for {} on {} returned {}',
          (str(type(self))),
          affordance,
          target,
          test_evaluation,
...
class DiagnosticActionTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    FACTORY_TUNABLES = {'target':TunableEnumEntry(description='\n            When this test runs, it checks against this participant\n            for sickness information.  If the affordance is targeting \n            a patient, it will typically be TargetSim.\n            ',
     ...
     'test':TunableVariant(description='\n            Type of test we are performing against the affordance that\n            runs or will run this test.\n            ',
       default='performed_exam',
       locked_args={'performed_exam':_PerformedExamTest(), 
      'performed_treatment':_PerformedTreatmentTest(), 
      'ruled_out_treatment':_RuledOutTreatmentTest(), 
      'is_treatment_available':_TreatmentAvailabilityTest(), 
      'is_correct_treatment':_CorrectTreatmentTest(), 
      'is_sickness_known':_DiscoveredSicknessTest()}), 
