...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\crafting\crafting_handlers.py
# Compiled at: 2015-06-15 15:34:42
...
archiver = GameplayArchiver('crafting', crafting_archive_schema, enable_archive_by_default=True,
  max_records=200,
...
def log_process(process, sim_id, interaction, logger_crafting):
    ...
    archive_data = {'recipe':process.recipe.__name__,  'affordance':interaction_name, 
     'phase':str(process.phase), 
     'crafter':_get_sim_name(process.crafter)}
...
def log_ingredient_calculation(process, sim_id, ingredient_log):
    archive_data = {'recipe':process.recipe.__name__, 
     'affordance':'ingredient consumption', 
     'phase':'ingredient consumption', 
     'crafter':_get_sim_name(process.crafter)}
...
def log_quality(process, sim_id, quality_entry):
    archive_data = {'recipe':process.recipe.__name__, 
     'affordance':'quality applied', 
     'phase':'quality applied', 
     'crafter':_get_sim_name(process.crafter)}
