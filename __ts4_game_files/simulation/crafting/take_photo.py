...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\crafting\take_photo.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 41228 bytes
...
class _TripodPhotoMode(_SimPhotoMode):
    ...
    FACTORY_TUNABLES = {'canvas_size': TunableEnumEntry(description='\n            The size of the canvas.\n            ',
                      tunable_type=PhotoSize,
                      default=(PhotoSize.LARGE))}
...
class _PaintByReferenceMode(_BasePhotoMode):
    FACTORY_TUNABLES = {'canvas_size':TunableEnumEntry(description='\n            The size of the canvas.\n            ',
       tunable_type=PhotoSize,
       default=PhotoSize.LARGE), 
     'locked_args':{'photo_target_sims_participants':None, 
      'photo_pose':None}}
...
class _RotateTargetSelfieMode(_BasePhotoMode):
    ...
    def enable_rotate_target(self, interaction):
        ...
        if rotate_target is None:
            logger.error('Got a None Sim {} trying to run interaction {}.', (self.rotate_target_sim),
              interaction, owner='shipark')
            return True
        return False
...
class _DecoratorPhotoMode(_BasePhotoMode):
    FACTORY_TUNABLES = {'stored_location':Tunable(description='\n            If True, store the location of the photographer on\n            the photo object. The photo object will provide\n            an interaction to return to the location.\n            ',
     ...
     'locked_args':{'filters_disabled':False, 
      'order_photo_target_sims':False, 
      'photo_target_sims_from_situation':None, 
      'photo_target_sims_participants':None, 
      'photo_pose':None}}
...
class TakePhoto(XevtTriggeredElement):
    FACTORY_TUNABLES = {'photo_mode':TunableVariant(description='\n            The photo mode to use for this photo session.\n            ',
       free_form_photo=_FreeFormPhotoMode.TunableFactory(),
       sim_photo=_SimPhotoMode.TunableFactory(),
       selfie_photo=_SelfiePhotoMode.TunableFactory(),
       two_sim_selfie_photo=_TwoSimSelfiePhotoMode.TunableFactory(),
       photo_studio_photo=_PhotoStudioPhotoMode.TunableFactory(),
       paint_by_reference=_PaintByReferenceMode.TunableFactory(),
       rotate_target_selfie=_RotateTargetSelfieMode.TunableFactory(),
       tripod=_TripodPhotoMode.TunableFactory(),
       decorator_mode=_DecoratorPhotoMode.TunableFactory(),
       cross_stitch_by_reference=_CrossStitchByReferenceMode.TunableFactory(),
       default='free_form_photo'), 
     'loot_to_apply':TunableList(description='\n            Loot defined here will be applied to the participants of the photography\n            interaction after the systems photography call is finished.\n            ',
       tunable=TunableVariant(description='\n            Select Default Take Photo Loot for most Camera Modes.\n            Select Rotate Target Photo Loot for Rotate Target Selfie Mode.\n            ',
       photoLoot=(DefaultTakePhotoLoot.TunableFactory()),
       targetPhotoLoot=(RotateTargetPhotoLoot.TunableFactory()),
       default='photoLoot'),
    ...
    def _order_group_photo_sims(self, photo_target_sims):
        ordered_sims = []
        tags = (
         Photography.GROUP_PHOTO_X_ACTOR_TAG, Photography.GROUP_PHOTO_Y_ACTOR_TAG, Photography.GROUP_PHOTO_Z_ACTOR_TAG)
    ...
    def _do_behavior(self):
        ...
        if situation_target_tuning:
            ...
            for photo_situation in photo_situations:
                if situation_target_tuning.photographer_job is None or photo_situation.get_current_job_for_sim(photographer_sim) is situation_target_tuning.photographer_job:
