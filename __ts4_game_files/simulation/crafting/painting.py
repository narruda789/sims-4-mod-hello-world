...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\crafting\painting.py
# Compiled at: 2021-03-08 19:03:20
...
class PaintingTexture(metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.RECIPE)):
    INSTANCE_TUNABLES = {'texture':TunableResourceKey(None, resource_types=[sims4.resources.Types.TGA], allow_none=True), 
     'tests':TunableTestSet(), 
     'canvas_types':TunableEnumFlags(CanvasType, CanvasType.NONE, description='\n            The canvas types (generally, aspect ratios) with which this texture\n            may be used.\n            ')}
...
class PaintingStyle(metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.RECIPE)):
    ...
    @classmethod
    def pick_texture(cls, crafter, canvas_types: CanvasType, random=None) -> PaintingTexture:
        ...
        for weighted_texture in cls._textures:
            ...
            if not canvas_types & texture.canvas_types or crafter is None or texture.tests.run_tests(resolver):
...
class PaintingRecipe(Recipe):
    ...
    @classmethod
    def pick_texture(cls, crafted_object, crafter, random=None) -> PaintingTexture:
        canvas_types = crafted_object.canvas_component.canvas_types
        texture = cls.painting_style.pick_texture(crafter, canvas_types, random=random)
        return texture
    ...
    def setup_crafted_object(cls, crafted_object, crafter, is_final_product, random=None):
        ...
        if crafted_object.canvas_component is not None:
            texture = cls.pick_texture(crafted_object, crafter, random=random)
            if texture is None:
...
lock_instance_tunables(PaintingRecipe, multi_serving_name=None,
  push_consume=False)
...
class PaintByReferenceRecipe(PaintingRecipe):
    ...
    @classmethod
    def setup_crafted_object(cls, crafted_object, crafter, is_final_product, random=None):
        cls._setup_crafted_object(crafted_object, crafter, is_final_product, random)