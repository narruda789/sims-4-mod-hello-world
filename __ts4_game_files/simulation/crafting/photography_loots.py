...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\crafting\photography_loots.py
# Compiled at: 2020-11-20 04:27:44
...
class RotateTargetPhotoLoot(HasTunableFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'photographer_loot':TunableReference(description='\n            Loot to apply to the Photographer.\n            ',
       ...
       class_restrictions=('LootActions', ),
       pack_safe=True), 
     'target_loot':TunableReference(description='\n            Loot to give to the Rotate Selfie Target.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.ACTION),
       class_restrictions=('LootActions', ),
       pack_safe=True), 
    ...
    def apply_loot(self, sim):
        ...
        if self.photographer is None:
            logger.error('Got a None Sim {} while applying loot to the photographer.', (self.photographer),
              owner='shipark')
        elif self.notification is None:
            return
