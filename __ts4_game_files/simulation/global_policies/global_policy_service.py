...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\global_policies\global_policy_service.py
# Compiled at: 2021-05-26 20:49:25
...
import date_and_time, elements, persistence_error_types, services, sims4
...
class GlobalPolicyService(Service):
    ...
    def add_global_policy_progress(self, global_policy_class, amount, resolver=None):
        ...
        if amount > 0:
            if global_policy.progress_state == GlobalPolicyProgressEnum.COMPLETE or global_policy.decay_handler is not None:
                return
        if resolver is None:
    ...
    def set_global_policy_progress(self, global_policy, new_value, resolver=None):
        ...
        if new_state != old_state:
            if new_state == GlobalPolicyProgressEnum.COMPLETE:
