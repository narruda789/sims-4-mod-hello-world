...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\global_policies\global_policy_tests.py
# Compiled at: 2019-05-02 20:39:07
...
class GlobalPolicyStateTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    FACTORY_TUNABLES = {'test_type':TunableVariant(description="\n            The type of test to run. Note, 'All Active Policies' will test against\n            the set of all policies that have ever had progress made towards them.\n            ",
       all_active_policies=_AllActivePolicies.TunableFactory(),
       specific_policy=_SpecificPolicy.TunableFactory(),
       default='all_active_policies'), 
     'progress_state':TunableEnumEntry(description='\n            The progress state against which to test.\n            ',
       tunable_type=GlobalPolicyProgressEnum,
       default=GlobalPolicyProgressEnum.IN_PROGRESS)}

    def get_test_events_to_register(self):
        return (
         TestEvent.GlobalPolicyProgress,)
