...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\drama_scheduler\picker_drama_node.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 22775 bytes
import elements, enum, random
...
import id_generator, services, sims4.log
...
class _ScheduleDramaNodePickerBehavior(_PickerDramaNodeBehavior):
    ...
    def on_picked(self, owner=None, associated_sim_info=None):
        services.drama_scheduler_service().schedule_node((self.drama_node), (SingleSimResolver(owner)),
          specific_time=(self._saved_node.get_picker_schedule_time()),
          drama_inst=(self._saved_node))
...
class _ScheduleCareerGigPickerBehavior(_PickerDramaNodeBehavior):
    ...
    def create_picker_row(self, owner, associated_sim_info=None, associated_sim_thumbnail_override=None, associated_sim_background=None, enabled=True, **kwargs):
        ...
        picker_row = (self.career_gig.create_picker_row)(scheduled_time=self._scheduled_time, 
         owner=owner, gig_customer=associated_sim_info, 
         customer_thumbnail_override=associated_sim_thumbnail_override, 
         customer_background=associated_sim_background, 
...
class PickerDramaNode(BaseDramaNode, AutoFactoryInit):
    ...
    INSTANCE_TUNABLES = {'behavior':TunableVariant(schedule_drama_node=_ScheduleDramaNodePickerBehavior.TunableFactory(description='\n                Drama node to schedule should the player pick this to run.\n                '),
     ...
     'on_pick_behavior':TunableEnumEntry(description='\n             Determines what happens to this PickerDramaNode if it is picked in\n             a picker. \n             ',
       tunable_type=PickBehavior,
       default=PickBehavior.DO_NOTHING), 
    ...
    def setup(self, resolver, gsi_data=None, **kwargs):
        ...
        if self.associated_sim_filter is not None:
            services.sim_filter_service().submit_filter((self.associated_sim_filter), callback=_on_filter_request_complete,
              allow_yielding=True,
                ...
                def schedule_new_node(timeline):
                    try:
                        try:
                            ...
                            yield from services.drama_scheduler_service().score_and_schedule_nodes_gen(nodes_in_bucket, 1,
                              specific_time=selected_time,
                            ...
                            try:
                                logger.exception('Exception while replacing a drama node', exc=exception,
    ...
    def create_picker_row(self, owner=None, run_visibility_tests=True, disable_row_if_visibily_tests_fail=False, **kwargs):
        ...
        if self.associated_sim_filter is not None:
            if self._associated_sim_info is None:
                results = services.sim_filter_service().submit_filter((self.associated_sim_filter), callback=None,
                ...
                if results:
                    ...
                    return
        elif run_visibility_tests and owner is not None:
            ...
            if not result:
                ...
                if not disable_row_if_visibily_tests_fail:
                    if not self.disable_if_visibility_tests_fail_with_tooltip or tooltip_override:
                        enabled = False
        else:
            return
        if self._disable_sim_ids is not None and owner.id in self._disable_sim_ids:
            enabled = False
        else:
            if self._disabled:
                enabled = False
                ...
                picker_row = self.behavior.create_picker_row(owner=owner,
                  associated_sim_info=(self._associated_sim_info),
                  associated_sim_thumbnail_override=(self.associated_sim_thumbnail_override),
                  associated_sim_background=(self.associated_sim_thumbnail_background),
                  customer_name=(self.associated_sim_name_override),
                  enabled=enabled)
