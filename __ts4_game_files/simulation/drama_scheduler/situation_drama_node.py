...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\drama_scheduler\situation_drama_node.py
# Compiled at: 2021-05-06 14:07:25
# Size of source mod 2**32: 13022 bytes
...
class SituationDramaNode(BaseDramaNode):
    ...
    def _verify_tuning_callback(cls):
        if cls.sender_sim_info_job is not None:
            if cls.sender_sim_info.type == DramaNodeParticipantOption.DRAMA_PARTICIPANT_OPTION_NONE:
        ...
        if cls.host_sim_info_job is not None:
            if cls.host_sim_participant is None:
    ...
    def _test(self, resolver, skip_run_tests=False):
        ...
        if self.homelot_only:
            if homelot_id != services.current_zone_id():
    ...
    def _run_situation(self):
        ...
        if self._sender_sim_info is not None:
            if self.sender_sim_info_job is not None:
        ...
        if host_sim_info is not None:
            if self.host_sim_info_job is not None:
        ...
        situation_id = services.get_zone_situation_manager().create_situation((self.situation_to_run), guest_list=guest_list,
          spawn_sims_during_zone_spin_up=(self.spawn_sims_during_zone_spin_up),
          user_facing=(self.user_facing),
          zone_id=zone_id)
        if self.household_milestone:
            if situation_id:
