...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\drama_scheduler\npc_invite_situation_drama_node.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 33678 bytes
import random
...
import build_buy, services, sims4.resources, telemetry_helper
...
class NPCInviteSituationDramaNode(BaseDramaNode):
    ...
    def _get_zone_id(self):
        if self._zone_id is not None:
            if self._situation_to_run.is_venue_location_valid(self._zone_id):
    ...
    def _get_host(self, additional_sims_to_bring):
        if self._use_alternate_host is None:
            ...
            host = services.sim_filter_service().submit_matching_filter(sim_filter=(self._use_alternate_host), requesting_sim_info=requesting_sim_info,
              blacklist_sim_ids=blacklist,
              allow_yielding=False,
              gsi_source_fn=(lambda : 'NPC Invite Situation: {} is the host filter for this situation'.format(str(self._use_alternate_host))))
            return host or None
    ...
    def _create_situation(self, additional_sims_to_bring=()):
        ...
        if guest_list is None:
            return
        services.get_zone_situation_manager().create_situation((self._situation_to_run), guest_list=guest_list,
          zone_id=(self._get_zone_id()),
          scoring_enabled=(self._NPC_hosted_situation_scoring_enabled),
          user_facing=(self._user_facing),
          allow_uninstanced_main_traveler=(self.simless),
    ...
    def _get_sim_infos_for_travel_picker(self):
        if self._sim_infos_for_travel_picker is not None:
            ...
            results = services.sim_filter_service().submit_filter((bring_other_sims_data.travel_with_filter), callback=None,
              requesting_sim_info=(self._receiver_sim_info),
              blacklist_sim_ids=blacklist,
              allow_yielding=False,
    ...
    def _handle_dialog(self, dialog):
        ...
        additional_valid_responses = (
         NPCHostedSituationDialog.BRING_OTHER_SIMS_RESPONSE_ID,
         NPCHostedSituationDialog.BRING_ONE_OTHER_SIM_RESPONSE_ID)
        if not dialog.accepted:
            if dialog.response not in additional_valid_responses:
                with telemetry_helper.begin_hook(telemetry_writer, TELEMETRY_HOOK_SITUATION_REJECTED, sim_info=(self._receiver_sim_info)) as (hook):
                    hook.write_guid(TELEMETRY_SITUATION_TYPE_ID, self._situation_to_run.guid64)
                services.drama_scheduler_service().complete_node(self.uid)
                return
    ...
    def _run(self):
        ...
        if self._chosen_dialog_data.dialog.bring_other_sims is not None:
            if not self._receiver_sim_info:
                if not sim_infos_to_bring:
                    return DramaNodeRunOutcome.FAILURE
        dialog_owner = self._receiver_sim_info if not self.simless else None
        target_sim_id = self._sender_sim_info.id if not self.simless else None
        dialog = self._chosen_dialog_data.dialog(dialog_owner, target_sim_id=target_sim_id,
          resolver=(self._get_resolver()),
          is_simless=(self.simless),
          sim_infos_to_bring=sim_infos_to_bring)
    ...
    def _test(self, resolver, skip_run_tests=False):
        if self._get_zone_id() is None:
            ...
            if not skip_run_tests:
                if not self.run_over_user_facing_situations:
    ...
    def _setup(self, *args, street_override=None, gsi_data=None, zone_id=None, **kwargs):
        ...
        if zone_id is not None:
            self._zone_id = zone_id
        elif street_override is not None:
