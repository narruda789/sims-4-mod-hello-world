...
# Compiled at: 2021-05-27 14:12:36
# Size of source mod 2**32: 27190 bytes
...
@sims4.commands.Command('qa.clubs.info', command_type=(CommandType.Automation))
def qa_get_clubs(_connection=None):
    club_service = _get_club_service(_connection)
    if club_service is None:
        return
    sims4.commands.automation_output('ClubInfo; Status:Begin', _connection)
    for club in club_service.clubs:
        if not club.invite_only:
            sims4.commands.automation_output('ClubInfo; Status:Data, ClubId:{}, ClubName:{}'.format(club.club_id, str(club)), _connection)

    sims4.commands.automation_output('ClubInfo; Status:End', _connection)