...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\clubs\club_gathering_situation.py
# Compiled at: 2020-11-13 15:39:19
# Size of source mod 2**32: 33411 bytes
...
import alarms, clock, enum, services, sims4
...
class ClubGatheringSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'_default_job':SituationJob.TunableReference(description='\n            The default job for all members of this situation.\n            '), 
     ...
     '_vibe_buffs':TunableMapping(description="        \n            A Mapping of ClubGatheringVibe to List of buffs.\n            \n            When setting the vibe for the gathering the type is found in the\n            mapping and then each buff is processed in order until one of them\n            can be added. Then evaluation stops.\n            \n            Example: The club vibe is getting set to ClubGatheringVibe.Angry.\n            That entry has 3 buffs associated with it in the mapping. Angry3,\n            Angry2, Angry1 in that order. Angry3 doesn't pass evaluation so it\n            is passed. Next Angry2 does pass evaluation and so we add Angry2\n            Vibe Buff to the gathering. Angry1 is never evaluated in this\n            situation. Angry1 is only ever evaluated if Angry3 and Angry2 both\n            fail.\n            ",
       ...
       tunable=(Buff.TunableReference()),
       minlength=1)), 
     '_gathering_buff_reason':TunableLocalizedString(description='\n            The reason the gathering buff was added. Displayed on the buff\n            tooltip.\n            '), 
     '_initial_disband_timer':TunableSimMinute(description='\n            The number of Sim minutes after a Gathering is created before it\n            can disband due to lack of members.\n            ',
       default=30,
       minimum=1), 
     '_initial_notification':TunableUiDialogNotificationSnippet(description='\n            A notification that shows up once the gathering starts.\n            '), 
    ...
    def load_situation(self):
        ...
        if result:
            if not (self.associated_club is None or self.is_validity_overridden() or self.associated_club.is_zone_valid_for_gathering(services.current_zone_id())):
                self._cleanup_gathering()
                return False
        return result
        ...
        if club_service is None:
            ...
            if start_source is not None:
                if start_source == ClubGatheringStartSource.APPLY_FOR_INVITE:
                    ...
                    if any((sim_info.is_selectable for sim_info in self._guest_list.invited_sim_infos_gen())):
    ...
    def _award_club_bucks(self, handle):
        ...
        if not qualified_sims:
            return
        elif any((sim for sim in self._situation_sims if club_tuning.ClubTunables.CLUB_BUCKS_REWARDS_MULTIPLIER.trait in sim.sim_info.trait_tracker.equipped_traits)):
        ...
        if bucks_tracker is None:
            return
        bucks_tracker.try_modify_bucks((ClubTunables.CLUB_BUCKS_TYPE), (int(self.reward_bucks_per_interval * multiplier)),
          reason='Time in club gathering')

    def _sim_satisfies_requirement_for_bucks(self, sim):
        if not sim.is_selectable:
            return False
        else:
            return sim.sim_info.is_instanced() or False
        return True
    ...
    def _on_outfit_changed(self, sim_info, outfit_category_and_index):
        ...
        for cas_part in cas_parts_remove:
            modifier = AppearanceModifier.SetCASPart(cas_part=cas_part, should_toggle=True, replace_with_random=False,
              update_genetics=False,
              _is_combinable_with_same_type=True,
              remove_conflicting=False,
              outfit_type_compatibility=None,
    ...
    def _process_time_in_gathering_event(self, sim, now=None):
        ...
        services.get_event_manager().process_event((test_events.TestEvent.TimeInClubGathering), sim_info=(sim.sim_info),
          amount=(int(elapsed_time.in_minutes())))
    ...
    def _issue_requests(self):
        ...
        request = AssociatedClubRequestFactory(self, callback_data=(_RequestUserData()),
          job_type=(self._default_job),
...
lock_instance_tunables(ClubGatheringSituation, exclusivity=(BouncerExclusivityCategory.CLUB_GATHERING),
...
class ClubGatheringSituationState(SituationState):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._interaction_rule_status = defaultdict(lambda : {ClubRuleEncouragementStatus.ENCOURAGED: set(), 
         ClubRuleEncouragementStatus.DISCOURAGED: set()})
