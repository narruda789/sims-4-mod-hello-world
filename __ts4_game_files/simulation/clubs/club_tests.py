...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\clubs\club_tests.py
# Compiled at: 2018-11-15 18:08:57
# Size of source mod 2**32: 26958 bytes
...
class ClubGatheringTest(event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'club':TunableVariant(description='\n            Define the Club to run this test against.\n            ',
       locked_args={'use_club_from_resolver':CLUB_USE_ASSOCIATED, 
     ...
     'club_gathering_status':OptionalTunable(TunableVariant(description='\n            If enabled, require the associated Club to either have a Gathering\n            going on or not.\n            ',
       locked_args={'Gathering Exists':True, 
      'No Gathering Exists':False})), 
     'subject_club_gathering_status':OptionalTunable(TunableVariant(description='\n            If enabled, require the tuned "subject" to either be or not be in a\n            Club Gathering for the associated Club.\n            ',
       locked_args={'In Gathering':True, 
      'Not In Gathering':False})), 
     'target_club_gathering_status':OptionalTunable(TunableVariant(description='\n            If enabled, require the tuned "target" to either be or not be in a\n            Club Gathering for the associated Club.\n            ',
       locked_args={'In Gathering':True, 
      'Not In Gathering':False}))}
    ...
    def test_gathering(self, club_to_test, test_subjects, test_targets):
        ...
        if self.club_gathering_status is not None:
            ...
            if gathering is not None:
                if not self.club_gathering_status:
        ...
        if self.subject_club_gathering_status is not None:
            for subject in test_subjects:
                ...
                if not sim_inst is None:
                    if sim_inst not in gathering_members:
                        if self.subject_club_gathering_status:
                            return TestResult(False, 'Subject {} not in Gathering for Club {} but should be.', subject, club_to_test, tooltip=(self.tooltip))
                    if sim_inst is not None and sim_inst in gathering_members:
                        return self.subject_club_gathering_status or TestResult(False, "Subject {} in Gathering for Club {} but shouldn't be.", subject, club_to_test, tooltip=(self.tooltip))
        ...
        if self.target_club_gathering_status is not None:
            for target in test_targets:
                ...
                if not sim_inst is None:
                    if sim_inst not in gathering_members:
                        if self.target_club_gathering_status:
                            return TestResult(False, 'Target {} not in Gathering for Club {} but should be.', target, club_to_test, tooltip=(self.tooltip))
                    if sim_inst is not None and sim_inst in gathering_members:
                        return self.target_club_gathering_status or TestResult(False, "Target {} in Gathering for Club {} but shouldn't be.", target, club_to_test, tooltip=(self.tooltip))
    ...
    def __call__(self, test_subjects=None, test_targets=None, associated_clubs=None):
        ...
        if self.club == self.CLUB_USE_ASSOCIATED or self.club == self.CLUB_FROM_EVENT_DATA:
            ...
            if self.club == self.CLUB_USE_ANY:
                ...
                return any((self.test_gathering(club, test_subjects, test_targets) for club in clubs_to_test)) or TestResult(False, 'Subjects {} or Targets {} failed the club gatherings test for {}.', test_subjects, test_targets, clubs_to_test)
            return TestResult.TRUE
...
class ClubTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'subject':TunableEnumEntry(description='\n            The subject whose Club status to check.\n            ',
     ...
     'affordance_rule':OptionalTunable(description='\n            If set, then the affordance being tested (should one exist) must\n            satisfy this rule requirement.\n            ',
       ...
       locked_args={'is_encouraged':AFFORDANCE_RULE_ENCOURAGED, 
    ...
    def _club_test_enabled--- This code section failed: ---

 L. 361         0  LOAD_FAST                'self'
                2  LOAD_ATTR                recent_member_status
                4  LOAD_CONST               None
                6  COMPARE_OP               is-not
                8  POP_JUMP_IF_TRUE     62  'to 62'

 L. 362        10  LOAD_FAST                'self'
               12  LOAD_ATTR                room_for_new_members
               14  LOAD_CONST               None
               16  COMPARE_OP               is-not
               18  POP_JUMP_IF_TRUE     62  'to 62'

 L. 363        20  LOAD_FAST                'self'
               22  LOAD_ATTR                invite_only
               24  LOAD_CONST               None
               26  COMPARE_OP               is-not
               28  POP_JUMP_IF_TRUE     62  'to 62'

 L. 364        30  LOAD_FAST                'self'
               32  LOAD_ATTR                subject_passes_membership_criteria
               34  LOAD_CONST               None
               36  COMPARE_OP               is-not
               38  POP_JUMP_IF_TRUE     62  'to 62'

 L. 365        40  LOAD_FAST                'self'
               42  LOAD_ATTR                subject_relationship_with_leader
               44  LOAD_CONST               None
               46  COMPARE_OP               is-not
               48  POP_JUMP_IF_TRUE     62  'to 62'

 L. 366        50  LOAD_FAST                'self'
               52  LOAD_ATTR                required_sim_count
               54  POP_JUMP_IF_TRUE     62  'to 62'

 L. 367        56  LOAD_FAST                'self'
               58  LOAD_ATTR                affordance_rule
               60  POP_JUMP_IF_FALSE    66  'to 66'
             62_0  COME_FROM            54  '54'
             62_1  COME_FROM            48  '48'
             62_2  COME_FROM            38  '38'
             62_3  COME_FROM            28  '28'
             62_4  COME_FROM            18  '18'
             62_5  COME_FROM             8  '8'

 L. 368        62  LOAD_CONST               True
               64  RETURN_VALUE     
             66_0  COME_FROM            60  '60'

 L. 369        66  LOAD_CONST               False
               68  RETURN_VALUE     
               -1  RETURN_LAST      
        ...
        if self.club_status is None:
            ...
            if not clubs:
                if self.club_status == NOT_MEMBER or self.club_status == NOT_LEADER:
                    return TestResult.TRUE
                return TestResult(False, 'Subject {} is not a member or leader of any clubs', subject,
                  tooltip=(self.tooltip))
            passing_clubs = 0
            for club in clubs:
                in_members_list = subject in club.members
                is_leader = subject is club.leader
                if self.club_status == MEMBER:
                    if not in_members_list:
                        if self.pass_if_any_clubs_pass:
                            continue
                        return TestResult(False, 'Subject {} not a member of Club {} but should be.', subject,
                          club, tooltip=(self.tooltip))
                if self.club_status == NOT_MEMBER:
                    if in_members_list:
                        if self.pass_if_any_clubs_pass:
                            continue
                        return TestResult(False, "Subject {} is a member of Club {} but shouldn't be.", subject,
                          club, tooltip=(self.tooltip))
                if self.club_status == LEADER:
                    if not is_leader:
                        if self.pass_if_any_clubs_pass:
                            continue
                        return TestResult(False, 'Subject {} is not the leader of Club {} but should be.', subject,
                          club, tooltip=(self.tooltip))
                if self.club_status == NOT_LEADER:
                    if is_leader:
                        if self.pass_if_any_clubs_pass:
                            continue
                        return TestResult(False, "Subject {} is the leader of Club {} but shouldn't be.", subject,
                          club, tooltip=(self.tooltip))
                passing_clubs += 1

            if self.pass_if_any_clubs_pass and passing_clubs == 0:
                return TestResult(False, 'Subject {} not in any clubs that pass the criteria.', subject,
                  tooltip=(self.tooltip))
        return TestResult.TRUE
    ...
    def _test_club(self, subject, club, affordance_data=None):
        ...
        if self.recent_member_status is not None:
            ...
            if self.recent_member_status != is_recent_member:
                return TestResult(False, "Subject {}'s recent member status in {} is {}, but the required status is {}", subject,
                  club, is_recent_member, (self.recent_member_status), tooltip=(self.tooltip))
        if self.room_for_new_members is not None:
            club_has_room = len(club.members) < club.get_member_cap()
            if self.room_for_new_members:
                if not club_has_room:
                    return TestResult(False, 'Club {} has no room for new members but is required to.', club,
                      tooltip=(self.tooltip))
            if not self.room_for_new_members:
                if club_has_room:
                    return TestResult(False, 'Club {} has room for new members but is required not to.', club,
                      tooltip=(self.tooltip))
        if self.invite_only is not None:
            if club.invite_only != self.invite_only:
                return TestResult(False, "Club {}'s invite_only status is expected to be {} but isn't.", club,
                  (self.invite_only), tooltip=(self.tooltip))
        if self.subject_passes_membership_criteria is not None:
            subject_result = club.validate_sim_info(subject)
            if subject_result:
                if not self.subject_passes_membership_criteria:
                    return TestResult(False, 'Subject {} passes the membership criteria for Club {} but is required not to.', subject,
                      club, tooltip=(self.tooltip))
            if not subject_result:
                if self.subject_passes_membership_criteria:
                    return TestResult(False, 'Subject {} does not pass the membership criteria for Club {} but is required to.', subject,
                      club, tooltip=(self.tooltip))
        if self.subject_relationship_with_leader is not None:
            if subject is club.leader:
                return TestResult(False, 'Subject {} requires relationship with the leader, but is the leader of Club {}', subject,
                  club, tooltip=(self.tooltip))
        ...
        if self.required_sim_count is not None:
            if not self.required_sim_count.compare(len(club.members)):
                return TestResult(False, "The club {} doesn't meet the required sim count of {}", club, (self.required_sim_count),
                  tooltip=(self.tooltip))
        if self.affordance_rule is not None:
            if club_service is None:
                return TestResult(False, 'Affordance {} does not satisfy the required Club rules requirements. There is no club service.', (affordance_data.affordance),
                  tooltip=(self.tooltip))
                ...
                if self.affordance_rule == self.AFFORDANCE_RULE_ENCOURAGED:
                    if rule_status != ClubRuleEncouragementStatus.ENCOURAGED:
                        return TestResult(False, 'Affordance {} does not satisfy the required Club rules requirements', (affordance_data.affordance),
                          tooltip=(self.tooltip))
                ...
                if rule_status == ClubRuleEncouragementStatus.ENCOURAGED:
                    return TestResult(False, 'Affordance {} does not satisfy the required Club rules requirements', (affordance_data.affordance),
                      tooltip=(self.tooltip))
            elif self.affordance_rule == self.AFFORDANCE_RULE_NOT_DISCOURAGED:
                if rule_status == ClubRuleEncouragementStatus.DISCOURAGED:
                    return TestResult(False, 'Affordance {} does not satisfy the required Club rules requirements', (affordance_data.affordance),
                      tooltip=(self.tooltip))
        return TestResult.TRUE
    ...
    def __call__(self, test_subjects=(), associated_clubs=(), common_test_subjects=(), affordance=None, affordance_targets=()):
        ...
        for subject in test_subjects:
            if self.subject_can_join_more_clubs is not None:
                ...
                if can_join_new_club:
                    if not self.subject_can_join_more_clubs:
                        return TestResult(False, "Subject {} is allowed to join more Clubs but shouldn't be.", subject,
                          tooltip=(self.tooltip))
                if not can_join_new_club:
                    if self.subject_can_join_more_clubs:
                        return TestResult(False, 'Subject {} is not allowed to join more Clubs but should be.', subject,
                          tooltip=(self.tooltip))
                ...
                if self._club_test_enabled():
                    if common_test_subjects:
                        ...
                        if not set(clubs) & common_test_clubs:
                            return TestResult(False, "Subject {} and {} don't share an appropriate common Club", subject,
                              common_test_subjects, tooltip=(self.tooltip))
                    affordance_data = self._AffordanceData(affordance, next(iter(affordance_targets), None))
                    return any((self._test_club(subject, club, affordance_data=affordance_data) for club in clubs)) or TestResult(False, 'Subject {} fails Club test for {}', subject,
                      clubs, tooltip=(self.tooltip))
