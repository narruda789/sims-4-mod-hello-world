...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\apartments\landlord_service.py
# Compiled at: 2020-06-08 21:15:28
...
class LandlordService(Service):
    ...
    def get_landlord_sim_info(self):
        ...
        if self._landlord_id is not None:
            if services.sim_filter_service().does_sim_match_filter((self._landlord_id), sim_filter=landlord_filter, gsi_source_fn=(self.get_sim_filter_gsi_name)):
                return services.sim_info_manager().get(self._landlord_id)
            self._landlord_id = None
        else:
            landlords = services.sim_filter_service().submit_matching_filter(sim_filter=landlord_filter, number_of_sims_to_find=1,
    ...
    def get_existing_landlord_sim_info(self):
        if self._landlord_id is not None:
            ...
            if landlord_filter is None:
                return
                landlords = services.sim_filter_service().submit_filter(sim_filter=landlord_filter, callback=None,
                  allow_yielding=False,
                  gsi_source_fn=(self.get_sim_filter_gsi_name))
                if landlords:
                    landlord_sim_info = landlords[0].sim_info
    ...
    def on_loading_screen_animation_finished(self):
        ...
        if household is not None:
            if not household.has_home_zone_been_active():
                if plex_service.is_zone_an_apartment((household.home_zone_id), consider_penthouse_an_apartment=False):
                    active_sim = services.get_active_sim()
                    landlord_sim_info = self.get_landlord_sim_info()
                    if active_sim is not None:
                        if landlord_sim_info is not None:
