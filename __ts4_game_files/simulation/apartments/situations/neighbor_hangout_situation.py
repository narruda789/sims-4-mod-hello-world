...
class NeighborHangoutSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'player_sim_job_and_default_role_state':TunableSituationJobAndRoleState(description='\n            The Situation Job and role stateto put player Sims in. \n            '), 
     ...
     '_wait_to_be_greeted_state':_NeighborWaitToBeGreetedState.TunableFactory(description='\n            The state for the neighbor to wait until the player invites them in\n            or they timeout.\n            ',
       tuning_group=GroupNames.STATE), 
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _RingDoorbellState, factory=(cls._ring_doorbell_state)),
         SituationStateData(2, _NeighborWaitToBeGreetedState, factory=(cls._wait_to_be_greeted_state)),
         SituationStateData(3, _NeighborHangoutState, factory=(cls._hangout_state)))
    ...
    def situation_meets_starting_requirements(cls, **kwargs):
        ...
        if not owning_household is None:
            return owning_household.get_sims_at_home() or False
        else:
            neighbor_results = cls.get_filter_results_for_job()
            return neighbor_results or False
    ...
    def get_predefined_guest_list(cls):
        ...
        guest_list = SituationGuestList(invite_only=True, host_sim_id=(neighbor.sim_info.sim_id),
...
lock_instance_tunables(NeighborHangoutSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
