...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\apartments\situations\neighbor_loud_common.py
# Compiled at: 2018-07-22 17:17:14
# Size of source mod 2**32: 16047 bytes
import random
...
import clock, filters, services, sims4.log
...
class _LoudNeighborState(CommonInteractionCompletedSituationState):
    ...
    def on_activate(self, reader=None):
        ...
        self._create_or_load_alarm((_LoudNeighborState.LOOT_ACTION_DELAY), (self._loot_actions_on_situation_start.delay),
          (lambda _: self._apply_loud_loot_actions()),
          should_persist=True,
          reader=reader)
...
class _ComplainState(CommonInteractionStartedSituationState):
    ...
    def _create_neighbor_response_situation(self):
        ...
        self.neighbor_response_situation_id = situation_manager.create_situation((self._neighbor_situation), guest_list=guest_list,
          user_facing=False)

    def _set_loud_door_state_off(self):
        if self._turn_off_loud_door_state:
            if self.owner._neighbor_door_id is not None:
                apartment_door = services.object_manager().get(self.owner._neighbor_door_id)
...
class LoudNeighborSituation(SituationComplexCommon):
    ...
    @classmethod
    def _states(cls):
        return (SituationStateData(1, _LoudNeighborState, factory=(cls.loud_neighbor_state)),
         SituationStateData(2, _ComplainState, factory=(cls.complain_state)))
    ...
    def _get_loud_neighbor(cls):
        ...
        for neighbor_filter in loud_neighbor_filters:
            neighbors = services.sim_filter_service().submit_matching_filter(sim_filter=neighbor_filter, allow_yielding=False,
            ...
            if not len(neighbor_sim_infos_at_home) > 1:
                if len(neighbor_sim_infos_at_home):
...
lock_instance_tunables(LoudNeighborSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
