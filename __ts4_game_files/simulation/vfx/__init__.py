...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\vfx\__init__.py
# Compiled at: 2019-06-17 14:36:15
# Size of source mod 2**32: 12428 bytes
...
class PlayEffect(distributor.ops.ElementDistributionOpMixin, HasTunableFactory, AutoFactoryInit):
    ...
    def __init__(self, target, effect_name='', joint_name=0, target_actor_id=0, target_joint_name_hash=0, mirror_effect=False, auto_on_effect=False, target_joint_offset=None, play_immediate=False, callback_event_id=None, store_target_position=False, transform_override=None, **kwargs):
        (super().__init__)(effect_name=effect_name, joint_name=joint_name, play_immediate=play_immediate, 
         immediate=play_immediate, **kwargs)
        self.target = target
    ...
    def start(self, *_, **__):
        if self.target is None:
            ...
            if self._is_relative_to_transform:
                ...
                return self._is_valid_target() or None
            self.is_attached or self.attach(self.target)
            logger.info('VFX {} on {} START'.format(self.effect_name, self.target))

    def start_one_shot(self):
        if self.target is not None:
            self.target.is_terrain or distributor.ops.record(self.target, self)
        else:
            Distributor.instance().add_op_with_no_owner(self)

    def stop(self, *_, immediate=False, **kwargs):
        return self.target is None or self.target.valid_for_distribution or None
        if self._is_relative_to_transform:
        ...
        if not self.target.valid_for_distribution:
            ...
            if zone is not None:
                ...
                if zone_spin_up_service is None:
                    logger.callstack('zone_spin_up_service was None in PlayEffect._is_valid_target(), for effect/target: {}/{}', self,
                      (self.target), owner='johnwilkinson', level=(sims4.log.LEVEL_ERROR))
                    return False
