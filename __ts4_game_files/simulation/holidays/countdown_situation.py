...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\holidays\countdown_situation.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 7391 bytes
...
class CountdownState(CommonSituationState):
    FACTORY_TUNABLES = {'countdown_affordance':TunableReference(manager=services.get_instance_manager(sims4.resources.Types.INTERACTION)), 
     'count_mixer':TunableReference(manager=services.get_instance_manager(sims4.resources.Types.INTERACTION)), 
     'celebrate_time':TunableTimeOfDay(description='\n            Time of Day to Celebrate\n            ',
       default_hour=0), 
     'time_to_start_count':TunableTimeOfDay(description='\n            Time to start performing the Count.\n            ',
       default_hour=11,
       default_minute=30), 
     'interval_between_counts':TunableSimMinute(description='\n            The interval between each count animation.\n            ',
       minimum=1,
       default=5)}
    ...
    def on_activate(self, reader=None):
        ...
        self._count_timer = alarms.add_alarm(self, time_till_first_count,
...
class CountdownSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'celebrate_state':CelebrateState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':None}), 
     'countdown_state':CountdownState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':None})}
    REMOVE_INSTANCE_TUNABLES = Situation.NON_USER_FACING_REMOVE_INSTANCE_TUNABLES

    @classmethod
    def _states(cls):
        return (SituationStateData(1, CountdownState, factory=(cls.countdown_state)),
         SituationStateData(2, CelebrateState, factory=(cls.celebrate_state)))
