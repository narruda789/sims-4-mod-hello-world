...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\ui\ui_dialog_service.py
# Compiled at: 2021-06-04 20:10:05
...
import alarms, clock, persistence_error_types, services
...
class UiDialogService(Service):
    ...
    def dialog_pick_result(self, dialog_id: int, picked_results=[], ingredient_check=None) -> bool:
        dialog = self._active_dialogs.get(dialog_id, None)
        if dialog is not None:
            if dialog.pick_results(picked_results=picked_results, ingredient_check=ingredient_check):
                return True
        return False
    ...
    def dialog_pick_result_def_ids_and_counts(self, dialog_id: int, picked_def_ids=[], obj_ids=[], picked_counts=[], ingredient_check=None) -> bool:
        dialog = self._active_dialogs.get(dialog_id, None)
        if dialog is not None:
            if dialog.pick_definitions_ids_and_counts(picked_def_ids, obj_ids, picked_counts, ingredient_check):
                return True
        return False
    ...
    def dialog_text_input(self, dialog_id: int, text_input_name, text_input_value) -> bool:
        dialog = self._active_dialogs.get(dialog_id, None)
        if dialog is not None:
            if dialog.on_text_input(text_input_name, text_input_value):
                return True
        return False
