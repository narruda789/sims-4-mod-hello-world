...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\buffs\buff.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 76764 bytes
...
import enum, event_testing.tests, interactions.base.mixer_interaction, services, sims4.log, sims4.resources, statistics.static_commodity, tag, topics.topic
...
class Buff(HasTunableReference, RouteEventProviderMixin, SuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin, MixerActorMixin, MixerProviderMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.BUFF)):
    INSTANCE_TUNABLES = {'buff_name':TunableLocalizedStringFactory(description='\n        Name of buff.\n        ',
     ...
     'buff_description':TunableLocalizedString(description='\n        Tooltip description of the Buff Effect.\n        ',
       allow_none=True,
       tuning_group=GroupNames.UI,
       export_modes=ExportModes.All), 
     ...
     'visible':Tunable(description='\n        Whether this buff should be visible in the UI.\n        ',
       ...
       default=True,
       tuning_group=GroupNames.UI), 
     'audio_sting_on_remove':TunableResourceKey(description='\n        The sound to play when this buff is removed.\n        ',
       default=None,
       resource_types=(
     ...
     'proximity_detection_tests':OptionalTunable(description="\n        Whether or not this buff should be added because of a Sim's proximity\n        to an object with a Proximity Component with this buff in its buffs\n        list.\n        ",
       ...
       disabled_name='no_proximity_detection',
       enabled_name='proximity_tests'), 
     'proximity_buff_added_reason':OptionalTunable(tunable=TunableLocalizedString(description="\n            If this is a proximity buff, this field will be the reason for why\n            the Sim received this buff. Doesn't use tokens.\n            "),
       enabled_by_default=False,
       disabled_name='no_proximity_add_reason',
       enabled_name='proximity_add_reason'), 
     '_add_test_set':OptionalTunable(description='\n        Whether or not this buff should be added.\n        ',
     ...
     'tags':TunableSet(description='\n        A list of tags associated with this buff.\n        ',
      ...
      tag.Tag.INVALID,),
       ...
       pack_safe=True)), 
     'motive_panel_overlays':TunableMapping(description='\n        Allows tuning an overlay on the motives panel.\n        ',
       key_name='Overlay Type',
       key_type=MotiveOverlayType,
       value_name='Linked Motive',
    ...
    def _verify_tuning_callback(cls):
        if cls.visible:
            if not cls.mood_type:
                logger.error('No mood type set for visible buff: {}.  Either provide a mood or make buff invisible.', cls, owner='Tuning')
        if cls._ignore_weather:
            if cls._temporary_commodity_info:
                if cls._temporary_commodity_info.persists:
    ...
    def _tuning_loaded_callback(cls):
        if cls._temporary_commodity_info is not None:
            if cls._owning_commodity is None:
                ...
                if issubclass(cls._owning_commodity, RuntimeCommodity):
    ...
    def __init__(self, owner, commodity_guid, replacing_buff_type, transition_into_buff_id, additional_static_commodities_to_add=None, remove_on_zone_unload=True):
        ...
        if self._ignore_weather:
            if not self._remove_on_zone_unload:
    ...
    @classproperty
    def add_test_set(cls):
        return cls._add_test_set
    ...
    def add_owning_commodity(cls, commodity):
        if cls._owning_commodity is None:
            ...
            if cls._operating_commodity is None:
                if cls._owning_commodity is not commodity:
    ...
    def commodity(cls, inst):
        if inst is not None:
            if inst._replacing_buff_type is not None:
    ...
    def _create_temporary_commodity(cls, proxied_commodity=None, create_buff_state=True, initial_value=DEFAULT):
        ...
        if create_buff_state:
            ...
            proxied_commodity.commodity_states = [
    ...
    @classmethod
    def has_tag(cls, tag):
        if not cls.tags or tag is None:
            return False
        return tag in cls.tags

    @classmethod
    def has_any_tag(cls, tags):
        return cls.tags and tags or False
    ...
    def on_add(self, from_load=False, apply_buff_loot=True):
        ...
        if sim is not None:
            ...
            if self.route_events is not None and not self.route_events.first_indoor:
        ...
        if self.appearance_modifier is not None:
            self._owner.appearance_tracker.add_appearance_modifiers((self.appearance_modifier.appearance_modifiers), (self.buff_type.guid),
              (self.appearance_modifier.priority),
              (self.appearance_modifier.apply_to_all_outfits),
              (self.appearance_modifier.additional_flags),
              source=(self.buff_type.__name__))
        ...
        if self._static_commodites_added:
            for static_commodity_type in self._static_commodites_added:
                ...
                if tracker is not None:
                    tracker.has_statistic(static_commodity_type) or tracker.add_statistic(static_commodity_type)
    ...
    def _start_providing_template_affordances(self):
        ...
        if template_affordance_tracker is not None:
    ...
    def provide_route_events(self, route_event_context, sim, path, failed_types=None, **kwargs):
        ...
        for route_event_cls in self.route_events.single_events:
            if self.can_provide_route_event(route_event_cls, failed_types, resolver):
                route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_context.route_event_already_scheduled(route_event_cls) or route_event_context.add_route_event(RouteEventType.LOW_SINGLE, route_event_cls(provider=self, provider_required=True))
        ...
        for route_event_cls in self.route_events.first_indoor:
            if self.can_provide_route_event(route_event_cls, failed_types, resolver):
                route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_context.add_route_event(RouteEventType.FIRST_INDOOR, route_event_cls(provider=self, provider_required=True))

        for route_event_cls in self.route_events.last_indoor:
            if self.can_provide_route_event(route_event_cls, failed_types, resolver):
                route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_context.add_route_event(RouteEventType.LAST_INDOOR, route_event_cls(provider=self, provider_required=True))
            ...
            if node is not None:
                ...
                if lot_level is not None:
                    ...
                    for route_event_data in self.route_events.on_level_change_indoors:
                        ...
                        if not self.can_provide_route_event(route_event_cls, failed_types, resolver):
                            if not route_event_context.route_event_already_fully_considered(route_event_cls, self):
                                continue
