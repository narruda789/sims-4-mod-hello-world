...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\buffs\appearance_modifier\appearance_tracker.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 24857 bytes
...
class AppearanceTracker:
    ...
    def add_appearance_modifier(self, modifier, guid, priority, apply_to_all_outfits, additional_flags=OutfitOverrideOptionFlags.DEFAULT, source=None, archive_single_entry=True):
        ...
        if archive_single_entry:
            if appearance_modifier_handlers.archiver.enabled:
                appearance_modifier_handlers.add_appearance_modifier_data(self._sim_info, (modifier,), priority, apply_to_all_outfits, additional_flags, source, modifier)

    def add_appearance_modifiers(self, appearance_modifiers_list, guid, priority, apply_to_all_outfits, additional_flags=OutfitOverrideOptionFlags.DEFAULT, source=None):
        permanent_modifiers = []
        ...
        for appearance_modifiers in appearance_modifiers_list:
            ...
            if modifier.is_permanent_modification:
                ...
                continue
            self.add_appearance_modifier(modifier, guid, priority, apply_to_all_outfits,
              additional_flags, archive_single_entry=False)
    ...
    def remove_appearance_modifiers(self, guid, source=None):
        ...
        for mod_type, appearance_modifier_list in self._active_appearance_modifier_infos.items():
            ...
            if modifier_infos_to_remove:
                ...
                self._active_appearance_modifier_infos[mod_type] or keys_to_remove.append(mod_type)
    ...
    def _apply_sim_info_override(self, original_sim_info, modified_sim_info, resulting_sim_info, outfit_category, outfit_index, apply_to_all_outfits, additional_flags, body_type_flags_to_set):
    ...
    def apply_appearance_modifiers(self, appearance_modifier_infos, original_sim_info, is_permanent_modification=False):
        ...
        modified_sim_info = sim_info_base_wrapper.SimInfoBaseWrapper(gender=(original_sim_info.gender), age=(original_sim_info.age),
            ...
            for modifier_info in appearance_modifier_infos:
                if modifier_info.should_display is False:
                    ...
                    if apply_to_all_outfits is None:
                        ...
                        if apply_to_all_outfits != modifier_info.apply_to_all_outfits:
                            logger.error('Sim: {} has a mix of appearance modifiers that have Apply to All Outfits tuned to true and false.This is not supported, so all modifiers will be applied to all outfits as a fallback.', original_sim_info,
                              owner='rrodgers')
                if additional_flags is None:
                    ...
                    if additional_flags != modifier_info.additional_flags:
                        logger.error('Sim: {} has multiple appearance modifiers with different flags set. This is not supported and all set flags will be applied for all appearance modifiers as a fallback.', original_sim_info,
                          owner='rrodgers')
        ...
        if is_permanent_modification:
            self._apply_sim_info_override(original_sim_info, modified_sim_info, original_sim_info, outfit_category, outfit_index, apply_to_all_outfits, additional_flags, body_type_flags_to_set)
        else:
            appearance_override_sim_info = sim_info_base_wrapper.SimInfoBaseWrapper(sim_id=(original_sim_info.sim_id))
            self._apply_sim_info_override(original_sim_info, modified_sim_info, appearance_override_sim_info, outfit_category, outfit_index, apply_to_all_outfits, additional_flags, body_type_flags_to_set)
