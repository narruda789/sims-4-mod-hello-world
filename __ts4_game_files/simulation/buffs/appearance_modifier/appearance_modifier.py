...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\buffs\appearance_modifier\appearance_modifier.py
# Compiled at: 2022-06-13 18:18:17
# Size of source mod 2**32: 30883 bytes
from buffs.appearance_modifier.appearance_modifier_type import AppearanceModifierType
from cas.cas import set_caspart, get_caspart_bodytype, randomize_part_color, randomize_skintone_from_tags, randomize_caspart, OutfitOverrideOptionFlags
...
from sims4.tuning.tunable import HasTunableSingletonFactory, AutoFactoryInit, TunableCasPart, TunableEnumEntry, TunableList, TunableVariant, Tunable, OptionalTunable, TunableTuple, TunableSet, TunableMapping, TunableEnumFlags, TunableEnumSet
...
class AppearanceModifier(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    class ReplaceCASPart(BaseAppearanceModification):
        ...
        def _verify_tunable_callback(instance_class, tunable_name, source, value, **kwargs):
            if len(value.replace_part_map) == 0:
                if value.default_set_part is None:
        ...
        def modify_sim_info(self, source_sim_info, modified_sim_info, random_seed):
            ...
            if set_caspart((source_sim_info._base), (modified_sim_info._base), part_to_set,
              False, False,
              (self.update_genetics), random_seed,
    ...
    class RandomizeSkintoneFromTags(BaseAppearanceModification):
        FACTORY_TUNABLES = {'tag_list':TunableList(TunableEnumEntry(description='\n                    A specific tag.\n                    ',
         ...
         'locked_args':{'_is_combinable_with_same_type': False}}
    ...
    @staticmethod
    def _verify_tunable_callback(instance_class, tunable_name, source, value, **kwargs):
        if not value.apply_to_all_outfits:
            if value.additional_flags & OutfitOverrideOptionFlags.IGNORE_BATHING:
        ...
        for tuned_modifiers in value.appearance_modifiers:
            if len(tuned_modifiers) == 1:
                if tuned_modifiers[0].weight.base_value != 1:
                    logger.error('An appearance modifier has only one entry\n                                    in the list of modifiers and the weight of\n                                    that modifier is != 0. Instead it is {}',
                      (tuned_modifiers[0].weight.base_value), owner='rfleig')
    ...
    FACTORY_TUNABLES = {'priority':TunableEnumEntry(description='\n            The priority of the appearance request. Higher priority will\n            take precedence over lower priority. Equal priority will favor\n            recent requests.\n            ',
     ...
     'appearance_modifiers':TunableList(description='\n            The specific appearance modifiers to use for this buff.\n            ',
       ...
       modifier=TunableVariant(set_cas_part=(SetCASPart.TunableFactory()),
       replace_cas_part=(ReplaceCASPart.TunableFactory()),
       randomize_cas_part=(RandomizeCASPart.TunableFactory()),
       randomize_body_type_color=(RandomizeBodyTypeColor.TunableFactory()),
       randomize_skintone_between_tags=(RandomizeSkintoneFromTags.TunableFactory()),
     ...
     'apply_to_all_outfits':Tunable(description='\n            If checked, the appearance modifiers will be applied to all outfits,\n            otherwise they will only be applied to the current outfit.\n            ',
       tunable_type=bool,
       default=True), 
     'additional_flags':TunableEnumFlags(description='\n            Additional flags that will be sent as part of this outfit override.\n            Note, ignore_bathing is only valid if apply_to_all_outfits is also\n            checked.\n            ',
       enum_type=OutfitOverrideOptionFlags,
       allow_no_flags=True,
       invalid_enums=(
      OutfitOverrideOptionFlags.OVERRIDE_ALL_OUTFITS,
      OutfitOverrideOptionFlags.MANNEQUIN_MODE,
      OutfitOverrideOptionFlags.APPLY_MODIFIER_VARIATION,
      OutfitOverrideOptionFlags.FROM_SCRATCH,
      OutfitOverrideOptionFlags.APPLY_GENETICS_FROM_OVERRIDE,
      OutfitOverrideOptionFlags.OVERRIDE_CUSTOM_TEXTURES,
      OutfitOverrideOptionFlags.OVERRIDE_HAIR_MATCH_FLAGS)), 
     'verify_tunable_callback':_verify_tunable_callback}