...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\buffs\tunable.py
...
import event_testing, services, sims4.log
...
class TunableBuffReference(TunableSingletonFactory):
    ...
    def __init__(self, reload_dependent=False, pack_safe=False, allow_none=False, **kwargs):
        (super().__init__)(buff_type=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.BUFF)), description='Buff that will get added to sim.',
...
class TunablePackSafeBuffReference(TunableSingletonFactory):
    ...
    def __init__(self, reload_dependent=False, allow_none=False, **kwargs):
        (super().__init__)(buff_type=TunablePackSafeReference(manager=(services.get_instance_manager(sims4.resources.Types.BUFF)), description='Buff that will get added to sim.',
  allow_none=allow_none,
  reload_dependent=reload_dependent), 
         buff_reason=OptionalTunable(description='\n                            If set, specify a reason why the buff was added.\n                            ',
  tunable=TunableLocalizedString(description='\n                                The reason the buff was added. This will be displayed in the\n                                buff tooltip.\n                                ')), **kwargs)
...
class TunableBuffElement(TunableFactory):
    ...
    def __init__(self, description='A buff that will get added to the subject when running the interaction if the tests succeeds.', **kwargs):
        (super().__init__)(subject=TunableEnumFlags(ParticipantType, (ParticipantType.Actor), description='Who will receive the buff.'), tests=event_testing.tests.TunableTestSet(), 
         buff_type=TunablePackSafeBuffReference(description='The buff type to be added to the Sim.'), 
         verify_tunable_callback=TunableBuffElement._verify_tunable_callback, 
