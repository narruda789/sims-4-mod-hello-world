...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\celebrity_fans\fan_situation_manager.py
# Compiled at: 2019-01-17 20:55:12
# Size of source mod 2**32: 14928 bytes
...
class FanSituationManager(HasTunableFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'fan_situation_interval':TunableSimMinute(description='\n            The amount of time, in Sim minutes, between attempts to create new\n            fan/stan situations.\n            ',
     ...
     'fan_situations':TunableVariant(description='\n            Situations to choose from when generating a fan situation.\n            ',
       situation_curve=SituationCurve.TunableFactory(get_create_params={'user_facing': False}),
       shiftless=ShiftlessDesiredSituations.TunableFactory(get_create_params={'user_facing': False}),
       default='shiftless'), 
     'stan_situation':TunablePackSafeReference(description='\n            Stan situation.  A stan situation is tied to a particular Sim\n            via a relationship bit defined in FanTuning.\n            ',
       manager=services.get_instance_manager(Types.SITUATION),
       class_restrictions='StanSituation')}
    ...
    def _create_fan_creation_alarm(self):
        self._fan_situation_alarm_handle = alarms.add_alarm(self, create_time_span(minutes=(self.fan_situation_interval)),
          (self._on_fan_creation_alarm),
          repeating=True)
    ...
    def _try_spawn_random_fan_situation(self):
        ...
        if situation_type is not None:
            situation_id = situation_manager.create_situation(situation_type, user_facing=False,
              creation_source='FanSituationManager: Fan Situation')
    ...
    def _try_spawn_stan_situation_for_sim(self, stanned_sim):
        if self.stan_situation is None:
            ...
            stan_results = services.sim_filter_service().submit_matching_filter(sim_filter=(FanTuning.STAN_FILTER), number_of_sims_to_find=1,
              requesting_sim_info=(stanned_sim.sim_info),
              allow_instanced_sims=True,
              allow_yielding=False,
              gsi_source_fn=(lambda : 'FanSituationManager: Stan for {}'.format(str(stanned_sim))))
            if not stan_results:
            ...
            if not in_stan_home_zone:
                if FanTuning.STAN_DISABLING_BITS & set(services.relationship_service().get_all_bits(stan_id, target_sim_id=(stanned_sim.sim_id))):
                    return
        ...
        situation_id = situation_manager.create_situation((self.stan_situation), guest_list=guest_list,
          user_facing=False,
          duration_override=duration_override,
          creation_source=('FanSituationManager: Stan Situation targetting {}'.format(str(stanned_sim))))
