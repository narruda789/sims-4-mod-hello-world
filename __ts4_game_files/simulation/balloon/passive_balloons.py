...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\balloon\passive_balloons.py
# Compiled at: 2018-03-01 17:13:11
...
import gsi_handlers, services, sims4.log
...
class PassiveBalloons:
    ...
    ROUTING_BALLOONS = TunableList(description='\n        A weighted list of passive routing balloons.\n        ',
      tunable=TunableTuple(balloon=TunableBalloon(locked_args={'balloon_delay':0,  'balloon_delay_random_offset':0,  'balloon_chance':100, 
     'balloon_target':None}),
    ...
    def request_routing_to_object_balloon(sim, interaction):
        ...
        if interaction.is_user_directed:
            if not balloon_tuning.also_show_user_directed:
                return
    ...
    def create_passive_ballon_request(sim, balloon_data):
        ...
        if balloon_icon is not None:
            if icon_info[0] is not None or icon_info[1] is not None:
