...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\narrative\narrative_handlers.py
# Compiled at: 2019-03-18 20:19:49
...
def generate_narrative_view():
    ...
    for narrative in narrative_tuning_manager.types.values():
        narratives.append({'narrative':str(narrative.__name__), 
         'groups':', '.join((group.name for group in narrative.narrative_groups)), 
         'active':narrative in narrative_service.active_narratives, 
         'previously_completed':narrative in narrative_service.completed_narratives, 
         'Linked Narratives':list(({'event':str(event.name),  'narrative':str(link.__name__)} for event, link in narrative.narrative_links.items()))})
...
def generate_narrative_links_view():
    ...
    for narrative_cls, narrative_instance in narrative_service.get_active_narrative_instances():
        for event, linked_narrative in narrative_cls.narrative_links.items():
            narrative_links.append({'event':str(event.name), 
             'narrative':str(narrative_cls.__name__), 
             'narrative_link':str(linked_narrative.__name__)})
        ...
        for event, progression_value in narrative_instance._narrative_progression.items():
            ...
            narrative_links.append({'event':str(event.name), 
             'progression_value':str(progression_value), 
             'above_link':None if narrative_threshold_link.above_link is None else str(narrative_threshold_link.above_link.__name__), 
             'above_threshold':None if narrative_threshold_link.above_link is None else str(narrative_threshold_link.interval.upper_bound), 
             'below_link':None if narrative_threshold_link.below_link is None else str(narrative_threshold_link.below_link.__name__), 
             'below_threshold':None if narrative_threshold_link.below_link is None else str(narrative_threshold_link.interval.lower_bound)})
