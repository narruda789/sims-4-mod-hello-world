...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\doors\door_service.py
# Compiled at: 2020-10-06 02:55:08
# Size of source mod 2**32: 23341 bytes
...
import persistence_error_types, routing, services, sims4.log
...
class DoorService(Service):
    ...
    def fix_up_doors(self, force_refresh=False):
        ...
        if not building_type == PlexBuildingType.DEFAULT:
            if building_type == PlexBuildingType.PENTHOUSE_PLEX or building_type == PlexBuildingType.COASTAL:
    ...
    def set_as_front_door(self, door):
        if door is None:
            if self._front_door_id is None:
                return
        if door is not None:
            if self._front_door_id == door.id:
                return
    ...
    def _choose_front_door(self, exterior_door_infos, preferred_door_id=None):
        if not self._zone_requires_front_door():
            ...
            return exterior_door_infos or None
    ...
    def _object_is_door(self, obj):
        if not isinstance(obj, Door):
            ...
            return obj.is_door_portal or False
        return True
    ...
    def _get_exterior_and_interior_doors(self):
        ...
        return (
         frozenset(exterior_door_to_infos.values()), interior_doors)
    ...
    def load(self, zone_data=None):
        ...
        if zone_data is not None:
            if zone_data.HasField('front_door_id'):
