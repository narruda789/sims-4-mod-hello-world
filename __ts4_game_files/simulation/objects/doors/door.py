...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\doors\door.py
# Compiled at: 2019-08-16 14:15:06
...
class Door(GameObject):
    ...
    def get_door_positions(self):
        ...
        return (front_location.position if front_location is not None else None,
         back_location.position if back_location is not None else None)
    ...
    def get_gsi_portal_items(self, key_name, value_name):
        ...
        if self.has_state(DoorTuning.FRONT_DOOR_STATE.enabled.state):
            door_items_list.append({key_name: 'Front Door State', 
             value_name: str(self.get_state(DoorTuning.FRONT_DOOR_STATE.enabled.state))})
        if self.has_state(DoorTuning.FRONT_DOOR_AVAILABILITY_STATE.enabled.state):
            door_items_list.append({key_name: 'Front Door Availability State', 
             value_name: str(self.get_state(DoorTuning.FRONT_DOOR_AVAILABILITY_STATE.enabled.state))})
        if self.has_state(DoorTuning.INACTIVE_APARTMENT_DOOR_STATE.enabled.state):
            door_items_list.append({key_name: 'Inactive Apartment Door State', 
             value_name: str(self.get_state(DoorTuning.INACTIVE_APARTMENT_DOOR_STATE.enabled.state))})
