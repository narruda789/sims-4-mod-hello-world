...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\definition.py
# Compiled at: 2020-02-13 18:44:33
# Size of source mod 2**32: 17124 bytes
...
@total_ordering
class Definition:
    __slots__ = ('_name', 'id', '_components', '_icon', '_model', '_material_variant',
                 '_thumbnail_geo_state_hash', '_rig', '_footprint', '_tuning_file_id',
                 '_slot', '_slots_resource', '_catalog_price', '_environment_mood_tags',
                 '_environment_scores', '_negative_environment_score', '_positive_environment_score',
                 'build_buy_tags', '_cls')
    ...
    def __eq__(self, other):
        if type(other) != type(self):
            return False
        return (
         self.id, self.cls.__name__) == (
         other.id, other.cls.__name__)
    ...
    def _safe_index(array, index):
        if not array:
            ...
            return isinstance(array, tuple) or array
        if len(array) == 1:
            return array[0]
        return array[index]
    ...
    def assign_build_buy_tags(self):
        ...
        if tags is not None:
            intern_service = services.get_intern_service()
            self.build_buy_tags = intern_service.intern(tags)
    ...
    def is_similar(self, new_definition, ignore_rig_footprint=False):
        if self.tuning_file_id != new_definition.tuning_file_id:
            return (
             False, 'definition.is_similar: tuning_file_id: {} != tuning_file_id: {}'.format(self.tuning_file_id, new_definition.tuning_file_id))
        else:
            if self._components != new_definition._components:
                return (
                 False, 'definition.is_similar: components: {} != components: {}'.format(self._components, new_definition._components))
            if not ignore_rig_footprint:
                if self._rig != new_definition._rig:
                    return (
                     False, 'definition.is_similar: rig: {} != rig: {}'.format(self._rig, new_definition._rig))
                if self._footprint != new_definition._footprint:
                    return (
                     False, 'definition.is_similar: footprint: {} != footprint: {}'.format(self._footprint, new_definition._footprint))
        return (True, True)