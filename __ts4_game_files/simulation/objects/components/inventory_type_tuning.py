...
class InventoryTypeTuning:
    ...
    def _verify_tuning_callback(cls):
        for inventory_type in set(InventoryType) - set(cls.INVENTORY_TYPE_DATA.keys()):
            logger.error('Inventory type {} has no tuned inventory type data. This can be fixed in the tuning for objects.components.inventory_enum.tuning -> InventoryTypeTuning -> Inventory Type Data.', (inventory_type.name),
              owner='bhill')
    ...
    def is_shared_between_objects(inventory_type):
        ...
        if tuning.shared_between_objects == ObjectShareability.NOT_SHARED:
            return False
        if tuning.shared_between_objects == ObjectShareability.SHARED_IF_NOT_IN_APARTMENT:
