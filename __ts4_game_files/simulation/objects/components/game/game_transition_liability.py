...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\game\game_transition_liability.py
# Compiled at: 2019-04-08 19:47:40
...
class GameTransitionDestinationNodeValidator:
    ...
    def _add_sim_to_team_if_necessary(self, sim):
        if self._teams is None:
            self._teams = []
        elif any((sim in team for team in self._teams)):
            return
            ...
            for other_sim, other_targets in self._destinations.items():
                ...
                if is_on_same_team:
                    if not all((self._game_type.can_be_on_same_team(target, other_target) for other_target in other_targets)):
                        ...
                        return all((self._game_type.can_be_on_opposing_team(target, other_target) for other_target in other_targets)) or False
