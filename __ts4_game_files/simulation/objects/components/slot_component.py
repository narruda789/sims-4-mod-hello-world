...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\slot_component.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 36441 bytes
...
import distributor.ops, native.animation, services, sims4.callback_utils, sims4.log
...
class SlotComponent(GetPutComponentMixin, NativeComponent, component_name=types.SLOT_COMPONENT, key=787604481):
    FACTORY_TUNABLES = {'autonomy_modifiers':TunableList(description='\n            Objects parented to this object will have these autonomy modifiers\n            applied to them.\n            ',
       tunable=TunableAutonomyModifier(locked_args={'relationship_multipliers': None})), 
     ...
     'state_values_tuning':TunableSet(description='\n            Objects parented to this object will have these state values \n            applied to them. The original value will be restored if the child \n            is removed.\n            ',
       ...
       pack_safe=True)),
       state_to_set=(ObjectStateValue.TunableReference()))), 
     'on_destroy_behavior':TunableMapping(description='\n            Mapping of slot type to placement behavior to execute on children \n            slotted in to slots of that type when this object is destroyed.\n            \n            This takes precedence over return owned objects.\n            ',
       key_type=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.SLOT_TYPE)),
       pack_safe=True),
       value_type=TunableTuple(loot=TunableList(description='\n                    Loot that will be applied to child when the parent is destroyed.\n                    ',
       tunable=LootActions.TunableReference(pack_safe=True)),
       placement=TunableVariant(description='\n                    Where the child will be placed when the parent is destroyed\n                    ',
       default='position',
       position=(_PlacementStrategyLocation.TunableFactory()),
       household_inventory=(_PlacementStrategyHouseholdInventoryFromSlot.TunableFactory()))))}
    _handles = None
    _state_values = None

    def __init__(self, *args, autonomy_modifiers=DEFAULT, return_owned_objects=DEFAULT, slot_provided_affordances=DEFAULT, state_values_tuning=DEFAULT, on_destroy_behavior=DEFAULT, **kwargs):
    ...
    def on_child_added(self, child, location):
        if self.autonomy_modifiers and child.statistic_component is not None:
            ...
            elif self.state_values_tuning:
                if self._state_values is None:
                    self._state_values = WeakKeyDictionary()
                if child not in self._state_values:
                    state_values = []
                    for state_value_tuning in self.state_values_tuning:
                        required_slots = set(state_value_tuning.required_slot_types)
                        if required_slots:
                            if not any((location.slot_hash == slot_hash and required_slots & slot_types for slot_hash, slot_types in self.get_containment_slot_infos())):
                                continue
    ...
    def enable_slot(self, slot_hash):
        if self._disabled_slot_hashes is None:
            ...
            if slot_hash in self._disabled_slot_hashes:
                ...
                self.resend_disabled_slot_hashes()
            self._disabled_slot_hashes = self._disabled_slot_hashes or None
    ...
    def get_containment_slot_infos_static(object_slots, rig, owner):
        ...
        for slot_index in range(object_slots.get_slot_count(sims4.ObjectSlots.SLOT_CONTAINMENT)):
            ...
            if deco_slot_hashes is not None:
                if DecorativeSlotTuning.slot_types_are_all_decorative(slot_types):
        ...
        if self.slot_provided_affordances:
            for provided_affordance_data in self.slot_provided_affordances:
                if not provided_affordance_data.object_filter.is_object_valid(target):
                    continue
                yield from (provided_affordance_data.affordance.potential_interactions)(target, 
                 context, **kwargs)
    ...
    def validate_definition(self):
        if self.owner.parts:
            ...
            if invalid_runtime_slots:
                part_tuning = []
                for part in sorted((self.owner.parts), key=(lambda p:                 if p.is_base_part:
-1 # Avoid dead code: p.subroot_index)):
