...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\live_drag_component.py
# Compiled at: 2016-08-10 18:11:32
# Size of source mod 2**32: 11837 bytes
...
import distributor.fields, distributor.ops, gsi_handlers, services, sims4.log
...
class LiveDragComponent(NativeComponent, HasTunableFactory, component_name=LIVE_DRAG_COMPONENT, key=2125782609):
    ...
    def log_can_live_drag(self):
        if gsi_handlers.live_drag_handlers.live_drag_archiver.enabled:
            gsi_handlers.live_drag_handlers.archive_live_drag('Can Live Drag', 'Operation',
              (LiveDragLocation.GAMEPLAY_SCRIPT),
              'Client',
              live_drag_object=(self.owner),
              live_drag_object_id=(self.owner.id))
    ...
    def _on_owner_in_use_list_changed(self, user, added):
        if added and user is not self._live_drag_user:
            ...
            if not any((user is not self._live_drag_user for user in self.owner.get_users())):
    ...
    def is_valid_drop_target(self, test_obj):
        ...
        if test_obj.is_sim:
            if owning_household_id is not None:
                if owning_household_id != test_obj.household_id:
                    return False
    ...
    def get_valid_drop_object_ids(self):
        ...
        if self.owner.inventoryitem_component is not None:
            return (
             drop_target_ids, self.owner.inventoryitem_component.get_stack_id())
        return (drop_target_ids, None)

    def start_live_dragging(self, reserver, start_system):
        if self.owner.in_use:
            if not self.owner.in_use_by(self._live_drag_user):
                return False
