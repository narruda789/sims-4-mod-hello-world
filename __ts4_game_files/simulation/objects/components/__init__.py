...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\__init__.py
...
def forward_to_components(func):
    ...
    def wrapped_method(self, *args, **kwargs):
        ...
        if result is not None:
            logger.error('Method {} (which will also be forwarded to components) returned a value, which was ignored: {}', (func.__name__),
              result, owner='bhill')
        for comp in self.components_sorted_gen():
            ...
            if comp_func is not NO_FORWARD:
                ...
                if comp_result is not None:
                    logger.error('Method {} (which was forwarded to a component) returned a value, which was ignored: {}', (func.__name__),
                      comp_result, owner='bhill')
    ...
    def add_component(self, component):
        ...
        if self.has_component(component_def):
            ...
            if not self.can_add_component(component_def):
                ...
                return self.is_valid_pack(component) or False
            return component.is_valid_to_add() or False
...
class ComponentMetaclass(type):
    ...
    def __new__(mcs, name, bases, cls_dict, component_name=None, key=None, persistence_key=None, persistence_priority=ComponentPriority.PRIORITY_DEFAULT, use_owner=True, allow_dynamic=False, **kwargs):
        ...
        if persistence_key:
            persistence_key_map[persistence_key] = (
             persistence_priority, component_name)
        cntc_key = (
        ...
        for func_name, func in inspect.getmembers(cls, lambda member: getattr(member, '_export_component_method', False)):
            if func_name in component_methods:
                ...
                if fallback is DEFAULT:
                    setattr(ComponentContainer, func_name, cls._default_none_fallback_method)
    ...
    @classmethod
    def _default_none_fallback_method(cls, *_, **__):
        pass
