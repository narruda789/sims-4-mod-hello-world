...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\components\inventory.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 38622 bytes
...
import build_buy, objects.system, services, sims4.log
...
class InventoryComponent(Component):
    ...
    def _update_last_updated_timestamp(self):
        self._last_update_timestamp = services.server_clock_service().ticks()
    ...
    def player_try_add_object(self, obj, hidden=False):
        if not self.can_add(obj):
            return False
        return self._insert_item(obj, force_add_to_hidden_inventory=hidden)
    ...
    def _insert_item(self, obj, force_add_to_hidden_inventory=False, compact=True):
        if not self._handle_parented_objects(obj):
            return False
    ...
    def update_object_stack_by_id(self, obj_id, new_stack_id):
        ...
        if obj_id in self._storage:
            ...
            if obj_id in self._hidden_storage:
    ...
    def try_split_object_from_stack_by_id(self, obj_id, count=1):
        ...
        if storage is None:
            return
        obj = storage[obj_id]
        return storage.try_split_from_stack(obj, count=count)

    def _allow_destruction_on_inventory_transfer(self, obj):
        if obj.consumable_component is not None:
            if obj.consumable_component.allow_destruction_on_inventory_transfer:
                return True
        else:
            if obj.has_servings_statistic():
                return True
            if obj.inventoryitem_component is not None and obj.inventoryitem_component.always_destroy_on_inventory_transfer:
                return True
        return False
    ...
    def _handle_parented_objects(self, obj):
        ...
        for child_obj in tuple(obj.children):
            if child_obj.is_sim:
                logger.error("Owner ({}) has a reference to a child object that is a sim. Don't send sims to inventories.", obj)
                return False
            else:
                ...
                if translation is not None and orientation is not None:
                    child_obj.set_parent(None, transform=(sims4.math.Transform(translation, orientation)),
        ...
        if objects_in_household_inventory:
            if self.PARENTED_OBJECT_MOVED_TO_HOUSEHOLD_INVENTORY_NOTIFICATION is not None:
                ...
                notification.show_dialog(additional_tokens=(
                 (LocalizationHelperTuning.get_bulleted_list)(*(None, ), *(LocalizationHelperTuning.get_object_name(obj) for obj in objects_in_household_inventory)),))
        return True
    ...
    def push_inventory_item_tag_update_msg(self, obj, tag_set):
        if not InventoryStorage.INVENTORY_FILTER_TAGS & tag_set:
            return
    ...
    def open_ui_panel_with_preselected_filters(self, filter_tag):
        self._storage.open_ui_panel_with_preselected_filters((self.owner), filter_tag=filter_tag)
    ...
    def load_items(self, object_list):
        if self.is_shared_inventory:
            ...
            return object_list.objects or None
    ...
    def _create_inventory_object(self, definition_id, obj_data):
        objects.system.create_object(definition_id, obj_id=(obj_data.object_id),
          loc_type=(obj_data.loc_type),
          post_add=(lambda o: self._post_create_object(o, obj_data)),
          disable_object_commodity_callbacks=True)
        ...
        try:
            ...
            if obj not in self:
                if self.can_add(obj):
                    ...
                    if not services.reset_and_delete_service.has_been_destroyed(obj):
    ...
    def push_items_to_household_inventory(self):
        ...
        for obj in list(self._storage):
            ...
            if self._allow_destruction_on_inventory_transfer(obj):
                continue
            try:
                if self.try_remove_object_by_id((obj.id), count=(obj.stack_count())):
