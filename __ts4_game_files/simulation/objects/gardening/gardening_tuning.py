...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\gardening\gardening_tuning.py
# Compiled at: 2021-03-16 04:52:14
# Size of source mod 2**32: 11545 bytes
...
class GardeningTuning:
    ...
    EXCLUSIVE_FRUITS = TunableSet(description='\n        A set of fruits, which, when added onto a plant, can restrict\n        what other fruits the plant produces to this set of fruits. \n        This is done by adjusting spawn weight of non-exclusive fruits \n        on the plant to zero. \n        ',
      tunable=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.OBJECT)),
      pack_safe=True))
    ...
    def is_spliced(cls, obj):
        if obj.has_state(cls.SPLICED_STATE_VALUE.state):
            if obj.get_state(cls.SPLICED_STATE_VALUE.state) == cls.SPLICED_STATE_VALUE:
                return True
        return False

    @classmethod
    def is_unidentified(cls, obj):
        if cls.UNIDENTIFIED_STATE_VALUE is not None:
            if obj.has_state(cls.UNIDENTIFIED_STATE_VALUE.state):
                if obj.get_state(cls.UNIDENTIFIED_STATE_VALUE.state) == cls.UNIDENTIFIED_STATE_VALUE:
                    return True
        return False
    ...
    CROP_FRUIT_QUALITY_STATE = ObjectState.TunablePackSafeReference(description='\n        The quality state used to determine the quality of crop fruits.  \n        ')