...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\terrain.py
# Compiled at: 2022-04-20 20:37:17
# Size of source mod 2**32: 46617 bytes
...
from sims4.tuning.tunable import TunableReference, Tunable, TunableTuple, TunableList, TunableRange, TunableEnumEntry, OptionalTunable, TunableVariant
from sims4.utils import setdefault_callable, classproperty, flexmethod, constproperty
import build_buy, caches, distributor, objects.system, routing, services, sims.sim_spawner, sims4.log, sims4.math, terrain
...
class TerrainInteractionMixin:
    ...
    def _get_target_position_surface_and_test_off_lot(cls, target, context):
        ...
        if target is not None or context.pick is not None:
            ...
            if position is None:
                return (
                 position, surface, TestResult(False, 'Cannot Travel without a pick or target.'))
            zone = services.current_zone()
            if context.sim is not target:
                if zone.lot.is_position_on_lot(position):
                    return (
                     position, surface, TestResult(False, 'Cannot Travel inside the bounds of the zone!'))
        return (
         position, surface, TestResult.TRUE)

    @classmethod
    def _get_position_and_surface(cls, target, context):
        if context.pick is not None:
            return (
             context.pick.location, context.pick.routing_surface)
...
lock_instance_tunables(TerrainSuperInteraction, basic_reserve_object=None,
  basic_focus=None)
    ...
    def _test(cls, target, context, **kwargs):
        ...
        if not result:
            ...
            if position is not None:
                if not terrain.is_position_in_street(position):
...
class GoHereSuperInteraction(TerrainSuperInteraction):
    ...
    def create_load_context(cls, sim, source, priority):
        ...
        pick = PickInfo(pick_type=pick_type, target=target,
          location=(location.transform.translation),
          routing_surface=(location.routing_surface))
        context = InteractionContext(sim, source,
          priority,
          pick=pick,
    ...
    def prepare_gen(self, timeline, *args, **kwargs):
        if self.target is not None:
            if self.sim == services.get_active_sim():
                if self.sim.can_sim_teleport_using_teleport_style():
                    ghost_route = self.sim.routing_context.ghost_route
                    self.sim.routing_context.ghost_route = True
                    target_location = self.target.routing_location
                    connectivity = routing.test_connectivity_pt_pt(self.sim.routing_location, target_location, self.sim.routing_context)
                    if not connectivity:
                        aop, teleport_context, _ = self.sim.get_teleport_style_interaction_aop(self)
                        if aop is not None:
                            self.sim.push_super_affordance((aop.affordance), (self.target),
                              teleport_context,
                              allow_posture_changes=True)
                            self.cancel((FinishingType.TRANSITION_FAILURE), cancel_reason_msg='Destination is unroutable: teleported there early.')
                            return InteractionQueuePreparationStatus.PUSHED_REPLACEMENT
                    elif self.sim.is_ghost:
                        self.sim.routing_context.ghost_route = ghost_route
                    else:
                        self.sim.routing_context.ghost_route = False
...
class DebugSetupLotInteraction(TerrainImmediateSuperInteraction):
    INSTANCE_TUNABLES = {'setup_lot_destroy_old_objects':Tunable(bool, False, description='Destroy objects previously created by this interaction.'), 
     'setup_lot_objects':TunableList(TunableTuple(definition=(TunableReference(definition_manager())),
       position=(TunableVector2(Vector2.ZERO())),
       angle=(TunableRange(int, 0, -360, 360)),
       children=(TunableList(TunableTuple(definition=TunableReference((definition_manager()), description='The child object to create.  It will appear in the first available slot in which it fits, subject to additional restrictions specified in the other values of this tuning.'),
       part_index=(OptionalTunable(Tunable(int, 0, description='If specified, restrict slot selection to the given part index.'))),
       bone_name=(OptionalTunable(Tunable(str, '_ctnm_chr_', description='If specified, restrict slot selection to one with this exact bone name.'))),
       slot_type=(OptionalTunable(TunableReference(manager=(services.get_instance_manager(Types.SLOT_TYPE)), description='If specified, restrict slot selection to ones that support this type of slot.'))),
    ...
    def find_nearest_point_on_lot(self, lot, root):
        ...
        segments = [(lot_corners[0], lot_corners[1]), (lot_corners[1], lot_corners[2]),
         (
          lot_corners[2], lot_corners[3]), (lot_corners[3], lot_corners[1])]
        dist = 0
        closest_point = None
    ...
    def _run_interaction_gen(self, timeline):
        if not self.species == Species.HUMAN:
            if self.breed_picker is None or self.breed_picker.breed_by_type == self.BREED_BY_TAG:
                ...
                breed = self.breed_picker.breed_tag if self.breed_picker is not None else None
                sim_creator = sims.sim_spawner.SimCreator(age=(self.age), gender=(self.gender), species=(self.species), additional_tags=((breed,) if breed is not None else ()))
                sim_info_list, _ = sims.sim_spawner.SimSpawner.create_sim_infos((sim_creator,), household=household,
                ...
                if sim_info.age == Age.BABY:
                    ...
                    sims.sim_spawner.SimSpawner.spawn_sim(sim_info, sim_position=position,
                      is_debug=True)
        else:
            self.sim.push_super_affordance((self.breed_picker.picker), (self.target),
              (self.context),
    ...
    def _get_ungreeted_overrides(self, context, **kwargs):
        ...
        if not zone_director.forward_ungreeted_front_door_interactions_to_terrain:
            ...
            if not context.pick.lot_id or context.pick.lot_id != services.active_lot().lot_id:
                return
            return services.get_zone_situation_manager().is_player_waiting_to_be_greeted() or None
...
lock_instance_tunables(Terrain, _persistence=(PersistenceType.NONE),
  _world_file_object_persists=False,
  provides_terrain_interactions=False)
...
class _LocationPoint(ProxyObject):
    ...
    @property
    def routing_location(self):
        return routing.Location((self._pick_location.transform.translation), orientation=(self._pick_location.transform.orientation),
    ...
    def check_line_of_sight(self, transform, verbose=False, **kwargs):
        ...
        if verbose:
            return (
             result, 0)
        return (
         result == routing.RAYCAST_HIT_TYPE_NONE, 0)
