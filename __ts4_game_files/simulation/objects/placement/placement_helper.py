...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\placement\placement_helper.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 43317 bytes
...
import interactions.utils.parent_object as parent_object
...
from placement import FGLSearchFlag, create_starting_location, create_fgl_context_for_object_off_lot, create_fgl_context_for_object, find_good_location, PositionIncrementInfo, ScoringFunctionPolygon, FGLSearchFlagsDeprecated
...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, TunableEnumEntry, TunableVariant, TunableTuple, OptionalTunable, TunableInterval, TunableAngle, Tunable, TunableReference, TunableList, TunableFactory, TunableEnumSet, TunableMapping
...
import build_buy, services, sims4.math, random as system_random
...
class _ObjectsFromTags(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'tags':TunableTags(description='\n            For each tag, in order, gather objects that match that have that\n            tag. If the placement fails, consider another object, then consider\n            objects for the next tag.\n            If should_shuffle is True, gather all objects matching any tag, then\n            pick a random one, if placement fails, pick another random one.\n            '), 
     'should_shuffle':Tunable(description='\n            If False, go through each tag in order, and try placement for each object (in\n            the order of their indices in object manager). \n            If True, we gather all objects matching any tag, then pick a random one,\n            if placement fails, pick another random one.\n            ',
       tunable_type=bool,
       default=False)}

    def get_objects_gen(self, resolver):
        if self.should_shuffle:
            objects_with_tags = list(services.object_manager().get_objects_matching_tags((self.tags), match_any=True))
...
class _LocationFromLot(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'lot_location_strategy': TunableEnumEntry(description='\n            Strategy for how we will determine a location on the lot.\n            Default: Tries to retrieve the position of the front door, otherwise\n            uses an arbitrary corner (will always be the same corner)\n            Random: Retrieves a random point on the lot\n            ',
                                tunable_type=LotPositionStrategy,
                                default=(LotPositionStrategy.DEFAULT))}
...
class _LocationFromFloor(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'participant': TunableEnumEntry(description='\n            The participant that determines the lot level to be used to find\n            an initial location.\n            ',
                      tunable_type=ParticipantTypeLotLevel,
                      default=(ParticipantTypeLotLevel.ActorLotLevel))}
        ...
        for target_obj in (self._get_reference_objects_gen)(obj, resolver, **kwargs):
            if target_obj is not None:
                if target_obj.is_sim:
                    target_obj = target_obj.sim_info.get_sim_instance(allow_hidden_flags=(HiddenReasonFlag.RABBIT_HOLE))
...
class _PlacementStrategyInventory(_PlacementStrategy):
    ...
    def try_place_object(self, obj, resolver, **kwargs):
        ...
        if not recipient.is_lot_inventory:
...
class _PlacementStrategyLocation(_PlacementStrategy):
    ...
    FACTORY_TUNABLES = {'search_flag_set':TunableEnumSet(description="\n            Set of flags for FGL Search that will be enabled if present in list.\n            If you're unsure how these flags work, consult your GPE partner.\n            ",
       enum_type=FGLSearchFlag,
       default_enum_list=[
      FGLSearchFlag.STAY_IN_CONNECTED_CONNECTIVITY_GROUP,
      FGLSearchFlag.CALCULATE_RESULT_TERRAIN_HEIGHTS,
      FGLSearchFlag.DONE_ON_MAX_RESULTS],
       invalid_enums=FGLSearchFlagsDeprecated), 
     'initial_location':TunableVariant(description='\n            The FGL search initial position is determined by this. If more than\n            one initial position is available, all are considered up to the\n            specified upper bound.\n            ',
       ...
       from_joint=_LocationFromJoint.TunableFactory(),
       front_door_object=_FrontDoorObject.TunableFactory(),
       from_floor=_LocationFromFloor.TunableFactory(),
    ...
    def _try_place_object_internal(self, obj, target_obj, resolver, ignored_object_ids=None, **kwargs):
        ...
        if x_range is not None:
            ...
            if self.raytest:
                ...
                raytest_kwargs.update({'raytest_radius':self.raytest.raytest_radius,  'raytest_start_offset':self.raytest.raytest_start_height_offset, 
                 'raytest_end_offset':self.raytest.raytest_end_height_offset})
        ...
        if self.allow_off_lot_placement:
            obj.location = active_lot.is_position_on_lot(start_position) or sims4.math.Location(sims4.math.Transform(start_position, start_orientation), routing_surface)
            ...
            context = create_fgl_context_for_object_off_lot(starting_location, obj, terrain_tags=terrain_tags, search_flags=search_flags, ignored_object_ids=(
 obj.id,), 
             restrictions=restrictions, min_water_depth=self.min_water_depth, 
             max_water_depth=self.max_water_depth, routing_context=routing_context, 
             scoring_functions=scoring_functions, **raytest_kwargs)
        else:
            if not self.allow_off_lot_placement:
            ...
            if not self.ignore_bb_footprints:
                ...
                context = create_fgl_context_for_object(starting_location, obj, terrain_tags=terrain_tags, search_flags=search_flags, ignored_object_ids=ignored_object_ids, 
...
class _PlacementStrategySlot(_PlacementStrategy):
    FACTORY_TUNABLES = {'parent':TunableVariant(description='\n            The object this object is going to be slotted into.\n            ',
       from_participant=_ObjectsFromParticipant.TunableFactory(participant_type_default=(
...
class TunablePlacementStrategyVariant(TunableVariant):
    ...
    def __init__(self, *args, **kwargs):
        (super().__init__)(args, inventory=_PlacementStrategyInventory.TunableFactory(), 
         location=_PlacementStrategyLocation.TunableFactory(), 
         slot=_PlacementStrategySlot.TunableFactory(), 
         household_inventory=_PlacementStrategyHouseholdInventory.TunableFactory(), 
