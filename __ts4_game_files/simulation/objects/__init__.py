...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\__init__.py
# Compiled at: 2019-06-06 13:53:41
# Size of source mod 2**32: 12091 bytes
...
import enum, services, sims4.hash_util, sims4.log, sims4.math
...
class PaintingState(collections.namedtuple('_PaintingState', ('texture_id', 'reveal_level', 'use_overlay',
                                          'effect', 'stage_texture_id', 'overlay_texture_id',
                                          'reveal_texture_id'))):
    REVEAL_LEVEL_MIN = 0
    REVEAL_LEVEL_MAX = 5
    ...
    def __new__(cls, texture_id, reveal_level=0, use_overlay=False, effect=sims4.math.MAX_UINT32, stage_texture_id=None, overlay_texture_id=None, reveal_texture_id=None):
...
class TunableStringOrDefault(OptionalTunable):
    ...
    def __init__(self, default, **kwargs):
        (super().__init__)(disabled_name='set_to_default_value', enabled_name='set_to_custom_value', 
         tunable=Tunable(str, default), **kwargs)
...
class TunableMaterialState(TunableSingletonFactory):
    ...
    def __init__(self, description='A material state.', **kwargs):
        (super().__init__)(state_name=TunableStringOrDefault('materialStateName', description='The name of the material state.'), 
         opacity=TunableRange(float, 1, 0, 1, description='Opacity of the material from ( 0.0 == transparent ) to ( 1.0 == opaque ). Not yet supported on the client.'), 
         transition=TunableSimMinute(0, description='Time to take when transitioning in sim minutes. Not yet supported on the client.'), 
         description=description, **kwargs)
...
class ModelSuiteStateIndex:
    ...
    def stop(self, *_, **__):
        if self._target is not None:
            if self._old_state_index is not None:
                if self._old_state_index != self._target.state_index:
