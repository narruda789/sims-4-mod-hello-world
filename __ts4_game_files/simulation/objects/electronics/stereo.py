...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\objects\electronics\stereo.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 18262 bytes
...
import element_utils, event_testing.state_tests, objects.components.state, services, sims4
...
class ListenSuperInteraction(SuperInteraction):
    ...
    def __init__(self, aop, context, required_station=None, off_state=None, **kwargs):
        ...
        if required_station is not None:
            ...
            if self.required_station is None:
                ...
                if self.required_station is None:
                    logger.error('Listen interaction {} does not have a required station. This must be tuned on the interaction or on the stereo component', self, owner='bteng')
            ...
            if self.off_state is None:
                logger.error('Listen interaction {} does not have an off state. This must be tuned on the stereo component of {}', self, (self.target), owner='bteng')
    ...
    def ensure_state(self, desired_station):
        ...
        if stereo_component is None:
            ...
            return
        audio_state_args = {'immediate_audio':stereo_component.immediate, 
    ...
    def _get_name(cls, inst, *args, required_station=None, **interaction_parameters):
        station_name = None
        if inst is not None and inst.required_station is not None:
            station_name = inst.required_station.display_name
        else:
            if required_station is not None:
                station_name = required_station.display_name
            else:
                if cls.required_station is not None:
    ...
    def get_rallyable_interaction_parameters(self):
        interaction_parameters = {'required_station':self.required_station, 
...
class CancelOnStateChangeInteraction(SuperInteraction):
    INSTANCE_TUNABLES = {'cancel_state_test':event_testing.state_tests.TunableStateTest(description="the state test to run when the object's state changes. If this test passes, the interaction will cancel"), 
     'required_station':OptionalTunable(description="\n            If enabled, specifies the radio station this affordance listens to.\n            Normally this is provided by the object's Stereo component and does \n            not need to be tuned here.\n            ",
       tunable=objects.components.state.TunableStateValueReference(description='\n                The station that this affordance listens to.\n                '),
    ...
    def _run_interaction_gen(self, timeline):
        result = yield from element_utils.run_child(timeline, element_utils.build_element([
         self._cancel_on_state_test_pass(self.cancel_state_test, super()._run_interaction_gen)]))
        return result
        if False:
            yield None
...
class StereoPieMenuChoicesInteraction(ImmediateSuperInteraction):
    INSTANCE_TUNABLES = {'push_additional_affordances':Tunable(bool, True, description="Whether to push affordances specified by the channel. This is used for stereo's turn on and listen to... interaction"), 
     'off_state_pie_menu_category':TunableTuple(pie_menu_category=TunableReference(description='\n                Pie menu category so we can display a submenu for each outfit category\n                ',
       manager=(services.get_instance_manager(sims4.resources.Types.PIE_MENU_CATEGORY)),
       allow_none=True),
       pie_menu_category_on_forwarded=TunableReference(description='\n                Pie menu category so when this interaction is forwarded from inventory\n                object to inventory owner.\n                ',
       manager=(services.get_instance_manager(sims4.resources.Types.PIE_MENU_CATEGORY)),
       allow_none=True))}
        ...
        if stereo is not None:
            ...
            if not stereo_component.is_stereo_turned_on():
                if from_inventory_to_owner:
                    if inst_or_cls.off_state_pie_menu_category.pie_menu_category_on_forwarded is not None:
    ...
    def potential_interactions(cls, target, context, from_inventory_to_owner=False, **kwargs):
        ...
        for client_state in stereo_component.get_available_picker_channel_states(context):
            yield AffordanceObjectPair(cls, target, cls, None, stereo=target,
              audio_channel=client_state,
