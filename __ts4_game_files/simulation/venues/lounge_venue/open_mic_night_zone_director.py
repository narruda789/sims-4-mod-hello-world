...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\venues\lounge_venue\open_mic_night_zone_director.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 16258 bytes
import random
...
class OpenMicContestant(SituationComplexCommon):
    INSTANCE_TUNABLES = {'wait_for_turn_state':WaitForTurnState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':True}), 
     'take_turn_state':TakeTurnState.TunableFactory(locked_args={'allow_join_situation': True}), 
     'contestant_job':TunableReference(description='\n            The job for the contestant Sim.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB))}
    REMOVE_INSTANCE_TUNABLES = Situation.NON_USER_FACING_REMOVE_INSTANCE_TUNABLES

    @classmethod
    def _states(cls):
        return (SituationStateData(1, WaitForTurnState, factory=(cls.wait_for_turn_state)),
         SituationStateData(2, TakeTurnState, factory=(cls.take_turn_state)))
    ...
    def sim_of_interest(self, sim_info):
        if self._contestant_sim is not None:
            if self._contestant_sim.sim_info is sim_info:
                return True
        return False
...
lock_instance_tunables(OpenMicContestant, creation_ui_option=(SituationCreationUIOption.NOT_AVAILABLE),
  duration=0,
...
class OpenMicPlayerControllerSituation(SituationComplexCommon):
    INSTANCE_TUNABLES = {'situation_state':PlayerSituationState.TunableFactory(locked_args={'time_out':None, 
      'allow_join_situation':True}), 
     'player_job':TunableReference(description='\n            The job for the player Sim.\n            ',
       manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB))}
    REMOVE_INSTANCE_TUNABLES = Situation.NON_USER_FACING_REMOVE_INSTANCE_TUNABLES
...
class OpenMicNightZoneDirector(ZoneDirectorBase):
    ...
    def on_startup(self):
        self._select_next_contestant_alarm = alarms.add_alarm(self, create_time_span(minutes=(self.check_contestant_alarm_timer)),
    ...
    def create_situations_during_zone_spin_up(self):
        ...
        while len(self._npc_contestant_sitaution_ids) < self.npc_contestant_count:
            ...
            situation_id = situation_manager.create_situation((self.npc_contestant_situation), guest_list=guest_list,
              user_facing=False,
        ...
        if situation_manager.get_situation_by_type(self.bartender_situation) is None:
            guest_list = SituationGuestList(invite_only=True)
            situation_manager.create_situation((self.bartender_situation), guest_list=guest_list,
              user_facing=False,
              scoring_enabled=False,
              spawn_sims_during_zone_spin_up=True)
        guest_list = SituationGuestList(invite_only=True)
        situation_manager.create_situation((self.player_controller_situation), guest_list=guest_list,
          user_facing=False,
          scoring_enabled=False)
    ...
    def request_player_turn(self, sim_info):
        ...
        situation_id = services.get_zone_situation_manager().create_situation((self.player_contestant_situation), guest_list=guest_list,
          user_facing=False,
          scoring_enabled=False)
    ...
    def select_next_contestant(self):
        ...
        if self._player_contestant_situation_ids:
            ...
            possible_contestants = [situation_id for situation_id in self._npc_contestant_sitaution_ids if not self._current_contestant_situation_id is None if situation_id != self._current_contestant_situation_id]
    ...
    def _load_custom_zone_director(self, zone_director_proto, reader):
        self._npc_contestant_sitaution_ids = list(reader.read_uint64s(NPC_CONTESTANTS_TOKEN, []))
        self._player_contestant_situation_ids = list(reader.read_uint64s(PLAYER_CONTESTANTS_TOKEN, []))
