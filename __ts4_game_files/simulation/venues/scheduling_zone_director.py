...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\venues\scheduling_zone_director.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 27975 bytes
...
import alarms, clock, enum, services, sims4.log
...
class SchedulingZoneDirectorMixin:
    ...
    def can_schedule_situation(self, situation, limit_based_on_sims=False, number_of_sims_desired=0, **kwargs):
        if services.game_clock_service().clock_speed == clock.ClockSpeedMode.SUPER_SPEED3:
            if not situation.allowed_in_super_speed_3:
                return False
        if limit_based_on_sims:
            if not situation.can_start_walkby(services.active_lot_id(), number_of_sims_desired):
                return False
    ...
    def _shift_change_alarm_callback(self, alarm_handle=None):
        ...
        if situation_shift.shift_strictness == SituationShiftStrictness.DESTROY:
            ...
            if situation_shift.shift_strictness == SituationShiftStrictness.OVERLAP:
                if situation_shift.count_based_on_expected_sims:
                    for situation_id in situation_ids:
                        ...
                        if sims_in_situation is None:
                            logger.error('Trying to get expected Sims for situation {} that does not provide that information.  Treating as a single Sim.  This situation does not support this behavior.  Contact your GPE partner to fix this.',
                              situation,
                              owner='jjacobson')
                            sims_in_situation = 1
        ...
        while number_sims_desired_for_shift > 0:
            ...
            for situation_source in situation_shift.additional_sources:
                additional_situations.extend(situation_source.get_additional_situations(predicate=(lambda potential_situation: self.can_schedule_situation(potential_situation, limit_based_on_sims=(situation_shift.count_based_on_expected_sims),
                  number_of_sims_desired=number_sims_desired_for_shift))))

            situation_to_start, params = situation_shift.shift_curve.get_situation_and_params(predicate=(lambda potential_situation: self.can_schedule_situation(potential_situation, limit_based_on_sims=(situation_shift.count_based_on_expected_sims),
              number_of_sims_desired=number_sims_desired_for_shift)),
            ...
            if situation_to_start is not None:
                ...
                if guest_list is None:
                    ...
                    situation_id = (situation_manager.create_situation)(situation_to_start, guest_list=guest_list, 
                     spawn_sims_during_zone_spin_up=True, 
                     creation_source=creation_source, **params)
    ...
    def _handle_situation_shift_churn(self, situation_shift, reserve_object_relationships=False):
        if situation_shift.shift_curve.is_shift_churn_disabled:
            ...
            if number_situations_desire > current_count:
                ...
                predicate = lambda situation: self.can_schedule_situation(situation, limit_based_on_sims=(situation_shift.count_based_on_expected_sims),
                  number_of_sims_desired=(number_situations_desire - current_count),
                  reserved_object_relationships=reserved_object_relationships)
                additional_situations = []
                for situation_source in situation_shift.additional_sources:
        ...
        if op == SituationChurnOperation.START_SITUATION:
            if situation_to_start is not None:
                ...
                pass
        if op == SituationChurnOperation.REMOVE_SITUATION:
            time_of_day = services.time_service().sim_now
