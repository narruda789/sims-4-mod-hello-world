...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\venues\cafe_venue\cafe_generic_situation.py
# Compiled at: 2018-07-12 18:54:26
# Size of source mod 2**32: 8208 bytes
...
class CafeGenericCustomerSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (SituationStateData(1, _PreOrderCoffeeState, factory=(cls.pre_order_coffee_state)),
         ...
         SituationStateData(3, _CafeGenericBehaviorState, factory=(cls.cafe_generic_state)))
    ...
    def sim_of_interest(self, sim_info):
        if self._cafe_sim is not None:
            if self._cafe_sim.sim_info is sim_info:
                return True
        return False


lock_instance_tunables(CafeGenericCustomerSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
...
class CafeGenericBackgroundSituation(SituationComplexCommon):
    ...
    def _issue_requests(self):
        request = BouncerRequestFactory(self, callback_data=_RequestUserData(role_state_type=(self.generic_sim_job.role_state)),
          job_type=(self.generic_sim_job.job),
          request_priority=(BouncerRequestPriority.BACKGROUND_MEDIUM),
          user_facing=False,
    ...
    def _on_set_sim_job(self, sim, job_type):
        ...
        situation_manager.create_situation((self.cafe_generic_customer_situation), guest_list=guest_list,
          user_facing=False)


lock_instance_tunables(CafeGenericBackgroundSituation, exclusivity=(BouncerExclusivityCategory.PRE_VISIT),
  creation_ui_option=(SituationCreationUIOption.NOT_AVAILABLE),
  duration=0,
  _implies_greeted_status=False)