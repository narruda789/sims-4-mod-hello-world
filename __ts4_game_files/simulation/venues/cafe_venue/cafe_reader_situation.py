...
class CafeReaderSituation(SituationComplexCommon):
    ...
    def _states(cls):
        return (SituationStateData(1, _PreOrderCoffeeState, factory=(cls.pre_order_coffee_state)),
         ...
         SituationStateData(3, _ReaderState, factory=(cls.reader_state)))
    ...
    def sim_of_interest(self, sim_info):
        if self._reader_sim is not None:
            if self._reader_sim.sim_info is sim_info:
                return True
        return False
...
lock_instance_tunables(CafeReaderSituation, exclusivity=(BouncerExclusivityCategory.NORMAL),
