...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\elements.py
# Compiled at: 2017-10-11 17:41:04
# Size of source mod 2**32: 41830 bytes
...
class GeneratorElementBase(ParentElement):
    ...
    def _run(self, timeline):
        ...
        try:
            ...
            assert _check_yield(child), 'Yielding non-Element handle: {}'.format(child)
            return child
    ...
    def _repr_helper(self, tracing=False):
        if self.generator is None:
            ...
            if self.generator.gi_running:
                ...
                if self.generator.gi_frame is not None:
                    status = 'active'
                    name = '{}@{}'.format(self.generator.gi_code.co_name, self.generator.gi_code.co_firstlineno)
                    if not tracing:
                        if self._child_handle is not None:
...
class BusyWaitElement(ParentElement):
    ...
    def __repr__(self):
        if self._test_callable is None:
            return '<{}; None>'.format(self.shortname())
...
class SequenceElement(ParentElement):
    ...
    def _run_next(self, timeline):
        ...
        while self.index < len(self.queue):
            ...
            if cancelled:
                if not isinstance(child_element, MustRunElement):
                    self.index += 1
                    continue
    ...
    def tracing_repr(self):
        active_element_str = self.queue[self.index].tracing_repr() if (self.queue is not None and self.index < len(self.queue)) else 'Done'
...
class CriticalSectionElement(ParentElement):
    ...
    def __repr__(self):
        return '<{}; {}; {}; {}; {}>'.format(self.shortname(), [
         'init', 'work', 'cleanup'][self.state], self.work, self.cleanup, self.result)

    def tracing_repr(self):
        return '<{}; {}; {}; {}; {}>'.format(self.shortname(), [
         'init', 'work', 'cleanup'][self.state], self.work.tracing_repr(), self.cleanup.tracing_repr(), self.result)
...
class WithFinallyElement(ParentElement):
    ...
    def _call_finally(self, timeline):
        if self._finally_callable is not None:
            if self._should_call_finally:
                self._should_call_finally = False
