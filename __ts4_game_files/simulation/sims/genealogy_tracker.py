...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\genealogy_tracker.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 17994 bytes
...
class GenealogyTracker(SimInfoTracker):
    ...
    def remove_family_link(self, relationship):
        if _genealogy_cache is not None:
            if relationship in self._family_relations:
    ...
    def get_family_relationship_bit(self, sim_id, output=None):
        ...
        if common_ancestor_ids:
            ...
            if sim_x.spouse_sim_id is not None and sim_x.spouse_sim_id == sim_y.sim_id:
                ...
                if sim_x.fiance_sim_id is not None:
                    if sim_x.fiance_sim_id == sim_y.sim_id:
    ...
    def get_grandparent_sim_ids_gen(self):
        for relation in (FamilyRelationshipIndex.FATHERS_FATHER,
         FamilyRelationshipIndex.FATHERS_MOM,
         FamilyRelationshipIndex.MOTHERS_FATHER,
         FamilyRelationshipIndex.MOTHERS_MOM):
            if relation in self._family_relations:
        ...
        def set_parent_node(relationship_index):
            ...
            if current[0] in sim_info_manager:
                ...
                if len(parent_ids) > relationship_index:
                    current = (
                     parent_ids[relationship_index], current[1] + 1)
                    return
            current = None
