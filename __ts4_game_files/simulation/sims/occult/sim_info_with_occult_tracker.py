...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\occult\sim_info_with_occult_tracker.py
...
class SimInfoWithOccultTracker(HasTraitTrackerMixin, SimInfoBaseWrapper):
    ...
    resend_current_occult_types = current_occult_types.get_resend()
        ...
        if success:
            for occult_type, trait_data in OccultTracker.OCCULT_DATA.items():
                if trait_data.occult_trait is trait:
                    ...
                    if self.is_baby:
                        if trait_data.add_current_occult_trait_to_babies:
