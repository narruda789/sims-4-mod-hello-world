...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\university\university_tuning.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 24048 bytes
import random
...
_UniversityDisplayMixin = get_display_mixin(has_description=True, has_icon=True, has_tooltip=True, enabled_by_default=True,
...
class University(HasTunableReference, _UniversityDisplayMixin, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(Types.UNIVERSITY)):
    COURSE_ELECTIVES = TunableTuple(description='\n        Tuning structure holding all electives data.\n        ',
      ...
      manager=(services.get_instance_manager(Types.UNIVERSITY_COURSE_DATA)),
      pack_safe=True),
    ...
    def _verify_tuning_callback(cls):
        ...
        if not prestige_degrees.issubset(all_degrees):
            logger.error('Prestige Degrees {} in University {} is not tuned as All Degrees in sims.university.university_tuning.', (prestige_degrees - all_degrees),
              (cls.__name__), owner='mkartika')
...
class UniversityMajor(HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(Types.UNIVERSITY_MAJOR)):
    INSTANCE_TUNABLES = {'courses':TunableList(description='\n            List of courses, in order, for this major\n            ',
       tunable=UniversityCourseData.TunableReference(),
       minlength=1), 
     'acceptance_score':TunableTuple(description='\n            Score requirement to be accepted in this major as prestige degree.\n            ',
    ...
    @classmethod
    def get_sim_acceptance_score(cls, sim_info, acceptance_score):
        resolver = SingleSimResolver(sim_info)
        return acceptance_score.score.get_multiplier(resolver)

    @classmethod
    def can_sim_be_accepted(cls, sim_info, acceptance_score):
        sim_score = cls.get_sim_acceptance_score(sim_info, acceptance_score)
        return acceptance_score.score_threshold.compare(sim_score)