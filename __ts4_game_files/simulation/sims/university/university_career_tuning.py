...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\university\university_career_tuning.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 16702 bytes
from careers.career_story_progression import CareerStoryProgressionParameters
from careers.career_tuning import Career, TunableCareerTrack, CareerLevel, ActiveCareerType
...
import interactions.context, services, sims4.log
...
class UniversityCourseCareerSlot(Career):
    ...
    def _get_classroom(self):
        ...
        if course_data is None:
            return
        university = degree_tracker.get_university()
        if university not in course_data.classroom_tag:
            return
        for classroom in (services.object_manager().get_objects_with_tags_gen)(*course_data.classroom_tag[university]):
            return classroom
    ...
    def go_to_work_early(self):
        sim = self._get_sim()
        if self.work_early_affordance is None or sim is None:
            return
        classroom = self._get_classroom()
        if sim.queue.has_duplicate_super_affordance(self.work_early_affordance, sim, classroom):
            return
        context = interactions.context.InteractionContext(sim, (interactions.context.InteractionContext.SOURCE_SCRIPT_WITH_USER_INTENT),
          (interactions.priority.Priority.High),
          insert_strategy=(QueueInsertStrategy.LAST))
        sim.push_super_affordance((self.work_early_affordance), classroom, context, career_uid=(self.guid64))

    def can_work_early(self):
        if self._sim_info.zone_id != services.current_zone_id():
            return False
        if self.career_messages.career_early_warning_time <= 0:
            return False
        time_until_work, _, _ = self.get_next_work_time()
        early_warning_time_span = create_time_span(hours=(self.career_messages.career_early_warning_time))
        if time_until_work > early_warning_time_span:
            return False
        return self._get_classroom() is not None


lock_instance_tunables(UniversityCourseCareerSlot, available_for_club_criteria=False,
  can_be_fired=False,
  career_availablity_tests=(CompoundTestList()),
  career_story_progression=CareerStoryProgressionParameters(joining=None, retiring=None,
  quitting=None),
  days_to_level_loss=0,
  demotion_buff=None,
  demotion_chance_modifiers=TunableMultiplier(base_value=0, multipliers=()),
  ...
  fired_buff=None,
  initial_pto=0,
  active_career_type=(ActiveCareerType.NON_ACTIVE),
  levels_lost_on_leave=0,
  promotion_buff=None,
  quittable_data=None,
  start_level_modifiers=TestedSum(base_value=0, modifiers=()),
  disable_pto=True,
  show_career_in_join_career_picker=False)
...
lock_instance_tunables(UniversityCourseSchedule, promotion_audio_sting=None,
  ageup_branch_career=None,
  promotion_reward=None,
  screen_slam=None,
  simolean_trait_bonus=[],
  simoleons_for_assignments_per_day=0,
  demotion_performance_level=(-100),
  fired_performance_level=(-100),
  promote_performance_level=100)