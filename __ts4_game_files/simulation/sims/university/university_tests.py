...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\university\university_tests.py
# Compiled at: 2020-11-12 10:43:21
# Size of source mod 2**32: 59654 bytes
...
import event_testing.test_base, services, sims4.log, sims4.resources
...
class UniversityEnrollmentTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    class _SpecificMajors(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def is_valid_major(self, sim_info, tooltip=None):
            ...
            if degree_tracker is None:
                return TestResult(False, '{0} has no degree tracker.', sim_info,
                  tooltip=tooltip)
            else:
                major = degree_tracker.get_major()
                return self._majors.test_item(major) or TestResult(False, "{0}'s enrolled majors do not match the tuned whitelist/blacklist.", sim_info,
                  tooltip=tooltip)
    ...
    class _AnyMajors(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def is_valid_major(self, sim_info, tooltip=None):
            ...
            if degree_tracker.get_enrolled_major() is None:
                return TestResult(False, '{0} is not enrolled in a major.', sim_info,
                  tooltip=tooltip)
            return TestResult.TRUE
        ...
        def is_valid_major(self, sim_info, tooltip=None):
            ...
            if degree_tracker is not None:
                if degree_tracker.get_enrolled_major() is not None:
                    return TestResult(False, '{0} is enrolled in a major.', sim_info,
                      tooltip=tooltip)
            return TestResult.TRUE
    ...
    FACTORY_TUNABLES = {'major':TunableVariant(description='\n            Which major(s) the sim must be pursuing.\n            ',
       ...
       no_major=_NoMajor.TunableFactory(),
        ...
        if self.major is not None:
            ...
            if self.university is not None:
                ...
                if current_university is None:
                    return TestResult(False, '{0} has no university specified in their degree tracker.', sim_info,
                      tooltip=tooltip)
                if current_university.guid64 != self.university.guid64:
                    return TestResult(False, '{0} is not enrolled in the correct university.', sim_info,
                      tooltip=tooltip)
            if self.enrollment_status is not None:
                return self.enrollment_status.test_item(degree_tracker._enrollment_status) or TestResult(False, '{0} does not pass the whitelist/blacklist for university enrollment status', sim_info,
                  tooltip=tooltip)
        return TestResult.TRUE
...
class UniversityClassroomTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'course':TunableVariant(description='\n            Which course to see if the target is the classroom for\n            ',
       ...
       pack_safe=True),
       locked_args={'course_type': REFERENCE_SOURCE}),
       from_participant=TunableTuple(description='\n                If selected we get the course slot id from the passed participant\n                i.e. pickedItemId\n                ',
       ...
       locked_args={'course_type': PARTICIPANT_SOURCE}),
     ...
     'during_office_hours_only':Tunable(description='\n            If checked, the office hour test set must pass for the course for this \n            test to pass.\n            ',
       tunable_type=bool,
       default=False)}

    def get_expected_args(self):
        return_args = {'subjects':self.subject, 
         'targets':self.target}
    ...
    def __call__(self, subjects=None, targets=None, career_guids=None, tooltip=None):
        ...
        for sim in subjects:
            ...
            if degree_tracker is None:
                ...
                return TestResult(False, "{} doesn't have degree tracker", sim,
                  tooltip=tooltip)
                ...
                if course_data is None:
                    ...
                    return TestResult(False, "{} doesn't have career {}", sim,
                      career_guid, tooltip=tooltip)
                for target in targets:
                    if not target.has_any_tag(course_data.classroom_tag[degree_tracker.get_university()]):
                        return TestResult(False, "{} isn't correct classroom", target,
                          tooltip=tooltip)
...
class UniversityTests(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    class AcceptedPrestigeDegreeCountTest(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if actor is None:
                ...
                return self.number_to_test.compare(num_acc_prestige_degrees) or TestResult(False, '{} of accepted prestige degrees, failed threshold {}:{}.', num_acc_prestige_degrees,
                  (self.number_to_test.comparison),
                  (self.number_to_test.value),
                  tooltip=tooltip)
            return TestResult.TRUE
    ...
    class CourseCareerSlotTest(HasTunableSingletonFactory, AutoFactoryInit):
            ...
            def test(self, actor, course_slots, invert, course_completion_status, tooltip=None):
                ...
                for career_slot in course_slots:
                    ...
                    if self.courses.test_item(course_data):
                        if invert:
                            return TestResult(False, "Actor {}'s course {} is associated with at least one of the specified course career slots", actor,
                              course_data,
                              tooltip=tooltip)
                        return TestResult.TRUE
                ...
                if invert:
                    return TestResult.TRUE
                return TestResult(False, "Actor {}'s course career slots and valid course datas do not match.", actor,
                  tooltip=tooltip)
        ...
        class _AnyCourse(HasTunableSingletonFactory, AutoFactoryInit):
            ...
            def test(self, actor, course_slots, invert, course_completion_status, tooltip=None):
                ...
                for career_slot in course_slots:
                    ...
                    if course_info is None:
                        continue
                    if not course_completion_status.test(actor, course_info):
                        continue
                    if invert:
                        return TestResult(False, 'Actor {} has valid course data in at least one of the course career slots.', actor,
                          tooltip=tooltip)
                    return TestResult.TRUE

                if invert:
                    return TestResult.TRUE
                return TestResult(False, 'Actor {} has no valid course data in any of the specified course career slots.', actor,
                  tooltip=tooltip)

        FACTORY_TUNABLES = {'actor':TunableEnumEntry(description='\n                The actor whose course career slot we will consider.\n                ',
           tunable_type=ParticipantTypeSingleSim,
           default=ParticipantTypeSingleSim.Actor), 
         'course_career_slots':TunableSet(description='\n                The set of course career slots to test. If the course data is \n                associated with any careers in the list, the test will evaluate to True.\n                ',
           tunable=TunablePackSafeReference(description='\n                    The course career slot we will test course data for.\n                    ',
           manager=(services.get_instance_manager(sims4.resources.Types.CAREER)),
           class_restrictions='UniversityCourseCareerSlot')), 
         'course_data':TunableVariant(description='\n                The course data we will test the course career slot(s) for.\n                ',
           specific_courses=_SpecificCourses.TunableFactory(),
           any_course=_AnyCourse.TunableFactory(),
           default='any_course'), 
         'invert':Tunable(description='\n                If checked, test will pass if all specified course career slots \n                do NOT match.\n                ',
           tunable_type=bool,
           default=False), 
         'course_completion_status':TunableVariant(description='\n                How we will test completed courses.\n                ',
           completed_courses_only=_CompletedCoursesOnly.TunableFactory(),
           ongoing_courses_only=_OngoingCoursesOnly.TunableFactory(),
           completed_and_ongoing_courses=_CompletedAndOngoingCourses.TunableFactory(),
           default='ongoing_courses_only')}
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if actor is None:
                return TestResult(False, "Actor {} doesn't exist.", (self.actor), tooltip=tooltip)
            return self.course_data.test(actor, (self.course_career_slots),
              (self.invert),
              (self.course_completion_status),
              tooltip=tooltip)
    ...
    class CourseGradeTest(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        class _KnownGrade(HasTunableSingletonFactory, AutoFactoryInit):
            ...
            def test(self, actor, final_grade, known_grade, grades_to_match, tooltip=None):
                if grades_to_match.test_item(known_grade):
                    return TestResult.TRUE
                return TestResult(False, "Actor {}'s course grade does not pass the white/black list.",
                  actor,
                  tooltip=tooltip)
        ...
        FACTORY_TUNABLES = {'actor':TunableEnumEntry(description='\n                The actor whose course grade we will test. \n                ',
           tunable_type=ParticipantTypeSingleSim,
           default=ParticipantTypeSingleSim.Actor), 
         'course_career_slot':TunablePackSafeReference(description='\n                The course career slot we will test the grade of.\n                ',
           manager=services.get_instance_manager(sims4.resources.Types.CAREER),
           class_restrictions='UniversityCourseCareerSlot'), 
         'grades':TunableWhiteBlackList(description="\n                The white/black list of grades that the sim's course grade must\n                match.\n                ",
           tunable=TunableEnumEntry(description='\n                    A grade to test against.\n                    ',
           tunable_type=Grade,
           default=(Grade.UNKNOWN))), 
         'grade_type':TunableVariant(description='\n                The grade type to compare against.\n                ',
           known_grade=_KnownGrade.TunableFactory(),
           final_grade=_FinalGrade.TunableFactory(),
           default='final_grade')}
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if course_data is None:
                return TestResult(False, "Actor {} doesn't have course data associated with career {}.", actor,
    ...
    class FinalRequirementTypeTest(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'actor':TunableEnumEntry(description='\n                The actor whose course career slot we will consider.\n                ',
         ...
         'course_career_slot':TunablePackSafeReference(description='\n                The course career slot we will test the final project type of.\n                ',
           manager=services.get_instance_manager(sims4.resources.Types.CAREER),
           class_restrictions='UniversityCourseCareerSlot'), 
         'final_requirement_type':TunableEnumEntry(description='\n                The final requirement type we will compare against.\n                ',
           tunable_type=FinalCourseRequirement,
           default=FinalCourseRequirement.EXAM)}
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if final_requirement_type is self.final_requirement_type:
                return TestResult.TRUE
            return TestResult(False, "Actor {}'s course {} has a final requirement type of {} but needed a final requirement type of {}", actor,
              course_data,
              (course_data.final_requirement_type),
              (self.final_requirement_type),
              tooltip=tooltip)
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if actor is None:
                ...
                if course_data.final_requirement_completed:
                    if self.invert:
                        return TestResult(False, 'Actor {} has not completed the final requirement for course {}.', actor,
                          course_data,
                          tooltip=tooltip)
            elif not self.invert:
                return TestResult(False, 'Actor {} has completed the final requirement for course {}.', actor,
                  course_data,
                  tooltip=tooltip)
            return TestResult.TRUE
    ...
    class GPATest(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'actor':TunableEnumEntry(description='\n                The actor whose GPA will be used. Must have a GPA greater than \n                or equal to the tuned GPA to pass the test.\n                ',
           tunable_type=ParticipantTypeSingleSim,
           default=ParticipantTypeSingleSim.Actor), 
         'gpa':TunableThreshold(description='\n                The GPA to compare against.\n                ',
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if gpa is not None:
                if self.gpa.compare(gpa):
                    return TestResult.TRUE
            return TestResult(False, 'Actor {} has a GPA of {}, but needs to have a GPA of {} or higher.', actor,
              gpa,
              (self.gpa),
              tooltip=tooltip)
    ...
    class HasDegreeToEnrollTest(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def test(self, tooltip=None, actors=()):
            ...
            if actor is None:
                ...
                return available_degrees or TestResult(False, 'No available degree to enroll for Actor {}.', actor, tooltip=tooltip)
            return TestResult.TRUE
    ...
    class HasSameMajorTest(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def test(self, tooltip=None, actors=(), targets=()):
            ...
            if actor_major is target_major:
                return TestResult.TRUE
            return TestResult(False, 'Actor {} has major {} , but Target {} has major {}.', actor,
              actor_major,
              target,
              target_major,
              tooltip=tooltip)
    ...
    class HasSameUniversityTest(HasTunableSingletonFactory, AutoFactoryInit):
        ...
        def test(self, tooltip=None, actors=(), targets=()):
            ...
            if actor_university is not target_university:
                if not self.invert:
                    return TestResult(False, 'Checking for same universities: Actor {} has university {} , but Target {} has university {}.', actor,
                      actor_university,
                      target,
                      target_university,
                      tooltip=tooltip)
            if actor_university is target_university:
                if self.invert:
                    return TestResult(False, 'Checking for different universities: Actor {} and Target {} both have university {}.', actor,
                      target,
                      target_university,
                      tooltip=tooltip)
            return TestResult.TRUE
    ...
    FACTORY_TUNABLES = {'test': TunableVariant(description='\n            The university test to perform.\n            ',
               accepted_prestige_degree_count_test=(AcceptedPrestigeDegreeCountTest.TunableFactory()),
               can_apply_test=(CanApplyTest.TunableFactory()),
               course_career_slot_test=(CourseCareerSlotTest.TunableFactory()),
               course_grade_test=(CourseGradeTest.TunableFactory()),
               final_requirement_completed_test=(FinalRequirementCompletedTest.TunableFactory()),
               final_requirement_type_test=(FinalRequirementTypeTest.TunableFactory()),
               gpa_test=(GPATest.TunableFactory()),
               has_degree_to_enroll_test=(HasDegreeToEnrollTest.TunableFactory()),
               has_same_degree_test=(HasSameMajorTest.TunableFactory()),
               has_same_university_test=(HasSameUniversityTest.TunableFactory()),
               default='has_degree_to_enroll_test')}
...
class UniversityProfessorTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
    ...
    class _ByParticipantType(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'course': OptionalTunable(description='\n                If enabled, the target Sim must be the professor of the\n                specified course. If not enabled the professor can be professor\n                of any course.\n                ',
                     ...
                     tunable_type=ParticipantType,
                     default=(ParticipantType.PickedItemId)),
                     student=TunableEnumEntry(description='\n                        The participant type to find the student in.\n                        ',
                     tunable_type=ParticipantTypeActorTargetSim,
                     default=(ParticipantTypeActorTargetSim.Actor))))}

        def get_expected_args(self):
            if self.course is not None:
                return {'course':self.course.course_participant, 
                 'students':self.course.student}
            return {}
                ...
                if course:
                    ...
                    for student in students:
                        ...
                        if degree_tracker is None:
                            ...
                            return target.has_trait(professor_trait) or TestResult(False, '{} is not the professor for {} in slot {}', target, course_data, career, tooltip=tooltip)

                else:
                    traits = target.trait_tracker.get_traits_of_type(TraitType.PROFESSOR)
                    return traits or TestResult(False, "{} doesn't have any PROFESSOR type traits.", target, tooltip=tooltip)
            ...
            for target in targets:
                for student in students:
                    ...
                    if degree_tracker is None:
                        ...
                        return target.trait_tracker.has_trait(course_data.professor_assignment_trait[student_university]) or TestResult(False, "{} is not in {}'s class for {}", student, target, (self.career_slot), tooltip=tooltip)
    ...
    FACTORY_TUNABLES = {'target':TunableEnumEntry(description='\n            The Sim to test the professorness of. If student is not specified\n            then test whether or not this Sim is the professor of ANY course.\n            ',
     ...
     'test':TunableVariant(description='\n            The test used to see if the target is a professor or not.\n            ',
       student=_ByStudent.TunableFactory(),
       participant=_ByParticipantType.TunableFactory(),
       career_slot=_ByCareerSlot.TunableFactory(),
       default='student'), 
