...
# Size of source mod 2**32: 144581 bytes
...
import alarms, caches, date_and_time, id_generator, objects.components.types, services, sims4.log
...
class DegreeTracker(SimInfoTracker):
    ...
    def _populate_gpa_thresholds(*args):
        ...
        for i in range(length_minus_one):
            GPA_THRESHOLDS.append(((sorted_grades[i][0] + sorted_grades[(i + 1)][0]) / 2,
             sorted_grades[i][1]))
    ...
    GRADE_INFO = TunableMapping(description='\n        Mapping of grade to info about that grade.\n        ',
      key_type=TunableEnumEntry(tunable_type=Grade,
      default=(Grade.UNKNOWN),
      invalid_enums=(
     Grade.UNKNOWN,)),
      value_type=TunableTuple(range=TunableInterval(description='\n                Performance range to receive this grade.\n                Lower bound is inclusive, upper bound is exclusive.\n                ',
      tunable_type=float,
      default_lower=0,
      default_upper=101),
      gp_value=TunableRange(description='\n                GP for this grade.\n                ',
      tunable_type=float,
      default=2.0,
      minimum=0),
    ...
    GRADE_REPORT_MAPPING = TunableMapping(description='\n        The mapping of all the possible in-progress grade report strings.\n        ',
      ...
      tunable_type=FinalCourseRequirement,
      default=(FinalCourseRequirement.NONE)),
    ...
    GRADE_SKILL_MODIFICATION = TunableCurve(description='\n        A mapping of skill delta to the amount that grade progress will be\n        modified\n        ',
      x_axis_name='Skill Delta',
      y_axis_name='Grade modification')
    ...
    ENROLLMENT_STATUS_LOOT_MAP = TunableMapping(description='\n        Mapping of degree enrollment status to loot actions.  Loot will be\n        applied when changing to mapped enrollment status.\n        ',
      key_type=TunableEnumEntry(tunable_type=EnrollmentStatus,
      default=(EnrollmentStatus.NONE),
      invalid_enums=(
    ...
    _ALL_UNIVERSITY_COURSE_CAREER_SLOTS = None

    @staticproperty
    def ALL_UNIVERSITY_COURSE_CAREER_SLOTS():
        if DegreeTracker._ALL_UNIVERSITY_COURSE_CAREER_SLOTS is None:
            career_manager = services.get_instance_manager(sims4.resources.Types.CAREER)
            DegreeTracker._ALL_UNIVERSITY_COURSE_CAREER_SLOTS = list(career_manager.get_ordered_types(only_subclasses_of=UniversityCourseCareerSlot))
        return DegreeTracker._ALL_UNIVERSITY_COURSE_CAREER_SLOTS
    ...
    def load(self, data):
        ...
        for course_info in data.current_courses:
            ...
            self._course_infos[course_info.course_slot] = DegreeTracker.CourseInfo(course_data, final_grade=(Grade(course_info.final_grade)),
              known_grade=(Grade(course_info.known_grade)),
              lectures=(course_info.lectures),
              final_completed=(course_info.final_requirement_completed),
              homework_cheated=(course_info.homework_cheated),
              initial_skill=(course_info.initial_skill))
        ...
        if data.HasField('reenrollment_time') or data.enrollment_status == EnrollmentStatus.SUSPENDED:
    ...
    def show_delayed_reenrollment_dialog(self):
        if DegreeTracker.IS_REENROLLMENT_DIALOGS_IN_PROCESS:
            return False
        if not self._show_reenrollment_dialog_on_spin_up:
            if not self._show_reenrollment_dialog_on_next_turn:
                return False
        self._show_reenrollment_dialog_on_spin_up = False
        self._show_reenrollment_dialog_on_next_turn = False
    ...
    def get_career_course_slots--- This code section failed: ---

 L. 941         0  BUILD_LIST_0          0 
                2  STORE_FAST               'active_course_slots'

 L. 942         4  LOAD_FAST                'self'
                6  LOAD_ATTR                _sim_info
                8  LOAD_ATTR                career_tracker
               10  LOAD_ATTR                careers
               12  STORE_FAST               'careers'

 L. 943        14  LOAD_FAST                'careers'
               16  POP_JUMP_IF_FALSE    62  'to 62'

 L. 944        18  SETUP_LOOP           86  'to 86'
               20  LOAD_FAST                'self'
               22  LOAD_ATTR                _course_infos
               24  LOAD_METHOD              keys
               26  CALL_METHOD_0         0  '0 positional arguments'
               28  GET_ITER         
             30_0  COME_FROM            40  '40'
               30  FOR_ITER             58  'to 58'
               32  STORE_FAST               'course_info'

 L. 945        34  LOAD_FAST                'course_info'
               36  LOAD_FAST                'careers'
               38  COMPARE_OP               in
               40  POP_JUMP_IF_FALSE    30  'to 30'

 L. 946        42  LOAD_FAST                'active_course_slots'
               44  LOAD_METHOD              append
               46  LOAD_FAST                'careers'
               48  LOAD_FAST                'course_info'
               50  BINARY_SUBSCR    
               52  CALL_METHOD_1         1  '1 positional argument'
               54  POP_TOP          
               56  JUMP_BACK            30  'to 30'
               58  POP_BLOCK        
               60  JUMP_FORWARD         86  'to 86'
             62_0  COME_FROM            16  '16'

 L. 947        62  LOAD_FAST                'self'
               64  LOAD_ATTR                _course_infos
               66  POP_JUMP_IF_FALSE    86  'to 86'

 L. 948        68  LOAD_GLOBAL              logger
               70  LOAD_ATTR                error
               72  LOAD_STR                 'Careers is none, but course info is present for Sim: {}'
               74  LOAD_FAST                'self'
               76  LOAD_ATTR                _sim_info
               78  LOAD_STR                 'amohananey'
               80  LOAD_CONST               ('owner',)
               82  CALL_FUNCTION_KW_3     3  '3 total positional and keyword args'
               84  POP_TOP          
             86_0  COME_FROM            66  '66'
             86_1  COME_FROM            60  '60'
             86_2  COME_FROM_LOOP       18  '18'

 L. 949        86  LOAD_FAST                'active_course_slots'
               88  RETURN_VALUE     
               -1  RETURN_LAST      
...
Parse error at or near `COME_FROM_LOOP' instruction at offset 86_2
    ...
    def get_grade(self, performance):
        for grade, grade_info in self.GRADE_INFO.items():
            if grade_info.range.lower_bound <= performance < grade_info.range.upper_bound:
                return grade
    ...
    def update_final_requirement_completion(self, career, completion_status):
        ...
        if course_info.lectures >= self.COURSE_LECTURE_COUNT:
            if course_info.final_requirement_completed:
    ...
    def handle_professor_assignment(self, course_info):
        ...
        result = services.sim_filter_service().submit_filter(sim_filter=dynamic_professor_filter, allow_yielding=False,
          callback=None,
          gsi_source_fn=(lambda : 'DegreeTracker: Check if professor is assigned to {} '.format(str(course_info))))
        if not result:
    ...
    def complete_term(self):
        ...
        if num_courses > 0:
            ...
            if self._current_credits >= self.CREDITS_REQUIRED:
                ...
                if term_gpa < self.GPA_PROBATION_THRESHOLD:
            ...
            if self._sim_info.is_selectable:
                ...
                for term_end_dialog in self.TERM_END_DIALOGS(resolver=resolver):
                    ...
                    end_term_dialog.show_dialog(row_data=grade_entries, additional_tokens=(
                     self.get_gpa_string(term_gpa),
                     self.get_gpa_string(overall_gpa),
                     enrollment_status_text),
                      on_response=(self._on_end_term_dialog_response))
    ...
    def _setup_mail_diploma_alarm(self, time_till_mail_diploma=None):
        ...
        self._diploma_handle = alarms.add_alarm(self, time_till_mail_diploma, (self._diploma_callback),
          cross_zone=True)
    ...
    def graduate(self, gpa=None):
        ...
        if gpa is None:
            ...
            return self._sim_info.is_selectable or None
    ...
    def _setup_reenrollment_alarm(self, duration_in_days=None, time_till_reenrollment=None):
        ...
        self._reenrollment_handle = alarms.add_alarm(self, time_till_reenrollment, (self._reenrollment_callback),
          cross_zone=True)
    ...
    def _reenrollment_callback(self, _):
        ...
        if self._enrollment_status == EnrollmentStatus.SUSPENDED:
            ...
            if self._enrollment_status == EnrollmentStatus.DROPOUT:
    ...
    def _set_enrollment_status(self, enrollment_status, from_complete_term=False):
        ...
        if enrollment_status == EnrollmentStatus.SUSPENDED:
            ...
            if not enrollment_status == EnrollmentStatus.NONE:
                if enrollment_status == EnrollmentStatus.DROPOUT or enrollment_status == EnrollmentStatus.NOT_ENROLLED:
            ...
            for degree in University.ALL_DEGREES:
                ...
                if is_prestige:
                    if degree.can_sim_be_accepted(self._sim_info, degree.acceptance_score):
                        ...
                        resolver = SingleSimResolver(self._sim_info)
                        if degree.basic_acceptance_tests.run_tests(resolver):
                            self.set_accepted_degree(university, degree)
    ...
    def _setup_start_of_term_alarm(self, time_till_start_term=None):
        ...
        if time_till_start_term is None:
            ...
            time_till_start_term is None or alarm_data or logger.error('Unable to find start term time for Sim {} based on current time {}', self._sim_info, str(now))
            return
        ...
        self._start_of_term_handle = alarms.add_alarm(self, time_till_start_term, (self._start_of_term_callback),
          cross_zone=True)
    ...
    def _setup_end_of_term_alarm(self, time_till_end_term=None):
        ...
        self._end_of_term_handle = alarms.add_alarm(self, time_till_end_term, (self._end_of_term_callback),
          cross_zone=True)
    ...
    def _setup_daily_update_alarm(self):
        ...
        self._daily_update_handle = alarms.add_alarm(self, (time_till_daily_update + TimeSpan.ONE), (self._daily_update_callback),
          cross_zone=True)
    ...
    def _set_degree_info_slot(self):
        ...
        course_slots = DegreeTracker.ALL_UNIVERSITY_COURSE_CAREER_SLOTS
    ...
    def _get_courses_to_take(self, major, university, course_count, credit_count, courses=()):
        if not self.TERM_COURSE_COUNT.lower_bound <= course_count <= self.TERM_COURSE_COUNT.upper_bound:
        ...
        while core_class_count > 0 and start_index < major_course_count:
            current_index = start_index
            while 1:
                if current_index >= 0:
                    course = major.courses[current_index]
                    if course not in courses_to_take:
    ...
    def _validate_and_update_courses_to_take(self, courses_to_take):
        ...
        if paper_course_count > paper_schedule_count or presentation_course_count > presentation_schedule_count:
            ...
            for course in courses_to_take:
                if course.final_requirement_type == FinalCourseRequirement.PAPER:
                    if paper_course_count > paper_schedule_count:
                        courses_to_take.remove(course)
                        if replacement_course is not None:
                            courses_to_take.append(replacement_course)
                        break
                if course.final_requirement_type == FinalCourseRequirement.PRESENTATION:
                    if presentation_course_count > presentation_schedule_count:
                        courses_to_take.remove(course)
                        if replacement_course is not None:
                            courses_to_take.append(replacement_course)
                    break
    ...
    def enroll(self, major, university, course_count, courses=()):
        if not self.allowed_to_enroll:
            ...
            if not self.is_accepted_degree(university, major):
                ...
                return
            if self._current_major is not major:
                if self._current_major is not None:
                    self.switch_majors(major)
            self._current_major = major
            if self._current_university is not None:
                if self._current_university is not university:
            ...
            career_manager = services.get_instance_manager(sims4.resources.Types.CAREER)
            available_course_slots = list(DegreeTracker.ALL_UNIVERSITY_COURSE_CAREER_SLOTS)
    ...
    def get_grade_report(self, career_guid):
        ...
        for performance_tuple in performance_strings_list:
            ...
            if performance_range.lower_bound <= performance < performance_range.upper_bound:
                ...
                if final_requirement_text_map:
                    if course_data.final_requirement_type in final_requirement_text_map.keys():
                ...
                icon_override = IconInfoData(icon_resource=(course_data.icon)) if course_data.icon is not None else DEFAULT
                report_dialog.show_dialog(icon_override=icon_override, additional_tokens=(
                 course_name, text, additional_text))
                return
    ...
    def get_major_status(self, university_id, major_id):
        ...
        if self._accepted_degrees is not None:
            if university_id in self._accepted_degrees:
                if major_id in self._accepted_degrees[university_id]:
    ...
    def assign_professor_to_course(self, course_info, professor_assignment_trait):
        ...
        professor = services.sim_filter_service().submit_matching_filter(sim_filter=archetype, callback=None,
          number_of_sims_to_find=1,
          allow_yielding=False,
          gsi_source_fn=(lambda : 'DegreeTracker: Assigning professor with {} to course'.format(str(professor_assignment_trait))))
        if professor:
    ...
    def get_scholarship_list_helper(self, header_string, scholarships, subject, first_list_entry, with_values=False):
        if not scholarships:
            return
        elif first_list_entry:
            list_entries = [
             header_string]
        else:
            list_entries = [
             EMPTY_LINE, header_string]
    ...
    def get_scholarships_list_by_status(self, status, subject, first_list_entry):
        ...
        if status == ScholarshipStatus.ACTIVE:
            list_entries = self.get_scholarship_list_helper((GetScholarshipStatusLoot.SCHOLARSHIP_STATUS_HEADER_ENTRY_ACTIVE), (self.get_active_scholarships()),
              subject,
              first_list_entry,
              with_values=True)
        else:
            if status == ScholarshipStatus.REJECTED:
                ...
                if status == ScholarshipStatus.ACCEPTED:
                    list_entries = self.get_scholarship_list_helper((GetScholarshipStatusLoot.SCHOLARSHIP_STATUS_HEADER_ENTRY_ACCEPTED), (self.get_accepted_scholarships()),
                      subject,
                      first_list_entry,
                      with_values=True)
                else:
                    if status == ScholarshipStatus.PENDING:
        ...
        for action in loot.loot_actions:
            if isinstance(action, ObjectRewardsOperation):
                if scholarship is ScholarshipTuning.MERIT_SCHOLARSHIP:
                    continue
                action.apply_with_post_create(self._sim_info, resolver, lambda *args: (self._post_create_func)(*args, *(scholarship,)))
            else:
    ...
    def process_scholarship_application(self, scholarship, delay_time):
        evaluation_alarm_handle = alarms.add_alarm(self, delay_time, (lambda _: self.evaluate_scholarship(scholarship)),
          cross_zone=True)
