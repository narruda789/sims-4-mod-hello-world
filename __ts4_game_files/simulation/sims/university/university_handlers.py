...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\university\university_handlers.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 10651 bytes
    ...
    if accepted_degrees is not None:
        for university_id, degree_ids in accepted_degrees.items():
            ...
            for degree_id in degree_ids:
                ...
                degrees_data.append({'degree':degree.__name__, 
                 'university':university.__name__, 
                 'prestige':degree in university.prestige_degrees})
    ...
    for university_id, degree_ids in not_yet_accepted_degrees.items():
        ...
        for degree_id in degree_ids:
            ...
            degrees_data.append({'degree':degree.__name__, 
             ...
             'degree_score':str(degree.acceptance_score.score_threshold.value) if is_prestige else 'Not Required', 
             'sim_score':str(degree.get_sim_acceptance_score(sim_info, degree.acceptance_score)) if is_prestige else '', 
             'is_qualify':degree.can_sim_be_accepted(sim_info, degree.acceptance_score) if is_prestige else True})
...
def get_current_courses(sim_info):
    ...
    for slot_id, course_info in degree_tracker.course_infos.items():
        ...
        data.append({'slot':course_slot_career_ref.__name__, 
         'course':course_data.__name__, 
         'core':degree_tracker.is_core_course(course_data), 
         'grade':None if course_slot is None else degree_tracker.get_grade(course_slot.work_performance).name, 
         'work performance':None if course_slot is None else course_slot.work_performance})
...
def get_previous_courses(sim_info):
    ...
    for course_data in degree_tracker.get_previous_courses():
        data.append({'course':course_data.__name__, 
         'core':degree_tracker.is_core_course(course_data)})
...
def generate_sim_university_view_data(sim_id: int=None):
    ...
    for sim_info in list(sim_info_manager.objects):
        ...
        if degree_tracker is None:
            continue
        university_data = {'sim':sim_info.full_name,  'sim_id':str(sim_info.sim_id), 
         'university':degree_tracker.get_university().__name__ if degree_tracker.get_university() is not None else 'None', 
         'major':degree_tracker.get_major().__name__ if degree_tracker.get_major() is not None else 'None', 
         'status':degree_tracker.get_enrollment_status().name, 
         'gpa':degree_tracker.get_gpa() if degree_tracker.get_gpa() is not None else 'None', 
         'credits':degree_tracker.current_credits, 
         'term_start':str(degree_tracker.term_started_time), 
         'current_day':str(degree_tracker.get_current_day_of_term())}
