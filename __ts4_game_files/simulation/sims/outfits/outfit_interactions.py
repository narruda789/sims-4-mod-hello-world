...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\outfits\outfit_interactions.py
# Compiled at: 2019-03-21 20:44:08
# Size of source mod 2**32: 14776 bytes
import functools
...
class OutfitChangeSituation(HasTunableSingletonFactory, AutoFactoryInit):
            ...
            if situation_job is not None:
                ...
                for entry in outfit_generators:
                    if entry.tests.run_tests(resolver):
                        yield AffordanceObjectPair(affordance, target, affordance, None, pie_menu_cateogory=affordance.category, 
                         outfit_tags=entry.generator.tags, **kwargs)
    ...
    def get_outfit_for_clothing_change(self, sim_info, outfit_change_category):
        return sim_info.get_outfit_for_clothing_change(None, (OutfitChangeReason.DefaultOutfit),
...
class _XevtOutfitChangeElement(ParentElement):
    ...
    def _run_xevt_outfit_change(self):
        ...
        sim.sim_info.generate_outfit((self._outfit_category), outfit_index=0,
          tag_list=(self._tag_list),
          filter_flag=(self._filter_flag),
          body_type_chance_overrides=(self._body_type_chance_overrides),
          body_type_match_not_found_overrides=(self._body_type_match_not_found_policy))
    ...
    def _run(self, timeline):
        ...
        def register_xevt(_):
            self._xevt_handle = self._interaction.animation_context.register_event_handler((lambda _: self._run_xevt_outfit_change()),
              handler_id=(self._xevt_id))
...
class OutfitChangeSelfInteraction(OutfitGeneratorRandomizationMixin, SuperInteraction):
    ...
    def build_basic_elements(self, sequence=()):
        ...
        if self.xevt_id is None:
            ...
            generate_outfit_fn = functools.partial((self._generate_outfit), (self.sim.sim_info),
              (self.outfit_change_category),
              outfit_index=0,
              tag_list=(self.outfit_tags))
            ...
            sequence = build_critical_section(sequence, self.sim.sim_info.get_change_outfit_element_and_archive_change_reason(new_outfit,
              interaction=self, change_reason=self), flush_all_animations)
