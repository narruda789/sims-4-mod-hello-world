...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\outfits\outfit_utils.py
...
class OutfitGeneratorRandomizationMixin:
    ...
    def _generate_outfit(self, sim_info, outfit_category, outfit_index=0, tag_list=(), seed=None):
        if outfit_handlers.archiver.enabled:
            outfit_handlers.log_outfit_generate(sim_info, outfit_category, outfit_index, tag_list, self.filter_flag, self.body_type_chance_overrides, self.body_type_match_not_found_policy)
        body_type_flags = BodyTypeFlag.CLOTHING_ALL | self.get_body_type_flags()
        sim_info.generate_outfit(outfit_category, outfit_index=outfit_index,
          tag_list=tag_list,
          filter_flag=(self.filter_flag),
          body_type_chance_overrides=(self.body_type_chance_overrides),
          body_type_match_not_found_overrides=(self.body_type_match_not_found_policy),
          seed=seed,
          body_type_flags=body_type_flags)