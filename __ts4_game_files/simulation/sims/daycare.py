...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\sims\daycare.py
# Compiled at: 2021-09-01 13:58:18
# Size of source mod 2**32: 47509 bytes
...
import enum, singletons, services, sims4.log
...
class DaycareTuning:
    ...
    NOTIFICATIONS = TunableMapping(description='\n        Notifications to send when sims/pets are sent to daycare facilities or\n        brought back.\n        facility type -> send/bring back -> single/multiple -> notification\n        ',
      ...
      tunable_type=DaycareNotificationType,
      default=(DaycareNotificationType.BABY_DAYCARE)),
      value_type=TunableMapping(description='\n            Notifications to send when sims/pets are sent to daycare facilities or\n            brought back.\n            send/bring back -> single/multiple -> notification\n            ',
      key_type=TunableEnumEntry(description='\n                Notifications for Sending or Returning.\n                ',
      tunable_type=DaycareNotificationDestination,
      default=(DaycareNotificationDestination.SEND_AWAY)),
      ...
      tunable_type=DaycareNotificationCount,
      default=(DaycareNotificationCount.SINGLE)),
...
class DaycareService(Service):
        ...
        if sim_info.is_baby:
            ...
            if sim_info.is_toddler:
                ...
                if daycare_interaction is None:
                    ...
                    context = InteractionContext(sim, (InteractionContext.SOURCE_SCRIPT), (Priority.High), insert_strategy=(QueueInsertStrategy.FIRST),
                      must_run_next=True)
        ...
        if sim_info.zone_id == residence_zone_id:
            if sim_info.zone_id != current_zone_id:
                if sim_info.career_tracker is None or sim_info.career_tracker.currently_at_work or services.hidden_sim_service().is_hidden(sim_info.id):
                    return False
                return True
            if sim_info not in self._unavailable_sims:
                return True
        return False
    ...
    def _start_nanny_service(self, household, sim_infos, send_to_daycare_callback_fn):
        ...
        hire_nanny_dialog.show_dialog(additional_tokens=(DaycareTuning.NANNY_SERVICE_NPC.cost_up_front,
         DaycareTuning.NANNY_SERVICE_NPC.cost_hourly),
    ...
    def is_daycare_service_npc_available(self, sim_info=None, household=None):
        ...
        if household.considers_current_zone_its_residence():
            ...
            if DaycareTuning.BUTLER_SERVICE_NPC is not None:
                ...
                if butler_situation is not None and butler_situation.is_in_childcare_state:
                    if sim_info is None:
                        return True
    ...
    def _enable_daycare_or_nanny_if_necessary(self, household):
        ...
        for sim_info in nanny_sim_infos:
            if not sim_info.trait_tracker.has_trait(DaycareTuning.NANNY_TRAIT_ON_KIDS_AND_PETS):
                if sim_info.is_pet:
                ...
                sim_info.is_at_home or sim_info.inject_into_inactive_zone(sim_info.vacation_or_home_zone_id)
        ...
        for residence_zone_id in household.zone_ids_considered_residence_gen(nanny_sim_infos):
            if not self._is_any_sim_available(household, residence_zone_id):
                ...
                if not daycare_sim_infos:
                    continue
                if is_active_household:
                    if current_zone_id == residence_zone_id:
                        if self._start_nanny_service(household, tuple(daycare_sim_infos), _on_send_to_daycare):
                            continue
            ...
            for residence_zone_id in household.zone_ids_considered_residence_gen():
                if self._is_any_sim_available(household, residence_zone_id):
                    ...
                    for sim_info in tuple(daycare_sim_infos):
                        if self._remove_daycare_effects_from_sim(sim_info):
                            if sim_info in returning_sim_infos:
                                daycare_sim_infos.remove(sim_info)
                            if self.is_sim_info_at_daycare(sim_info):
                                self.remove_sim_info_from_daycare(sim_info)

                    returned_children or self._show_notification(DaycareNotificationType.BABY_DAYCARE, DaycareNotificationDestination.BRING_BACK, household, daycare_sim_infos)
    ...
    def get_abandoned_toddlers(self, household, residence_zone_id=None, sims_infos_to_ignore=()):
        ...
        for sim_info in household:
            ...
            if residence_zone_id is not None:
                if sim_info.vacation_or_home_zone_id != residence_zone_id:
                    continue
            if not sim_info.is_toddler:
    ...
    def get_sim_infos_for_nanny(self, household, check_for_vacation=True):
        ...
        for sim_info in household:
            if not sim_info.is_child_or_younger:
                if not sim_info.is_pet:
                    continue
                else:
                    if sim_info.is_in_travel_group():
                        continue
                    if check_for_vacation and sim_info.zone_id == current_zone_id:
                        continue
    ...
    def refresh_daycare_status(self, baby):
        ...
        if household is None:
            return
        elif self.is_daycare_enabled(household, baby.zone_id):
    ...
    def _show_notification(self, facility_type, destination, household, sim_infos):
        if not sim_infos:
            ...
            if not services.current_zone().is_zone_running:
                return
            return household.is_active_household or None
