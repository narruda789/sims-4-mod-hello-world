...
# Size of source mod 2**32: 24618 bytes
...
import build_buy, camera, distributor, placement, services, sims4, tag
...
class Baby(GameObject, HasSimInfoBasicMixin):
    ...
    def get_baby_skin_tone_state(cls, sim_info):
        ...
        baby_skin_enum, _ = BabyTuning.get_baby_skin_tone_info(sim_info)
    ...
    def on_state_changed(self, state, old_value, new_value, from_init):
        ...
        if self._sim_info is not None:
            ...
            if sim_info_associated_baby is None:
                pass
            elif sim_info_associated_baby.id is not self.id:
                self.baby_cloth = BabyTuning.get_baby_skin_tone_info(self._sim_info)[1]
            else:
                self._sim_info.resend_baby_skin_tone()
