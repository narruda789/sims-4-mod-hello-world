...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\event_testing\scholarship_tests.py
# Compiled at: 2019-09-26 20:12:01
...
class ScholarshipStatusTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    class _SpecificScholarships(HasTunableSingletonFactory, AutoFactoryInit):
        FACTORY_TUNABLES = {'_scholarships':TunableWhiteBlackList(description='\n                Scholarships against which to test against application status.\n                ',
         ...
         '_status':TunableEnumEntry(description='\n                Status of the scholarship(s).\n                ',
        ...
        def is_valid_scholarship(self, sim_info, tooltip=None):
            ...
            if degree_tracker is None:
                return TestResult(False, '{} has no degree tracker.', sim_info,
                  tooltip=tooltip)
            ...
            def _scholarship_container_helper(scholarships_of_status, sim_info, tooltip):
                ...
                if not self._scholarships.test_collection(scholarship_insts_of_status):
                    return TestResult(False, "{0}'s scholarships do not match the tuned whitelist/blacklist.", sim_info,
                      tooltip=tooltip)
            ...
            if degree_tracker is None:
                ...
                if self._status == ScholarshipStatus.ACTIVE:
                    if not degree_tracker.get_active_scholarships():
                        if self.negate:
                            return TestResult.TRUE
                        return TestResult(False, '{} has no active scholarships', sim_info,
                          tooltip=tooltip)
            elif self._status == ScholarshipStatus.REJECTED and not degree_tracker.get_rejected_scholarships():
                if self.negate:
                    return TestResult.TRUE
                return TestResult(False, '{} has no rejected scholarships', sim_info,
                  tooltip=tooltip)
            else:
                if self._status == ScholarshipStatus.ACCEPTED and not degree_tracker.get_accepted_scholarships():
                    if self.negate:
                        return TestResult.TRUE
                    return TestResult(False, '{} has no accepted scholarships', sim_info,
                      tooltip=tooltip)
                else:
                    if self._status == ScholarshipStatus.PENDING:
                        if not degree_tracker.get_pending_scholarships():
                            if self.negate:
                                return TestResult.TRUE
                            return TestResult(False, '{} has no pending scholarships', sim_info,
                              tooltip=tooltip)
            if self.negate:
                return TestResult(False, "{}'s scholarships are of                                        status ({}), and cannot  be.",
                  sim_info,
                  (self._status),
                  tooltip=tooltip)
            return TestResult.TRUE
    ...
    FACTORY_TUNABLES = {'target':TunableEnumEntry(description='\n            The Sim against which to test scholarship status.\n            ',
     ...
     'test':TunableVariant(description='\n            The test used to check scholarship status.\n            ',
       specific_scholarship=_SpecificScholarships.TunableFactory(),
       any_scholarship=_AnyScholarship.TunableFactory(),
       default='specific_scholarship')}
