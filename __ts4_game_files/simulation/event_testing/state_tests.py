...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\event_testing\state_tests.py
# Compiled at: 2020-11-12 10:43:18
# Size of source mod 2**32: 7535 bytes
...
import algos, event_testing.test_base, services, sims4.resources
...
class StateTest(event_testing.test_base.BaseTest):
    ...
    FACTORY_TUNABLES = {'description':"\n        Gate availability by object state.  By default, the test will use the\n        state's linked stat as a fallback in case the target doesn't have the\n        state involved.\n        ", 
     ...
     'value':TunablePackSafeReference(description='\n            The value to compare to.',
     ...
     'fallback_behavior':TunableVariant(description="\n            What to do if the given object doesn't have the state in question.\n            ",
       default=ALWAYS_FAIL,
       locked_args={ALWAYS_PASS: ALWAYS_PASS, 
      ALWAYS_FAIL: ALWAYS_FAIL})}
        ...
        for target in test_targets:
            ...
            if self.operator_enum.category == sims4.math.Operator.EQUAL:
                if not self.operator(curr_value, self.value):
                    ...
                    operator_symbol = self.operator(curr_value.value, self.value.value) or self.operator_enum.symbol
                    return TestResult(False, '{} failed state check: {}.{} {} {} (current value: {})', (self.who.name), (target.__class__.__name__), (self.value.state), operator_symbol, (self.value), curr_value, tooltip=(self.tooltip))
    ...
    def _get_make_true_value(self):
        ...
        return (
         TestResult(False, 'Could not find value to satisfy operation: {} {} {}', self.value.state if self.value is not None else '<Unavailable>', operator_symbol, self.value), None)
    ...
    def __call__(self, test_targets=tuple()):
        for target in test_targets:
            if target.state_component is None:
                return TestResult(False, '{} does not have a state component',
                  target,
                  tooltip=(self.tooltip))
                current_states = list(target.state_component.values())
                return self.states.test_collection(current_states) or TestResult(False, "{}'s current states do not match the WhiteBlackList that has been defined.",
                  target,
                  tooltip=(self.tooltip))
