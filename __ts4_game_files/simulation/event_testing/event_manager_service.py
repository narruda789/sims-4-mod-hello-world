...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\event_testing\event_manager_service.py
# Compiled at: 2020-07-27 20:03:11
...
import caches, event_testing.resolver, services, sims4.log
...
class DataMapHandler:
    ...
    def __call__(self, func):
        ...
        if callbacks is None:
            data_store_event_test_event_callback_map[self.event_enum] = [
             func.__name__]
        else:
...
class EventManagerService(Service):
    ...
    def _is_valid_handler(self, handler, event_types):
        if hasattr(handler, 'handle_event'):
            return True
        logger.error('Cannot register {} due to absence of expected callback method.  Registered event_types: {}.', handler,
          event_types, owner='manus')
        return False
    ...
    def register_single_event(self, handler, event_type):
        logger.assert_raise((self._enabled), 'Attempting to register event:{} \n            with handler:{} when the EventManagerService is disabled.',
          (str(event_type)),
          (str(handler)), owner='sscholl')
    ...
    def _process_test_event(self, sim_info, event_type, custom_keys=tuple(), **kwargs):
        ...
        if handlers:
            ...
            return original_handlers or None
        resolver = event_testing.resolver.DataResolver(sim_info, event_kwargs=kwargs, custom_keys=custom_keys)
