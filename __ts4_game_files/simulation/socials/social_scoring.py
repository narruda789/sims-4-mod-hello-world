...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\social_scoring.py
# Compiled at: 2020-03-05 17:57:07
# Size of source mod 2**32: 10312 bytes
...
import sims4.math, socials.geometry, terrain
...
class PetGroupCostFunction(HasTunableFactory, AutoFactoryInit, CostFunctionBase):
    ...
    def constraint_cost(self, position, orientation, routing_surface):
        ...
        if angle <= PetGroupCostFunction.SIDE_ARC_START:
            if angle >= PetGroupCostFunction.SIDE_ARC_END:
                return -3.0
        return 0.0
...
class ThrowingGroupCostFunction(HasTunableFactory, AutoFactoryInit, CostFunctionBase):
    ...
    def _score_location(self, position):
        if self.location_tests.validate_snowmask is not None:
            if terrain.get_snowmask_value(position) > self.location_tests.validate_snowmask:
                return ThrowingGroupCostFunction.INVALID_GOAL_SCORE
        if self.location_tests.validate_is_outside is not None:
            if self.location_tests.validate_is_outside != is_location_outside(position, self._sim.level):
                return ThrowingGroupCostFunction.INVALID_GOAL_SCORE
        return 0.0
