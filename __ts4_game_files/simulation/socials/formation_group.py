...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\socials\formation_group.py
# Compiled at: 2021-05-04 13:25:15
# Size of source mod 2**32: 22730 bytes
from itertools import islice
from interactions.interaction_finisher import FinishingType
from event_testing.test_events import TestEvent
import enum, random, services
...
class FormationSocialGroup(SocialGroup, RouteEventProviderMixin):
    INSTANCE_TUNABLES = {'routing_object':TunableReference(description='\n            An object with a routing component that will be used as the anchor object for this group.\n            ',
     ...
     'routing_events':TunableList(description='\n            Events and weight for paired events.\n            Only one pair can occur within the specified routing event delay.\n            To avoid overlap, minimum delay must be longer than duration of\n            route event.\n            ',
       ...
       pack_safe=True),
       target_event=RouteEventPaired.TunableReference(description='\n                    A paired route event for the sim tested as the target sim.\n                    ',
       pack_safe=True),
    ...
    def handle_event(self, _, event, resolver):
        if event == TestEvent.ObjectDestroyed:
            ...
            if self.anchor is destroyed_obj:
                logger.warn("Anchor object of formation social group {} has been destroyed while the formation is running. This shouldn't happen and may cause strange visual behavior.", self)
                self.shutdown(FinishingType.OBJECT_CHANGED)
    ...
    def shutdown(self, finishing_type):
        ...
        if self.anchor:
            if self._state == FormationSocialGroupState.ROUTING:
                self.anchor.routing_component.unregister_routing_stage_event(RoutingStageEvent.OBJECT_ROUTE_FAIL, self._on_anchor_route_fail)
    ...
    def _add(self, sim, attaching_si):
        ...
        if self._state == FormationSocialGroupState.WAITING_FOR_SIMS:
    ...
    def route_to_waypoint(self, locator_ids=None):
        if locator_ids:
            object_routing_component = self.anchor.routing_component.get_object_routing_component()
            object_routing_component.locators = locator_ids
        self._start_routing()
    ...
    def _start_routing(self):
        ...
        self.anchor.routing_component.register_routing_stage_event(RoutingStageEvent.OBJECT_ROUTE_FAIL, self._on_anchor_route_fail)
    ...
    def _on_anchor_route_fail(self, *_, **__):
        if self._situation is not None:
            self._situation.on_anchor_route_fail()
    ...
    def provide_route_events(self, route_event_context, sim, path, start_time=0, **kwargs):
        if self._planned_route_events is None:
            ...
            self._planned_route_events or self._populate_planned_route_events(start_time, duration)
        for route_event in self._planned_route_events[sim]:
            if route_event.processed:
                continue
            if route_event.time < start_time:
                continue
            if route_event_context.route_event_already_scheduled((type(route_event)), provider=self, time=(route_event.time), epsilon_override=EPSILON):
                continue
...
lock_instance_tunables(FormationSocialGroup, adjust_sim_positions_dynamically=False,
