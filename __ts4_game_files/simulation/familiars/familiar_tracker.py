...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\familiars\familiar_tracker.py
# Compiled at: 2020-03-02 17:33:19
# Size of source mod 2**32: 32585 bytes
import id_generator
...
class FamiliarTracker(SimInfoTracker):
    FAMILIAR_DATA = TunableMapping(description='\n        A mapping between the familiar type and data associated with that familiar type.\n        ',
      ...
      disabled_name='pet_based_familiar',
      enabled_name='object_based_familiar')))
    ...
    PET_FAMILIAR_BUFF = TunableMapping(description='\n        A buff added to pet familiars based on age and species.\n        ',
      ...
      tunable_type=Age,
      default=(Age.ADULT)),
      value_type=TunableMapping(key_type=TunableEnumEntry(description='\n                The species that this buff will be applied to.\n                ',
    ...
    def unbind_familiar(self, familiar_uid):
        ...
        if self._active_familiar_id is not None:
            if self._active_familiar_id == familiar_uid:
            ...
            try:
                ...
                if services.current_zone().is_zone_running:
                    if pet_familiar is None:
                        ...
                        context = InteractionContext(pet_familiar, (InteractionContext.SOURCE_SCRIPT),
                          (Priority.Critical),
                          insert_strategy=(QueueInsertStrategy.NEXT))
                        pet_familiar.push_super_affordance(FamiliarTracker.PET_FAMILIAR_SET_ACTIVE_AFFORDANCE, None, context)
            except Exception as e:
                try:
                    self._on_familiar_summon_failure('Exception encountered when trying to create familiar.  Deactivating familiar.', familiar_object=pet_familiar,
                      exc=e)
                finally:
                    e = None
                    del e
        ...
        if familiar is None:
            try:
                ...
                if not FamiliarTracker.FAMILIAR_PLACEMENT.try_place_object(familiar, resolver):
                    self._on_familiar_summon_failure('Failure to create familiar object.  Deactivating familiar.', familiar_object=familiar,
                      warn=True)
                    return
                self._active_familiar_obj_id = familiar.id
            except Exception as e:
                try:
                    self._on_familiar_summon_failure('Exception encountered when trying to create familiar.  Deactivating familiar.', familiar_object=familiar,
                      exc=e)
                    return
                finally:
                    e = None
    ...
    def set_active_familiar(self, familiar_uid):
        ...
        if self._active_familiar_obj_id is not None:
            ...
            if active_familiar_obj is not None:
                if not active_familiar_obj.is_sim:
