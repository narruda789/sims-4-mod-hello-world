...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\waypoints\waypoint_generator_unobstructed_line.py
# Compiled at: 2019-04-12 17:15:04
# Size of source mod 2**32: 8693 bytes
...
import placement, routing, sims4.geometry
...
class _WaypointGeneratorUnobstructedLine(_WaypointGeneratorBase):
    FACTORY_TUNABLES = {'line_length':TunableRange(description='\n            The radius, in meters, of the generated constraint around the \n            target object where the waypoints will be generated.\n            ',
       ...
       default=3,
       minimum=0), 
     'fgl_parameters':TunableTuple(description='\n            Arguments that will affect the FGL.\n            ',
    ...
    def get_waypoint_constraints_gen(self, routing_agent, waypoint_count):
        ...
        fgl_context = placement.FindGoodLocationContext(starting_location_for_sample, object_polygons=(
         polygon,),
          ignored_object_ids=[
         routing_agent.id, self._context.sim.sim_id],
          max_distance=0,
        ...
        for waypoint_index in range(0, waypoint_count):
            ...
            constraint = SmallAreaConstraint(geometry=geometry, routing_surface=(self._routing_surface), min_water_depth=(water_constraint.get_min_water_depth()),
              max_water_depth=(water_constraint.get_max_water_depth()))
            waypoint_constraints.append(constraint)

        end = oriented_line_offset + start
        polygon = build_rectangle_from_two_points_and_radius(start, end, object_radius)
        self._footprint = PolygonFootprint(polygon, routing_surface=(self._routing_surface), cost=(routing.get_default_discouragement_cost()),
