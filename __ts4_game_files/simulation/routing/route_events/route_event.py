...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\route_events\route_event.py
# Compiled at: 2020-11-17 22:57:36
# Size of source mod 2**32: 16081 bytes
import weakref
...
import alarms, id_generator, services, sims4.log
...
class RouteEvent(RouteEventBase, HasTunableReference, metaclass=TunedInstanceMetaclass, manager=services.get_instance_manager(resources.Types.SNIPPET)):
    INSTANCE_TUNABLES = {'event_type':TunableVariant(description='\n            Define what is the event that is played.\n            ',
       ...
       create_carry=RouteEventTypeCreateCarry.TunableFactory(locked_args={'loots_on_xevt': None}),
       exit_carry=RouteEventTypeExitCarry.TunableFactory(locked_args={'loots_on_xevt': None}),
    ...
    def _get_tuning_suggestions(cls, print_suggestion):
        if hasattr(cls.event_type.factory, GET_TUNING_SUGGESTIONS):
            ...
            cls.skippable or print_suggestion('Skippable is disabled. This is a highly uncommonoption. Please read its description.', owner='rrodgers')
    ...
    def on_executed(self, sim, path=None, force_alarm=False):
        ...
        if self.retest_on_execute:
            if not self.test(self.get_resolver(sim)):
                return
        ...
        if self.duration == 0:
            ...
            if force_alarm or self.loot_at_end:
                self.actor = sim
