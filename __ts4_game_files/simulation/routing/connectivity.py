...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\connectivity.py
# Compiled at: 2021-04-23 17:53:29
# Size of source mod 2**32: 27836 bytes
...
import gsi_handlers.routing_handlers, placement, routing, services
...
class RoutingHandle(Handle):
    ...
    def __init__(self, sim, constraint, geometry, los_reference_point=None, routing_surface_override=None, locked_params=frozendict(), target=None):
        if routing_surface_override is not None:
            ...
            if constraint.routing_surface is not None:
    ...
    def _get_kwargs_for_clone(self, kwargs):
        kwargs.update(sim=(self.sim), constraint=(self.constraint), geometry=(self.geometry), los_reference_point=(self.los_reference_point),
          routing_surface_override=(self.routing_surface),
    ...
    def get_goals(self, max_goals=None, relative_object=None, single_goal_only=False, for_carryable=False, for_source=False, goal_height_limit=None, target_reference_override=None, always_reject_invalid_goals=False, perform_los_check=True, out_result_info=None, check_height_clearance=True):
        ...
        if (force_multi_surface or self.constraint).multi_surface:
            routing_surfaces = for_source or self.constraint.get_all_valid_routing_surfaces(force_multi_surface=force_multi_surface)
        else:
            routing_surfaces = {
        ...
        if max_goals is None:
            ...
            if hasattr(self, 'var_map'):
                ...
                if carry_target is not None:
                    if getattr(carry_target, 'is_sim', False):
            ...
            if relative_object is not None:
                try:
                    if (relative_object.is_sim or relative_object).is_part:
                        if relative_object.part_owner is None:
                            return []
            ...
            if los_routing_context is not None and all_blocking_edges_block_los and not self.sim.extended_species is SpeciesExtended.HUMAN:
                if sim_is_big_dog:
                ...
                routing_context = self.sim.get_routing_context()
                ...
                for routing_surface in routing_surfaces:
                    ...
                    surface_min_water_depth, surface_max_water_depth = OceanTuning.make_depth_bounds_safe_for_surface(routing_surface,
                      wading_interval=wading_interval, min_water_depth=min_water_depth, max_water_depth=max_water_depth)
                    goals = placement.generate_routing_goals_for_polygon((self.sim),
                      (self.geometry.polygon), routing_surface, orientation_restrictions,
                      objects_to_ignore, flush_planner=(self.constraint._flush_planner), los_reference_pt=los_reference_pt,
                      max_points=max_goals,
                      ignore_outer_penalty_amount=(self.constraint._ignore_outer_penalty_threshold),
                      single_goal_only=single_goal_only,
                      los_routing_context=los_routing_context,
                      all_blocking_edges_block_los=all_blocking_edges_block_los,
                      provided_points=provided_points,
                      min_water_depth=surface_min_water_depth,
                      max_water_depth=surface_max_water_depth,
                      min_pond_water_depth=surface_min_water_depth,
            ...
            if not generated_goals:
                ...
                if gsi_handlers.routing_handlers.archive_goals_enabled():
                    gsi_handlers.routing_handlers.archive_goals(self, goal_list, out_result_info, max_goals=max_goals,
                      ...
                      always_reject_invalid_goals=always_reject_invalid_goals,
        ...
        for tag, (location, cost, validation, failure, height_clearance) in enumerate(generated_goals):
            ...
            if invalid_goal and failure == GoalFailureType.OutOfWaterDepth:
                if out_result_info is not None:
                    ...
                    continue
                if invalid_goal:
                    if failure == GoalFailureType.TerrainTagViolations:
                        if out_result_info is not None:
    ...
    def create_goal(self, location, full_cost, tag, failure, height_clearance):
        return routing.Goal(location, cost=full_cost, tag=tag, requires_los_check=(self.los_reference_point is not None),
    ...
    def _is_geometry_single_point(self):
        if len(self.geometry.polygon) == 1:
            if len(self.geometry.polygon[0]) == 1:
                return True
        return False
    ...
    def create_goal(self, location, full_cost, tag, failure, height_clearance):
        ...
        if reference_transform is None:
            ...
            if location.orientation == sims4.math.Quaternion.ZERO():
                goal_location = routing.Location((location.position), orientation=target_orientation,
                  routing_surface=(location.routing_surface))
            else:
                goal_location = location
        return SlotGoal(goal_location, containment_transform=(self.constraint.containment_transform), cost=full_cost,
          tag=tag,
          requires_los_check=(self.los_reference_point is not None),
          connectivity_handle=self,
          slot_params=locked_params,
          failure_reason=failure,
          height_clearance=height_clearance)
...
class UniversalSlotRoutingHandle(SlotRoutingHandle):
    ...
    def _is_generated_goal_location_valid(self, location, goal_height_limit=None, target_height=None):
        if not self._validate_y_delta(location):
            ...
            return self._validate_raycast(location) or False
        return True
    ...
    def _validate_raycast(self, location):
        ...
        if location.routing_surface.type == routing.SurfaceType.SURFACETYPE_OBJECT:
            ...
            if footprint_polygon:
                if footprint_polygon.contains(sims4.math.Vector2(start_pos.x, start_pos.z)):
                    return True
