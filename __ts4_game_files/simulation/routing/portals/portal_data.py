...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\portals\portal_data.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 27289 bytes
...
class _Portal(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'traversal_cost':TunablePortalCostVariant(description='\n            Define how expensive it is, in meters, to traverse this portal.\n            '), 
     ...
     'use_vehicle_after_traversal':OptionalTunable(description='\n            When enabled allows this object to act as a sort of docking location\n            for vehicles (like the water scooter).\n            ',
       ...
       default=15.0))), 
     'allow_teleport_style_interaction_to_skip_portal':Tunable(description='\n            If true, a Sim can skip the portal when using a teleport style.  \n            If false, a Sim must use the portal rather than teleporting past it.\n            ',
       tunable_type=bool,
       default=True)}

    class _PortalInstance:
        INVALID_PORTAL_ID = 4294967295
        __slots__ = ('there', 'portal_template', 'there_exit', 'back_entry', 'obj',
                     'back', 'there_entry', 'back_exit', 'there_cost', 'back_cost',
                     '_cost_override', '_cost_override_map')
        ...
        def swap_there_and_back(self):
            self.there, self.back = self.back, self.there
            self.there_entry, self.back_entry = self.back_entry, self.there_entry
            self.there_exit, self.back_exit = self.back_exit, self.there_exit
            self.there_cost, self.back_cost = self.back_cost, self.there_cost
        ...
        def get_portal_locations(self, portal_id):
            if portal_id == self.there:
                return (
                 self.there_entry, self.there_exit)
            if portal_id == self.back:
                return (
                 self.back_entry, self.back_exit)
            return (None, None)
        ...
        def set_portal_cost_override(self, cost, sim=None, is_sim_specific=False):
            if sim is None:
                ...
                self._refresh_portal_cost(sim=sim, sim_override_cost=sim_override_cost, is_sim_specific=is_sim_specific)

        def clear_portal_cost_override(self, sim=None, is_sim_specific=False):
            if sim is None:
                self._cost_override = None
            else:
                if self._cost_override_map is not None:
                    if sim in self._cost_override_map:
                        del self._cost_override_map[sim]
                        if not self._cost_override_map:
                            self._cost_override_map = None
                        self._refresh_portal_cost(sim=sim, is_sim_specific=is_sim_specific)

        def _refresh_portal_cost(self, sim=None, sim_override_cost=None, is_sim_specific=False):
            if sim is not None:
            ...
            def refresh_cost(portal_id):
                if is_sim_specific:
                    if self._cost_override is not None:
                        update_portal_cost(portal_id, self._cost_override)
                    if sim_cost is not None:
                        routing_context.override_portal_cost(portal_id, sim_cost)
                else:
                    cost = self.get_portal_cost(portal_id) if portal_cost_override is None else portal_cost_override
                    if sim_cost is not None:
                        if sim_override_cost is not None:
        ...
        def get_required_flags(self):
            if self.portal_template.required_flags is None:
                return 0
            return int(self.portal_template.required_flags)
    ...
    def get_portal_instances(self, obj, portal_creation_mask=None):
        ...
        if hasattr(obj, 'proxied_obj'):
            if hasattr(obj.proxied_obj, 'transform'):
                if not obj.proxied_obj.parts:
                    portal_base_object = obj.proxied_obj
        base_object_is_proxy = portal_base_object is not obj
        obj_ids_to_ignore = [
         obj.id] + services.object_manager().get_objects_to_ignore_portal_validation_cache() + [additional_obj.id for additional_obj in obj.get_ignored_objects_for_line_of_sight()]
        for there_entry, there_exit, back_entry, back_exit, additional_required_flags in self.traversal_type.get_portal_locations(portal_base_object):
            if portal_creation_mask is not None:
                if portal_creation_mask & additional_required_flags == 0:
                    continue
            ...
            if not all((location is None or location.routing_surface.valid for location in (there_entry, there_exit, back_entry, back_exit))):
                continue
            if not self._validate_portal_positions(obj_ids_to_ignore, there_entry, there_exit):
                continue
            ...
            def _add_portal_instance_id(portal_entry, portal_exit, debug_name):
                if portal_entry is not None:
                    if portal_exit is not None:
                        ...
                        logger.error('_PortalInstance: {} bad args for add_portal {} portal instance - args: {}', obj, debug_name, (
                         portal_entry, portal_exit, self.traversal_type.portal_type, obj.id, cost,
                         allowed_key_mask, self.usage_penalty, discouraged_key_mask))
                        portal_id = None
                        cost = None
                else:
                    portal_id = None
            ...
            if there is not None or back is not None:
