...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\routing\portals\portal_data_ocean_ladders.py
# Compiled at: 2020-02-11 14:30:00
# Size of source mod 2**32: 6614 bytes
import sims4.log
...
class _PortalTypeDataOceanLadders(RouteEventProviderMixin, _PortalTypeDataLadders):
    ...
    def get_portal_locations(self, obj):
        ...
        locations = [
         (Location((up_start.position), orientation=(up_start.orientation), routing_surface=(up_start.routing_surface)),
          Location((up_end.position), orientation=(up_end.orientation), routing_surface=(up_end.routing_surface)),
          Location((down_start.position), orientation=(down_start.orientation), routing_surface=(down_start.routing_surface)),
          Location((down_end.position), orientation=(down_end.orientation), routing_surface=(down_end.routing_surface)),
    ...
    def provide_route_events(self, route_event_context, sim, path, is_mirrored=True, node=None, **kwargs):
        ...
        if not route_event_context.route_event_already_scheduled(route_event, provider=self):
            if not route_event_context.route_event_already_fully_considered(route_event, self):
