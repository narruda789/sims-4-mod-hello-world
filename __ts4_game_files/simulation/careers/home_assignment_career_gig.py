...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\careers\home_assignment_career_gig.py
# Compiled at: 2021-04-22 16:28:05
...
class HomeAssignmentGig(Gig):
    ...
    def is_objective_active(self, objective):
        ...
        bonus_aspiration = self.get_bonus_gig_aspiration()
        if bonus_aspiration is not None:
            return not aspiration_tracker.objective_completed(objective) and objective in aspiration_tracker.get_objectives(bonus_aspiration)
        return super().is_objective_active(objective)
    ...
    def _determine_gig_outcome(self, **kwargs):
        ...
        if completed_objectives < len(gig_objectives):
            if completed_objectives == 0:
                ...
                if self.critical_failure_test is not None:
            ...
            if self.great_success_remaining_time and remaining_time > self.great_success_remaining_time():
                self._gig_result = GigResult.GREAT_SUCCESS
                self._send_gig_telemetry(TELEMETRY_GIG_PROGRESS_COMPLETE)
            else:
                if completed_bonus_objectives > 0:
                    self._send_gig_telemetry(TELEMETRY_GIG_PROGRESS_COMPLETE)
...
lock_instance_tunables(HomeAssignmentGig, gig_prep_tasks=None,
