...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\careers\career_ops.py
# Compiled at: 2022-08-26 18:13:12
# Size of source mod 2**32: 20498 bytes
...
class CareerLevelOp(BaseLootOperation):
    FACTORY_TUNABLES = {'career':TunableReference(description="\n            The career upon which we'll be promoting/demoting the Sim.\n            If the Sim doesn't have this career or there's a reason the career\n            can't be promoted/demoted, nothing will happen.\n            ",
     ...
     'operation':TunableEnumEntry(description='\n            The operation to perform on the career.\n            ',
       tunable_type=CareerLevelOps,
       default=CareerLevelOps.PROMOTE)}
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        ...
        if not career is None:
            return career.can_change_level(demote=demote) or None
        elif demote:
            career.demote()
        else:
            career.promote()
...
class CareerLootOp(BaseLootOperation):
    ...
    def career_options(*, pack_safe=False):
        return {'career': TunableVariant(description='\n                The career to apply loot to.\n                ',
                     ...
                     reference=TunableReference(manager=(services.get_instance_manager(sims4.resources.Types.CAREER)),
                     pack_safe=pack_safe),
                     locked_args={'id_type': CareerLootOp.CAREER_REFERENCE}),
                     ...
                     participant=TunableEnumFlags(enum_type=ParticipantType,
                     default=(ParticipantType.PickedItemId)),
                     locked_args={'id_type': CareerLootOp.CAREER_PARTICIPANT}),
                     all_careers=TunableTuple(description="\n                    Apply the operation to all of the Sim's careers.\n                    ",
                     locked_args={'id_type': CareerLootOp.CAREER_ALL}),
    ...
    def _verify_tunable_callback(instance_class, tunable_name, source, value, *args, **kwargs):
        if value.career.id_type is CareerLootOp.CAREER_REFERENCE:
            ...
            for op in value.operations:
                if op.operation_type is CareerLootOp.OP_JOIN and op.level_to_join is not None and not isinstance(op.level_to_join, int):
                    if op.level_to_join.career is not career:
    ...
    FACTORY_TUNABLES = {'operations':TunableList(description='\n            A list of career loot ops.\n            ',
       ...
       tunable_type=bool,
       default=False),
       show_notification=Tunable(description='\n                        If checked then the appropriate "You got Fired" notification will be displayed. Only set this\n                        to False if you plan on handling the notification with a loot or something else. \n                        \n                        Example:\n                        High School needed different strings for droping out and graduating early, so the different\n                        strings were handled as loot dialogs.\n                        ',
       tunable_type=bool,
       default=True),
       locked_args={'operation_type': OP_FIRE}),
       quit=TunableTuple(description='\n                    Have the specified Sim quit their career. \n                    ',
       show_notification=Tunable(description='\n                        If checked then the appropriate "Sim just Quit" notification will be displayed. Only set this\n                        to False if you plan on handling the notification with a loot or something else. \n                \n                        Example:\n                        High School needed different strings for dropping out and graduating early, so the different\n                        strings were handled as loot dialogs.\n                        ',
       tunable_type=bool,
       default=True),
       locked_args={'operation_type': OP_QUIT}),
       join=TunableTuple(description='\n                    Have the specified Sim join the career.\n                    ',
       ...
       minimum=1,
       default=1),
       level_reference=TunableReference(description='\n                                A specific CareerLevel to join the career at.\n                                ',
       ...
       tunable_type=bool,
       default=False),
       locked_args={'operation_type': OP_JOIN}),
       default='performance')), 
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        ...
        for career in self._get_careers(subject, resolver):
            if career is None or career.is_visible_career:
    ...
    def _apply_to_career(self, sim_info, career, resolver):
        for op in self.operations:
            if op.operation_type == CareerLootOp.OP_PERFORMANCE:
                ...
                if op.operation_type == CareerLootOp.OP_DEMOTE:
    ...
    def _get_careers(self, sim_info, interaction):
        ...
        if self.career.id_type == CareerLootOp.CAREER_REFERENCE:
            career_ids = (
             self.career.reference.guid64,)
        else:
            if self.career.id_type == CareerLootOp.CAREER_PARTICIPANT:
                career_ids = interaction.get_participants(self.career.participant)
            else:
                if self.career.id_type == CareerLootOp.CAREER_ALL:
                    career_ids = careers
...
class CareerStayLateOp(BaseLootOperation):
    FACTORY_TUNABLES = {'stay_late_extension_override': OptionalTunable(description='\n            If enabled, extends the active work shift by the tuned amount.\n            If disabled, the Stay Late Extension tuning on the Career Tuning\n            is used instead.\n            ',
                                       tunable=TunableSimMinute(default=120,
                                       minimum=0))}
