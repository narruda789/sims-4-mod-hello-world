...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\vet\vet_customer_group_situation.py
# Compiled at: 2018-07-22 17:25:27
# Size of source mod 2**32: 36490 bytes
import itertools
...
import enum, filters, services, sims4.tuning, situations.bouncer
    ...
    def _on_dialog_response(self, dialog):
        ...
        if dialog.response == dialog.response == ButtonType.DIALOG_RESPONSE_OK:
            ...
            if assigned_vet is None or pet is None:
                self.owner._self_destruct()
                return
            context = InteractionContext(assigned_vet, (InteractionContext.SOURCE_SCRIPT_WITH_USER_INTENT),
              (Priority.High),
...
class _ReceivingServiceState(_VetCustomerGroupSituationStateBase):
    ...
    def handle_event(self, sim_info, event, resolver):
        ...
        if event == TestEvent.InteractionComplete:
            ...
            if resolver(self.vet_reassignment_interaction):
                if self.owner.is_sim_info_in_situation(resolver.get_participant(ParticipantType.TargetSim)):
...
class VetCustomerGroupSituation(BusinessSituationMixin, SituationComplexCommon):
    INSTANCE_TUNABLES = {'situation_job_mapping':TunableMapping(description="\n            A mapping of filter term tag to situation job.\n            \n            The filter term tag is returned as part of the sim filters used to \n            create the guest list for this particular situation.\n            \n            The situation job is the job that the Sim will be assigned to in\n            the situation.\n            \n            e.g. Map a human to 'pet owner', map a cat or dog to 'pet'\n            ",
     ...
     '_arrival_state':_ArrivalState.TunableFactory(display_name='1. Arrival State',
       tuning_group=GroupNames.STATE), 
     '_waiting_state':_WaitingForServiceState.TunableFactory(display_name='2. Waiting For Service State',
       tuning_group=GroupNames.STATE), 
     '_service_state':_ReceivingServiceState.TunableFactory(display_name='3. Receiving Service State',
       tuning_group=GroupNames.STATE), 
     '_complete_state':_ServiceCompleteState.TunableFactory(display_name='4. Service Complete State',
       tuning_group=GroupNames.STATE)}
    ...
    def _states(cls):
        return (
         SituationStateData(CustomerStateTypes.SPAWN, _SpawnGate),
         SituationStateData((CustomerStateTypes.ARRIVAL), _ArrivalState, factory=(cls._arrival_state)),
         SituationStateData((CustomerStateTypes.WAITING), _WaitingForServiceState, factory=(cls._waiting_state)),
         SituationStateData((CustomerStateTypes.RECEIVING), _ReceivingServiceState, factory=(cls._service_state)),
         SituationStateData((CustomerStateTypes.COMPLETE), _ServiceCompleteState, factory=(cls._complete_state)))
    ...
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.owner_job_and_initial_role.job, cls.owner_job_and_initial_role.role_state),
         (
          cls.pet_job_and_initial_role.job, cls.pet_job_and_initial_role.role_state)]
    ...
    def get_predefined_guest_list(cls):
        ...
        filter_results = services.sim_filter_service().submit_matching_filter(sim_filter=customer_filter, allow_yielding=False,
    ...
    def start_situation(self):
        ...
        if host_sim is not None:
            host_sim.is_npc or self._change_state(self._waiting_state())
        else:
    ...
    def began_waiting(self):
        ...
        automation_event(*('VetCustomerBeganWaiting', ), SituationId=self.id, 
         PetSimId=pet.sim_id, 
...
sims4.tuning.instances.lock_instance_tunables(VetCustomerGroupSituation, duration=0,
