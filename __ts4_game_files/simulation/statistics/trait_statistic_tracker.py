...
# Size of source mod 2**32: 17499 bytes
...
class TraitStatisticTracker(ContinuousStatisticTracker):
    ...
    def _load_delayed_active_statistics(self):
        ...
        for trait_statistic_data in self._delayed_active_lod_statistics:
            ...
            if not stat.trait_unlocked:
                continue
            group_being_changed = stat.group
            ...
            self._trait_statistic_groups[group_being_changed].add(statistic_type)

        self._delayed_active_lod_statistics = None
        if self._trait_statistic_groups is None:
            return
        groups_to_limit = []
        for trait_statistic_group, unlocked_trait_statistics in self._trait_statistic_groups.items():
            if len(unlocked_trait_statistics) < TraitStatistic.GROUPS[trait_statistic_group]:
                continue
            groups_to_limit.append(trait_statistic_group)

        for statistic in self._statistics.values():
            if statistic.group not in groups_to_limit:
                continue
    ...
    def _on_statistic_state_changed(self, changed_statistic):
        ...
        if group_being_changed == TraitStatisticGroup.NO_GROUP:
            ...
            del (self._trait_statistic_groups[group_being_changed] or self._trait_statistic_groups)[group_being_changed]
