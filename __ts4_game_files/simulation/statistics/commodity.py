...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\statistics\commodity.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 110409 bytes
import contextlib, itertools, operator, random, weakref
...
import clock, date_and_time, enum, event_testing.resolver, interactions.context, interactions.priority, services, sims4.log, sims4.resources, telemetry_helper
...
class CommodityState:
    __slots__ = ('_value', '_buff', '_buff_overrides', '_icon', '_icon_overrides',
                 '_fill_level', '_buff_add_threshold', '_data_description', '_data_description_overrides',
                 '_fill_color', '_background_color', '_tooltip_icon_list', '_tooltip_icon_list_overrides',
                 'loot_list_on_enter_always', 'loot_list_on_enter_increasing', 'loot_list_on_enter_decreasing',
                 'apply_loot_on_load', '_ui_container_animation_increasing', '_ui_container_animation_decreasing',
                 'body_type_client_level')

    def __init__(self, value=0, buff=None, buff_overrides=None, icon=None, icon_overrides=None, fill_level=None, buff_add_threshold=None, data_description=None, data_description_overrides=None, fill_color=None, background_color=None, tooltip_icon_list=None, tooltip_icon_list_overrides=None, loot_list_on_enter_always=None, loot_list_on_enter_increasing=None, loot_list_on_enter_decreasing=None, apply_loot_on_load=False, ui_container_animation_increasing='', ui_container_animation_decreasing='', body_type_client_level=None):
        self._value = value
        self._buff = buff
        ...
        self.loot_list_on_enter_always = loot_list_on_enter_always
        self.loot_list_on_enter_increasing = loot_list_on_enter_increasing
        self.loot_list_on_enter_decreasing = loot_list_on_enter_decreasing
        ...
        self.body_type_client_level = body_type_client_level
...
class TunableCommodityDistress(TunableTuple):
  ...
  tunable_type=SkewerAlertType,
  default=(SkewerAlertType.PET_DISTRESS),
  invalid_enums=(SkewerAlertType.NONE))), **kwargs)
...
class TunableArrowData(TunableTuple):
    ...
    def __init__(self, **kwargs):
        (super().__init__)(positive_single_arrow=Tunable(float, 1, description='If the change rate for commodity is between this value and less than second arrow value, a single arrow will show up during commodity change.'), positive_double_arrow=Tunable(float, 20, description='If the change rate for commodity is between this value and less than triple arrow value, a double arrow will show up during commodity change.'), 
         positive_triple_arrow=Tunable(float, 30, description='If the change rate for commodity is above this value then triple arrows will show up during commodity change.'), 
         negative_single_arrow=Tunable(float, (-1), description='If the change rate for commodity is between this value and less than second arrow value, a single arrow will show up during commodity change.'), 
         negative_double_arrow=Tunable(float, (-20), description='If the change rate for commodity is between this value and less than triple arrow value, a double arrow will show up during commodity change.'), 
         negative_triple_arrow=Tunable(float, (-30), description='If the change rate for commodity is above this value then triple arrows will show up during commodity change.'), **kwargs)
...
class Commodity(HasTunableReference, TunedContinuousStatistic, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.STATISTIC)):
    ...
    INSTANCE_TUNABLES = {'stat_name':TunableLocalizedString(description='\n                Localized name of this commodity.\n                ',
       allow_none=True,
       export_modes=ExportModes.All), 
     'stat_name_overrides':TunableMapping(description='\n                A mapping of Traits to TunableLocalizedString, used to\n                override the stat name field.\n                ',
       key_type=TunableReference(description='\n                    The trait.\n                    ',
       manager=(services.get_instance_manager(sims4.resources.Types.TRAIT)),
       pack_safe=True),
       value_type=TunableLocalizedString(description='\n                    Localized name of this commodity.\n                    ',
       allow_none=True),
       tuple_name='StatNameOverrideMapping',
       export_modes=ExportModes.ClientBinary), 
     'min_value_tuning':Tunable(description='\n                The minimum value for this stat.\n                ',
       tunable_type=float,
       default=-100,
       export_modes=ExportModes.All), 
     'max_value_tuning':Tunable(description='\n                The maximum value for this stat.',
       tunable_type=float,
       default=100,
       export_modes=ExportModes.All), 
     'best_value_tuning':TunableVariant(description='\n            When creating clones or using the cheat to set all motives to\n            their best value, the max_value_tuning may not be the BEST\n            value, so this is a place to specify what the BEST value is\n            for this commodity.',
       same_as_max_value_tuning=TunableTuple(locked_args={'best_value_tuning_method': CommodityBestValueTuningMethod.USE_MAX_VALUE_TUNING}),
       same_as_min_value_tuning=TunableTuple(locked_args={'best_value_tuning_method': CommodityBestValueTuningMethod.USE_MIN_VALUE_TUNING}),
       default='same_as_max_value_tuning'), 
     'ui_render_style':TunableEnumEntry(description='\n                The render style for this commodity. For example, the powers\n                bar for Vampires have a different render style.\n                ',
       tunable_type=MotiveUIStyle,
       default=MotiveUIStyle.DEFAULT,
       export_modes=ExportModes.All), 
     'ui_sort_order':TunableRange(description='\n                Order in which the commodity will appear in the motive panel.\n                Commodities sort from lowest to highest.\n                ',
       ...
       minimum=0,
       export_modes=ExportModes.All), 
     'ui_visible_distress_threshold':TunableTuple(description="\n                When current value of commodity passes this threshold, commodity\n                will appear in the motive panel tab to show it's in distress.\n                ",
       threshold_value=Tunable(description='\n                    The value of the threshold.\n                    ',
       tunable_type=float,
       default=0),
       comparison=TunableEnumEntry(description='\n                    Comparison operator to describe how this threshold passes.\n                    e.g: if the value is 50 and comparison is LESS, this commodity shows in distress\n                    if its value is less than 50.\n                    ',
       tunable_type=(sims4.math.Operator),
       default=(sims4.math.Operator.LESS)),
       export_modes=ExportModes.ClientBinary,
       export_class_name='TunableDistressThreshold'), 
     'ad_data':TunableList(description='\n                A list of Vector2 points that define the desire curve for this\n                commodity.\n                ',
    ...
    def _verify_tuning_callback(cls):
        ...
        if cls.visible:
            if cls.ui_visible_distress_threshold.threshold_value < min_value or cls.ui_visible_distress_threshold.threshold_value > max_value:
                logger.error('{} visible distress value {} is outside the min{} / max {} range.  Please update tuning', cls, cls.ui_visible_distress_threshold.threshold_value, cls.min_value, cls.max_value)
    ...
    def __init__(self, tracker, core=False):
        ...
        if tracker is not None:
            if not tracker.recovery_add_in_progress:
    ...
    def instance_required(self):
        if self._buff_handle is not None or self._distress_buff_handle is not None or self.is_visible_commodity():
            return True
        if self.core:
            if not self.initial_as_default:
                return True
    ...
    def added_by_default(cls, min_range=None, max_range=None):
        ...
        if min_range is not None:
            if min_range <= cls.default_value <= max_range:
                return False
            return True
        return False
    ...
    def _setup_commodity_states(self, apply_state_enter_loot=False):
        if self.commodity_states:
            ...
            if self._current_state_index != new_state_index:
                self._set_state(new_state_index, current_value,
                  apply_state_enter_loot=apply_state_enter_loot,
                  send_client_update=False)
    ...
    def _commodity_telemetry(self, hook, desired_state_index):
        if not self._send_telemetry_on_state_change:
            return
        sim_instance = self.tracker.get_sim() if self.tracker.owner.is_sim else None
        with telemetry_helper.begin_hook(writer, hook, sim=sim_instance) as (hook):
    ...
    def _update_state_up(self, stat_instance):
        with self._suppress_client_updates_context_manager():
            ...
            elif desired_state_index != self._current_state_index:
                self._remove_state_callback()
    ...
    def _can_low_level_simulate(self, sim):
        if sim.is_baby:
            return False
        if self._off_lot_simulation == self.OFF_LOT_SIM_DISABLED:
            return False
        if self._off_lot_simulation == self.OFF_LOT_SIM_SELECTABLE:
            return sim.is_selectable
        return True

    def start_low_level_simulation(self):
        sim = self.tracker.owner
        can_simulate = self._can_low_level_simulate(sim)
        self.decay_enabled = can_simulate
        if (can_simulate or self).is_visible:
    ...
    def _add_state_callback(self):
        ...
        if next_state_index < len(self.commodity_states):
            self._current_state_ge_callback_data = self.create_and_add_callback_listener((Threshold(self.commodity_states[next_state_index].value, operator.ge)), (self._update_state_up),
              on_callback_alarm_reset=(self._state_reset_callback))
        if self.commodity_states[self._current_state_index].value > self.min_value:
            self._current_state_lt_callback_data = self.create_and_add_callback_listener((Threshold(self.commodity_states[self._current_state_index].value, operator.lt)), (self._update_state_down),
              on_callback_alarm_reset=(self._state_reset_callback))
        ...
        if self._current_state_index is not None:
            if self._current_state_index > 0:
                ...
                if self.convergence_value <= current_value:
                    commodity_state = self.commodity_states[(self._current_state_index - 1)]
                    buff_tunable_ref = self._get_applicable_buff_override(commodity_state)
                else:
                    next_state_index = self._current_state_index + 1
                    if next_state_index < len(self.commodity_states):
                        commodity_state = self.commodity_states[next_state_index]
                        buff_tunable_ref = self._get_applicable_buff_override(commodity_state)
                    elif buff_tunable_ref is not None:
    ...
    def _add_buff_from_state(self, commodity_state, apply_buff_loot=True):
        owner = self.tracker.owner
        if owner.is_sim:
            buff_tuning = self._get_applicable_buff_override(commodity_state)
            transition_into_buff_id = self._get_next_buff_commodity_decaying_to() if buff_tuning.buff_type.visible else 0
            try:
                self._buff_handle = owner.add_buff((buff_tuning.buff_type), buff_reason=(buff_tuning.buff_reason),
                  commodity_guid=(self.guid64),
            ...
            if self._buff_handle is None:
                if buff_tuning.buff_type._temporary_commodity_info is not None:
                    logger.warn('Sim {} has temporary commodity {} from buff {}, but this buff was not able to be re-added. The commodity will be removed.', owner, self, buff_tuning.buff_type)
                    self.tracker.remove_statistic(self.stat_type)
    ...
    def _set_state(self, new_state_index, current_value, apply_state_enter_loot=False, send_client_update=True):
        ...
        if self._buff_threshold_callback is not None:
            ...
            if new_state.buff.buff_type:
                if new_state.buff_add_threshold is not None:
                    self._buff_threshold_callback = self.force_apply_buff_on_start_up or new_state.buff_add_threshold.compare(current_value) or self.create_and_add_callback_listener(new_state.buff_add_threshold, self._add_buff_callback)
                else:
        ...
        if old_state_index is not None or apply_state_enter_loot or new_state.apply_loot_on_load:
            ...
            def get_state_loot():
                total_loot = list()
                if new_state.loot_list_on_enter_always:
                    total_loot.extend(new_state.loot_list_on_enter_always)
                if old_state_index is None:
                    return total_loot
                if new_state_index > old_state_index and new_state.loot_list_on_enter_increasing:
                    total_loot.extend(new_state.loot_list_on_enter_increasing)
                else:
                    if new_state_index < old_state_index:
                        if new_state.loot_list_on_enter_decreasing:
                            total_loot.extend(new_state.loot_list_on_enter_decreasing)
                return total_loot

            loot_to_give = get_state_loot()
            if loot_to_give:
                ...
                for loot_action in loot_to_give:
    ...
    def _enter_distress(self, stat_instance):
        if not self.tracker.owner.is_sim:
            logger.error('Distress for commodity {} is trying to be called on object {}, which is not a sim.  This is not valid.', self,
              (self.tracker.owner),
              owner='jjacobson')
            return
        sim = self.tracker.owner.get_sim_instance()
        ...
        context = interactions.context.InteractionContext((self.tracker.owner.get_sim_instance()), (interactions.context.InteractionContext.SOURCE_AUTONOMY),
    ...
    def _setup_distress_essentials(self, sim):
        if self.commodity_distress.buff.buff_type is not None:
            if self._distress_buff_handle is None:
                ...
                logger.error('Distress Buff Handle is not none when entering Commodity Distress for {}.', self,
                  owner='jjacobson')
    ...
    def _exit_distress(self, stat_instance, on_removal=False):
        if self._distress_buff_handle is not None:
            ...
            if self.commodity_distress.buff.buff_type is not None:
                if not on_removal:
                    logger.error('Distress Buff Handle is none when exiting Commodity Distress for {}.', self,
                      owner='jjacobson')
            elif self._exit_distress_callback_data is not None:
                self.remove_callback_listener(self._exit_distress_callback_data)
                self._exit_distress_callback_data = None
            else:
                if not on_removal:
                    logger.error('Exit distress called before exit distress callback has been setup for {}.', self,
                      owner='jjacobson')
            self.tracker.owner.exit_distress(self)

    def _commodity_fail_object(self, stat_instance):
        context = interactions.context.InteractionContext(None, (interactions.context.InteractionContext.SOURCE_SCRIPT),
          (interactions.priority.Priority.Critical),
          bucket=(interactions.context.InteractionBucketType.DEFAULT))
        owner = self.tracker.owner
        for failure_interaction in self.commodity_failure.failure_interactions:
            if failure_interaction.immediate:
                if not failure_interaction.simless:
    ...
    def fixup_for_time(self, last_update_time, is_locked, is_baby=False, is_npc=False, decay_enabled=DEFAULT):
        if self.time_passage_fixup_type() == CommodityTimePassageFixupType.FIXUP_USING_TIME_ELAPSED:
            ...
            if self.time_passage_fixup_type() == CommodityTimePassageFixupType.FIXUP_USING_AUTOSATISFY_CURVE:
                if self._off_lot_simulation == self.OFF_LOT_SIM_DISABLED or is_baby or is_npc and self._off_lot_simulation != self.OFF_LOT_SIM_ALL:
    ...
    def set_to_auto_satisfy_value(self):
        if self.use_autosatisfy_curve:
            if self._auto_satisfy_curve:
                now = services.time_service().sim_now
                time_sim_was_saved = self.tracker.owner.time_sim_was_saved
                if not time_sim_was_saved is None or self.use_auto_satisfy_curve_as_initial_value:
    ...
    def _send_value_update_event(self):
        ...
        if owner is not None:
            if owner.is_sim:
                if not owner.is_npc:
                    services.get_event_manager().process_event((test_events.TestEvent.StatValueUpdate),
                      sim_info=owner,
    ...
    def save_for_delayed_active_lod(cls, commodity_proto, commodities, skills, ranked_statistics):
        commodities.append(commodity_proto)
    ...
    def save_statistic(self, commodities, skills, ranked_statistics, tracker):
        commodities.append(self.get_save_message(tracker))
    ...
    def is_visible_commodity(self):
        if not self.tracker is None:
            return self.tracker.owner.is_sim or False
        else:
            if not self.visible:
                return False
            return self.commodity_states or False
