...
class BaseStatisticTracker(SimInfoTracker):
    ...
    def get_communicable_statistic_set(self):
        if self._statistics is None:
            return set()
        return {stat_type for stat_type in self._statistics if stat_type.communicable_by_interaction_tag is not None if stat_type.communicable_by_interaction_tag is not None}
