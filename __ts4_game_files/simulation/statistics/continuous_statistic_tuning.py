...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\statistics\continuous_statistic_tuning.py
# Compiled at: 2021-01-26 15:42:16
# Size of source mod 2**32: 21851 bytes
import operator
...
import services, sims4.tuning.tunable, sims4.utils, statistics.continuous_statistic, statistics.tunable
...
class TunedContinuousStatistic(statistics.continuous_statistic.ContinuousStatistic):
    ...
    def _tuning_loaded_callback(cls):
        ...
        if cls.delayed_decay_rate:
            cls._delayed_decay_override_list = cls._initialize_decay_override_list((cls.delayed_decay_rate.decay_rate_overrides), (cls.delayed_decay_rate.delayed_decay_rate),
              delay=True)
    ...
    def _initialize_decay_override_list(cls, override_tuning, default_decay, delay=False):
        ...
        if final_decay_override_list:
            if final_decay_override_list[0].lower_bound > cls.min_value:
    ...
    def _create_new_override_callbacks(self):
        ...
        if self.delayed_decay_rate is not None:
            if self.delayed_decay_rate.decay_rate_overrides:
