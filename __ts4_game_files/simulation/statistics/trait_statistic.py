...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\statistics\trait_statistic.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 46964 bytes
from protocolbuffers import SimObjectAttributes_pb2
import enum, operator, services
...
class TraitStatisticData(TunableTuple):
  ...
  manager=(services.get_instance_manager(Types.TRAIT))), 
         set_in_progress_from_locked_value=Tunable(description='\n                The value at which this Trait Statistic will be set in progress\n                from being locked.  This is also the point that it will be\n                unhidden if it has been marked hidden.\n                ',
  tunable_type=float,
  default=set_in_progress_from_locked_default), 
         set_locked_from_in_progress_value=Tunable(description='\n                The value at which this Trait Statistic will be set to locked\n                from being in progress.\n                ',
  tunable_type=float,
  default=set_locked_from_in_progress_default), 
         set_unlocked_from_in_progress_value=Tunable(description='\n                The value at which this Trait Statistic will be set to unlocked\n                from being in progress.\n                ',
  tunable_type=float,
  default=set_unlocked_from_in_progress_default), 
         set_at_risk_from_unlocked_value=Tunable(description='\n                The value at which this Trait Statistic will be set to at\n                risk from being unlocked.\n                ',
  tunable_type=float,
  default=set_at_risk_from_unlocked_default), 
         set_at_locked_from_at_risk_value=Tunable(description='\n                The value at which this Trait Statistic will be set to at\n                locked from being at risk.\n                ',
  tunable_type=float,
  default=set_at_locked_from_at_risk_default), 
         set_unlocked_from_at_risk_value=Tunable(description='\n                The value at which this Trait Statistic will be set to unlocked\n                from being at risk.\n                ',
  tunable_type=float,
  default=set_unlocked_from_at_risk_default), 
         unlocked_notification=OptionalTunable(description='\n                If enabled then a notification will be played when this trait statistic is unlocked.\n                ',
...
class TraitStatistic(HasTunableReference, TunedContinuousStatistic, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(Types.STATISTIC)):
    GROUPS = TunableMapping(description='\n        A mapping between Groups for trait statistics and the maximum number\n        of traits that are allowed to be unlocked for that group.\n        ',
      ...
      tunable_type=TraitStatisticGroup,
      default=(TraitStatisticGroup.NO_GROUP),
      invalid_enums=(
     TraitStatisticGroup.NO_GROUP,)),
      ...
      default=1,
      minimum=1))
    INSTANCE_TUNABLES = {'trait_data':TraitStatisticData(), 
     'opposing_trait_data':OptionalTunable(description='\n            If enabled then this trait statistic will have a separate trait\n            data representing an opposing trait along the trait tracker.\n            ',
       tunable=TraitStatisticData(set_in_progress_from_locked_default=(-50),
       set_locked_from_in_progress_default=(-25),
       set_unlocked_from_in_progress_default=(-75),
       set_at_risk_from_unlocked_default=(-50),
       set_at_locked_from_at_risk_default=(-25),
       set_unlocked_from_at_risk_default=(-75))), 
     'periodic_tests':TestedSum.TunableFactory(description='\n            A Tested Sum of values that will be added to the statistic on\n            an interval determined in the Trait Statistic Tracker.\n            '), 
    ...
    def _verify_trait_data_in_bounds(cls, trait_data, prefix=''):
        if not cls.min_value < trait_data.set_in_progress_from_locked_value < cls.max_value:
            logger.error(prefix + 'Set In Progress From Locked Value is outside the min or max bounds of {}', cls)
        if not cls.min_value < trait_data.set_locked_from_in_progress_value < cls.max_value:
            logger.error(prefix + 'Set Locked From In Progress Value is outside the min or max bounds of {}', cls)
        if not cls.min_value < trait_data.set_unlocked_from_in_progress_value < cls.max_value:
            logger.error(prefix + 'Set Unlocked From In Progress Value is outside the min or max bounds of {}', cls)
        if not cls.min_value < trait_data.set_at_risk_from_unlocked_value < cls.max_value:
            logger.error(prefix + 'Set At Risk From Unlocked Value is outside the min or max bounds of {}', cls)
        if not cls.min_value < trait_data.set_at_locked_from_at_risk_value < cls.max_value:
            logger.error(prefix + 'Set Locked From At Risk Value is outside the min or max bounds of {}', cls)
        if not cls.min_value < trait_data.set_unlocked_from_at_risk_value < cls.max_value:
    ...
    def can_add(cls, owner, force_add=False, **kwargs):
        if owner.age not in cls.ages:
            return False
        if cls.group != TraitStatisticGroup.NO_GROUP:
            if not services.lifestyle_service().can_add_trait_statistic(cls):
                return False
    ...
    def _setup_locked_state_listeners(self):
        if self.opposing_trait_data is None:
            ...
            self._setup_callbacks(next_positive_callback=(self._enter_in_progress_state), next_positive_callback_value=(self.trait_data.set_in_progress_from_locked_value),
              next_negative_callback=(self._enter_opposing_in_progress_state),
              next_negative_callback_value=(self.opposing_trait_data.set_in_progress_from_locked_value))
    ...
    def _setup_in_progress_state_listeners(self):
        self._setup_callbacks(next_positive_callback=(self._enter_unlocked_state), next_positive_callback_value=(self.trait_data.set_unlocked_from_in_progress_value),
          next_negative_callback=(self._enter_locked_state),
          next_negative_callback_value=(self.trait_data.set_locked_from_in_progress_value))
    ...
    def _setup_at_risk_state_listeners(self):
        self._setup_callbacks(next_positive_callback=(self._enter_unlocked_state), next_positive_callback_value=(self.trait_data.set_unlocked_from_at_risk_value),
          next_negative_callback=(self._enter_locked_state),
          next_negative_callback_value=(self.trait_data.set_at_locked_from_at_risk_value))
    ...
    def _enter_at_risk_state(self, _):
        ...
        self._send_at_risk_message()
        if self.trait_data.at_risk_notification is not None:
            if not self._suppress_at_risk_notification:
                sim_info = self.tracker.owner
    ...
    def _setup_opposing_in_progress_state_listeners(self):
        self._setup_callbacks(next_positive_callback=(self._enter_locked_state), next_positive_callback_value=(self.opposing_trait_data.set_locked_from_in_progress_value),
          next_negative_callback=(self._enter_opposing_unlocked_state),
          next_negative_callback_value=(self.opposing_trait_data.set_unlocked_from_in_progress_value))
    ...
    def _setup_opposing_at_risk_state_listeners(self):
        self._setup_callbacks(next_positive_callback=(self._enter_locked_state), next_positive_callback_value=(self.opposing_trait_data.set_at_locked_from_at_risk_value),
          next_negative_callback=(self._enter_opposing_unlocked_state),
          next_negative_callback_value=(self.opposing_trait_data.set_unlocked_from_at_risk_value))
    ...
    def _enter_opposing_at_risk_state(self, _):
        ...
        self._send_at_risk_message()
        if self.opposing_trait_data.at_risk_notification is not None:
            if not self._suppress_at_risk_notification:
                sim_info = self.tracker.owner
    ...
    def on_trait_added(cls, sim_info, trait):
        ...
        if trait is cls.trait_data.trait and not trait_statistic.state == TraitStatisticStates.UNLOCKED:
            if trait_statistic.state == TraitStatisticStates.AT_RISK:
                return
            trait_statistic.set_value((cls.max_value), ignore_caps=True)
            trait_statistic.reset_daily_caps()
        else:
            if trait is cls.opposing_trait_data.trait and not trait_statistic.state == TraitStatisticStates.OPPOSING_UNLOCKED:
                if trait_statistic.state == TraitStatisticStates.OPPOSING_AT_RISK:
                    return
    ...
    def on_trait_removed(cls, sim_info, trait):
        ...
        try:
            ...
            if trait is cls.trait_data.trait:
                ...
                if trait is cls.opposing_trait_data.trait:
    ...
    def startup_statistic(self, from_load=False):
        ...
        elif self._state == TraitStatisticStates.LOCKED:
            self._setup_locked_state_listeners()
        else:
            if self._state == TraitStatisticStates.IN_PROGRESS:
                self._setup_in_progress_state_listeners()
                ...
                if self._state == TraitStatisticStates.UNLOCKED:
                    ...
                    if self._state == TraitStatisticStates.AT_RISK:
                        ...
                        if self._state == TraitStatisticStates.OPPOSING_IN_PROGRESS:
                            ...
                            if self._state == TraitStatisticStates.OPPOSING_UNLOCKED:
                                self._setup_opposing_unlocked_state_listeners()
                            else:
                                if self._state == TraitStatisticStates.OPPOSING_AT_RISK:
                                    self._setup_opposing_at_risk_state_listeners()
                                else:
    ...
    def _handle_neglect(self, trait_data):
        if TraitStatisticStates.OPPOSING_UNLOCKED < self._state < TraitStatisticStates.UNLOCKED:
            return
        elif self._value_added:
            return
    ...
    def _check_for_value_gain(self, old_value, new_value):
        if not self.default_value <= old_value < new_value:
            if self.default_value >= old_value > new_value:
    ...
    def load(self, msg):
        ...
        elif msg.HasField('min_daily_cap'):
            self._min_daily_cap = msg.min_daily_cap
        else:
            self.startup_statistic(from_load=True)
            if self._state >= TraitStatisticStates.UNLOCKED:
                self.tracker.owner.has_trait(self.trait_data.trait) or self.tracker.owner.add_trait(self.trait_data.trait)
            else:
                if self._state <= TraitStatisticStates.OPPOSING_UNLOCKED:
                    if not self.tracker.owner.has_trait(self.opposing_trait_data.trait):
                        self.tracker.owner.add_trait(self.opposing_trait_data.trait)


lock_instance_tunables(TraitStatistic, min_lod_value=(SimInfoLODLevel.ACTIVE),
  respect_lod_on_add=True)