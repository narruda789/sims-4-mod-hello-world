...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\adoption\adoption_situation.py
# Compiled at: 2020-05-05 14:26:11
# Size of source mod 2**32: 17569 bytes
...
from tag import Tag
import placement, services, sims4.math
...
class _WaitForPetsToDespawnState(CommonSituationState):
    ...
    def on_remove_sim_from_situation(self, sim):
        ...
        elif (self._pets_to_despawn or self.owner.pet_crate) is None:
            self._change_state(self.owner.leave_state())
        else:
            self._change_state(self.owner.pick_up_adoption_crate_state())
...
class SituationComplexAdoptionPet(SituationComplexAdoption):
    ...
    def _states(cls):
        return (SituationStateData(1, _HasFrontDoorArrivalState, factory=(cls.has_front_door_arrival_state)),
         ...
         SituationStateData(3, _SpawnPetsState, factory=(cls.spawn_pets_state)),
         SituationStateData(4, _InteractWithPetsState, factory=(cls.interact_with_pets_state)),
         SituationStateData(5, _WaitForPetsToDespawnState, factory=(cls.wait_for_pets_to_despawn_state)),
         SituationStateData(6, _PickUpAdoptionCrateState, factory=(cls.pick_up_adoption_crate_state)),
         SituationStateData(7, _LeaveState, factory=(cls.leave_state)))

    @classmethod
    def _get_tuned_job_and_default_role_state_tuples(cls):
        return [(cls.adoption_officer_job_and_role.job, cls.adoption_officer_job_and_role.role_state),
         (
          cls.pet_adoption_candidate_job_and_role.job, cls.pet_adoption_candidate_job_and_role.role_state)]
...
lock_instance_tunables(SituationComplexAdoptionPet, exclusivity=(BouncerExclusivityCategory.NORMAL),
