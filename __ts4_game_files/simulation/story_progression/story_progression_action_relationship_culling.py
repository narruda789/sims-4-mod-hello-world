...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\story_progression\story_progression_action_relationship_culling.py
# Compiled at: 2021-05-28 13:45:03
...
import gsi_handlers, performance, services, sims4, telemetry_helper
...
class StoryProgressionRelationshipCulling(_StoryProgressionAction):
    ...
    def _trigger_relationship_telemetry(cls, hook_name, culling_event_id):
        ...
        with telemetry_helper.begin_hook(writer, hook_name) as (hook):
            ...
            hook.write_int('psim', metrics.played_sims_with_sentiments)
            hook.write_int('prel', metrics.rels_on_played_sims_with_sentiments)
            hook.write_int('psnt', metrics.num_sentiments_on_player_sims)
            hook.write_int('tsim', metrics.num_sims_with_sentiments)
            hook.write_int('trel', metrics.rels_with_sentiments)
            hook.write_int('tsnt', metrics.total_num_sentiments)
    ...
    def _do_npc_relationship_culling(cls):
        ...
        for sim_info in sorted((services.sim_info_manager().values()), key=(operator.attrgetter('is_player_sim', 'is_played_sim'))):
            if not sim_info.household.id == active_household_id:
