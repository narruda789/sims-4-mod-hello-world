...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\audio\primitive.py
# Compiled at: 2021-01-11 20:40:12
...
class PlaySound(distributor.ops.ElementDistributionOpMixin):
    ...
    def __init__(self, target, sound_id, sound_name=None, is_vox=False, joint_name_hash=None, play_on_active_sim_only=False, immediate=False):
    ...
    def build_sound_start_msg(self):
        ...
        if self.sound_name is not None:
            start_msg.sound_name = self.sound_name
...
class TunablePlayAudio(TunableFactory):
    ...
    @staticmethod
    def _factory(owner, audio, joint_name_hash, play_on_active_sim_only, immediate_audio):
        return PlaySound(owner, (audio.instance), joint_name_hash=joint_name_hash,
          play_on_active_sim_only=play_on_active_sim_only,
          immediate=immediate_audio)
