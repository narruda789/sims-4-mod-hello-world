...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\visualization\constraint_visualizer.py
# Compiled at: 2021-04-16 15:05:13
# Size of source mod 2**32: 20525 bytes
import itertools
...
import contour, routing, sims4.color
DONE_POSTURE_EVENTS = {
 PostureEvent.TRANSITION_FAIL, PostureEvent.TRANSITION_COMPLETE}
MULTISURFACE_OFFSET = 0.5

def _draw_constraint(layer, constraint, color, altitude=0, modify_altitiude=True, anywhere_position=None, draw_contours=False):
            ...
            for sub_constraint in constraint._constraints:
                if sub_constraint._geometry is not None:
                    if sub_constraint._geometry in drawn_geometry:
                        _draw_constraint(layer, (sub_constraint.generate_alternate_geometry_constraint(None)), color,
                          altitude, modify_altitiude=modify_altitiude)
                        continue
                    drawn_geometry.append(sub_constraint._geometry)
                _draw_constraint(layer, sub_constraint, color, altitude, modify_altitiude=modify_altitiude)
                if modify_altitiude:
                    altitude += 0.1
        ...
        if constraint.geometry is not None:
            if constraint.geometry.polygon is not None:
                    ...
                    if poly not in drawn_polys:
                        ...
                        if constraint.multi_surface:
                            if constraint.routing_surface.type in routing.object_routing_surfaces:
                                layer.add_polygon(poly, color=color, altitude=(altitude + 0.1 - MULTISURFACE_OFFSET))
                            else:
                                layer.add_polygon(poly, color=color, altitude=(altitude + 0.1 + MULTISURFACE_OFFSET))
                        layer.add_polygon(poly, color=color, altitude=(altitude + 0.1))

                    def draw_facing(point, color):
                        facing_altitude = altitude + 0.1
                        ...
                        if valid and interval is not None:
                            if interval.a != interval.b:
                                if interval.angle >= sims4.math.TWO_PI:
                                    ...
                                    angles = [
                                     (
                                      interval.a, False), (interval.ideal, True), (interval.b, False)]
                            else:
                                angles = [
                                 (
                                  interval.a, True)]
                            for angle, arrowhead in angles:
                                facings_key = (
                                 point, angle, arrowhead)
                                if facings_key not in drawn_facings:
                                    drawn_facings.append(facings_key)
                                    layer.add_arrow(point, angle, end_arrow=arrowhead, length=0.2, color=color, altitude=facing_altitude)
                            ...
                            if point_key not in drawn_points:
                                ...
                                if constraint.multi_surface:
                                    if constraint.routing_surface.type in routing.object_routing_surfaces:
                                        layer.add_point(point, color=color, altitude=(facing_altitude - MULTISURFACE_OFFSET))
                                    else:
                                        layer.add_point(point, color=color, altitude=(facing_altitude + MULTISURFACE_OFFSET))
                                layer.add_point(point, color=color, altitude=facing_altitude)
                ...
                if draw_contours:
                    ...
                    pt1 = poly[0][0]
                    lb, ub = sims4.math.Vector2(pt1.x, pt1.z), sims4.math.Vector2(pt1.x, pt1.z)
                    for poly in constraint.geometry.polygon:
                        for vertex in poly:
                    ...
                    if isolines:
                        value_min, value_max = isolines[0][0], isolines[(-1)][0]
                        for value, line in isolines:
                            ...
                            for i in range(len(line) - 1):
                                v1 = line[i]
                                v2 = line[(i + 1)]
                                layer.add_segment((sims4.math.Vector3(v1.x, 0, v1.y)),
                                  (sims4.math.Vector3(v2.x, 0, v2.y)),
                                  color=contour_color,
            ...
            if isinstance(constraint, RequiredSlotSingle):
                for routing_transform, reference_joint, _ in itertools.chain(constraint._slots_to_params_entry or (), constraint._slots_to_params_exit or ()):
                    if routing_transform is None:
                        ...
                        _draw_constraint(layer, universal_constraint, color=semitransparent,
                          altitude=altitude,
                          draw_contours=draw_contours,
                          modify_altitiude=modify_altitiude)
                    else:
                        layer.add_arrow_for_transform(routing_transform, length=0.1, color=semitransparent, altitude=altitude)
                        layer.add_segment((routing_transform.translation), (constraint.containment_transform.translation), color=transparent,
                          altitude=altitude)

    elif isinstance(constraint, Anywhere):
        if anywhere_position is not None:
            layer.add_circle(anywhere_position, radius=0.28, color=transparent, altitude=altitude)
            layer.add_circle(anywhere_position, radius=0.3, color=semitransparent, altitude=altitude)


def _draw_contour(layer, contour, color, altitude=None, wrap_final_edge=True):
    if not contour:
        return
    length = len(contour)
    if length != 0:
        for i in range(length - 1):
            layer.add_segment((contour[i]), (contour[(i + 1)]), color=color, altitude=altitude)

        if wrap_final_edge:
            layer.add_segment((contour[(length - 1)]), (contour[0]), color=color, altitude=altitude)
...
class SimLOSVisualizer:
    ...
    def _on_posture_event(self, change, dest_state, track, source_posture, dest_posture):
        return self._sim is None or PostureTrack.is_body(track) or None
            ...
            for si in sim.si_state:
                ...
                for si_constraint in si.constraint_intersection(participant_type=participant_type):
                    ...
                    if si.is_guaranteed():
                        ...
                        si_altitude = 0.02
                    _draw_constraint(layer, si_constraint, si_color, altitude=si_altitude)

            _draw_constraint(layer, constraint, color, altitude=0.03, anywhere_position=(sim.position), draw_contours=True)
