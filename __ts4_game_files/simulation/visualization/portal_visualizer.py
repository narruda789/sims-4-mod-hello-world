...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\visualization\portal_visualizer.py
# Compiled at: 2019-01-02 18:20:34
# Size of source mod 2**32: 4478 bytes
    ...
    def _draw_portal_obj(self, portal_obj, *args, portal_id=0, **kwargs):
        with Context((self.layer), preserve=True) as (layer):
            for portal_instance in portal_obj.get_portal_instances():
                if portal_id:
                    if not portal_id == portal_instance.there:
                        if not portal_id == portal_instance.back:
                            continue
