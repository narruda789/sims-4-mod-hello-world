...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\lot_decoration\decoratable_lot.py
# Compiled at: 2018-08-24 16:26:52
...
import build_buy, services, enum
...
class DecoratableLot:
    ...
    def save_deco_states_to_proto(self, lot_decorations_proto):
        if not self._decoration_states:
            if self.deco_type_id == DEFAULT_DECORATION_PROVIDER.decoration_type_id:
                return
    ...
    def reset_decorations(self):
        if self.deco_type_id in self._decoration_states:
            ...
            del (self.deco_state.custom_decorations or self._decoration_states)[self.deco_type_id]
        else:
