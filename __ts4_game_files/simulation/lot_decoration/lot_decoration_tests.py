...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\lot_decoration\lot_decoration_tests.py
# Compiled at: 2018-11-05 17:56:38
# Size of source mod 2**32: 5532 bytes
    ...
    def perform_test(self, tooltip):
        ...
        if lot_decoration_service is None:
            ...
            if deco_type_id is None:
                ...
                deco_type_id = 0
            return lot_decoration_service.does_lot_have_custom_decorations(deco_type_id) or TestResult(False, 'Lot has no custom decorations for id {}.', (self.occasion()), tooltip=tooltip)
        return TestResult.TRUE
...
class LotDecorationTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    FACTORY_TUNABLES = {'test': TunableVariant(description='\n            The test we want to run.\n            ',
               decorations_available=(_LotDecorationHasDecorationsAvailableTest.TunableFactory()),
               holiday_decoratable=(_LotDecorationDecoratableHolidayTest.TunableFactory()),
               custom_decorations=(_LotDecorationHasCustomDecorationsTest.TunableFactory()),
               active_lot_decorated_for_occasion=(_ActiveLotIsDecoratedForSomeOccasionTest.TunableFactory()),
               default='decorations_available')}
