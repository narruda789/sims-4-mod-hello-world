...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\distributor\system.py
# Compiled at: 2018-11-02 15:02:39
# Size of source mod 2**32: 25274 bytes
...
import elements, gsi_handlers, reset, services, sims4.reload
__all__ = [
 'Distributor']
...
class Journal:
    JournalEntry = namedtuple('JournalEntry', ('object', 'protocol_buffer', 'payload_type',
    ...
    def add(self, obj, op, ignore_deferral=False):
        ...
        if op.block_on_task_owner:
            ...
            if obj.manager is not None:
                if obj.manager.id == MGR_SIM_INFO:
    ...
    def _build_journal_entry(self, journal_seed):
        ...
        if op._force_execution_on_tag:
            ...
            elif mask_override is not None and mask_override != DEFAULT_MASK:
...
class Distributor:
    ...
    def add_object(self, obj):
        ...
        if not obj.visible_to_client:
            ...
            return services.client_manager() or None
    ...
    def remove_object(self, obj, **kwargs):
        ...
        if was_visible:
            if hasattr(obj, 'on_remove_from_client'):
    ...
    def _debug_validate_op(self, obj, op):
        ...
        if objs:
            ...
            if obj not in self.created_objects:
                logger.callstack('Target of Op ({}) is not on the client. Operation type: {}. \n                    This will result in errors in client state.',
                  obj,
                  op, level=(sims4.log.LEVEL_ERROR))
            if not getattr(obj, 'valid_for_distribution', True):
                if not getattr(op, 'is_create_op', False):
                    logger.callstack('Operation is being added for {}, but it is not valid_for_distribution. \n                        Operation type: {}. This will result in errors in client state.',
                      obj,
                      op, level=(sims4.log.LEVEL_ERROR))
