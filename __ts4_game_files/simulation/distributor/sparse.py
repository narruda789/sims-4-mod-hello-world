...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\distributor\sparse.py
# Compiled at: 2015-03-23 18:20:47
# Size of source mod 2**32: 18613 bytes
import functools
...
def get_sparse_data_attribute_for_message(message_or_type):
    ...
    for field_number in SPARSE_MESSAGE_DATA_FIELD_NUMBERS:
        if field_number in fields_by_number:
...
class RepeatedFieldWrapper:
    ATTRS_TO_WRAP = [
     '__delitem__', '__setitem__', 'append', 'extend', 'insert', 'remove', 'sort']
...
class SparseMessage:
    ...
    def __setattr__(self, name, value):
        ...
        elif name in self._repeated_attrs:
    ...
    def __delattr__(self, name):
        if self._is_self_attr(name):
            ...
            ret = self._set_fields or self._clear_field(self._sparse_data_attr)
        return ret
...
class SparseField(Field):
    ...
    def __get__(self, *args, for_create=False, **kwargs):
        value = (super().__get__)(args, for_create=for_create, **kwargs)
        if for_create:
