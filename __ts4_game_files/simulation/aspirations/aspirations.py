...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\aspirations\aspirations.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 37329 bytes
import collections
import event_testing.event_data_tracker as data_tracker
import event_testing.test_events as test_events
import gsi_handlers.aspiration_handlers, services, sims4.log, telemetry_helper, weakref
...
class AspirationTracker(data_tracker.EventDataTracker, SimInfoTracker):
    ...
    def register_additional_objectives(self, aspiration, objectives, owner=None):
        if aspiration in self._additional_objectives:
            ...
            if addtl_obj.owner is not None:
                if owner is not None:
                    if addtl_obj.owner is not owner:
                        ...
                        return
            if addtl_obj.owner is None and owner is not None:
    ...
    def initialize_aspiration(self, from_load=False):
        if self.owner_sim_info is not None:
            if not self.owner_sim_info.is_baby:
    ...
    def _should_handle_event(self, milestone, event, resolver):
        ...
        if aspiration.aspiration_type == AspriationType.FULL_ASPIRATION:
            if aspiration.do_not_register_events_on_load:
        ...
        if aspiration.aspiration_type == AspriationType.CAREER:
            ...
            return actor is None or actor.is_sim or False
    ...
    def gsi_event(self, event):
        return {'sim':self._owner_ref().full_name if self._owner_ref() is not None else 'None', 
         'event':str(event)}
    ...
    def _check_and_complete_aspiration_track(self, aspiration_track, completed_aspiration_id, sim_info):
        if all((self.milestone_completed(track_aspiration) for track_aspiration in aspiration_track.aspirations.values())):
            ...
            dialog.show_dialog(icon_override=IconInfoData(icon_resource=(aspiration_track.icon)), secondary_icon_override=IconInfoData(obj_instance=sim_info),
              additional_tokens=(
             reward_text,),
    ...
    def complete_milestone(self, aspiration, sim_info):
        ...
        if aspiration_type == AspriationType.FULL_ASPIRATION and not aspiration.is_valid_for_sim(sim_info):
            return
            ...
            if aspiration.reward is not None:
                ...
                if track is None:
                    ...
                    if aspiration_type == AspriationType.FAMILIAL:
                        super().complete_milestone(aspiration, sim_info)
                        ...
                        if aspiration_type == AspriationType.WHIM_SET:
                            ...
                            if not (tutorial_service is None or tutorial_service.is_tutorial_running()):
                                dialog = aspiration.notification(sim_info, SingleSimResolver(sim_info))
                                dialog.show_dialog(event_id=(aspiration.guid64))
                            super().complete_milestone(aspiration, sim_info)
                        else:
                            if aspiration_type == AspriationType.ASSIGNMENT:
                                super().complete_milestone(aspiration, sim_info)
                                aspiration.satisfy_assignment(sim_info)
                            else:
                                if aspiration_type == AspriationType.GIG:
                                    super().complete_milestone(aspiration, sim_info)
                                    aspiration.satisfy_assignment(sim_info)
                                else:
                                    if aspiration_type == AspriationType.ZONE_DIRECTOR:
                                        super().complete_milestone(aspiration, sim_info)
                                        zone_director = services.venue_service().get_zone_director()
                                        zone_director.on_zone_director_aspiration_completed(aspiration, sim_info)
                                    else:
                                        if aspiration_type == AspriationType.TIMED_ASPIRATION:
                                            super().complete_milestone(aspiration, sim_info)
                                            self._timed_aspirations[aspiration].complete()
                                        else:
                                            if aspiration_type == AspriationType.CAREER:
                                                super().complete_milestone(aspiration, sim_info)
                                                if aspiration.screen_slam is not None:
    ...
    def complete_objective(self, objective_instance, aspiration):
        ...
        if owner_ref is not None:
            objective_instance.apply_completion_loot(owner_ref)
            if objective_instance.satisfaction_points > 0:
                owner_ref.apply_satisfaction_points_delta((objective_instance.satisfaction_points), (SetWhimBucks.ASPIRATION),
                  source=(objective_instance.guid64))
        return result
    ...
    def force_send_data_update(self):
        for aspiration in services.get_instance_manager(sims4.resources.Types.ASPIRATION).types.values():
            ...
            if aspiration_type != AspriationType.FULL_ASPIRATION:
                if aspiration_type != AspriationType.SIM_INFO_PANEL:
                    continue
            for objective in self.get_objectives(aspiration):
                self.update_objective(objective, 0,
                  (objective.goal_value()),
                  (objective.is_goal_value_money),
                  (objective.show_progress),
                  from_init=True)
