...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\bucks\club_bucks_tracker.py
# Compiled at: 2020-05-14 19:52:55
...
class ClubBucksTracker(BucksTrackerBase):
    ...
    BUCKS_TRACKER_REWARDS_CATEGORIES = TunableMapping(description='\n        Ordered list of Club Bucks Reward categories that will appear in the \n        Club Bucks rewards UI along with the perks that belong in the category.\n        ',
      ...
      pack_safe=True),
      tuple_name='RewardCategoryMapping'),
      export_class_name='RewardCategoryInfoTuple'),
      tuple_name='CategoryMapping',
      export_modes=(ExportModes.ClientBinary))

    def try_modify_bucks(self, bucks_type, amount, allow_distribute=True, reason=None, from_load=False, suppress_telemetry=False):
        result = super().try_modify_bucks(bucks_type, amount, allow_distribute=allow_distribute, reason=reason, from_load=from_load, suppress_telemetry=suppress_telemetry)
        if amount > 0:
    ...
    def load_data(self, owner_proto):
        ...
        if ClubBucksTracker.EP02_CLUB_BUCKS in self._bucks:
            ...
            self.try_modify_bucks((ClubTunables.CLUB_BUCKS_TYPE), old_bucks_amount, allow_distribute=False)