...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\commodity_tracker_gsi_util.py
# Compiled at: 2020-08-06 16:34:33
...
def generate_data_from_commodity(stat, statistic_component):
    ...
    entry = {'stat_guid':str(stat.guid64), 
     ...
     'instanced':'x' if instanced else '', 
     'decay_rate':stat.get_decay_rate() if instanced else '', 
     'change_rate':stat.get_change_rate() if instanced else '', 
     'decay_enabled':'x' if stat.decay_enabled else '', 
     'convergence_value':stat.convergence_value if instanced else '', 
     'time_till_callback':str(stat._alarm_handle.get_remaining_time()) if instanced and stat._alarm_handle is not None else '', 
     'active_callback':str(stat._active_callback) if instanced and stat._active_callback is not None else '', 
     'delayed_decay_timer':str(stat.get_time_till_decay_starts()) if instanced else ''}
