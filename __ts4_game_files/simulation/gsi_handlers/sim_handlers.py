...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\sim_handlers.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 72887 bytes
import itertools
...
import alarms, date_and_time, objects.components.types, services, sims4
...
FILTER_AVAILABLE_FOR_SIM = 'Available for Sim'
FILTER_UNAVAILABLE_FOR_SIM = 'Unavailable for Sim'
...
def generate_sim_skill_view_data(sim_id: int=None):
    ...
    if cur_sim_info is not None:
        for stat in cur_sim_info.all_skills():
            ...
            entry = {'simId':str(sim_id),  'skill_guid':str(stat.guid64), 
             'skill_name':type(stat).__name__, 
             'skill_value':stat.get_value(), 
             'skill_level':skill_level, 
             'skill_effective_level':effective_skill_level}
            ...
            if effective_skill_level != skill_level:
                for buff_type, modifier in cur_sim_info.effective_skill_modified_buff_gen(stat):
                    buff_entry = {'buff':buff_type.__class__.__name__, 
...
commodity_data_schema = create_schema_for_commodity_tracker('Statistics/Continuous Statistic Data', sim_specific=True)
...
def generate_sim_commodity_data_view_data(sim_id: int=None):
    ...
    if commodity_tracker is None:
        return []
    statistic_component = cur_sim_info.get_component(objects.components.types.STATISTIC_COMPONENT)
    stat_data = []
    for stat in commodity_tracker.all_statistics():
        entry = generate_data_from_commodity(stat, statistic_component)
        if stat.is_commodity:
            if hasattr(stat, '_buff_handle'):
            ...
            if hasattr(stat, '_distress_buff_handle'):
            ...
            if hasattr(stat, '_skill_level_buff'):
...
def generate_sim_info_toolbar_data(*args, zone_id: int=None, **kwargs):
    ...
    for sim_info in list(sim_info_manager.objects):
        if not sim_info.first_name:
            ...
            cur_data_entry = {'simId':str(hex(sim_info.sim_id)),  'fullName':full_name,  'selectable':sim_info.is_selectable, 
             'isInstanced':sim_info.is_instanced()}
...
with sim_info_schema.add_has_many('occult', GsiGridSchema, label='Occult') as (sub_schema):
    ...
    sub_schema.add_field('occult_skin_tone_val_shift', label='Skin Tone Val Shift')
...
with sim_info_schema.add_has_many('outfits', GsiGridSchema, label='Outfits') as (sub_schema):
    ...
    sub_schema.add_field('title', label='Title')
...
def generate_sim_info_data(*args, zone_id: int=None, **kwargs):
    ...
    for sim_info in list(sim_info_manager.objects):
        ...
        entry = {'simId':str(hex(sim_info.sim_id)),  'firstName':sim_info.first_name, 
         'lastName':sim_info.last_name, 
         'fullName':sim_info.full_name, 
         'gender':format_enum_name(sim_info.gender), 
         'age':format_enum_name(sim_info.age), 
         'species':species_str, 
         'lod':format_enum_name(sim_info.lod), 
         'selectable':sim_info.is_selectable, 
         'zone_id':sim_info.zone_id}
        sim = sim_info.get_sim_instance(allow_hidden_flags=ALL_HIDDEN_REASONS)
        aging_info = []
        entry['aging'] = aging_info
        entry['aging'].append({'aging_field_name':'Can Age', 
         'aging_field_value':str(sim_info.can_age_up())})
        entry['aging'].append({'aging_field_name':'Can Die', 
         'aging_field_value':str(not sim_info.is_death_disabled())})
        entry['aging'].append({'aging_field_name':'Progress', 
         'aging_field_value':'{}, {:.2%}'.format(sim_info.age_progress, sim_info.age_progress / sim_info.get_age_transition_data(sim_info.age).get_age_duration(sim_info))})
        entry['aging'].append({'aging_field_name':'Time to Notification', 
         'aging_field_value':str(sim_info._almost_can_age_handle.get_remaining_time()) if sim_info._almost_can_age_handle is not None else '-'})
        entry['aging'].append({'aging_field_name':'Time to Availability', 
         'aging_field_value':str(sim_info._can_age_handle.get_remaining_time()) if sim_info._can_age_handle is not None else '-'})
        entry['aging'].append({'aging_field_name':'Time to Auto-Age', 
         'aging_field_value':str(sim_info._auto_age_handle.get_remaining_time()) if sim_info._auto_age_handle is not None else '-'})
        occult_info = []
        ...
        for occult_type in OccultType:
            ...
            occult_entry = {'occult_type':str(occult_type),  'occult_is_available':'X' if sim_info.occult_tracker.has_occult_type(occult_type) else '', 
             'occult_is_current':'X' if sim_info.current_occult_types == occult_type else '', 
             'occult_facial_attributes':str(occult_sim_info.facial_attributes)[:32] if occult_sim_info is not None else '', 
             'occult_physique':str(occult_sim_info.physique) if occult_sim_info is not None else '', 
             'occult_skin_tone':str(occult_sim_info.skin_tone) if occult_sim_info is not None else '', 
             'occult_skin_tone_val_shift':str(occult_sim_info.skin_tone_val_shift) if occult_sim_info is not None else '', 
             'occult_voice_actor':str(occult_sim_info.voice_actor) if occult_sim_info is not None else '', 
             'occult_voice_pitch':str(occult_sim_info.voice_pitch) if occult_sim_info is not None else '', 
             'occult_voice_effect':str(occult_sim_info.voice_effect) if occult_sim_info is not None else '', 
             'occult_plumbbob_override':str(sim_info.plumbbob_override) if sim_info.current_occult_types == occult_type else '', 
             'occult_outfit_categories':occult_outfit_categories, 
             'occult_current_outfit':str(occult_sim_info.get_current_outfit()) if occult_sim_info is not None else ''}
        ...
        if current_outfit:
            ...
            for outfit_entry in all_outfit_entries:
                ...
                entry['outfits'].append({'outfit_category':str(outfit_entry[0]), 
                 'outfit_index':str(outfit_entry[1]), 
                 'outfit_id':str(outfit_data.outfit_id), 
                 'title':str(outfit_data.title), 
                 'outfit_flags':str(outfit_data.outfit_flags), 
                 'outfit_extra_info':'Current Outfit' if outfit_entry == current_outfit else 'Previous Outfit' if outfit_entry == previous_outfit else '', 
                 'part_ids':', '.join((str(part_id) for part_id in outfit_data.part_ids))})

            if current_outfit not in all_outfit_entries:
                entry['outfits'].append({'outfit_category':str(current_outfit[0]), 
                 'outfit_index':str(current_outfit[1]), 
                 'outfit_id':'<INVALID OUTFIT>', 
                 'title':'<INVALID OUTFIT TITLE>', 
                 'outfit_extra_info':'Current Outfit'})
            else:
                if previous_outfit not in all_outfit_entries:
                    entry['outfits'].append({'outfit_category':str(current_outfit[0]), 
                     ...
                     'outfit_id':'<INVALID OUTFIT>', 
                     'title':'<INVALID OUTFIT TITLE>', 
                     'outfit_extra_info':'Previous Outfit'})
                entry['pregnancy'] = []
                entry['pregnancy'].append({'pregnancy_field_name':'Is Pregnant', 
                 'pregnancy_field_value':str(sim_info.is_pregnant)})
                if sim_info.is_pregnant:
                    ...
                    entry['pregnancy'].append({'pregnancy_field_name':'Progress', 
                     'pregnancy_field_value':'<None>' if pregnancy_commodity is None else '{:.2%}'.format(pregnancy_commodity.get_value() / pregnancy_commodity.max_value)})
                    entry['pregnancy'].append({'pregnancy_field_name':'Parents', 
                     'pregnancy_field_value':', '.join((('<None>' if p is None else p.full_name) for p in pregnancy_tracker.get_parents()))})
                    entry['pregnancy'].append({'pregnancy_field_name':'Seed', 
                     'pregnancy_field_value':str(pregnancy_tracker._seed)})
                    entry['pregnancy'].append({'pregnancy_field_name':'Origin', 
                     'pregnancy_field_value':str(pregnancy_tracker._origin)})
        ...
        for tracker_attr in SimInfo.SIM_INFO_TRACKERS:
            entry['trackers'].append({'tracker':tracker_attr, 
             'exists':bool_to_str(getattr(sim_info, tracker_attr, None) != None)})
...
def create_state_info_entry(interaction, interaction_pos):
    ...
    entry = {'interactionId':interaction.id, 
     ...
     'interactionPos':interaction_pos, 
     'group_id':interaction.group_id, 
     'running':bool_to_str(interaction.running), 
     'priority':interaction.priority.name, 
     'isSuper':bool_to_str(interaction.is_super), 
     'isFinishing':bool_to_str(interaction.is_finishing), 
     'allowAuto':bool_to_str(interaction.allow_autonomous), 
     'allowUser':bool_to_str(interaction.allow_user_directed), 
     'visible':bool_to_str(interaction.visible), 
     'is_guaranteed':bool_to_str(interaction.is_guaranteed()), 
     'icon':str(interaction.get_icon_info())}
    ...
    if interaction._conditional_action_manager is not None:
        ...
        for group in interaction._conditional_action_manager:
            ...
            if not interaction.progress_bar_enabled.bar_enabled:
                ...
                if interaction.progress_bar_enabled.force_listen_statistic:
    ...
    if cur_sim is not None:
        if cur_sim.posture_state is not None:
            for posture_aspect in cur_sim.posture_state.aspects:
                ...
                cur_posture_info = {'postureType':type(posture_aspect).name,  'postureName':str(posture_aspect), 
                 'postureTarget':str(posture_aspect.target), 
                 'postureSpec':str(cur_sim.posture_state.spec), 
                 'sourceInteraction':str(posture_aspect.source_interaction), 
                 'owningInteraction':' '.join(owning_interaction_strs), 
                 'asm_registry':posture_aspect.get_asm_registry_text()}
                sim_posture_info.append(cur_posture_info)

        else:
            cur_posture_info = {'postureType':'---', 
             'postureName':'Sim Posture State is None'}
            sim_posture_info.append(cur_posture_info)
...
def generate_sim_ui_manager_view_data(sim_id: int=None):
    ...
    if cur_sim is not None:
        for int_info in cur_sim.ui_manager.get_interactions_gen():
            entry = {'interaction_id':int_info.interaction_id, 
             'insert_after_id':int_info.insert_after_id, 
             'target':str(int_info.target), 
             'canceled':int_info.canceled, 
             'ui_state':str(int_info.ui_state), 
             'to_be_canceled':str(int_info.interactions_to_be_canceled), 
             'super_id':int_info.super_id, 
             'associated_skill':str(int_info.associated_skill), 
             'visual_type':_visual_type_to_string(int_info.ui_visual_type), 
             'priority':str(int_info.priority)}
...
def generate_sim_topics_view_data(sim_id: int=None):
    ...
    if cur_sim is not None:
        for topic in cur_sim.get_topics_gen():
            ...
            topics_view_data.append({'sim_id':str(sim_id),  'topic_type':topic.__class__.__name__, 
             'current_relevancy':str(topic.current_relevancy), 
             'target':str(topic_target), 
...
def generate_multi_motive_view_data(sim_id: int=None):
    ...
    if cur_sim is not None:
        for multi_motive_tracker in cur_sim._multi_motive_buff_trackers:
            ...
            entry = {'buff_to_add':str(buff_to_add), 
             'buff_added':'x' if cur_sim.has_buff(buff_to_add) else '', 
             'count':'{}/{}'.format(multi_motive_tracker._motive_count, len(multi_motive_tracker._multi_motive_buff_motives)), 
             'watcher_add':'x' if multi_motive_tracker._watcher_handle is not None else '', 
             'statistics':[]}
            for stat_type, callback_data in tuple(multi_motive_tracker._commodity_callback.items()):
                ...
                stat_entry = {'statistic':str(stat_type), 
                 'tuned_threshold':str(threshold), 
                 'callback_threshold':str(callback_data.threshold) if callback_data is not None else 'Stat not available'}
...
with sim_buff_schema.add_has_many('reference_modifiers', GsiGridSchema, label='Reference Modifiers') as (sub_schema):
    ...
    sub_schema.add_field('interaction_tags', label='Interaction Tags')
    sub_schema.add_field('blacklist_interaction_tags', label='Blacklist Interaction Tags')
    ...
    if sim_info is not None:
        ...
        for buff in buff_component:
            entry = {'sim_id':str(sim_id), 
             'name':buff.__class__.__name__, 
             'visible':str(buff.visible), 
             'success_modifier':buff.success_modifier, 
             'mood':buff.mood_type.__name__ if buff.mood_type is not None else 'None', 
             'mood_weight':buff.mood_weight, 
             'mood_override':buff.mood_override.__name__ if buff.mood_override is not None else 'None', 
             'exclusive_index':str(buff.exclusive_index)}
            ...
            if buff.interactions is not None:
                for mixer_affordance in buff.interactions.interaction_items:
                    ...
                    if mixer_affordance.lock_out_time is not None:
                        ...
                        max_lockout = 0
                    if sim is not None:
                        if mixer_affordance.lock_out_time_initial is not None or mixer_affordance.lock_out_time is not None:
                    ...
                    entry['interactions'].append({'affordance':mixer_affordance.__name__,  'min_lockout_initial':min_lockout_initial, 
                     'max_lockout_initial':max_lockout_initial, 
                     'min_lockout':min_lockout, 
            ...
            for index, modifier in enumerate(buff.game_effect_modifier._game_effect_modifiers):
                if isinstance(modifier, AutonomyModifier):
                    for attribute_name, attribute_value in vars(modifier).items():
                        entry['autonomy_modifiers'].append({'game_effect_index':index,  'attribute_name':attribute_name, 
                         'attribute_value':str(attribute_value)})

                else:
                    buff_entry = {'type': str(modifier.modifier_type)}
                    if modifier.modifier_type == GameEffectType.AFFORDANCE_MODIFIER:
                        buff_entry['reference'] = (
                         '{}'.format([reference_name for reference_name in modifier.debug_affordances_gen()]),)
                        buff_entry['interaction_tags'] = ('{}'.format([tag.name for tag in modifier._interaction_category_tags]),)
                        buff_entry['blacklist_interaction_tags'] = ('{}'.format([tag.name for tag in modifier._interaction_category_blacklist_tags]),)
                        buff_entry['score_modifier'] = (modifier._score_bonus,)
                        buff_entry['success_modifier'] = (modifier._success_modifier,)
                    else:
                        if modifier.modifier_type == GameEffectType.EFFECTIVE_SKILL_MODIFIER:
...
sim_buff_list_schema = GsiGridSchema(label='Buffs Available', auto_refresh=False, exclude_from_dump=True, sim_specific=True)
sim_buff_list_schema.add_field('sim_id', label='SimId', hidden=True)
sim_buff_list_schema.add_field('buff_id', label='Buff ID', type=(GsiFieldVisualizers.INT))
sim_buff_list_schema.add_field('buff_name', label='Name')
sim_buff_list_schema.add_field('visible', label='Visible')
sim_buff_list_schema.add_field('add_test_set', label='Add Test Set')
sim_buff_list_schema.add_filter(FILTER_AVAILABLE_FOR_SIM)
sim_buff_list_schema.add_filter(FILTER_UNAVAILABLE_FOR_SIM)
with sim_buff_list_schema.add_view_cheat('sims.add_buff', label='Add Selected Buff') as (cheat):
...
for pack_name in sorted(pack_names):
    sim_buff_list_schema.add_filter(pack_name)

@GsiHandler('buff_list', sim_buff_list_schema)
def generate_available_buffs_list(*args, sim_id: int=None, filter=None, **kwargs):
    filter_list = parse_filter_to_list(filter)
    all_buffs = []
    sim = services.object_manager().get(sim_id)
    sim_info = sim.sim_info
    buff_instance_manager = services.get_instance_manager(Types.BUFF)
    for key, buff in buff_instance_manager.types.items():
        if key in buff_instance_manager.remapped_keys:
            pack_specific_key = buff_instance_manager.remapped_keys[key]
            pack_id = str(Pack(pack_specific_key.group))
        else:
            pack_id = str(Pack.BASE_GAME)
        if filter_list is not None:
            if pack_id not in filter_list:
                continue
        elif buff.can_add(sim_info):
            if FILTER_AVAILABLE_FOR_SIM not in filter:
                continue
        elif FILTER_UNAVAILABLE_FOR_SIM not in filter:
            continue
        all_buffs.append({'sim_id':str(sim_id),  'buff_id':buff.guid64, 
         'buff_name':buff.__name__, 
         'visible':buff.visible, 
         'add_test_set':str(buff.add_test_set)})
    ...
    if sim_info is not None:
        for trait in sim_info.trait_tracker.equipped_traits:
            entry = {'sim_id':str(sim_id), 
             ...
             'num_buffs':len(trait.buffs)}
            entry['linked_buffs'] = []
            for buff in trait.buffs:
                buff_type = buff.buff_type
                buff_entry = {'name':buff_type.__name__, 
                 'visible':str(buff_type.visible), 
                 'commodity':buff_type.commodity.__name__ if buff_type.commodity is not None else 'None', 
                 'mood':buff_type.mood_type.__name__ if buff_type.mood_type is not None else 'None', 
                 'mood_weight':buff_type.mood_weight}
...
sim_motives_graph_schema = GsiLineGraphSchema(label='Motives Graph', x_axis_label='X-Axis', y_axis_label='Y-Axis', sim_specific=True, y_min=(-100),
  y_max=100)
...
def generate_sim_vital_view_data(sim_id: int=None):
    ...
    entry = {'sim_id':str(sim_id), 
     'name':'LOD', 
     'type':sim_info.lod.__class__.__name__, 
     'visible':'False', 
     'data':'value={}'.format(sim_info.lod)}
    ...
    if buff_component is not None:
        for buff in buff_component:
            entry = {'sim_id':str(sim_id), 
             ...
             'type':'Buff', 
             'visible':str(buff.visible), 
             'data':'visible={}'.format(buff.visible)}
            vital_view_data.append(entry)
    ...
    if sim_info.trait_tracker.equipped_traits is not None:
        for trait in sim_info.trait_tracker.equipped_traits:
            entry = {'sim_id':str(sim_id), 
             'name':trait.__name__, 
             'type':'Trait', 
             'data':'type={}'.format(trait.trait_type.name)}
            vital_view_data.append(entry)

    if sim_info.statistic_tracker is not None:
        for stat in list(sim_info.statistic_tracker):
            entry = {'simId':str(sim_id), 
             'name':type(stat).__name__, 
             'type':'Statistic', 
             'data':'min={}, max={}, current={}'.format(stat.min_value, stat.max_value, stat.get_value())}
            vital_view_data.append(entry)
    ...
    if sim_info.static_commodity_tracker is not None:
        for stat in list(sim_info.static_commodity_tracker):
            entry = {'name':type(stat).__name__, 
             'type':'Static Commodity', 
             'data':'ad_data={}'.format(stat.ad_data)}
            vital_view_data.append(entry)
...
commodity_and_stat_view_schema.add_filter('werewolfpackvalue')
...
def generate_sim_commodity_and_stat_view_data(sim_id: int=None, filter=None):
    filter_list = parse_filter_to_list(filter)
    if filter_list is not None:
        if FILTER_WORKING_SET in filter_list:
            filter_list.extend((f for f in FILTER_WORKING_SET_FILTERS if f not in filter_list))

    def stat_name_passes_filters(stat_name):
        if filter_list is not None:
            if not any((a_filter.lower() in stat_name.lower() for a_filter in filter_list)):
                return False
        return True
    ...
    if cur_sim_info is not None:
        if cur_sim_info.static_commodity_tracker is not None:
            for stat in cur_sim_info.commodity_tracker.all_statistics():
                if not stat.is_commodity:
                    continue
                stat_name = stat.stat_type.__name__
                if not stat_name_passes_filters(stat_name):
                    continue
                stat_value = stat.get_value()
                data.append({'simId':str(sim_id),  'name':stat_name, 
                 'value':stat_value, 
                 'percentFull':stat_value / stat.max_value * 100 if stat.max_value != 0 else 0})

        if cur_sim_info.statistic_tracker is not None:
            for stat in cur_sim_info.statistic_tracker.all_statistics():
                stat_name = stat.stat_type.__name__
                if not stat_name_passes_filters(stat_name):
                    continue
                stat_value = stat.get_value()
            ...
            for stat in cur_sim_info.all_skills():
                ...
                data.append({'simId':str(sim_id),  'name':stat_name, 
                 ...
                 'percentFull':stat.get_value() / stat.max_value * 100})
