...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\drama_handlers.py
# Compiled at: 2021-03-30 14:24:27
...
FILTER_STRINGS = [
 NONPICKERDRAMANODES_DRAMA_NODES_FILTER, 'actorcareer', 'freelancer', 'oddjob', 'interiordecorator']

def drama_node_string_matches_filters(drama_node_string, filter_list):
    if filter_list is None:
        return True
    drama_node_string = drama_node_string.lower()
    if NONPICKERDRAMANODES_DRAMA_NODES_FILTER in filter_list:
        if PICKER_DRAMA_NODE_SUBSTRING not in drama_node_string:
            return True
    if any((a_filter in drama_node_string for a_filter in filter_list)):
        return True
    return False
...
def generate_drama_scheduler_data(zone_id: int=None, filter=None):
    ...
    for drama_node in drama_scheduler.active_nodes_gen():
        if drama_node_matches_filters(drama_node):
            all_nodes.append({'drama_node_id':str(drama_node.uid), 
             'status':'Active', 
             'drama_node':str(drama_node), 
             'receiver_sim':str(drama_node.get_receiver_sim_info()), 
             'sender_sim':str(drama_node.get_sender_sim_info())})
    ...
    for drama_node in drama_scheduler.scheduled_nodes_gen():
        if drama_node_matches_filters(drama_node):
            all_nodes.append({'drama_node_id':str(drama_node.uid), 
             'drama_node':str(drama_node), 
             'status':'Scheduled', 
             'time_left':str(drama_node.get_time_remaining()), 
             'receiver_sim':str(drama_node.get_receiver_sim_info()), 
             'sender_sim':str(drama_node.get_sender_sim_info())})
...
class GSIRejectedDramaNodeScoringData:
    ...
    def get_gsi_view_dictionary(self):
        return {'drama_node':str(self.drama_node), 
         'reason':self.reason, 
...
with drama_scheduler_archive_schema.add_has_many('Chosen Nodes', GsiGridSchema) as (sub_schema):
    ...
    sub_schema.add_field('sender', label='Sender')
    sub_schema.add_field('score_details', label='Score Details', width=6)
scoring_archiver = GameplayArchiver('drama_scoring_archive', drama_scheduler_archive_schema,
...
def archive_drama_scheduler_scoring(scoring_data):
    ...
    entry = {'game_time':str(time), 
     'bucket':str(scoring_data.bucket), 
     'nodes_to_schedule':scoring_data.nodes_to_schedule, 
     'nodes_scheduled':len(scoring_data.chosen_nodes), 
     'Rejected Nodes':[node.get_gsi_view_dictionary() for node in scoring_data.rejected_nodes], 
     'Potential Nodes':[node.get_gsi_view_dictionary() for node in scoring_data.potential_nodes], 
     'Chosen Nodes':[node.get_gsi_view_dictionary() for node in scoring_data.chosen_nodes]}
...
drama_node_log = GameplayArchiver('drama_node_log', drama_node_log_schema,
  add_to_archive_enable_functions=True)
...
def log_drama_node_scoring(drama_node, action, *args):
    ...
    if args:
        ...
        reason = ''
    entry = {'game_time':str(time), 
     'drama_node_id':str(drama_node.uid), 
     'drama_node':str(drama_node), 
     'action':action.name, 
     'reason':reason}
