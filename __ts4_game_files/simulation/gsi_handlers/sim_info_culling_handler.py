...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\sim_info_culling_handler.py
# Compiled at: 2019-07-08 17:25:22
...
import services
CullingCensus = namedtuple('CullingCensus', ('player_households', 'player_sims', 'households',
                                             'sims', 'lod_counts'))
...
with sim_info_culling_archive_schema.add_has_many('households_schema', GsiGridSchema, label='Households') as (sub_schema):
    ...
    sub_schema.add_field('info', label='Info')
    sub_schema.add_field('action', label='Action')
archiver = GameplayArchiver('sim_info_culling', sim_info_culling_archive_schema,
...
class CullingArchive:
    ...
    def add_household_cullability(self, household, score=-1, info=''):
        ...
        self.household_cullabilities[household.id] = (
         score, info)
    ...
    def add_sim_info_cullability(self, sim_info, score=-1, info='', rel_score=None, inst_score=None, importance_score=None):
        ...
        self.sim_info_cullabilities[sim_info.id] = (
         score, info, rel_score,
         inst_score,
         importance_score)
    ...
    def apply(self):
        data = {'game_time':str(services.time_service().sim_now), 
         'reason':self.reason, 
         'player_households':'{} -> {}'.format(self.census_before.player_households, self.census_after.player_households), 
         'player_sims':'{} -> {}'.format(self.census_before.player_sims, self.census_after.player_sims), 
         'households':'{} -> {}'.format(self.census_before.households, self.census_after.households), 
         'sims':'{} -> {}'.format(self.census_before.sims, self.census_after.sims), 
         'full':'{} -> {}'.format(self.census_before.lod_counts[SimInfoLODLevel.FULL], self.census_after.lod_counts[SimInfoLODLevel.FULL]), 
         'interacted':'{} -> {}'.format(self.census_before.lod_counts[SimInfoLODLevel.INTERACTED], self.census_after.lod_counts[SimInfoLODLevel.INTERACTED]), 
         'base':'{} -> {}'.format(self.census_before.lod_counts[SimInfoLODLevel.BASE], self.census_after.lod_counts[SimInfoLODLevel.BASE]), 
         'background':'{} -> {}'.format(self.census_before.lod_counts[SimInfoLODLevel.BACKGROUND], self.census_after.lod_counts[SimInfoLODLevel.BACKGROUND]), 
         'minimum':'{} -> {}'.format(self.census_before.lod_counts[SimInfoLODLevel.MINIMUM], self.census_after.lod_counts[SimInfoLODLevel.MINIMUM])}
        ...
        for sim_id, name in self.sim_id_to_names.items():
            ...
            entry = {'name':name, 
             'score':score, 
             'info':info, 
             'action':self.sim_info_actions.get(sim_id, ''), 
             'rel_score':rel_score, 
        ...
        for household_id, name in self.household_id_to_names.items():
            ...
            entry = {'name':name, 
             'score':score, 
             'info':info, 
             'action':self.household_actions.get(household_id, '')}
