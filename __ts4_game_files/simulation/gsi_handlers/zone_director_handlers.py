...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\zone_director_handlers.py
...
def log_zone_director_event(zone_director, zone, op):
    ...
    archive_data = {'zone_director_type':zone_director.instance_name,  'zone_id':zone.id, 
     'op':op, 
     'neighborhood':neighborhood_data.name, 
     'lot_id':zone.lot.lot_id, 
     'active_venue':type(venue_service.active_venue).__name__, 
     'source_venue':type(venue_service.source_venue).__name__}
    ...
    if provider:
        ...
        for policy in provider.get_civic_policies(tuning=True):
            ...
            entry = {'civic_policy':str(policy), 
             'enacted':'X' if policy in enacted_policies else '', 
             'votes':votes}
