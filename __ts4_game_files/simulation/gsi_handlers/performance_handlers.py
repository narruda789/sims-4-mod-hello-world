...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\performance_handlers.py
# Compiled at: 2019-07-03 15:52:01
# Size of source mod 2**32: 14860 bytes
...
performance_metrics = []
archive_data = {'autonomy_queue_time':0, 
 'autonomy_queue_length':0, 
 'ticks_per_sec':0, 
 'num_sims':0, 
 'num_sim_infos':0, 
 'num_objects_active_lot':0, 
 'num_objects_open_street':0, 
 'num_props':0, 
 'total_objects_props':0}

def enable_performance_logging(*args, enableLog=False, **kwargs):
    global performance_log_alarm
    global previous_log_time_stamp
    global previous_log_time_ticks
    if enableLog:
...
def generate_statistics():
        ...
        for obj in objects:
            ...
            for sa in obj.super_affordances():
                ...
                if sa.allow_user_directed:
                    ...
                    in_user_directed_list = in_user_directed_list or True
    ...
    metrics = [
     (
      consts.OBJS_ACTIVE_LOT_INTERACTIVE, lambda : len(objects_active_lot_interactive)),
     (
      consts.OBJS_ACTIVE_LOT_DECORATIVE, lambda : len(objects_active_lot_decorative)),
     (
      consts.OBJS_OPEN_STREET_INTERACTIVE, lambda : len(objects_open_street_interactive)),
     (
      consts.OBJS_OPEN_STREET_DECORATIVE, lambda : len(objects_open_street_decorative)),
     (
     ...
     (
      consts.OBJS_ACTIVE_LOT_AUTONOMOUS_AFFORDANCE, lambda : len(objects_active_lot_autonomous)),
     (
      consts.OBJS_OPEN_STREET_AUTONOMOUS_AFFORDANCE, lambda : len(objects_open_street_autonomous)),
     (
      consts.OBJS_AUTONOMOUS_AFFORDANCE, lambda : sum(autonomous_counter.values()))]
    details = list()
