...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\game_component_handlers.py
...
def generate_game_info_data():
    ...
    if object_manager is None:
        return game_info
    for obj in object_manager.get_all_objects_with_component_gen(objects.components.types.GAME_COMPONENT):
        ...
        entry = {'current_game':str(game.current_game), 
         'target_object':str(game.target_object), 
         'number_of_players':str(game.number_of_players), 
         'winning_sims':winning_sims, 
         'joinable':str(game.is_joinable()), 
         'requires_setup':str(game.requires_setup), 
         'game_over':str(game.game_has_ended), 
         'high_score':game.high_score, 
         'high_score_sim_ids':high_score_sim_ids}
        entry['teams'] = [{'team_name':game.get_team_name(i),  'team_members':';'.join((str(sim) for sim in team.players)), 
         'team_score':team.score,  'team_rounds_taken':team.rounds_taken} for i, team in enumerate(game._teams)]
...
def archive_game_log_entry(game_object, log_entry_str):
    entry = {'game_object':str(game_object), 
     'log':log_entry_str}
