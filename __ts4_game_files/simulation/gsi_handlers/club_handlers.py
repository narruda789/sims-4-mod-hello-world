...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\club_handlers.py
# Compiled at: 2015-10-22 18:53:54
...
def generate_club_info_data():
    ...
    for club in club_service.clubs:
        ...
        entry = {'name':str(club), 
         ...
         'hangout':club_hangout_str, 
         'associated_color':str(club.associated_color) if club.associated_color else 'None', 
         'uniform_male_child':str(bool(club.uniform_male_child)), 
         'uniform_female_child':str(bool(club.uniform_female_child)), 
         'uniform_male_adult':str(bool(club.uniform_male_adult)), 
         'uniform_female_adult':str(bool(club.uniform_female_adult))}
        members_info = []
        entry['club_members'] = members_info
        for sim in club.members:
            group_members_entry = {'sim_id':str(sim.id), 
