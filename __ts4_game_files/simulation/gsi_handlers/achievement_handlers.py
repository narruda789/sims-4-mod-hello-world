...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\achievement_handlers.py
# Compiled at: 2015-05-05 20:45:02
# Size of source mod 2**32: 4379 bytes
...
def generate_achievement_view_data(sim_id: int=None):
    ...
    for achievement_id in achievement_manager.types:
        ...
        if not sim_info.account.achievement_tracker.milestone_completed(achievement):
            for objective in achievement.objectives:
                objective_data = {}
                objective_data['objective'] = str(objective)
                if sim_info.account.achievement_tracker.objective_completed(objective):
