...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\group_handlers.py
...
def generate_group_data():
    group_data = []
    social_group_manager = services.social_group_manager()
    if social_group_manager is None:
        return group_data
    for group in services.social_group_manager().values():
        entry = {'type':repr(group), 
         'count':len(group), 
         'shutting_down':'x' if group.has_been_shutdown else '', 
         'anchor':str(getattr(group, '_anchor', None))}
        state_info = []
        entry['states'] = state_info
        if group.state_component is not None:
            for state, value in group.state_component.items():
                state_entry = {'state':str(state), 
                 'value':str(value)}
        ...
        for sim in group:
            ...
            group_members_entry = {'sim_id':str(sim.id),  'sim_name':sim.full_name, 
             'registered_si':str(interactions), 
             'social_context':str(sim.get_social_context())}
