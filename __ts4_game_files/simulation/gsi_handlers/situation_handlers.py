...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\situation_handlers.py
# Compiled at: 2022-02-09 13:21:47
# Size of source mod 2**32: 18599 bytes
...
import date_and_time, services, tag
...
def generate_situation_data(zone_id: int=None):
    ...
    for sit in situations:
        ...
        for sim, situation_sim in tuple(sit._situation_sims.items()):
            if sim:
                sim_data.append({'sim_Id':str(hex(sim.sim_id)), 
                 'sim_name':sim.full_name, 
                 'sim_job':situation_sim.current_job_type.__name__ if situation_sim.current_job_type is not None else 'None', 
                 'sim_role':situation_sim.current_role_state_type.__name__ if situation_sim.current_role_state_type is not None else 'None', 
                 'sim_emotion':situation_sim.emotional_buff_name, 
                 'sim_on_active_lot':sim.is_on_active_lot()})

        goal_data = []
        goals = sit.get_situation_goal_info()
        if goals is not None:
            for goal, tuned_goal_set in goals:
                goal_data.append({'goal':goal.get_gsi_name(), 
                 'goal_set':tuned_goal_set.__name__ if tuned_goal_set is not None else 'None', 
                 'time_created':str(goal.created_time), 
                 'time_completed':str(goal.completed_time), 
                 'score':goal.score})
        ...
        for job_data in sit.gsi_all_jobs_data_gen():
            if job_data.gsi_has_churn():
                churn_data.append({'sim_job':job_data.gsi_get_job_name(), 
                 'min':job_data.gsi_get_churn_min(), 
                 'max':job_data.gsi_get_churn_max(), 
                 'here':job_data.gsi_get_num_churn_sims_here(), 
                 'coming':job_data.gsi_get_num_churn_sims_coming(), 
                 'time_left':str(job_data.gsi_get_remaining_time_until_churn())})

        shift_data = []
        for job_data in sit.gsi_all_jobs_data_gen():
            if job_data.gsi_has_shifts():
                shift_data.append({'sim_job':job_data.gsi_get_job_name(), 
                 'num':job_data.gsi_get_shifts_staffing(), 
                 'here':job_data.gsi_get_num_churn_sims_here(), 
                 'coming':job_data.gsi_get_num_churn_sims_coming(), 
                 'change_time_left':str(job_data.gsi_get_remaining_time_until_shift_change()), 
                 'churn_time_left':str(job_data.gsi_get_remaining_time_until_churn())})

        tag_data = [{'tag': tag.name} for tag in sit.tags]
        additional_data = sit.gsi_additional_data('field', 'data')
        all_situations.append({'situation_id':str(sit.id), 
         'situation':str(sit), 
         'time_left':str(sit._get_remaining_time_for_gsi()) if sit._get_remaining_time_for_gsi() is not None else 'Forever', 
         'state':sit.get_phase_state_name_for_gsi(), 
         'sim_count':len(sit._situation_sims), 
         'score':sit.score, 
         'level':str(sit.get_level()), 
         'exclusivity':sit.exclusivity.name, 
         'creation_source':sit.creation_source, 
         'Sims':sim_data, 
         'Goals':goal_data, 
         'Churn':churn_data, 
         'Shifts':shift_data, 
...
def _get_bouncer_request_gsi_data(request):
    return {'bouncer_id':str(request._creation_id), 
     'situation':str(request._situation), 
     'situation_id':str(request._situation.id), 
     'job':request._job_type.__name__, 
     'filter':request._sim_filter.__name__ if request._sim_filter is not None else 'None', 
     'status':request._status.name, 
     'klout':request._get_request_klout() if request._get_request_klout() is not None else 10000, 
     'priority':request._request_priority.name, 
     'sim_name':request.assigned_sim.full_name if request.assigned_sim is not None else 'None', 
     'spawning_option':request.spawning_option.name, 
     'additional_filter_terms':str(request.get_additional_filter_terms()), 
     'unique':str(id(request))}
...
sim_situation_schema.add_field('is_household_sim', label='Is Household Sim')
sim_situation_schema.add_field('route_on_lot', label='Route On Active Lot')
sim_situation_schema.add_field('portal_disallowance_tags', label='Portal Disallowance Tags')
...
with sim_situation_schema.add_has_many('Current Situations', GsiGridSchema) as (sub_schema):
    ...
    sub_schema.add_field('route_on_lot', label='Route On Active Lot')
    sub_schema.add_field('portal_disallowance_tags', label='Portal Disallowance Tags')
...
def generate_sim_situation_view(zone_id: int=None):
    ...
    for sim_info in tuple(services.sim_info_manager().values()):
        display_sim = False
        on_lot_time_string = ''
        on_lot_route_any = False
        is_household_sim_string = ''
        portal_disallowance_tags_set = set()
        blacklist_data = []
        situation_data = []
        ...
        if sim is not None:
            ...
            if on_lot_time is not None:
                display_sim = True
                on_lot_time_string = str(on_lot_time)
            is_household_sim_string = str(sim.household_id == active_household_id)
            for situation in situation_manager.get_situations_sim_is_in(sim):
                role_state = situation.get_current_role_state_for_sim(sim)
                display_sim = True
                on_lot_route = role_state.allow_npc_routing_on_active_lot
                on_lot_route_any = on_lot_route_any or on_lot_route
                portal_disallowance_tags_set = portal_disallowance_tags_set.union(role_state.portal_disallowance_tags)
                situation_data.append({'situation':str(situation), 
                 'job':str(situation.get_current_job_for_sim(sim)), 
                 'role':str(role_state), 
                 'route_on_lot':str(on_lot_route), 
                 'portal_disallowance_tags':', '.join((tag.name for tag in role_state.portal_disallowance_tags))})
        ...
        if blacklist_info is not None:
            for tag, blacklist_time in blacklist_info:
                display_sim = True
                blacklist_data.append({'tag':str(tag), 
                 'blacklist_time':str(blacklist_time)})

        if display_sim:
            sim_data.append({'sim_id':str(hex(sim_info.id)), 
             'sim':sim_info.full_name, 
             'time_on_lot':on_lot_time_string, 
             'creation_source':format_enum_name(sim_info.creation_source), 
             'route_on_lot':str(on_lot_route_any), 
             'is_household_sim':is_household_sim_string, 
             'portal_disallowance_tags':', '.join((tag.name for tag in portal_disallowance_tags_set)), 
             'Current Situations':situation_data, 
...
class SituationDataArchiver(GameplayArchiver):
    ...
    def archive_event(self, situation, event, sub_event=False, final_event=False):
        ...
        self._situation_data[situation.id].append({'game_time':game_time, 
         'event':event, 
         'source':str(situation.creation_source)})
        if not sub_event:
            data = {'situation_id':situation.id,  'situation':str(situation), 
             'event':event, 
             'Situation Events':self._situation_data[situation.id]}
            self.archive(data)
        if final_event:
