...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\gsi_handlers\relationship_culling_handlers.py
# Compiled at: 2019-09-20 16:27:49
...
with relationship_culling_archive_schema.add_has_many('culled_relationships', GsiGridSchema) as (sub_schema):
    ...
    sub_schema.add_field('total_depth', label='Total Depth', type=(GsiFieldVisualizers.INT), width=0.2)
    sub_schema.add_field('rel_bits', label='Relationship Bits')
archiver = GameplayArchiver('relationship_culling', relationship_culling_archive_schema,
...
RelationshipGSIData = namedtuple('RelationshipGSIData', ('sim_info', 'target_sim_info',
                                                         'total_depth', 'formated_rel_bits',
                                                         'culled_status', 'culled_reason'))

def _add_rel_data(rel_data: RelationshipGSIData, relationships_data):
    rel_entry = {'sim_info':str(rel_data.sim_info), 
     'target_sim_info':str(rel_data.target_sim_info), 
     'total_depth':rel_data.total_depth, 
     'rel_bits':rel_data.formated_rel_bits, 
     'culled_status':rel_data.culled_status, 
     'culled_reason':rel_data.culled_reason}
...
def archive_relationship_culling(total_culled_count, relationship_data, culled_relationship_data):
    entry = {'relationships_culled':total_culled_count, 
     'game_time':str(services.time_service().sim_now)}
