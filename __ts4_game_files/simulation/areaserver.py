...
# Embedded file name: T:\InGame\Gameplay\Scripts\Server\areaserver.py
# Compiled at: 2022-07-21 21:49:30
# Size of source mod 2**32: 22133 bytes
import time
...
import clock, game_services, indexed_manager, native.animation, paths, server.account, services, sims.sim_spawner, sims4.core_services, sims4.geometry, sims4.gsi.http_service, sims4.log, sims4.perf_log, sims4.zone_utils, telemetry_helper
        ...
        if zone is not None:
            if zone.is_instantiated:
                ...
                if persistence_service is not None:
                    if persistence_service.save_timeline:
...
def c_api_client_connect(session_id, account_id, household_id, persona_name, zone_id, callback, active_sim_id, locale='none', edit_lot_mode=False):
    ...
    if account is None:
        ...
        if paths.MEM_REPORT_ZONE_SNAPSHOT:
            from sims4.commands import client_cheat
            client_cheat('|mem.report native', session_id)
        if paths.TREE_DUMP_TUNING_SNAPSHOT_AFTER_ZONE:
        ...
        return result or EXCEPTION_ERROR_CODE
    return SUCCESS_CODE
...
def c_api_get_household_funds(zone_id, household_id):
    ...
    if business_manager is not None:
        if business_manager.is_household_owner(household_id):
