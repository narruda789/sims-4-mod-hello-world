...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\sims4\localization\localization_tunables.py
# Compiled at: 2017-08-01 15:18:50
# Size of source mod 2**32: 6289 bytes
...
class LocalizedStringHouseholdNameSelector(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def _get_string_for_humans(self, sim, household, *args, **kwargs):
        ...
        if self.empty_household is not None:
            return household is None or humans or (self.empty_household)(sim, *args, **kwargs)
        ...
        if self.single_sim is not None:
            if len(humans) == 1:
                return (self.single_sim)(humans[0], *(args + (sim,)), **kwargs)
        if self.single_family is not None:
            if all((sim_info.last_name == sim.last_name for sim_info in humans)):
                if sim.last_name == household.name:
