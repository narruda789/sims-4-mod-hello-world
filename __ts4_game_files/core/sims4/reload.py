...
def _is_supported_as_literal_value(newobj):
    if type(newobj).__module__ in SUPPORTED_BUILTIN_MODULES:
        if isinstance(newobj, SUPPORTED_BUILTIN_TYPES):
            return True
...
def _update_flexmethod(oldfm, newfm):
    oldfm.__wrapped_method__ = _update(oldfm.__wrapped_method__, newfm.__wrapped_method__)
    return oldfm