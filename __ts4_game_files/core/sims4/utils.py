...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\sims4\utils.py
# Compiled at: 2021-01-07 12:41:48
# Size of source mod 2**32: 21663 bytes
import builtins, functools, inspect, os.path, paths, random, sys, time, weakref, sims4.log, sims4.repr_utils
...
def filename_to_module_fqn(filename):
    prefix_list = sorted([os.path.commonprefix([os.path.abspath(m), filename]) for m in sys.path if filename.startswith(os.path.abspath(m))],
      key=len,
      reverse=True)
    if not prefix_list:
...
def get_nested_class_list(cls):
    return [
     cls.__module__] + cls.__qualname__.split('.')
...
def create_text_file(filename: str, file_extension: str='txt', callback=None, connection=None):
    if filename is None or callback is None:
        ...
        file_path = '{}_{}.{}'.format(filename, current_time, file_extension)
        file_path = os.path.isabs(file_path) or os.path.join(paths.DUMP_ROOT, file_path)
...
def create_csv(filename: str, callback=None, connection=None):
    create_text_file(filename, 'csv', callback, connection)
