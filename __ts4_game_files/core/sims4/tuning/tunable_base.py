...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\sims4\tuning\tunable_base.py
# Compiled at: 2020-07-01 15:53:14
# Size of source mod 2**32: 24127 bytes
import inspect, sys, sims4.log, sims4.reload, sims4.resources
...
tunable_type_mapping = {int: int, 
 float: float, 
 str: str, 
 bool: BoolWrapper, 
 BoolWrapper: BoolWrapper, 
 sims4.resources.Key: sims4.resources.ResourceKeyWrapper, 
 sims4.resources.ResourceKeyWrapper: sims4.resources.ResourceKeyWrapper}
...
BASIC_DESC_KEY = (
 ...
 Attributes.Filter,
 Attributes.Group,
 Attributes.ValidationCategory,
 Attributes.TuningState,
 Attributes.ExportModes)
...
class TdescFragMetaClass(type):
    ...
    def __new__(cls, name, *args, is_fragment=False, **kwargs):
        ...
        if is_fragment:
            if not DISABLE_FRAG_DUP_NAME_CHECK:
                if name in TDESC_FRAG_DICT_GLOBAL:
                    assert sims4.reload.currently_reloading, 'Frag Class with name {} already exists'.format(name)
...
RESERVED_KWARGS = set(['description', 'category', 'callback', 'verify_tunable_callback',
 'export_modes', 'display_name', '_display_name', 'deferred', 'needs_tuning',
 'tuning_group', 'tuning_filter', 'default', 'is_fragment', 'cache_key',
 'deprecated', 'locked_args', 'subclass_args', 'items', 'keys', 'values'])

class TunableBase(metaclass=TdescFragMetaClass):
    __slots__ = ('callback', 'deferred', 'needs_deferring', 'is_fragment', '_cache_key',
                 '_has_callback')
    ...
    def export_desc(self):
        ...
        export_dict = {Attributes.Name: self.name, 
         Attributes.DisplayName: self.display_name, 
         Attributes.Description: description, 
         Attributes.Class: self.export_class, 
         Attributes.Filter: self.tuning_filter, 
         Attributes.Group: self.group, 
         Attributes.Deprecated: self._deprecated}
        if self._category:
