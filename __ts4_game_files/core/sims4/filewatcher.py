...
FILTER_FLAG_NAMES = {flag:name for name, flag in vars(_filemonitor).items() if name.startswith('FILTER_') if name.startswith('FILTER_')}
ACTION_NAMES = {value:name for name, value in vars(_filemonitor).items() if name.startswith('ACTION_') if name.startswith('ACTION_')}
