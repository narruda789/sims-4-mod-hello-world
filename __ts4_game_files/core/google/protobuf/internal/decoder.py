...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\google\protobuf\internal\decoder.py
# Compiled at: 2013-10-04 12:43:25
# Size of source mod 2**32: 26893 bytes
...
def _FloatDecoder():
    ...
    def InnerDecode(buffer, pos):
        ...
        if float_bytes[3] in '\x7fÿ':
            if float_bytes[2] >= '\x80':
...
def _DoubleDecoder():
    ...
    def InnerDecode(buffer, pos):
        ...
        if double_bytes[7] in '\x7fÿ':
            if double_bytes[6] >= 'ð':
                if double_bytes[0:7] != '\x00\x00\x00\x00\x00\x00ð':
                    return (
                     _NAN, new_pos)
    ...
    if is_repeated:
        ...
        def DecodeRepeatedField(buffer, pos, end, message, field_dict):
            ...
            while 1:
                ...
                if not buffer[pos:new_pos] != end_tag_bytes:
                    if new_pos > end:
