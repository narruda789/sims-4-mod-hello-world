...
# Embedded file name: T:\InGame\Gameplay\Scripts\Core\google\protobuf\internal\python_message.py
# Compiled at: 2013-10-04 12:43:25
# Size of source mod 2**32: 43778 bytes
...
import google.protobuf.internal as message_listener_mod
from google.protobuf.internal import type_checkers
from google.protobuf.internal import wire_format
import google.protobuf as descriptor_mod
import google.protobuf as message_mod
...
def _VerifyExtensionHandle(message, extension_handle):
    if not isinstance(extension_handle, _FieldDescriptor):
        ...
        assert extension_handle.containing_type, '"%s" is missing a containing_type.' % extension_handle.full_name
    if extension_handle.containing_type is not message.DESCRIPTOR:
        raise KeyError('Extension "%s" extends message type "%s", but this message is of type "%s".' % (
...
def _IsMessageSetExtension(field):
    return field.is_extension and field.containing_type.has_options and field.containing_type.GetOptions().message_set_wire_format and field.type == _FieldDescriptor.TYPE_MESSAGE and field.message_type == field.extension_scope and field.label == _FieldDescriptor.LABEL_OPTIONAL
...
def _AttachFieldHelpers(cls, field_descriptor):
    ...
    def AddDecoder(wiretype, is_packed):
        ...
        cls._decoders_by_tag[tag_bytes] = type_checkers.TYPE_TO_DECODER[field_descriptor.type](field_descriptor.number, is_repeated, is_packed, field_descriptor, field_descriptor._default_constructor)

    AddDecoder(type_checkers.FIELD_TYPE_TO_WIRE_TYPE[field_descriptor.type], False)
    if is_repeated:
        if wire_format.IsTypePackable(field_descriptor.type):
...
def _AddStaticMethods(cls):
    ...
    def RegisterExtension(extension_handle):
        ...
        if actual_handle is not extension_handle:
            raise AssertionError('Extensions "%s" and "%s" both try to extend message type "%s" with field number %d.' % (
...
def _IsPresent(item):
    if item[0].label == _FieldDescriptor.LABEL_REPEATED:
        return bool(item[1])
    if item[0].cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE:
...
def _AddEqualsMethod(message_descriptor, cls):
    ...
    def __eq__(self, other):
        if not isinstance(other, message_mod.Message) or other.DESCRIPTOR != self.DESCRIPTOR:
            ...
            if self is other:
                return True
            return self.ListFields() == other.ListFields() or False
...
def _AddSerializeToStringMethod(message_descriptor, cls):
    ...
    def SerializeToString(self):
        ...
        if not self.IsInitialized():
            raise message_mod.EncodeError('Message is missing required fields: ' + ','.join(self.FindInitializationErrors()))
    ...
    def IsInitialized(self, errors=None):
        for field in required_fields:
            if not field not in self._fields:
                if field.cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE:
                    if self._fields[field]._is_present_in_parent or errors is not None:
        ...
        for field, value in self._fields.items():
            if field.cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE:
                if field.label == _FieldDescriptor.LABEL_REPEATED:
                    ...
                    if value.IsInitialized() or errors is not None:
                        errors.extend(self.FindInitializationErrors())
                    return False
...
class _ExtensionDict(object):
    ...
    def __getitem__(self, extension_handle):
        ...
        if result is not None:
            return result
        elif extension_handle.label == _FieldDescriptor.LABEL_REPEATED:
