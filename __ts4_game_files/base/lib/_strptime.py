...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\_strptime.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 26092 bytes
...
def _strptime(data_string, format='%a %b %d %H:%M:%S %Y'):
    ...
    if not found:
        raise ValueError('time data %r does not match format %r' % (
         data_string, format))
    elif len(data_string) != found.end():
        ...
        if year is None and iso_year is not None and not iso_week is None:
            if weekday is None:
