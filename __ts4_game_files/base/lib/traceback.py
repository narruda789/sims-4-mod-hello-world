...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\traceback.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 23695 bytes
...
def format_exception(etype, value, tb, limit=None, chain=True):
    return list(TracebackException((type(value)),
      value, tb, limit=limit).format(chain=chain))
...
class FrameSummary:
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, tuple):
            return (
             self.filename, self.lineno, self.name, self.line) == other
        return NotImplemented

    def __getitem__(self, pos):
        return (
         self.filename, self.lineno, self.name, self.line)[pos]

    def __iter__(self):
        return iter([self.filename, self.lineno, self.name, self.line])

    def __repr__(self):
        return '<FrameSummary file {filename}, line {lineno} in {name}>'.format(filename=(self.filename),
          lineno=(self.lineno),
          name=(self.name))
    ...
    def extract(klass, frame_gen, *, limit=None, lookup_lines=True, capture_locals=False):
        ...
        for f, lineno in frame_gen:
            ...
            if capture_locals:
                ...
                f_locals = None
            result.append(FrameSummary(filename,
              lineno, name, lookup_line=False, locals=f_locals))
    ...
    def __init__(self, exc_type, exc_value, exc_traceback, *, limit=None, lookup_lines=True, capture_locals=False, _seen=None):
        if _seen is None:
            ...
            if exc_value and exc_value.__cause__ is not None and id(exc_value.__cause__) not in _seen:
                cause = TracebackException((type(exc_value.__cause__)),
                  (exc_value.__cause__),
            ...
            if exc_value and exc_value.__context__ is not None and id(exc_value.__context__) not in _seen:
                context = TracebackException((type(exc_value.__context__)),
        ...
        if chain:
            if self.__cause__ is not None:
                ...
                if self.__context__ is not None:
                    if not self.__suppress_context__:
