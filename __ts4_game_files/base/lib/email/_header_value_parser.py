...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\email\_header_value_parser.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 102416 bytes
import re, urllib
...
def get_local_part(value):
    ...
    if not value:
        ...
        if not value or value[0] == '\\' or value[0] not in PHRASE_ENDS:
...
def get_obs_local_part(value):
    ...
    while not value or value[0] == '\\' or value[0] not in PHRASE_ENDS:
        if value[0] == '.':
            ...
            if value[0] == '\\':
                ...
                last_non_ws_was_dot = False
                continue
            elif obs_local_part and obs_local_part[(-1)].token_type != 'dot':
    ...
    if (obs_local_part[0].token_type == 'dot' or obs_local_part[0].token_type) == 'cfws':
        if obs_local_part[1].token_type == 'dot':
            obs_local_part.defects.append(errors.InvalidHeaderDefect("Invalid leading '.' in local part"))
    if (obs_local_part[(-1)].token_type == 'dot' or obs_local_part[(-1)].token_type) == 'cfws':
        if obs_local_part[(-2)].token_type == 'dot':
