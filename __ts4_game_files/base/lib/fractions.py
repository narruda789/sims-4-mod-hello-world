...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\fractions.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 24282 bytes
...
class Fraction(numbers.Rational):
    ...
    def __eq__(a, b):
        if type(b) is int:
            ...
            if isinstance(b, numbers.Complex):
                if b.imag == 0:
                    b = b.real
            if isinstance(b, float) and not math.isnan(b):
                if math.isinf(b):
                    return 0.0 == b
    ...
    def _richcmp(self, other, op):
        if isinstance(other, numbers.Rational):
            ...
            if isinstance(other, float) and not math.isnan(other):
                if math.isinf(other):
                    return op(0.0, other)
