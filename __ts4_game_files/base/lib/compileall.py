...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\compileall.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 13613 bytes
...
def _walk_dir(dir, ddir=None, maxlevels=10, quiet=0):
    ...
    for name in names:
        ...
        if maxlevels > 0 and name != os.curdir and name != os.pardir and os.path.isdir(fullname):
            yield from os.path.islink(fullname) or _walk_dir(fullname, ddir=dfile, maxlevels=(maxlevels - 1),
              quiet=quiet)
...
def compile_dir(dir, maxlevels=10, ddir=None, force=False, rx=None, quiet=0, legacy=False, optimize=-1, workers=1, invalidation_mode=py_compile.PycInvalidationMode.TIMESTAMP):
    ...
    if workers is not None and workers != 1 and ProcessPoolExecutor is not None:
        ...
        with ProcessPoolExecutor(max_workers=workers) as (executor):
            results = executor.map(partial(compile_file, ddir=ddir,
              force=force,
              rx=rx,
              quiet=quiet,
              legacy=legacy,
              optimize=optimize,
              invalidation_mode=invalidation_mode), files)
...
def main():
    ...
    parser.add_argument('-l', action='store_const', const=0, default=10,
      dest='maxlevels',
    ...
    parser.add_argument('--invalidation-mode', default='timestamp', choices=(sorted(invalidation_modes)),
    ...
    elif args.recursion is not None:
        ...
        if compile_dests:
            for dest in compile_dests:
                if os.path.isfile(dest):
                    if not compile_file(dest, (args.ddir), (args.force), (args.rx), (args.quiet),
                      (args.legacy), invalidation_mode=invalidation_mode):
                        success = False
                else:
                    success = compile_dir(dest, maxlevels, (args.ddir), (args.force),
                      (args.rx), (args.quiet), (args.legacy),
                      workers=(args.workers), invalidation_mode=invalidation_mode) or False
