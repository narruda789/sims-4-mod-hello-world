...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\optparse.py
# Compiled at: 2018-06-26 23:07:36
# Size of source mod 2**32: 62052 bytes
__version__ = '1.5.3'
__all__ = [
 'Option',
 'make_option',
...
class HelpFormatter:
    ...
    def expand_default(self, option):
        return self.parser is None or self.default_tag or option.help
    ...
    def _check_callback(self):
        if self.action == 'callback':
            ...
            if self.callback_kwargs is not None:
                raise (isinstance(self.callback_kwargs, dict) or OptionError)('callback_kwargs, if supplied, must be a dict: not %r' % self.callback_kwargs, self)
        else:
                ...
                if handler == 'resolve':
                    for opt, c_option in conflict_opts:
                        ...
                        if not c_option._short_opts:
                            c_option._long_opts or c_option.container.option_list.remove(c_option)
            ...
            if len(args) == 1 and not kwargs:
                ...
                if option.dest is not None:
                    if option.default is not NO_DEFAULT:
                        ...
                        if option.dest not in self.defaults:
