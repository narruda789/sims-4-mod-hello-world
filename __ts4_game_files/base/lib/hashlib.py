...
# Embedded file name: T:\InGame\Gameplay\Scripts\Lib\hashlib.py
                ...
                if name in ('SHA256', 'sha256', 'SHA224', 'sha224'):
                    ...
                    if name in ('SHA512', 'sha512', 'SHA384', 'sha384'):
                        ...
                        if name in ('blake2b', 'blake2s'):
                            ...
                            if name in frozenset({'sha3_256', 'sha3_224', 'sha3_384', 'shake_128', 'shake_256', 'sha3_512'}):
                                import _sha3
...
try:
    ...
    def pbkdf2_hmac(hash_name, password, salt, iterations, dklen=None):
        if not isinstance(hash_name, str):
            ...
            salt = isinstance(salt, (bytes, bytearray)) or bytes(memoryview(salt))
        inner = new(hash_name)
        outer = new(hash_name)
